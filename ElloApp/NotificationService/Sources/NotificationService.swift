...
private func getCurrentRenderedTotalUnreadCount(accountManager: AccountManager<ElloAppAccountManagerTypes>, postbox: Postbox) -> Signal<(Int32, RenderedTotalUnreadCountType), NoError> {
...
private final class NotificationServiceHandler {
    ...
    init?(queue: Queue, updateCurrentContent: @escaping (NotificationContent) -> Void, completed: @escaping () -> Void, payload: [AnyHashable: Any]) {
        ...
        self.accountManager = AccountManager<ElloAppAccountManagerTypes>(basePath: rootPath + "/accounts-metadata", isTemporary: true, isReadOnly: false, useCaches: false, removeDatabaseOnError: false)
        ...
        |> deliverOn(self.queue)).start(next: { [weak self] records, sharedData in
            ...
            |> deliverOn(strongSelf.queue)).start(next: { stateManager in
                ...
                strongSelf.notificationKeyDisposable.set((combineLatest(queue: strongSelf.queue,
                    ...
                    var downloadNotificationSound: (file: ElloAppMediaFile, path: String, fileName: String)?
                        ...
                        case let .call(callData):
                            if let stateManager = strongSelf.stateManager {
                                ...
                                let _ = (stateManager.postbox.transaction { transaction -> String? in
                                    if let peer = transaction.getPeer(callData.fromId) as? ElloAppUser {
                                        return peer.phone
                                    } else {
                                        return nil
                                    }
                                ...
                                let pollCompletion: (NotificationContent) -> Void = { content in
                                    ...
                                    queue.async {
                                        ...
                                        if let mediaAttachment = mediaAttachment {
                                            var fetchResource: ElloAppMultipartFetchableResource?
                                            if let image = mediaAttachment as? ElloAppMediaImage, let representation = largestImageRepresentation(image.representations), let resource = representation.resource as? ElloAppMultipartFetchableResource {
                                                fetchResource = resource
                                            } else if let file = mediaAttachment as? ElloAppMediaFile {
                                                if file.isSticker {
                                                    fetchResource = file.resource as? ElloAppMultipartFetchableResource
                                                } else if file.isVideo {
                                                    fetchResource = file.previewRepresentations.first?.resource as? ElloAppMultipartFetchableResource
                                                }
                                            }
                                        ...
                                        if let (downloadNotificationSound, _, _) = downloadNotificationSound {
                                            var fetchResource: ElloAppMultipartFetchableResource?
                                            fetchResource = downloadNotificationSound.resource as? ElloAppMultipartFetchableResource
                                        ...
                                        |> deliverOn(queue)).start(next: { mediaData, notificationSoundData in
                                            ...
                                            |> deliverOn(strongSelf.queue)).start(next: { value in
                                                ...
                                                if let image = mediaAttachment as? ElloAppMediaImage, let resource = largestImageRepresentation(image.representations)?.resource {
