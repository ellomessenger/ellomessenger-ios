...
class BazelCommandLine:
    ...
    def invoke_clean(self):
        ...
        print('ElloAppBuild: running {}'.format(combined_arguments))
        call_executable(combined_arguments)

    def get_define_arguments(self):
        return [
            '--define=buildNumber={}'.format(self.build_number),
            '--define=elloappVersion={}'.format(self.build_environment.app_version)
        ]
    ...
    def invoke_build(self):
        ...
        if self.custom_target is not None:
            ...
            combined_arguments += ['ElloApp/ElloApp']
        ...
        if self.disable_provisioning_profiles:
            combined_arguments += ['--//ElloApp:disableProvisioningProfiles']
...
def resolve_codesigning(arguments, base_path, build_configuration, provisioning_profiles_path, additional_codesigning_output_path) -> ResolvedCodesigningData:
    ...
    if arguments.gitCodesigningRepository is not None:
        password = os.getenv('ELLOAPP_CODESIGNING_GIT_PASSWORD')
        if password is None:
            print('ELLOAPP_CODESIGNING_GIT_PASSWORD environment variable is not set')
            sys.exit(1)
        ...
        private_key = os.getenv('ELLOAPP_CODESIGNING_PRIVATE_KEY')
...
def build(bazel, arguments):
    ...
    if arguments.outputBuildArtifactsPath is not None:
        ...
        if os.path.exists(artifacts_path + '/ElloApp.ipa'):
            os.remove(artifacts_path + '/ElloApp.ipa')
        ...
        ipa_paths = glob.glob('bazel-out/applebin_ios-ios_arm*-opt-ST-*/bin/ElloApp/ElloApp.ipa')
        if len(ipa_paths) == 0:
            print('Could not find the IPA at bazel-out/applebin_ios-ios_arm*-opt-ST-*/bin/ElloApp/ElloApp.ipa')
            sys.exit(1)
        elif len(ipa_paths) > 1:
            print('Multiple matching IPA files found: {}'.format(ipa_paths))
            sys.exit(1)
        shutil.copyfile(ipa_paths[0], artifacts_path + '/ElloApp.ipa')

        dsym_paths = glob.glob('bazel-out/applebin_ios-ios_arm*-opt-ST-*/bin/ElloApp/*.dSYM')
...
def add_codesigning_common_arguments(current_parser: argparse.ArgumentParser):
    ...
    codesigning_group.add_argument(
        ...
        help='''
            ...
            ELLOAPP_CODESIGNING_GIT_PASSWORD environment variable must be set.
            ''',
        metavar='path'
    )
    codesigning_group.add_argument(
