...
def remote_build(darwin_containers_host, bazel_cache_host, configuration, build_input_data_path):
    ...
    os.system('find . -type f -a -not -regex "\\." -a -not -regex ".*\\./git" -a -not -regex ".*\\./git/.*" -a -not -regex "\\./bazel-bin" -a -not -regex "\\./bazel-bin/.*" -a -not -regex "\\./bazel-out" -a -not -regex "\\./bazel-out/.*" -a -not -regex "\\./bazel-testlogs" -a -not -regex "\\./bazel-testlogs/.*" -a -not -regex "\\./bazel-elloapp-ios" -a -not -regex "\\./bazel-elloapp-ios/.*" -a -not -regex "\\./buildbox" -a -not -regex "\\./buildbox/.*" -a -not -regex "\\./buck-out" -a -not -regex "\\./buck-out/.*" -a -not -regex "\\./\\.buckd" -a -not -regex "\\./\\.buckd/.*" -a -not -regex "\\./build" -a -not -regex "\\./build/.*" -print0 | tar cf "{buildbox_dir}/transient-data/source.tar" --null -T -'.format(buildbox_dir=buildbox_dir))
    ...
    with darwinContainers.workingImageSession(name=image_name) as session:
        ...
        session_scp_upload(session=session, source_path=build_input_data_path, destination_path='elloapp-build-input')
        ...
        guest_build_sh = '''
            ...
            mkdir /Users/Shared/elloapp-ios
            cd /Users/Shared/elloapp-ios

            tar -xf $HOME/source.tar

            python3 build-system/Make/ImportCertificates.py --path $HOME/elloapp-build-input/certs
        ...
        guest_build_sh += '--configurationPath=$HOME/elloapp-build-input/configuration.json \\'
        guest_build_sh += '--codesigningInformationPath=$HOME/elloapp-build-input \\'
        guest_build_sh += '--outputBuildArtifactsPath=/Users/Shared/elloapp-ios/build/artifacts \\'

        guest_build_file_path = tempfile.mktemp()
        with open(guest_build_file_path, 'w+') as file:
            file.write(guest_build_sh)
        session_scp_upload(session=session, source_path=guest_build_file_path, destination_path='guest-build-elloapp.sh')
        os.unlink(guest_build_file_path)

        print('Executing remote build...')

        session_ssh(session=session, command='bash -l guest-build-elloapp.sh')
        ...
        session_scp_download(session=session, source_path='/Users/Shared/elloapp-ios/build/artifacts/*', destination_path='{artifacts_path}/'.format(artifacts_path=artifacts_path))

        if os.path.exists(artifacts_path + '/ElloApp.ipa'):
            print('Artifacts have been stored at {}'.format(artifacts_path))
        else:
            print('ElloApp.ipa not found')
            sys.exit(1)
...
def remote_deploy_testflight(darwin_containers_host, ipa_path, dsyms_path, username, password):
    ...
    with darwinContainers.workingImageSession(name=image_name) as session:
        ...
        guest_upload_sh = '''
            ...
            FASTLANE_PASSWORD="{password}" xcrun altool --upload-app --type ios --file "ElloApp.ipa" --username "{username}" --password "@env:FASTLANE_PASSWORD"
        '''.format(username=username, password=password)

        guest_upload_file_path = tempfile.mktemp()
        with open(guest_upload_file_path, 'w+') as file:
            file.write(guest_upload_sh)
        session_scp_upload(session=session, source_path=guest_upload_file_path, destination_path='guest-upload-elloapp.sh')
        os.unlink(guest_upload_file_path)

        print('Executing remote upload...')
        session_ssh(session=session, command='bash -l guest-upload-elloapp.sh')
