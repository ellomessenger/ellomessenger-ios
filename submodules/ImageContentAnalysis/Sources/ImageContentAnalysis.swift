...
private func cachedImageRecognizedContent(engine: ElloAppEngine, messageId: MessageId) -> Signal<CachedImageRecognizedContent?, NoError> {
    ...
    return engine.data.get(ElloAppEngine.EngineData.Item.ItemCache.Item(collectionId: ApplicationSpecificItemCacheCollectionId.cachedImageRecognizedContent, id: key))
    |> map { entry -> CachedImageRecognizedContent? in
        return entry?.get(CachedImageRecognizedContent.self)
    }
}

private func updateCachedImageRecognizedContent(engine: ElloAppEngine, messageId: MessageId, content: CachedImageRecognizedContent?) -> Signal<Never, NoError> {
    let key = ValueBoxKey(length: 20)
    key.setInt32(0, value: messageId.namespace)
    key.setInt32(4, value: messageId.peerId.namespace._internalGetInt32Value())
    key.setInt64(8, value: messageId.peerId.id._internalGetInt64Value())
...
public func recognizedContent(engine: ElloAppEngine, image: @escaping () -> UIImage?, messageId: MessageId) -> Signal<[RecognizedContent], NoError> {
