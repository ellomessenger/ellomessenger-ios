...
final class InstantPageAudioNode: ASDisplayNode, InstantPageNode {
    ...
    init(context: AccountContext, strings: PresentationStrings, theme: InstantPageTheme, webPage: ElloAppMediaWebpage, media: InstantPageMedia, openMedia: @escaping (InstantPageMedia) -> Void) {
        ...
        if let file = self.media.media as? ElloAppMediaFile {
        ...
        /*if let applicationContext = account.applicationContext as? ElloAppApplicationContext, let (playlistId, itemId) = instantPageAudioPlaylistAndItemIds(webpage: webpage, media: self.media) {
