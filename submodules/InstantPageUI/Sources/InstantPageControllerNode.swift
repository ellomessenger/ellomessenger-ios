...
final class InstantPageControllerNode: ASDisplayNode, UIScrollViewDelegate {
    ...
    private var webPage: ElloAppMediaWebpage?
    ...
    func updateWebPage(_ webPage: ElloAppMediaWebpage?, anchor: String?, state: InstantPageStoredState? = nil) {
    ...
    func updateVisibleItems(visibleBounds: CGRect, animated: Bool = false) {
        ...
        for item in self.currentLayoutItemsWithNodes {
            ...
            if let imageItem = item as? InstantPageImageItem, imageItem.media.media is ElloAppMediaWebpage {
                embedIndex += 1
            }
    ...
    private func longPressMedia(_ media: InstantPageMedia) {
        let controller = ContextMenuController(actions: [ContextMenuAction(content: .text(title: self.strings.Conversation_ContextMenuCopy, accessibilityLabel: self.strings.Conversation_ContextMenuCopy), action: { [weak self] in
            if let strongSelf = self, let image = media.media as? ElloAppMediaImage {
                let media = ElloAppMediaImage(imageId: MediaId(namespace: 0, id: 0), representations: image.representations, immediateThumbnailData: image.immediateThumbnailData, reference: nil, partialReference: nil, flags: [])
            ...
            if let strongSelf = self, let webPage = strongSelf.webPage, let image = media.media as? ElloAppMediaImage {
    ...
    private func openMedia(_ media: InstantPageMedia) {
        ...
        if let file = media.media as? ElloAppMediaFile, (file.isVoice || file.isMusic) {
            var medias: [InstantPageMedia] = []
            var initialIndex = 0
            for item in items {
                for itemMedia in item.medias {
                    if let itemFile = itemMedia.media as? ElloAppMediaFile, (itemFile.isVoice || itemFile.isMusic) {
        ...
        if media.media is ElloAppMediaWebpage {
            ...
            medias = medias.filter {
                return $0.media is ElloAppMediaImage || $0.media is ElloAppMediaFile
            }
