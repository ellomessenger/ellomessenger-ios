...
private struct InstantImageGalleryThumbnailItem: GalleryThumbnailItem {
    ...
    func image(synchronous: Bool) -> (Signal<(TransformImageArguments) -> DrawingContext?, NoError>, CGSize) {
        if let imageReferene = mediaReference.concrete(ElloAppMediaImage.self), let representation = largestImageRepresentation(imageReferene.media.representations) {
            return (mediaGridMessagePhoto(account: self.account, photoReference: imageReferene), representation.dimensions.cgSize)
        } else if let fileReference = mediaReference.concrete(ElloAppMediaFile.self), let dimensions = fileReference.media.dimensions {
...
final class InstantImageGalleryItemNode: ZoomableContentGalleryItemNode {
    ...
    override func visibilityUpdated(isVisible: Bool) {
        ...
        if let (context, media) = self.contextAndMedia, let fileReference = media.concrete(ElloAppMediaFile.self) {
            if isVisible {
