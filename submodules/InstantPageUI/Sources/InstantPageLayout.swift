...
func layoutInstantPageBlock(webpage: ElloAppMediaWebpage, rtl: Bool, block: InstantPageBlock, boundingWidth: CGFloat, horizontalInset: CGFloat, safeInset: CGFloat, isCover: Bool, previousItems: [InstantPageItem], fillToSize: CGSize?, media: [MediaId: Media], mediaIndexCounter: inout Int, embedIndexCounter: inout Int, detailsIndexCounter: inout Int, theme: InstantPageTheme, strings: PresentationStrings, dateTimeFormat: PresentationDateTimeFormat, webEmbedHeights: [Int : CGFloat] = [:], excludeCaptions: Bool) -> InstantPageLayout {
    ...
    switch block {
        ...
        case let .image(id, caption, url, webpageId):
            if let image = media[id] as? ElloAppMediaImage, let largest = largestImageRepresentation(image.representations) {
        ...
        case let .video(id, caption, autoplay, _):
            if let file = media[id] as? ElloAppMediaFile, let dimensions = file.dimensions {
        ...
        case let .postEmbed(_, _, avatarId, author, date, blocks, caption):
            ...
            if !author.isEmpty {
                let avatar: ElloAppMediaImage? = avatarId.flatMap { media[$0] as? ElloAppMediaImage }
        ...
        case let .slideshow(items: subItems, caption: caption):
            ...
            for subBlock in subItems {
                switch subBlock {
                    case let .image(id, caption, url, webpageId):
                        if let image = media[id] as? ElloAppMediaImage, let imageSize = largestImageRepresentation(image.representations)?.dimensions {
                            let mediaIndex = mediaIndexCounter
                            mediaIndexCounter += 1
        ...
        case let .webEmbed(url, html, dimensions, caption, stretchToWidth, allowScrolling, coverId):
            ...
            if let url = url, let coverId = coverId, let image = media[coverId] as? ElloAppMediaImage {
                let loadedContent = ElloAppMediaWebpageLoadedContent(url: url, displayUrl: url, hash: 0, type: "video", websiteName: nil, title: nil, text: nil, embedUrl: url, embedType: "video", embedSize: PixelDimensions(size), duration: nil, author: nil, image: image, file: nil, attributes: [], instantPage: nil)
                let content = ElloAppMediaWebpageContent.Loaded(loadedContent)
                
                item = InstantPageImageItem(frame: frame, webPage: webpage, media: InstantPageMedia(index: embedIndex, media: ElloAppMediaWebpage(webpageId: MediaId(namespace: Namespaces.Media.LocalWebpage, id: -1), content: content), url: nil, caption: nil, credit: nil), attributes: [], interactive: true, roundCorners: false, fit: false)
        ...
        case let .audio(audioId, caption):
            ...
            if let file = media[audioId] as? ElloAppMediaFile {
                let mediaIndex = mediaIndexCounter
                mediaIndexCounter += 1
        ...
        case let .relatedArticles(title, articles):
            ...
            for (i, article) in articles.enumerated() {
                var cover: ElloAppMediaImage?
                if let coverId = article.photoId {
                    cover = media[coverId] as? ElloAppMediaImage
                }
        ...
        case let .map(latitude, longitude, zoom, dimensions, caption):
            ...
            let map = ElloAppMediaMap(latitude: latitude, longitude: longitude, heading: nil, accuracyRadius: nil, geoPlace: nil, venue: nil, liveBroadcastingTimeout: nil, liveProximityNotificationRadius: nil)
...
func instantPageLayoutForWebPage(_ webPage: ElloAppMediaWebpage, boundingWidth: CGFloat, safeInset: CGFloat, strings: PresentationStrings, theme: InstantPageTheme, dateTimeFormat: PresentationDateTimeFormat, webEmbedHeights: [Int : CGFloat] = [:]) -> InstantPageLayout {
    var maybeLoadedContent: ElloAppMediaWebpageLoadedContent?
