...
private func extractFileMedia(_ item: InstantPageMedia) -> ElloAppMediaFile? {
    return item.media as? ElloAppMediaFile
}

final class InstantPageMediaPlaylistItem: SharedMediaPlaylistItem {
    let webPage: ElloAppMediaWebpage
    let id: SharedMediaPlaylistItemId
    let item: InstantPageMedia
    
    init(webPage: ElloAppMediaWebpage, item: InstantPageMedia) {
            ...
            for attribute in file.attributes {
                switch attribute {
                    case let .Audio(isVoice, _, _, _, _):
                        if isVoice {
                            return SharedMediaPlaybackData(type: .voice, source: .elloappFile(reference: .webPage(webPage: WebpageReference(self.webPage), media: file), isCopyProtected: false))
                        } else {
                            return SharedMediaPlaybackData(type: .music, source: .elloappFile(reference: .webPage(webPage: WebpageReference(self.webPage), media: file), isCopyProtected: false))
                        }
                    case let .Video(_, _, flags):
                        if flags.contains(.instantRoundVideo) {
                            return SharedMediaPlaybackData(type: .instantVideo, source: .elloappFile(reference: .webPage(webPage: WebpageReference(self.webPage), media: file), isCopyProtected: false))
                        } else {
                            return nil
                        }
                    default:
...
final class InstantPageMediaPlaylist: SharedMediaPlaylist {
    ...
    init(webPage: ElloAppMediaWebpage, items: [InstantPageMedia], initialItemIndex: Int) {
