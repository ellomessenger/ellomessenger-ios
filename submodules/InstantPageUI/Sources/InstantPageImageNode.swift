...
final class InstantPageImageNode: ASDisplayNode, InstantPageNode {
    ...
    init(context: AccountContext, sourcePeerType: MediaAutoDownloadPeerType, theme: InstantPageTheme, webPage: ElloAppMediaWebpage, media: InstantPageMedia, attributes: [InstantPageImageAttribute], interactive: Bool, roundCorners: Bool, fit: Bool, openMedia: @escaping (InstantPageMedia) -> Void, longPressMedia: @escaping (InstantPageMedia) -> Void, activatePinchPreview: ((PinchSourceContainerNode) -> Void)?, pinchPreviewFinished: ((InstantPageNode) -> Void)?) {
        ...
        if let image = media.media as? ElloAppMediaImage, let largest = largestImageRepresentation(image.representations) {
        ...
        } else if let file = media.media as? ElloAppMediaFile {
        ...
        } else if let map = media.media as? ElloAppMediaMap {
        ...
        } else if let webPage = media.media as? ElloAppMediaWebpage, case let .Loaded(content) = webPage.content, let image = content.image {
            let imageReference = ImageMediaReference.webPage(webPage: WebpageReference(webPage), media: image)
            self.imageNode.setSignal(chatMessagePhoto(postbox: context.account.postbox, photoReference: imageReference))
            self.fetchedDisposable.set(chatMessagePhotoInteractiveFetched(context: context, photoReference: imageReference, displayAtSize: nil, storeToDownloadsPeerType: nil).start())
            self.statusNode.transitionToState(.play(.white), animated: false, completion: {})
    ...
    override func layout() {
        ...
        if self.currentSize != size || self.themeUpdated {
            ...
            if let image = self.media.media as? ElloAppMediaImage, let largest = largestImageRepresentation(image.representations) {
            ...
            } else if let file = self.media.media as? ElloAppMediaFile, let dimensions = file.dimensions {
                ...
                apply()
            } else if self.media.media is ElloAppMediaMap {
                for attribute in self.attributes {
                    if let mapAttribute = attribute as? InstantPageMapAttribute {
                ...
                pinApply()
            } else if let webPage = media.media as? ElloAppMediaWebpage, case let .Loaded(content) = webPage.content, let image = content.image, let largest = largestImageRepresentation(image.representations) {
                let imageSize = largest.dimensions.cgSize.aspectFilled(size)
                let boundingSize = size
                let radius: CGFloat = self.roundCorners ? floor(min(imageSize.width, imageSize.height) / 2.0) : 0.0
                let makeLayout = self.imageNode.asyncLayout()
                    ...
                    if let fetchStatus = self.fetchStatus {
                        switch fetchStatus {
                            case .Local:
                                switch gesture {
                                    case .tap:
                                        if self.media.media is ElloAppMediaImage && self.media.index == -1 {
                                            return
                                        }
