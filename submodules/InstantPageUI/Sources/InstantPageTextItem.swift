...
final class InstantPageTextItem: InstantPageItem {
    ...
    func linkSelectionRects(at point: CGPoint) -> [CGRect] {
        if let (index, dict) = self.attributesAtPoint(point) {
            if let _ = dict[NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL)] {
                if let rects = self.attributeRects(name: NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL), at: index) {
    ...
    func urlAttribute(at point: CGPoint) -> InstantPageUrlItem? {
        if let (_, dict) = self.attributesAtPoint(point) {
            if let url = dict[NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL)] as? InstantPageUrlItem {
                return url
            }
        }
        return nil
...
func attributedStringForRichText(_ text: RichText, styleStack: InstantPageTextStyleStack, url: InstantPageUrlItem? = nil, boundingWidth: CGFloat? = nil) -> NSAttributedString {
    switch text {
        ...
        case let .plain(string):
            ...
            if let url = url {
                attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL)] = url
            }
...
func layoutTextItemWithString(_ string: NSAttributedString, boundingWidth: CGFloat, horizontalInset: CGFloat = 0.0, alignment: NSTextAlignment = .natural, offset: CGPoint, media: [MediaId: Media] = [:], webpage: ElloAppMediaWebpage? = nil, minimizeWidth: Bool = false, maxNumberOfLines: Int = 0, opaqueBackground: Bool = false) -> (InstantPageTextItem?, [InstantPageItem], CGSize) {
    if string.length == 0 {
        return (nil, [], CGSize())
    }
    ...
    if let webpage = webpage {
        ...
        for line in textItem.lines {
            ...
            for imageItem in line.imageItems {
                if let image = media[imageItem.id] as? ElloAppMediaFile {
