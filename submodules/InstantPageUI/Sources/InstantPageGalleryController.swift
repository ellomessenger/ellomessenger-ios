...
public struct InstantPageGalleryEntry: Equatable {
    ...
    func item(context: AccountContext, webPage: ElloAppMediaWebpage, message: Message?, presentationData: PresentationData, fromPlayingVideo: Bool, landscape: Bool, openUrl: @escaping (InstantPageUrlItem) -> Void, openUrlOptions: @escaping (InstantPageUrlItem) -> Void) -> GalleryItem {
        ...
        if let image = self.media.media as? ElloAppMediaImage {
        ...
        } else if let file = self.media.media as? ElloAppMediaFile {
            if file.isVideo {
                ...
                if let dimensions = file.dimensions {
                    representations.append(ElloAppMediaImageRepresentation(dimensions: dimensions, resource: file.resource, progressiveSizes: [], immediateThumbnailData: nil))
                }
                let image = ElloAppMediaImage(imageId: MediaId(namespace: 0, id: 0), representations: representations, immediateThumbnailData: file.immediateThumbnailData, reference: nil, partialReference: nil, flags: [])
                return InstantImageGalleryItem(context: context, presentationData: presentationData, itemId: self.index, imageReference: .webPage(webPage: WebpageReference(webPage), media: image), caption: caption, credit: credit, location: self.location, openUrl: openUrl, openUrlOptions: openUrlOptions)
            }
        } else if let embedWebpage = self.media.media as? ElloAppMediaWebpage, case let .Loaded(webpageContent) = embedWebpage.content {
...
public class InstantPageGalleryController: ViewController, StandalonePresentableController {
    ...
    private let webPage: ElloAppMediaWebpage
