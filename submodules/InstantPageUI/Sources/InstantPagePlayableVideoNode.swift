...
final class InstantPagePlayableVideoNode: ASDisplayNode, InstantPageNode, GalleryItemTransitionNode {
    ...
    init(context: AccountContext, webPage: ElloAppMediaWebpage, theme: InstantPageTheme, media: InstantPageMedia, interactive: Bool, openMedia: @escaping (InstantPageMedia) -> Void) {
        ...
        if let file = media.media as? ElloAppMediaFile, let presentation = smallestImageRepresentation(file.previewRepresentations) {
            let image = ElloAppMediaImage(imageId: MediaId(namespace: 0, id: 0), representations: [presentation], immediateThumbnailData: file.immediateThumbnailData, reference: nil, partialReference: nil, flags: [])
