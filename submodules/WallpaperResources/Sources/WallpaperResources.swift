...
public func wallpaperDatas(account: Account, accountManager: AccountManager<ElloAppAccountManagerTypes>, fileReference: FileMediaReference? = nil, representations: [ImageRepresentationWithReference], alwaysShowThumbnailFirst: Bool = false, thumbnail: Bool = false, onlyFullSize: Bool = false, autoFetchFullSize: Bool = false, synchronousLoad: Bool = false) -> Signal<(Data?, Data?, Bool), NoError> {
    ...
    |> map { (thumbnailData, fullSizeData, fullSizeComplete) in
        return { arguments in
            ...
            if blurred, let fullSizeImageValue = fullSizeImage {
                ...
                elloappFastBlurMore(Int32(thumbnailContextSize.width), Int32(thumbnailContextSize.height), Int32(thumbnailContext.bytesPerRow), thumbnailContext.bytes)
...
private func patternWallpaperDatas(account: Account, accountManager: AccountManager<ElloAppAccountManagerTypes>, representations: [ImageRepresentationWithReference], mode: PatternWallpaperDrawMode, autoFetchFullSize: Bool = false) -> Signal<(Data?, Bool), NoError> {
...
public func patternWallpaperImage(account: Account, accountManager: AccountManager<ElloAppAccountManagerTypes>, representations: [ImageRepresentationWithReference], mode: PatternWallpaperDrawMode, autoFetchFullSize: Bool = false) -> Signal<((TransformImageArguments) -> DrawingContext?)?, NoError> {
...
public func elloappThemeData(account: Account, accountManager: AccountManager<ElloAppAccountManagerTypes>, reference: MediaResourceReference, synchronousLoad: Bool = false) -> Signal<Data?, NoError> {
...
public enum ThemeImageSource {
    ...
    case settings(ElloAppThemeSettings)
}

public func themeImage(account: Account, accountManager: AccountManager<ElloAppAccountManagerTypes>, source: ThemeImageSource, synchronousLoad: Bool = false) -> Signal<(TransformImageArguments) -> DrawingContext?, NoError> {
            ...
            if case let .file(file) = theme.chat.defaultWallpaper {
                ...
                |> mapToSignal { wallpaper -> Signal<(PresentationTheme?, WallpaperImage?, Data?), NoError> in
                    if let wallpaper = wallpaper, case let .file(file) = wallpaper.wallpaper {
                        ...
                        convertedRepresentations.append(ImageRepresentationWithReference(representation: ElloAppMediaImageRepresentation(dimensions: PixelDimensions(width: 100, height: 100), resource: file.file.resource, progressiveSizes: [], immediateThumbnailData: nil), reference: .wallpaper(wallpaper: .slug(file.slug), resource: file.file.resource)))
...
public func themeIconImage(account: Account, accountManager: AccountManager<ElloAppAccountManagerTypes>, theme: PresentationThemeReference, color: PresentationThemeAccentColor?, wallpaper: ElloAppWallpaper? = nil, nightMode: Bool? = nil, emoticon: Bool = false, large: Bool = false, qr: Bool = false, message: Bool = false) -> Signal<(TransformImageArguments) -> DrawingContext?, NoError> {
    ...
    if case let .cloud(theme) = theme, theme.theme.settings != nil, let nightMode = nightMode {
        ...
        themeSignal = elloappThemeData(account: account, accountManager: accountManager, reference: reference, synchronousLoad: false)
...
public func wallpaperThumbnail(account: Account, accountManager: AccountManager<ElloAppAccountManagerTypes>, fileReference: FileMediaReference, wallpaper: ElloAppWallpaper, synchronousLoad: Bool) -> Signal<(TransformImageArguments) -> DrawingContext?, NoError> {
    switch wallpaper {
    case let .file(file):
