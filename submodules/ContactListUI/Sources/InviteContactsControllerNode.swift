...
private final class InviteContactsInteraction {
    ...
    let shareElloApp: () -> Void
    
    init(toggleContact: @escaping (String) -> Void, shareElloApp: @escaping () -> Void) {
        self.toggleContact = toggleContact
        self.shareElloApp = shareElloApp
    }
}
    ...
    func item(context: AccountContext, presentationData: PresentationData, interaction: InviteContactsInteraction) -> ListViewItem {
        switch self {
            ...
            case let .peer(_, id, contact, count, selection, theme, strings, nameSortOrder, nameDisplayOrder):
                ...
                let peer: EnginePeer = .user(ElloAppUser(id: EnginePeer.Id(namespace: .max, id: EnginePeer.Id.Id._internalFromInt64Value(0)), accessHash: nil, firstName: contact.firstName, lastName: contact.lastName, username: nil, phone: nil, photo: [], botInfo: nil, email: nil, country: nil, gender: nil, birthday: nil, restrictionInfo: nil, flags: [], emojiStatus: nil, isBusiness: false, isPublic: false, botDescription: nil))
...
private func inviteContactsEntries(accountPeer: EnginePeer?, sortedContacts: [(DeviceContactStableId, DeviceContactBasicData, Int32)]?, selectionState: InviteContactsGroupSelectionState, theme: PresentationTheme, strings: PresentationStrings, nameSortOrder: PresentationPersonNameOrder, nameDisplayOrder: PresentationPersonNameOrder, interaction: InviteContactsInteraction) -> [InviteContactsEntry] {
    ...
    entries.append(.option(0, ContactListAdditionalOption(title: strings.Contacts_ShareElloApp, icon: .generic(UIImage(bundleImageName: "Contact List/InviteActionIcon")!), action: {
        interaction.shareElloApp()
    }), theme, strings))
...
final class InviteContactsControllerNode: ASDisplayNode {
    ...
    init(context: AccountContext) {
        ...
        }, shareElloApp: { [weak self] in
            self?.requestShareElloApp?()
        })
        ...
        |> mapToSignal { existingNumbersAndPeerIds, contacts -> Signal<[(DeviceContactStableId, DeviceContactBasicData, Int32)]?, NoError> in
            let mappedContacts: [(String, [DeviceContactNormalizedPhoneNumber])] = []
//            for (id, basicData) in contacts {
//                mappedContacts.append((id: id, basicData.userNames.map({ userName in
//                    return DeviceContactNormalizedPhoneNumber(rawValue: formatPhoneNumber(userName.value))
//                })))
//            }
            ...
            |> map { counts -> [(DeviceContactStableId, DeviceContactBasicData, Int32)]? in
                ...
                for (id, basicData) in contacts {
                    var found = false
                    if basicData.userNames.isEmpty {
                        existing.insert(id)
                        continue
                    }
                    
                    for number in basicData.userNames {
