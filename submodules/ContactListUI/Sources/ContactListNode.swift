...
private extension EnginePeer.IndexName {
    func isLessThan(other: EnginePeer.IndexName, ordering: PresentationPersonNameOrder) -> ComparisonResult {
        switch self {
        case let .title(lhsTitle, _, _):
            let rhsString: String
            switch other {
            case let .title(title, _, _):
                rhsString = title
...
private func preparedContactListNodeTransition(context: AccountContext, presentationData: PresentationData, from fromEntries: [ContactListNodeEntry], to toEntries: [ContactListNodeEntry], interaction: ContactListNodeInteraction, firstTime: Bool, isEmpty: Bool, isEmptyPeer: Bool, generateIndexSections: Bool, animation: ContactListAnimation, isSearch: Bool) -> ContactsListNodeTransition {
    ...
    return ContactsListNodeTransition(deletions: deletions, insertions: insertions, updates: updates, indexSections: indexSections, firstTime: firstTime, isEmpty: isEmpty, isEmptyPeer: isEmptyPeer, scrollToItem: scrollToItem, animation: animation)
}
...
public final class ContactListNode: ASDisplayNode {
            ...
            if value != self.enableUpdatesValue {
                ...
                if value {
                    self.contactPeersViewPromise.set(self.context.engine.data.subscribe(
                        ...
                        ElloAppEngine.EngineData.Item.Peer.Peer(id: self.context.engine.account.peerId)
                    )
    ...
    public init(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, presentation: Signal<ContactListPresentation, NoError>, filters: [ContactListFilter] = [.excludeSelf], selectionState: ContactListNodeGroupSelectionState? = nil, displayPermissionPlaceholder: Bool = true, displaySortOptions: Bool = false, displayCallIcons: Bool = false, contextAction: ((EnginePeer, ASDisplayNode, ContextGesture?, CGPoint?) -> Void)? = nil, isSearch: Bool = false, multipleSelection: Bool = false, mode: ContactMultiselectionControllerMode? = .none, isSearchActive: Bool = false) {
        ...
        let title: String
        let text: String
        var showPromoButton = false
        switch mode {
        case .groupCreation:
            title = presentationData.strings.Contacts_CreateGroup_EmptyState_Title
            text = presentationData.strings.Contacts_CreateGroup_EmptyState_Text
        case .channelCreation:
            title = presentationData.strings.Contacts_CreateChannel_EmptyState_Title
            text = presentationData.strings.Contacts_CreateChannel_EmptyState_Text
        case .publicChannelCreation:
            title = presentationData.strings.Contacts_CreatePublicChannel_EmptyState_Title
            text = presentationData.strings.Contacts_CreatePublicChannel_EmptyState_Text
            showPromoButton.toggle()
        default:
            title = presentationData.strings.Contacts_EmptyState_Title
            text = presentationData.strings.Contacts_EmptyState_Text
        }
        
        self.emptyStateNode = ContactListEmptyStateNode(
            context: context,
            theme: presentationData.theme,
            strings: presentationData.strings,
            kind: PermissionKind.contacts.rawValue,
            icon: .animation("anim_no-contacts"),
            title: title,
            text: text,
            showPromoButton: showPromoButton)
        self.emptyStateNode.isHidden = true
        self.emptyStateNode.isUserInteractionEnabled = false
        
        self.emptySearchStateNode = ContactsSearchEmptyContainerNode(context: context)
        self.emptySearchStateNode.isHidden = true
        ...
        self.addSubnode(self.emptyStateNode)
        if isSearchActive {
            self.addSubnode(self.emptySearchStateNode)
        }
            ...
            if case let .search(query, searchChatList, searchDeviceContacts, searchGroups, searchChannels, globalSearch) = presentation {
                        ...
                        |> mapToSignal { peers -> Signal<([FoundPeer], [EnginePeer.Id: EnginePeer.Presence]), NoError> in
                            ...
                            for peer in peers {
                                if searchGroups || searchChannels {
                                    ...
                                    if let _ = mainPeer as? ElloAppUser {
                                    } else if let _ = mainPeer as? ElloAppGroup {
                                    } else if let channel = mainPeer as? ElloAppChannel {
                            ...
                            return context.engine.data.get(
                                EngineDataMap(resultPeers.map(\.peer.id).map(ElloAppEngine.EngineData.Item.Peer.Presence.init)),
                                EngineDataMap(resultPeers.map(\.peer.id).map(ElloAppEngine.EngineData.Item.Peer.ParticipantCount.init))
                            )
                            |> map { presenceMap, participantCountMap -> ([FoundPeer], [EnginePeer.Id: EnginePeer.Presence]) in
                                ...
                                for peer in resultPeers {
                                    ...
                                    if let _ = peer.peer as? ElloAppChannel {
                        ...
                        let signal = deferred { () -> Signal<ContactsListNodeTransition, NoError> in
                            ...
                            for peer in localPeersAndStatuses.0 {
                                if !existingPeerIds.contains(peer.peer.id) {
                                    ...
                                    if searchDeviceContacts, let user = peer.peer as? ElloAppUser, let phone = user.phone {
                            ...
                            for peer in remotePeers.0 {
                                let matches: Bool
                                if peer.peer is ElloAppUser {
                                    matches = true
                                } else if searchGroups || searchChannels {
                                    if peer.peer is ElloAppGroup && searchGroups {
                                        matches = true
                                    } else if let channel = peer.peer as? ElloAppChannel {
                                        if case .group = channel.info {
                            ...
                            outer: for (stableId, contact) in deviceContacts {
                                inner: for phoneNumber in contact.0.userNames {
                            ...
                            return .single(preparedContactListNodeTransition(context: context, presentationData: presentationData, from: previous ?? [], to: entries, interaction: interaction, firstTime: previous == nil, isEmpty: false, isEmptyPeer: peers.isEmpty, generateIndexSections: generateSections, animation: .none, isSearch: isSearch))
                        }
                    ...
                    |> mapToSignal { view, _ -> Signal<[(EnginePeer, Int32)], NoError> in
                        return context.engine.data.get(EngineDataMap(
                            view.entries.compactMap { entry -> EnginePeer.Id? in
                                ...
                                return nil
                            }.map(ElloAppEngine.EngineData.Item.Peer.ParticipantCount.init)
                        ))
                            ...
                            for entry in view.entries {
                                ...
                                case let .MessageEntry(_, _, _, _, _, renderedPeer, _, _, _, _):
                                    if let peer = renderedPeer.peer {
                                        ...
                                        } else if let channel = peer as? ElloAppChannel, case .group = channel.info {
                ...
                |> mapToQueue { view, chatListPeers, selectionState, presentationData, authorizationStatus, warningSuppressed -> Signal<ContactsListNodeTransition, NoError> in
                    let signal = deferred { () -> Signal<ContactsListNodeTransition, NoError> in
                        ...
                        return .single(preparedContactListNodeTransition(context: context, presentationData: presentationData, from: previous ?? [], to: entries, interaction: interaction, firstTime: previous == nil, isEmpty: isEmpty, isEmptyPeer: peers.isEmpty, generateIndexSections: generateSections, animation: animation, isSearch: isSearch))
                    }
        ...
        |> deliverOnMainQueue).start(next: { [weak self] presentationData in
            if let strongSelf = self {
                ...
                if previousTheme !== presentationData.theme || previousStrings !== presentationData.strings {
                    ...
                    let emptyStatePreviousHidden = strongSelf.emptyStateNode.isHidden
                    let emptyStatePreviousUserInteractionEnabled = strongSelf.emptyStateNode.isUserInteractionEnabled
                    strongSelf.emptyStateNode.removeFromSupernode()
                    strongSelf.emptyStateNode = ContactListEmptyStateNode(
                        context: context,
                        theme: presentationData.theme,
                        strings: presentationData.strings,
                        kind: PermissionKind.contacts.rawValue,
                        ...
                        text: text,
                        showPromoButton: showPromoButton)
                    strongSelf.emptyStateNode.isHidden = emptyStatePreviousHidden
                    strongSelf.emptyStateNode.isUserInteractionEnabled = emptyStatePreviousUserInteractionEnabled
                    strongSelf.addSubnode(strongSelf.emptyStateNode)
                    
                    let emptySearchStatePreviousHidden = strongSelf.emptySearchStateNode.isHidden
                    strongSelf.emptySearchStateNode.removeFromSupernode()
                    strongSelf.emptySearchStateNode = ContactsSearchEmptyContainerNode(context: context)
                    strongSelf.emptySearchStateNode.isHidden = emptySearchStatePreviousHidden
                    strongSelf.addSubnode(strongSelf.emptySearchStateNode)
    ...
    public func containerLayoutUpdated(_ layout: ContainerViewLayout, headerInsets: UIEdgeInsets, transition: ContainedViewLayoutTransition) {
        ...
        emptyStateNode.updateLayout(size: layout.size, additionalInsets: layout.additionalInsets, insets: insets, transition: transition)
        transition.updateFrame(node: emptyStateNode, frame: CGRect(origin: .zero, size: layout.size))
        
        emptySearchStateNode.updateLayout(size: layout.size, insets: insets, transition: transition)
        transition.updateFrame(node: emptySearchStateNode, frame: CGRect(origin: .zero, size: layout.size))
    ...
    private func dequeueTransitions() {
        if self.validLayout != nil {
            while !self.queuedTransitions.isEmpty {
                ...
                let isEmptyStateNodeHidden = !transition.isEmptyPeer || !authorizationNode.isHidden || !displayPermissionPlaceholder
                emptyStateNode.isHidden = isEmptyStateNodeHidden
                emptyStateNode.isUserInteractionEnabled = !isEmptyStateNodeHidden
                emptySearchStateNode.isHidden = !isSearchActive || !transition.isEmptyPeer || transition.firstTime
            }
        }
    }
    ...
    public func clearSearchState() {
        emptySearchStateNode.removeFromSupernode()
    }
}
