...
//        self.navigationItem.rightBarButtonItem?.accessibilityLabel = self.presentationData.strings.Contacts_VoiceOver_AddContact
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(didEnterBackgroundMode), name: UIApplication.didEnterBackgroundNotification, object: nil)
    }
...
//        }
    ...
    override public func loadDisplayNode() {
                ...
                switch peer {
                    ...
                    case let .deviceContact(id, _):
                        ...
                        |> deliverOnMainQueue).start(next: { value in
                            ...
                            (strongSelf.navigationController as? NavigationController)?.pushViewController(strongSelf.context.sharedContext.makeDeviceContactInfoController(context: strongSelf.context, subject: .vcard(nil, id, value), isTitleHidden: false, completed: nil, cancelled: nil), completion: { [weak self] in
                                if let strongSelf = self {
                                    strongSelf.contactsNode.contactListNode.listNode.clearHighlightAnimated(true)
                                }
                            })
        ...
        self.contactsNode.contactListNode.openPrivacyPolicy = { [weak self] in
            if let strongSelf = self {
                strongSelf.context.sharedContext.openExternalUrl(context: strongSelf.context, urlContext: .generic, url: "https://www.elloapp.com/privacy", forceExternal: true, presentationData: strongSelf.presentationData, navigationController: strongSelf.navigationController as? NavigationController, dismissInput: {})
            }
        }
        ...
        self.contactsNode.requestAddContact = { [weak self] userName in
            if let strongSelf = self {
...
//        }
                        ...
                        if let strongSelf = self {
                            ...
                            |> deliverOnMainQueue).start(next: { [weak self, weak controller] peer in
                                if let strongSelf = self, let controller = controller {
                                    controller.present(strongSelf.context.sharedContext.makeChatQrCodeScreen(context: strongSelf.context, peer: peer, inviteLink: ""), in: .window(.root))
                                }
                            })
                        }
                    }
    ...
    @objc func didEnterBackgroundMode() {
        self.contactsNode.contactListNode.clearSearchState()
    }
            ...
            switch status {
                case .allowed:
                    let contactData = DeviceContactExtendedData(
                        basicData: DeviceContactBasicData(firstName: "",
                                                          lastName: "",
                    ...
                    if let navigationController = strongSelf.context.sharedContext.mainWindow?.viewController as? NavigationController {
                        navigationController.pushViewController(strongSelf.context.sharedContext.makeDeviceContactInfoController(context: strongSelf.context, subject: .create(peer: nil, contactData: contactData, isSharing: false, shareViaException: false, completion: { peer, stableId, contactData in
                            ...
                            if let peer = peer {
                                ...
                                if let navigationController = strongSelf.context.sharedContext.mainWindow?.viewController as? NavigationController {
                                    navigationController.pushViewController(strongSelf.context.sharedContext.makeDeviceContactInfoController(context: strongSelf.context, subject: .vcard(nil, stableId, contactData), isTitleHidden: false, completed: nil, cancelled: nil))
                                }
                            }
                        }), isTitleHidden: true, completed: nil, cancelled: nil))
                    }
                case .notDetermined:
                    DeviceAccess.authorizeAccess(to: .contacts)
                default:
