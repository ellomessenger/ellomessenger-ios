...
class ItemListPeerActionItemNode: ListViewItemNode {
    ...
    func asyncLayout() -> (_ item: ItemListPeerActionItem, _ params: ListViewItemLayoutParams, _ neighbors: ItemListNeighbors) -> (ListViewItemNodeLayout, (Bool) -> Void) {
        ...
        return { item, params, neighbors in
            ...
            let titleFont = Font.regular(item.presentationData.fontSize.itemListBaseFontSize * 15.0 / 17.0)
            ...
            case .compactInsets:
                ...
                let iconLeading = 12.0
                let iconTrailing = 8.0
                leftInset = if let icon = item.icon {
                    iconLeading + icon.size.width + iconTrailing + params.leftInset
                } else {
                    iconLeading + params.leftInset
                }
            }
