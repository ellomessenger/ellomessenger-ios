...
private final class ImportManager {
    ...
    private let entries: [(SSZipEntry, String, ElloAppEngine.HistoryImport.MediaType)]
    
    private var session: ElloAppEngine.HistoryImport.Session?
    ...
    private var pendingEntries: [(SSZipEntry, String, ElloAppEngine.HistoryImport.MediaType)]
    ...
    init(account: Account, peerId: PeerId, mainFile: TempBoxFile, archivePath: String?, entries: [(SSZipEntry, String, ElloAppEngine.HistoryImport.MediaType)]) {
        ...
        self.disposable.set((ElloAppEngine(account: self.account).historyImport.initSession(peerId: peerId, file: mainFile, mediaCount: Int32(entries.count))
    ...
    private func complete() {
        ...
        self.disposable.set((ElloAppEngine(account: self.account).historyImport.startImport(session: session)
    ...
    private func updateState() {
        ...
        while true {
            ...
            |> mapToSignal { tempFile -> Signal<Float, ImportError> in
                ...
                return ElloAppEngine(account: account).historyImport.uploadMedia(session: session, file: tempFile, disposeFileAfterDone: true, fileName: entry.0.path, mimeType: mimeType, type: entry.2)
                |> mapError { error -> ImportError in
                    switch error {
                    case .chatAdminRequired:
                        return .chatAdminRequired
...
//            self.backgroundColor = self.presentationData.theme.list.plainBackgroundColor
            ...
            self.addSubnode(self.animationNode)// TODO: Need update animation sticker
            self.addSubnode(self.doneAnimationNode)// TODO: Need update animation sticker
    ...
    private let totalBytes: Int64
    private let totalMediaBytes: Int64
    private let otherEntries: [(SSZipEntry, String, ElloAppEngine.HistoryImport.MediaType)]
    ...
    public init(context: AccountContext, cancel: @escaping () -> Void, peerId: PeerId, archivePath: String?, mainEntry: TempBoxFile, otherEntries: [(SSZipEntry, String, ElloAppEngine.HistoryImport.MediaType)]) {
        self.context = context
        self.cancel = cancel
        self.peerId = peerId
        self.archivePath = archivePath
        ...
        self.otherEntries = otherEntries.map { entry -> (SSZipEntry, String, ElloAppEngine.HistoryImport.MediaType) in
