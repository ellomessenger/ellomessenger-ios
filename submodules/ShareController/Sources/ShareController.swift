...
private func collectExternalShareItems(strings: PresentationStrings, dateTimeFormat: PresentationDateTimeFormat, nameOrder: PresentationPersonNameOrder, engine: ElloAppEngine, postbox: Postbox, collectableItems: [CollectableExternalShareItem], takeOne: Bool = true) -> Signal<ExternalShareItemsState, NoError> {
    ...
    let peerTitles = engine.data.get(EngineDataMap(
        authorsPeerIds.map(ElloAppEngine.EngineData.Item.Peer.Peer.init)
    ))
    ...
    for item in collectableItems {
        if let mediaReference = item.mediaReference, let file = mediaReference.media as? ElloAppMediaFile {
                        ...
                        case let .done(data):
                            if file.isSticker, !file.isAnimatedSticker, let dimensions = file.dimensions {
                                ...
                                if let value = file.fileName {
                                    ...
                                    fileName = "elloapp_video.mp4"
                                } else if file.isVoice {
                                    fileName = "elloapp_audio.ogg"
                                } else {
                                    fileName = "file"
                                }
        ...
        } else if let mediaReference = item.mediaReference, let image = mediaReference.media as? ElloAppMediaImage, let largest = largestImageRepresentation(image.representations) {
        ...
        } else if let mediaReference = item.mediaReference, let poll = mediaReference.media as? ElloAppMediaPoll {
            var text = "📊 \(poll.text)"
        ...
        } else if let mediaReference = item.mediaReference, let contact = mediaReference.media as? ElloAppMediaContact {
            ...
            if let vCard = contact.vCardData, let vCardData = vCard.data(using: .utf8), let parsed = DeviceContactExtendedData(vcard: vCardData) {
                ...
                contactData = DeviceContactExtendedData(basicData: DeviceContactBasicData(firstName: contact.firstName,
                                                                                          lastName: contact.lastName,
                                                                                          userNames: [DeviceContactUserNameData(
                                                                                            label: "_$!<Mobile>!$_",
...
public final class ShareController: ViewController {
    ...
    public convenience init(context: AccountContext, 
                            ...
                            switchableAccounts: [AccountWithInfo] = [],
    ...
    public init(sharedContext: SharedAccountContext, 
        ...
        switch subject {
            ...
            case let .media(mediaReference):
                var canSave = false
                var isVideo = false
                if mediaReference.media is ElloAppMediaImage {
                    canSave = true
                } else if let file = mediaReference.media as? ElloAppMediaFile {
                    canSave = true
            ...
            case let .messages(messages):
                if case .saveToCameraRoll = preferredAction {
                    ...
                    } else if let chatPeer = message.peers[message.id.peerId] as? ElloAppChannel, messages.count == 1 || sameGroupingKey {
                        if message.id.namespace == Namespaces.Message.Cloud {
                            self.defaultAction = ShareControllerAction(title: self.presentationData.strings.ShareMenu_CopyShareLink, action: { [weak self] in
                                guard let strongSelf = self else {
                                    return
                                }
                                let _ = (ElloAppEngine(account: strongSelf.currentAccount).messages.exportMessageLink(peerId: chatPeer.id, messageId: message.id)
    ...
    override public func loadDisplayNode() {
        ...
        if case let .messages(messages) = self.subject, let message = messages.first, let peer = message.peers[message.id.peerId] as? ElloAppChannel, case .broadcast = peer.info {
        ...
        self.controllerNode.share = { [weak self] text, peerIds, showNames, silently in
            ...
            case let .media(mediaReference):
                ...
                for peerId in peerIds {
                    ...
                    messages.append(.message(text: sendTextAsCaption ? text : "",
                                             attributes: [],
                                             inlineStickers: [:],
                    ...
                    messages = transformMessages(messages, showNames: showNames, silently: silently)
                    shareSignals.append(enqueueMessages(account: strongSelf.currentAccount,
                                                        peerId: peerId,
                                                        messages: messages))
                }
            case let .mapMedia(media):
                    ...
                    for (id, status, error) in statuses {
                        if let error = error {
                            Queue.mainQueue().async {
                                let _ = ElloAppEngine(account: account).messages.deleteMessagesInteractively(messageIds: [id], type: .forEveryone).start()
                                let _ = (ElloAppEngine(account: account).data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: id.peerId))
...
//                var messagesToShare: [Message]?
                switch subject {
                    ...
                    case let .image(representations):
                        let media = ElloAppMediaImage(imageId: MediaId(namespace: Namespaces.Media.LocalImage, id: Int64.random(in: Int64.min ... Int64.max)), representations: representations.map({ $0.representation }), immediateThumbnailData: nil, reference: nil, partialReference: nil, flags: [])
...
//                        messagesToShare = messages
                        for message in messages {
                            ...
                            loop: for media in message.media {
                                switch media {
                                    case _ as ElloAppMediaImage, _ as ElloAppMediaFile:
                                        selectedMedia = media
                                        break loop
                            ...
                            if let chatPeer = message.peers[message.id.peerId] as? ElloAppChannel {
                                if message.id.namespace == Namespaces.Message.Cloud, let addressName = chatPeer.addressName, !addressName.isEmpty {
                                    url = "https://ello.team/\(addressName)/\(message.id.id)"
                                    if messageUrl == nil {
                                        messageUrl = url
                                    }
                                }
...
//                                    }
    ...
    private func saveToCameraRoll(representations: [ImageRepresentationWithReference]) {
        let media = ElloAppMediaImage(imageId: MediaId(namespace: 0, id: 0), representations: representations.map({ $0.representation }), immediateThumbnailData: nil, reference: nil, partialReference: nil, flags: [])
        let context: AccountContext
        if self.currentContext.account.id == self.currentAccount.id {
            context = self.currentContext
        } else {
    ...
    private func switchToAccount(account: Account, animateIn: Bool) {
        ...
        self.peers.set(combineLatest(
            ElloAppEngine(account: self.currentAccount).data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: self.currentAccount.peerId)),
        ...
        |> mapToSignal { maybeAccountPeer, view -> Signal<([(EngineRenderedPeer, EnginePeer.Presence?)], EnginePeer), NoError> in
            ...
            return ElloAppEngine(account: account).data.subscribe(EngineDataMap(
                peers.map { ElloAppEngine.EngineData.Item.Peer.Presence(id: $0.peerId) }
            ))
...
final class MessageStoryRenderer {
    ...
    init(context: AccountContext, messages: [Message]) {
        ...
        self.addressNode.attributedText = NSAttributedString(string: "https://ello.team/\(addressName)/\(message.id.id)", font: Font.medium(14.0), textColor: UIColor(rgb: 0xffffff))
...
public class ShareToInstagramActivity: UIActivity {
    ...
    public override var activityType: UIActivity.ActivityType? {
        return UIActivity.ActivityType(rawValue: "org.elloapp.ElloApp.ShareToInstagram")
    }
