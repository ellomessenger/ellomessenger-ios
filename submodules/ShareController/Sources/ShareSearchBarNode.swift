...
final class ShareSearchBarNode: ASDisplayNode, UITextFieldDelegate {
    ...
    private let inputInsets = UIEdgeInsets(top: 10.0, left: 16.0, bottom: 10.0, right: 16.0)
    ...
    func updateLayout(width: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        let searchImageSize = searchIconNode.image?.size ?? .zero
        if let image = self.searchIconNode.image {
            transition.updateFrame(node: self.searchIconNode, frame: CGRect(origin: CGPoint(x: backgroundFrame.minX + 8.0, y: backgroundFrame.midY - searchImageSize.height / 2), size: image.size))
        }
        
        let clearImageSize = clearButton.image(for: [])?.size ?? .zero
        ...
        transition.updateFrame(node: self.textInputNode, frame: CGRect(origin: CGPoint(x: backgroundFrame.minX + inputInsets.left + searchImageSize.width, y: backgroundFrame.minY + UIScreenPixel), size: CGSize(width: backgroundFrame.size.width - inputInsets.left - inputInsets.right - searchImageSize.width - clearImageSize.width, height: backgroundFrame.size.height)))
    }
