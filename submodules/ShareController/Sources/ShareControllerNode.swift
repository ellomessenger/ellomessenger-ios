...
final class ShareControllerNode: ViewControllerTracingNode, UIScrollViewDelegate {
        ...
        if let signal = self.share?(self.inputFieldNode.text, peerIds, showNames, silently) {
            ...
            |> deliverOnMainQueue).start(next: { [weak self] status in
                ...
                if fromForeignApp, case let .preparing(long) = status, !transitioned {
                    transitioned = true
                    if long {
                        strongSelf.transitionToContentNode(ShareProlongedLoadingContainerNode(theme: strongSelf.presentationData.theme,
                                                                                              strings: strongSelf.presentationData.strings,
                                                                                              forceNativeAppearance: true,
                                                                                              account: strongSelf.context?.account,
                                                                                              sharedContext: strongSelf.sharedContext),
                                                           fastOut: true)
                    } else {
                        strongSelf.transitionToContentNode(ShareLoadingContainerNode(theme: strongSelf.presentationData.theme,
                                                                                     forceNativeAppearance: true),
                                                           fastOut: true)
                    }
                }
    ...
    func updatePeers(context: AccountContext, switchableAccounts: [AccountWithInfo], peers: [(EngineRenderedPeer, EnginePeer.Presence?)], accountPeer: EnginePeer, defaultAction: ShareControllerAction?) {
        ...
        if let peerId = self.immediatePeerId {
            ...
            let _ = (context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.RenderedPeer(id: peerId))
