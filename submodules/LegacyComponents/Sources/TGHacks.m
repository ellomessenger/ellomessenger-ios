...
+ (void)elloph_setAnimationDuration:(NSTimeInterval)duration;
...
+ (void)elloph_setAnimationDuration:(NSTimeInterval)duration
{
    [self elloph_setAnimationDuration:(duration * animationDurationFactor)];
}

+ (void)elloph_animateWithDuration:(NSTimeInterval)duration delay:(NSTimeInterval)delay options:(UIViewAnimationOptions)options animations:(void (^)(void))animations completion:(void (^)(BOOL finished))completion
{
    if (forceSystemCurve) {
        options |= (7 << 16);
    }
    [self elloph_animateWithDuration:duration * secondaryAnimationDurationFactor delay:delay options:options animations:animations completion:completion];
}
...
{
    SwizzleClassMethod([UIView class], @selector(setAnimationDuration:), @selector(elloph_setAnimationDuration:));
    SwizzleClassMethod([UIView class], @selector(animateWithDuration:delay:options:animations:completion:), @selector(elloph_animateWithDuration:delay:options:animations:completion:));
