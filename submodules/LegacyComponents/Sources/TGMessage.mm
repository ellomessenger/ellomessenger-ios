...
static void *NSTextCheckingResultElloAppHiddenLinkKey = &NSTextCheckingResultElloAppHiddenLinkKey;

@implementation NSTextCheckingResult (TGMessage)

- (void)setIsElloAppHiddenLink:(bool)value {
    objc_setAssociatedObject(self, NSTextCheckingResultElloAppHiddenLinkKey, @(value), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (bool)isElloAppHiddenLink {
    return [objc_getAssociatedObject(self, NSTextCheckingResultElloAppHiddenLinkKey) boolValue];
}
    ...
    if (entities != nil) {
        ...
        for (TGMessageEntity *entity in entities) {
            ...
            if ([entity isKindOfClass:[TGMessageEntityBold class]]) {
                ...
                [result setIsElloAppHiddenLink:true];
