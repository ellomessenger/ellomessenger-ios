...
private final class PlayPauseIconNode: ManagedAnimationNode {
    ...
    init() {
        ...
        self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause_new"), frames: .range(startFrame: kPauseFrame, endFrame: kPauseFrame), duration: 0.01))
    }
    ...
    func enqueueState(_ state: PlayPauseIconNodeState, animated: Bool) {
        guard self.iconState != state else {
            if !isInHierarchy {
                switch state {
                case .play:
                    self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause_new"), frames: .range(startFrame: kPlayFrame, endFrame: kPlayFrame), duration: 0.01))
                case .pause:
                    self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause_new"), frames: .range(startFrame: kPauseFrame, endFrame: kPauseFrame), duration: 0.01))
                }
            }
        ...
        switch previousState {
            case .pause:
                switch state {
                    case .play:
                        if animated {
                            self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause_new"), frames: .range(startFrame: kPauseFrame, endFrame: kPlayFrame), duration: self.duration))
                        } else {
                            self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause_new"), frames: .range(startFrame: kPlayFrame, endFrame: kPlayFrame), duration: 0.01))
                        }
                    case .pause:
            ...
            case .play:
                switch state {
                    case .pause:
                        if animated {
                            self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause_new"), frames: .range(startFrame: kPlayFrame, endFrame: kPauseFrame), duration: self.duration))
                        } else {
                            self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause_new"), frames: .range(startFrame: kPauseFrame, endFrame: kPauseFrame), duration: 0.01))
                        }
                    case .play:
