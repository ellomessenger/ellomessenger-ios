    ...
    final class State: ComponentState {
        ...
        init(context: AccountContext) {
            ...
            let reactionOverrideMessages = self.context.engine.data.get(
                EngineDataMap(accountSpecificReactionOverrides.map(\.messageId).map(ElloAppEngine.EngineData.Item.Messages.Message.init))
            )
            ...
            let stickerOverrideMessages = self.context.engine.data.get(
                EngineDataMap(accountSpecificStickerOverrides.map(\.messageId).map(ElloAppEngine.EngineData.Item.Messages.Message.init))
            )
            ...
            self.disposable = (combineLatest(
                ...
                self.context.engine.data.get(
                    ElloAppEngine.EngineData.Item.Peer.Peer(id: self.context.account.peerId),
                    ElloAppEngine.EngineData.Item.Configuration.PremiumPromo()
                ),
                reactionOverrideMessages,
                stickerOverrideMessages
            )
            |> map { reactions, items, data, reactionOverrideMessages, stickerOverrideMessages -> ([AvailableReactions.Reaction], [ElloAppMediaFile], Bool?, PremiumPromoConfiguration?) in
                var reactionOverrides: [MessageReaction.Reaction: ElloAppMediaFile] = [:]
                for item in accountSpecificReactionOverrides {
                    if let maybeMessage = reactionOverrideMessages[item.messageId], let message = maybeMessage {
                        for media in message.media {
                            if let file = media as? ElloAppMediaFile, file.fileId == item.mediaId {
                ...
                var stickerOverrides: [MessageReaction.Reaction: ElloAppMediaFile] = [:]
