    ...
    final class State: ComponentState {
        ...
        init(context: AccountContext, source: PremiumSource, updateInProgress: @escaping (Bool) -> Void, present: @escaping (ViewController) -> Void, completion: @escaping () -> Void) {
            ...
            if case let .gift(fromPeerId, toPeerId, _) = source {
                ...
                otherPeerName = context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: otherPeerId))
                |> map { peer -> String? in
                ...
                otherPeerName = context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId))
                |> map { peer -> String? in
                    return peer?.displayTitle(strings: presentationData.strings, displayOrder: presentationData.nameDisplayOrder)
                }
            ...
            self.disposable = combineLatest(
                ...
                availableProducts,
                context.engine.data.subscribe(ElloAppEngine.EngineData.Item.Configuration.PremiumPromo()),
