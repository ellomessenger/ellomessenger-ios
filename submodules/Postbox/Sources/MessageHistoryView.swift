...
public final class MessageHistoryView {
    ...
    init(_ mutableView: MutableMessageHistoryView) {
        ...
        case let .loaded(state):
            ...
            // TODO: - research crash
            debugPrint("🔴 assert(Set(entries.map({ $0.message.stableId })).count == entries.count) \(#function)")
            var set = Set<UInt32>()
            entries = entries.filter { set.insert($0.message.stableId).inserted } //Set(entries.map({ $0.message.stableId }))
