...
private let sharedQueue = Queue(name: "org.elloapp.postbox.Postbox")
...
final class PostboxImpl {
    ...
    fileprivate func searchPeers(query: String) -> [RenderedPeer] {
        ...
        var (chatPeerIds, contactPeerIds) = self.peerNameIndexTable.matchingPeerIds(tokens: (regular: stringIndexTokens(query, transliteration: .none), 
                                                                                             transliterated: stringIndexTokens(query, transliteration: .transliterated)),
                                                                                    categories: [.chats, .contacts],
                                                                                    chatListIndexTable: self.chatListIndexTable,
                                                                                    contactTable: self.contactsTable)
...
public class Postbox {
    ...
    public func getPeerWithId(_ id: PeerId) -> Signal<Peer?, NoError> {
        transaction { transaction -> Peer? in
