...
final class AuthorizationSequencePhoneEntryControllerNode: ASDisplayNode {
    ...
    private func refreshQrToken() {
        ...
        |> mapToSignal { activeAccountsAndInfo -> Signal<ExportAuthTransferTokenResult, ExportAuthTransferTokenError> in
            ...
            return ElloAppEngineUnauthorized(account: account).auth.exportAuthTransferToken(accountManager: sharedContext.accountManager, otherAccountUserIds: account.testingEnvironment ? allTestingUserIds : allProductionUserIds, syncContacts: true)
        }
