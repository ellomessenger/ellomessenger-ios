...
private enum LocalizationError {
    enum SignUp: String {
        case userWasRegistered = "this username was been registered"
        case emailWasRegistered = "this email was been registered"
        case otpNotConfirmedError = "email not confirmed for user"
    }
    enum SignIn: String {
        case userNotFound = "No such user"
        case wrongPassword = "Password is incorrect"
        case otpNotConfirmedError = "email not confirmed for user"
    }
    enum SignUpOTP: String {
        case incorrectCode = "incorrect code"
    }
}
...
public final class AuthorizationSequenceController: NavigationController, MFMailComposeViewControllerDelegate, ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    static func navigationBarTheme(_ theme: PresentationTheme) -> NavigationBarTheme {
        return NavigationBarTheme(buttonColor: theme.intro.accentTextColor,
                                  disabledButtonColor: theme.intro.disabledTextColor,
    ...
    private var isWelcomSlideHidden:Bool
    public init(sharedContext: SharedAccountContext,
                ...
                apiId: Int32,
                apiHash: String,
                isWelcomSlideHidden:Bool,
                authorizationCompleted: @escaping () -> Void) {
        ...
        self.isWelcomSlideHidden = isWelcomSlideHidden
        ...
        super.init(mode: .single,
                   theme: NavigationControllerTheme(statusBar: navigationStatusBar,
                                                    navigationBar: AuthorizationSequenceController.navigationBarTheme(presentationData.theme),
                                                    emptyAreaColor: .black),
                   isFlat: true)
        
        self.stateDisposable = (ElloAppEngineUnauthorized(account: self.account).auth.state()
    ...
    private func splashController() -> AuthorizationSequenceSplashController {
        ...
        if let currentController = currentController {
            ...
            controller = AuthorizationSequenceSplashController(accountManager: self.sharedContext.accountManager,
            ...
            controller.nextPressed = { [weak self] strings in
                if let strongSelf = self {
                    ...
                    let _ = ElloAppEngineUnauthorized(account: strongSelf.account).auth.setState(
                        state: UnauthorizedAccountState(isTestingEnvironment: isTestingEnvironment,
                                                        masterDatacenterId: masterDatacenterId,
                                                        contents: .phoneEntry(countryCode: countryCode,
                                                                              number: ""))).start()
                }
            }
            
            controller.onSignIn = { [weak self, weak controller] username, password, completionHandler in
                guard let self else { return }
                
                let accounts = self.otherAccountPhoneNumbers.1.map{ $0.0 }
                if accounts.contains(username) {
                    DispatchQueue.main.async {
                        controller?.present(
                            standardTextAlertController(
                                theme: AlertControllerTheme(presentationData: self.presentationData),
                                title: self.presentationData.strings.SignIn_UserNotFoundError_Title,
                                text: self.presentationData.strings.SignIn_UserExist_Text,
                                actions: [
                                    TextAlertAction(
                                        type: .defaultAction,
                ...
                controller?.onTapResend = { [weak self, weak controller] vc in
                    self?.resendCode(email: username, verifyVC: vc, controller: controller)
                }
                
                self.actionDisposable.set((authorizeNewWith(username: username, password: password, account: self.account, accountManager: self.sharedContext.accountManager)
                                           |> deliverOnMainQueue)
                    .start(next: { result in
                        completionHandler?(true)
                        controller?.verifyRegistrationCode(
                            for: result.email,
                            till: result.confirmation_expire,
                            password: ""
                        )
                        controller?.onTapResend = { [weak self, weak controller] vc in
                            self?.resendCode(email: result.email, verifyVC: vc, controller: controller)
                        }
                        ...
                        Queue.mainQueue().async {[weak self]in
                            ...
                            if case let .generic(message) = error, message.contains("email not confirmed") {
                                _ = (resendSignUpConfirmationRequest(username: username, account: self.account)
                                     |> deliverOnMainQueue)
                                .start(next: { result in
                                    controller?.verifyRegistrationCode(
                                        for: result.email,
...
//                            resendSignUpConfirmationRequest
                            var title: String?
                            var text = "\(error)"
                            if case .generic(let message) = error, message.contains("INTERNAL_TIMEOUT") {
                                text = "Common.Error".localized
                            } else if case .generic(let errorText) = error,
                               let localizedError = LocalizationError.SignIn(
                                rawValue: errorText.replacingOccurrences(of: " \(username)", with: "")
                               ) {
                                switch localizedError {
                                case .userNotFound:
                                    title = self.presentationData.strings.SignIn_UserNotFoundError_Title
                                    text = self.presentationData.strings.SignIn_UserNotFoundError_Text
                                case .wrongPassword:
                                    title = self.presentationData.strings.SignIn_WrongPasswordError_Title
                                    text = self.presentationData.strings.SignIn_WrongPasswordError_Text
                                case .otpNotConfirmedError:
                                    text = self.presentationData.strings.SignIn_OTPNotConfirmedError_Title
                                }
                            }
            ...
            controller.onSignUp = { [weak self, weak controller] object in
                guard let weak = self ,
                      let object = object else {return}
                
                controller?.onTapResend = { [weak self, weak controller] vc in
                    self?.resendCode(email: object.email, verifyVC: vc, controller: controller)
                }
                
                weak.actionDisposable.set(
                    (signUpNewWith (username: object.username,
                                    password: object.password,
                                    email: object.email,
                                    gender: object.gender,
                                    countryCode: object.country ?? "",
                                    dateOfBirth: object.date_of_birth.stringWithFormat(.iso8601_4, isGMT: true),
                                    kind: object.vizibility == .public ? "public" : "private",
                                    type: object.profile == .business ? "business" : "personal",
                                    account: weak.account,
                                    referalCode: object.referalCode)
                     |> deliverOnMainQueue)
                    .start(next:{ result in
                        controller?.verifyRegistrationCode(for: result.email,
                                                           till: result.confirmation_expire,
                                                           password: object.password
                        )
                        ...
                        Queue.mainQueue().async {
                            ...
                            var title: String?
                            var text = "\(error)"
                            print(text.replacingOccurrences(of: " \(object.username)", with: ""))
//                            switch error {
//                            case .limitExceeded:
...
//                            }
                            ...
                            if case .generic(let message) = error, message.contains("INTERNAL_TIMEOUT") {
                               ...
                               let localizedError = LocalizationError.SignUp(
                                rawValue: errorText.replacingOccurrences(of: " \(object.username)", with: "")
                               ) {
                                switch localizedError {
                                    case .emailWasRegistered:
                                        title = self.presentationData.strings.SignUp_EmailRegisteredError_Title
                                        text = self.presentationData.strings.SignUp_EmailRegisteredError_Text
                                    case .userWasRegistered:
                                        title = self.presentationData.strings.SignUp_UserRegisteredError_Title
                                        text = self.presentationData.strings.SignUp_UserRegisteredError_Text
                                    case .otpNotConfirmedError:
                                        text = self.presentationData.strings.SignUp_OTPNotConfirmedError_Title
                                }
                            }
            ...
            let onCheckUsername: EventClosure<ELWelcomeUI.WelcomeController.CheckUsernameParams> = { [weak self] item in
                guard let self else {
                    return
                }
                
                self.actionDisposable.set((checkUsername(username: item.username, network: self.account.network) |> deliverOnMainQueue |> delay(0.5, queue: .concurrentDefaultQueue())).start(next: { result in
                    item.completion(result.boolValue, nil)
                }, error: { error in
                    item.completion(false, error.errorDescription)
                }))
            }
            
            controller.onCheckUsername = onCheckUsername
            let onCheckEmail: EventClosure<ELWelcomeUI.WelcomeController.CheckEmailParams> = { [weak self] item in
                guard let self else {
                    return
                }
                
                self.actionDisposable.set((checkEmailAvailability(email: item.email, network: self.account.network) |> deliverOnMainQueue |> delay(0.5, queue: .concurrentDefaultQueue())).start(next: { result in
                    item.completion(result == .boolTrue)
                }, error: { [weak self, weak controller] error in
//                    print(error)
                    guard let self else {
                        return
                    }
                    controller?.present(standardTextAlertController(theme: AlertControllerTheme(presentationData: self.presentationData), title: self.presentationData.strings.SignUp_EmailRegisteredError_Title, text: error.errorDescription ?? self.presentationData.strings.Login_UnknownError, actions: [TextAlertAction(type: .defaultAction, title: self.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
                }))
            }
            
            controller.onCheckEmail = onCheckEmail
            controller.onCodeToVerify = { [weak self, weak controller] code, email, password, isNeedConfirmEmail in
                debugPrint(email, code ?? "")
                guard let self, let code = code else {return}
                if isNeedConfirmEmail {
                    self.actionDisposable.set(
                        (commonCodeVerification(email: email, code: code, account: self.account)
                         |> deliverOnMainQueue)
                        .start(next: { [weak self] result in
                            guard let self else {
                                return
                            }
                            _ = (signInCodeVerification(username: email, code: code, account: account, accountManager: sharedContext.accountManager)
                             |> deliverOnMainQueue)
                            .start()
                        }, error: { error in
                            Queue.mainQueue().async {
                                if let controller = controller {
                                    var text = "\(error.localizedDescription.capitalizeFirst())"
                                    if case .generic(let errorText) = error,
                                       let localizedError = LocalizationError.SignUpOTP(rawValue: errorText) {
                                        switch localizedError {
                                        case .incorrectCode:
                                            text = self.presentationData.strings.SignUp_OTPCodeIncorrectError_Text
                                        }
                                    }
                                    controller.present(standardTextAlertController(theme: AlertControllerTheme(presentationData: self.presentationData), title: nil, text: text, actions: [TextAlertAction(type: .defaultAction, title: self.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
                                }
                            }
                        })
                    )
                    ...
                    self.actionDisposable.set(
                        (signInCodeVerification(username: email, code: code, account: account, accountManager: sharedContext.accountManager)
                         |> deliverOnMainQueue)
                        .start(next: { result in
                            switch result {
                            case let .signUp(data):
                                debugPrint("should to sugnup with: \(data)")
                            case .loggedIn:
                    ...
                    self.actionDisposable.set(
                        (signUpCodeVerification(email: email, code: code, account: self.account, accountManager: sharedContext.accountManager)
                         |> deliverOnMainQueue)
                        .start(next: { _ in
                        }, error: { error in
                            Queue.mainQueue().async {
            ...
            controller.onResetPassword = { [weak self, weak controller] email, showErrorAlert, result in
                guard let self else {
                    return
                }
                
                self.actionDisposable.set(
                    (resetPassword(with: email, network: self.account.network)
                     |> deliverOnMainQueue)
                    .start(next: { response in
                        debugPrint(response)
                        
                        if let controller = controller {
                            controller.verifyPasswordResetCode(for: response.email, till: response.confirmation_expire)
                            controller.onTapResend = { [weak self, weak controller] vc in
                                self?.resendCode(email: response.email, verifyVC: vc, controller: controller)
                            }
                        ...
                        Queue.mainQueue().async {
                            ...
                            if case let .message(errorText) = error {
                                text = errorText
                            }
            ...
            controller.onResetPasswordCodeToVerify = { [weak self, weak controller] code, email in
                guard let weak = self,
                      let code = code else {return}
                weak.actionDisposable.set(
                    (commonCodeVerification(email: email, code: code, account: weak.account)
                     |> deliverOnMainQueue)
                    .start(next: {
                        if let controller = controller {
                            controller.passwordResetVerified(email: email, code: code) { [weak controller] error in
                                controller?.present(standardTextAlertController(theme: .init(presentationData: weak.presentationData), title: nil, text: "\(error.localizedDescription)", actions: [.init(type: .defaultAction, title: weak.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
                        ...
                        Queue.mainQueue().async {
                            if let controller = controller {
                                var text: String = ""
                                text = "\(error)"
                                controller.present(standardTextAlertController(theme: AlertControllerTheme(presentationData: weak.presentationData), title: nil, text: text, actions: [TextAlertAction(type: .defaultAction, title: weak.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
                            }
                        }
                    })
                )
            ...
            controller.onNewPassword = { [weak self, weak controller] email, code, password in
                guard let self else { return }
                self.actionDisposable.set(
                    (createNewPassword(with: email, code: code, password: password, network: self.account.network)
                     |> deliverOnMainQueue)
                    .start(next: { response in
                        controller?.onSignIn?(email, password, nil)
                    }, error: { error in
                        Queue.mainQueue().async {
        ...
        controller.onOpenMailApp = { [weak self] in
            self?.sharedContext.applicationBindings.openUrl(defineMessageApp())
        }
        return controller
    }
        ...
        if let currentController = currentController {
            ...
            controller = AuthorizationSequencePhoneEntryController(sharedContext: self.sharedContext, account: self.account, isTestingEnvironment: self.account.testingEnvironment, otherAccountPhoneNumbers: self.otherAccountPhoneNumbers, network: self.account.network, presentationData: self.presentationData, openUrl: { [weak self] url in
                ...
                if !strongSelf.otherAccountPhoneNumbers.1.isEmpty {
                    ...
                    let _ = ElloAppEngineUnauthorized(account: strongSelf.account).auth.setState(state: UnauthorizedAccountState(isTestingEnvironment: strongSelf.account.testingEnvironment, masterDatacenterId: strongSelf.account.masterDatacenterId, contents: .empty)).start()
                }
            })
    ...
    private func resendCode(email: String?, verifyVC: CodeVerificationViewController?, controller: ViewController?) {
        actionDisposable.set((resendSignUpConfirmationRequest(username: email ?? "", account: account) |> deliverOnMainQueue)
            .start(next: { [weak verifyVC, weak controller] response in
                verifyVC?.timer = response.confirmation_expire - Int(Date().timeIntervalSince1970)
                controller?.present(standardTextAlertController(theme: .init(presentationData: self.presentationData), title: nil, text: self.presentationData.strings.TwoStepAuth_SetupResendEmailCodeAlert, actions: [.init(type: .defaultAction, title: self.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
    ...
    private func codeEntryController(number: String, email: String?, type: SentAuthorizationCodeType, nextType: AuthorizationCodeNextType?, timeout: Int32?, termsOfService: (UnauthorizedAccountTermsOfService, Bool)?) -> AuthorizationSequenceCodeEntryController {
        ...
        if let currentController = currentController {
            ...
            controller = AuthorizationSequenceCodeEntryController(presentationData: self.presentationData, openUrl: { [weak self] url in
                ...
                let _ = ElloAppEngineUnauthorized(account: strongSelf.account).auth.setState(state: UnauthorizedAccountState(isTestingEnvironment: strongSelf.account.testingEnvironment, masterDatacenterId: strongSelf.account.masterDatacenterId, contents: .phoneEntry(countryCode: countryCode, number: ""))).start()
            })
                                    ...
                                    if case .invalidCode = error {
                                        ...
                                        switch error {
                                            ...
                                            case .codeExpired:
                                                ...
                                                let _ = ElloAppEngineUnauthorized(account: strongSelf.account).auth.setState(state: UnauthorizedAccountState(isTestingEnvironment: account.testingEnvironment, masterDatacenterId: account.masterDatacenterId, contents: .empty)).start()
                                            case .timeout:
                                                text = strongSelf.presentationData.strings.Login_NetworkError
                                ...
                                case let .signUp(data):
                                    if let (termsOfService, explicit) = termsOfService, explicit {
                                        ...
                                        let presentAlertImpl: () -> Void = {
                                            ...
                                            contentNode.textAttributeAction = (NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL), { value in
    ...
    private func passwordEntryController(hint: String, suggestReset: Bool, syncContacts: Bool) -> AuthorizationSequencePasswordEntryController {
        ...
        controller.forgot = { [weak self, weak controller] in
            if let strongSelf = self, let strongController = controller {
                ...
                strongSelf.actionDisposable.set((ElloAppEngineUnauthorized(account: strongSelf.account).auth.requestTwoStepVerificationPasswordRecoveryCode()
                |> deliverOnMainQueue).start(next: { pattern in
                    if let strongSelf = self, let strongController = controller {
                        strongController.inProgress = false

                        let _ = (ElloAppEngineUnauthorized(account: strongSelf.account).auth.state()
                        |> take(1)
                        |> deliverOnMainQueue).start(next: { state in
                            ...
                            if case let .unauthorized(state) = state, case let .passwordEntry(hint, number, code, _, syncContacts) = state.contents {
                                let _ = ElloAppEngineUnauthorized(account: strongSelf.account).auth.setState(state: UnauthorizedAccountState(isTestingEnvironment: strongSelf.account.testingEnvironment, masterDatacenterId: strongSelf.account.masterDatacenterId, contents: .passwordRecovery(hint: hint, number: number, code: code, emailPattern: pattern, syncContacts: syncContacts))).start()
                            }
                        })
                    }
                }, error: { error in
        ...
        controller.passwordRecoveryFailed = { [weak self] in
            ...
            |> deliverOnMainQueue).start(next: { state in
                ...
                if case let .unauthorized(state) = state, case let .passwordRecovery(hint, number, code, _, syncContacts) = state.contents {
                    let _ = ElloAppEngineUnauthorized(account: strongSelf.account).auth.setState(state: UnauthorizedAccountState(isTestingEnvironment: strongSelf.account.testingEnvironment, masterDatacenterId: strongSelf.account.masterDatacenterId, contents: .passwordEntry(hint: hint, number: number, code: code, suggestReset: true, syncContacts: syncContacts))).start()
                }
            })
        }
        return controller
            ...
            controller.signUpWithName = { [weak self, weak controller] firstName, lastName, avatarData, avatarAsset, avatarAdjustments in
                if let strongSelf = self {
                    ...
                    if let avatarAsset = avatarAsset as? AVAsset {
                        let account = strongSelf.account
                        avatarVideo = Signal<ElloAppMediaResource?, NoError> { subscriber in
                        ...
                        |> mapToSignal { resource -> Signal<UploadedPeerPhotoData?, NoError> in
                            if let resource = resource {
                                return ElloAppEngineUnauthorized(account: account).auth.uploadedPeerVideo(resource: resource) |> map(Optional.init)
                            } else {
                                return .single(nil)
                            }
                        }
            ...
            switch state {
                case .empty:
                    if let _ = self.viewControllers.last as? AuthorizationSequenceSplashController {
                        ...
                        let controllers: [ViewController] = [self.splashController()]
                        self.setViewControllers(controllers, animated: !self.viewControllers.isEmpty)
                        
                        if isWelcomSlideHidden {
                            WelcomeController.showLoginRegister(isHideBack: false)
                        }
// remove to add Onboarding
                        else {
                            WelcomeController.showLoginRegister(isHideBack: true)
                        }
                        
                        WelcomeController.onBack = {
                            if let currentAccount = self.otherAccountPhoneNumbers.0 {
                                let transaction = self.sharedContext.accountManager.transaction({ transaction -> Void in
                                    let _ = transaction.removeAuth()
                                }).start()
                                self.sharedContext.switchToAccount(id: currentAccount.1,
                ...
                case let .signUp(_, _, firstName, lastName, termsOfService, _):
                    ...
                    controllers.append(self.signUpController(firstName: firstName, 
                                                             lastName: lastName,
