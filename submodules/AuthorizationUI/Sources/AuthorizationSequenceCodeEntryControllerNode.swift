...
//            (self.signInWithAppleButton as? ASAuthorizationAppleIDButton)?.cornerRadius = 11
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        if let codeType = self.codeType {
            ...
            default:
                self.titleNode.attributedText = NSAttributedString(string: self.strings.Login_EnterCodeElloAppTitle, font: Font.semibold(28.0), textColor: self.theme.list.itemPrimaryTextColor)
            }
        } else {
            self.titleNode.attributedText = NSAttributedString(string: self.strings.Login_EnterCodeElloAppTitle, font: Font.semibold(40.0), textColor: self.theme.list.itemPrimaryTextColor)
        }
