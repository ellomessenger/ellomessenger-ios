...
private enum DebugAccountsControllerEntry: ItemListNodeEntry {
    case record(PresentationTheme, AccountRecord<ElloAppAccountManagerTypes.Attribute>, Bool)
...
private func debugAccountsControllerEntries(view: AccountRecordsView<ElloAppAccountManagerTypes>, presentationData: PresentationData) -> [DebugAccountsControllerEntry] {
...
public func debugAccountsController(context: AccountContext, accountManager: AccountManager<ElloAppAccountManagerTypes>) -> ViewController {
            ...
            ActionSheetItemGroup(items: [
                ActionSheetButtonItem(title: "Production", color: .accent, action: {
                    ...
                    if case .internal = context.sharedContext.applicationBindings.appBuildType {
                        context.sharedContext.beginNewAuth(testingEnvironment: false, isWelcomHidden:nil)
                    }
                }),
                ActionSheetButtonItem(title: "Test", color: .accent, action: {
                    dismissAction()
                    context.sharedContext.beginNewAuth(testingEnvironment: true, isWelcomHidden:nil)
                })
            ]),
