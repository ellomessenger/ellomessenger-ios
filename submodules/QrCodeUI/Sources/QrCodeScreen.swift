...
public func shareQrCode(context: AccountContext, link: String, ecl: String, view: UIView) {
    let _ = (qrCode(string: link, 
                    color: .black,
                    backgroundColor: .white,
                    icon: .custom(UIImage(bundleImageName: "Chat/Links/QrLogo1")),
                    ecl: ecl)
...
public func shareUserLink(context: AccountContext, link: String, view: UIView) {
...
public final class QrCodeScreen: ViewController {
    public enum Subject {
        ...
        var link: String {
            switch self {
                case let .peer(peer):
                    return "https://ello.team/\(peer.addressName ?? "")"
