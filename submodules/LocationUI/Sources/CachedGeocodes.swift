...
private func cachedGeocode(engine: ElloAppEngine, address: DeviceContactAddressData) -> Signal<CachedGeocode?, NoError> {
    let key = ValueBoxKey(length: 8)
    key.setInt64(0, value: Int64(bitPattern: address.string.persistentHashValue))
    
    return engine.data.get(ElloAppEngine.EngineData.Item.ItemCache.Item(collectionId: ApplicationSpecificItemCacheCollectionId.cachedGeocodes, id: key))
    |> map { entry -> CachedGeocode? in
        return entry?.get(CachedGeocode.self)
    }
}

private func updateCachedGeocode(engine: ElloAppEngine, address: DeviceContactAddressData, latitude: Double, longitude: Double) -> Signal<(Double, Double), NoError> {
    let key = ValueBoxKey(length: 8)
    key.setInt64(0, value: Int64(bitPattern: address.string.persistentHashValue))
...
public func geocodeAddress(engine: ElloAppEngine, address: DeviceContactAddressData) -> Signal<(Double, Double)?, NoError> {
