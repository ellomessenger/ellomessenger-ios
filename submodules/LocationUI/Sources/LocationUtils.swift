...
public func nearbyVenues(context: AccountContext, latitude: Double, longitude: Double, query: String? = nil) -> Signal<[ElloAppMediaMap], NoError> {
    return context.engine.data.get(ElloAppEngine.EngineData.Item.Configuration.SearchBots())
    |> mapToSignal { searchBotsConfiguration in
        ...
        |> map { contextResult -> [ElloAppMediaMap] in
            guard let contextResult = contextResult else {
                return []
            }
            var list: [ElloAppMediaMap] = []
