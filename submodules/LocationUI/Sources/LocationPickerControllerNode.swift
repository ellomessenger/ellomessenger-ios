...
private enum LocationPickerEntry: Comparable, Identifiable {
    case location(PresentationTheme, String, String, ElloAppMediaMap?, CLLocationCoordinate2D?)
    case liveLocation(PresentationTheme, String, String, CLLocationCoordinate2D?)
    case header(PresentationTheme, String)
    case venue(PresentationTheme, ElloAppMediaMap?, Int)
    ...
    func item(engine: ElloAppEngine, presentationData: PresentationData, interaction: LocationPickerInteraction?) -> ListViewItem {
        switch self {
...
private func preparedTransition(from fromEntries: [LocationPickerEntry], to toEntries: [LocationPickerEntry], isLoading: Bool, isEmpty: Bool, crossFade: Bool, engine: ElloAppEngine, presentationData: PresentationData, interaction: LocationPickerInteraction?) -> LocationPickerTransaction {
...
//            if let user = peerViewMainPeer(view) as? ElloAppUser, let userName = user.username {
...
//                    var stableId: String? = basicData.id
...
//        }
        ...
        let personalVenues: Signal<[ElloAppMediaMap]?, NoError> = .single(nil)
//        |> then(
//            personalAddresses
//            |> mapToSignal { homeAndWorkAddresses -> Signal<[ElloAppMediaMap]?, NoError> in
...
//                    |> map { homeCoordinate, workCoordinate -> [ElloAppMediaMap]? in
//                        var venues: [ElloAppMediaMap] = []
//                        if let (latitude, longitude) = homeCoordinate, let address = homeAddress {
//                            venues.append(ElloAppMediaMap(latitude: latitude, longitude: longitude, heading: nil, accuracyRadius: nil, geoPlace: nil, venue: MapVenue(title: presentationData.strings.Map_Home, address: address.displayString, provider: nil, id: "home", type: "home"), liveBroadcastingTimeout: nil, liveProximityNotificationRadius: nil))
//                        }
//                        if let (latitude, longitude) = workCoordinate, let address = workAddress {
//                            venues.append(ElloAppMediaMap(latitude: latitude, longitude: longitude, heading: nil, accuracyRadius: nil, geoPlace: nil, venue: MapVenue(title: presentationData.strings.Map_Work, address: address.displayString, provider: nil, id: "work", type: "work"), liveBroadcastingTimeout: nil, liveProximityNotificationRadius: nil))
//                        }
//                        return venues
//                    }
//                } else {
...
//        )
        ...
        let venues: Signal<[ElloAppMediaMap]?, NoError> = .single(nil)
        |> then(
            throttledUserLocation(userLocation)
            |> mapToSignal { location -> Signal<[ElloAppMediaMap]?, NoError> in
                if let location = location, location.horizontalAccuracy > 0 {
                    ...
                    |> map { nearbyVenues, personalVenues -> [ElloAppMediaMap]? in
                        var resultVenues: [ElloAppMediaMap] = []
        ...
        let foundVenues: Signal<([ElloAppMediaMap], CLLocation)?, NoError> = .single(nil)
        |> then(
            self.searchVenuesPromise.get()
            |> distinctUntilChanged
            |> mapToSignal { coordinate -> Signal<([ElloAppMediaMap], CLLocation)?, NoError> in
                if let coordinate = coordinate {
                    return (.single(nil)
                    |> then(
                        nearbyVenues(context: context, latitude: coordinate.latitude, longitude: coordinate.longitude)
                        |> map { venues -> ([ElloAppMediaMap], CLLocation)? in
