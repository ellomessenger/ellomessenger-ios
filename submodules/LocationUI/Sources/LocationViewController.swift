...
public class LocationViewParams {
    let sendLiveLocation: (ElloAppMediaMap) -> Void
    let stopLiveLocation: (MessageId?) -> Void
    let openUrl: (String) -> Void
    let openPeer: (Peer) -> Void
    let showAll: Bool
        
    public init(sendLiveLocation: @escaping (ElloAppMediaMap) -> Void, stopLiveLocation: @escaping (MessageId?) -> Void, openUrl: @escaping (String) -> Void, openPeer: @escaping (Peer) -> Void, showAll: Bool = false) {
...
class LocationViewInteraction {
    ...
    let requestDirections: (ElloAppMediaMap, String?, OpenInLocationDirections) -> Void
    let share: () -> Void
            ...
            DeviceAccess.authorizeAccess(to: .location(.live), locationManager: strongSelf.locationManager, presentationData: strongSelf.presentationData, present: { c, a in
                ...
                if let distance = distance {
                    ...
                    |> deliverOnMainQueue).start(next: { coordinate in
                        params.sendLiveLocation(ElloAppMediaMap(coordinate: coordinate, liveBroadcastingTimeout: 30 * 60, proximityNotificationRadius: distance))
                    })
                    ...
                    |> deliverOnMainQueue).start(next: { peer in
                        ...
                        let sendLiveLocationImpl: (Int32) -> Void = { [weak controller] period in
                            ...
                            let _ = (strongSelf.controllerNode.coordinate
                            |> deliverOnMainQueue).start(next: { coordinate in
                                params.sendLiveLocation(ElloAppMediaMap(coordinate: coordinate, liveBroadcastingTimeout: period))
                            })
