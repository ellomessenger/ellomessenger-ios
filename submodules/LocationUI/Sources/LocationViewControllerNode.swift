...
func getLocation(from message: Message) -> ElloAppMediaMap? {
    return message.media.first(where: { $0 is ElloAppMediaMap } ) as? ElloAppMediaMap
}
...
private enum LocationViewEntry: Comparable, Identifiable {
    case info(PresentationTheme, ElloAppMediaMap, String?, Double?, ExpectedTravelTime, ExpectedTravelTime, ExpectedTravelTime)
...
final class LocationViewControllerNode: ViewControllerTracingNode, CLLocationManagerDelegate {
    ...
    init(context: AccountContext, presentationData: PresentationData, subject: Message, interaction: LocationViewInteraction, locationManager: LocationManager) {
        ...
        let selfPeer = context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: context.account.peerId))
        ...
        |> deliverOnMainQueue).start(next: { [weak self] presentationData, state, selfPeer, liveLocations, userLocation, distance, address, eta, travelTimes in
            if let strongSelf = self, let location = getLocation(from: subject) {
                ...
                if location.liveBroadcastingTimeout == nil {
                    ...
                    if let channel = subject.author as? ElloAppChannel, case .broadcast = channel.info, activeOwnLiveLocation == nil {
                    } else {
                ...
                for message in effectiveLiveLocations {
                    if let location = getLocation(from: message) {
                        if let channel = message.peers[message.id.peerId] as? ElloAppChannel, case .broadcast = channel.info, message.threadId != nil {
                            continue
                        }
                ...
                if let channel = subject.author as? ElloAppChannel, case .broadcast = channel.info {
