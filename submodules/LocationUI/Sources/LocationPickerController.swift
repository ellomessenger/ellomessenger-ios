...
class LocationPickerInteraction {
    ...
    let sendVenue: (ElloAppMediaMap) -> Void
...
public final class LocationPickerController: ViewController, AttachmentContainable {
    ...
    private let completion: (ElloAppMediaMap, String?) -> Void
    ...
    public init(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, mode: LocationPickerMode, completion: @escaping (ElloAppMediaMap, String?) -> Void) {
        ...
        let locationWithTimeout: (CLLocationCoordinate2D, Int32?) -> ElloAppMediaMap = { coordinate, timeout in
            return ElloAppMediaMap(latitude: coordinate.latitude, longitude: coordinate.longitude, heading: nil, accuracyRadius: nil, geoPlace: nil, venue: nil, liveBroadcastingTimeout: timeout, liveProximityNotificationRadius: nil)
        }
        ...
        self.interaction = LocationPickerInteraction(sendLocation: { [weak self] coordinate in
            ...
            DeviceAccess.authorizeAccess(to: .location(.live), locationManager: strongSelf.locationManager, presentationData: strongSelf.presentationData, present: { c, a in
                ...
                if case let .share(peer, _, _) = strongSelf.mode, let receiver = peer as? ElloAppUser {
                    ...
                    ActionSheetItemGroup(items: [
                        ...
                        ActionSheetButtonItem(title: strongSelf.presentationData.strings.Map_LiveLocationFor15Minutes, color: .accent, action: { [weak self, weak controller] in
                            ...
                            if let strongSelf = self {
                                strongSelf.completion(ElloAppMediaMap(coordinate: coordinate, liveBroadcastingTimeout: 15 * 60), nil)
                                strongSelf.dismiss()
                            }
                        }),
                        ActionSheetButtonItem(title: strongSelf.presentationData.strings.Map_LiveLocationFor1Hour, color: .accent, action: { [weak self, weak controller] in
                            controller?.dismissAnimated()
                            if let strongSelf = self {
                                strongSelf.completion(ElloAppMediaMap(coordinate: coordinate, liveBroadcastingTimeout: 60 * 60 - 1), nil)
                                strongSelf.dismiss()
                            }
                        }),
                        ActionSheetButtonItem(title: strongSelf.presentationData.strings.Map_LiveLocationFor8Hours, color: .accent, action: { [weak self, weak controller] in
                            controller?.dismissAnimated()
                            if let strongSelf = self {
                                strongSelf.completion(ElloAppMediaMap(coordinate: coordinate, liveBroadcastingTimeout: 8 * 60 * 60), nil)
                                strongSelf.dismiss()
                            }
                        })
                    ]),
            ...
            if ["home", "work"].contains(venueType) {
                completion(ElloAppMediaMap(latitude: venue.latitude, longitude: venue.longitude, heading: nil, accuracyRadius: nil, geoPlace: nil, venue: nil, liveBroadcastingTimeout: nil, liveProximityNotificationRadius: nil), nil)
            } else {
