...
public enum ItemListAvatarAndNameInfoItemName: Equatable {
    ...
    case title(title: String, description: String, type: ItemListAvatarAndNameInfoItemTitleType)
    ...
    public init(_ peer: EnginePeer) {
        ...
        case let .title(title, description, _):
            ...
            self = .title(title: title, description: description, type: type)
        }
    }
    ...
    public var composedDesription: String? {
        switch self {
        case let .title(_, description, _):
            return description
        default:
            return nil
        }
...
public final class ItemListAvatarAndNameInfoItemContext {
    public var hiddenAvatarRepresentation: ElloAppMediaImageRepresentation?
    
    public init(hiddenAvatarRepresentation: ElloAppMediaImageRepresentation? = nil) {
...
public enum ItemListAvatarAndNameInfoItemUpdatingAvatar: Equatable {
    case image(ElloAppMediaImageRepresentation, Bool)
...
extension ItemListAvatarAndNameInfoItemMode {
    public var isUpperAvatar:Bool { self == .upperAvatar}
}
...
public class ItemListAvatarAndNameInfoItem: ListViewItem, ItemListItem {
    ...
    public init(accountContext: AccountContext,
                ...
                editingNameUpdated: @escaping (ItemListAvatarAndNameInfoItemName) -> Void,
                editingNameCompleted: @escaping () -> Void = {},
                avatarTapped: @escaping () -> Void,
                context: ItemListAvatarAndNameInfoItemContext? = nil,
                updatingImage: ItemListAvatarAndNameInfoItemUpdatingAvatar? = nil,
                call: (() -> Void)? = nil,
                action: (() -> Void)? = nil,
    ...
    public func updateNode(async: @escaping (@escaping () -> Void) -> Void,
...
public class ItemListAvatarAndNameInfoItemNode: ListViewItemNode, ItemListItemNode, ItemListItemFocusableNode, UITextFieldDelegate {
    ...
    public func asyncLayout() -> (_ item: ItemListAvatarAndNameInfoItem, _ params: ListViewItemLayoutParams, _ neighbors: ItemListNeighbors) -> (ListViewItemNodeLayout, (Bool, Bool) -> Void) {
        ...
        return { item, params, neighbors in
            ...
            if let updatingName = item.state.updatingName {
                ...
                displayTitle = .title(title: "", description: "", type: .group)
            }
            ...
            let (nameNodeLayout, nameNodeApply) = layoutNameNode(TextNodeLayoutArguments(
                attributedString: NSAttributedString(string: displayTitle.composedDisplayTitle(strings: item.presentationData.strings),
                                                     font: nameFont,
                                                     textColor: item.presentationData.theme.list.itemPrimaryTextColor),
                backgroundColor: nil,
                maximumNumberOfLines: nameMaximumNumberOfLines,
                truncationType: .end,
                constrainedSize: CGSize(width: baseWidth - 20 - 94.0 - (item.call != nil ? 36.0 : 0.0) - additionalTitleInset,
            ...
            if case let .user(peer) = item.peer {
                ...
                case .generic, .contact, .editSettings, .upperAvatar:
            ...
            let (statusNodeLayout, statusNodeApply) = layoutStatusNode(TextNodeLayoutArguments(attributedString: NSAttributedString(string: statusText, 
                                                                                                                                    font: statusFont, 
                                                                                                                                    textColor: statusColor),
                                                                                               backgroundColor: nil, 
                                                                                               maximumNumberOfLines: 3,
                                                                                               truncationType: .end,
                                                                                               constrainedSize: CGSize(width: availableStatusWidth,
                                                                                                                       height: CGFloat.greatestFiniteMagnitude),
                                                                                               alignment: .natural,
                                                                                               cutout: nil,
                                                                                               insets: UIEdgeInsets()))
            ...
            case .plain:
                ...
                contentSize = item.mode.isUpperAvatar ? CGSize(width: params.width, height: max(baseHeight, verticalInset + 198.0)) : CGSize(width: params.width, height: max(baseHeight, verticalInset * 2.0 + 66.0))
            ...
            case let .blocks(withTopInset, withExtendedBottomInset):
                ...
                if withTopInset || hasCorners {
                    insets = item.mode.isUpperAvatar ? UIEdgeInsets.zero : itemListNeighborsGroupedInsets(neighbors, params)
                } else {
                ...
                if let strongSelf = self {
                    ...
                    switch item.style {
                        ...
                        case .blocks:
                            ...
                            strongSelf.backgroundNode.frame = item.mode.isUpperAvatar ?
                            CGRect(origin: CGPoint(x: 0.0, y: 110.0), size: CGSize(width: params.width, height: 88.0)) :
                            CGRect(origin: CGPoint(x: 0.0, y: 0.0), size: CGSize(width: params.width, height: contentSize.height))
                    ...
                    if let peer = item.peer {
                        ...
                        if let updatingImage = item.updatingImage {
                            ...
                            case let .imageWithEditIcon(representation):
                                overrideImage = .imageWithEditIcon(representation)
                            }
                        ...
                        strongSelf.avatarNode.setPeer(context: item.accountContext,
                                                      theme: item.presentationData.theme,
                                                      peer: peer,
                                                      overrideImage: overrideImage,
                                                      emptyColor: ignoreEmpty ? nil : item.presentationData.theme.list.mediaPlaceholderColor,
                                                      clipStyle:.corner(item.mode.isUpperAvatar ? 40.0 : 27.0),
                                                      synchronousLoad: synchronousLoads)
                    }
                    
                    let avatarFrame = item.mode.isUpperAvatar ?
                        CGRect(origin: CGPoint(x: floor((layout.contentSize.width - 90.0) / 2.0), y: 0),
                               size: CGSize(width: 90.0, height: 90.0))
                     :
                        CGRect(origin: CGPoint(x: params.leftInset + 15.0, y: floor((layout.contentSize.height - 66.0) / 2.0)),
                               size: CGSize(width: 66.0, height: 66.0))
                    ...
                    let nameFrame = CGRect(origin: CGPoint(x: item.mode.isUpperAvatar ? (layout.contentSize.width - nameNodeLayout.size.width) / 2.0 : params.leftInset + 94.0, y: nameY),
                                           size: nameNodeLayout.size)
                    ...
                    if let editingName = item.state.editingName {
                        ...
                        switch editingName {
                            case let .personName(firstName, lastName, _):
                                ...
                                if item.mode == .upperAvatar {
                                    strongSelf.inputFirstField?.frame = CGRect(origin: CGPoint(x: params.leftInset + 20.0, y: 110.0), size: CGSize(width: params.width - params.leftInset - params.rightInset - 20.0 - 36.0, height: 40.0))
                                    strongSelf.inputSeparator?.frame = CGRect(origin: CGPoint(x: params.leftInset + 20.0, y: 154.0), size: CGSize(width: params.width - params.leftInset - 20.0, height: separatorHeight))
                                    strongSelf.inputSecondField?.frame = CGRect(origin: CGPoint(x: params.leftInset + 20.0, y: 154.0), size: CGSize(width: params.width - params.leftInset - params.rightInset - 20.0 - 36.0, height: 40.0))
                                }
                                else {
                            ...
                            case let .title(title, _, type):
                                ...
                                switch type {
                                case .group:
                                    placeholderTitle = item.presentationData.strings.GroupInfo_GroupNamePlaceholder
                                case .channel:
                                    placeholderTitle = item.presentationData.strings.GroupInfo_ChannelListNamePlaceholder
                                case .onlineCourse:
                                    placeholderTitle = item.presentationData.strings.GroupInfo_OnlineCourseListNamePlaceholder
                                }
                            
                                strongSelf.inputFirstField?.attributedPlaceholder = NSAttributedString(string: placeholderTitle, font: Font.regular(floor(item.presentationData.fontSize.itemListBaseFontSize * 19.0 / 17.0)), textColor: item.presentationData.theme.list.itemPlaceholderTextColor)
                                if strongSelf.inputFirstField?.keyboardAppearance != keyboardAppearance {
                                    strongSelf.inputFirstField?.keyboardAppearance = keyboardAppearance
                                }
                                ...
                                strongSelf.inputSecondField?.attributedPlaceholder = NSAttributedString(string: placeholderDescription, font: Font.regular(floor(item.presentationData.fontSize.itemListBaseFontSize * 19.0 / 17.0)), textColor: item.presentationData.theme.list.itemPlaceholderTextColor)
                                if strongSelf.inputSecondField?.keyboardAppearance != keyboardAppearance {
                                    strongSelf.inputSecondField?.keyboardAppearance = keyboardAppearance
                                }
    ...
    @objc private func textFieldDidChange(_ inputField: UITextField) {
        if let item = self.item, let currentEditingName = item.state.editingName {
            ...
            if let inputFirstField = self.inputFirstField, let inputSecondField = self.inputSecondField, case .personName = item.peer?.indexName {
                editingName = .personName(firstName: inputFirstField.text ?? "", lastName: inputSecondField.text ?? "", phone: "")
            } else if let inputFirstField = self.inputFirstField, let inputSecondField = self.inputSecondField {
                if case let .title(_, _, type) = currentEditingName {
                    editingName = .title(title: inputFirstField.text ?? "", description: inputSecondField.text ?? "", type: type)
                }
            } else if let inputFirstField = self.inputFirstField {
                if case let .title(_, _, type) = currentEditingName {
                    editingName = .title(title: inputFirstField.text ?? "", description: "", type: type)
                }
            }
