...
private final class LiveLocationSummaryContext {
            ...
            if self.messageIds != oldValue {
                if self.messageIds.isEmpty {
                    ...
                    self.disposable.set((self.engine.data.subscribe(
                        ElloAppEngine.EngineData.Item.Messages.Messages(ids: self.messageIds)
                    )
    ...
    init(queue: Queue, engine: ElloAppEngine) {
        self.queue = queue
        self.engine = engine
    }
...
private final class LiveLocationPeerSummaryContext {
    ...
    init(queue: Queue, engine: ElloAppEngine, accountPeerId: EnginePeer.Id, peerId: EnginePeer.Id, becameEmpty: @escaping () -> Void) {
        self.queue = queue
        self.engine = engine
...
public final class LiveLocationSummaryManagerImpl: LiveLocationSummaryManager {
    ...
    init(queue: Queue, engine: ElloAppEngine, accountPeerId: EnginePeer.Id) {
