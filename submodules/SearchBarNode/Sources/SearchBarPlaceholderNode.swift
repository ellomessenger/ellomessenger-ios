...
//                    var totalWidth = labelLayoutResult.size.width
                    let iconLeading: CGFloat = 16.0
                    let spacing: CGFloat = 8.0
                    
                    if let iconImage = strongSelf.iconNode.image {
                        iconSize = iconImage.size
//                        totalWidth += iconSize.width + iconLeading
                        transition.updateFrame(
                            node: strongSelf.iconNode,
                            frame: CGRect(
                                origin: CGPoint(x: iconLeading, y: floorToScreenPixels((height - iconSize.height) / 2.0)),
                                size: iconSize
                            )
                        )
                    }
                    ...
                    let labelFrame = CGRect(origin: CGPoint(x: iconLeading + iconSize.width + spacing, y: floorToScreenPixels((height - labelLayoutResult.size.height) / 2.0) + textOffset), size: labelLayoutResult.size)
