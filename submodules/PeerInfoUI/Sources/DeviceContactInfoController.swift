...
private enum DeviceContactInfoEntry: ItemListNodeEntry {
    ...
    case userName(Int, Int, PresentationTheme, String, String, String, Bool?)
    ...
    var stableId: DeviceContactInfoEntryId {
        switch self {
            ...
            case let .userName(_, catIndex, _, _, _, _, _):
                return .userName(catIndex)
    ...
    static func ==(lhs: DeviceContactInfoEntry, rhs: DeviceContactInfoEntry) -> Bool {
        switch lhs {
            ...
            case let .userName(lhsIndex, lhsCatIndex, lhsTheme, lhsTitle, lhsLabel, lhsValue, lhsSelected):
                if case let .userName(rhsIndex, rhsCatIndex, rhsTheme, rhsTitle, rhsLabel, rhsValue, rhsSelected) = rhs, lhsCatIndex == rhsCatIndex, lhsIndex == rhsIndex, lhsTheme === rhsTheme, lhsTitle == rhsTitle, lhsLabel == rhsLabel, lhsValue == rhsValue, lhsSelected == rhsSelected {
                    return true
                } else {
                    return false
                }
    ...
    private var sortIndex: Int {
        switch self {
            ...
            case let .note(index, _, _, _, _):
                return index
            case let .userName(index, _, _, _, _, _, _):
                return index
        }
    }
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        switch self {
            case let .info(_, _, _, dateTimeFormat, peer, state, jobSummary, _):
                return ItemListAvatarAndNameInfoItem(accountContext: arguments.context,
                                                     ...
                                                     style: arguments.isPlain ? .plain : .blocks(withTopInset: false,
                                                                                                 withExtendedBottomInset: true),
            ...
            case let .userName(_, _, _, _, _, value, _):
                return ItemListDisclosureItem(presentationData: presentationData,
                                              icon: UIImage(bundleImageName: "Mail"),
                                              title: value,
                                              titleLeftInset: 62.0,
                                              label: "",
                                              sectionId: self.section,
            ...
            case let .url(_, index, _, title, label, value, selected):
                return ItemListTextWithLabelItem(presentationData: presentationData, 
                                                 label: title,
                                                 text: value,
                                                 style: arguments.isPlain ? .plain : .blocks,
                                                 textColor: .accent,
                                                 enabledEntityTypes: [],
                                                 multiline: false,
                                                 selected: selected,
...
private func filteredContactData(contactData: DeviceContactExtendedData, excludedComponents: Set<DeviceContactInfoDataId>) -> DeviceContactExtendedData {
    let userNames = contactData.basicData.userNames.filter({ userName in
        return !excludedComponents.contains(.userName(userName.label, userName.value))
    })
...
private func deviceContactInfoEntries(account: Account, 
                                      ...
                                      isContact: Bool,
                                      state: DeviceContactInfoState,
                                      selecting: Bool,
                                      editingPhoneNumbers: Bool,
                                      isUrlsHidden:Bool = false) -> [DeviceContactInfoEntry] {
    ...
    entries.append(.info(entries.count,
                         presentationData.theme,
                         presentationData.strings,
                         presentationData.dateTimeFormat,
                         peer: peer ?? ElloAppUser(id: PeerId(namespace: .max, id: PeerId.Id._internalFromInt64Value(0)),
                                                   accessHash: nil,
                                                   firstName: firstName,
                                                   lastName: isOrganization ? nil : personName.1,
                                                   username: contactData.basicData.userNames.first?.value ?? "",
                                                   phone: nil,
                                                   photo: [],
                                                   botInfo: nil,
                                                   email: nil,
                                                   ...
                                                   isPublic: false,
                                                   botDescription: nil),
                         state: ItemListAvatarAndNameInfoItemState(editingName: editingName, updatingName: nil),
                         job: isOrganization ? nil : jobSummary,
                         isPlain: !isShare))
    ...
    if !selecting {
        if let _ = peer {
            ...
            entries.append(.invite(entries.count, presentationData.theme, presentationData.strings.Contacts_InviteToElloApp))
        }
...
//    }
    ...
    var userNameIndex = 0
    for userName in contactData.basicData.userNames {
        entries.append(.userName(entries.count, userNameIndex, presentationData.theme, localizedGenericContactFieldLabel(label: userName.label, strings: presentationData.strings),
                                 userName.label,
                                 userName.value,
                                 selecting ? !state.excludedComponents.contains(.userName(userName.label, userName.value)) : nil))
        userNameIndex += 1
    }
...
public func deviceContactInfoController(context: AccountContext,
                                        updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil,
                                        subject: DeviceContactInfoSubject,
                                        isTitleHidden:Bool = false,
                                        completed: (() -> Void)?,
                                        cancelled: (() -> Void)?) -> ViewController {
    var initialState = DeviceContactInfoState()
    
    func editingState(peer: Peer?, contactData: DeviceContactExtendedData) -> DeviceContactInfoEditingState {
        ...
        if let peer = peer as? ElloAppUser {
            ...
            if let username = peer.username {
                ...
                initialState.phoneNumbers.append(EditingPhoneNumber(id: initialState.nextPhoneNumber, label: "_$!<Mobile>!$_", value: username))
                initialState.nextPhoneNumber += 1
            }
        }
        
        for userName in contactData.basicData.userNames {
            if peerUserName != userName.value {
                initialState.phoneNumbers.append(EditingPhoneNumber(id: initialState.nextPhoneNumber, label: userName.label, value: userName.value))
                initialState.nextPhoneNumber += 1
            }
        }
        
        return DeviceContactInfoEditingState(editingName: .personName(firstName: firstName, lastName: lastName, phone: ""))
    }
    
    switch subject {
    case let .create(peer, contactData, _, _, _), let .filter(peer, _, contactData, _):
        initialState.editingState = editingState(peer: peer, contactData: contactData)
    default:
        break
    }
    ...
    let callImpl: (String) -> Void = { number in
        let user: Signal<ElloAppUser?, NoError>
        if let peer = subject.peer {
            user = context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peer.id))
            |> map { peer -> ElloAppUser? in
            ...
            if let user = user, let phone = user.phone, formatPhoneNumber(phone) == formatPhoneNumber(number) {
                ...
                controller.setItemGroups([
                    ActionSheetItemGroup(items: [
                        ActionSheetButtonItem(title: presentationData.strings.UserInfo_ElloAppCall, action: {
                            dismissAction()
    ...
    var isPlane = !isShare
    if case .filter = subject { isPlane = false}
    let arguments = DeviceContactInfoControllerArguments(context: context, isPlain: isPlane, updateEditingName: { editingName in
        ...
        switch action {
            case .invite:
                ...
                if subject.contactData.basicData.userNames.count == 1 {
                    inviteAction(subject.contactData.basicData.userNames[0].value)
                } else {
                    let presentationData = context.sharedContext.currentPresentationData.with { $0 }
                    let controller = ActionSheetController(presentationData: presentationData)
                    let dismissAction: () -> Void = { [weak controller] in
                    ...
                    for userName in subject.contactData.basicData.userNames {
                        items.append(ActionSheetButtonItem(title: userName.value, action: {
                            dismissAction()
                            inviteAction(userName.value)
                        }))
                    }
            ...
            case .createContact:
                pushControllerImpl?(deviceContactInfoController(context: context, 
                                                                subject: .create(peer: subject.peer,
                                                                                 contactData: subject.contactData,
                                                                                 isSharing: false,
    ...
    let stateSignal = combineLatest(presentationData, statePromise.get(), contactData)
    |> map { presentationData, state, peerAndContactData -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        if state.savingData {
            ...
            rightNavigationButton = ItemListNavigationButton(content: .text(presentationData.strings.ShareMenu_Send), style: .bold, enabled: !filteredData.basicData.userNames.isEmpty, action: {
                completion(peerAndContactData.0, filteredData)
                dismissImpl?(true)
            })
        } else if case let .create(createForPeer, _, _, _, completion) = subject {
            let filteredData = filteredContactData(contactData: peerAndContactData.2, excludedComponents: state.excludedComponents)
            var filteredPhoneNumbers: [DeviceContactUserNameData] = []
            for phoneNumber in state.phoneNumbers {
                if !phoneNumber.value.isEmpty && phoneNumber.value != "+" {
                    filteredPhoneNumbers.append(DeviceContactUserNameData(label: phoneNumber.label, value: phoneNumber.value))
                }
            }
            ...
            if let editingName = state.editingState?.editingName, case let .personName(firstName, lastName, _) = editingName, (!firstName.isEmpty || !lastName.isEmpty) {
                ...
                composedContactData = DeviceContactExtendedData(
                    basicData: DeviceContactBasicData(firstName: firstName,
                                                      lastName: lastName,
                                                      userNames: filteredPhoneNumbers),
                    middleName: filteredData.middleName,
                    prefix: filteredData.prefix,
                    suffix: filteredData.suffix,
                    organization: filteredData.organization,
                    jobTitle: filteredData.jobTitle,
                    department: filteredData.department,
                    emailAddresses: filteredData.emailAddresses,
                    urls: urls,
                    addresses: filteredData.addresses,
                    birthdayDate: filteredData.birthdayDate,
                    socialProfiles: filteredData.socialProfiles,
                    instantMessagingProfiles: filteredData.instantMessagingProfiles,
                            ...
                            if filteredPhoneNumbers.count <= 1 {
                                switch subject {
                                    case let .create(peer, _, share, shareViaException, _):
                                        if share, let peer = peer {
                                            ...
                                            |> then(
                                                context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peer.id))
                                                |> castError(AddContactError.self)
                                ...
                                |> mapToSignal { peerId -> Signal<(DeviceContactStableId, DeviceContactExtendedData, Peer?)?, AddContactError> in
                                    if let peerId = peerId {
                                        return context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId))
                                        |> castError(AddContactError.self)
                                        |> map { result -> (DeviceContactStableId, DeviceContactExtendedData, Peer?)? in
                                            return (id, data, result?._asPeer())
                                        }
        ...
        switch subject {
            case .vcard:
            title = isTitleHidden ? "" : presentationData.strings.UserInfo_Title
            case .filter:
                selecting = true
                title = isTitleHidden ? "" : presentationData.strings.UserInfo_Title
            case .create:
...
//        }
        ...
        var isUrlsHidden = false
        if case .filter = subject { isUrlsHidden = true}
        let listState = ItemListNodeState(presentationData: ItemListPresentationData(presentationData),
                                          entries: deviceContactInfoEntries(account: context.account,
                                                                            engine: context.engine,
                                                                            presentationData: presentationData,
                                                                            peer: peerAndContactData.0,
                                                                            isShare: isShare,
                                                                            shareViaException: shareViaException,
                                                                            contactData: peerAndContactData.2,
                                                                            isContact: peerAndContactData.1 != nil,
                                                                            state: state,
                                                                            selecting: selecting,
                                                                            editingPhoneNumbers: editingPhones,
                                                                            isUrlsHidden: isUrlsHidden),
                                          style: isShare  || isUrlsHidden ? .blocks : .plain,
    ...
    let controller = DeviceContactInfoController(context: context, state: stateSignal)
    controller.customTopOffset = 8.0
...
//                    guard let contact = contact as? ElloAppUser, let userName = contact.username else {
//                        return
//                    }
                ...
                if let contactDataManager = context.sharedContext.contactDataManager {
                    ...
                    |> deliverOnMainQueue).start(next: { contactData in
                        ...
                        let _ = (context.engine.data.get(
                            ElloAppEngine.EngineData.Item.Contacts.List(includePresences: false)
                        )
                        |> deliverOnMainQueue).start(next: { view in
                            let usernames = Set<String>(contactData.basicData.userNames.map {
                                return $0.value
                            })
                            
                            var foundPeer: EnginePeer?
                            for peer in view.peers {
                                if case let .user(user) = peer, let username = user.username {
    ...
    controller.setItemGroups([
        ActionSheetItemGroup(items: [
            ActionSheetButtonItem(title: presentationData.strings.Profile_CreateNewContact, action: { [weak controller] in
                ...
                }), isTitleHidden:false, completed: nil, cancelled: nil), in: .window(.root), with: ViewControllerPresentationArguments(presentationAnimation: .modalSheet))
                dismissAction()
            }),
