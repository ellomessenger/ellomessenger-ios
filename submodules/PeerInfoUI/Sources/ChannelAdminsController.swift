...
public func channelAdminsController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, peerId initialPeerId: PeerId, loadCompleted: @escaping () -> Void = {}) -> ViewController {
    ...
    let transferedOwnership: (PeerId) -> Void = { memberId in
        ...
        |> mapToSignal { peerId in
            return context.engine.data.get(
                ...
                ElloAppEngine.EngineData.Item.Peer.Peer(id: memberId)
            )
        }
                    ...
                    if let peer = peerView.peers[participant.peerId] {
                        switch participant {
                            ...
                            case .admin:
                                ...
                                peers[peer.id] = peer
                                result.append(RenderedChannelParticipant(participant: .member(id: peer.id, invitedAt: 0, adminInfo: ChannelParticipantAdminInfo(rights: ElloAppChatAdminRights(rights: .groupSpecific), promotedBy: creator.id, canBeEditedByAccountPeer: creator.id == context.account.peerId), banInfo: nil, rank: nil), peer: peer, peers: peers))
                            case .member:
                                break
                        }
                    }
    ...
    |> map { presentationData, state, view, admins -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        if let admins = admins, admins.count > 1 {
            ...
            } else if let peer = view.peers[peerId] as? ElloAppChannel, peer.flags.contains(.isCreator) {
        ...
        var isGroup = true
        if let peer = view.peers[peerId] as? ElloAppChannel, case .broadcast = peer.info {
            isGroup = false
        } else if let _ = view.peers[peerId] as? ElloAppGroup {
            isGroup = true
        }
