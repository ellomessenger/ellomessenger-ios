...
private func commitChannelOwnershipTransferController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, peer: Peer, member: ElloAppUser, present: @escaping (ViewController, Any?) -> Void, completion: @escaping (PeerId?) -> Void) -> ViewController {
...
/// Change owner without confirm password allert
private func commitChannelOwnershipTransfer(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, peer: Peer, member: ElloAppUser, present: @escaping (ViewController, Any?) -> Void, completion: @escaping (PeerId?) -> Void) {
    let presentationData = updatedPresentationData?.initial ?? context.sharedContext.currentPresentationData.with { $0 }
    
    let signal: Signal<PeerId?, ChannelOwnershipTransferError>
    if let peer = peer as? ElloAppChannel {
        signal = context.peerChannelMemberCategoriesContextsManager.transferOwnership(engine: context.engine, peerId: peer.id, memberId: member.id, password: " ") |> mapToSignal { _ in
            return .complete()
        }
        |> then(.single(nil))
    } else if let peer = peer as? ElloAppGroup {
        ...
        |> mapToSignal { upgradedPeerId -> Signal<PeerId?, ChannelOwnershipTransferError> in
            ...
            return context.peerChannelMemberCategoriesContextsManager.transferOwnership(engine: context.engine, peerId: upgradedPeerId, memberId: member.id, password: " ") |> mapToSignal { _ in
                return .complete()
            }
            |> then(.single(upgradedPeerId))
        }
    ...
    _ = (signal |> deliverOnMainQueue).start(next: { upgradedPeerId in
        completion(upgradedPeerId)
    }, error: { error in
        var isGroup = true
        if let channel = peer as? ElloAppChannel, case .broadcast = channel.info {
        ...
        switch error {
        case .tooMuchJoined:
            present(oldChannelsController(context: context, intent: .upgrade), nil)
            return
        case .invalidPassword:
            break
        case .limitExceeded:
...
private func confirmChannelOwnershipTransferController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, peer: Peer, member: ElloAppUser, present: @escaping (ViewController, Any?) -> Void, completion: @escaping (PeerId?) -> Void) -> ViewController {
    let presentationData = updatedPresentationData?.initial ?? context.sharedContext.currentPresentationData.with { $0 }
...
//        present(commitChannelOwnershipTransferController(context: context, peer: peer, member: member, present: present, completion: completion), nil)
        commitChannelOwnershipTransfer(context: context, peer: peer, member: member, present: present, completion: completion)
    }), TextAlertAction(type: .defaultAction, title: presentationData.strings.Common_Cancel, action: {
    })], actionLayout: .vertical)
    return controller
}

func channelOwnershipTransferController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, peer: Peer, member: ElloAppUser, initialError: ChannelOwnershipTransferError? = nil, present: @escaping (ViewController, Any?) -> Void, completion: @escaping (PeerId?) -> Void) -> ViewController {
//    let presentationData = updatedPresentationData?.initial ?? context.sharedContext.currentPresentationData.with { $0 }
//    let theme = AlertControllerTheme(presentationData: presentationData)
