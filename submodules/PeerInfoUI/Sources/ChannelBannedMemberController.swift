...
private final class ChannelBannedMemberControllerArguments {
    ...
    let toggleRight: (ElloAppChatBannedRightsFlags, Bool) -> Void
    let toggleRightWhileDisabled: (ElloAppChatBannedRightsFlags) -> Void
    let openTimeout: () -> Void
    let delete: () -> Void
    
    init(context: AccountContext, toggleRight: @escaping (ElloAppChatBannedRightsFlags, Bool) -> Void, toggleRightWhileDisabled: @escaping (ElloAppChatBannedRightsFlags) -> Void, openTimeout: @escaping () -> Void, delete: @escaping () -> Void) {
...
private enum ChannelBannedMemberEntryStableId: Hashable {
    case info
    case rightsHeader
    case right(ElloAppChatBannedRightsFlags)
    case timeout
    case exceptionInfo
    case delete
}
...
private enum ChannelBannedMemberEntry: ItemListNodeEntry {
    ...
    case rightItem(PresentationTheme, Int, String, ElloAppChatBannedRightsFlags, Bool, Bool)
...
private struct ChannelBannedMemberControllerState: Equatable {
    ...
    var updatedFlags: ElloAppChatBannedRightsFlags?
    var updatedTimeout: Int32?
    var updating: Bool = false
}

func completeRights(_ flags: ElloAppChatBannedRightsFlags) -> ElloAppChatBannedRightsFlags {
    var result = flags
...
private func channelBannedMemberControllerEntries(presentationData: PresentationData, state: ChannelBannedMemberControllerState, accountPeerId: PeerId, channelPeer: EnginePeer?, memberPeer: EnginePeer?, memberPresence: EnginePeer.Presence?, initialParticipant: ChannelParticipant?, initialBannedBy: EnginePeer?) -> [ChannelBannedMemberEntry] {
    ...
    if case let .channel(channel) = channelPeer, let defaultBannedRights = channel.defaultBannedRights, let member = memberPeer {
        ...
        let currentRightsFlags: ElloAppChatBannedRightsFlags
...
public func channelBannedMemberController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, peerId: PeerId, memberId: PeerId, initialParticipant: ChannelParticipant?, updated: @escaping (ElloAppChatBannedRights?) -> Void, upgradedToSupergroup: @escaping (PeerId, @escaping () -> Void) -> Void) -> ViewController {
    ...
    let arguments = ChannelBannedMemberControllerArguments(context: context, toggleRight: { rights, value in
        ...
        |> deliverOnMainQueue).start(next: { view in
            var defaultBannedRightsFlagsValue: ElloAppChatBannedRightsFlags?
            guard let peer = view.peers[peerId] else {
                return
            }
            if let channel = peer as? ElloAppChannel, let initialRightFlags = channel.defaultBannedRights?.flags {
                defaultBannedRightsFlagsValue = initialRightFlags
            } else if let group = peer as? ElloAppGroup, let initialRightFlags = group.defaultBannedRights?.flags {
                defaultBannedRightsFlagsValue = initialRightFlags
            }
    ...
    var peerDataItems: [ElloAppEngine.EngineData.Item.Peer.Peer] = []
    peerDataItems.append(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId))
    peerDataItems.append(ElloAppEngine.EngineData.Item.Peer.Peer(id: memberId))
    if let banInfo = initialParticipant?.banInfo {
        peerDataItems.append(ElloAppEngine.EngineData.Item.Peer.Peer(id: banInfo.restrictedBy))
    }
    
    let peersMap = context.engine.data.subscribe(
        EngineDataMap(peerDataItems),
        ElloAppEngine.EngineData.Item.Peer.Presence(id: memberId)
    )
        ...
        if state.updating {
            ...
            rightNavigationButton = ItemListNavigationButton(content: .text(presentationData.strings.GroupPermission_ApplyAlertAction), style: .bold, enabled: true, action: {
                ...
                |> deliverOnMainQueue).start(next: { view in
                    ...
                    var resolvedRights: ElloAppChatBannedRights?
                    if let initialParticipant = initialParticipant {
                        var updateFlags: ElloAppChatBannedRightsFlags?
                        ...
                        if updateFlags != nil || updateTimeout != nil {
                            ...
                            resolvedRights = ElloAppChatBannedRights(flags: completeRights(currentRightsFlags), untilDate: currentTimeout)
                        }
                        ...
                        if let updateFlags = updateFlags, let updateTimeout = updateTimeout {
                           resolvedRights = ElloAppChatBannedRights(flags: completeRights(updateFlags), untilDate: updateTimeout)
                        }
                    }
                    
                    var previousRights: ElloAppChatBannedRights?
                    ...
                    if let resolvedRights = resolvedRights, previousRights != resolvedRights {
                        ...
                        let cleanResolvedRights = ElloAppChatBannedRights(flags: cleanResolvedRightsFlags, untilDate: resolvedRights.untilDate)
