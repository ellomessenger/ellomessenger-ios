...
private final class ChannelAdminControllerArguments {
    ...
    let toggleRight: (ElloAppChatAdminRightsFlags, ElloAppChatAdminRightsFlags) -> Void
    let toggleRightWhileDisabled: (ElloAppChatAdminRightsFlags, ElloAppChatAdminRightsFlags) -> Void
    ...
    init(context: AccountContext, updateAdminRights: @escaping (Bool) -> Void, toggleRight: @escaping (ElloAppChatAdminRightsFlags, ElloAppChatAdminRightsFlags) -> Void, toggleRightWhileDisabled: @escaping (ElloAppChatAdminRightsFlags, ElloAppChatAdminRightsFlags) -> Void, transferOwnership: @escaping () -> Void, updateRank: @escaping (String, String) -> Void, updateFocusedOnRank: @escaping (Bool) -> Void, dismissAdmin: @escaping () -> Void, dismissInput: @escaping () -> Void, animateError: @escaping () -> Void) {
...
private enum ChannelAdminEntryStableId: Hashable {
    ...
    case adminRights
    case rightsTitle
    case right(ElloAppChatAdminRightsFlags)
    case addAdminsInfo
    case transfer
    case dismiss
}
...
private enum ChannelAdminEntry: ItemListNodeEntry {
    ...
    case rightItem(PresentationTheme, Int, String, ElloAppChatAdminRightsFlags, ElloAppChatAdminRightsFlags, Bool, Bool)
...
private struct ChannelAdminControllerState: Equatable {
    ...
    let updatedFlags: ElloAppChatAdminRightsFlags?
    let updatedRank: String?
    let updating: Bool
    let focusedOnRank: Bool
    
    init(adminRights: Bool = true, updatedFlags: ElloAppChatAdminRightsFlags? = nil, updatedRank: String? = nil, updating: Bool = false, focusedOnRank: Bool = false) {
    ...
    func withUpdatedUpdatedFlags(_ updatedFlags: ElloAppChatAdminRightsFlags?) -> ChannelAdminControllerState {
...
private func stringForRight(strings: PresentationStrings, right: ElloAppChatAdminRightsFlags, isGroup: Bool, isChannel: Bool, defaultBannedRights: ElloAppChatBannedRights?) -> String {
...
private func rightDependencies(_ right: ElloAppChatAdminRightsFlags) -> [ElloAppChatAdminRightsFlags] {
    if right.contains(.canChangeInfo) {
        return []
    } else if right.contains(.canPostMessages) {
        return []
...
private func canEditAdminRights(accountPeerId: PeerId, channelPeer: Peer, initialParticipant: ChannelParticipant?) -> Bool {
    if let channel = channelPeer as? ElloAppChannel {
    ...
    } else if let group = channelPeer as? ElloAppGroup {
...
private func rightEnabledByDefault(channelPeer: Peer, right: ElloAppChatAdminRightsFlags) -> Bool {
    if let channel = channelPeer as? ElloAppChannel {
        ...
        switch right {
        case .canPinMessages:
            return false/*!defaultBannedRights.flags.contains(.banPinMessages)*/
        case .canChangeInfo:
...
private func areAllAdminRightsEnabled(_ flags: ElloAppChatAdminRightsFlags, group: Bool, except: ElloAppChatAdminRightsFlags) -> Bool {
    if group {
        return ElloAppChatAdminRightsFlags.groupSpecific.subtracting(except).intersection(flags) == ElloAppChatAdminRightsFlags.groupSpecific.subtracting(except)
    } else {
        return ElloAppChatAdminRightsFlags.broadcastSpecific.subtracting(except).intersection(flags) == ElloAppChatAdminRightsFlags.broadcastSpecific.subtracting(except)
    }
}
...
private func channelAdminControllerEntries(presentationData: PresentationData, state: ChannelAdminControllerState, accountPeerId: PeerId, channelPeer: EnginePeer?, adminPeer: EnginePeer?, adminPresence: EnginePeer.Presence?, initialParticipant: ChannelParticipant?, invite: Bool, canEdit: Bool) -> [ChannelAdminEntry] {
    ...
    if case let .channel(channel) = channelPeer, let admin = adminPeer {
        ...
        let isGroup: Bool
        var maskRightsFlags: ElloAppChatAdminRightsFlags
        let rightsOrder: [ElloAppChatAdminRightsFlags]
...
//                    .canManageCalls,
        ...
        if isCreator {
            if isGroup {
                ...
                let accountUserRightsFlags: ElloAppChatAdminRightsFlags
                if channel.flags.contains(.isCreator) {
                ...
                let currentRightsFlags: ElloAppChatAdminRightsFlags
        ...
        if isCreator {
            ...
            if !invite || state.adminRights {
                ...
                let isGroup = true
                let isChannel = false
                let maskRightsFlags: ElloAppChatAdminRightsFlags = .groupSpecific
                let rightsOrder: [ElloAppChatAdminRightsFlags] = [
                    .canChangeInfo,
                    .canDeleteMessages,
                    .canBanUsers,
...
//                    .canManageCalls,
                ...
                let accountUserRightsFlags: ElloAppChatAdminRightsFlags = maskRightsFlags
...
public func channelAdminController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, peerId: PeerId, adminId: PeerId, initialParticipant: ChannelParticipant?, invite: Bool = false, initialAdminRights: ElloAppChatAdminRightsFlags? = nil, updated: @escaping (ElloAppChatAdminRights?) -> Void, upgradedToSupergroup: @escaping (PeerId, @escaping () -> Void) -> Void, transferedOwnership: @escaping (PeerId) -> Void) -> ViewController {
    ...
    let arguments = ChannelAdminControllerArguments(context: context, updateAdminRights: { value in
        ...
        let _ = (context.engine.data.get(
            ...
            ElloAppEngine.EngineData.Item.Peer.Peer(id: adminId)
        )
...
//            transferOwnershipDisposable.set((context.engine.peers.checkOwnershipTranfserAvailability(memberId: adminId) |> deliverOnMainQueue).start(error: { error in
            let controller = channelOwnershipTransferController(context: context, updatedPresentationData: updatedPresentationData, peer: peer._asPeer(), member: member, initialError: .generic, present: { c, a in
...
//            }))
    ...
    let signal = combineLatest(
        ...
        context.engine.data.subscribe(
            ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId),
            ElloAppEngine.EngineData.Item.Peer.Peer(id: adminId),
            ElloAppEngine.EngineData.Item.Peer.Presence(id: adminId)
        )
    )
    |> deliverOnMainQueue
        ...
        let rightButtonActionImpl = {
            if invite && !state.adminRights {
                ...
                if let initialParticipant = initialParticipant {
                    var updateFlags: ElloAppChatAdminRightsFlags?
                    ...
                    let maskRightsFlags: ElloAppChatAdminRightsFlags
                    switch channel.info {
                        case .broadcast:
                            maskRightsFlags = .broadcastSpecific
                        case .group:
                    ...
                    var currentFlags: ElloAppChatAdminRightsFlags?
                    switch initialParticipant {
                    ...
                    if updateFlags != currentFlags {
                        ...
                        updateRightsDisposable.set((context.peerChannelMemberCategoriesContextsManager.updateMemberAdminRights(engine: context.engine, peerId: peerId, memberId: adminId, adminRights: ElloAppChatAdminRights(rights: updateFlags ?? []), rank: effectiveRank) |> deliverOnMainQueue).start(error: { error in
                            updateState { current in
                                return current.withUpdatedUpdating(false)
                            }
                            ...
                            updated(ElloAppChatAdminRights(rights: updateFlags ?? []))
                            dismissImpl?()
                        }))
                        ...
                        updateRightsDisposable.set((context.peerChannelMemberCategoriesContextsManager.updateMemberAdminRights(engine: context.engine, peerId: peerId, memberId: adminId, adminRights: ElloAppChatAdminRights(rights: currentFlags), rank: updateRank) |> deliverOnMainQueue).start(error: { _ in
                            
                        }, completed: {
                            updated(ElloAppChatAdminRights(rights: currentFlags))
                            dismissImpl?()
                        }))
                    } else {
                        dismissImpl?()
                    ...
                    if let updateFlags = updateFlags {
                        ...
                        updateRightsDisposable.set((context.peerChannelMemberCategoriesContextsManager.updateMemberAdminRights(engine: context.engine, peerId: peerId, memberId: adminId, adminRights: ElloAppChatAdminRights(rights: updateFlags), rank: updateRank) |> deliverOnMainQueue).start(error: { error in
                ...
                if let updateFlags = updateFlags {
                    if initialParticipant?.adminInfo == nil && updateFlags == defaultFlags && updateRank == nil {
                        ...
                        |> mapToSignal { upgradedPeerId -> Signal<PeerId?, WrappedUpdateChannelAdminRightsError> in
                            ...
                            return context.peerChannelMemberCategoriesContextsManager.updateMemberAdminRights(engine: context.engine, peerId: upgradedPeerId, memberId: adminId, adminRights: ElloAppChatAdminRights(rights: updateFlags), rank: updateRank)
