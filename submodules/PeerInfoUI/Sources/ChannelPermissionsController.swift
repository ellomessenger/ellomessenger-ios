...
private final class ChannelPermissionsControllerArguments {
    ...
    let updatePermission: (ElloAppChatBannedRightsFlags, Bool) -> Void
    ...
    let openKicked: () -> Void
    let presentRestrictedPermissionAlert: (ElloAppChatBannedRightsFlags) -> Void
    let presentConversionToBroadcastGroup: () -> Void
    let openChannelExample: () -> Void
    let updateSlowmode: (Int32) -> Void
    
    init(context: AccountContext, updatePermission: @escaping (ElloAppChatBannedRightsFlags, Bool) -> Void, setPeerIdWithRevealedOptions: @escaping (PeerId?, PeerId?) -> Void, addPeer: @escaping  () -> Void, removePeer: @escaping (PeerId) -> Void, openPeer: @escaping (ChannelParticipant) -> Void, openPeerInfo: @escaping (Peer) -> Void, openKicked: @escaping () -> Void, presentRestrictedPermissionAlert: @escaping (ElloAppChatBannedRightsFlags) -> Void, presentConversionToBroadcastGroup: @escaping () -> Void, openChannelExample: @escaping () -> Void, updateSlowmode: @escaping (Int32) -> Void) {
...
private enum ChannelPermissionsEntry: ItemListNodeEntry {
    ...
    case permission(PresentationTheme, Int, String, Bool, ElloAppChatBannedRightsFlags, Bool?)
    ...
    case peerItem(PresentationTheme, PresentationStrings, PresentationDateTimeFormat, PresentationPersonNameOrder, Int32, RenderedChannelParticipant, ItemListPeerItemEditing, Bool, Bool, ElloAppChatBannedRightsFlags)
...
private struct ChannelPermissionsControllerState: Equatable {
    ...
    var modifiedRightsFlags: ElloAppChatBannedRightsFlags?
    var modifiedSlowmodeTimeout: Int32?
}

func stringForGroupPermission(strings: PresentationStrings, right: ElloAppChatBannedRightsFlags) -> String {
...
func compactStringForGroupPermission(strings: PresentationStrings, right: ElloAppChatBannedRightsFlags) -> String {
    if right.contains(.banSendMessages) {
...
public let allGroupPermissionList: [(ElloAppChatBannedRightsFlags, ElloAppChannelPermission)] = [
...
let publicGroupRestrictedPermissions: ElloAppChatBannedRightsFlags = [
//    .banPinMessages,
    .banChangeInfo
]

func groupPermissionDependencies(_ right: ElloAppChatBannedRightsFlags) -> ElloAppChatBannedRightsFlags {
...
private func channelPermissionsControllerEntries(context: AccountContext, presentationData: PresentationData, view: PeerView, state: ChannelPermissionsControllerState, participants: [RenderedChannelParticipant]?) -> [ChannelPermissionsEntry] {
    ...
    if let channel = view.peers[view.peerId] as? ElloAppChannel, let participants = participants, let cachedData = view.cachedData as? CachedChannelData, let defaultBannedRights = channel.defaultBannedRights {
        let effectiveRightsFlags: ElloAppChatBannedRightsFlags
...
//        entries.append(.add(presentationData.theme, presentationData.strings.GroupInfo_Permissions_AddException))
    ...
    } else if let group = view.peers[view.peerId] as? ElloAppGroup, let _ = view.cachedData as? CachedGroupData {
        let defaultBannedRights = group.defaultBannedRights ?? ElloAppChatBannedRights(flags: [], untilDate: 0)
...
public func channelPermissionsController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, peerId originalPeerId: PeerId, loadCompleted: @escaping () -> Void = {}) -> ViewController {
    ...
    let arguments = ChannelPermissionsControllerArguments(context: context, updatePermission: { rights, value in
        ...
        |> deliverOnMainQueue).start(next: { view in
            if let channel = view.peers[view.peerId] as? ElloAppChannel, let _ = view.cachedData as? CachedChannelData {
                updateState { state in
                    var state = state
                    var effectiveRightsFlags: ElloAppChatBannedRightsFlags
                    if let modifiedRightsFlags = state.modifiedRightsFlags {
                        effectiveRightsFlags = modifiedRightsFlags
                    } else if let defaultBannedRightsFlags = channel.defaultBannedRights?.flags {
                        effectiveRightsFlags = defaultBannedRightsFlags
                    } else {
                        effectiveRightsFlags = ElloAppChatBannedRightsFlags()
                    }
                    if value {
                ...
                if let modifiedRightsFlags = state.modifiedRightsFlags {
                    updateDefaultRightsDisposable.set((context.engine.peers.updateDefaultChannelMemberBannedRights(peerId: view.peerId, rights: ElloAppChatBannedRights(flags: completeRights(modifiedRightsFlags), untilDate: Int32.max))
        ...
        |> take(1)
        |> deliverOnMainQueue).start(next: { view in
            guard let channel = view.peers[view.peerId] as? ElloAppChannel else {
                return
            }
        ...
        |> take(1)
        |> deliverOnMainQueue).start(next: { view in
            if let _ = view.peers[view.peerId] as? ElloAppChannel, let _ = view.cachedData as? CachedChannelData {
                updateState { state in
                    var state = state
            ...
            } else if let _ = view.peers[view.peerId] as? ElloAppGroup, let _ = view.cachedData as? CachedGroupData {
                updateState { state in
                    var state = state
                    state.modifiedSlowmodeTimeout = value
                    return state
    ...
    |> map { presentationData, state, viewAndParticipants -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        if state.searchingMembers {
            searchItem = ChannelMembersSearchItem(context: context, peerId: view.peerId, searchContext: nil, searchMode: .searchBanned, cancel: {
                ...
                if let participant = rendered?.participant, case .member = participant, let _ = peerViewMainPeer(view) as? ElloAppChannel {
                    updateState { state in
                        var state = state
                        state.searchingMembers = false
                        return state
