...
class IncreaseLimitHeaderItemNode: ListViewItemNode {
    ...
    override func didLoad() {
        ...
        let limitCounterView = ComponentHostView<Empty>()
        self.hostView = limitCounterView
        self.view.addSubview(limitCounterView)
        
        if let (component, containerSize, layout, textSize) = self.params {
            var size = limitCounterView.update(
            ...
            limitCounterView.frame = CGRect(origin: CGPoint(x: floorToScreenPixels((layout.size.width - size.width) / 2.0), y: -30.0), size: size)
//            limitCounterView.isHidden = true
        ...
        return { item, params, neighbors in
            ...
            return (layout, { [weak self] in
                if let strongSelf = self {
                    ...
                    if  item.icon == .link {
                        gradientColors = [UIColor(rgb: 0x0A49A5), 
                                          UIColor(rgb: 0x0A49A5)]
                    } else if item.isPremiumDisabled {
                        gradientColors = [
