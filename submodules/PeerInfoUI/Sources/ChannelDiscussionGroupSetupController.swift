    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        switch self {
            ...
            case let .group(_, _, strings, peer, nameOrder):
                let text: String
                if let peer = peer as? ElloAppChannel, let addressName = peer.addressName, !addressName.isEmpty {
                    text = "@\(addressName)"
                } else if let peer = peer as? ElloAppChannel, case .broadcast = peer.info {
                    text = strings.Channel_DiscussionGroup_PrivateChannel.capitalizeFirst()
                } else {
                    text = strings.Channel_DiscussionGroup_PrivateGroup.capitalizeFirst()
                }
            ...
            return ItemListTextItem(presentationData: presentationData, text: .custom(fontSize: 12.0, weight: .regular, text: title, color: UIColor(rgb: 0x929298)), sectionId: self.section)
...
private func channelDiscussionGroupSetupControllerEntries(presentationData: PresentationData, view: PeerView, groups: [Peer]?) -> [ChannelDiscussionGroupSetupControllerEntry] {
    guard let peer = view.peers[view.peerId] as? ElloAppChannel, let cachedData = view.cachedData as? CachedChannelData else {
        return []
    }
...
public func channelDiscussionGroupSetupController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, peerId: PeerId) -> ViewController {
    ...
    let arguments = ChannelDiscussionGroupSetupControllerArguments(context: context, createGroup: {
        ...
        let _ = (context.engine.data.get(
            ...
            ElloAppEngine.EngineData.Item.Peer.Peer(id: groupId)
        )
                ...
                ActionSheetButtonItem(title: presentationData.strings.Channel_DiscussionGroup_LinkGroup, color: .accent, action: { [weak actionSheet] in
                    ...
                    if case let .legacyGroup(legacyGroup) = groupPeer {
                        ...
                        |> mapToSignal { resultPeerId -> Signal<Bool, ChannelDiscussionGroupError> in
                            ...
                            return context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: resultPeerId))
    ...
    |> map { presentationData, state, view, groups -> (ItemListControllerState, (ItemListNodeState, Any)) in
        let title: String
        if let peer = view.peers[view.peerId] as? ElloAppChannel, case .broadcast = peer.info {
