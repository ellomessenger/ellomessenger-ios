...
private enum ChannelMembersSearchEntry: Comparable, Identifiable {
    ...
    case contact(Int, Peer, ElloAppUserPresence?)
    ...
    func item(context: AccountContext, presentationData: PresentationData, nameSortOrder: PresentationPersonNameOrder, nameDisplayOrder: PresentationPersonNameOrder, interaction: ChannelMembersSearchInteraction) -> ListViewItem {
        ...
        case let .peer(_, participant, editing, label, enabled, isChannel, isContact):
            ...
            if let label = label {
                ...
                let presence = participant.presences[participant.peer.id] ?? ElloAppUserPresence(status: .none, lastActivity: 0)
                ...
                for participant in participants.participants {
                    ...
                    switch mode {
                            ...
                            for filter in filters {
                                switch filter {
                                    ...
                                    case .excludeNonMembers:
                                        break
                                    case .excludeBots:
                                        if let user = peer as? ElloAppUser, user.botInfo != nil {
                                            continue
                                        }
                                }
                            }
                        ...
                        case .inviteToCall:
                            ...
                            if let user = peer as? ElloAppUser, user.botInfo != nil || user.flags.contains(.isSupport) {
                                continue
                            }
                            for filter in filters {
                                switch filter {
                    ...
                    contactsLoop: for participant in contactsState.list {
                        ...
                        for filter in filters {
                            ...
                            case .excludeNonMembers:
                                break
                            case .excludeBots:
                                if let user = participant.peer as? ElloAppUser, user.botInfo != nil {
                                    continue contactsLoop
                                }
                            }
                        }
                ...
                participantsLoop: for participant in state.list {
                    ...
                    switch mode {
                        ...
                        case .inviteToCall:
                            ...
                            if let user = participant.peer as? ElloAppUser, user.botInfo != nil || user.flags.contains(.isSupport) {
                                continue
                            }
                            for filter in filters {
                                switch filter {
