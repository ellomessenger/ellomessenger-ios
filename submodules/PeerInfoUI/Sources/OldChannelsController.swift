...
func localizedOldChannelDate(peer: InactiveChannel, strings: PresentationStrings) -> String {
    ...
    if let channel = peer.peer as? ElloAppChannel, case .group = channel.info {
...
private enum OldChannelsEntry: ItemListNodeEntry {
    ...
    static func ==(lhs: OldChannelsEntry, rhs: OldChannelsEntry) -> Bool {
        ...
        case let .peer(lhsIndex, lhsPeer, lhsSelected, _):
            if case let .peer(rhsIndex, rhsPeer, rhsSelected, _) = rhs {
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        switch self {
        case let .info(count, limit, premiumLimit, text, isPremiumDisabled):
            return IncreaseLimitHeaderItem(theme: presentationData.theme,
        ...
        case let .peersHeader(title):
            return ItemListSectionHeaderItem(presentationData: presentationData, 
                                             text: title,
                                             sectionId: self.section)
        case let .peer(_, peer, _, completion): // (_, peer, selected)
            return ContactsPeerItem(presentationData: presentationData,
                                    ...
                                    peerMode: .peer,
                                    peer: .peer(peer: EnginePeer(peer), chatPeer: EnginePeer(peer)),
                                    status: .none, // .custom(string: localizedOldChannelDate(peer: peer, strings: presentationData.strings), multiline: false),
                                    badge: nil, 
                                    enabled: true,
                                    selection: .none, // ContactsPeerItemSelection.selectable(selected: selected),
                                    editing: ContactsPeerItemEditing(editable: true, editing: false, revealed: false),
                                    options: [],
                                    actionIcon: .none,
                                    index: nil,
                                    header: nil,
                                    action: { _ in arguments.togglePeer(peer.id, true) },
                                    setPeerIdWithRevealedOptions: nil,
                                    deletePeer: { peer in deletePeer(peer, completion: completion) },
                                    itemHighlighting: nil,
                                    contextAction: nil)
        }
        
        func deletePeer(_ peer:EnginePeer.Id, completion: (()->Void)?) {
            _ = (arguments.context.engine.peers.deleteChannel(peerId: peer) |> deliverOnMainQueue).start(error: { error in
                print(error)
            }, completed: {
                completion?()
            })
...
private func oldChannelsEntries(presentationData: PresentationData, 
                                state: OldChannelsState,
                                isPremium: Bool,
                                isPremiumDisabled: Bool,
                                limit: Int32,
                                premiumLimit: Int32,
                                peers: [InactiveChannel]?,
                                adminedPeers: [Peer]?,
                                intent: OldChannelsControllerIntent,
                                action: (()->())?) -> [OldChannelsEntry] {
    var entries: [OldChannelsEntry] = []
    
    let count = peers?.count.int32 ?? adminedPeers?.count.int32 ?? limit
    var text: String?
    if isPremium {
        switch intent {
            case .create:
                text = presentationData.strings.OldChannels_TooManyCommunitiesCreateFinalText("\(count)").string
            case .upgrade:
                text = presentationData.strings.OldChannels_TooManyCommunitiesUpgradeFinalText("\(count)").string
            case .join:
                text = presentationData.strings.OldChannels_TooManyCommunitiesFinalText("\(count)").string
        }
    } else {
        if isPremiumDisabled {
            switch intent {
                case .create:
                    text = presentationData.strings.OldChannels_TooManyCommunitiesCreateNoPremiumLinksText
                case .upgrade:
                    text = presentationData.strings.OldChannels_TooManyCommunitiesUpgradeNoPremiumText("\(count)").string
                case .join:
    ...
    if let adminedPeers, !adminedPeers.isEmpty {
        entries.append(.peersHeader(presentationData.strings.OldChannels_ChannelsHeader))
        
        for peer in adminedPeers {
            entries.append(.peer(entries.count, peer, state.selectedPeers.contains(peer.id), action))
        }
    }
...
public func oldChannelsController(context: AccountContext, 
                                  updatedPresentationData: (initial: PresentationData,
                                                            signal: Signal<PresentationData, NoError>)? = nil,
                                  intent: OldChannelsControllerIntent,
                                  completed: @escaping (Bool) -> Void = { _ in }) -> ViewController {
...
//                }
    ...
    let signal = combineLatest(
        ...
        context.engine.peers.adminedPublicChannels(scope: .all) |> deliverOnMainQueue
    )
    |> map { presentationData, state, peers, limits, adminedPeers -> (ItemListControllerState, (ItemListNodeState, Any)) in
        let leftNavigationButton = ItemListNavigationButton(content: .text(presentationData.strings.Common_Cancel), 
                                                            style: .regular,
                                                            enabled: true,
                                                            action: {
            dismissImpl?()
        ...
        let controllerState = ItemListControllerState(presentationData: ItemListPresentationData(presentationData), 
                                                      title: .text(presentationData.strings.OldChannels_Title),
                                                      leftNavigationButton: leftNavigationButton,
                                                      rightNavigationButton: nil,
                                                      backNavigationButton: ItemListBackButton(title: presentationData.strings.Common_Back))
        
        var searchItem: OldChannelsSearchItem?
        searchItem = OldChannelsSearchItem(context: context, 
                                           theme: presentationData.theme,
                                           placeholder: presentationData.strings.Common_Search,
                                           activated: state.isSearching,
                                           updateActivated: { value in
            if !value {
...
//        let premiumConfiguration = PremiumConfiguration.with(appConfiguration: context.currentAppConfiguration.with { $0 })
//        let _ = (context.engine.peers.adminedPublicChannels(scope: .all) |> deliverOnMainQueue |> map { peers -> () in
//            print(peers)
//        }).start()
        ...
        let listState = ItemListNodeState(presentationData: ItemListPresentationData(presentationData),
                                          entries: oldChannelsEntries(presentationData: presentationData,
                                                                      ...
                                                                      limit: limits.maxChannelsCount,
                                                                      premiumLimit: premiumLimits.maxChannelsCount, 
                                                                      peers: peers,
                                                                      adminedPeers: adminedPeers,
                                                                      intent: intent,
                                                                      action: actionImpl),
                                          style: .blocks,
                                          ...
                                          initialScrollToItem: ListViewScrollToItem(index: 0,
                                                                                    position: .top(-navigationBarSearchContentHeight),
                                                                                    animated: false,
                                                                                    curve: .Default(duration: 0.0), directionHint: .Up),
                                          crossfadeState: peersAreEmptyUpdated,
    ...
    actionImpl = { [weak controller] in
        ...
        controller?.navigationController?.pushViewController(context.sharedContext.makeComposeController(context: context), animated: false)
    }
