...
final class VoiceChatPeerProfileNode: ASDisplayNode {
    ...
    private let additionalEntry: Signal<(ElloAppMediaImageRepresentation, Float)?, NoError>
    ...
    init(context: AccountContext, size: CGSize, sourceSize: CGSize, peer: Peer, text: VoiceChatParticipantItem.ParticipantText, customNode: ASDisplayNode? = nil, additionalEntry: Signal<(ElloAppMediaImageRepresentation, Float)?, NoError>, requestDismiss: (() -> Void)?) {
    ...
    func updateInfo(size: CGSize, sourceSize: CGSize, animate: Bool) {
        ...
        if let user = self.peer as? ElloAppUser {
