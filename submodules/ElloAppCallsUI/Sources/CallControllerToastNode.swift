...
private class CallControllerToastItemNode: ASDisplayNode {
    ...
    func update(width: CGFloat, content: Content, transition: ContainedViewLayoutTransition) -> CGFloat {
        ...
        if self.currentContent != content || self.currentWidth != width {
            ...
            let iconSize = CGSize(width: kDefaultHeight, height: kDefaultHeight)
            ...
            let backgroundSize = CGSize(width: iconSize.width + iconSpacing + textSize.width + 6.0 * 2.0, height: max(kDefaultHeight, textSize.height + 4.0 * 2.0))
        ...
        return self.currentHeight ?? kDefaultHeight
    }
    
    func animateIn() {
        let targetFrame = self.clipNode.frame
        let initialFrame = CGRect(x: floor((self.frame.width - 44.0) / 2.0), y: 0.0, width: 44.0, height: kDefaultHeight)
