                ...
                for output in availableOutputs {
                    ...
                    switch output {
                        ...
                        case .speaker:
                            ...
                            icon = generateScaledImage(image: UIImage(bundleImageName: "Call/AudioMenu/Speaker"), size: CGSize(width: 48.0, height: 48.0), opaque: false)
                        case .headphones:
                            title = strongSelf.presentationData.strings.Call_AudioRouteHeadphones
                        icon = generateScaledImage(image: UIImage(bundleImageName: "Call/AudioMenu/Headphones"), size: CGSize(width: 48.0, height: 48.0), opaque: false)
                        case let .port(port):
                            title = port.name
                            if port.type == .bluetooth {
                                var image = UIImage(bundleImageName: "Call/AudioMenu/Bluetooth")
                                let portName = port.name.lowercased()
                                if portName.contains("airpods max") {
                                    image = UIImage(bundleImageName: "Call/AudioMenu/AirPodsMax")
                                } else if portName.contains("airpods pro") {
                                    image = UIImage(bundleImageName: "Call/AudioMenu/AirPodsPro")
                                } else if portName.contains("airpods") {
                                    image = UIImage(bundleImageName: "Call/AudioMenu/AirPods")
                                }
