...
final class CallControllerStatusNode: ASDisplayNode {
    ...
    private let encryptNode: TextNode
    ...
    override init() {
        ...
        self.statusContainerNode.addSubnode(self.encryptNode)
    ...
    func updateLayout(constrainedWidth: CGFloat, transition: ContainedViewLayoutTransition) -> CGFloat {
        ...
        let (encryptLayout, encryptApply) = TextNode.asyncLayout(self.encryptNode)(TextNodeLayoutArguments(
            attributedString: NSAttributedString(string: "End-to-end Encrypted".localized,
                                                         font: statusFont,
                                                         textColor: .white),
                    backgroundColor: nil,
        ...
        self.titleNode.frame = CGRect(origin: CGPoint(x: floor((constrainedWidth - titleLayout.size.width) / 2.0), y: 0.0), 
                                      size: titleLayout.size)
        self.statusNode.frame = CGRect(origin: CGPoint(x: floor((constrainedWidth - statusMeasureLayout.size.width) / 2.0) + statusOffset,
                                                       y: titleLayout.size.height + spacing),
                                       size: statusLayout.size)
        self.encryptNode.frame = CGRect(origin: CGPoint(x: floor((constrainedWidth - encryptLayout.size.width) / 2.0),
                                                        y: self.statusNode.frame.maxY + spacing + spacing),
                                        size: encryptLayout.size)
        self.statusContainerNode.frame = CGRect(origin: CGPoint(x: 0.0, y: 0.0),
                                                size: CGSize(width: constrainedWidth, height: self.encryptNode.frame.maxY))

        self.receptionNode.frame = CGRect(origin: CGPoint(x: self.statusNode.frame.minX - receptionNodeSize.width, 
                                                          y: self.statusNode.frame.minY + (statusLayout.size.height - receptionNodeSize.height) / 2.0),
        ...
        return self.statusContainerNode.frame.maxY
    }
}
