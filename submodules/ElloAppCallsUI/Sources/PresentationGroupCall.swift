...
public final class AccountGroupCallContextImpl: AccountGroupCallContext {
    ...
    public init(account: Account, engine: ElloAppEngine, peerId: PeerId, isChannel: Bool, call: EngineGroupCallDescription) {
        ...
        self.disposable = (combineLatest(queue: .mainQueue(),
            state,
            engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId))
        )
...
public final class AccountGroupCallContextCacheImpl: AccountGroupCallContextCache {
    public class Impl {
        ...
        public func get(account: Account, engine: ElloAppEngine, peerId: PeerId, isChannel: Bool, call: EngineGroupCallDescription) -> AccountGroupCallContextImpl.Proxy {
            let result: Record
        ...
        public func leaveInBackground(engine: ElloAppEngine, id: Int64, accessHash: Int64, source: UInt32) {
...
public final class PresentationGroupCallImpl: PresentationGroupCall {
    ...
    private func switchToTemporaryParticipantsContext(sourceContext: GroupCallParticipantsContext?, oldMyPeerId: PeerId) {
        ...
        let myPeerData = self.accountContext.engine.data.get(
            ElloAppEngine.EngineData.Item.Peer.Peer(id: myPeerId),
            ElloAppEngine.EngineData.Item.Peer.AboutText(id: myPeerId)
        )
    ...
    private func switchToTemporaryScheduledParticipantsContext() {
        ...
        if peerId.namespace == Namespaces.Peer.CloudChannel {
            ...
            rawAdminIds = accountContext.engine.data.subscribe(
                ElloAppEngine.EngineData.Item.Peer.LegacyGroupParticipants(id: peerId)
            )
        ...
        let adminIds = combineLatest(queue: .mainQueue(),
            rawAdminIds,
            accountContext.engine.data.subscribe(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId))
        )
            ...
            if case let .established(callInfo, _, _, _, initialState) = internalState {
                ...
                |> mapToSignal { event -> Signal<PresentationGroupCallMemberEvent, NoError> in
                    return engine.data.get(
                        ElloAppEngine.EngineData.Item.Peer.Peer(id: event.peerId),
                        ElloAppEngine.EngineData.Item.Peer.IsContact(id: event.peerId),
                        ElloAppEngine.EngineData.Item.Messages.ChatListIndex(id: event.peerId)
                    )
    ...
    public func reconnect(as peerId: PeerId) {
        ...
        let _ = (self.accountContext.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId))
