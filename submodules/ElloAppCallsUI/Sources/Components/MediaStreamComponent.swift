...
public final class MediaStreamComponent: CombinedComponent {
    ...
    public final class State: ComponentState {
        ...
        init(call: PresentationGroupCallImpl) {
            ...
            let callPeer = call.accountContext.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: call.peerId))
    ...
    func presentShare(links inviteLinks: GroupCallInviteLinks) {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] peer, callState in
            if let strongSelf = self {
                ...
                if let peer = peer as? ElloAppChannel, case .group = peer.info, !peer.flags.contains(.isGigagroup), !(peer.addressName ?? "").isEmpty, let defaultParticipantMuteState = callState.defaultParticipantMuteState {
