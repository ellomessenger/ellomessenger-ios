...
final class VoiceChatParticipantItem: ListViewItem {
    ...
    let getUpdatingAvatar: () -> Signal<(ElloAppMediaImageRepresentation, Float)?, NoError>
    
    public let selectable: Bool = true
    
    public init(presentationData: ItemListPresentationData, dateTimeFormat: PresentationDateTimeFormat, nameDisplayOrder: PresentationPersonNameOrder, context: AccountContext, peer: Peer, text: ParticipantText, expandedText: ParticipantText?, icon: Icon, getAudioLevel: (() -> Signal<Float, NoError>)?, action: ((ASDisplayNode?) -> Void)?, contextAction: ((ASDisplayNode, ContextGesture?) -> Void)? = nil, getIsExpanded: @escaping () -> Bool, getUpdatingAvatar: @escaping () -> Signal<(ElloAppMediaImageRepresentation, Float)?, NoError>) {
...
class VoiceChatParticipantItemNode: ItemListRevealOptionsItemNode {
    ...
    func asyncLayout() -> (_ item: VoiceChatParticipantItem, _ params: ListViewItemLayoutParams, _ first: Bool, _ last: Bool) -> (ListViewItemNodeLayout, (Bool, Bool) -> Void) {
        ...
        return { item, params, first, last in
            ...
            if let user = item.peer as? ElloAppUser {
            ...
            } else if let user = item.peer as? ElloAppUser, let emojiStatus = user.emojiStatus, !premiumConfiguration.isPremiumDisabled {
