...
final class CallControllerButtonItemNode: HighlightTrackingButtonNode {
    ...
    init(largeButtonSize: CGFloat = 80.0) {
    ...
    func update(size: CGSize, content: Content, text: String, transition: ContainedViewLayoutTransition) {
        ...
        if self.currentContent != content || self.size != size {
            ...
            let contentImage = generateImage(CGSize(width: self.largeButtonSize, height: self.largeButtonSize), contextGenerator: { size, context in
                ...
                let imageScale: CGFloat = isSmall ? 1.0 : 1.25
                switch content.appearance {
                ...
                switch content.image {
                case .cameraOff:
                    image = generateTintedImage(image: UIImage(bundleImageName: "Call/CallCameraOffButton"), color: imageColor)
                case .cameraOn:
                    image = generateTintedImage(image: UIImage(bundleImageName: "Call/CallCameraOnButton"), color: imageColor)
                case .camera:
        ...
        if let animationNode = self.animationNode {
            transition.updateTransformScale(node: animationNode, scale: isSmall ? 0.415 : 0.482)
        }
