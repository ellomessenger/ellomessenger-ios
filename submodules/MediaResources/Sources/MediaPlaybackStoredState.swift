...
public func mediaPlaybackStoredState(engine: ElloAppEngine, messageId: MessageId) -> Signal<MediaPlaybackStoredState?, NoError> {
    ...
    return engine.data.get(ElloAppEngine.EngineData.Item.ItemCache.Item(collectionId: ApplicationSpecificItemCacheCollectionId.mediaPlaybackStoredState, id: key))
    |> map { entry -> MediaPlaybackStoredState? in
        return entry?.get(MediaPlaybackStoredState.self)
    }
}

public func updateMediaPlaybackStoredStateInteractively(engine: ElloAppEngine, messageId: MessageId, state: MediaPlaybackStoredState?) -> Signal<Never, NoError> {
    let key = ValueBoxKey(length: 20)
    key.setInt32(0, value: messageId.namespace)
    key.setInt32(4, value: messageId.peerId.namespace._internalGetInt32Value())
    key.setInt64(8, value: messageId.peerId.id._internalGetInt64Value())
