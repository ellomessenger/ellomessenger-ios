    ...
    for i in 0 ..< entities.count {
        ...
        switch entity.type {
            case .Url:
                ...
                string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL), value: nsString!.substring(with: range), range: range)
            case .Email:
                string.addAttribute(NSAttributedString.Key.foregroundColor, value: linkColor, range: range)
                if nsString == nil {
                    nsString = text as NSString
                ...
                string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL), value: "mailto:\(nsString!.substring(with: range))", range: range)
            case .PhoneNumber:
                string.addAttribute(NSAttributedString.Key.foregroundColor, value: linkColor, range: range)
                if nsString == nil {
                    nsString = text as NSString
                ...
                string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL), value: "tel:\(nsString!.substring(with: range))", range: range)
            case let .TextUrl(url):
                string.addAttribute(NSAttributedString.Key.foregroundColor, value: linkColor, range: range)
                if nsString == nil {
                    nsString = text as NSString
                ...
                if external {
                    ...
                    string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL), value: url, range: range)
                }
            case .Bold:
            ...
            case .Mention:
                ...
                string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.PeerTextMention), value: nsString!.substring(with: range), range: range)
            case .Strikethrough:
            ...
            case let .TextMention(peerId):
                ...
                string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.PeerMention), value: ElloAppPeerMention(peerId: peerId, mention: mention), range: range)
            case .Hashtag:
                if nsString == nil {
                    nsString = text as NSString
                }
                    ...
                    if case .Mention = entities[i + 1].type {
                        ...
                        if nextRange.location == range.location + range.length + 1 && nsString!.character(at: range.location + range.length) == 43 {
                            ...
                            if nextRange.length > 0 {
                                ...
                                string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.Hashtag), value: ElloAppHashtag(peerName: peerName, hashtag: hashtag), range: combinedRange)
                            }
                        }
                    }
                }
                if !skipEntity {
                    ...
                    string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.Hashtag), value: ElloAppHashtag(peerName: nil, hashtag: hashtag), range: range)
                }
            case .BotCommand:
                string.addAttribute(NSAttributedString.Key.foregroundColor, value: linkColor, range: range)
                if underlineLinks && underlineAllLinks {
                ...
                string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.BotCommand), value: nsString!.substring(with: range), range: range)
            case .Code, .Pre:
                string.addAttribute(NSAttributedString.Key.font, value: fixedFont, range: range)
                if nsString == nil {
                    nsString = text as NSString
                }
                string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.Pre), value: nsString!.substring(with: range), range: range)
            case .BlockQuote:
                if let fontAttribute = fontAttributes[range] {
            ...
            case .BankCard:
                ...
                string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.BankCard), value: nsString!.substring(with: range), range: range)
            case .Spoiler:
                if external {
                    string.addAttribute(NSAttributedString.Key.backgroundColor, value: UIColor.gray, range: range)
                } else {
                    string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.Spoiler), value: true as NSNumber, range: range)
                }
            case let .Custom(type):
                if type == ApplicationSpecificEntityType.Timecode {
                    ...
                    if let time = parseTimecodeString(text) {
                        string.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.Timecode), value: ElloAppTimecode(time: time, text: text), range: range)
                    }
                }
            case let .CustomEmoji(_, fileId):
                string.addAttribute(ChatTextInputAttributes.customEmoji, value: ChatTextInputTextCustomEmojiAttribute(interactivelySelectedFromPackId: nil, fileId: fileId, file: message?.associatedMedia[MediaId(namespace: Namespaces.Media.CloudFile, id: fileId)] as? ElloAppMediaFile), range: range)
            default:
                break
        }
