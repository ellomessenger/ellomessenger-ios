...
final class JoinLinkPreviewPeerContentNode: ASDisplayNode, ShareContentContainerNode {
    enum Content {
        case invite(isGroup: Bool, image: ElloAppMediaImageRepresentation?, title: String, memberCount: Int32, members: [EnginePeer])
        case request(isGroup: Bool, image: ElloAppMediaImageRepresentation?, title: String, about: String?, memberCount: Int32)
        ...
        var image: ElloAppMediaImageRepresentation? {
            switch self {
    ...
    init(context: AccountContext, theme: PresentationTheme, strings: PresentationStrings, content: JoinLinkPreviewPeerContentNode.Content) {
        ...
        let peer = ElloAppGroup(id: EnginePeer.Id(0), title: content.title, userName: nil, description: nil, photo: content.image.flatMap { [$0] } ?? [], participantCount: Int(content.memberCount), role: .member, membership: .Left, flags: [], defaultBannedRights: nil, migrationReference: nil, creationDate: 0, version: 0)
