...
final class ChatMediaGalleryThumbnailItem: GalleryThumbnailItem {
    ...
    init?(account: Account, mediaReference: AnyMediaReference) {
        ...
        if let imageReference = mediaReference.concrete(ElloAppMediaImage.self) {
            self.thumbnail = .image(imageReference)
        } else if let fileReference = mediaReference.concrete(ElloAppMediaFile.self) {
...
class ChatImageGalleryItem: GalleryItem {
    ...
    func node(synchronous: Bool) -> GalleryItemNode {
        ...
        for media in self.message.media {
            if let invoice = media as? ElloAppMediaInvoice, let extendedMedia = invoice.extendedMedia, case let .full(fullMedia) = extendedMedia, let image = fullMedia as? ElloAppMediaImage {
                ...
                node.setImage(imageReference: .message(message: MessageReference(self.message), media: image))
                break
            } else if let file = media as? ElloAppMediaFile, file.mimeType.hasPrefix("image/") {
    ...
    func thumbnailItem() -> (Int64, GalleryThumbnailItem)? {
        if let id = self.message.groupInfo?.stableId {
            ...
            for m in self.message.media {
                ...
                } else if let m = m as? ElloAppMediaFile {
                    mediaReference = .message(message: MessageReference(self.message), media: m)
                }
            }
...
final class ChatImageGalleryItemNode: ZoomableContentGalleryItemNode {
    ...
    override func visibilityUpdated(isVisible: Bool) {
        ...
        if let (_, mediaReference) = self.contextAndMedia, let _ = mediaReference.concrete(ElloAppMediaFile.self) {
            if isVisible {
            } else {
                self.fetchDisposable.set(nil)
            }
    ...
    @objc func statusPressed() {
        if let (_, mediaReference) = self.contextAndMedia, let status = self.status {
            ...
            if let fileReference = mediaReference.concrete(ElloAppMediaFile.self) {
            ...
            } else if let imageReference = mediaReference.concrete(ElloAppMediaImage.self ) {
