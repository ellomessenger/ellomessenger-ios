...
final class ChatItemGalleryFooterContentNode: GalleryFooterContentNode, UIScrollViewDelegate {
    ...
    private var currentWebPageAndMedia: (ElloAppMediaWebpage, Media)?
    ...
    init(context: AccountContext, presentationData: PresentationData, present: @escaping (ViewController, Any?) -> Void = { _, _ in }) {
        ...
        self.textNode.highlightAttributeAction = { attributes in
            let highlightedAttributes = [ElloAppTextAttributes.URL,
                                         ElloAppTextAttributes.PeerMention,
                                         ElloAppTextAttributes.PeerTextMention,
                                         ElloAppTextAttributes.BotCommand,
                                         ElloAppTextAttributes.Hashtag,
                                         ElloAppTextAttributes.Timecode]
    ...
    private func actionForAttributes(_ attributes: [NSAttributedString.Key: Any], _ index: Int) -> GalleryControllerInteractionTapAction? {
        if let url = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL)] as? String {
            var concealed = true
            if let (attributeText, fullText) = self.textNode.attributeSubstring(name: ElloAppTextAttributes.URL, index: index) {
                concealed = !doesUrlMatchText(url: url, text: attributeText, fullText: fullText)
            }
            return .url(url: url, concealed: concealed)
        } else if let peerMention = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.PeerMention)] as? ElloAppPeerMention {
            return .peerMention(peerMention.peerId, peerMention.mention)
        } else if let peerName = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.PeerTextMention)] as? String {
            return .textMention(peerName)
        } else if let botCommand = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.BotCommand)] as? String {
            return .botCommand(botCommand)
        } else if let hashtag = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.Hashtag)] as? ElloAppHashtag {
            return .hashtag(hashtag.peerName, hashtag.hashtag)
        } else if let timecode = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.Timecode)] as? ElloAppTimecode {
    ...
    func setMessage(_ message: Message, displayInfo: Bool = true) {
        ...
        for media in message.media {
            if media is ElloAppMediaImage {
                canEdit = true
            } else if let media = media as? ElloAppMediaFile, !media.isAnimated {
                var isVideo = false
            ...
            } else if let media = media as? ElloAppMediaWebpage, case let .Loaded(content) = media.content {
        ...
        if let peer = message.peers[message.id.peerId] {
            if peer is ElloAppUser || peer is ElloAppSecretChat {
                canDelete = true
        ...
        for media in message.media {
            ...
            } else if media is ElloAppMediaInvoice {
                hasCaption = true
            }
        }
        if hasCaption {
    ...
    func setWebPage(_ webPage: ElloAppMediaWebpage, media: Media) {
    ...
    override func updateLayout(size: CGSize, metrics: LayoutMetrics, leftInset: CGFloat, rightInset: CGFloat, bottomInset: CGFloat, contentInset: CGFloat, transition: ContainedViewLayoutTransition) -> CGFloat {
        ...
        if let image = self.backwardButton.backgroundIconNode.image {
            self.backwardButton.frame = CGRect(origin: CGPoint(x: floor((width - image.size.width) / 2.0) - 66.0,
                                                               y: panelHeight - bottomInset - 44.0 - 2),
                                               size: image.size)
        }
        
        if let image = self.forwardButton.backgroundIconNode.image {
            self.forwardButton.frame = CGRect(origin: CGPoint(x: floor((width - image.size.width) / 2.0) + 66.0, 
                                                              y: panelHeight - bottomInset - 44.0 - 2),
                                              size: image.size)
        }
        
        self.playbackControlButton.frame = CGRect(origin: CGPoint(x: floor((width - 44.0) / 2.0), 
                                                                  y: panelHeight - bottomInset - 44.0 - 2.0),
    ...
    @objc func deleteButtonPressed() {
        if let currentMessage = self.currentMessage {
            let _ = (self.context.engine.data.get(ElloAppEngine.EngineData.Item.Messages.MessageGroup(id: currentMessage.id))
    ...
    private func commitDeleteMessages(_ messages: [EngineMessage], ask: Bool) {
        self.messageContextDisposable.set((self.context.sharedContext.chatAvailableMessageActions(engine: self.context.engine, accountPeerId: self.context.account.peerId, messageIds: Set(messages.map { $0.id })) |> deliverOnMainQueue).start(next: { [weak self] actions in
            if let strongSelf = self, let controllerInteration = strongSelf.controllerInteraction, !actions.options.isEmpty {
                ...
                if let user = messages[0].peers[messages[0].id.peerId] as? ElloAppUser {
                    personalPeerName = EnginePeer(user).compactDisplayTitle
                } else if let channel = messages[0].peers[messages[0].id.peerId] as? ElloAppChannel, case .broadcast = channel.info {
                    isChannel = true
                }
                ...
                if let strongSelf = self, !messages.isEmpty {
                    ...
                    if messages.count == 1 {
                        ...
                        for m in messages[0].media {
                            if let image = m as? ElloAppMediaImage {
                            ...
                            } else if let webpage = m as? ElloAppMediaWebpage, case let .Loaded(content) = webpage.content {
                            ...
                            } else if let file = m as? ElloAppMediaFile {
...
private final class PlayPauseIconNode: ManagedAnimationNode {
    ...
    private let kPauseFrame = 30
    private let kPlayFrame = 83

    init() {
        super.init(size: CGSize(width: 40.0, height: 40.0))
        
        self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause"), frames: .range(startFrame: kPauseFrame, endFrame: kPauseFrame), duration: 0.01))
    }
        ...
        switch previousState {
            case .pause:
                switch state {
                    case .play:
                        if animated {
                            self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause"), frames: .range(startFrame: kPauseFrame, endFrame: kPlayFrame), duration: self.duration))
                        } else {
            ...
            case .play:
                switch state {
                    case .pause:
                        if animated {
                            self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause"), frames: .range(startFrame: 0, endFrame: kPauseFrame), duration: self.duration))
                        } else {
                            self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause"), frames: .range(startFrame: kPauseFrame, endFrame: kPauseFrame), duration: 0.01))
                        }
                    case .play:
