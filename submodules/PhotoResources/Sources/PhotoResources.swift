...
public func largestRepresentationForPhoto(_ photo: ElloAppMediaImage) -> ElloAppMediaImageRepresentation? {
...
public func representationFetchRangeForDisplayAtSize(representation: ElloAppMediaImageRepresentation, dimension: Int?) -> Range<Int64>? {
...
public func chatMessageWebFileInteractiveFetched(account: Account, image: ElloAppMediaWebFile) -> Signal<FetchResourceSourceType, FetchResourceError> {
...
public func chatMessageWebFileCancelInteractiveFetch(account: Account, image: ElloAppMediaWebFile) {
...
public func chatWebpageSnippetFile(account: Account, mediaReference: AnyMediaReference, representation: ElloAppMediaImageRepresentation) -> Signal<(TransformImageArguments) -> DrawingContext?, NoError> {
...
public func chatWebFileImage(account: Account, file: ElloAppMediaWebFile) -> Signal<(TransformImageArguments) -> DrawingContext?, NoError> {
...
public func albumArtThumbnailData(engine: ElloAppEngine, thumbnail: ExternalMusicAlbumArtResource, attemptSynchronously: Bool = false) -> Signal<Data?, NoError> {
...
private func albumArtFullSizeDatas(engine: ElloAppEngine, file: FileMediaReference?, thumbnail: ExternalMusicAlbumArtResource, fullSize: ExternalMusicAlbumArtResource, autoFetchFullSize: Bool = true) -> Signal<Tuple3<Data?, Data?, Bool>, NoError> {
    return engine.resources.custom(
...
public func playerAlbumArt(postbox: Postbox, 
                           ...
                           drawPlaceholderWhenEmpty: Bool = true,
                           attemptSynchronously: Bool = false) -> Signal<(TransformImageArguments) -> DrawingContext?, NoError> {
    ...
    if let fileReference = fileReference {
        ...
        |> then(
            postbox.mediaBox.cachedResourceRepresentation(fileReference.media.resource, 
                                                          representation: CachedAlbumArtworkRepresentation(size: size),
                                                          complete: false,
                                                          fetch: true)
            |> map { data -> Data? in
...
public func securePhoto(account: Account, resource: ElloAppMediaResource, accessContext: SecureIdAccessContext) -> Signal<(TransformImageArguments) -> DrawingContext?, NoError> {
    return securePhotoInternal(account: account, resource: resource, accessContext: accessContext) |> map { $0.1 }
}

public func securePhotoInternal(account: Account, resource: ElloAppMediaResource, accessContext: SecureIdAccessContext) -> Signal<(() -> CGSize?, (TransformImageArguments) -> DrawingContext?), NoError> {
