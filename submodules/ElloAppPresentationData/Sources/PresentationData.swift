...
public final class PresentationData: Equatable {
    ...
    public let chatWallpaper: ElloAppWallpaper
    ...
    public init(strings: PresentationStrings, theme: PresentationTheme, autoNightModeTriggered: Bool, chatWallpaper: ElloAppWallpaper, chatFontSize: PresentationFontSize, chatBubbleCorners: PresentationChatBubbleCorners, listsFontSize: PresentationFontSize, dateTimeFormat: PresentationDateTimeFormat, nameDisplayOrder: PresentationPersonNameOrder, nameSortOrder: PresentationPersonNameOrder, reduceMotion: Bool, largeEmoji: Bool) {
    ...
    public func withUpdated(chatWallpaper: ElloAppWallpaper) -> PresentationData {
    ...
    public func withUpdated(aiBotColorAccent: UIColor) -> PresentationData {
        let inputPanel = self.theme.chat.inputPanel.withUpdated(
            panelControlBackgroundColor: aiBotColorAccent,
            panelControlTextColor: .white,
            actionControlFillColor: aiBotColorAccent
        )
        var chat = self.theme.chat.withUpdated(inputPanel: inputPanel)
        let outgoing = chat.message.outgoing
        chat = chat.withUpdated(
            message: chat.message.withUpdated(
                outgoing: outgoing.withUpdated(
                    bubble: outgoing.bubble.withUpdated(
                        withWallpaper: outgoing.bubble.withWallpaper.withUpdated(
                            fill: [aiBotColorAccent],
                            highlightedFill: aiBotColorAccent
                        ),
                        withoutWallpaper: outgoing.bubble.withoutWallpaper.withUpdated(
                            fill: [aiBotColorAccent],
                            highlightedFill: aiBotColorAccent
                        )
                    )
        ...
        let theme = self.theme.withUpdated(chat: chat)
        return PresentationData(strings: self.strings, theme: theme, autoNightModeTriggered: self.autoNightModeTriggered, chatWallpaper: self.chatWallpaper, chatFontSize: self.chatFontSize, chatBubbleCorners: self.chatBubbleCorners, listsFontSize: self.listsFontSize, dateTimeFormat: self.dateTimeFormat, nameDisplayOrder: self.nameDisplayOrder, nameSortOrder: self.nameSortOrder, reduceMotion: self.reduceMotion, largeEmoji: self.largeEmoji)
    }
...
public func currentPresentationDataAndSettings(accountManager: AccountManager<ElloAppAccountManagerTypes>, systemUserInterfaceStyle: WindowUserInterfaceStyle) -> Signal<InitialPresentationDataAndSettings, NoError> {
    return accountManager.transaction { transaction -> InitialPresentationDataAndSettings in
        ...
        var preferredBaseTheme: ElloAppBaseTheme?
        ...
        let effectiveChatWallpaper: ElloAppWallpaper = (themeSettings.themeSpecificChatWallpapers[coloredThemeIndex(reference: effectiveTheme, accentColor: effectiveColors)] ?? themeSettings.themeSpecificChatWallpapers[effectiveTheme.index]) ?? theme.chat.defaultWallpaper
...
public func serviceColor(for wallpaper: (ElloAppWallpaper, UIImage?)) -> UIColor {
    switch wallpaper.0 {
        case .builtin:
...
private var serviceBackgroundColorForWallpaper: (ElloAppWallpaper, UIColor)?

public func chatServiceBackgroundColor(wallpaper: ElloAppWallpaper, mediaBox: MediaBox) -> Signal<UIColor, NoError> {
...
public func updatedPresentationData(accountManager: AccountManager<ElloAppAccountManagerTypes>, applicationInForeground: Signal<Bool, NoError>, systemUserInterfaceStyle: Signal<WindowUserInterfaceStyle, NoError>) -> Signal<PresentationData, NoError> {
    ...
    |> mapToSignal { sharedData, systemUserInterfaceStyle -> Signal<PresentationData, NoError> in
        ...
        let currentWallpaper: ElloAppWallpaper
