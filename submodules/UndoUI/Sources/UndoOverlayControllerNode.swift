...
final class UndoOverlayControllerNode: ViewControllerTracingNode {
    ...
    init(presentationData: PresentationData, content: UndoOverlayContent, elevatedLayout: Bool, placementPosition: UndoOverlayController.Position, action: @escaping (UndoOverlayAction) -> Bool, dismiss: @escaping () -> Void) {
        ...
        let undoTextColor = UIColor.white
        ...
        switch content {
            ...
            case let .paymentSent(currencyValue, itemTitle):
                ...
                displayUndo = false
                self.originalRemainingSeconds = 5
            case let .messagesUnpinned(title, text, undo, _):
                self.avatarNode = nil
                self.iconCheckNode = nil
                self.iconNode = ASImageNode()
                self.iconNode?.displayWithoutProcessing = true
                self.iconNode?.displaysAsynchronously = false
                self.iconNode?.image = generateTintedImage(image: UIImage(bundleImageName: "Chat/Input/Accessory Panels/PinnedIcon"), color: UIColor(rgb: 0x74747B))
//                self.animationNode = AnimationNode(animation: isHidden ? "anim_message_hidepin" : "anim_message_unpin", colors: ["info1.info1.stroke": UIColor.red, "info2.info2.Fill": UIColor.green], scale: 1.0)
                self.animationNode = nil
                self.animatedStickerNode = nil
                ...
                let link = MarkdownAttributeSet(font: Font.regular(14.0), textColor: undoTextColor)
                let attributedText = parseMarkdownIntoAttributedString(text, attributes: MarkdownAttributes(body: body, bold: bold, link: link, linkAttribute: { _ in return nil }), textAlignment: .natural)
            self.titleNode.attributedText = NSAttributedString(string: title, font: UIFont.Custom.sfProDisplay(ofSize: 16, weight: .regular), textColor: UIColor(rgb: 0x070708))
                if !text.isEmpty {
                    self.textNode.attributedText = attributedText
                }
            ...
            case let .stickersModified(title, text, undo, info, topItem, context):
                ...
                enum StickerPackThumbnailItem {
                    case still(ElloAppMediaImageRepresentation)
                ...
                if let thumbnail = info.thumbnail {
                    ...
                    } else if let dimensions = item.file.dimensions, let resource = chatMessageStickerResource(file: item.file, small: true) as? ElloAppMediaResource {
                        thumbnailItem = .still(ElloAppMediaImageRepresentation(dimensions: dimensions, resource: resource, progressiveSizes: [], immediateThumbnailData: nil))
            ...
            case let .sticker(context, file, title, text, customUndoText, _):
                ...
                } else if let dimensions = file.dimensions, let resource = chatMessageStickerResource(file: file, small: true) as? ElloAppMediaResource {
                    thumbnailItem = .still(ElloAppMediaImageRepresentation(dimensions: dimensions, resource: resource, progressiveSizes: [], immediateThumbnailData: nil))
        ...
        case .archivedChat, .hidArchive, .revealedArchive, .autoDelete, .succeed, .emoji, .swipeToReply, .actionSucceeded, .stickersModified, .chatAddedToFolder, .chatRemovedFromFolder, .setProximityAlert, .invitedToVoiceChat, .linkCopied, .banned, .importedMessage, .audioRate, .forward, .gigagroupConversion, .linkRevoked, .voiceChatRecording, .voiceChatFlag, .voiceChatCanSpeak, .copy, .mediaSaved, .paymentSent, .image, .inviteRequestSent, .notificationSoundAdded, .universal:
            if self.textNode.tapAttributeAction != nil || displayUndo {
                self.isUserInteractionEnabled = true
            } else {
                self.isUserInteractionEnabled = false
            }
        case .messagesUnpinned:
            self.panelNode.backgroundColor = UIColor(rgb: 0xDEDEE1)
    ...
    override func didLoad() {
        super.didLoad()
        switch content {
        case .removedChat,.archivedChat, .hidArchive, .revealedArchive, .autoDelete, .succeed, .emoji, .swipeToReply, .actionSucceeded, .stickersModified, .chatAddedToFolder, .chatRemovedFromFolder, .setProximityAlert, .invitedToVoiceChat, .linkCopied, .banned, .importedMessage, .audioRate, .forward, .gigagroupConversion, .linkRevoked, .voiceChatRecording, .voiceChatFlag, .voiceChatCanSpeak, .copy, .mediaSaved, .paymentSent, .image, .inviteRequestSent, .notificationSoundAdded, .universal, .info, .dice, .sticker:
