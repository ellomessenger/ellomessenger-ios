...
public final class ListMessageSnippetItemNode: ListMessageNode {
    ...
    private var currentIconImageRepresentation: ElloAppMediaImageRepresentation?
    ...
    override public func asyncLayout() -> (_ item: ListMessageItem, _ params: ListViewItemLayoutParams, _ mergedTop: Bool, _ mergedBottom: Bool, _ dateHeaderAtBottom: Bool) -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation) -> Void) {
        ...
        return { [weak self] item, params, _, _, dateHeaderAtBottom in
            ...
            var iconImageReferenceAndRepresentation: (AnyMediaReference, ElloAppMediaImageRepresentation)?
            ...
            var previewWallpaper: ElloAppWallpaper?
            var previewWallpaperFileReference: FileMediaReference?
            
            var selectedMedia: ElloAppMediaWebpage?
            var processed = false
                ...
                for media in message.media {
                    if let webpage = media as? ElloAppMediaWebpage {
                        ...
                        if case let .Loaded(content) = webpage.content {
                                ...
                                if content.type == "elloapp_background" {
                                    if let wallpaper = parseWallpaperUrl(content.url) {
                                        ...
                                        case let .slug(slug, _, colors, intensity, angle):
                                            ...
                                            previewWallpaper = .file(ElloAppWallpaper.File(id: file.fileId.id, accessHash: 0, isCreator: false, isDefault: false, isPattern: true, isDark: false, slug: slug, file: file, settings: WallpaperSettings(blur: false, motion: false, colors: colors, intensity: intensity, rotation: angle)))
                                        default:
                                            break
                                        }
                                    }
                            ...
                            urlString.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL), value: content.url, range: NSMakeRange(0, urlString.length))
                            linkText = urlString
                            ...
                            switch entity.type {
                                case .Url, .Email:
                                    ...
                                    if let url = parsedUrl, let host = host {
                                        ...
                                        urlAttributedString.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL), value: urlString, range: NSMakeRange(0, urlAttributedString.length))
            ...
            if currentIconImageRepresentation != iconImageReferenceAndRepresentation?.1 {
                if let previewWallpaper = previewWallpaper, let fileReference = previewWallpaperFileReference {
                    ...
                    if let imageReference = iconImageReferenceAndRepresentation.0.concrete(ElloAppMediaImage.self) {
                        updateIconImageSignal = chatWebpageSnippetPhoto(account: item.context.account, photoReference: imageReference)
                    } else if let fileReference = iconImageReferenceAndRepresentation.0.concrete(ElloAppMediaFile.self) {
    ...
    func activateMedia() {
        if let item = self.item, let message = item.message, let currentPrimaryUrl = self.currentPrimaryUrl {
            if let webpage = self.currentMedia as? ElloAppMediaWebpage, case let .Loaded(content) = webpage.content {
                if content.instantPage != nil {
                    ...
                    if isElloAppMeLink(content.url) || !item.interaction.openMessage(message, .link) {
                        ...
                        case .tap, .longTap:
                            if let item = self.item, let message = item.message, let url = self.urlAtPoint(location) {
                                if case .longTap = gesture {
                                    ...
                                    if let webpage = self.currentMedia as? ElloAppMediaWebpage, case let .Loaded(content) = webpage.content, content.instantPage != nil {
                                        item.interaction.openInstantPage(message, nil)
                                    } else {
