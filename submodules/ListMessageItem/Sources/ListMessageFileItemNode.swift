...
private enum FileIconImage: Equatable {
    case imageRepresentation(Media, ElloAppMediaImageRepresentation)
    case albumArt(ElloAppMediaFile, SharedMediaPlaybackAlbumArt)
    case roundVideo(ElloAppMediaFile)
        ...
        return { [weak self] item, params, mergedTop, mergedBottom, dateHeaderAtBottom in
            ...
            if let message = message, let selectedMedia = selectedMedia {
                if mediaUpdated {
                    ...
                    updatedFetchControls = FetchControls(fetch: { [weak self] in
                        if let strongSelf = self {
                            if let file = selectedMedia as? ElloAppMediaFile {
                            ...
                            } else if let image = selectedMedia as? ElloAppMediaImage, let representation = image.representations.last {
                ...
                if statusUpdated && item.displayFileInfo {
                    ...
                    } else if let image = selectedMedia as? ElloAppMediaImage {
                    ...
                    if let iconImage = iconImage {
                        switch iconImage {
                            ...
                            case let .albumArt(file, albumArt):
                                updateIconImageSignal = playerAlbumArt(postbox: item.context.account.postbox, 
                                                                       engine: item.context.engine,
                                                                       fileReference: .message(message: MessageReference(message),
                                                                                               media: file),
                                                                       albumArt: albumArt,
                                                                       thumbnail: true,
                                                                       overlayColor: UIColor(white: 0.0, alpha: 0.3),
                                                                       emptyColor: item.presentationData.theme.theme.list.itemAccentColor)
                            case let .roundVideo(file):
            ...
            return (nodeLayout, { animation in
                if let strongSelf = self {
                    ...
                    if item.message == nil {
                        ...
                        if let media = selectedMedia as? ElloAppMediaFile, media.isInstantVideo {
    ...
    private func updateProgressFrame(size: CGSize, leftInset: CGFloat, rightInset: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        if let resourceStatus = self.resourceStatus {
            ...
            case .Fetching(_, let progress), .Paused(let progress):
                if let file = self.currentMedia as? ElloAppMediaFile, let size = file.size {
