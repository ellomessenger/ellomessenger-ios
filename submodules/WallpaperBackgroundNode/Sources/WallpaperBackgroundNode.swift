...
private func generateBlurredContents(image: UIImage) -> UIImage? {
    ...
    elloappFastBlurMore(Int32(context.size.width), Int32(context.size.height), Int32(context.bytesPerRow), context.bytes)
    elloappFastBlurMore(Int32(context.size.width), Int32(context.size.height), Int32(context.bytesPerRow), context.bytes)
...
public protocol WallpaperBackgroundNode: ASDisplayNode {
    ...
    func update(wallpaper: ElloAppWallpaper)
...
//                        needsCleanBackground = false
    ...
    public var blurredBackgroundContents: UIImage?
    ...
    public let bakedBackgroundView: UIImageView
    ...
    func hasBubbleBackground(for type: WallpaperBubbleType) -> Bool {
        ...
        let graphicsWallpaper: ElloAppWallpaper
    ...
    func update(wallpaper: ElloAppWallpaper) {
        ...
        case let .file(file):
            ...
            if let dimensions = file.file.dimensions {
                let representation = ElloAppMediaImageRepresentation(dimensions: dimensions, resource: file.file.resource, progressiveSizes: [], immediateThumbnailData: file.file.immediateThumbnailData)
...
public func createWallpaperBackgroundNode(context: AccountContext, 
                                          forChatDisplay: Bool,
                                          chatLocation: ChatLocation? = nil,
                                          useSharedAnimationPhase: Bool = false,
                                          useExperimentalImplementation: Bool = false) -> WallpaperBackgroundNode {
    ...
    let wallpaper = WallpaperBackgroundNodeImpl(context: context, useSharedAnimationPhase: false)
    
    if case let .peer(id) = chatLocation {
        if id == PeerId.aiPhoenixBotPeerID {
            let imageView = UIImageView(image: UIImage(named: "Wallet/bot-phoenix-background",
                                                       in: Bundle(for: WallpaperBackgroundNodeMergedImpl.self),
                                                       with: nil)!)
            imageView.contentMode = .scaleAspectFill
            imageView.frame = wallpaper.contentNode.view.bounds
            imageView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
            wallpaper.contentNode.view.addSubview(imageView)
        }
        else if id == PeerId.aiBusinessForStudentsBotPeerID {
            let imageView = UIImageView(image: UIImage(named: "Wallet/bot-businessForStudents-background",
                                                       in: Bundle(for: WallpaperBackgroundNodeMergedImpl.self),
                                                       with: nil)!)
            imageView.contentMode = .scaleAspectFill
            imageView.frame = wallpaper.contentNode.view.bounds
        ...
        else if id == PeerId.aiCancerPreventationBotPeerID {
            let imageView = UIImageView(image: UIImage(named: "Wallet/bot-cancerPreventation-background",
                                                       in: Bundle(for: WallpaperBackgroundNodeMergedImpl.self),
                                                       with: nil)!)
            imageView.contentMode = .scaleAspectFill
            imageView.frame = wallpaper.contentNode.view.bounds
