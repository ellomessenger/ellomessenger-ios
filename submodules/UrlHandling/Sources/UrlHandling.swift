...
private let baseElloAppMePaths = ["ello.team"]
private let baseElloPhPaths = [
    "ello.team/iv?",
    "elloapp.com/blog/",
    "elloapp.com/tour/"
]
            ...
            if pathComponents.count == 1 {
                if let queryItems = components.queryItems {
                    if peerName == "socks" || peerName == "proxy" {
                        ...
                        if let _ = url {
                            return .internalInstantView(url: "https://ello.team/\(query)")
                        }
                    ...
                    } else if peerName == "invite_referral" {
                        if let referralCode = queryItems.first(where: { "code" == $0.name })?.value {
...
private func resolveInternalUrl(context: AccountContext, url: ParsedInternalUrl) -> Signal<ResolvedUrl?, NoError> {
    switch url {
        ...
        case let .privateMessage(messageId, threadId, timecode):
            return context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: messageId.peerId))
            |> mapToSignal { peer -> Signal<ResolvedUrl?, NoError> in
                let foundPeer: Signal<EnginePeer?, NoError>
                if let peer = peer {
                    foundPeer = .single(peer)
                } else {
                    foundPeer = ElloAppEngine(account: context.account).peers.findChannelById(channelId: messageId.peerId.id._internalGetInt64Value())
                }
                return foundPeer
        ...
        case let .invoice(slug):
            ...
            |> map(Optional.init)
            |> `catch` { _ -> Signal<ElloAppMediaInvoice?, NoError> in
                return .single(nil)
            }
        ...
        case .referralCode(let code):
            return .single(.referralCode(code: code))
        }
}

public func isElloAppMeLink(_ url: String) -> Bool {
    let schemes = ["http://", "https://", ""]
    for basePath in baseElloAppMePaths {
...
public func isElloPhLink(_ url: String) -> Bool {
    let schemes = ["http://", "https://", ""]
    for basePath in baseElloAppMePaths {
        for scheme in schemes {
            let basePrefix = scheme + basePath + "/"
...
public func parseReferralCode(_ url: String) -> (String?)? {
    let schemes = ["http://", "https://", ""]
    for basePath in baseElloAppMePaths {
        for scheme in schemes {
            let basePrefix = scheme + basePath + "/"
            if url.lowercased().hasPrefix(basePrefix) {
                if let internalUrl = parseInternalUrl(query: String(url[basePrefix.endIndex...])), case let .referralCode(code) = internalUrl {
                    return (code)
                }
            }
        }
...
public func resolveUrlImpl(context: AccountContext, peerId: PeerId?, url: String, skipUrlAuth: Bool) -> Signal<ResolvedUrl, NoError> {
    ...
    |> mapToSignal { linkPreviews -> Signal<ResolvedUrl, NoError> in
        return context.engine.data.get(ElloAppEngine.EngineData.Item.Configuration.App())
        |> mapToSignal { appConfiguration -> Signal<ResolvedUrl, NoError> in
            ...
            for basePath in baseElloPhPaths {
                for scheme in schemes {
                    let basePrefix = scheme + basePath
                    if url.lowercased().hasPrefix(basePrefix) {
                        return resolveInstantViewUrl(account: context.account, url: url)
