...
public enum PeerInfoAvatarListItem: Equatable {
    ...
    case image(ElloAppMediaImageReference?, [ImageRepresentationWithReference], [VideoRepresentationWithReference], Data?)
...
public final class PeerInfoAvatarListContainerNode: ASDisplayNode {
    ...
    /*private*/public var activeStripNode: ASImageNode
    ...
    public func deleteItem(_ item: PeerInfoAvatarListItem) -> Bool {
        ...
        if let peer = self.peer, peer is ElloAppGroup || peer is ElloAppChannel, deletedIndex == 0 {
            self.galleryEntries = []
            self.items = []
            self.itemsUpdated?([])
            self.currentIndex = 0
    ...
    public func update(size: CGSize, peer: Peer?, customNode: ASDisplayNode? = nil, additionalEntry: Signal<(ElloAppMediaImageRepresentation, Float)?, NoError> = .single(nil), isExpanded: Bool, transition: ContainedViewLayoutTransition) {
