...
public enum AvatarGalleryEntry: Equatable {
    ...
    case image(MediaId, ElloAppMediaImageReference?, [ImageRepresentationWithReference], [VideoRepresentationWithReference], Peer?, Int32?, GalleryItemIndexData?, MessageId?, Data?, String?)
    
    public init(representation: ElloAppMediaImageRepresentation, peer: Peer) {
...
public func initialAvatarGalleryEntries(account: Account, engine: ElloAppEngine, peer: Peer) -> Signal<[AvatarGalleryEntry]?, NoError> {
    ...
    if peer is ElloAppChannel || peer is ElloAppGroup, let peerReference = PeerReference(peer) {
        return engine.data.get(ElloAppEngine.EngineData.Item.Peer.Photo(id: peer.id))
        |> map { peerPhoto in
            var initialPhoto: ElloAppMediaImage?
...
public func fetchedAvatarGalleryEntries(engine: ElloAppEngine, account: Account, peer: Peer) -> Signal<[AvatarGalleryEntry], NoError> {
...
public func fetchedAvatarGalleryEntries(engine: ElloAppEngine, account: Account, peer: Peer, firstEntry: AvatarGalleryEntry) -> Signal<(Bool, [AvatarGalleryEntry]), NoError> {
        ...
        |> map { photos -> (Bool, [AvatarGalleryEntry]) in
            ...
            if photos.isEmpty {
                ...
                if [Namespaces.Peer.CloudGroup, Namespaces.Peer.CloudChannel].contains(peer.id.namespace) {
                    ...
                    let photosForResutls: [ElloAppPeerPhoto]
                    if let avatarPhoto = photos.first(where: { photo in
                        guard let imageID = photo.image.id?.id else {
                            return false
                        }
                        return firstEntry.representations.first?.representation.resource.id.stringRepresentation.contains("\(imageID)") ?? false
                    }) {
                        var replacingPhotos = photos
                        replacingPhotos.removeAll(where: { $0.image.imageId == avatarPhoto.image.imageId })
                        replacingPhotos.insert(avatarPhoto, at: 0)
                        photosForResutls = replacingPhotos
                    } else {
                        photosForResutls = photos
                    }
                    for photo in photosForResutls {
                        let indexData = GalleryItemIndexData(position: index, totalCount: Int32(photos.count))
                        if result.isEmpty, let first = initialEntries.first {
                            result.append(.image(photo.image.imageId, photo.image.reference, first.representations, photo.image.videoRepresentations.map({ VideoRepresentationWithReference(representation: $0, reference: MediaResourceReference.avatarList(peer: peerReference, resource: $0.resource)) }), peer, photo.date, indexData, photo.messageId, photo.image.immediateThumbnailData, nil))
                        } else {
...
public class AvatarGalleryController: ViewController, StandalonePresentableController {
    ...
    private var canDelete: Bool {
        ...
        if self.peer.id == self.context.account.peerId {
            canDelete = true
        } else if let group = self.peer as? ElloAppGroup {
            switch group.role {
                case .creator, .admin:
                    canDelete = true
                case .member:
        ...
        } else if let channel = self.peer as? ElloAppChannel {
    ...
    private func deleteEntry(_ rawEntry: AvatarGalleryEntry) {
        ...
        let items: [ActionSheetItem] = [
            ActionSheetButtonItem(title: "Remote TEST", color: .destructive, action: { [weak actionSheet] in
                actionSheet?.dismissAnimated()
                proceed()
            })
        ]
