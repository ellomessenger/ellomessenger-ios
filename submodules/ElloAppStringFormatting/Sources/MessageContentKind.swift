...
public func messageTextWithAttributes(message: EngineMessage) -> NSAttributedString {
    ...
    if let entities = entities?.entities {
        ...
        for entity in entities.sorted(by: { $0.range.lowerBound > $1.range.lowerBound }) {
            ...
            updatedAttributes[ChatTextInputAttributes.customEmoji] = ChatTextInputTextCustomEmojiAttribute(interactivelySelectedFromPackId: nil, fileId: fileId, file: message.associatedMedia[MediaId(namespace: Namespaces.Media.CloudFile, id: fileId)] as? ElloAppMediaFile)
