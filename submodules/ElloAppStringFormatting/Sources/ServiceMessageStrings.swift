...
private func spoilerAttributes(primaryTextColor: UIColor) -> MarkdownAttributeSet {
    return MarkdownAttributeSet(font: titleFont, textColor: primaryTextColor, additionalAttributes: [ElloAppTextAttributes.Spoiler: true])
}
...
private func peerMentionAttributes(primaryTextColor: UIColor, peerId: EnginePeer.Id) -> MarkdownAttributeSet {
    return MarkdownAttributeSet(font: titleBoldFont, textColor: primaryTextColor, additionalAttributes: [ElloAppTextAttributes.PeerMention: ElloAppPeerMention(peerId: peerId, mention: "")])
}
...
public func universalServiceMessageString(presentationData: (PresentationTheme, ElloAppWallpaper)?, 
                                          strings: PresentationStrings,
                                          nameDisplayOrder: PresentationPersonNameOrder,
                                          dateTimeFormat: PresentationDateTimeFormat,
                                          message: EngineMessage,
                                          accountPeerId: EnginePeer.Id,
                                          forChatList: Bool) -> NSAttributedString? {
    ...
    for media in message.media {
        if let action = media as? ElloAppMediaAction {
            ...
            var isChannel = false
            var isPaid = false
            if message.id.peerId.namespace == Namespaces.Peer.CloudChannel, let peer = message.peers[message.id.peerId] as? ElloAppChannel, case .broadcast = peer.info {
                isChannel = true
                isCourse = peer.isCourse
                isPaid = peer.isPaid
            }
            
            switch action.action {
            case let .groupCreated(title):
                attributedString = if isChannel {
                    if isCourse {
                        NSAttributedString(
                            string: strings.Notification_CreatedCourse,
                            font: titleFont,
                            textColor: primaryTextColor)
                    } else if isPaid {
                        NSAttributedString(
                            string: strings.Notification_CreatedPaidChannel, 
                            font: titleFont,
                            textColor: primaryTextColor)
                    } else {
                        NSAttributedString(
                            string: strings.Notification_CreatedChannel, 
                            font: titleFont,
                            textColor: primaryTextColor)
                    }
                } else {
                    if /*forChatList*/"".isEmpty {
                        NSAttributedString(
                            string: strings.Notification_CreatedGroup,
                            font: titleFont,
                            textColor: primaryTextColor)
                    } else {
                        addAttributesToStringWithRanges(
                            strings.Notification_CreatedChatWithTitle(authorName, title)._tuple,
                            body: bodyAttributes,
                            argumentAttributes: peerMentionsAttributes(
                                primaryTextColor: primaryTextColor,
                                peerIds: [(0, message.author?.id)])
                        )
                    }
                }
            case let .addedMembers(peerIds):
                if let peerId = peerIds.first, peerId == message.author?.id {
                    if let peer = message.peers[message.id.peerId] as? ElloAppChannel, case .broadcast = peer.info {
            ...
            case .pinnedMessageUpdated:
                enum PinnnedMediaType {
                    ...
                    case location
                    case contact
                    case poll(ElloAppMediaPollKind)
                    case deleted
                }
                    ...
                    for entity in entities {
                        ...
                        if length > 0 {
                            ...
                            case let .CustomEmoji(_, fileId):
                                let index = ranges.count
                                ranges.append((ranges.count, NSRange(location: location, length: length)))
                                attributes[index] = customEmojiAttributes(primaryTextColor: primaryTextColor, emoji: ChatTextInputTextCustomEmojiAttribute(interactivelySelectedFromPackId: nil, fileId: fileId, file: message.associatedMedia[MediaId(namespace: Namespaces.Media.CloudFile, id: fileId)] as? ElloAppMediaFile))
                            default:
                                break
                            }
                        }
            ...
            case let .messageAutoremoveTimeoutUpdated(timeout):
                ...
                if timeout > 0 {
                    ...
                    let string: String
                    if let _ = messagePeer as? ElloAppUser {
                        if message.author?.id == accountPeerId {
                    ...
                    } else if let _ = messagePeer as? ElloAppGroup {
                        if message.author?.id == accountPeerId {
                            string = strings.Conversation_AutoremoveTimerSetUserYou(timeValue).string
                        } else {
                            string = strings.Conversation_AutoremoveTimerSetGroup(timeValue).string
                        }
                    } else if let channel = messagePeer as? ElloAppChannel {
                        if message.author?.id == accountPeerId {
                            string = strings.Conversation_AutoremoveTimerSetUserYou(timeValue).string
                        } else {
            ...
            case let .gameScore(gameId: _, score):
                ...
                inner: for attribute in message.attributes {
                    if let attribute = attribute as? ReplyMessageAttribute, let message = message.associatedMessages[attribute.messageId] {
                        for media in message.media {
                            if let game = media as? ElloAppMediaGame {
            ...
            case let .paymentSent(currency, totalAmount, _, isRecurringInit, isRecurringUsed):
                ...
                if let invoiceMessage = invoiceMessage {
                    for media in invoiceMessage.media {
                        if let invoice = media as? ElloAppMediaInvoice {
            ...
            break
        } else if let expiredMedia = media as? ElloAppMediaExpiredContent {
