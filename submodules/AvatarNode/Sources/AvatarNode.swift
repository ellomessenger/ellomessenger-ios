...
private let aiBotIcon = UIImage(bundleImageName: "Avatar/AIBotIcon")?.precomposed()
private let aiPhoenixBotIcon = UIImage(bundleImageName: "Avatar/AIPhoenixBotIcon")?.precomposed()
private let aiBusinessForStudentsBot = UIImage(bundleImageName: "Avatar/AIBusinessForStudentsBot")?.precomposed()
private let aiCancerPrevention = UIImage(bundleImageName: "Avatar/AICancerPrevention")?.precomposed()
private let notificationsIcon = UIImage(bundleImageName: "Avatar/notificationsIcon")?.precomposed()
private let supportAvatarIcon = UIImage(bundleImageName: "Avatar/supportAvatarIcon")?.precomposed()
private let phoneIcon = generateTintedImage(image: UIImage(bundleImageName: "Avatar/PhoneIcon"), color: .white)
public let savedMessagesIcon = UIImage(bundleImageName: "Avatar/SavedMessagesIcon")?.precomposed()
...
private let forwardIcon = UIImage(bundleImageName: "Avatar/ForwardMessages")?.precomposed()
...
public enum AvatarNodeClipStyle {
    ...
    public static let defaultCornerRadius:CGFloat = 40.0
    public static let defaultCorner:AvatarNodeClipStyle = .corner(AvatarNodeClipStyle.defaultCornerRadius)
}
...
private class AvatarNodeParameters: NSObject {
    ...
    func withUpdatedHasImage(_ hasImage: Bool) -> AvatarNodeParameters {
        return AvatarNodeParameters(theme: self.theme, 
                                    accountPeerId: self.accountPeerId,
                                    peerId: self.peerId,
                                    letters: self.letters,
                                    font: self.font,
                                    icon: self.icon,
                                    explicitColorIndex: self.explicitColorIndex,
                                    hasImage: hasImage,
                                    clipStyle: self.clipStyle)
    }
}
...
private enum AvatarNodeState: Equatable {
    case empty
    case peerAvatar(EnginePeer.Id, [String], ElloAppMediaImageRepresentation?)
...
private enum AvatarNodeIcon: Equatable {
    ...
    case phoneIcon
    case aiBotIcon
    case aiPhoenixBotIcon
    case aiBusinessForStudentsBotIcon
    case aiCancerPreventionBotIcon
    case notificationsIcon
    case supportAvatarIcon
}

public enum AvatarNodeImageOverride: Equatable {
    case none
    case image(ElloAppMediaImageRepresentation)
    case imageWithEditIcon(ElloAppMediaImageRepresentation)
    case savedMessagesIcon
    case repliesIcon
    case archivedChatsIcon(hiddenByDefault: Bool)
...
public final class AvatarNode: ASDisplayNode {
    ...
    public func setPeer(context genericContext: AccountContext,
                        account: Account? = nil,
                        theme: PresentationTheme = defaultPresentationTheme,
                        peer: EnginePeer?,
        ...
        var representation: ElloAppMediaImageRepresentation?
        var icon = AvatarNodeIcon.none
        
        if peer?.isAIBot ?? false {
            representation = nil
            icon = .aiBotIcon
        }
 
        if peer?.isAIPhoenixBot ?? false {
            representation = nil
            icon = .aiPhoenixBotIcon
        }
        
        if peer?.isAIBusinessForStudentsBot ?? false {
            representation = nil
            icon = .aiBusinessForStudentsBotIcon
        }
        
        if peer?.isAICancerPreventationBot ?? false {
            representation = nil
            icon = .aiCancerPreventionBotIcon
        }

        if peer?.isNotificationsBot ?? false {
            representation = nil
            icon = .notificationsIcon
        }

        if peer?.id == PeerId(776998) {
            representation = nil
            icon = .supportAvatarIcon
        }
        ...
        if let overrideImage = overrideImage {
            ...
            case let .imageWithEditIcon(image):
                representation = image
                synchronousLoad = false
                icon = .editAvatarIcon
            case .savedMessagesIcon:
        ...
        if updatedState != self.state || overrideImage != self.overrideImage || theme !== self.theme {
            ...
            if let peer, !peer.isAIBot, !peer.isAIPhoenixBot,
                let signal = peerAvatarImage(account: account,
                                             ...
                                             provideUnrounded: storeUnrounded) {
                self.contents = nil
                ...
                parameters = AvatarNodeParameters(theme: theme,
                ...
                parameters = AvatarNodeParameters(theme: theme,
                                                  accountPeerId: account.peerId,
                                                  peerId: peer?.id ?? EnginePeer.Id(0),
                                                  letters: peer?.displayLetters ?? [],
                                                  font: self.font,
                                                  icon: icon,
                                                  explicitColorIndex: nil,
    ...
    @objc override public class func draw(_ bounds: CGRect, withParameters parameters: Any?, isCancelled: () -> Bool, isRasterizing: Bool) {
        ...
        if let parameters = parameters as? AvatarNodeParameters {
            ...
            case .corner(let value):
                context.beginPath()
                context.addPath(roundedRect(CGRect(x: 0, y: 0, width: bounds.size.width, height: bounds.size.height), round: value))
                context.clip()
            }
        ...
        if let parameters = parameters as? AvatarNodeParameters, parameters.icon != .none {
            ...
            } else if parameters.icon == .aiBotIcon {
                colorsArray = [UIColor.clear.cgColor, UIColor.clear.cgColor]
            } else if parameters.icon == .aiPhoenixBotIcon {
                colorsArray = [UIColor.clear.cgColor, UIColor.clear.cgColor]
            } else if parameters.icon == .aiBusinessForStudentsBotIcon {
                colorsArray = [UIColor.clear.cgColor, UIColor.clear.cgColor]
            } else if parameters.icon == .aiCancerPreventionBotIcon {
                colorsArray = [UIColor.clear.cgColor, UIColor.clear.cgColor]
            } else {
                colorsArray = grayscaleColors
            }
        ...
        if let parameters = parameters as? AvatarNodeParameters {
            if case .aiBotIcon = parameters.icon, let aiBotIcon {
                let factor = bounds.size.width / aiBotIcon.size.width
                context.translateBy(x: bounds.size.width / 2.0, y: bounds.size.height / 2.0)
                ...
                context.draw(
                    ...
                    in: CGRect(
                        origin: CGPoint(
                            x: floor((bounds.size.width - aiBotIcon.size.width) / 2.0),
                            y: floor((bounds.size.height - aiBotIcon.size.height) / 2.0)
                        ),
                        size: aiBotIcon.size
                    )
                )
            } else if case .aiPhoenixBotIcon = parameters.icon, let aiPhoenixBotIcon {
                    let factor = bounds.size.width / aiPhoenixBotIcon.size.width
                    context.translateBy(x: bounds.size.width / 2.0, y: bounds.size.height / 2.0)
                    context.scaleBy(x: factor, y: -factor)
                    context.translateBy(x: -bounds.size.width / 2.0, y: -bounds.size.height / 2.0)
                    
                    context.draw(
                        aiPhoenixBotIcon.cgImage!,
                        in: CGRect(
                            origin: CGPoint(
                                x: floor((bounds.size.width - aiPhoenixBotIcon.size.width) / 2.0),
                                y: floor((bounds.size.height - aiPhoenixBotIcon.size.height) / 2.0)
                            ),
                            size: aiPhoenixBotIcon.size
                        )
                    )
            } else if case .aiBusinessForStudentsBotIcon = parameters.icon, let aiBusinessForStudentsBot {
                    let factor = bounds.size.width / aiBusinessForStudentsBot.size.width
                    context.translateBy(x: bounds.size.width / 2.0, y: bounds.size.height / 2.0)
                    context.scaleBy(x: factor, y: -factor)
                    context.translateBy(x: -bounds.size.width / 2.0, y: -bounds.size.height / 2.0)
                    
                    context.draw(
                        aiBusinessForStudentsBot.cgImage!,
                        in: CGRect(
                            origin: CGPoint(
                                x: floor((bounds.size.width - aiBusinessForStudentsBot.size.width) / 2.0),
                                y: floor((bounds.size.height - aiBusinessForStudentsBot.size.height) / 2.0)
                            ),
                            size: aiBusinessForStudentsBot.size
                        )
                    )
            } else if case .aiCancerPreventionBotIcon = parameters.icon, let aiCancerPrevention {
                    let factor = bounds.size.width / aiCancerPrevention.size.width
                    context.translateBy(x: bounds.size.width / 2.0, y: bounds.size.height / 2.0)
                    context.scaleBy(x: factor, y: -factor)
                    context.translateBy(x: -bounds.size.width / 2.0, y: -bounds.size.height / 2.0)
                    
                    context.draw(
                        aiCancerPrevention.cgImage!,
                        in: CGRect(
                            origin: CGPoint(
                                x: floor((bounds.size.width - aiCancerPrevention.size.width) / 2.0),
                                y: floor((bounds.size.height - aiCancerPrevention.size.height) / 2.0)
                            ),
                            size: aiCancerPrevention.size
                        )
                    )
            } else if case .supportAvatarIcon = parameters.icon {
                let factor = bounds.size.width / 60.0
                context.translateBy(x: bounds.size.width / 2.0, y: bounds.size.height / 2.0)
                context.scaleBy(x: factor, y: -factor)
                context.translateBy(x: -bounds.size.width / 2.0, y: -bounds.size.height / 2.0)
                
                if let supportAvatarIcon {
                    context.draw(supportAvatarIcon.cgImage!, in: CGRect(origin: CGPoint(x: floor((bounds.size.width - supportAvatarIcon.size.width) / 2.0), y: floor((bounds.size.height - supportAvatarIcon.size.height) / 2.0)), size: supportAvatarIcon.size))
                }
            } else if case .notificationsIcon = parameters.icon, let notificationsIcon {
                let factor = bounds.size.width / notificationsIcon.size.width
                context.translateBy(x: bounds.size.width / 2.0, y: bounds.size.height / 2.0)
                context.scaleBy(x: factor, y: -factor)
                context.translateBy(x: -bounds.size.width / 2.0, y: -bounds.size.height / 2.0)
                context.draw(notificationsIcon.cgImage!, in: CGRect(origin: CGPoint(x: floor((bounds.size.width - notificationsIcon.size.width) / 2.0), y: floor((bounds.size.height - notificationsIcon.size.height) / 2.0)), size: notificationsIcon.size))
            } else if case .deletedIcon = parameters.icon, let deletedIcon {
                let factor = bounds.size.width / deletedIcon.size.width
                context.translateBy(x: bounds.size.width / 2.0, y: bounds.size.height / 2.0)
                context.scaleBy(x: factor, y: -factor)
                context.translateBy(x: -bounds.size.width / 2.0, y: -bounds.size.height / 2.0)
            ...
            } else if case .savedMessagesIcon = parameters.icon, let savedMessagesIcon {
                let factor = bounds.size.width / savedMessagesIcon.size.width
                context.translateBy(x: bounds.size.width / 2.0, y: bounds.size.height / 2.0)
                context.scaleBy(x: factor, y: -factor)
                context.translateBy(x: -bounds.size.width / 2.0, y: -bounds.size.height / 2.0)
            ...
            } else if case .repliesIcon = parameters.icon, let forwardIcon {
                let factor = bounds.size.width / forwardIcon.size.width
                context.translateBy(x: bounds.size.width / 2.0, y: bounds.size.height / 2.0)
                context.scaleBy(x: factor, y: -factor)
                context.translateBy(x: -bounds.size.width / 2.0, y: -bounds.size.height / 2.0)
                
                context.draw(forwardIcon.cgImage!, in: CGRect(origin: CGPoint(x: floor((bounds.size.width - forwardIcon.size.width) / 2.0), y: floor((bounds.size.height - forwardIcon.size.height) / 2.0)), size: forwardIcon.size))
...
public func drawPeerAvatarLetters(context: CGContext, 
                                  size: CGSize,
                                  clipStyle: AvatarNodeClipStyle = .round,
                                  font: UIFont,
                                  letters: [String],
    ...
    case .corner(let radius):
        context.beginPath()
        context.addPath(CGPath(roundedRect: CGRect(x: 0, y: 0, width: size.width, height: size.height),
                               cornerWidth: radius,
                               cornerHeight: radius,
                               transform: nil))
        context.clip()
    ...
    let lineOffset = CGPoint(x: string == "B" ? 1.0 : 0.0, y: 0.0)
    let lineOrigin = CGPoint(x: floorToScreenPixels(-lineBounds.origin.x + (size.width - lineBounds.size.width) / 2.0) + lineOffset.x, 
