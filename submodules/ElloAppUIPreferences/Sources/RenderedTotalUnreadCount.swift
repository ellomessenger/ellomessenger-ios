...
public func renderedTotalUnreadCount(accountManager: AccountManager<ElloAppAccountManagerTypes>, engine: ElloAppEngine) -> Signal<(Int32, RenderedTotalUnreadCountType), NoError> {
    return combineLatest(
        accountManager.sharedData(keys: [ApplicationSpecificSharedDataKeys.inAppNotificationSettings]),
        engine.data.subscribe(
            ElloAppEngine.EngineData.Item.Messages.TotalReadCounters()
        )
    )
