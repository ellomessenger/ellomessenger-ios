...
private func botReceiptControllerEntries(presentationData: PresentationData, invoice: ElloAppMediaInvoice?, formInvoice: BotPaymentInvoice?, formInfo: BotPaymentRequestedInfo?, shippingOption: BotPaymentShippingOption?, paymentMethodTitle: String?, botPeer: EnginePeer?, tipAmount: Int64?) -> [BotReceiptEntry] {
...
final class BotReceiptControllerNode: ItemListControllerNode {
    ...
    private let receiptData = Promise<(BotPaymentInvoice, BotPaymentRequestedInfo?, BotPaymentShippingOption?, String?, ElloAppMediaInvoice, Int64?)?>(nil)
    ...
    init(controller: ItemListController?, navigationBar: NavigationBar, context: AccountContext, messageId: EngineMessage.Id, dismissAnimated: @escaping () -> Void) {
        ...
        let signal: Signal<(ItemListPresentationData, (ItemListNodeState, Any)), NoError> = combineLatest(
            ...
            context.engine.data.get(
                ElloAppEngine.EngineData.Item.Peer.Peer(id: messageId.peerId)
            )
        )
