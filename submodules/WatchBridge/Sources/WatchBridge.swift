...
private func makeBridgeImage(_ image: ElloAppMediaImage?) -> TGBridgeImageMediaAttachment? {
...
func makeBridgeDocument(_ file: ElloAppMediaFile?) -> TGBridgeDocumentMediaAttachment? {
    if let file = file {
...
func makeBridgeMedia(message: Message, strings: PresentationStrings, chatPeer: Peer? = nil, filterUnsupportedActions: Bool = true) -> [TGBridgeMediaAttachment] {
    ...
    for m in message.media {
        if let image = m as? ElloAppMediaImage, let bridgeImage = makeBridgeImage(image) {
        ...
        } else if let action = m as? ElloAppMediaAction {
        ...
        } else if let poll = m as? ElloAppMediaPoll {
            let bridgeAttachment = TGBridgeUnsupportedMediaAttachment()
            ...
            bridgeMedia.append(bridgeAttachment)
        } else if let contact = m as? ElloAppMediaContact {
            ...
            bridgeContact.phoneNumber = contact.username
            bridgeContact.prettyPhoneNumber = formatPhoneNumber(contact.username)
            bridgeMedia.append(bridgeContact)
        } else if let map = m as? ElloAppMediaMap {
        ...
        } else if let webpage = m as? ElloAppMediaWebpage {
        ...
        } else if let game = m as? ElloAppMediaGame {
            let bridgeAttachment = TGBridgeUnsupportedMediaAttachment()
            ...
            bridgeMedia.append(bridgeAttachment)
        } else if let invoice = m as? ElloAppMediaInvoice {
            let bridgeAttachment = TGBridgeUnsupportedMediaAttachment()
            ...
            bridgeMedia.append(bridgeAttachment)
        } else if let _ = m as? ElloAppMediaUnsupported {
            let bridgeAttachment = TGBridgeUnsupportedMediaAttachment()
...
func makeBridgeChat(_ entry: ChatListEntry, strings: PresentationStrings) -> (TGBridgeChat, [Int64 : TGBridgeUser])? {
    if case let .MessageEntry(index, messages, readState, _, _, renderedPeer, _, _, hasFailed, _) = entry {
        ...
        if let user = renderedPeer.peer as? ElloAppUser, user.id != message?.author?.id, let bridgeUser = makeBridgeUser(user, presence: nil) {
            bridgeUsers[bridgeUser.identifier] = bridgeUser
        }
...
func makeBridgeChat(_ peer: Peer?, view: PeerView? = nil) -> (TGBridgeChat, [Int64 : TGBridgeUser]) {
    ...
    if let group = peer as? ElloAppGroup {
...
func makeBridgePeers(_ message: Message) -> [Int64 : Any] {
    ...
    for (_, peer) in message.peers {
        if peer is ElloAppUser, let bridgeUser = makeBridgeUser(peer, presence: nil) {
            bridgeUsers[bridgeUser.identifier] = bridgeUser
        } else if peer is ElloAppGroup || peer is ElloAppChannel {
