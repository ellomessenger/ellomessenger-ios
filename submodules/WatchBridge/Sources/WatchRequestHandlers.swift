            ...
            |> mapToSignal({ context -> Signal<Bool, NoError> in
                if let context = context {
                    ...
                    if let args = subscription as? TGBridgeSendTextMessageSubscription {
                        ...
                        let peerId = makePeerIdFromBridgeIdentifier(args.peerId)
                        let map = ElloAppMediaMap(latitude: location.latitude, longitude: location.longitude, heading: nil, accuracyRadius: nil, geoPlace: nil, venue: makeVenue(from: location.venue), liveBroadcastingTimeout: nil, liveProximityNotificationRadius: nil)
...
private func mediaForSticker(documentId: Int64, account: Account) -> Signal<ElloAppMediaFile?, NoError> {
    ...
    static func handle(subscription: TGBridgeSubscription, manager: WatchCommunicationManager) -> SSignal {
        if let args = subscription as? TGBridgeMediaAvatarSubscription, let peerId = makePeerIdFromBridgeIdentifier(args.peerId) {
                    ...
                    if let context = context {
                        ...
                        |> mapToSignal { peer -> Signal<EnginePeer?, NoError> in
                            if let peer = peer, case let .secretChat(secretChat) = peer {
                                return context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: secretChat.regularPeerId))
                            } else {
                                return .single(peer)
                            }
                        }
            ...
            return SSignal { subscriber in
                ...
                |> mapToSignal({ context -> Signal<UIImage?, NoError> in
                    if let context = context {
                        var mediaSignal: Signal<(ElloAppMediaFile, FileMediaReference)?, NoError>? = nil
                        if args.stickerPackId != 0 {
                            mediaSignal = mediaForSticker(documentId: args.documentId, account: context.account)
                            |> map { media -> (ElloAppMediaFile, FileMediaReference)? in
                                if let media = media {
                            ...
                            mediaSignal = context.engine.data.get(ElloAppEngine.EngineData.Item.Messages.Message(id: MessageId(peerId: peerId, namespace: Namespaces.Message.Cloud, id: args.stickerMessageId)))
                            |> map { message -> (ElloAppMediaFile, FileMediaReference)? in
                                if let message = message {
            ...
            return SSignal { subscriber in
                ...
                |> mapToSignal({ context -> Signal<UIImage?, NoError> in
                    if let context = context, let peerId = makePeerIdFromBridgeIdentifier(args.peerId) {
                        ...
                        return context.engine.data.get(ElloAppEngine.EngineData.Item.Messages.Message(id: MessageId(peerId: peerId, namespace: Namespaces.Message.Cloud, id: args.messageId)))
                        |> mapToSignal { message -> Signal<(TransformImageArguments) -> DrawingContext?, NoError> in
                            if let message = message, !message._asMessage().containsSecretMedia {
                                ...
                                for media in message.media {
                                    if let image = media as? ElloAppMediaImage, let resource = largestImageRepresentation(image.representations)?.resource {
                                        ...
                                        candidateMediaReference = .message(message: MessageReference(message._asMessage()), media: media)
                                        break
                                    } else if let webPage = media as? ElloAppMediaWebpage, case let .Loaded(content) = webPage.content, let image = content.image, let resource = largestImageRepresentation(image.representations)?.resource  {
                                        self.disposable.add(messageMediaImageInteractiveFetched(context: context, message: message._asMessage(), image: image, resource: resource, storeToDownloadsPeerType: nil).start())
                                        candidateMediaReference = .webPage(webPage: WebpageReference(webPage), media: image)
                                        break
                                    }
                                }
                                if let imageReference = candidateMediaReference?.concrete(ElloAppMediaImage.self) {
                                ...
                                } else if let fileReference = candidateMediaReference?.concrete(ElloAppMediaFile.self) {
        ...
        if let args = subscription as? TGBridgePeerSettingsSubscription {
            return SSignal { subscriber in
                ...
                let disposable = signal.start(next: { view in
                    ...
                    if let notificationSettings = view.notificationSettings as? ElloAppPeerNotificationSettings, case let .muted(until) = notificationSettings.muteState, until >= Int32(CFAbsoluteTimeGetCurrent() + NSTimeIntervalSince1970) {
                        muted = true
                    }
