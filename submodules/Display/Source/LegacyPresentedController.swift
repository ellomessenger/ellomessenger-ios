...
open class LegacyPresentedController: ViewController {
    ...
    override open func viewWillAppear(_ animated: Bool) {
        ...
        if !asPresentable {
            self.legacyController.beginAppearanceTransition(true, animated: animated && passControllerAppearanceAnimated(presentation: self.presentation))
        }
    }
    ...
    override open func viewWillDisappear(_ animated: Bool) {
        ...
        if !asPresentable {
            self.legacyController.beginAppearanceTransition(false, animated: animated && passControllerAppearanceAnimated(presentation: self.presentation))
        }
    }
        ...
        if asPresentable {
            if !loadedController {
                ...
                switch self.presentation {
                    ...
                    case .custom:
                        self.legacyController.modalPresentationStyle = .overCurrentContext
                        self.present(self.legacyController, animated: false, completion: nil)
                }
            }
        } else {
