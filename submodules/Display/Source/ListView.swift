...
extension ListView: UIScrollViewAccessibilityDelegate, UIGestureRecognizerDelegate {
...
//                print("closest \(closestIndexValue) offset \(offset)")
    ...
    private func nodeForItem(synchronous: Bool, 
                             synchronousLoads: Bool,
                             item: ListViewItem,
                             previousNode: QueueLocalObject<ListViewItemNode>?,
                             index: Int,
                             previousItem: ListViewItem?,
                             nextItem: ListViewItem?,
                             params: ListViewItemLayoutParams,
                             updateAnimationIsAnimated: Bool,
                             updateAnimationIsCrossfade: Bool,
                             completion: @escaping (QueueLocalObject<ListViewItemNode>, ListViewItemNodeLayout, @escaping () -> (Signal<Void, NoError>?, (ListViewItemApply) -> Void)) -> Void) {
    ...
    public func transaction(deleteIndices: [ListViewDeleteItem], 
        ...
        self.transactionQueue.addTransaction({ [weak self] transactionCompletion in
            if let strongSelf = self {
                ...
                strongSelf.deleteAndInsertItemsTransaction(deleteIndices: deleteIndices, 
    ...
    private func deleteAndInsertItemsTransaction(deleteIndices: [ListViewDeleteItem], insertIndicesAndItems: [ListViewInsertItem], updateIndicesAndItems: [ListViewUpdateItem], options: ListViewDeleteAndInsertOptions, scrollToItem: ListViewScrollToItem?, additionalScrollDistance: CGFloat, updateSizeAndInsets: ListViewUpdateSizeAndInsets?, stationaryItemRange: (Int, Int)?, updateOpaqueState: Any?, completion: @escaping () -> Void) {
        ...
        let actions = {
            ...
            self.fillMissingNodes(synchronous: options.contains(.Synchronous), 
                                  synchronousLoads: options.contains(.PreferSynchronousResourceLoading),
                                  animated: animated,
                                  inputAnimatedInsertIndices: animated ? insertedIndexSet : Set<Int>(),
                                  insertDirectionHints: insertDirectionHints,
                                  inputState: state,
                                  inputPreviousNodes: previousNodes,
                                  inputOperations: operations,
                                  inputCompletion: { updatedState, operations in
        ...
        while true {
            if self.items.count == 0 {
                ...
                if let insertionItemIndexAndDirection = insertionItemIndexAndDirection {
                    ...
                    self.nodeForItem(synchronous: synchronous, 
                                     synchronousLoads: synchronousLoads,
                                     item: self.items[index],
                                     previousNode: previousNodes[index],
                                     index: index,
                                     previousItem: index == 0 ? nil : self.items[index - 1],
                                     nextItem: self.items.count == index + 1 ? nil : self.items[index + 1],
                                     params: ListViewItemLayoutParams(width: state.visibleSize.width,
                                                                      leftInset: state.insets.left,
                                                                      rightInset: state.insets.right,
                                                                      availableHeight: state.visibleSize.height - state.insets.top - state.insets.bottom),
                                     updateAnimationIsAnimated: animated,
                                     updateAnimationIsCrossfade: false,
                                     completion: { (node, layout, apply) in
