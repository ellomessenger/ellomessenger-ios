...
public final class NavigationBarTheme {
    public static func generateBackArrowImage(color: UIColor) -> UIImage? {
        let image = UIImage(bundleImageName: "Navigation/arrow-back")
        let tinted = generateTintedImage(image: image, color: color)
        return tinted
    }
            ...
            if let transitionState = self.transitionState {
                ...
                switch transitionState.role {
                    case .top:
                        ...
                        self.backButtonArrow.frame = CGRect(origin: CGPoint(x: leftInset + 8.0 - progress * size.width, y: contentVerticalOrigin + floor((nominalHeight - 24.0) / 2.0)), size: CGSize(width: 18, height: 24))
