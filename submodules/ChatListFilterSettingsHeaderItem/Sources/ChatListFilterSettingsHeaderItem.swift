...
//            let leftInset: CGFloat = 32.0 + params.leftInset
            ...
            var size = 192.0
            var insetDifference = 100.0 // Text inset from top
            ...
            case .discussionGroupSetup:
                ...
                let pointSize = 160.0
                size = pointSize * UIScreenScale
                insetDifference = pointSize * (UIScreenScale - 1)
            case .autoRemove:
                animationName = "MessageAutoRemove"
                size = 260.0
                insetDifference = 120.0
                playbackMode = .once
            ...
            let (titleLayout, titleApply) = makeTitleLayout(TextNodeLayoutArguments(attributedString: attributedText, backgroundColor: nil, maximumNumberOfLines: 0, truncationType: .end, constrainedSize: CGSize(width: params.width - params.rightInset - params.leftInset/*leftInset * 2.0*/, height: CGFloat.greatestFiniteMagnitude), alignment: .center, cutout: nil, insets: UIEdgeInsets()))
            ...
            return (layout, { [weak self] in
                if let strongSelf = self {
                    if strongSelf.item == nil {
                        strongSelf.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: animationName), width: Int(size), height: Int(size), playbackMode: playbackMode, mode: .direct(cachePathPrefix: nil))
                    ...
                    let iconSize = CGSize(width: CGFloat(size) / UIScreenScale, height: CGFloat(size) / UIScreenScale)
