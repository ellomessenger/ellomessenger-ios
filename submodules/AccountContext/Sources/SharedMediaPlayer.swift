...
public enum SharedMediaPlaybackDataSource: Equatable {
    case elloappFile(reference: FileMediaReference, isCopyProtected: Bool)
    
    public static func ==(lhs: SharedMediaPlaybackDataSource, rhs: SharedMediaPlaybackDataSource) -> Bool {
        switch lhs {
        case let .elloappFile(lhsFileReference, lhsIsCopyProtected):
            if case let .elloappFile(rhsFileReference, rhsIsCopyProtected) = rhs {
