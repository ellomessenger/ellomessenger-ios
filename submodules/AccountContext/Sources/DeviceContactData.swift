...
public final class DeviceContactUserNameData: Equatable {
    ...
    public static func == (lhs: DeviceContactUserNameData, rhs: DeviceContactUserNameData) -> Bool {
...
private let phonebookUsernamePrefix = "https://elloapp.com/" + phonebookUsernamePathPrefix

public extension DeviceContactUrlData {
    convenience init(appProfile: PeerId) {
        self.init(label: "Elloapp", value: "\(phonebookUsernamePrefix)\(appProfile.id._internalGetInt64Value())")
    }
}
...
public final class DeviceContactBasicData: Equatable {
    ...
    public let userNames: [DeviceContactUserNameData]
    
    public init(firstName: String, lastName: String, userNames: [DeviceContactUserNameData]) {
...
public extension DeviceContactExtendedData {
    ...
    func asMutableCNContact() -> CNMutableContact {
        ...
        contact.phoneNumbers = self.basicData.userNames.map { userName -> CNLabeledValue<CNPhoneNumber> in
            return CNLabeledValue<CNPhoneNumber>(label: userName.label, value: CNPhoneNumber(stringValue: userName.value))
        }
    ...
    convenience init(contact: CNContact) {
        var userNames: [DeviceContactUserNameData] = []
 //       for givenName in contact.givenName {
        userNames.append(DeviceContactUserNameData(label: contact.givenName, value: contact.givenName))
//        }
        ...
        let basicData = DeviceContactBasicData(firstName: contact.givenName, lastName: contact.familyName, userNames: userNames)
    ...
    var isPrimitive: Bool {
        if self.basicData.userNames.count > 1 {
            return false
        }
...
public extension DeviceContactExtendedData {
    convenience init?(peer: Peer) {
        guard let user = peer as? ElloAppUser else {
            return nil
        }
        
        var userNames: [DeviceContactUserNameData] = []
        if let username = user.username, !username.isEmpty {
            userNames.append(DeviceContactUserNameData(label: "_$!<Mobile>!$_", value: username))
        }
        
        self.init(basicData: DeviceContactBasicData(firstName: user.firstName ?? "",
