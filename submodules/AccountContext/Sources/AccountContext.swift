...
public final class ElloAppApplicationOpenUrlCompletion {
...
public enum ElloAppAppBuildType {
    case `internal`
    case `public`
}

public final class ElloAppApplicationBindings {
    public let isMainApp: Bool
    public let appBundleId: String
    public let appBuildType: ElloAppAppBuildType
    public let containerPath: String
    public let appSpecificScheme: String
    public let openUrl: (String) -> Void
    public let openUniversalUrl: (String, ElloAppApplicationOpenUrlCompletion) -> Void
...
public struct ResolvedBotAdminRights: OptionSet {
    ...
    public var chatAdminRights: ElloAppChatAdminRightsFlags? {
        var flags = ElloAppChatAdminRightsFlags()
...
public enum ResolvedUrl {
    ...
    case instantView(ElloAppMediaWebpage, String?)
    ...
    case invoice(slug: String, invoice: ElloAppMediaInvoice)
    case premiumOffer(reference: String?)
    case referralCode(code: String)
}
...
public final class ChatGreetingData: Equatable {
    ...
    public let sticker: Signal<ElloAppMediaFile?, NoError>
    
    public init(uuid: UUID, sticker: Signal<ElloAppMediaFile?, NoError>) {
        self.uuid = uuid
...
public final class NavigateToChatControllerParams {
    ...
    public init(navigationController: NavigationController, 
                ...
                useExisting: Bool = true, useBackAnimation: Bool = false,
...
public protocol SharedAccountContext: AnyObject {
    ...
    var accountManager: AccountManager<ElloAppAccountManagerTypes> { get }
    ...
    var applicationBindings: ElloAppApplicationBindings { get }
    ...
    func openChats()
    func openFeedController(activateRecomended: Bool)
    func openContactsController()
    ...
    func makeDeviceContactInfoController(context: AccountContext, subject: DeviceContactInfoSubject, isTitleHidden:Bool, completed: (() -> Void)?, cancelled: (() -> Void)?) -> ViewController
    ...
    func chatAvailableMessageActions(engine: ElloAppEngine, accountPeerId: EnginePeer.Id, messageIds: Set<EngineMessage.Id>) -> Signal<ChatAvailableMessageActions, NoError>
    func chatAvailableMessageActions(engine: ElloAppEngine, accountPeerId: EnginePeer.Id, messageIds: Set<EngineMessage.Id>, messages: [EngineMessage.Id: EngineMessage], peers: [EnginePeer.Id: EnginePeer]) -> Signal<ChatAvailableMessageActions, NoError>
    ...
    func openAddContact(context: AccountContext, firstName: String, lastName: String, userName: String, label: String, present: @escaping (ViewController, Any?) -> Void, pushController: @escaping (ViewController) -> Void, completed: @escaping () -> Void)
    ...
    func makeChatQrCodeScreen(context: AccountContext, peer: Peer, inviteLink: String) -> ViewController
    ...
    func beginNewAuth(testingEnvironment: Bool, isWelcomHidden:Bool?)
}
...
public enum PremiumIntroSource {
    ...
    case profile(PeerId)
    case emojiStatus(PeerId, Int64, ElloAppMediaFile?, LoadedStickerPack?)
}
