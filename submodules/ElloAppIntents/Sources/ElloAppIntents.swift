...
private let savedMessagesAvatar: UIImage = {
    return generateImage(CGSize(width: 60.0, height: 60.0), rotatedContext: { size, context in
        if let savedMessagesIcon = UIImage(bundleImageName: "Avatar/SavedMessagesIcon")?.precomposed() {
            let factor = size.width / savedMessagesIcon.size.height
            ...
            if unlocked {
                ...
                |> mapToSignal { sharedData -> Signal<[(Peer, SendMessageIntentSubject)], NoError> in
                    ...
                    return ElloAppEngine(account: account).data.get(
                        EngineDataMap(peerIds.map(ElloAppEngine.EngineData.Item.Peer.Peer.init)),
                        EngineDataMap(peerIds.map(ElloAppEngine.EngineData.Item.Peer.IsContact.init)),
                        EngineDataMap(peerIds.map(ElloAppEngine.EngineData.Item.Messages.ChatListGroup.init))
                    )
                ...
                |> mapToSignal { peers -> Signal<[(Peer, SendMessageIntentSubject, UIImage?)], NoError> in
                    ...
                    for (peer, subject) in peers {
                        if peer.id == account.peerId {
                            ...
                            let peerAndAvatar = (peerAvatarImage(account: account,
                                                                 peerReference: PeerReference(peer),
                                                                 authorOfMessage: nil,
                                                                 representation: peer.smallProfileImage,
                                                                 clipStyle: .none) ?? .single(nil))
