        ...
        else
        {
            ...
            for (MTRequest *request in _requests)
            {
                ...
                if (request.requestContext != nil && request.requestContext.messageId == rpcResultMessage.requestMessageId)
                {
                    ...
                    if (rpcError != nil)
                    {
                        // error 403 - blocked account
                        // USER_PRIVACY_RESTRICTED - user restrict incoming calls
                        BOOL isBlockedAccount = rpcError.errorCode == 403 && ![rpcError.errorDescription isEqual: @"USER_PRIVACY_RESTRICTED"];
                        if (rpcError.errorCode == 401 || isBlockedAccount)
                        {
