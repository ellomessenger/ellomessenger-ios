...
private final class StickerPackContainer: ASDisplayNode {
        ...
        controller: StickerPackScreenImpl?)
    {
        ...
        self.titleNode.highlightAttributeAction = { attributes in
            if let _ = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.PeerTextMention)] {
                return NSAttributedString.Key(rawValue: ElloAppTextAttributes.PeerTextMention)
            } else {
                return nil
            }
        }
        self.titleNode.tapAttributeAction = { attributes, _ in
            if let mention = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.PeerTextMention)] as? String, mention.count > 1 {
        ...
        self.itemsDisposable = combineLatest(queue: Queue.mainQueue(), fetchedStickerPacks, context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: context.account.peerId))).start(next: { [weak self] contents, peer in
            guard let strongSelf = self else {
                return
            }
        ...
        if !self.currentStickerPacks.isEmpty {
            ...
            for (info, _, _) in self.currentStickerPacks {
                ...
                if info.id.namespace == Namespaces.ItemCollection.CloudEmojiPacks {
                    links += "https://ello.team/addemoji/\(info.shortName)"
                } else {
                    links += "https://ello.team/addstickers/\(info.shortName)"
                }
            }
            text = links
            ...
            if info.id.namespace == Namespaces.ItemCollection.CloudEmojiPacks {
                text = "https://ello.team/addemoji/\(info.shortName)"
            } else {
                text = "https://ello.team/addstickers/\(info.shortName)"
            }
