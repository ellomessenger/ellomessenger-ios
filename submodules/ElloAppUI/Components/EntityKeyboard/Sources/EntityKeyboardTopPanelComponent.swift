...
public final class EntityKeyboardTopPanelComponent: Component {
    ...
    public final class View: UIView, UIScrollViewDelegate, ComponentTaggedView {
        private struct ItemLayout {
            ...
            init(isExpanded: Bool, containerSideInset: CGFloat, height: CGFloat, items: [ItemDescription], maxWidth: CGFloat? = nil) {
                let inset = containerSideInset + 7.0
                self.sideInset = inset
                ...
                let maxWidth = maxWidth.map {
                    $0 - inset * 2
                } ?? 134.0
                let size = CGSize(width: maxWidth, height: self.staticItemSize.height)
                self.staticExpandedItemSize = self.isExpanded ?  size : CGSize(width: maxWidth, height: 28.0)
        ...
        func update(component: EntityKeyboardTopPanelComponent, availableSize: CGSize, state: EmptyComponentState, environment: Environment<EnvironmentType>, transition: Transition) -> CGSize {
            ...
            }, maxWidth: scrollView.bounds.width)
            self.itemLayout = itemLayout
