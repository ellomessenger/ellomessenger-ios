...
public final class EmojiSuggestionsComponent: Component {
    ...
    public static func suggestionData(context: AccountContext, isSavedMessages: Bool, query: String) -> Signal<[ElloAppMediaFile], NoError> {
        ...
        |> take(1)
        |> map { view, featuredEmojiPacks, hasPremium, sharedData -> [ElloAppMediaFile] in
    ...
    public let action: (ElloAppMediaFile) -> Void
    ...
    public init(
        ...
        files: [ElloAppMediaFile],
        action: @escaping (ElloAppMediaFile) -> Void
    ) {
        ...
        @objc private func tapGesture(_ recognizer: UITapGestureRecognizer) {
            if case .ended = recognizer.state {
                ...
                if self.scrollView.bounds.contains(location) {
                    var closestFile: (file: ElloAppMediaFile, distance: CGFloat)?
