...
class ContactMultiselectionControllerImpl: ViewController, ContactMultiselectionController {
    ...
    init(_ params: ContactMultiselectionControllerParams) {
        ...
        self.limitsConfigurationDisposable = (context.engine.data.get(ElloAppEngine.EngineData.Item.Configuration.Limits())
        ...
        case let .chatSelection(_, selectedChats, additionalCategories, _):
            let _ = (self.context.engine.data.get(
                EngineDataList(
                    selectedChats.map(ElloAppEngine.EngineData.Item.Peer.Peer.init)
                )
            )
    ...
    private func updateTitle() {
        ...
        case .groupCreation:
            ...
            self.titleView.title = CounterContollerTitle(title: self.presentationData.strings.Group_Members_AddMembers.capitalizeFirst(), counter: "\(count)/\(maxCount)")
        ...
        case .channelCreation, .publicChannelCreation:
            self.titleView.title = CounterContollerTitle(title: self.presentationData.strings.Channel_Members_AddMembers.capitalizeFirst(), counter: self.presentationData.strings.Channel_Members_UnlimitedSubscrubers)
            let rightNavigationButton = UIBarButtonItem(title: self.presentationData.strings.Common_Next, style: .done, target: self, action: #selector(self.rightNavigationButtonPressed))
            self.rightNavigationButton = rightNavigationButton
            self.navigationItem.rightBarButtonItem = self.rightNavigationButton
        ...
        self.contactsNode.openPeer = { [weak self] peer in
            if let strongSelf = self, case let .peer(peer, _, _) = peer {
                ...
                if let updatedCount = updatedCount {
                    ...
                    case .peerSelection, .channelCreation, .chatSelection, .publicChannelCreation:
                        break
                    }
                }
