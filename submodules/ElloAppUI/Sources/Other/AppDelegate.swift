...
private func extractAccountManagerState(records: AccountRecordsView<ElloAppAccountManagerTypes>) -> AccountManagerState {
...
@objc(AppDelegate) class AppDelegate: UIResponder, UIApplicationDelegate, PKPushRegistryDelegate, UNUserNotificationCenterDelegate {
    ...
    private var accountManager: AccountManager<ElloAppAccountManagerTypes>?
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]? = nil) -> Bool {
        ...
        let networkArguments = NetworkInitializationArguments(apiId: apiId, 
                                                              apiHash: apiHash, 
        ...
        let applicationBindings = ElloAppApplicationBindings(isMainApp: true, 
...
//            if #available(iOS 10, *) {
//                switch INPreferences.siriAuthorizationStatus() { // @slava: crash
//                    case .authorized:
//                        return .allowed
...
//            }
        ...
        let accountManager = AccountManager<ElloAppAccountManagerTypes>(basePath: rootPath + "/accounts-metadata", 
                                                                        isTemporary: false,
                                                                        isReadOnly: false,
                                                                        useCaches: true,
        ...
        |> mapToSignal { accountManager, initialPresentationDataAndSettings -> Signal<(SharedApplicationContext, LoggingSettings), NoError> in
            // Color of initial root controller
            self.mainWindow?.hostView.containerView.backgroundColor = UIColor(red: 0.349, green: 0.6, blue: 0.969, alpha: 1) //initialPresentationDataAndSettings.presentationData.theme.chatList.backgroundColor
            ...
            let appLockContext = AppLockContextImpl(rootPath: rootPath, window: self.mainWindow!, rootController: self.window?.rootViewController, applicationBindings: applicationBindings, accountManager: accountManager, presentationDataSignal: presentationDataPromise.get(), lockIconInitialFrame: {
                return (self.mainWindow?.viewController as? ElloAppRootController)?.chatListController?.lockViewFrame
            })
            
            var setPresentationCall: ((PresentationCall?) -> Void)?
            let sharedContext = SharedAccountContextImpl(mainWindow: self.mainWindow, 
            ...
            let notificationManager = SharedNotificationManager(episodeId: self.episodeId, 
                                                                application: application,
                                                                clearNotificationsManager: clearNotificationsManager,
                                                                inForeground: applicationBindings.applicationInForeground,
                                                                accounts: sharedContext.activeAccountContexts |> map { primary, accounts, _ in accounts.map({
                isWelcomSlideHidden = 0 < accounts.count
                let isSelf = $0.1.account.id == primary?.account.id
                if let rootController = self.mainWindow.viewController as? ElloAppRootController {
                    if accounts.isEmpty || (1 == accounts.count && isSelf) {
                        let image = UIImage(bundleImageName: "Chat List/Tabs/IconSettings")
                        rootController.accountSettingsController?.tabBarItem.ringSelection = false
                        rootController.accountSettingsController?.tabBarItem.image = image?.preparingThumbnail(of: CGSize(width: 24, height: 24))
                        rootController.accountSettingsController?.tabBarItem.selectedImage = image?.preparingThumbnail(of: CGSize(width: 24, height: 24))
                    }
                }
                
                return ($0.1.account, isSelf) }) },
                                                                pollLiveLocationOnce: { accountId in
                let _ = (self.context.get()
                |> filter {
                    return $0 != nil
                }
            ...
            let wakeupManager = SharedWakeupManager(beginBackgroundTask: { name, expiration in application.beginBackgroundTask(withName: name, expirationHandler: expiration) }, 
                                                    endBackgroundTask: { id in application.endBackgroundTask(id) },
                                                    backgroundTimeRemaining: { application.backgroundTimeRemaining },
                                                    activeAccounts: sharedContext.activeAccountContexts |> map { 
                return ($0.0?.account, $0.1.map { ($0.0, $0.1.account) }) },
                                                    liveLocationPolling: liveLocationPolling,
                                                    watchTasks: watchTasks,
                                                    inForeground: applicationBindings.applicationInForeground,
                                                    hasActiveAudioSession: self.hasActiveAudioSession.get(),
                                                    notificationManager: notificationManager,
                                                    mediaManager: sharedContext.mediaManager,
                                                    callManager: sharedContext.callManager,
                                                    accountUserInterfaceInUse: { id in
        ...
        |> mapToSignal { sharedApplicationContext -> Signal<AuthorizedApplicationContext?, NoError> in
            ...
            |> map { accountAndSettings -> AuthorizedApplicationContext? in
                return accountAndSettings.flatMap { context, callListSettings in
                    return AuthorizedApplicationContext(sharedApplicationContext: sharedApplicationContext, 
                                                        mainWindow: self.mainWindow,
                                                        watchManagerArguments: watchManagerArgumentsPromise.get(),
        ...
        |> mapToSignal { sharedApplicationContext -> Signal<UnauthorizedApplicationContext?, NoError> in
            ...
            |> mapToSignal { authAndAccounts -> Signal<(UnauthorizedAccount, ((String, AccountRecordId, Bool)?, [(String, AccountRecordId, Bool)]))?, NoError> in
                if let (primary, auth, accounts) = authAndAccounts {
                    let userNames = combineLatest(accounts.map { context -> Signal<(AccountRecordId, String, Bool)?, NoError> in
                        return context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: context.account.peerId))
                        |> map { peer -> (AccountRecordId, String, Bool)? in
                            if case let .user(user) = peer, let username = user.username {
                                return (context.account.id, username, context.account.testingEnvironment)
                            } else {
                                return nil
                            }
                        }
                    ...
                    return userNames
                    |> map { userNames -> (UnauthorizedAccount, ((String, AccountRecordId, Bool)?, [(String, AccountRecordId, Bool)]))? in
                        var primaryNumber: (String, AccountRecordId, Bool)?
                        if let primary = primary {
                            for idAndNumber in userNames {
                        ...
                        return (auth, (primaryNumber, userNames.compactMap({ $0.flatMap({ ($0.1, $0.0, $0.2) }) })))
                    }
                } else {
                    return .single(nil)
                }
            ...
            |> map { accountAndSettings -> UnauthorizedApplicationContext? in
                return accountAndSettings.flatMap { account, otherAccountPhoneNumbers in
                    return UnauthorizedApplicationContext(apiId: buildConfig.apiId,
        ...
        |> deliverOnMainQueue).start(next: { context in
            ...
            isFirstTime = self.contextValue == nil
        ...
        |> take(1)
        |> mapToSignal { sharedContext -> Signal<(AccountManager<ElloAppAccountManagerTypes>, Set<PeerId>), NoError> in
            ...
            |> map { loggedOutAccountPeerIds -> (AccountManager<ElloAppAccountManagerTypes>, Set<PeerId>) in
        ...
        |> deliverOnMainQueue).start(next: { sharedApplicationContext in
            ...
            |> deliverOnMainQueue).start(next: { activeAccounts in
                ...
                for (_, context, _) in activeAccounts.accounts {
                    if context.account.peerId.id._internalGetInt64Value() == accountId.int64 {
    ...
    private func openUrl(url: URL) {
        ...
        |> deliverOnMainQueue).start(next: { _, context, authContext in
            if let context = context {
                ...
                } else if let referralCode = parseReferralCode(url.absoluteString) {
                    ELRegisterController.referralCode = referralCode
                }
            }
        })
    }
                            ...
                            switch handle.type {
                                ...
                                case .phoneNumber:
                                    ...
                                    if !phoneNumber.isEmpty {
                                        ...
                                        let _ = (context.engine.data.get(ElloAppEngine.EngineData.Item.Contacts.List(includePresences: false))
    ...
    private func openChatWhenReady(accountId: AccountRecordId?, peerId: PeerId, messageId: MessageId? = nil, activateInput: Bool = false) {
        ...
        |> mapToSignal { sharedApplicationContext -> Signal<AuthorizedApplicationContext, NoError> in
            if let accountId = accountId {
                ...
                |> filter { context in
//                    context.context.account.id == accountId
                    context.context.account.peerId.id._internalGetInt64Value() == accountId.int64
                }
                |> take(1)
            } else {
                return self.authorizedContext()
                ...
                |> mapToSignal { sharedContext -> Signal<Void, NoError> in
                    ...
                    |> mapToSignal { account -> Signal<Void, NoError> in
                        if let messageId = messageIdFromNotification(peerId: peerId, notification: response.notification) {
                            let _ = ElloAppEngine(account: account).messages.applyMaxReadIndexInteractively(index: MessageIndex(id: messageId, timestamp: 0)).start()
                        }
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, willPresent notification: UNNotification, withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void) {
        ...
        |> deliverOnMainQueue).start(next: { accountId in
            if let context = self.contextValue {
                if let accountId = accountId, context.context.account.peerId.id._internalGetInt64Value() != accountId.int64 {
        ...
        if let encryptedData = encryptedData, let notificationKeyId = notificationPayloadKey(data: encryptedData) {
            ...
            |> mapToSignal { sharedContext -> Signal<AccountRecordId?, NoError> in
                ...
                |> map { _, contexts, _ -> AccountRecordId? in
                    isWelcomSlideHidden = 0 < contexts.count
