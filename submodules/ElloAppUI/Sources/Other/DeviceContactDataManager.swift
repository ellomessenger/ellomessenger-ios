...
private protocol DeviceContactDataContext {
    ...
    func appendUserName(_ userName: DeviceContactUserNameData, to stableId: DeviceContactStableId) -> DeviceContactExtendedData?
...
//        })
    ...
    private static func parseContact(_ contact: CNContact) -> (DeviceContactStableId, DeviceContactBasicData) {
        var phoneNumbers: [DeviceContactUserNameData] = []
        for number in contact.phoneNumbers {
            phoneNumbers.append(DeviceContactUserNameData(label: number.label ?? "", value: number.value.stringValue))
        }
        return (contact.identifier, DeviceContactBasicData(firstName: contact.givenName, lastName: contact.familyName, userNames: phoneNumbers))
    }
...
//        let mutableContact = current.mutableCopy() as! CNMutableContact
//        
//        var userNames = mutableContact.phoneNumbers
//        let appendUserNames: [CNLabeledValue<CNPhoneNumber>] = [CNLabeledValue<CNPhoneNumber>(label: userName.label, value: CNPhoneNumber(stringValue: userName.value))]
//        for appendUserName in appendUserNames {
//            var found = false
//            inner: for n in userNames {
//                if n.value.stringValue == appendUserName.value.stringValue {
//                    found = true
//                    break inner
//                }
//            }
//            if !found {
//                userNames.insert(appendUserName, at: 0)
//            }
//        }
//        
//        mutableContact.phoneNumbers = userNames
//        
//        let saveRequest = CNSaveRequest()
//        saveRequest.update(mutableContact)
//        let _ = try? self.store.execute(saveRequest)
...
private final class DeviceContactDataManagerPrivateImpl {
    ...
    private func updateAll(_ stableIdToBasicContactData: [DeviceContactStableId: DeviceContactBasicData]) {
        ...
        for (stableId, basicData) in self.stableIdToBasicContactData {
            for phoneNumber in basicData.userNames {
        ...
        for (stableId, basicData) in self.stableIdToBasicContactData {
            for userName in basicData.userNames {
                var replace = false
                var currentLocalIdentifiers: [String] = []
                if let current = importableContactData[userName.value] {
                ...
                if replace {
                    ...
                    importableContactData[userName.value] = (stableId, ImportableDeviceContactData(firstName: basicData.firstName, lastName: basicData.lastName, localIdentifiers: currentLocalIdentifiers))
                }
            }
        }
    ...
    func appendUserName(_ userName: DeviceContactUserNameData, to stableId: DeviceContactStableId, completion: @escaping (DeviceContactExtendedData?) -> Void) {
        let result = self.dataContext?.appendUserName(userName, to: stableId)
        completion(result)
    }
...
public final class DeviceContactDataManagerImpl: DeviceContactDataManager {
    ...
    public func appendUserName(_ userName: DeviceContactUserNameData, to stableId: DeviceContactStableId) -> Signal<DeviceContactExtendedData?, NoError> {
        return Signal { subscriber in
            let disposable = MetaDisposable()
            self.impl.with({ impl in
                impl.appendUserName(userName, to: stableId, completion: { next in
                    subscriber.putNext(next)
                    subscriber.putCompletion()
                })
            })
