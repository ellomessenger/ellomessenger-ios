...
func topMessageReactions(context: AccountContext, message: Message) -> Signal<[ReactionItem], NoError> {
    ...
    let allowedReactionsWithFiles: Signal<(reactions: AllowedReactions, files: [Int64: ElloAppMediaFile])?, NoError> = peerMessageAllowedReactions(context: context, message: message)
    |> mapToSignal { allowedReactions -> Signal<(reactions: AllowedReactions, files: [Int64: ElloAppMediaFile])?, NoError> in
        ...
        if case let .set(reactions) = allowedReactions {
            ...
            |> map { files -> (reactions: AllowedReactions, files: [Int64: ElloAppMediaFile]) in
