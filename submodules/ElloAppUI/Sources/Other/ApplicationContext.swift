...
final class UnauthorizedApplicationContext {
    ...
    init(apiId: Int32,
        ...
        self.rootController = AuthorizationSequenceController(sharedContext: sharedContext,
                                                              account: account,
                                                              ...
                                                              apiId: apiId,
                                                              apiHash: apiHash,
                                                              isWelcomSlideHidden: isWelcomSlideHidden,
...
final class AuthorizedApplicationContext {
    ...
    let rootController: ElloAppRootController
    ...
    init(sharedApplicationContext: SharedApplicationContext, mainWindow: Window1, watchManagerArguments: Signal<WatchManagerArguments?, NoError>, context: AccountContextImpl, accountManager: AccountManager<ElloAppAccountManagerTypes>, showCallsTab: Bool, reinitializedNotificationSettings: @escaping () -> Void) {
        ...
        self.rootController = ElloAppRootController(context: context)
        ...
        |> mapToSignal { messageList -> Signal<[([Message], PeerGroupId, Bool)], NoError> in
            return engine.data.get(EngineDataMap(
                messageList.compactMap { item -> ElloAppEngine.EngineData.Item.Messages.ChatListIndex? in
                    if let message = item.0.first {
                        return ElloAppEngine.EngineData.Item.Messages.ChatListIndex(id: message.id.peerId)
                    } else {
                        return nil
                    }
                }
                               ...
                               let presentationData = strongSelf.context.sharedContext.currentPresentationData.with { $0 }
                                strongSelf.notificationController.enqueue(ChatMessageNotificationItem(context: strongSelf.context, strings: presentationData.strings, dateTimeFormat: presentationData.dateTimeFormat, nameDisplayOrder: presentationData.nameDisplayOrder, messages: messages, tapAction: {
                                    if let strongSelf = self {
                                        ...
                                        var processed = false
                                        for media in firstMessage.media {
                                            if let action = media as? ElloAppMediaAction, case .geoProximityReached = action.action {
    ...
    func openUrl(_ url: URL) {
        if self.rootController.rootTabController != nil {
            ...
            self.context.sharedContext.openExternalUrl(context: self.context, 
                                                       urlContext: .generic,
