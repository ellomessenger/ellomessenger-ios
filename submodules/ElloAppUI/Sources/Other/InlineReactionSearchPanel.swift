...
private final class InlineReactionSearchStickersNode: ASDisplayNode, UIScrollViewDelegate {
    private final class DisplayItem {
        ...
        init(file: ElloAppMediaFile, frame: CGRect) {
            self.file = file
            self.frame = frame
        }
    }
    ...
    private var items: [ElloAppMediaFile] = []
    ...
    var previewedStickerItem: ElloAppMediaFile?
    ...
    private func updatePreviewingItem(file: ElloAppMediaFile?, animated: Bool) {
    ...
    func updateItems(items: [ElloAppMediaFile]) {
        self.items = items
...
final class InlineReactionSearchPanel: ChatInputContextPanelNode {
    ...
    func updateResults(results: [ElloAppMediaFile], query: String?) {
        self.query = query
