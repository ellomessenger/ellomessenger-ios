...
private func animationItem(account: Account, emojis: Signal<[ElloAppMediaFile], NoError>, configuration: Signal<InteractiveEmojiConfiguration?, NoError> = .single(nil), emoji: String, value: Int32?, immediate: Bool = false, roll: Bool = false, loop: Bool = false, successCallback: (() -> Void)? = nil) -> Signal<ManagedAnimationItem?, NoError> {
...
private func rollingAnimationItem(account: Account, emojis: Signal<[ElloAppMediaFile], NoError>, emoji: String) -> Signal<ManagedAnimationItem?, NoError> {
    switch emoji {
        case "🎲":
...
final class ManagedDiceAnimationNode: ManagedAnimationNode, GenericAnimatedStickerNode {
    ...
    private let emojis = Promise<[ElloAppMediaFile]>()
    ...
    init(context: AccountContext, emoji: String) {
        ...
        |> mapToSignal { stickerPack -> Signal<[ElloAppMediaFile], NoError> in
            switch stickerPack {
                case let .result(_, items, _):
                    var emojiStickers: [ElloAppMediaFile] = []
                    for item in items {
