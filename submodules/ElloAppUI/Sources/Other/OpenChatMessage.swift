    ...
    if let mediaData = chatMessageGalleryControllerData(context: params.context, chatLocation: params.chatLocation, chatLocationContextHolder: params.chatLocationContextHolder, message: params.message, navigationController: params.navigationController, standalone: params.standalone, reverseMessageGalleryOrder: params.reverseMessageGalleryOrder, mode: params.mode, source: params.gallerySource, synchronousLoad: false, actionInteraction: params.actionInteraction) {
        switch mediaData {
            ...
            case let .other(otherMedia):
                params.dismissInput()
                if let contact = otherMedia as? ElloAppMediaContact {
                    let paramsSignal: Signal<(EnginePeer?, Bool), NoError>
                    if let peerId = contact.peerId {
                        let peersSignal = params.context.engine.data.get(
                            ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId),
                            ElloAppEngine.EngineData.Item.Peer.IsContact(id: peerId)
                        )
                        paramsSignal = params.context.engine.peers.fetchAndUpdateUserFull(peerId: peerId)
                        |> mapToSignal { peer -> Signal<(EnginePeer?, Bool), NoError> in
                            return peersSignal
                        }
                    } else {
                    ...
                    var showAlert: (() -> Void)? = {
                        ...
                        let alertController = textAlertController(
                            ...
                            title: nil,
                            text: presentationData.strings.PrivateLinkWarning,
                            actions: [
                                TextAlertAction(
                                    type: .defaultAction,
                                    title: presentationData.strings.Common_OK,
                                    action: { })])
                        params.present(alertController, nil)
                    }
                    
                    let _ = (paramsSignal |> deliverOnMainQueue)
                        .start(next: { peer, isContact in
                        ...
                        //                                basicData: DeviceContactBasicData(firstName: contact.firstName,
                        ...
                        //                        let controller = deviceContactInfoController(context: params.context,
                        //                                                                     updatedPresentationData: params.updatedPresentationData,
                        //                                                                     subject: .vcard(peer?._asPeer(), nil, contactData),
                        //                                                                     isTitleHidden: true,
                        //                                                                     completed: nil,
                        //                                                                     cancelled: nil)
                        //                        let controller = PeerInfoScreenImpl(context: params.context,
                        //                                                            updatedPresentationData: nil,
                        //                                                            peerId: params.context.account.peerId,
                        ...
                        if let peer, ((peer._asPeer() as? ElloAppUser)?.isPublic ?? true) {
                            if let infoController = params.context.sharedContext.makePeerInfoController(
                                context: params.context,
                                updatedPresentationData: params.updatedPresentationData,
                                peer: peer._asPeer(),
                                mode: PeerInfoControllerMode.generic,
                                avatarInitiallyExpanded: !peer.isPhotoEmpty,
                                fromChat: true,
                                requestsContext: nil
                            ) {
                                params.navigationController?.pushViewController(infoController)
                            }
                            showAlert = nil
                        } else {
                            showAlert?()
                ...
                if case let .wallpaper(parameter) = resolvedUrl {
                    ...
                    switch parameter {
                        ...
                        case let .gradient(colors, rotation):
                            source = .wallpaper(.gradient(ElloAppWallpaper.Gradient(id: nil, colors: colors, settings: WallpaperSettings(rotation: rotation))), nil, [], nil, rotation, message)
                    }
