...
final class VerticalListContextResultsChatInputPanelItemNode: ListViewItemNode {
    ...
    private var currentIconImageResource: ElloAppMediaResource?
            ...
            if updatedIconImageResource {
                if let imageResource = imageResource {
                    if let stickerFile = stickerFile {
                        ...
                        let tmpRepresentation = ElloAppMediaImageRepresentation(dimensions: PixelDimensions(width: 55, height: 55), resource: imageResource, progressiveSizes: [], immediateThumbnailData: nil)
