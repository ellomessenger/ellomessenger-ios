...
private struct CreateGroupArguments {
    ...
    let updateWithVenue: (ElloAppMediaMap) -> Void
}
...
private enum CreateGroupEntry: ItemListNodeEntry {
    ...
    case groupDescription(PresentationTheme, _ placeholder: String, _ text: String)
    case descriptionFooter(String)
    ...
    case venue(Int32, PresentationTheme, ElloAppMediaMap)
    
    var section: ItemListSectionId {
        switch self {
        case .groupInfo, .setProfilePhoto:
            return CreateGroupSection.info.rawValue
        case .groupDescription, .descriptionHeader, .descriptionFooter:
            return CreateGroupSection.description.rawValue
        case .member:
    ...
    var stableId: Int32 {
        ...
        case .groupDescription: 3
        case .descriptionFooter: 4
        case let .member(index, _, _, _, _, _, _): 5 + index
        case .locationHeader: 10000
        case .location: 10001
        case .changeLocation: 10002
        case .locationInfo: 10003
        case .venueHeader: 10004
        case let .venue(index, _, _): 10005 + index
        }
    }
    ...
    static func ==(lhs: CreateGroupEntry, rhs: CreateGroupEntry) -> Bool {
        ...
        case let .descriptionHeader(lhsText):
            if case let .descriptionHeader(rhsText) = rhs {
                return lhsText == rhsText
            } else {
                return false
            }
        case let .groupDescription(lhsTheme, lhsPlaceholder, lhsText):
            if case let .groupDescription(rhsTheme, rhsPlaceholder, rhsText) = rhs {
                return lhsTheme === rhsTheme && lhsPlaceholder == rhsPlaceholder && lhsText == rhsText
            } else {
                return false
            }
        case let .descriptionFooter(lhsText):
            if case let .descriptionFooter(rhsText) = rhs {
                return lhsText == rhsText
            } else {
                return false
            }
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        case let .descriptionHeader(text):
            return ItemListSectionHeaderItem(
                ...
                text: text,
                multiline: false,
                accessoryText: ItemListSectionHeaderAccessoryText(
                    value: "255",
                    color: .generic,
                    icon: nil),
                sectionId: self.section
            )
        case let .groupDescription(_, placeholder, text):
            return ItemListMultilineInputItem(
                ...
                tag: CreateGroupEntryTag.description)
        case let .descriptionFooter(text):
...
private struct CreateGroupState: Equatable {
    ...
    static func ==(lhs: CreateGroupState, rhs: CreateGroupState) -> Bool {
        ...
        if lhs.editingDescription != rhs.editingDescription {
            return false
        }
...
private func createGroupEntries(presentationData: PresentationData, state: CreateGroupState, peerIds: [PeerId], view: MultiplePeersView, venues: [ElloAppMediaMap]?) -> [CreateGroupEntry] {
    var entries: [CreateGroupEntry] = []
    
    let groupInfoState = ItemListAvatarAndNameInfoItemState(editingName: state.editingName, updatingName: nil)
    
    let peer = ElloAppGroup(id: PeerId(namespace: .max, id: PeerId.Id._internalFromInt64Value(0)), title: state.editingName.composedTitle, userName: nil, description: state.editingName.composedDesription, photo: [], participantCount: 0, role: .creator(rank: nil), membership: .Member, flags: [], defaultBannedRights: nil, migrationReference: nil, creationDate: 0, version: 0)
    ...
    entries.append(.groupDescription(presentationData.theme, presentationData.strings.Channel_Edit_AboutItem, state.editingDescription))
    entries.append(.descriptionFooter(presentationData.strings.CreateGroup_Description_Footer))
    ...
    peers.sort(by: { lhs, rhs in
        let lhsPresence = view.presences[lhs.id] as? ElloAppUserPresence
        let rhsPresence = view.presences[rhs.id] as? ElloAppUserPresence
        if let lhsPresence = lhsPresence, let rhsPresence = rhsPresence {
...
public func createGroupControllerImpl(context: AccountContext, peerIds: [PeerId], initialTitle: String? = nil, mode: CreateGroupMode = .supergroup, completion: ((PeerId, @escaping () -> Void) -> Void)? = nil) -> ViewController {
    var location: PeerGeoLocation?
    ...
    let initialState = CreateGroupState(creating: false, editingName: .title(title: initialTitle ?? "", description: "", type: .group), editingDescription: "", nameSetFromVenue: false, avatar: nil, location: location)
    ...
    let venuesPromise = Promise<[ElloAppMediaMap]?>(nil)
    if case let .locatedGroup(latitude, longitude, address) = mode {
    ...
    let arguments = CreateGroupArguments(context: context, updateEditingName: { editingName in
        ...
        updateState { current in
            var current = current
            current.editingDescription = String(text.prefix(255))
            return current
        }
    }, done: {
        let (creating, title, description, location) = stateValue.with { state -> (Bool, String, String?, PeerGeoLocation?) in
            return (state.creating, state.editingName.composedTitle, state.editingDescription, state.location)
        }
        ...
        if !creating && !title.isEmpty {
            ...
            switch mode {
                ...
                case .supergroup:
                    createSignal = context.engine.peers.createSupergroup(title: title, description: description)
                    |> map(Optional.init)
                ...
                case .locatedGroup:
                    ...
                    |> mapToSignal { address -> Signal<PeerId?, CreateGroupError> in
                        ...
                        return context.engine.peers.createSupergroup(title: title, description: description, location: (location.latitude, location.longitude, address))
                        |> map(Optional.init)
                        |> mapError { error -> CreateGroupError in
                            switch error {
                                case .generic:
            ...
            |> mapToSignal{ peerId -> Signal<PeerId?, CreateGroupError> in
                ...
                if let description = description {
                    return context.engine.peers.updatePeerDescription(peerId: peerId, description: description)
                    |> ignoreValues
                    |> `catch` { _ -> Signal<Never, CreateGroupError> in
                        return .complete()
                    }
            ...
            |> mapToSignal{ peerId -> Signal<PeerId?, CreateGroupError> in
                ...
                return context.engine.peers.addChannelMembers(peerId: peerId, memberIds: peerIds)
                |> ignoreValues
                |> `catch` { _ -> Signal<Never, CreateGroupError> in
                    return .complete()
                }
            ...
            |> afterDisposed {
                ...
                let localizedStrings = context.sharedContext.currentPresentationData.with { $0 }.strings
                let text: String
                switch error {
                case .privacy:
                    text = localizedStrings.Privacy_GroupsAndChannels_InviteToChannelMultipleError
                case .generic:
                    text = localizedStrings.Login_UnknownError
                case .restricted:
                    text = localizedStrings.Common_ActionNotAllowedError
                case .tooMuchJoined:
                    pushImpl?(oldChannelsController(context: context, intent: .create))
                    return
                case .tooMuchLocationBasedGroups:
                    text = localizedStrings.CreateGroup_ErrorLocatedGroupsTooMuch
                ...
                let alertController = textAlertController(
                    ...
                    text: text,
                    actions: [
                        TextAlertAction(type: .defaultAction, title: localizedStrings.Common_OK, action: {})
                    ]
                )
        ...
        updateState { current in
            ...
            if current.editingName.isEmpty || current.nameSetFromVenue {
                current.editingName = .title(title: venueData.title, description: "", type: .group)
