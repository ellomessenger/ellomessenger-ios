...
public enum PrefetchMediaItem {
    ...
    case animatedEmojiSticker(ElloAppMediaFile)
}
...
private final class PrefetchManagerInnerImpl {
    ...
    fileprivate let preloadedGreetingStickerPromise = Promise<ElloAppMediaFile?>(nil)

    init(queue: Queue, sharedContext: SharedAccountContext, account: Account, engine: ElloAppEngine, fetchManager: FetchManager) {
        self.queue = queue
        ...
        let orderedPreloadMedia = combineLatest(account.viewTracker.orderedPreloadMedia, ElloAppEngine(account: account).stickers.loadedStickerPack(reference: .animatedEmoji, forceActualized: false), appConfiguration)
            ...
            switch mediaItem {
                case let .chatHistory(mediaItem):
                    ...
                    if mediaItem.media.authorIsContact {
                        peerType = .contact
                    } else if let channel = mediaItem.media.peer as? ElloAppChannel {
                        if case .group = channel.info {
                            peerType = .group
                        } else {
                            peerType = .channel
                        }
                    } else if mediaItem.media.peer is ElloAppGroup {
                        peerType = .group
                    } else {
                    ...
                    if let elloappImage = mediaItem.media.media as? ElloAppMediaImage {
                        mediaResource = largestRepresentationForPhoto(elloappImage)?.resource
                        if shouldDownloadMediaAutomatically(settings: automaticDownloadSettings, peerType: peerType, networkType: networkType, authorPeerId: nil, contactsPeerIds: [], media: elloappImage) {
                            automaticDownload = .full
                        }
                    } else if let elloappFile = mediaItem.media.media as? ElloAppMediaFile {
                        mediaResource = elloappFile.resource
                        if shouldDownloadMediaAutomatically(settings: automaticDownloadSettings, peerType: peerType, networkType: networkType, authorPeerId: nil, contactsPeerIds: [], media: elloappFile) {
                            automaticDownload = .full
                        } else if shouldPredownloadMedia(settings: automaticDownloadSettings, peerType: peerType, networkType: networkType, media: elloappFile) {
...
final class PrefetchManagerImpl: PrefetchManager {
    ...
    init(sharedContext: SharedAccountContext, account: Account, engine: ElloAppEngine, fetchManager: FetchManager) {
    ...
    var preloadedGreetingSticker: ChatGreetingData {
        let signal: Signal<ElloAppMediaFile?, NoError> = Signal { subscriber in
