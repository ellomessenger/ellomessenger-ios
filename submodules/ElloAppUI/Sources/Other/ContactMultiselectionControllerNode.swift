...
final class ContactMultiselectionControllerNode: ASDisplayNode {
    ...
    init(navigationBar: NavigationBar?, context: AccountContext, presentationData: PresentationData, mode: ContactMultiselectionControllerMode, options: [ContactListAdditionalOption], filters: [ContactListFilter], limit: Int32?, reachedSelectionLimit: ((Int32) -> Void)?) {
        ...
        switch mode {
            ...
            default:
                placeholder = self.presentationData.strings.Compose_TokenListPlaceholderSearch
        }
        ...
        if case let .chatSelection(_, selectedChats, additionalCategories, chatListFilters) = mode {
            ...
            self.contentNode = .contacts(ContactListNode(context: context, presentation: .single(.natural(options: options, includeChatList: includeChatList)), filters: filters, selectionState: ContactListNodeGroupSelectionState(), mode: mode))
        }
            ...
            if let strongSelf = self {
                ...
                if text.isEmpty {
                    ...
                    if strongSelf.searchResultsNode == nil {
                        ...
                        switch mode {
                        case .groupCreation, .channelCreation, .publicChannelCreation:
                            globalSearch = true
                        ...
                        let searchResultsNode = ContactListNode(context: context, presentation: .single(.search(signal: searchText.get(), searchChatList: searchChatList, searchDeviceContacts: false, searchGroups: searchGroups, searchChannels: searchChannels, globalSearch: globalSearch)), filters: filters, selectionState: selectionState, isSearch: true, mode: mode, isSearchActive: true)
