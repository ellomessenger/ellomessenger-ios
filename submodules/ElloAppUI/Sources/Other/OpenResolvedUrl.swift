...
func openResolvedUrlImpl(_ resolvedUrl: ResolvedUrl, 
    ...
    switch resolvedUrl {
        ...
        case let .peer(peerId, navigation):
            if let peerId = peerId {
                ...
                present(textAlertController(context: context, 
                                            updatedPresentationData: updatedPresentationData,
                                            title: nil,
                                            text: presentationData.strings.Resolve_ErrorNotFound, 
            ...
            if let to = to {
                if to.hasPrefix("@") {
                    ...
                    |> deliverOnMainQueue).start(next: { (peers, _) in
                        for case let peer as ElloAppUser in peers {
                            if peer.phone == query {
                                context.sharedContext.applicationBindings.dismissNativeController()
                                continueWithPeer(peer.id)
                                break
        ...
        case let .wallpaper(parameter):
            ...
            let signal: Signal<ElloAppWallpaper, GetWallpaperError>
            ...
            switch parameter {
                ...
                case let .gradient(colors, rotation):
                    signal = .single(.gradient(ElloAppWallpaper.Gradient(id: nil, colors: colors, settings: WallpaperSettings(rotation: rotation))))
            }
        ...
        case let .theme(slug):
            ...
            |> mapToSignal { themeInfo -> Signal<(Data?, ElloAppThemeSettings?, ElloAppTheme), GetThemeError> in
                return Signal<(Data?, ElloAppThemeSettings?, ElloAppTheme), GetThemeError> { subscriber in
        ...
        case .importStickers:
            dismissInput()
            if let navigationController = navigationController, let data = UIPasteboard.general.data(forPasteboardType: "org.elloapp.third-party.stickerset"), let stickerPack = ImportStickerPack(data: data), !stickerPack.stickers.isEmpty {
        ...
        case .referralCode(code: let code):
            ELRegisterController.referralCode = code
            if let topController = navigationController?.topViewController as? ELRegisterController {
                topController.referalCodeInput.text = code
            }
        }
}
