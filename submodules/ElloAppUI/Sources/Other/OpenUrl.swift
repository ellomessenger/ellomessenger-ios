...
func openExternalUrlImpl(context: AccountContext, 
    ...
    let continueHandling: () -> Void = {
        let handleResolvedUrl: (ResolvedUrl) -> Void = { resolved in
            if case let .externalUrl(value) = resolved {
                ...
                context.sharedContext.openResolvedUrl(resolved, 
                                                      context: context,
                                                      urlContext: .generic,
                                                      navigationController: navigationController, 
                                                      forceExternal: false,
                                                      openPeer: { peerId, navigation in
                    switch navigation {
                        case .info:
            ...
            if let query = parsedUrl.query {
                if parsedUrl.host == "localpeer" {
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let invite = invite {
                            convertedUrl = "https://ello.team/joinchat/\(invite)"
                        }
                    }
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let set = set {
                            convertedUrl = "https://ello.team/addstickers/\(set)"
                        }
                    }
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let set = set {
                            convertedUrl = "https://ello.team/addemoji/\(set)"
                        }
                    }
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let slug = slug {
                            convertedUrl = "https://ello.team/invoice/\(slug)"
                        }
                    }
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let lang = lang {
                            convertedUrl = "https://ello.team/setlanguage/\(lang)"
                        }
                    }
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let shareUrl = shareUrl {
                            var resultUrl = "https://ello.team/share/url?url=\(urlEncodedStringFromString(shareUrl))"
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let server = server, !server.isEmpty, let port = port, let _ = Int32(port) {
                            var result = "https://ello.team/proxy?proxy=\(server)&port=\(port)"
                            if let user = user {
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let id = id, !id.isEmpty, let idValue = Int64(id), idValue > 0 {
                            let _ = (context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: PeerId(namespace: Namespaces.Peer.CloudUser, id: PeerId.Id._internalFromInt64Value(idValue))))
                            |> deliverOnMainQueue).start(next: { peer in
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let code = code {
                            convertedUrl = "https://ello.team/login/\(code)"
                        }
                    }
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let phone = phone, let hash = hash {
                            convertedUrl = "https://ello.team/confirmphone?phone=\(phone)&hash=\(hash)"
                        }
                    }
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let parameter = parameter {
                            convertedUrl = "https://ello.team/bg/\(parameter)\(queryString)"
                        }
                    }
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let parameter = parameter {
                            convertedUrl = "https://ello.team/addtheme/\(parameter)"
                        }
                    }
                    ...
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let channelId = channelId, let postId = postId {
                            convertedUrl = "https://ello.team/c/\(channelId)/\(postId)"
                        }
                    }
                }
                ...
                if parsedUrl.host == "resolve" {
                    if let components = URLComponents(string: "/?" + query) {
                        ...
                        if let phone = phone {
                            convertedUrl = "https://ello.team/+\(phone)"
                        } else if let domain = domain {
                            var result = "https://ello.team/\(domain)"
...
//                        navigationController?.pushViewController(controller)
    ...
    if parsedUrl.scheme == "http" || parsedUrl.scheme == "https" {
        let nativeHosts = ["ello.team"]
        if let host = parsedUrl.host, nativeHosts.contains(host) {
            continueHandling()
        } else {
            context.sharedContext.applicationBindings.openUniversalUrl(url, ElloAppApplicationOpenUrlCompletion(completion: { success in
                if !success {
                    continueHandling()
                }
            }))
