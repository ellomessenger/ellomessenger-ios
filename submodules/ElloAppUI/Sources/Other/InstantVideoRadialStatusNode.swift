...
private final class InstantVideoRadialStatusNodeParameters: NSObject {
    ...
    init(color: UIColor, progress: CGFloat, dimProgress: CGFloat, playProgress: CGFloat, blinkProgress: CGFloat, hasSeek: Bool, progressLineWidth: CGFloat) {
        self.color = color
...
final class InstantVideoRadialStatusNode: ASDisplayNode, UIGestureRecognizerDelegate {
    ...
    init(color: UIColor, progressLineWidth: CGFloat, hasSeek: Bool) {
        self.color = color
        self.hasSeek = hasSeek
        self.progressLineWidth = progressLineWidth
    ...
    override func drawParameters(forAsyncLayer layer: _ASDisplayLayer) -> NSObjectProtocol? {
        return InstantVideoRadialStatusNodeParameters(color: self.color, progress: self.effectiveProgress, dimProgress: self.effectiveDimProgress, playProgress: self.effectivePlayProgress, blinkProgress: self.effectiveBlinkProgress, hasSeek: self.hasSeek, progressLineWidth: self.progressLineWidth)
    }
    ...
    @objc override class func draw(_ bounds: CGRect, withParameters parameters: Any?, isCancelled: () -> Bool, isRasterizing: Bool) {
        ...
        if let parameters = parameters as? InstantVideoRadialStatusNodeParameters {
            ...
            if parameters.hasSeek {
                parameters.progressLineWidth += 1.0 * parameters.dimProgress
            }
            
            var pathDiameter = bounds.size.width - (parameters.progressLineWidth * 3)
            if parameters.hasSeek {
                pathDiameter -= (18.0 * 2.0) * parameters.dimProgress
            }
                
            if !parameters.dimProgress.isZero {
                context.setLineWidth(parameters.progressLineWidth)
                ...
                if !parameters.playProgress.isZero {
                    ...
                    let _ = try? drawSvgPath(context, path: "M4.7189,1.4207 C2.396,-0.17 -0.03,1.52 0,3.688 L0,14.2565 C-0.03,16.557 2.39,18.0 4.4331,16.7519 L13.0,11.8 C15.7,10.5263 15.7,7.5 13.0,6.205 L4.7189,1.4207 ")
                    
                    // OLD PLAY ICON
//                    "M1.71891969,0.209353049 C0.769586558,-0.350676705 0,0.0908839327 0,1.18800046 L0,16.8564753 C0,17.9569971 0.750549162,18.357187 1.67393713,17.7519379 L14.1073836,9.60224049 C15.0318735,8.99626906 15.0094718,8.04970371 14.062401,7.49100858 L1.71891969,0.209353049 "
            ...
            path.lineWidth = parameters.progressLineWidth
