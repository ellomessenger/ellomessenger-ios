...
public class EmojiThumbnailResource: ElloAppMediaResource {
    public let emoji: String
...
public class EmojiSpriteResource: ElloAppMediaResource {
    public let packId: UInt8
    public let stickerId: UInt8
...
func messageIsElligibleForLargeCustomEmoji(_ message: Message) -> Bool {
    ...
    for entity in entities {
        if case let .CustomEmoji(_, fileId) = entity.type {
            if let _ = message.associatedMedia[MediaId(namespace: Namespaces.Media.CloudFile, id: fileId)] as? ElloAppMediaFile {
...
func fetchEmojiSpriteResource(account: Account, resource: EmojiSpriteResource) -> Signal<MediaResourceDataFetchResult, MediaResourceDataFetchError> {
    ...
    return ElloAppEngine(account: account).stickers.loadedStickerPack(reference: .name(packName), forceActualized: false)
