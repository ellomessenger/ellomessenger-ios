...
public struct ShareRootControllerInitializationData {
    ...
    public init(appBundleId: String, appBuildType: ElloAppAppBuildType, appGroupPath: String, apiId: Int32, apiHash: String, languagesCategory: String, encryptionParameters: (Data, Data), appVersion: String, bundleData: Data?) {
...
public class ShareRootControllerImpl {
    ...
    public func viewDidLayoutSubviews(view: UIView, traitCollection: UITraitCollection) {
        if self.mainWindow == nil {
            ...
            let applicationBindings = ElloAppApplicationBindings(isMainApp: false, appBundleId: self.initializationData.appBundleId, appBuildType: self.initializationData.appBuildType, containerPath: self.initializationData.appGroupPath, appSpecificScheme: "tg", openUrl: { _ in
            ...
            let accountManager = AccountManager<ElloAppAccountManagerTypes>(basePath: rootPath + "/accounts-metadata", isTemporary: true, isReadOnly: false, useCaches: false, removeDatabaseOnError: false)
            ...
            |> mapToSignal { sharedContext, account, otherAccounts -> Signal<(AccountContext, PostboxAccessChallengeData, [AccountWithInfo]), ShareAuthorizationError> in
                let limitsConfigurationAndContentSettings = ElloAppEngine(account: account).data.get(
                                ...
                                switch content[0] {
                                    case let .contact(data):
                                        let controller = deviceContactInfoController(context: context, subject: .filter(peer: nil, contactId: nil, contactData: data, completion: { peer, contactData in
                                            let username = contactData.basicData.userNames[0].value
                                            if let vCardData = contactData.serializedVCard() {
                                                subscriber.putNext([.media(.media(.standalone(
                                                    media: ElloAppMediaContact(
                                                        firstName: contactData.basicData.firstName,
                                                        lastName: contactData.basicData.lastName,
                                                        username: username,
                                                        peerId: nil,
                                                        vCardData: vCardData,
                                                        userid: peer?.id.id._internalGetInt64Value() ?? 0))))])
                                            }
                            ...
                            if attachment.hasItemConformingToTypeIdentifier(UTType.fileURL.identifier) {
                                attachment.loadItem(forTypeIdentifier: UTType.fileURL.identifier, completionHandler: { result, error in
                                    Queue.mainQueue().async {
                                        ...
                                        var otherEntries: [(SSZipEntry, String, ElloAppEngine.HistoryImport.MediaType)] = []
                                                ...
                                                for entry in entries {
                                                    ...
                                                    if entryPath == mainFileName {
                                                        ...
                                                        if !entryFileName.isEmpty {
                                                            let mediaType: ElloAppEngine.HistoryImport.MediaType
                                                            let fullRange = NSRange(entryFileName.startIndex ..< entryFileName.endIndex, in: entryFileName)
                                                    ...
                                                    attemptSelectionImpl = { peer in
                                                        ...
                                                        if let errorText = errorText {
                                                            ...
                                                            |> deliverOnMainQueue).start(next: { result in
                                                                ...
                                                                } else if let _ = peer as? ElloAppUser {
                                                                } else {
                                                                    errorText = presentationData.strings.ChatImport_SelectionErrorGroupGeneric
                                                                }
