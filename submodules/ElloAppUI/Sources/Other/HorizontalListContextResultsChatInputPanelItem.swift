...
final class HorizontalListContextResultsChatInputPanelItemNode: ListViewItemNode {
    ...
    private var currentImageResource: ElloAppMediaResource?
    private var currentVideoFile: ElloAppMediaFile?
    private var currentAnimatedStickerFile: ElloAppMediaFile?
    ...
    func asyncLayout() -> (_ item: HorizontalListContextResultsChatInputPanelItem, _ params: ListViewItemLayoutParams, _ mergedTop: Bool, _ mergedBottom: Bool) -> (ListViewItemNodeLayout, (Bool, ListViewItemUpdateAnimation) -> Void) {
        ...
        return { [weak self] item, params, mergedTop, mergedBottom in
            ...
            var stickerFile: ElloAppMediaFile?
            var animatedStickerFile: ElloAppMediaFile?
            var videoFile: ElloAppMediaFile?
            ...
            switch item.result {
                case let .externalReference(externalReference):
                    ...
                    if externalReference.type == "gif", let thumbnailResource = externalReference.thumbnail?.resource, let content = externalReference.content, let dimensions = content.dimensions {
                        videoFile = ElloAppMediaFile(fileId: MediaId(namespace: Namespaces.Media.LocalFile, id: 0), partialReference: nil, resource: thumbnailResource, previewRepresentations: [], videoThumbnails: [], immediateThumbnailData: nil, mimeType: "video/mp4", size: nil, attributes: [.Animated, .Video(duration: 0, size: dimensions, flags: [])])
                        imageResource = nil
                    }
            ...
            if updatedImageResource {
                if let imageResource = imageResource {
                    if let stickerFile = stickerFile {
                        ...
                        let tmpRepresentation = ElloAppMediaImageRepresentation(dimensions: PixelDimensions(CGSize(width: fittedImageDimensions.width * 2.0, height: fittedImageDimensions.height * 2.0)), resource: imageResource, progressiveSizes: [], immediateThumbnailData: nil)
                        let tmpImage = ElloAppMediaImage(imageId: MediaId(namespace: 0, id: 0), representations: [tmpRepresentation], immediateThumbnailData: nil, reference: nil, partialReference: nil, flags: [])
