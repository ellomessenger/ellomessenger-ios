...
private func pollResultsControllerEntries(presentationData: PresentationData, poll: ElloAppMediaPoll, state: PollResultsControllerState, resultsState: PollResultsState) -> [PollResultsEntry] {
        ...
        if isEmpty {
            if let voterCount = optionVoterCount[i], voterCount != 0 {
                ...
                for peerIndex in 0 ..< displayCount {
                    let fakeUser = ElloAppUser(id: PeerId(namespace: .max, id: PeerId.Id._internalFromInt64Value(0)), accessHash: nil, firstName: "", lastName: "", username: nil, phone: nil, photo: [], botInfo: nil, restrictionInfo: nil, flags: [], emojiStatus: nil, isBusiness: false, isPublic: false, botDescription: nil)
...
public func pollResultsController(context: AccountContext, messageId: MessageId, poll: ElloAppMediaPoll, focusOnOptionWithOpaqueIdentifier: Data? = nil) -> ViewController {
