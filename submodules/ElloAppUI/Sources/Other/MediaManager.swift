...
public final class MediaManagerImpl: NSObject, MediaManager {
    ...
    init(accountManager: AccountManager<ElloAppAccountManagerTypes>, inForeground: Signal<Bool, NoError>, presentationData: Signal<PresentationData, NoError>) {
        ...
        |> mapToSignal { value -> Signal<UIImage?, NoError> in
            if let (account, value) = value {
                return albumArtThumbnailData(engine: ElloAppEngine(account: account), thumbnail: value.fullSizeResource)
                |> map { data -> UIImage? in
            ...
            if let (account, stateOrLoading, type) = accountStateAndType {
                ...
                if let minimumStoreDuration = minimumStoreDuration, case let .state(state) = stateOrLoading, state.status.duration >= minimumStoreDuration, case .playing = state.status.status {
                    if let item = state.item as? MessageMediaPlaylistItem {
                        ...
                        let _ = updateMediaPlaybackStoredStateInteractively(engine: ElloAppEngine(account: account), messageId: item.message.id, state: storedState).start()
                    }
                }
            }
        }))
    ...
    public func audioRecorder(beginWithTone: Bool, applicationBindings: ElloAppApplicationBindings, beganWithTone: @escaping (Bool) -> Void) -> Signal<ManagedAudioRecorder?, NoError> {
        return Signal { subscriber in
        ...
        if let (account, playlist) = playlist {
            ...
            |> mapToSignal { sharedData -> Signal<(Account, SharedMediaPlaylist, MusicPlaybackSettings, MediaPlaybackStoredState?)?, NoError> in
                ...
                if let location = playlist.location as? PeerMessagesPlaylistLocation, let messageId = location.messageId {
                    return mediaPlaybackStoredState(engine: ElloAppEngine(account: account), messageId: messageId)
                    |> map { storedState in
