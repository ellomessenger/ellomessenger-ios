...
final class EmojisChatInputPanelItem: ListViewItem {
    ...
    fileprivate let file: ElloAppMediaFile?
    fileprivate let animationCache: AnimationCache
    fileprivate let animationRenderer: MultiAnimationRenderer
    private let emojiSelected: (String, ElloAppMediaFile?) -> Void
    
    let selectable: Bool = true
    
    public init(context: AccountContext, theme: PresentationTheme, symbol: String, text: String, file: ElloAppMediaFile?, animationCache: AnimationCache, animationRenderer: MultiAnimationRenderer, emojiSelected: @escaping (String, ElloAppMediaFile?) -> Void) {
