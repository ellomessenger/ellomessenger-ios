...
final class OverlayPlayerControlsNode: ASDisplayNode {
    ...
    private let engine: ElloAppEngine
    ...
    init(account: Account, engine: ElloAppEngine, accountManager: AccountManager<ElloAppAccountManagerTypes>, presentationData: PresentationData, status: Signal<(Account, SharedMediaPlayerItemPlaybackStateOrLoading, MediaManagerPlayerType)?, NoError>) {
        ...
        self.albumArtNode.view.backgroundColor = .lightGray
        ...
        self.backwardButton.icon = generateTintedImage(image: UIImage(bundleImageName: "GlobalMusicPlayer/Previous"), color: self.presentationData.theme.list.itemAccentColor)
        self.forwardButton.icon = generateTintedImage(image: UIImage(bundleImageName: "GlobalMusicPlayer/Next"), color: self.presentationData.theme.list.itemAccentColor)
...
//        self.playPauseButton.addSubnode(self.playPauseIconNode)
        ...
        |> deliverOnMainQueue).start(next: { [weak self] value in
            ...
            if strongSelf.displayData != displayData {
                ...
                if let (_, valueOrLoading, _) = value, case let .state(value) = valueOrLoading, let source = value.item.playbackData?.source {
                    switch source {
                        case let .elloappFile(fileReference, isCopyProtected):
    ...
    private func updateLabels(transition: ContainedViewLayoutTransition) {
        ...
        if self.currentAlbumArt != albumArt || !self.currentAlbumArtInitialized {
            ...
            self.albumArtNode.setSignal(playerAlbumArt(postbox: self.postbox, 
                                                       engine: self.engine,
                                                       fileReference: self.currentFileReference,
                                                       albumArt: albumArt,
                                                       thumbnail: true))
            
            if let largeAlbumArtNode = self.largeAlbumArtNode {
                largeAlbumArtNode.setSignal(playerAlbumArt(postbox: self.postbox, 
                                                           engine: self.engine,
                                                           fileReference: self.currentFileReference,
                                                           albumArt: albumArt,
                                                           thumbnail: false))
...
//            self.playPauseIconNode.enqueueState(.play, animated: animated)
            self.playPauseButton.icon = UIImage(bundleImageName: "GlobalMusicPlayer/Play")
        } else {
//            self.playPauseIconNode.enqueueState(.pause, animated: animated)
            self.playPauseButton.icon = UIImage(bundleImageName: "GlobalMusicPlayer/PauseButtonIcon")
        }
    }
    ...
    private func updateOrderButton(_ order: MusicPlaybackSettingsOrder) {
        switch order {
            case .regular:
                self.orderButton.icon = generateTintedImage(image: UIImage(bundleImageName: "GlobalMusicPlayer/OrderReverse"), 
                                                            color: self.presentationData.theme.list.itemSecondaryTextColor)
            case .reversed:
                self.orderButton.icon = generateTintedImage(image: UIImage(bundleImageName: "GlobalMusicPlayer/OrderReverse"), 
                                                            color: self.presentationData.theme.list.itemAccentColor)
            case .random:
                self.orderButton.icon = generateTintedImage(image: UIImage(bundleImageName: "GlobalMusicPlayer/OrderRandom"), 
                                                            color: self.presentationData.theme.list.itemAccentColor)
        }
    }
...
private final class PlayPauseIconNode: ManagedAnimationNode {
    ...
    private let fileName = "anim_playpause"

    init() {
        super.init(size: CGSize(width: 76.0, height: 76.0))
        
        self.trackTo(item: ManagedAnimationItem(source: .local(fileName),
                                                frames: .range(startFrame: kPauseFrame, endFrame: kPauseFrame),
                                                duration: 0.01))
    }
            ...
            case .pause:
                switch state {
                    case .play:
                        if animated {
                            self.trackTo(item: ManagedAnimationItem(source: .local(fileName),
                                                                    frames: .range(startFrame: kPauseFrame, endFrame: kPlayFrame),
            ...
            case .play:
                switch state {
                    case .pause:
                        if animated {
                            self.trackTo(item: ManagedAnimationItem(source: .local(fileName),
                                                                    frames: .range(startFrame: 0, endFrame: kPauseFrame),
                                                                    duration: self.duration))
                        } else {
                            self.trackTo(item: ManagedAnimationItem(source: .local(fileName),
                                                                    frames: .range(startFrame: kPauseFrame, endFrame: kPauseFrame),
