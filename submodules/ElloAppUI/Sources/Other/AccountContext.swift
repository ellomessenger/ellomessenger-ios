    ...
    func update(account: Account, deviceContactDataManager: DeviceContactDataManager, references: [PeerId: DeviceContactBasicDataWithReference]) {
        ...
        for (peerId, reference) in references {
            ...
            if context.reference != reference {
                ...
                let signal = ElloAppEngine(account: account).data.subscribe(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId))
                |> map { peer -> String? in
                ...
                |> mapToSignal { username -> Signal<Never, NoError> in
                    guard let username = username else {
                        return .complete()
                    }
                    
                    if !reference.basicData.userNames.contains(where: {$0.value == username}) {
                        return deviceContactDataManager.appendUserName(DeviceContactUserNameData(label: "_$!<Mobile>!$_",
                                                                                                    value: username),
                                                                          to: reference.stableId)
                        |> ignoreValues
                    } else {
                        return .complete()
                    }
...
public final class AccountContextImpl: AccountContext {
    ...
    public let engine: ElloAppEngine
    ...
    public init(sharedContext: SharedAccountContextImpl, 
                account: Account,
                ...
                temp: Bool = false)
    {
        ...
        self.engine = ElloAppEngine(account: account)
        ...
        |> mapToSignal { peerView -> Signal<EngineConfiguration.UserLimits, NoError> in
            if let peer = peerView.peers[peerView.peerId] {
                return self.engine.data.subscribe(ElloAppEngine.EngineData.Item.Configuration.UserLimits(isPremium: peer.isPremium))
            } else {
                return .complete()
            }
        }
...
//        let _ = (self.engine.peers.adminedPublicChannels(scope: .all) |> deliverOnMainQueue |> map { peers -> () in
//            print(peers)
//        }).start()
    }
            ...
            if currentPeerId == peerId {
                ...
                if let currentPeerId = currentPeerId {
                    dataInput = self.engine.data.get(
                        ...
                        ElloAppEngine.EngineData.Item.Peer.Peer(id: currentPeerId)
                    )
                } else {
                    dataInput = self.engine.data.get(
                        ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId)
