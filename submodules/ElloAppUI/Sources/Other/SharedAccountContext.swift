...
var isWelcomSlideHidden:Bool = false

public final class SharedAccountContextImpl: SharedAccountContext {
    public let mainWindow: Window1?
    public let applicationBindings: ElloAppApplicationBindings
    public let sharedContainerPath: String
    public let basePath: String
    public let accountManager: AccountManager<ElloAppAccountManagerTypes>
    ...
    public init(mainWindow: Window1?, sharedContainerPath: String, basePath: String, encryptionParameters: ValueBoxEncryptionParameters, accountManager: AccountManager<ElloAppAccountManagerTypes>, appLockContext: AppLockContext, applicationBindings: ElloAppApplicationBindings, initialPresentationDataAndSettings: InitialPresentationDataAndSettings, networkArguments: NetworkInitializationArguments, hasInAppPurchases: Bool, rootPath: String, legacyBasePath: String?, apsNotificationToken: Signal<Data?, NoError>, voipNotificationToken: Signal<Data?, NoError>, setNotificationCall: @escaping (PresentationCall?) -> Void, navigateToChat: @escaping (AccountRecordId, PeerId, MessageId?) -> Void, displayUpgradeProgress: @escaping (Float?) -> Void = { _ in }) {
        ...
        |> deliverOnMainQueue).start(next: { primaryId, records, authRecord in
            ...
            for (id, attributes) in records {
                if self.activeAccountsValue?.accounts.firstIndex(where: { $0.0 == id}) == nil {
                    addedSignals.append(accountWithId(accountManager: accountManager, 
                                                      networkArguments: networkArguments,
                                                      id: id,
                                                      encryptionParameters: encryptionParameters,
                                                      supplementary: !applicationBindings.isMainApp,
                                                      rootPath: rootPath,
                                                      beginWithTestingEnvironment: attributes.isTestingEnvironment,
                                                      backupData: attributes.backupData,
                                                      auxiliaryMethods: elloappAccountAuxiliaryMethods)
                    |> mapToSignal { result -> Signal<AddedAccountResult, NoError> in
                        switch result {
                            case let .authorized(account):
                                ...
                                return ElloAppEngine(account: account).data.get(
                                    ElloAppEngine.EngineData.Item.Configuration.Limits(),
                                    ElloAppEngine.EngineData.Item.Configuration.ContentSettings(),
            ...
            |> deliverOnMainQueue).start(next: { mappedAddedAccounts, authAccount in
                ...
                for accountRecord in addedAccounts {
                    if let account = accountRecord.1 {
                        if existingAccountPeerKeys.contains(AccountPeerKey(peerId: account.peerId, isTestingEnvironment: account.testingEnvironment)) {
                            ...
                            let context = AccountContextImpl(sharedContext: self, 
                                                             account: account,
                                                             limitsConfiguration: accountRecord.3 ?? .defaultValue,
                                                             contentSettings: accountRecord.4 ?? .default,
                                                             appConfiguration: accountRecord.5 ?? .defaultValue)
                ...
                if self.activeAccountsValue!.primary == nil && self.activeAccountsValue!.currentAuth == nil {
                    self.beginNewAuth(testingEnvironment: false, isWelcomHidden: false)
                }
            }))
        })
        ...
        |> mapToSignal { primary, accounts, _ -> Signal<(primary: AccountRecordId?, accounts: [AccountWithInfo]), NoError> in
            return combineLatest(accounts.map { _, context, _ -> Signal<AccountWithInfo?, NoError> in
                return context.engine.data.subscribe(ElloAppEngine.EngineData.Item.Peer.Peer(id: context.account.peerId))
        ...
        let _ = managedCleanupAccounts(networkArguments: networkArguments, accountManager: self.accountManager, rootPath: rootPath, auxiliaryMethods: elloappAccountAuxiliaryMethods, encryptionParameters: encryptionParameters).start()
        ...
        |> mapToSignal { backupData -> Signal<Never, NoError> in
            ...
            return self.accountManager.transaction { transaction -> Void in
                transaction.updateRecord(account.id, { record in
                    ...
                    var attributes: [ElloAppAccountManagerTypes.Attribute] = record.attributes.filter { attribute in
    ...
    var transaction:Signal<Void, NoError>?
    ...
    public func beginNewAuth(testingEnvironment: Bool, isWelcomHidden:Bool? = nil) {
        isWelcomSlideHidden = isWelcomHidden ?? true
        _ = self.accountManager.transaction({ transaction -> Void in
...
//        }
        ...
        if let rootController = self.mainWindow?.viewController as? ElloAppRootController {
    ...
    public func openFeedController(activateRecomended: Bool) {
        if let rootController = self.mainWindow?.viewController as? ElloAppRootController {
            rootController.openFeedController(activateRecomended: activateRecomended)
        }
    }
    
    public func openContactsController() {
        if let rootController = self.mainWindow?.viewController as? ElloAppRootController {
            rootController.openContactsController()
        }
    }
    ...
    public func chatAvailableMessageActions(engine: ElloAppEngine, accountPeerId: EnginePeer.Id, messageIds: Set<EngineMessage.Id>) -> Signal<ChatAvailableMessageActions, NoError> {
        return chatAvailableMessageActionsImpl(engine: engine, accountPeerId: accountPeerId, messageIds: messageIds)
    }
    
    public func chatAvailableMessageActions(engine: ElloAppEngine, accountPeerId: EnginePeer.Id, messageIds: Set<EngineMessage.Id>, messages: [EngineMessage.Id: EngineMessage] = [:], peers: [EnginePeer.Id: EnginePeer] = [:]) -> Signal<ChatAvailableMessageActions, NoError> {
...
//                params.openPeer(peer, .info)
            ...
            let _ = (context.engine.data.get(ElloAppEngine.EngineData.Item.Messages.Message(id: messageId))
    ...
    public func openResolvedUrl(_ resolvedUrl: ResolvedUrl, 
                                context: AccountContext,
                                urlContext: OpenURLContext,
                                navigationController: NavigationController?,
                                forceExternal: Bool,
                                openPeer: @escaping (PeerId, ChatControllerInteractionNavigateToPeer) -> Void,
                                sendFile: ((FileMediaReference) -> Void)?,
                                sendSticker: ((FileMediaReference, UIView, CGRect) -> Bool)?,
                                requestMessageActionUrlAuth: ((MessageActionUrlSubject) -> Void)?,
    ...
    public func makeDeviceContactInfoController(context: AccountContext, subject: DeviceContactInfoSubject, isTitleHidden:Bool = false, completed: (() -> Void)?, cancelled: (() -> Void)?) -> ViewController {
        return deviceContactInfoController(context: context, subject: subject, isTitleHidden:isTitleHidden, completed: completed, cancelled: cancelled)
    }
    ...
    public func openAddContact(context: AccountContext, firstName: String, lastName: String, userName: String, label: String, present: @escaping (ViewController, Any?) -> Void, pushController: @escaping (ViewController) -> Void, completed: @escaping () -> Void) {
        openAddContactImpl(context: context, firstName: firstName, lastName: lastName, userName: userName, label: label, present: present, pushController: pushController, completed: completed)
    }
    ...
    public func makeChatMessagePreviewItem(context: AccountContext, messages: [Message], theme: PresentationTheme, strings: PresentationStrings, wallpaper: ElloAppWallpaper, fontSize: PresentationFontSize, chatBubbleCorners: PresentationChatBubbleCorners, dateTimeFormat: PresentationDateTimeFormat, nameOrder: PresentationPersonNameOrder, forcedResourceStatus: FileMediaResourceStatus?, tapMessage: ((Message) -> Void)?, clickThroughMessage: (() -> Void)? = nil, backgroundNode: ASDisplayNode?, availableReactions: AvailableReactions?, isCentered: Bool) -> ListViewItem {
    ...
    public func makeChatMessageDateHeaderItem(context: AccountContext, timestamp: Int32, theme: PresentationTheme, strings: PresentationStrings, wallpaper: ElloAppWallpaper, fontSize: PresentationFontSize, chatBubbleCorners: PresentationChatBubbleCorners, dateTimeFormat: PresentationDateTimeFormat, nameOrder: PresentationPersonNameOrder) -> ListViewItemHeader {
    ...
    public func makeChatQrCodeScreen(context: AccountContext, peer: Peer, inviteLink: String) -> ViewController {
        return ChatQrCodeScreen(context: context, subject: .peer(peer, inviteLink))
    }
...
private func peerInfoControllerImpl(context: AccountContext, 
                                    updatedPresentationData: (PresentationData, Signal<PresentationData, NoError>)?,
                                    peer: Peer,
                                    mode: PeerInfoControllerMode,
                                    avatarInitiallyExpanded: Bool,
                                    isOpenedFromChat: Bool,
                                    requestsContext: PeerInvitationImportersContext? = nil) -> ViewController? {
    if let _ = peer as? ElloAppGroup {
        return PeerInfoScreenImpl(context: context, 
    ...
    } else if peer is ElloAppSecretChat {
        return PeerInfoScreenImpl(context: context, 
                                  updatedPresentationData: updatedPresentationData,
                                  peerId: peer.id,
                                  avatarInitiallyExpanded: avatarInitiallyExpanded,
