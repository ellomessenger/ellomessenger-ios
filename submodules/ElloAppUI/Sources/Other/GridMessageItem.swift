...
final class GridMessageItemNode: GridItemNode {
    ...
    private func progressPressed() {
        ...
        if let (context, media, _) = self.currentState, let resourceStatus = self.resourceStatus, let file = media as? ElloAppMediaFile {
        ...
        switch recognizer.state {
            case .ended:
                if let (gesture, _) = recognizer.lastRecognizedGestureAndLocation {
                    switch gesture {
                        case .tap:
                            if let (_, media, _) = self.currentState, let file = media as? ElloAppMediaFile {
