...
/*private final class VisualMediaItemNode: ASDisplayNode {
                ...
                if case .tap = gesture {
                    if let (item, _, _, _) = self.item, let message = item.message {
                        ...
                        for value in message.media {
                            if let image = value as? ElloAppMediaImage {
                                media = image
                                break
                            } else if let file = value as? ElloAppMediaFile {
                                media = file
                                break
                            }
                        }
    ...
    private func progressPressed() {
        ...
        if let resourceStatus = self.resourceStatus, let file = media as? ElloAppMediaFile {
    ...
    func update(size: CGSize, item: VisualMediaItem?, theme: PresentationTheme, synchronousLoad: Bool) {
        ...
        if let item = item, let message = item.message, let file = media as? ElloAppMediaFile, file.isAnimated {
        ...
        if let item = item, let message = item.message, let media = media, (self.item?.1 == nil || !media.isEqual(to: self.item!.1!)) {
            ...
            if let image = media as? ElloAppMediaImage, let largestSize = largestImageRepresentation(image.representations)?.dimensions {
    ...
    func updateIsVisible(_ isVisible: Bool) {
        ...
        /*if isVisible {
            if let item = self.item?.0, let file = self.item?.1 as? ElloAppMediaFile, !file.isAnimated {
        ...
        for i in 0 ..< items.count {
            ...
            if self.useListItems {
                ...
                if let selectedMedia = selectedMedia {
                    ...
                    if let file = selectedMedia as? ElloAppMediaFile, !file.isAnimated {
    ...
    init(context: AccountContext, chatControllerInteraction: ChatControllerInteraction, peerId: PeerId, contentType: ContentType, captureProtected: Bool) {
        ...
        |> mapToSignal { contentType -> Signal<(ContentType, [MessageTags: Int32]), NoError> in
            ...
            return context.engine.data.subscribe(EngineDataMap(
                summaries.map { ElloAppEngine.EngineData.Item.Messages.MessageCount(peerId: peerId, tag: $0) }
            ))
...
func visualMediaStoredState(engine: ElloAppEngine, peerId: PeerId, messageTag: MessageTags) -> Signal<VisualMediaStoredState?, NoError> {
    let key = ValueBoxKey(length: 8 + 4)
    key.setInt64(0, value: peerId.toInt64())
    key.setUInt32(8, value: messageTag.rawValue)
    
    return engine.data.get(ElloAppEngine.EngineData.Item.ItemCache.Item(collectionId: ApplicationSpecificItemCacheCollectionId.visualMediaStoredState, id: key))
    |> map { entry -> VisualMediaStoredState? in
        return entry?.get(VisualMediaStoredState.self)
    }
}

func updateVisualMediaStoredState(engine: ElloAppEngine, peerId: PeerId, messageTag: MessageTags, state: VisualMediaStoredState?) -> Signal<Never, NoError> {
    let key = ValueBoxKey(length: 8 + 4)
    key.setInt64(0, value: peerId.toInt64())
    key.setUInt32(8, value: messageTag.rawValue)
