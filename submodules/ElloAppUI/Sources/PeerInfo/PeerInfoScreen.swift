...
private final class PeerInfoScreenItemSectionContainerNode: ASDisplayNode {
    ...
    func update(width: CGFloat, 
        ...
        for i in 0 ..< items.count {
            ...
            let itemHeight = itemNode.update(width: width,
...
// MARK: - PeerInfoSelectionPanelNode
...
private func settingsItems(data: PeerInfoScreenData?, 
                           context: AccountContext,
                           presentationData: PresentationData,
                           interaction: PeerInfoInteraction,
                           isExpanded: Bool) -> [(AnyHashable, [PeerInfoScreenItem])] {
    ...
    for section in SettingsSection.allCases {
        // If peer is not private user - don't add invite section
        if (data.peer as? ElloAppUser)?.isPublic == true, section == .invite {
            continue
        }
...
//    }
    ...
    if let settings = data.globalSettings {
        if settings.suggestPhoneNumberConfirmation, let peer = data.peer as? ElloAppUser {
...
//            }))
        ...
        if !settings.accountsAndPeers.isEmpty {
            for (peerAccountContext, peer, badgeCount) in settings.accountsAndPeers {
                ...
                items[.accounts]!.append(
                    PeerInfoScreenMemberItem(id: member.id,
                                             context: context.sharedContext.makeTempAccountContext(account: peerAccountContext.account),
                                             enclosingPeer: nil,
                                             member: member,
                                             badge: badgeCount > 0 ? "\(compactNumericCountString(Int(badgeCount), decimalSeparator: presentationData.dateTimeFormat.decimalSeparator))" : nil,
                                             action: { action in
                    switch action {
                        case .open:
        ...
        items[.accounts]!.append(PeerInfoScreenActionItem(id: 100,
                                                          text: presentationData.strings.Settings_AddAccount,
                                                          color: .text,
        ...
        if !settings.proxySettings.servers.isEmpty {
            ...
            if settings.proxySettings.enabled, let activeServer = settings.proxySettings.activeServer {
                switch activeServer.connection {
                    case .mtp:
                        proxyType = presentationData.strings.SocksProxySetup_ProxyElloApp
                    case .socks5:
...
//    if notification
    ...
    if data.globalSettings?.notificationAuthorizationStatus == .denied {
        items[.permissions]?.append(PeerInfoScreenPermissionItem(context: context, theme: presentationData.theme, strings: presentationData.strings, id: 0, title: presentationData.strings.Permissions_NotificationsTitle_v0, text: presentationData.strings.Permissions_NotificationsText_v0, icon: UIImage(bundleImageName: "Settings/Permissions/Notifications"),  buttonTitle: presentationData.strings.Permissions_NotificationsAllowInSettings_v0, action: { [weak context] in
    ...
    if let user = data.peer as? ElloAppUser {
        if user.isPublic {
            let link = "https://ello.team/\(user.username ?? "n/a")"
            
            items[.qr]?.append(PeerInfoItemQR(id: 0, link: link, action: {
                interaction.openSettings(.qrCode($0))
            }, copyAction: {
                interaction.openSettings(.qrCopy($0))
            }, shareAction: {
                interaction.openSettings(.qrCodeShare($0))
            }))
        }/* else {
          data.privateUserLink ?? "Loading..."
          }*/
    }
     
     // MARK: Bio
     if let cachedData = data.cachedData as? CachedUserData {
         items[.bio]?.append(PeerInfoScreenBioItem(id: 0, label: "Bio", text: cachedData.about ?? cachedData.botDescription ?? "", linkItemAction: { [weak interaction] action, item in
             interaction?.performBioLinkAction(action, item)
         }))
     } else {
         items[.bio]?.append(PeerInfoScreenBioItem(id: 0, label: "Loading...", text: ""))
     }
    
    items[.ello]?.append(PeerInfoScreenSettingsItem(id: 0,
                                                    text: presentationData.strings.Settings_ReferralProgram,
                                                    icon: PresentationResourcesSettings.loyalty,
                                                    action: {
        interaction.openSettings(.loyalty)
    }))
    
    items[.ello]?.append(PeerInfoScreenSettingsItem(id: 1, text: presentationData.strings.Settings_AISpace,
                                                    icon: PresentationResourcesSettings.elloAI,
                                                    action: {
        interaction.openSettings(.elloAI)
    }))
    
    items[.ello]?.append(PeerInfoScreenSettingsItem(id: 2, text: presentationData.strings.Settings_SavedMessages,
                                                    icon: PresentationResourcesSettings.cloud,
                                                    action: {
        interaction.openSettings(.cloudStorage)
    }))

    items[.ello]?.append(PeerInfoScreenSettingsItem(id: 3, text: presentationData.strings.Settings_Notifications, 
                                                    icon: PresentationResourcesSettings.profileNotifications,
                                                    action: {
        interaction.openSettings(.notifications)
    }))
    
    // MARK: - New Items with interactions
    
    items[.payment]?.append(PeerInfoScreenSettingsItem(id: 0, 
                                                       text: presentationData.strings.Settings_Wallet,
                                                       icon: PresentationResourcesSettings.wallet,
                                                       action: {
        interaction.openSettings(.wallet)
    }))
    // EM-2743
//    items[.payment]?.append(PeerInfoScreenDisclosureItem(id: 1, text: presentationData.strings.Settings_Purchases, icon: PresentationResourcesSettings.purchases, action: {
//        interaction.openSettings(.purchases)
//    }))
    if let subscriptionsCount = data.subscriptionsCount {
        items[.payment]?.append(PeerInfoScreenSettingsItem(id: 2, label: .badge("\(subscriptionsCount)", .clear), text: presentationData.strings.Settings_Subscriptions, icon: PresentationResourcesSettings.subscriptions, action: {
            interaction.openSettings(.subscriptions)
        }))
    } else {
        items[.payment]?.append(PeerInfoScreenSettingsItem(id: 2, text: presentationData.strings.Settings_Subscriptions, icon: PresentationResourcesSettings.subscriptions, action: {
            interaction.openSettings(.subscriptions)
        }))
    }
    
    let inviteSection: SettingsSection
    let inviteText: String
    if let user = data.peer as? ElloAppUser, !user.isPublic {
        inviteSection = .invite
        inviteText = presentationData.strings.Settings_InviteFriends
    } else {
        inviteSection = .advanced
        inviteText = presentationData.strings.Settings_Invite
    }
    
    items[inviteSection]?.append(PeerInfoScreenSettingsItem(
        id: 0,
        text: inviteText,
        icon: PresentationResourcesSettings.invite,
        action: {
            interaction.openSettings(.invite)
        })
    )
    // EM-2743
//    items[.advanced]?.append(PeerInfoScreenDisclosureItem(id: 1, text: presentationData.strings.Settings_Folders, icon: PresentationResourcesSettings.folders, action: {
//        interaction.openSettings(.folders)
//    }))
    
    items[.advanced]?.append(PeerInfoScreenSettingsItem(id: 2,
                                                        text: presentationData.strings.Settings_Settings,
                                                        icon: PresentationResourcesSettings.settings,
                                                        action: {
        interaction.openSettings(.settings)
    }))
    
    // MARK: Support
    items[.extra]!.append(PeerInfoScreenSettingsItem(id: 1, text: "Information", icon: UIImage(bundleImageName: "Settings/ElloSettings/info"), action: {
        interaction.openSettings(.info)
    }))
    
    items[.extra]!.append(PeerInfoScreenSettingsItem(id: 2, text: "Support", icon: UIImage(bundleImageName: "Settings/ElloSettings/support"), action: {
        interaction.openSettings(.support)
    }))
    
// MARK: - Logout
    
    items[.support]?.append(PeerInfoScreenActionItem(id: 1, 
...
// Previous Edit Info Items

private func peerStatus(data: PeerInfoScreenData?, presentationData: PresentationData) -> String {
    guard let user = data?.peer as? ElloAppUser else {
        return ""
    }
...
//    let ItemPhoneNumber = 3
    ...
    items[.info]!.append(PeerInfoScreenDisclosureItem(id: ItemUsername, label: .status(peerStatus(data: data, presentationData: presentationData), username, UIColor.black), text: presentationData.strings.Settings_Username, action: {
          interaction.openSettings(.username)
    }))
...
//        if data.peer?.id.isAIBotID ?? false {
//            items[.peerInfo]?.append(
//                PeerInfoItemQR(
//                    id: 0,
//                    link: "@\(data.peer?.addressName ?? "n/a")",
...
//                PeerInfoItemQR(
//                    id: 0,
//                    link: "https://ello.team/\(user.username ?? "n/a")",
//                    isButtonsHidden:true,
//                    action: {
//                        interaction.openSettings(.qrCode($0))
//                    },
...
//            )
        ...
        if let username = user.username, user.isPublic {
                items[.peerInfo]!.append(PeerInfoScreenLabeledValueItem(
                    id: 0,
                    label: presentationData.strings.User_LinkItem,
                    text: "@\(username)",
                    textColor: .accent,
                    icon: .qrCode,
                    action: { _ in
...
//        }
        ...
        if let cachedData = data.cachedData as? CachedUserData {
            if user.isFake {
                items[.peerInfo]!.append(PeerInfoScreenLabeledValueItem(id: 1, label: user.botInfo == nil ? presentationData.strings.Profile_About : presentationData.strings.Profile_BotInfo, text: user.botInfo != nil ? presentationData.strings.UserInfo_FakeBotWarning : presentationData.strings.UserInfo_FakeUserWarning, textColor: .primary, textBehavior: .multiLine(maxLines: 100, enabledEntities: user.botInfo != nil ? enabledPrivateBioEntities : []), action: nil, requestLayout: {
                    interaction.requestLayout(false)
                }))
            } else if user.isScam {
                items[.peerInfo]!.append(PeerInfoScreenLabeledValueItem(id: 1, label: user.botInfo == nil ? presentationData.strings.Profile_About : presentationData.strings.Profile_BotInfo, text: user.botInfo != nil ? presentationData.strings.UserInfo_ScamBotWarning : presentationData.strings.UserInfo_ScamUserWarning, textColor: .primary, textBehavior: .multiLine(maxLines: 100, enabledEntities: user.botInfo != nil ? enabledPrivateBioEntities : []), action: nil, requestLayout: {
                    interaction.requestLayout(false)
                }))
            } else if let about = cachedData.about, !about.isEmpty {
                items[.peerInfo]!.append(PeerInfoScreenLabeledValueItem(id: 1, label: user.botInfo == nil ? presentationData.strings.Profile_About : presentationData.strings.Profile_BotInfo, text: about, textColor: .primary, textBehavior: .multiLine(maxLines: 100, enabledEntities: user.isPremium ? enabledPublicBioEntities : enabledPrivateBioEntities), action: nil, longTapAction: bioContextAction, linkItemAction: bioLinkAction, requestLayout: {
                    interaction.requestLayout(false)
                }))
            } else if let botDescription = cachedData.botDescription, !botDescription.isEmpty {
                items[.peerInfo]!.append(PeerInfoScreenLabeledValueItem(id: 1, label: user.botInfo == nil ? presentationData.strings.Profile_About : presentationData.strings.Profile_BotInfo, text: botDescription, textColor: .primary, textBehavior: .multiLine(maxLines: 100, enabledEntities: user.isPremium ? enabledPublicBioEntities : enabledPrivateBioEntities), action: nil, longTapAction: bioContextAction, linkItemAction: bioLinkAction, requestLayout: {
                    interaction.requestLayout(false)
                }))
            }
        }
        ...
        if let reactionSourceMessageId = reactionSourceMessageId, !data.isContact {
        ...
        } else if user.id.isNotificationsBotID {
        } else {
            if !data.isContact {
                if user.botInfo == nil {
            ...
            if isBlocked {
                ...
                if user.flags.contains(.isSupport) {
                    ...
                    if user.botInfo == nil {
                        items[.peerInfo]!.append(PeerInfoScreenActionItem(id: 4, text: presentationData.strings.Conversation_Block, color: .destructive, action: {
    ...
    } else if let channel = data.peer as? ElloAppChannel {
        let ItemUsername = 1
        let ItemAbout = 2
        ...
        let ItemSubscriptionStartDate = 8
        let ItemSubscriptionEndDate = 9
        let ItemSubscriptionCost = 10
        ...
        if let username = channel.username {
            if let cost = channel.cost {
                if channel.isCourse, let startDateTI = channel.startDate, let endDateTI = channel.endDate {
                    let startDate = Date(timeIntervalSince1970: TimeInterval(startDateTI))
                    let endDate = Date(timeIntervalSince1970: TimeInterval(endDateTI))
                    items[.paidChannel]!.append(PeerInfoScreenCostItem(
                        id: ItemSubscriptionStartDate,
                        label: .text(startDate.toString(dateFormat: .MMMddyyyy)),
                        text: presentationData.strings.PeerInfo_StartDateCourse,
                        icon: UIImage(bundleImageName: "Peer Info/Calendar"),
                        action: nil)
                    )
                    items[.paidChannel]!.append(PeerInfoScreenCostItem(
                        id: ItemSubscriptionEndDate,
                        label: .text(endDate.toString(dateFormat: .MMMddyyyy)),
                        text: presentationData.strings.PeerInfo_EndDate,
                        icon: UIImage(bundleImageName: "Peer Info/Calendar"),
                        action: nil)
                    )
                } else if !channel.isCreator, channel.isSubscription, let nextDueDateTI = data.nextDueDate {
                    let nextDueDate = Date(timeIntervalSince1970: nextDueDateTI)
                    items[.paidChannel]!.append(PeerInfoScreenCostItem(
                        id: ItemSubscriptionEndDate,
                        label: .text(nextDueDate.toString(dateFormat: .MMMddyyyy)),
                        text: presentationData.strings.PeerInfo_NextDueDate,
                        icon: UIImage(bundleImageName: "Peer Info/Calendar"),
                        action: nil)
                    )
                }
                
                let text = channel.isCourse ? presentationData.strings.PeerInfo_SubscriptionOnlineCourse : presentationData.strings.PeerInfo_SubscriptionCost
                
                items[.paidChannel]!.append(PeerInfoScreenCostItem(
                    id: ItemSubscriptionCost,
                    label: .textWithImage(
                        cost.stringFinanceFormatWithDollar,
                        nil/*UIImage(systemName: "dollarsign")*/),
                    text: text,
                    icon: UIImage(bundleImageName: "Peer Info/Withdraw"),
                    action: nil)
                )
            }
        ...
        if let cachedData = data.cachedData as? CachedChannelData {
            ...
            if case .broadcast = channel.info {
                ...
                if channel.hasPermission(.inviteMembers) || channel.hasPermission(.banMembers) {
                    canEditMembers = true
                }
                if canEditMembers {
                    let isCreator = channel.flags.contains(.isCreator)
                    if channel.adminRights != nil || isCreator {
    ...
    } else if let group = data.peer as? ElloAppGroup {
    ...
    if let peer = data.peer, let members = data.members, case let .shortList(_, memberList) = members {
        ...
        if let group = data.peer as? ElloAppGroup {
            switch group.role {
                case .admin, .creator:
                    canAddMembers = true
                case .member:
...
private func editingItems(data: PeerInfoScreenData?, context: AccountContext, presentationData: PresentationData, interaction: PeerInfoInteraction) -> [(AnyHashable, [PeerInfoScreenItem])] {
    ...
    if let data = data {
        if let _ = data.peer as? ElloAppUser {
            let ItemDelete = 0
            if data.isContact {
...
//                let ItemSignMessagesHelp = 5
                ...
                if isCreator {
                    ...
                    if channel.isSubscription {
                        linkText = presentationData.strings.Channel_Setup_TypeSubscription
                    } else if channel.isCourse {
                        linkText = presentationData.strings.Channel_Setup_TypeOnlineCourse
                    } else if let _ = channel.username {
                        linkText = presentationData.strings.Channel_Setup_TypePublic
                    } else {
                        linkText = presentationData.strings.Channel_Setup_TypePrivate
                    }
                    
                    items[.peerSettings]!.append(PeerInfoScreenDisclosureItem(id: ItemUsername, 
                                                                              label: .text(linkText),
                                                                              text: presentationData.strings.Channel_TypeSetup_Title,
                                                                              icon: UIImage(bundleImageName: "Chat/Info/GroupChannelIcon"),
                                                                              action: {
...
//                let ItemStickerPack = 105
                ...
                if let cachedData = data.cachedData as? CachedChannelData {
                    ...
                    if isCreator || (channel.adminRights != nil && channel.hasPermission(.pinMessages)) {
                        if cachedData.peerGeoLocation != nil {
                            if isCreator {
                                ...
                                items[.peerSettings]!.append(PeerInfoScreenDisclosureItem(id: ItemUsername, 
                                                                                          label: .text(linkText),
                                                                                          text: presentationData.strings.GroupInfo_PublicLink,
                                                                                          icon: UIImage(bundleImageName: "Chat/Info/GroupLinksIcon"),
                                                                                          action: {
                                    interaction.editingOpenPublicLinkSetup()
                                }))
                            }
                            ...
                            if cachedData.flags.contains(.canChangeUsername) {
                                items[.peerPublicSettings]!.append(PeerInfoScreenDisclosureItem(id: ItemUsername, 
                                                                                                label: .text(isPublic ? presentationData.strings.Group_Setup_TypePublic : presentationData.strings.Group_Setup_TypePrivate),
                                                                                                text: presentationData.strings.GroupInfo_GroupType,
                                                                                                icon: UIImage(bundleImageName: "Chat/Info/GroupTypeIcon"),
                                                                                                action: {
                                    interaction.editingOpenPublicLinkSetup()
                                }))
                            }
                    ...
                    if (isCreator && (channel.username?.isEmpty ?? true) && cachedData.peerGeoLocation == nil) || (!isCreator && channel.adminRights?.rights.contains(.canInviteUsers) == true) {
                        ...
                        items[.peerPublicSettings]!.append(PeerInfoScreenDisclosureItem(id: ItemInviteLinks, 
                                                                                        label: .text(invitesText),
                                                                                        text: presentationData.strings.GroupInfo_InviteLinks,
                                                                                        icon: UIImage(bundleImageName: "Chat/Info/GroupLinksIcon"),
                                                                                        action: {
                            interaction.editingOpenInviteLinksSetup()
                        }))
...
final class PeerInfoScreenNode: ViewControllerTracingNode, UIScrollViewDelegate {
    ...
    private var customAvatarDisposable: Disposable?
    ...
    private var privateUserLink: String?
    ...
    public static var onChangeEmailCodeToVerify: ((_ email: String, _ code: String?) -> ())?
    
    init(controller: PeerInfoScreenImpl,
         context: AccountContext,
         peerId: PeerId,
        ...
        self.paneContainerNode = PeerInfoPaneContainerNode(context: context,
        ...
        self._chatInterfaceInteraction = ChatControllerInteraction(openMessage: { [weak self] message, mode in
            ...
            |> deliverOnMainQueue).start(next: { actions in
                ...
                items.append(.action(ContextMenuActionItem(text: strongSelf.presentationData.strings.SharedMedia_ViewInChat, 
                                                           icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/GoToMessage"), color: theme.contextMenu.primaryColor) }, 
                                                           action: { c, _ in
                ...
                if actions.options.contains(.deleteLocally) || actions.options.contains(.deleteGlobally) {
                    ...
                    items.append(.action(ContextMenuActionItem(text: strongSelf.presentationData.strings.Conversation_ContextMenuDelete, textColor: .destructive, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Delete"), color: theme.contextMenu.destructiveColor) }, action: { c, _ in
                        c.setItems(context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: message.id.peerId))
        ...
        self.headerNode.navigationButtonContainer.performAction = { [weak self] key, source, gesture in
            ...
            switch key {
            case .edit, .settingEdit:
                    // Current user Edit data presenting
                if strongSelf.peerId == strongSelf.context.account.peerId {
                    
                    let onUpdateProfile: EventClosure<ProfileEditObject?> = {
                        guard let data = strongSelf.data, let cachedData = data.cachedData as? CachedUserData else {
                            return
                        }
                        if let peer = data.peer as? ElloAppUser {
                            var hasProgress = false
                            var updateProfileSignal: Signal<Void, NoError> = .complete()
                            if let firstName = $0?.firstName, let lastName = $0?.lastName, let birthday = $0?.dateOfBirth , let country = $0?.country, let gender = $0?.gender.rawValue,
                               peer.firstName != firstName || peer.lastName != lastName || peer.birthday != Int32(birthday.timeIntervalSince1970) || peer.country != country || peer.gender != gender || $0?.bio != cachedData.about {
                                updateProfileSignal = context.engine.accountData.changeProfile(firstName: firstName, lastName: lastName, about: $0?.bio, birthday: birthday.stringWithFormat(.iso8601_4, isGMT: true), country: country, gender: gender, username: $0?.userName)
                                |> mapToSignal {
                                    strongSelf.context.engine.peers.fetchAndUpdateCachedPeerData(peerId: strongSelf.context.account.peerId)
                                    |> mapToSignal { result -> Signal<Void, NoError> in
                                        fetchUpdatedCachedPeerData(peerId: strongSelf.context.account.peerId, accountPeerId: strongSelf.context.account.peerId, network: strongSelf.context.account.network, postbox: strongSelf.context.account.postbox)
                                        |> deliverOnMainQueue
                                        |> mapToSignal { result -> Signal<Void, NoError> in
                                            guard let cachedData = result, let data = strongSelf.data else {
                                                return .complete()
                                            }
                                            strongSelf.updateData(data.withCachedData(cachedData))
                                            return .complete()
                                        }
                                    }
                                }
                            ...
                            var updateUsername: Signal<Void, NoError> = .complete()
                            if let userName = $0?.userName, userName != peer.username {
                                updateUsername = context.engine.peers.updateAddressName(domain: .account, name: userName)
                            ...
                            strongSelf.activeActionDisposable.set((combineLatest(updateProfileSignal, updateUsername) |> deliverOnMainQueue).start(completed: {
                                dismissStatus?()
                                
                                strongSelf.controller?.navigationController?.popViewController(animated: true)
                                
                            }))
                        }
                    }
                    
                    let peer = strongSelf.data?.peer as? ElloAppUser
                    
                    let avatar = strongSelf.headerNode.avatarListNode.listContainerNode.currentItemNode?.imageNode.image ?? strongSelf.headerNode.placeholderImage()
                    
                    typealias CheckUsername = (username: String?, completion: EventClosure<Bool>)
                    let checkUserNameIfNeed: EventClosure<CheckUsername> = { [weak self, weak peer] item in
                        guard item.username != peer?.username else {
                        ...
                        strongSelf.activeActionDisposable.set((context.account.network.request(Api.functions.channels.checkUsername(channel: .inputChannelEmpty, username: item.username ?? "")) |> deliverOnMainQueue |> delay(0.5, queue: .concurrentDefaultQueue())).start(next: { result in
                            if .boolTrue == result {
                                item.completion(true)
                            } else {
                    ...
                    func showEditProfile(_ profile: ProfileEditObject) {
                        ELProfileController.navigationController = strongSelf.controller?.navigationController
                        let vc = ELProfileController.profileEdit(with: profile, aboutMaxLength: Int(strongSelf.data?.globalSettings?.userLimits.maxAboutLength ?? 70), avatarChange: { controller in
                            let avatarContainer = ((controller as? AdaptingController)?.altController as? AvatarUpdatable)
                            strongSelf.openAvatarForEditing(fromGallery: true, fromController: controller) {
                                avatarContainer?.avatarUploading()
                            } finished: {
                                avatarContainer?.updateAvatar($0)
                            }
                        }, onUpdateProfile: { profile in
                                onUpdateProfile(profile)
                        }) { (username, vc) in
                            checkUserNameIfNeed(CheckUsername(username, { isFree in
                                vc.usernameAvailable(isFree)
                            }))
                        }
                        
                        
                        ELProfileController.countriesSignal = Signal<[ELProfileUI.Country], Never> { observer in
                            return (ElloAppEngine(account: strongSelf.context.account).localization.getCountriesList(accountManager: strongSelf.context.sharedContext.accountManager, langCode: "", forceUpdate: true)
                                    |> deliverOnMainQueue)
                            .start(next: { countries in
                                let processed = countries.map {
                                    ELProfileUI.Country(name: $0.name, code: $0.countryCodes.first?.code ?? "", flagCode: $0.localizedName ?? "", flag: URL(string: $0.id))
                                }
                        ...
                        ////                            strongSelf.openAvatarForEditing()
                        //                        }
                        strongSelf.controller?.navigationController?.pushViewController(vc, animated: true)
                    }
                    if let cachedData = strongSelf.data?.cachedData as? CachedUserData {
                        let profile = ProfileEditObject(firstName: peer?.firstName ?? "",
                                                        lastName: peer?.lastName ?? "",
                                                        userName: peer?.username ?? "",
                                                        bio: cachedData.about ?? "",
                                                        dateOfBirth: Date(timeIntervalSince1970: TimeInterval(cachedData.birthday ?? 0)),
                                                        gender: .init(rawValue: cachedData.gender ?? "") ?? .Other,
                                                        country: cachedData.country,
                                                        currentAvatar: avatar,
                                                        isBusiness: peer?.isBusiness ?? false)
                        showEditProfile(profile)
                    } else {
                        
                        _ = (strongSelf.context.engine.peers.fetchAndUpdateCachedPeerData(peerId: strongSelf.context.account.peerId) |> deliverOnMainQueue).start(next: { _ in
                           _ = (fetchUpdatedCachedPeerData(peerId: strongSelf.context.account.peerId, accountPeerId: strongSelf.context.account.peerId, network: strongSelf.context.account.network, postbox: strongSelf.context.account.postbox)
                             |> deliverOnMainQueue).start(next: { [weak self] cachedData  in
                                if let self, let cachedData = cachedData as? CachedUserData, let data = self.data {
                                    self.updateData(data.withCachedData(cachedData))
                                    let peer = data.peer as? ElloAppUser
                                    let profile = ProfileEditObject(firstName: peer?.firstName ?? "",
                                                                    lastName: peer?.lastName ?? "",
                                                                    userName: peer?.username ?? "",
                                                                    bio: cachedData.about ?? "",
                        ...
                        if strongSelf.isSettings, let cachedData = data.cachedData as? CachedUserData {
                            ...
                            if peer.firstName != firstName || peer.lastName != lastName || (bio != nil && bio != cachedData.about) {
                                ...
                                if peer.firstName != firstName || peer.lastName != lastName {
                                    updateNameSignal = context.engine.accountData.updateAccountPeer(firstName: firstName, lastName: lastName)
                                    hasProgress = true
                                }
                                ...
                                if firstName.isEmpty && lastName.isEmpty {
                                    ...
                                    |> deliverOnMainQueue).start(error: { _ in
                                        ...
                                        |> mapToSignal { peer -> Signal<Void, NoError> in
                                            guard case let .user(peer) = peer, let username = peer.username, !username.isEmpty else {
                                                return .complete()
                                            }
                                            return (context.sharedContext.contactDataManager?.basicDataForNormalizedPhoneNumber(DeviceContactNormalizedPhoneNumber(rawValue: formatPhoneNumber(username))) ?? .single([]))
                                            |> take(1)
                                            |> mapToSignal { records -> Signal<Void, NoError> in
                                                ...
                                                if let contactDataManager = context.sharedContext.contactDataManager {
                                                    for (id, basicData) in records {
                                                        signals.append(
                                                            contactDataManager.appendContactData(
                                                                DeviceContactExtendedData(
                                                                    basicData: DeviceContactBasicData(firstName: firstName,
                                                                                                      lastName: lastName,
                                                                                                      userNames: basicData.userNames),
                                                                    middleName: "",
                                                                    prefix: "",
                                                                    suffix: "",
                                                                    organization: "",
...
//                                strongSelf.presentationData
                                strongSelf.headerNode.navigationButtonContainer.performAction?(.cancel, nil, nil)
                            }
                        } else {
                            strongSelf.headerNode.navigationButtonContainer.performAction?(.cancel, nil, nil)
                        }
                    } else if let group = data.peer as? ElloAppGroup, canEditPeerInfo(context: strongSelf.context, peer: group) {
                        ...
                        if title.isEmpty {
                            ...
                            var updateDataSignals: [Signal<Never, UpdatePeerTitleOrDescriptionError>] = []
                            ...
                            |> deliverOnMainQueue).start(error: { error in
                                ...
                                case .elloContaining(let message):
                    ...
                    } else if let channel = data.peer as? ElloAppChannel, canEditPeerInfo(context: strongSelf.context, peer: channel) {
                        let title = strongSelf.headerNode.editingContentNode.editingTextForKey(.title) ?? ""
                        let description = strongSelf.headerNode.editingContentNode.editingTextForKey(.description) ?? ""
                    ...
                    UIView.transition(with: strongSelf.view, duration: 0.3, options: [.transitionCrossDissolve], animations: {}, completion: nil)
        ...
        if self.isSettings {
            ...
            self.headerNode.displayCopyContextMenu = { [weak self] node, copyPhone, copyUsername in
                guard let strongSelf = self, let data = strongSelf.data, let user = data.peer as? ElloAppUser else {
                    return
                }
...
//                }
            ...
            self.headerNode.displayPremiumIntro = { [weak self] sourceView, _, _, _ in
                ...
                if let peer = strongSelf.data?.peer {
                    if let user = peer as? ElloAppUser, let emojiStatus = user.emojiStatus {
            ...
            self.headerNode.displayPremiumIntro = { [weak self] sourceView, peerStatus, emojiStatusFileAndPack, white in
                ...
                let _ = (strongSelf.context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: strongSelf.context.account.peerId))
        ...
        |> deliverOnMainQueue).start(next: { [weak self] data in
            ...
            data.privateUserLink = strongSelf.privateUserLink
            strongSelf.updateData(data)
            strongSelf.cachedDataPromise.set(.single(data.cachedData))
        })
        
        let _ = (context.account.network.request(Api.paidSubscription.subscriptions(filterType: .active))
                 |> deliverOnMainQueue
                 |> delay(0.5, queue: .mainQueue())
            )
            .start(next: { [weak self] item in
                guard let strongSelf = self, let data = strongSelf.data else {
                    return
                }
                let updatedData = data
                updatedData.subscriptionsCount = item.items.count
                strongSelf.updateData(updatedData)
                strongSelf.cachedDataPromise.set(.single(updatedData.cachedData))
            })
    ...
    deinit {
        ...
        self.customAvatarDisposable?.dispose()
    ...
    override func didLoad() {
        ...
        var invitesContext: PeerExportedInvitationsContext?
        _ = (context.account.viewTracker.peerView(peerId, updateData: false)
             |> deliverOnMainQueue
             |> mapToSignal({ [weak self] peerView -> Signal<PeerExportedInvitationsState, NoError>  in
            guard let self else {
                return .complete()
            }
            
            guard let peer = peerViewMainPeer(peerView) as? ElloAppUser, !peer.isPublic else {
                return .complete()
            }
            
            guard self.data?.cachedData != nil else {
                return .complete()
            }
            
            if invitesContext == nil {
                invitesContext = self.context.engine.peers.peerExportedInvitations(
                    peerId: self.peerId,
                    adminId: nil,
                    revoked: false,
                    forceUpdate: true
            ...
            return (invitesContext?.state ?? .complete())
            |> deliverOnMainQueue
        })).start(next: { [weak self] invites in
            guard let self else { return }
            guard let data else { return }
            
            if invites.hasLoadedOnce, invites.invitations.isEmpty, invites.count == 0 {
                _ = (context.engine.peers.createPeerExportedInvitation(
                    peerId: peerId,
                    title: nil,
                    expireDate: 0,
                    usageLimit: 0,
            ...
            privateUserLink = invites.invitations.first?.link
            data.privateUserLink = privateUserLink
    ...
    private func updateData(_ data: PeerInfoScreenData) {
        ...
        if let (layout, navigationHeight) = self.validLayout {
            ...
            self.containerLayoutUpdated(layout: layout, 
                                        navigationHeight: navigationHeight,
                                        transition: self.didSetReady && (membersUpdated || infoUpdated) ? .animated(duration: 0.3, curve: .spring) : .immediate)
        }
    }
    ...
    private func openMessage(id: MessageId) -> Bool {
        ...
        return self.context.sharedContext.openChatMessage(OpenChatMessageParams(context: self.context, chatLocation: nil, chatLocationContextHolder: nil, message: galleryMessage, standalone: false, reverseMessageGalleryOrder: true, navigationController: navigationController, dismissInput: { [weak self] in
        ...
        }, addContact: { [weak self] userName in
            if let strongSelf = self {
                strongSelf.context.sharedContext.openAddContact(context: strongSelf.context,
                                                                firstName: "",
                                                                lastName: "",
                                                                userName: userName,
            ...
            let _ = (strongSelf.context.engine.data.get(ElloAppEngine.EngineData.Item.Messages.Message(id: messageId))
            ...
            if let strongSelf = self {
                if let peer = peer {
                    ...
                    if case .default = navigation {
                        if let peer = peer as? ElloAppUser, peer.botInfo != nil {
...
//                })))
                ...
                items.append(.action(ContextMenuActionItem(text: self.presentationData.strings.PeerInfo_MuteForever, textColor: .primary, icon: { theme in
        ...
        case .more:
            guard let data = self.data, let peer = data.peer, let _ = data.chatPeer else {
                return
            }
            let presentationData = self.presentationData
...
//                        }
//                    }
//                } else if let user = chatPeer as? ElloAppUser {
//                    if user.id != strongSelf.context.account.peerId && user.botInfo == nil {
//                        canSetupAutoremoveTimeout = true
//                    }
//                } else if let channel = chatPeer as? ElloAppChannel {
...
//                    }
                    ...
                    if user.botInfo == nil && data.isContact {
                        items.append(.action(ContextMenuActionItem(text: presentationData.strings.Profile_ShareContactButton, icon: { theme in
                            ...
                            if let peer = strongSelf.data?.peer as? ElloAppUser {
                                let contact = ElloAppMediaContact(firstName: peer.firstName ?? "",
                                                                  lastName: peer.lastName ?? "",
                                                                  username: peer.username ?? "",
                                                                  peerId: peer.id,
                                                                  vCardData: nil,
                                                                  userid: peer.id.id._internalGetInt64Value())
                                
                                let shareController = ShareController(context: strongSelf.context,
                                                                      subject: .media(.standalone(media: contact)),
                                                                      externalShare: false, // EM-3125
                                                                      updatedPresentationData: strongSelf.controller?.updatedPresentationData)
                                shareController.completed = { [weak self] peerIds in
                                    if let strongSelf = self {
                                        let _ = (strongSelf.context.engine.data.get(
                                            EngineDataList(
                                                peerIds.map(ElloAppEngine.EngineData.Item.Peer.Peer.init)
                                            )
                                        )
                                        |> deliverOnMainQueue).start(next: { [weak self] peerList in
                                            ...
                                            strongSelf.controller?.present(UndoOverlayController(
                                                presentationData: presentationData,
...
//                    }
                    ...
                    if strongSelf.peerId.namespace == Namespaces.Peer.CloudUser && user.botInfo == nil && !user.flags.contains(.isSupport) {
                        if data.isContact {
                            if let cachedData = data.cachedData as? CachedUserData, cachedData.isBlocked {
                                ...
                                items.append(.action(ContextMenuActionItem(text: presentationData.strings.Conversation_BlockUser, textColor: .primary, icon: { theme in
                    ...
                    if let cachedData = strongSelf.data?.cachedData as? CachedChannelData, cachedData.flags.contains(.canViewStats), channel.isAdmin, case .broadcast = channel.info {
...
//                    }
                    ...
                    switch channel.info {
                    case .broadcast:
                        if !channel.isCreator, case .member = channel.participationStatus, !headerButtons.contains(.leave) {
                            if !items.isEmpty {
                                items.append(.separator)
                            }
                            
                            items.append(.action(ContextMenuActionItem(text: presentationData.strings.Channel_LeaveChannel, 
                    ...
                    case .group:
                        if case .member = channel.participationStatus, !headerButtons.contains(.leave) {
                            if !channel.isCreator {
                                ...
                                items.append(.action(ContextMenuActionItem(text: presentationData.strings.Group_LeaveGroup, 
                                                                           textColor: .primary,
                                                                           icon: { theme in
                            ...
                            if let cachedData = data.cachedData as? CachedChannelData, cachedData.flags.contains(.canDeleteHistory) {
                                items.append(.action(ContextMenuActionItem(text: presentationData.strings.Group_DeleteGroup, 
                                                                           textColor: .destructive,
                                                                           icon: { theme in
...
//                    }
            ...
            guard let sourceNode = self.headerNode.buttonNodes[.more]?.referenceNode else {
                return
            }
...
//            }
    ...
    private func openStartSecretChat() {
        ...
        let _ = (combineLatest(
            self.context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: self.peerId)),
    ...
    private func openUsername(value: String) {
        let shareController = ShareController(context: self.context, subject: .url("https://ello.team/\(value)"), updatedPresentationData: self.controller?.updatedPresentationData)
        shareController.completed = { [weak self] peerIds in
            guard let strongSelf = self else {
                return
            }
    ...
    private func requestCall(isVideo: Bool, gesture: ContextGesture? = nil, contextController: ContextControllerProtocol? = nil, result: ((ContextMenuActionResult) -> Void)? = nil, backAction: ((ContextControllerProtocol) -> Void)? = nil) {
        ...
        guard let peer = self.data?.peer as? ElloAppUser, let cachedUserData = self.data?.cachedData as? CachedUserData else {
            return
        }
            ...
            let startCall: (Bool) -> Void = { [weak self] endCurrentIfAny in
                ...
                |> deliverOnMainQueue).start(next: { [weak self] info in
                    ...
                    case .anonymousNotAllowed:
                        if let channel = strongSelf.data?.peer as? ElloAppChannel, case .broadcast = channel.info {
    ...
    private func openPhone(value: String, node: ASDisplayNode, gesture: ContextGesture?) {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] peer in
            ...
            let elloappCallAction: (Bool) -> Void = { [weak self] isVideo in
                guard let strongSelf = self else {
                    return
                }
            ...
            if case let .user(peer) = peer, let peerPhoneNumber = peer.phone, formatPhoneNumber(value) == formatPhoneNumber(peerPhoneNumber) {
                items.append(.action(ContextMenuActionItem(text: presentationData.strings.UserInfo_ElloAppCall, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Call"), color: theme.contextMenu.primaryColor) }, action: { c, _ in
                    c.dismiss {
                        elloappCallAction(false)
                    }
                })))
                items.append(.action(ContextMenuActionItem(text: presentationData.strings.UserInfo_ElloAppVideoCall, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/VideoCall"), color: theme.contextMenu.primaryColor) }, action: { c, _ in
                    c.dismiss {
                        elloappCallAction(true)
                    }
                })))
    ...
    private func editingOpenNotificationSettings() {
        let _ = (combineLatest(
            self.context.engine.data.get(
                ElloAppEngine.EngineData.Item.Peer.NotificationSettings(id: self.peerId),
                ElloAppEngine.EngineData.Item.NotificationSettings.Global()
            ),
    ...
    private func openStats() {
        ...
        if let channel = peer as? ElloAppChannel, case .group = channel.info {
            statsController = groupStatsController(context: self.context, 
                                                   updatedPresentationData: self.controller?.updatedPresentationData,
                                                   peerId: peer.id,
                                                   statsDatacenterId: statsDatacenterId)
        } 
        else {
            statsController = channelStatsController(context: self.context,
                                                     updatedPresentationData: self.controller?.updatedPresentationData,
                                                     peerId: peer.id,
                                                     statsDatacenterId: statsDatacenterId)
        }
    ...
    private func openVoiceChatOptions(defaultJoinAsPeerId: PeerId?, gesture: ContextGesture? = nil, contextController: ContextControllerProtocol? = nil) {
        ...
        |> map { currentAccountPeer, availablePeers -> [FoundPeer] in
            ...
            switch chatPeer {
            case let group as ElloAppGroup:
                if case .creator = group.role {
                    canCreateStream = true
                }
            case let channel as ElloAppChannel:
                if channel.flags.contains(.isCreator) {
                    canCreateStream = true
    ...
    private func openVoiceChatDisplayAsPeerSelection(completion: @escaping (PeerId) -> Void, gesture: ContextGesture? = nil, contextController: ContextControllerProtocol? = nil, result: ((ContextMenuActionResult) -> Void)? = nil, backAction: ((ContextControllerProtocol) -> Void)? = nil) {
        ...
        |> map { currentAccountPeer, availablePeers -> [FoundPeer] in
            ...
            if peers.count == 1, let peer = peers.first {
                ...
                var isGroup = false
                for peer in peers {
                    if peer.peer is ElloAppGroup {
                        isGroup = true
                        break
                    } else if let peer = peer.peer as? ElloAppChannel, case .group = peer.info {
                        isGroup = true
                        break
                    }
                }
                ...
                for peer in peers {
                    ...
                    if peer.peer.id.namespace == Namespaces.Peer.CloudUser {
                        ...
                        if let peer = peer.peer as? ElloAppChannel, case .broadcast = peer.info {
    ...
    private func openShareBot() {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] peer in
            ...
            if case let .user(peer) = peer, let username = peer.username {
                let shareController = ShareController(context: strongSelf.context, subject: .url("https://ello.team/\(username)"), updatedPresentationData: strongSelf.controller?.updatedPresentationData)
                shareController.completed = { [weak self] peerIds in
                    guard let strongSelf = self else {
                        return
                    }
    ...
    private func editingOpenPublicLinkSetup() {
        ...
        let controller = channelVisibilityController(context: self.context, 
                                                     updatedPresentationData: self.controller?.updatedPresentationData,
                                                     peerId: self.peerId,
                                                     mode: .generic,
    ...
    private func openParticipantsSection(section: PeerInfoParticipantsSection) {
        ...
        case .admins:
            ...
            } else if peer is ElloAppChannel {
                self.controller?.push(channelAdminsController(context: self.context, updatedPresentationData: self.controller?.updatedPresentationData, peerId: self.peerId))
            }
        case .banned:
    ...
    private func openPeerInfo(peer: Peer, isMember: Bool) {
        ...
        let showPrivateUserAlertController = {
            let alertController = standardTextAlertController(
                theme: AlertControllerTheme(presentationData: self.presentationData),
                title: self.presentationData.strings.PeerInfo_PrivateAccount_Alert_Title,
                text: self.presentationData.strings.PeerInfo_PrivateAccount_Alert_Text,
                actions: [
                    TextAlertAction(
                        type: .defaultAction,
                        title: self.presentationData.strings.Common_OK,
        ...
        // Doesn't open private profile
        if let peer = peer as? ElloAppUser, !peer.isPublic {
            _ = (self.context.account.postbox.transaction { transaction -> Bool in
                transaction.isPeerContact(peerId: peer.id)
            } |> deliverOn(Queue.mainQueue())).start(next: { isContact in
                if isContact {
                    showInfoController()
                } else {
                    showPrivateUserAlertController()
                }
            })
        } else {
            showInfoController()
    ...
    private func openPeerInfoContextMenu(subject: PeerInfoContextSubject, sourceNode: ASDisplayNode) {
        ...
        case .bio:
            var text: String?
            if let cachedData = data.cachedData as? CachedUserData {
                text = cachedData.about ?? cachedData.botDescription
            } else if let cachedData = data.cachedData as? CachedGroupData {
        ...
        case .link:
            if let addressName = peer.addressName {
                ...
                if peer is ElloAppChannel {
                    text = "https://ello.team/\(addressName)"
    ...
    private func openDeletePeer() {
        let _ = (self.context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: self.peerId))
        |> deliverOnMainQueue).start(next: { [weak self] peer in
            guard let strongSelf = self, let peer = peer else {
                return
            }
    ...
    private func openLeavePeer(delete: Bool) {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] peer in
            ...
            var isGroup = false
            var payType: ElloAppChannelPayType = .free
            if case let .channel(channel) = peer {
                if case .group = channel.info {
                    isGroup = true
                }
            ...
            if delete {
                if isGroup {
                    title = self.presentationData.strings.PeerInfo_DeleteGroupTitle
                    text = self.presentationData.strings.PeerInfo_DeleteGroupText(peer.debugDisplayTitle).string
                } else {
                    title = self.presentationData.strings.PeerInfo_DeleteChannelTitle
                    text = self.presentationData.strings.PeerInfo_DeleteChannelText(peer.debugDisplayTitle).string
                }
                actionText = self.presentationData.strings.Common_Delete
            } else {
                if isGroup {
//                    title = strongSelf.presentationData.strings.PeerInfo_LeaveGroupTitle
                    text = self.presentationData.strings.PeerInfo_LeaveGroupText
                    actionText = self.presentationData.strings.PeerInfo_AlertLeaveAction
                } else {
                    switch payType {
                    case .free:
//                        title = strongSelf.presentationData.strings.PeerInfo_LeaveChannelTitle
                        text = self.presentationData.strings.PeerInfo_LeaveChannelText
                    case .onlineCourse:
                        text = self.presentationData.strings.PeerInfo_LeaveOnlineCourseText
                    case .subscription:
                        text = self.presentationData.strings.PeerInfo_LeaveSubscriptionChannelText
                    }
                    actionText = self.presentationData.strings.PeerInfo_AlertUnsubscribeAction
                }
                title = nil
            ...
            let alertController = standardTextWithAvatarAlertController(
                theme: AlertControllerTheme(presentationData: self.presentationData),
                avatar: (context: self.context, peer: peer),
                title: title,
                text: text,
                actions: [
                    TextWithAvatarAlertAction(
                        type: .defaultAction,
    ...
    private func updateProfilePhoto(_ image: UIImage, completion: EventClosure<UIImage>? = nil) {
    ...
    private func updateProfileVideo(_ image: UIImage, asset: Any?, adjustments: TGVideoEditAdjustments?) {
        ...
        let context = self.context
        let signal = Signal<ElloAppMediaResource, UploadPeerPhotoError> { [weak self] subscriber in
    ...
    private func openAvatarForEditing(fromGallery: Bool = false, fromController:ViewController? = nil, completion: @escaping () -> Void = {}, finished: EventClosure<UIImage>? = nil) {
        ...
        let peerId = self.peerId
        let _ = (self.context.engine.data.get(
            ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId),
        ...
        |> deliverOnMainQueue).start(next: { [weak self] peer, searchBotsConfiguration in
            ...
            let legacyController = LegacyController(presentation: .custom, theme: strongSelf.presentationData.theme, initialLayout: strongSelf.validLayout?.0)
            ...
            paintStickersContext.presentStickersController = { [weak self] completion in
                guard let strongSelf = self else {
                    return nil
                }
                let controller = DrawingStickersScreen(context: strongSelf.context, selectSticker: { fileReference, view, rect in
            ...
            let mixin = TGMediaAvatarMenuMixin(context: legacyController.context, parentController: emptyController, hasSearchButton: true, hasDeleteButton: hasPhotos && !fromGallery, hasViewButton: false, personalPhoto: strongSelf.isSettings, isVideo: false, saveEditedPhotos: false, saveCapturedMedia: false, signup: false)!
            ...
            mixin.requestSearchController = { [weak self] assetsController in
                ...
                let controller = WebSearchController(context: strongSelf.context, updatedPresentationData: strongSelf.controller?.updatedPresentationData,  peer: peer, chatLocation: nil, configuration: searchBotsConfiguration, mode: .avatar(initialQuery: strongSelf.isSettings ? nil : peer.displayTitle(strings: strongSelf.presentationData.strings, displayOrder: strongSelf.presentationData.nameDisplayOrder), completion: { [weak self] result in
                    assetsController?.dismiss()
                    self?.updateProfilePhoto(result, completion: finished)
                }))
            ...
            mixin.didFinishWithImage = { [weak self, weak legacyController] image in
                if let image = image {
                    completion()
                    self?.updateProfilePhoto(image, completion: finished)
                }
                let _ = self?.currentAvatarMixin.swap(nil)
                legacyController?.dismiss()
            }
            mixin.didFinishWithVideo = { [weak self, weak legacyController] image, asset, adjustments in
            ...
            mixin.didFinishWithDelete = { [weak self, weak legacyController] in
                guard let strongSelf = self else {
                    return
                }
                ...
                let items: [ActionSheetItem] = [
                    ActionSheetButtonItem(title: strongSelf.presentationData.strings.Settings_RemoveConfirmation, color: .destructive, action: { [weak actionSheet] in
                        actionSheet?.dismissAnimated()
                        proceed()
                    })
                ]
            ...
            if let menuController = menuController {
                menuController.customRemoveFromParentViewController = { [weak navigationController] in
            ...
            legacyController.blocksBackgroundWhenInOverlay = true
            strongSelf.view.endEditing(true)
            if let vc = fromController {
                vc.present(legacyController, in: .window(.root))
            } else {
                strongSelf.controller?.present(legacyController, in: .window(.root), completion: {
                    print("===complete===")
                })
            }
        })
    ...
    private func openQrCode(link: String) {
        guard let data = self.data, let peer = data.peer, let controller = self.controller else {
            return
        }
        
        controller.present(ChatQrCodeScreen(context: self.context, subject: .peer(peer, link)), in: .window(.root))
    }
    ...
    fileprivate func openSettings(section: PeerInfoSettingsSection) {
        ...
        case .privacyAndSecurity:
            self.setupPrivacyAndSecurity { vc in
                push(vc)
            }
        ...
        case .support:
            self.openUrl(url: "https://ello.team/ElloSupport", concealed: true, external: true)
            
        case .info:
            if let user = self.data?.peer as? ElloAppUser, let email = (data?.cachedData as? CachedUserData)?.email {
                let lc = ELProfileController.infoViewController(input: .init(username: "\(user.firstName ?? "") \(user.lastName ?? "")", userID: "\(user.id.id.description)", email: email, openLink: { [weak self] in
                    self?.openUrl(url: $0, concealed: false, external: false)
                }))
                push(lc)
            }
            
        case .phoneNumber:
            if let user = self.data?.peer as? ElloAppUser, let phoneNumber = user.phone {
        ...
        case .settings:
            ELProfileController.navigationController = self.controller?.navigationController
            let lc = ELProfileController.settings(onTapOption:{ [weak self] in
                guard let self else { return }
                switch $0 {
                case .privacy:
                    self.showPrivacyConrtoller()
//                case .language:
//                    self?.showLanguageController()
                case .changePasswordAndEmail:
                    self.showChangeController()
                case .devices:
                    debugPrint("Devices")
                    let activeSessionsContext = self.context.engine.privacy.activeSessions()
                    let webSessionsContext = self.context.engine.privacy.webSessions()

                    ELProfileController.navigationController?.pushViewController(
                        recentSessionsController(context: self.context,
            ...
            onDeleteAccount: { [weak self] in
                guard let self else {return}
                if let _ = self.data?.peer as? ElloAppUser {
                    let privacy = ELProfileController.deletePrivacy { [weak self] in
                        guard let self else { return }
                        self.controller?.navigationController?.pushViewController(
                            ELProfileController.accountInfo(context: self.context, onOpenAiBot: { [weak self] in
                                guard let self else { return }
                                self.interaction.openSettings(.elloAI)
                            }, onOpenOwner: { [weak self] in
                                self?.controller?.navigationController?.popToRootViewController(animated: false)
                                self?.context.sharedContext.openChats()
                            }, onOpenChat: { [weak self] peerId in
                                guard let self, let navigationController = self.controller?.navigationController as? NavigationController else { return }
                                let _ = self.context.sharedContext.navigateToChatController(NavigateToChatControllerParams(navigationController: navigationController, context: self.context, chatLocation: .peer(id: peerId), keepStack: .never, useExisting: true))
                            }, onTapDelete: { [weak self] in
                                guard let self else { return }
                                self.showDeleteAccountController()
                            }), animated: true)
                    }
                    ELProfileController.navigationController?.pushViewController(privacy, animated: true)
                }
            })
            push(lc)
            
        case let .qrCode(link):
            openQrCode(link: link)
            
        case let .qrCodeShare(link):
            guard let v = self.controller?.view else {return}
            shareUserLink(context: context, link: link, view: v)
            
        case let .qrCopy(link):
        ...
        case .earnings:
            ELProfileController.navigationController = self.controller?.navigationController
            let lc = ELProfileController.earnings(onTapOption:{ [weak self] in
                switch $0 {
                case .subscriptionChannels: self?.showSubscriptionChannels()
                case .totalBalance: self?.showTransactionItems()
                default: break
                }
            })
            push(lc)
        ...
        case .subscriptions:
            ELProfileController.navigationController = self.controller?.navigationController
            let lc = ELProfileController.subscribedChannels(accountContext: context)
            push(lc)
            
        case .wallet:
            setupWallet { controller in
                push(controller)
            }
        ...
        case .elloAI:
            if let controller = self.controller, let navigationController = controller.navigationController as? NavigationController {
                ...
                if let vc = AIUniverseViewController.controller {
                    adopt.altController = vc
                    vc.requestService = RequestService(accountContext: self.context)
                    vc.onTapBack = {
                        _ = navigationController.popViewController(animated: true)
                    }
                    
                    vc.onSelectedBot = {[weak self] botPeerId in
                        guard let self else { return }
                         _ = (self.context.engine.peers.fetchAndUpdateCachedBotData(peerId: botPeerId) |> deliverOnMainQueue
                        |> mapToSignal { [weak self] _ -> Signal<Void, NoError> in
                            guard let self else { return .complete() }
                            self.context.sharedContext.navigateToChatController(NavigateToChatControllerParams(navigationController: navigationController, context: self.context, chatLocation: .peer(id: botPeerId)))
                            return .complete()
                        }).start()
                    }
                    
                    vc.onBuyAiPack = {[weak self, vc]in
                        guard let self else {return}
                        if let aiPurchaseViewController = AIPurchaseViewController.controller {
                            aiPurchaseViewController.context = context
                            aiPurchaseViewController.navigation_setNavigationController(navigationController)

                            aiPurchaseViewController.didPurchase = { [weak self] in
                                DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
                                    guard let self else {return}
                                    
                                    _ = (self.context.engine.aiChatBot.subscriptionInfo() |> deliverOnMainQueue).start { [weak vc] info in
                                        vc?.refresh()
                                    }
                                }
                            }
                            
                            aiPurchaseViewController.onTapBack = { [weak navigationController] in
                                _ = navigationController?.popViewController(animated: true)
                            }

                            let adaptingVC = AdaptingController(viewController: aiPurchaseViewController)
                            navigationController.pushViewController(adaptingVC)
                        }
                    }
                }
        ...
        case .notifications:
            if let controller = self.controller, let navigationController = controller.navigationController as? NavigationController {
                let peerId = PeerId.notificationsBotPeerID
                _ = (context.engine.peers.fetchAndUpdateCachedBotData(peerId: peerId) |> deliverOnMainQueue
                |> mapToSignal { [weak self] _ -> Signal<Void, NoError> in
                    guard let self else {
                        return .complete()
                    }
                    self.context.sharedContext.navigateToChatController(NavigateToChatControllerParams(navigationController: navigationController, context: self.context, chatLocation: .peer(id: peerId)))
                    return .complete()
                }).start()
            }
        ...
        case .folders:
            // TODO: Open folders
            debugPrint("Open folders")
//            if let controller = self.controller, let navigationController = controller.navigationController as? NavigationController {
//                self.context.sharedContext.navigateToChatController(NavigateTo)
//            }
            
        case .invite:
 //           ELProfileController.navigationController = self.controller?.navigationController
 //           let lc = ELProfileController.invite()
            let lc = inviteLinkListController(context: self.context, peerId: peerId, admin: nil)
            push(lc)

        case .loyalty:
 //           ELProfileController.navigationController = self.controller?.navigationController
 //           let lc = ELProfileController.invite()
            let lc = ELProfileController.layalityViewController(accountContext: context)
//            let lc = inviteLinkListController(context: self.context, peerId: peerId, admin: nil)
            push(lc)
        ...
        case .addAccount:
            ...
            |> deliverOnMainQueue
                ...
                if count >= maximumAvailableAccounts {
                    ...
                    let controller = PremiumLimitScreen(context: strongSelf.context, 
                                                        subject: .accounts,
                                                        count: Int32(count),
                                                        action: {
                    ...
                    strongSelf.context.sharedContext.beginNewAuth(testingEnvironment: strongSelf.context.account.testingEnvironment, isWelcomHidden:nil)
                }
            })
    ...
    private func showWalletDashboard() {
       
       
    }
    
    private func showDeleteAccount(completion: @escaping VoidClosure) {
        let presentationData = context.sharedContext.currentPresentationData.with { $0 }
        let alertController = textAlertController(context: context, title: "", text: "Are you sure you want to delete your account?", actions: [
        ...
        (controller?.navigationController?.topViewController as? ViewController)?.present(alertController, in: .window(.root), with: ViewControllerPresentationArguments(presentationAnimation: .modalSheet))
    }
    
    private func showSubscriptionChannels() {
        let vc = ELProfileController.subscribedChannels(accountContext: context)
        ELProfileController.navigationController?.pushViewController(vc, animated: true)
    }
    
    private func showTransactionItems() {
        let transactions = [Transaction(owner: Transaction.Owner(name:"Test"), totalAmount: 1, commissions: 2, state: .pending),
                            Transaction(owner: Transaction.Owner(name:"Test2"), totalAmount: 2, commissions: 3, state: .approved),
                            Transaction(owner: Transaction.Owner(name:"Test3"), totalAmount: 3, commissions: 4, state: .rejected)]
        let vc = ELProfileController.transactions(transactions) {
            print("\($0)")
        }
        ELProfileController.navigationController?.pushViewController(vc, animated: true)
    }
    
    private func showPrivacyConrtoller() {
        let optionTransform: ((SelectivePrivacySettings?)->(PrivacyDataSelectionOption)) = { option in
            switch option {
                case .enableEveryone: return .everybody
                case .enableContacts: return .myContacts
                case .disableEveryone: return .nobody
                default: return .everybody
            }
        }
        
        let vc = ELProfileController.privacy(with: PrivacyDataObject(),
                                             onUpdate: { [weak self] in
            guard let weak = self else {return PrivacyDataObject()}
            let settings = weak.data?.globalSettings?.privacySettings

            var data = PrivacyDataObject()
            
            data.lastSeen = optionTransform(settings?.presence)
            data.groupsAndChannels = optionTransform(settings?.groupInvitations)
            data.calls = optionTransform(settings?.voiceCalls)
            data.profilePhoto = optionTransform(settings?.profilePhoto)
            data.forwardedMessages = optionTransform(settings?.forwards)
                       
            let blockedContext = BlockedPeersContext(account: weak.context.account)
            weak.dataDisposable = blockedContext.state.start(next: {
                if $0.canLoadMore {
                    blockedContext.loadMore()
                }
                ELProfileController.blockedUsersCount = $0.peers.count
            })
            
            return data
        },
                                             onSelectOption: { [weak self] in
            var obj = PrivacyDataOptionObject(dataOption:$0)
            let settings = self?.data?.globalSettings?.privacySettings
            let shortOptionsList: [PrivacyDataSelectionOption] = [.everybody, .myContacts]
            switch $0 {
                case .lastSeen:
                    obj.selected = optionTransform(settings?.presence)
                    self?.showPrivacySetupConrtoller(with: obj)
                case .groupsAndChannels:
                    obj.selected = optionTransform(settings?.groupInvitations)
                    obj.setups = shortOptionsList
                    self?.showPrivacySetupConrtoller(with: obj)
                case .calls:
                    obj.selected = optionTransform(settings?.voiceCalls)
                    self?.showPrivacySetupConrtoller(with: obj)
                case .profilePhoto:
                    obj.selected = optionTransform(settings?.profilePhoto)
                    obj.setups = shortOptionsList
                    self?.showPrivacySetupConrtoller(with: obj)
                case .forwardedMessages:
                    obj.selected = optionTransform(settings?.forwards)
                    self?.showPrivacySetupConrtoller(with: obj)
                    
                case .blockedUsers:
                    guard let weak = self else {return}
                    
                    let _ = (weak.blockedPeers.get()
                    |> take(1)
                    |> deliverOnMainQueue).start(next: { blockContext in
                        
                        let blockContext = blockContext ?? BlockedPeersContext(account: weak.context.account)
                        weak.setupBlockedUsers(blockedPeersContext: blockContext) { vc in
                            ELProfileController.navigationController?.pushViewController(vc, animated: true)
                        }
                    })
                default: break
    ...
    private func showPrivacySetupConrtoller(with object: PrivacyDataOptionObject) {
        
        let enableDisablePeers:((SelectivePrivacySettings)->([PeerId: SelectivePrivacyPeer], [PeerId: SelectivePrivacyPeer])) = { option in
            switch option {
                case let .enableEveryone(disable):
                    return ([:], disable)
                case let .enableContacts(enable, disable):
                    return (enable, disable)
                case let .disableEveryone(enable):
                    return (enable, [:])
            }
        }
        
        let transform:((PrivacyDataSelectionOption, [PeerId: SelectivePrivacyPeer], [PeerId: SelectivePrivacyPeer])->(SelectivePrivacySettings)) = { option, enable, disable in
            switch option {
                case .everybody: return .enableEveryone(disableFor: disable)
                case .myContacts: return .enableContacts(enableFor: enable, disableFor: disable)
                case .nobody: return .disableEveryone(enableFor: enable)
            }
        }
        
        let settings = self.data?.globalSettings?.privacySettings

        let vc = ELProfileController.privacyData(object, onUpdate: { [weak self] updatedObject in
            print(updatedObject as Any)
            guard let self else {
                return
            }
            
            guard let settings = settings, let updatedObject = updatedObject else {return}
            
            var updatedSettings: AccountPrivacySettings?
            let newPrivacy: (UpdateSelectiveAccountPrivacySettingsType, SelectivePrivacySettings)?
            switch updatedObject.dataOption {
            case .lastSeen:
                let enableDisable = enableDisablePeers(settings.presence)
                let updatedOption = transform(updatedObject.selected, enableDisable.0, enableDisable.1)
                updatedSettings = AccountPrivacySettings(presence: updatedOption, groupInvitations: settings.groupInvitations, voiceCalls: settings.voiceCalls, voiceCallsP2P: settings.voiceCallsP2P, profilePhoto: settings.profilePhoto, forwards: settings.forwards, phoneNumber: settings.phoneNumber, phoneDiscoveryEnabled: settings.phoneDiscoveryEnabled, voiceMessages: settings.voiceMessages, automaticallyArchiveAndMuteNonContacts: settings.automaticallyArchiveAndMuteNonContacts, accountRemovalTimeout: settings.accountRemovalTimeout)
                newPrivacy = (.presence, updatedOption)
                
            case .groupsAndChannels:
                let enableDisable = enableDisablePeers(settings.groupInvitations)
                let updatedOption = transform(updatedObject.selected, enableDisable.0, enableDisable.1)
                updatedSettings = AccountPrivacySettings(presence: settings.presence, groupInvitations: updatedOption, voiceCalls: settings.voiceCalls, voiceCallsP2P: settings.voiceCallsP2P, profilePhoto: settings.profilePhoto, forwards: settings.forwards, phoneNumber: settings.phoneNumber, phoneDiscoveryEnabled: settings.phoneDiscoveryEnabled, voiceMessages: settings.voiceMessages, automaticallyArchiveAndMuteNonContacts: settings.automaticallyArchiveAndMuteNonContacts, accountRemovalTimeout: settings.accountRemovalTimeout)
                newPrivacy = (.groupInvitations, updatedOption)
            case .calls:
                let enableDisable = enableDisablePeers(settings.voiceCalls)
                let updatedOption = transform(updatedObject.selected, enableDisable.0, enableDisable.1)
                updatedSettings = AccountPrivacySettings(presence: settings.presence, groupInvitations: settings.groupInvitations, voiceCalls: updatedOption, voiceCallsP2P: settings.voiceCallsP2P, profilePhoto: settings.profilePhoto, forwards: settings.forwards, phoneNumber: settings.phoneNumber, phoneDiscoveryEnabled: settings.phoneDiscoveryEnabled, voiceMessages: settings.voiceMessages, automaticallyArchiveAndMuteNonContacts: settings.automaticallyArchiveAndMuteNonContacts, accountRemovalTimeout: settings.accountRemovalTimeout)
                newPrivacy = (.voiceCalls, updatedOption)
            case .profilePhoto:
                let enableDisable = enableDisablePeers(settings.profilePhoto)
                let updatedOption = transform(updatedObject.selected, enableDisable.0, enableDisable.1)
                updatedSettings = AccountPrivacySettings(presence: settings.presence, groupInvitations: settings.groupInvitations, voiceCalls: settings.voiceCalls, voiceCallsP2P: settings.voiceCallsP2P, profilePhoto: updatedOption, forwards: settings.forwards, phoneNumber: settings.phoneNumber, phoneDiscoveryEnabled: settings.phoneDiscoveryEnabled, voiceMessages: settings.voiceMessages, automaticallyArchiveAndMuteNonContacts: settings.automaticallyArchiveAndMuteNonContacts, accountRemovalTimeout: settings.accountRemovalTimeout)
                newPrivacy = (.voiceCallsP2P, updatedOption)
            case .forwardedMessages:
                let enableDisable = enableDisablePeers(settings.forwards)
                let updatedOption = transform(updatedObject.selected, enableDisable.0, enableDisable.1)
                updatedSettings = AccountPrivacySettings(presence: settings.presence, groupInvitations: settings.groupInvitations, voiceCalls: settings.voiceCalls, voiceCallsP2P: settings.voiceCallsP2P, profilePhoto: settings.profilePhoto, forwards: updatedOption, phoneNumber: settings.phoneNumber, phoneDiscoveryEnabled: settings.phoneDiscoveryEnabled, voiceMessages: settings.voiceMessages, automaticallyArchiveAndMuteNonContacts: settings.automaticallyArchiveAndMuteNonContacts, accountRemovalTimeout: settings.accountRemovalTimeout)
                newPrivacy = (.forwards, updatedOption)
            default:
                newPrivacy = nil
            }
            guard let updated = updatedSettings, let newPrivacy else {return}
            _ = self.context.engine.privacy.updateSelectiveAccountPrivacySettings(type: newPrivacy.0, settings: newPrivacy.1).start(completed: { [weak self] in
                self?.privacySettings.set(.single(updated))
            })
    ...
    private func showLanguageController()  {
        let vc = ELProfileController.languages(selectedLanguage: {
            print($0)
        })
        ELProfileController.navigationController?.pushViewController(vc, animated: true)
    }
         
    private func showChangeController() {
        let vc = ELProfileController.changePassAndEmail(selectedOption: { [weak self] in
            print($0)
            switch $0 {
                case .changeEmail:
                    self?.showChangeEmailController()
                case .changePassword:
                    self?.showChangePasswordController()
            }
        })
        ELProfileController.navigationController?.pushViewController(vc, animated: true)
    }
                       
    private func setupBlockedUsers(blockedPeersContext: BlockedPeersContext, complete: ((ViewController)->())?) {
        complete?(
        blockedPeersController(context: context, blockedPeersContext: blockedPeersContext)
        )
    }
    
    private func setupPrivacyAndSecurity(result: ((ViewController)->())?) {
        if let settings = self.data?.globalSettings {
            let _ = (combineLatest(self.blockedPeers.get(), self.hasTwoStepAuth.get())
                ...
                if let strongSelf = self {
                    ...
                    result?(privacyAndSecurityController(context: strongSelf.context, initialSettings: settings.privacySettings, updatedSettings: { [weak self] settings in
    ...
    private func showChangeEmailController() {
        let errorBlock: (BaseError) -> () =  { [weak self] in
            guard let self else {
                return
            }
            if let controller = ELProfileController.navigationController?.topViewController as? ViewController {
                controller.present(standardTextAlertController(theme: .init(presentationData: self.presentationData), title: nil, text: "\($0)", actions: [.init(type: .defaultAction, title: self.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
            }
        }
        let vc = ELProfileController.changeEmail(helpSupport: { [weak self] in
            guard let weak = self else {return}
            if MFMailComposeViewController.canSendMail(), let controller = weak.controller {
                weak.presentEmailComposeController(address: "help@ello.team", subject: "Email change", body: "", from: controller)
            } else {
                let vcc = ELProfileController.navigationController?.topViewController as? ViewController
                vcc?.present(standardTextAlertController(theme: AlertControllerTheme(presentationData: weak.presentationData), title: nil, text: weak.presentationData.strings.Login_EmailNotConfiguredError, actions: [TextAlertAction(type: .defaultAction, title: weak.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
            }
        }, changeEmailTo: { [weak self] email in
            guard let self else { return }
            self.showVerifyController(verificationRequest: changeEmailCode(with: email, network: self.context.account.network)) { [weak self] code in
                guard let self else {return}
                self.activeActionDisposable.set((saveNewEmail(with: email, code: code ?? "", network: self.context.account.network) |> deliverOnMainQueue)
                    .start(next: { response in
                        let vcc = ELProfileController.navigationController?.topViewController as? ViewController
                        vcc?.present(standardTextAlertController(theme: .init(presentationData: self.presentationData), title: nil, text: self.presentationData.strings.Login_EmailChanged, actions: [.init(type: .defaultAction, title: self.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
    ...
    func showVerifyController(verificationRequest: Signal<Api.Response.SendVerificationCode, BaseError>, onVerifyCode: @escaping (String?) -> (), errorBlock: @escaping (BaseError) -> ()) {
        self.activeActionDisposable.set((verificationRequest |> deliverOnMainQueue)
            .start(next: { result in
                if let controller = ELProfileController.navigationController,
                   nil == ((controller.viewControllers.last as? AdaptingController)?.altController as? CodeVerificationViewController),
                   let verifyVC = WelcomeController.verifyCode(for: result.email, till: result.confirmation_expire, isBusiness: false) {
                    
                    let adaptedVerifyVC = AdaptingController(viewController: verifyVC)
                    controller.pushViewController(adaptedVerifyVC, animated: true)
                    verifyVC.onVerifyCode = onVerifyCode
                    verifyVC.onTapBack = { [weak controller] in
                        controller?.popViewController(animated: true)
                    }
                    
                    verifyVC.onTapResend = { [weak self] in
                        guard let self else {
                            return
                        }
                        
                        self.activeActionDisposable.set((verificationRequest |> deliverOnMainQueue)
                            .start(next: { [weak verifyVC] response in
                                verifyVC?.timer = response.confirmation_expire - Int(Date().timeIntervalSince1970)
                                adaptedVerifyVC.present(standardTextAlertController(theme: .init(presentationData: self.presentationData), title: nil, text: self.presentationData.strings.TwoStepAuth_SetupResendEmailCodeAlert, actions: [.init(type: .defaultAction, title: self.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
                            }, error: errorBlock))
                    }
                    WelcomeController.onOpenMailApp = {[weak self] in
                        guard let self else {
                            return
                        }
                        self.context.sharedContext.applicationBindings.openUrl(defineMessageApp())
                    }
                }
            }, error: errorBlock))
    }
    ...
    private func showChangePasswordController() {
        let handleError: ErrorCallback = { [weak self] error in
            guard let self else {
                return
            }
            if let controller = ELProfileController.navigationController?.topViewController as? ViewController {
                controller.present(standardTextAlertController(theme: .init(presentationData: self.presentationData), title: nil, text: "\(error.localizedDescription)", actions: [.init(type: .defaultAction, title: self.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
            }
        }
        let vc = ELProfileController.changePassword({ [weak self] result in
            guard let self else {
                return
            }
            let signal: Signal<Api.UserFull, BaseError> = (verifyPassword(network: self.context.account.network, password: result.0.password)  |> deliverOnMainQueue)
            |> mapError { _ -> BaseError in
               result.1(false)
                return BaseError.message("Invalid password")
           }
            |> mapToSignal { _ -> Signal<Api.UserFull, BaseError> in
                return getFullUser(network: self.context.account.network)
            }
            self.activeActionDisposable.set((signal |> deliverOnMainQueue).start(next: { [weak self] user in
                    guard let self else {
                        return
                    }
                    switch user {
                    case let .userFull(_, _, _, _, _, _, _, email, _, _, _, _, _, _, _, _, _, _, _, _):
                        self.showVerifyController(verificationRequest: sendVerifyCode(with: email ?? "", network: self.context.account.network) , onVerifyCode: { code in
                            self.activeActionDisposable.set((changePassword(with: result.0.password, newPassword: result.0.newPassword, code: code ?? "", network: self.context.account.network) |> deliverOnMainQueue)
                                .start(next: { response in
                                    let vcc = ELProfileController.navigationController?.topViewController as? ViewController
                                    vcc?.present(standardTextAlertController(theme: .init(presentationData: self.presentationData), title: nil, text: self.presentationData.strings.TwoStepAuth_PasswordChangeSuccess, actions: [.init(type: .defaultAction, title: self.presentationData.strings.Common_OK, action: {})]), in: .window(.root))
                                    (self.controller?.navigationController as? NavigationController)?.popToRoot(animated: true)
                                }, error: handleError))
    ...
    private func showDeleteAccountController() {
        let handleError: ErrorCallback = { [weak self] error in
            guard let self else {
                return
            }
        ...
        let vc = ELProfileController.deleteAccount({ [weak self] result in
            guard let self else {
                return
            }
            
            self.showVerifyController(verificationRequest: deleteAccount(email: result.email, password: result.password, network: self.context.account.network), onVerifyCode: { code in
                self.showDeleteAccount {
                    self.activeActionDisposable.set((confirmDeleteAccount(code: code ?? "", network: self.context.account.network) |> deliverOnMainQueue)
                        .start(next: { response in
                            let _ = logoutFromAccount(id: self.context.account.id, 
    ...
    private func presentEmailComposeController(address: String, subject: String, body: String, from controller: ViewController) {
        if MFMailComposeViewController.canSendMail() {
            ...
            composeController.mailComposeDelegate = self.controller
    ...
    private func presentLogoutAlert() {
        if let user = self.data?.peer as? ElloAppUser, let _ = user.username {
            let presentationData = context.sharedContext.currentPresentationData.with { $0 }
            let alertController = textAlertController(context: context, 
                                                      title: presentationData.strings.Settings_LogoutConfirmationTitle,
                                                      text: presentationData.strings.Settings_LogoutConfirmationDescription,
                                                      actions: [
                TextAlertAction(type: .genericAction, 
                                title: presentationData.strings.Common_Cancel,
                                action: {
                                    
                }),
                TextAlertAction(type: .destructiveAction, 
                                title: presentationData.strings.Settings_Logout,
                                action: {
                    let _ = logoutFromAccount(id: self.context.account.id, accountManager:  self.context.sharedContext.accountManager, alreadyLoggedOutRemotely: false).start()
                    //                            onDismiss?()
                })
            ])
...
//                    with: ViewControllerPresentationArguments(presentationAnimation: .modalSheet)
    ...
    private func logoutAccount(id: AccountRecordId) {
        ...
        items.append(ActionSheetButtonItem(title: self.presentationData.strings.Settings_Logout, color: .destructive, action: { [weak self] in
            dismissAction()
            if let strongSelf = self {
                let _ = logoutFromAccount(id: id, 
                                          accountManager: strongSelf.context.sharedContext.accountManager,
                                          alreadyLoggedOutRemotely: false).start()
            }
        }))
    ...
    private func accountContextMenu(id: AccountRecordId, node: ASDisplayNode, gesture: ContextGesture?) {
        ...
        if let selectedAccount = selectedAccount {
            ...
            let contextController = ContextController(account: accountContext.account, 
                                                      presentationData: self.presentationData,
                                                      source: .controller(ContextControllerContentSourceImpl(controller: chatListController, sourceNode: node)),
                                                      items: accountContextMenuItems(context: accountContext,
                                                                                     logout: { [weak self] in
        ...
        if let messageIds = messageIds ?? self.state.selectedMessageIds, !messageIds.isEmpty {
            ...
            |> deliverOnMainQueue).start(next: { [weak self] actions in
                if let strongSelf = self, let peer = strongSelf.data?.peer, !actions.options.isEmpty {
                    ...
                    if let user = peer as? ElloAppUser {
                        personalPeerName = EnginePeer(user).compactDisplayTitle
                    } else if let channel = peer as? ElloAppChannel, case .broadcast = channel.info {
                        isChannel = true
                    }
        ...
        if let messageIds = messageIds ?? self.state.selectedMessageIds, !messageIds.isEmpty {
            ...
            peerSelectionController.multiplePeersSelected = { [weak self, weak peerSelectionController] peers, peerMap, messageText, mode, forwardOptions in
                ...
                for peer in peers {
                    ...
                    if let secretPeer = peer as? ElloAppSecretChat {
    ...
    private func displayMediaGalleryContextMenu(source: ContextReferenceContentNode, gesture: ContextGesture?) {
        ...
        let _ = (self.context.engine.data.get(EngineDataMap([
            ElloAppEngine.EngineData.Item.Messages.MessageCount(peerId: peerId, tag: .photo),
            ElloAppEngine.EngineData.Item.Messages.MessageCount(peerId: peerId, tag: .video)
        ]))
    ...
    func setupWallet(completionHandler: @escaping (_ controller: ViewController) -> Void) {
        ELProfileController.navigationController = self.controller?.navigationController
        ELProfileController.accountContext = context
        
        ELProfileController.walletsController(network: context.account.network) { controller in
            completionHandler(controller)
        }
        
        let user = self.data?.peer as? ElloAppUser
        ELProfileController.user = user
        
        ELProfileController.countriesSignal = Signal<[ELProfileUI.Country], Never> { observer in
            return (ElloAppEngine(account: self.context.account).localization.getCountriesList(accountManager: self.context.sharedContext.accountManager, langCode: "", forceUpdate: true)
                    |> deliverOnMainQueue)
            .start(next: { countries in
                let processed = countries.map {
                    ELProfileUI.Country(name: $0.name, code: $0.countryCodes.first?.code ?? "", flagCode: $0.localizedName ?? "", flag: URL(string: $0.id))
        ...
        ELProfileController.onSendVerifyRequest = { [weak self] completion in
            guard let self else { return }

            let verifySignal = self.context.account.network.request(Api.wallet.sendAuthEmail())
            |> mapError { error -> BaseError in
                debugPrint(error)
                return .message(error.errorDescription)
            }
            
            self.showVerifyController(verificationRequest: verifySignal, onVerifyCode: { code in
                completion(code ?? "")
            }, errorBlock: handleError)
        }
    }
    ...
    func containerLayoutUpdated(layout: ContainerViewLayout, navigationHeight: CGFloat, transition: ContainedViewLayoutTransition, additive: Bool = false) {
        ...
        if !self.isMediaOnly {
            ...
            for (sectionId, sectionItems) in items {
                ...
                let sectionHeight = sectionNode.update(width: sectionWidth,
                                                       safeInsets: UIEdgeInsets(),
                                                       hasCorners: !insets.left.isZero,
                                                       presentationData: self.presentationData,
                                                       items: sectionItems,
                                                       transition: transition)
            ...
            let editItems = self.isSettings ? settingsEditingItems(data: self.data,
                                                                   state: self.state,
                                                                   context: self.context,
                                                                   presentationData: self.presentationData,
                                                                   interaction: self.interaction)
                                            : editingItems(data: self.data,
                                                           context: self.context,
                                                           presentationData: self.presentationData,
                                                           interaction: self.interaction)
            ...
            if let current = self.paneContainerNode.selectionPanelNode {
                ...
                selectionPanelNode = PeerInfoSelectionPanelNode(context: self.context, presentationData: self.presentationData, peerId: self.peerId, deleteMessages: { [weak self] in
                    ...
                    let _ = (strongSelf.context.engine.data.get(EngineDataMap(
                        messageIds.map(ElloAppEngine.EngineData.Item.Messages.Message.init)
                    ))
    ...
    fileprivate func updateNavigation(transition: ContainedViewLayoutTransition, additive: Bool) {
        ...
        if let (layout, navigationHeight) = self.validLayout {
            if !additive {
                ...
                let _ = self.headerNode.update(width: layout.size.width,
                                               containerHeight: layout.size.height,
                                               containerInset: headerInset,
                                               statusBarHeight: layout.statusBarHeight ?? 0.0,
                                               navigationHeight: navigationHeight,
                                               isModalOverlay: layout.isModalOverlay,
                                               isMediaOnly: self.isMediaOnly,
                                               contentOffset: self.isMediaOnly ? 212.0 : offsetY,
                                               paneContainerY: self.paneContainerNode.frame.minY,
                                               presentationData: self.presentationData,
                                               peer: self.data?.peer,
                                               cachedData: self.data?.cachedData,
            ...
            self.paneContainerNode.update(size: self.paneContainerNode.bounds.size,
                                          sideInset: layout.safeInsets.left,
                                          bottomInset: bottomInset,
                                          visibleHeight: visibleHeight,
                                          expansionFraction: effectiveAreaExpansionFraction,
                                          presentationData: self.presentationData,
                                          data: self.data,
                                          transition: transition)
          
            transition.updateFrame(node: self.headerNode.navigationButtonContainer,
                                   frame: CGRect(origin: CGPoint(x: layout.safeInsets.left,
                                                                 y: layout.statusBarHeight ?? 0.0),
                                                 size: CGSize(width: layout.size.width - layout.safeInsets.left * 2.0,
                                                              height: navigationBarHeight)))
            self.headerNode.navigationButtonContainer.isWhite = self.headerNode.isAvatarExpanded
            
            let leftNavigationButtons: [PeerInfoHeaderNavigationButtonSpec] = []
...
//                    rightNavigationButtons.append(PeerInfoHeaderNavigationButtonSpec(key: .edit, isForExpandedView: false))
                    rightNavigationButtons.append(PeerInfoHeaderNavigationButtonSpec(key: .settingEdit, isForExpandedView: false))
//                    rightNavigationButtons.append(PeerInfoHeaderNavigationButtonSpec(key: .search, isForExpandedView: true))
            ...
            self.headerNode.navigationButtonContainer.update(size: CGSize(width: layout.size.width - layout.safeInsets.left * 2.0,
                                                                          height: navigationBarHeight),
                                                             presentationData: self.presentationData,
                                                             leftButtons: leftNavigationButtons,
                                                             rightButtons: rightNavigationButtons,
                                                             expandFraction: effectiveAreaExpansionFraction,
                                                             transition: transition)
        }
    }
...
// MARK: - PeerInfoScreenImpl

public final class PeerInfoScreenImpl: ViewController, PeerInfoScreen, KeyShortcutResponder, MFMailComposeViewControllerDelegate {
    private let context: AccountContext
...
//            if let user = self.data?.peer as? ElloAppUser, let _ = user.phone {
//                let presentationData = context.sharedContext.currentPresentationData.with { $0 }
...
//                    })
//                ])
//                self.controller?.present(alertController, in: .window(.root), with: ViewControllerPresentationArguments(presentationAnimation: .modalSheet))
//            }
//            print("On logout...")
//        }
        ...
        if isSettings {
            ...
            let accountTabBarAvatar: Signal<(UIImage, UIImage)?, NoError> = combineLatest(
                self.accountsAndPeers.get(), context.sharedContext.presentationData)
            ...
            |> mapToSignal { primary -> Signal<(UIImage, UIImage)?, NoError> in
                if let primary = primary {
                    ...
                    if let signal = peerAvatarImage(account: primary.0,
                                                    peerReference: PeerReference(primary.1._asPeer()),
                                                    authorOfMessage: nil,
                                                    representation: primary.1.profileImageRepresentations.first,
...
//            }
            ...
            let tabBarItem: Signal<(String, UIImage?, UIImage?, String?, Bool, Bool), NoError> =
                combineLatest(queue: .mainQueue(),
                              self.context.sharedContext.presentationData,
                              notificationsAuthorizationStatus.get(),
                              notificationsWarningSuppressed.get(), 
                              getServerProvidedSuggestions(account: self.context.account),
                              accountTabBarAvatar,
            ...
            |> map {    presentationData,
                        ...
                        suggestions,
                        accountTabBarAvatar,
                        accountTabBarAvatarBadge -> (String, UIImage?, UIImage?, String?, Bool, Bool) in
                ...
                let isManyAccounts = 0 < self.accountsAndPeersValue?.1.count ?? 0
                return (presentationData.strings.Settings_Title,
                        isManyAccounts ? accountTabBarAvatar?.0 ?? icon : icon,
                        isManyAccounts ? accountTabBarAvatar?.1 ?? icon : icon,
                        notificationsWarning || phoneNumberWarning || passwordWarning ? "!" : otherAccountsBadge,
                        isManyAccounts,
                        presentationData.reduceMotion)
            }
            ...
            self.tabBarItemDisposable = (tabBarItem |> deliverOnMainQueue).start(next: { [weak self] title, image, selectedImage, badgeValue, isAvatar, reduceMotion in
                if let strongSelf = self {
                    strongSelf.tabBarItem.title = PresentationTheme.isTabbarTitleShown ? title : ""
                    strongSelf.tabBarItem.image = image?.preparingThumbnail(of: CGSize(width: 24, height: 24))
                    strongSelf.tabBarItem.selectedImage = selectedImage?.preparingThumbnail(of: CGSize(width: 24, height: 24))
...
//        }
            ...
            if let strongSelf = self {
                ...
                if previousTheme !== presentationData.theme || previousStrings !== presentationData.strings {
                    ...
                    if strongSelf.navigationItem.backBarButtonItem != nil {
                        strongSelf.navigationItem.backBarButtonItem = UIBarButtonItem(title: strongSelf.presentationData.strings.Common_Back, 
                                                                                      style: .plain, 
                                                                                      target: nil,
                                                                                      action: nil)
                    }
    ...
    override public func loadDisplayNode() {
        self.displayNode = PeerInfoScreenNode(controller: self, 
                                              context: self.context,
                                              peerId: self.peerId,
        ...
        if !chatNavigationStack.isEmpty {
            ...
            self.navigationBar?.backButtonNode.activated = { [weak self] gesture, _ in
                ...
                let _ = (strongSelf.context.engine.data.get(EngineDataList(
                    chatNavigationStack.map(ElloAppEngine.EngineData.Item.Peer.Peer.init)
                ))
                |> deliverOnMainQueue).start(next: { peerList in
                    ...
                    var items: [ContextMenuItem] = []
                    for peer in peers {
                        items.append(.action(ContextMenuActionItem(text: peer.displayTitle(strings: strongSelf.presentationData.strings,
                                                                                           displayOrder: strongSelf.presentationData.nameDisplayOrder),
                                                                   icon: { _ in return nil },
                                                                   iconSource: ContextMenuActionItemIconSource(size: avatarSize,
                                                                                                               signal: peerAvatarCompleteImage(account: strongSelf.context.account, peer: peer, size: avatarSize)),
                                                                   action: { _, f in
                            f(.default)
                    ...
                    let contextController = ContextController(account: strongSelf.context.account,
                                                              presentationData: strongSelf.presentationData,
                                                              source: .reference(ChatControllerContextReferenceContentSource(controller: strongSelf,
                                                                                                                             sourceView: backButtonNode.view,
                                                                                                                             insets: UIEdgeInsets(),
                                                                                                                             contentInsets: UIEdgeInsets(top: 0.0, left: -15.0, bottom: 0.0, right: -15.0))),
    ...
    override public func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        self.controllerNode.containerLayoutUpdated(layout: layout, 
                                                   navigationHeight: navigationHeight,
                                                   transition: transition)
    }
    ...
    override public func tabBarItemContextAction(sourceNode: ContextExtractedContentContainingNode, gesture: ContextGesture) {
        ...
        items.append(.custom(AccountPeerContextItem(context: self.context,
...
//        let avatarSize = CGSize(width: 28.0, height: 28.0)
//        
//        items.append(.action(ContextMenuActionItem(text: primary.1.displayTitle(strings: strings,
//                                                                                displayOrder: presentationData.nameDisplayOrder),
//                                                   icon: { _ in nil },
//                                                   iconSource: ContextMenuActionItemIconSource(size: avatarSize,
//                                                                                               signal: peerAvatarCompleteImage(account:primary.0.account,
//                                                                                                                               peer: primary.1,
//                                                                                                                               size: avatarSize)),
//                                                   action: { _, f in
//            f(.default)
//        })))
        ...
        let controller = ContextController(account: primary.0.account, 
                                           presentationData: self.presentationData,
                                           source: .extracted(SettingsTabBarContextExtractedContentSource(controller: self, sourceNode: sourceNode)),
                                           items: .single(ContextController.Items(content: .list(items))),
                                           recognizer: nil,
                                           gesture: gesture)
...
private func getUserPeer(engine: ElloAppEngine, peerId: EnginePeer.Id) -> Signal<EnginePeer?, NoError> {
    return engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId))
    |> mapToSignal { peer -> Signal<EnginePeer?, NoError> in
        guard let peer = peer else {
            return .single(nil)
        }
        if case let .secretChat(secretChat) = peer {
            return engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: secretChat.regularPeerId))
        } else {
            return .single(peer)
        }
    }
    ...
    func update(containerSize: CGSize, fraction: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        if let previousTitleView = bottomNavigationBar.titleView as? ChatTitleView, let _ = (bottomNavigationBar.rightButtonNode.singleCustomNode as? ChatAvatarNavigationNode)?.avatarNode, let (previousTitleContainerNode, previousTitleNode) = self.previousTitleNode, let (previousStatusContainerNode, previousStatusNode) = self.previousStatusNode {
            ...
            if let (layout, _) = self.screenNode.validLayout {
                ...
                topHeight = self.headerNode.update(width: layout.size.width,
                                                   containerHeight: layout.size.height,
                                                   containerInset: headerInset,
                                                   statusBarHeight: layout.statusBarHeight ?? 0.0,
                                                   navigationHeight: topNavigationBar.bounds.height,
                                                   isModalOverlay: layout.isModalOverlay,
                                                   isMediaOnly: false,
                                                   contentOffset: 0.0,
                                                   paneContainerY: 0.0,
                                                   presentationData: self.presentationData,
                                                   peer: self.screenNode.data?.peer,
                                                   cachedData: self.screenNode.data?.cachedData,
                                                   notificationSettings: self.screenNode.data?.notificationSettings,
                                                   statusData: self.screenNode.data?.status,
                                                   panelStatusData: (nil, nil, nil),
                                                   isSecretChat: self.screenNode.peerId.namespace == Namespaces.Peer.SecretChat,
                                                   isContact: self.screenNode.data?.isContact ?? false,
                                                   isSettings: self.screenNode.isSettings,
                                                   state: self.screenNode.state,
                                                   metrics: layout.metrics,
                                                   transition: transition,
                                                   additive: false)
            }
...
func presentAddMembers(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)?, parentController: ViewController, groupPeer: Peer, selectAddMemberDisposable: MetaDisposable, addMemberDisposable: MetaDisposable) {
    ...
    |> deliverOnMainQueue).start(next: { [weak parentController] recentIds in
        ...
        if let group = groupPeer as? ElloAppGroup {
            switch group.role {
            case .creator:
        ...
        } else if let channel = groupPeer as? ElloAppChannel, (channel.addressName?.isEmpty ?? true) {
                    ...
                    |> `catch` { error -> Signal<Void, NoError> in
                        ...
                        switch error {
                            ...
                            case .notMutualContact:
                                if let peer = groupPeer as? ElloAppChannel, case .broadcast = peer.info {
                                    text = presentationData.strings.Channel_AddUserLeftError
                                } else {
                                    text = presentationData.strings.GroupInfo_AddUserLeftError
                                }
                            case let .bot(memberId):
                                guard let peer = groupPeer as? ElloAppChannel else {
...
struct ClearPeerHistory {
    ...
    init(context: AccountContext, peer: Peer, chatPeer: Peer, cachedData: CachedPeerData?) {
        if peer.id == context.account.peerId {
        ...
        } else if chatPeer is ElloAppSecretChat {
            canClearCache = false
            ...
            if let user = chatPeer as? ElloAppUser, user.botInfo != nil {
                canClearForEveryone = nil
            } else {
...
// MARK: - AccountPeerContextItemNode
