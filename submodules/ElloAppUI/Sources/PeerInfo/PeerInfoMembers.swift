...
enum PeerInfoMember: Equatable {
    ...
    case legacyGroupMember(peer: RenderedPeer, role: PeerInfoMemberRole, invitedBy: PeerId?, presence: ElloAppUserPresence?)
    ...
    var presence: ElloAppUserPresence? {
        switch self {
        case let .channelMember(channelMember):
            return channelMember.presences[channelMember.peer.id] as? ElloAppUserPresence
    ...
    init(queue: Queue, context: AccountContext, peerId: PeerId) {
        ...
        if peerId.namespace == Namespaces.Peer.CloudChannel {
            ...
            |> deliverOn(self.queue)).start(next: { [weak self] view in
                ...
                if let channel = peerViewMainPeer(view) as? ElloAppChannel {
            ...
            |> deliverOn(self.queue)).start(next: { [weak self] view in
                ...
                for participant in participantsData.participants {
                    if let peer = view.peers[participant.peerId] {
                        ...
                        unsortedMembers.append(.legacyGroupMember(peer: RenderedPeer(peer: peer), role: role, invitedBy: invitedBy, presence: view.peerPresences[participant.peerId] as? ElloAppUserPresence))
                    }
                }
                
                if let group = peerViewMainPeer(view) as? ElloAppGroup {
                    var canAddMembers = false
                    switch group.role {
                        case .admin, .creator:
                            canAddMembers = true
    ...
    func removeMember(memberId: PeerId) {
        if removingMemberIds[memberId] == nil {
            ...
            if self.peerId.namespace == Namespaces.Peer.CloudChannel {
                signal = context.peerChannelMemberCategoriesContextsManager.updateMemberBannedRights(engine: self.context.engine, peerId: self.peerId, memberId: memberId, bannedRights: ElloAppChatBannedRights(flags: [.banReadMessages], untilDate: Int32.max))
                |> ignoreValues
            } else {
