...
final class PeerInfoScreenBioItem: PeerInfoScreenItem {
    let id: AnyHashable
    let label: String
    let text: String
    ...
    init(
        ...
        textBehavior: PeerInfoScreenLabeledValueTextBehavior = .multiLine(maxLines: 100, enabledEntities: EnabledEntityTypes.all)
    ) {
        self.id = id
        self.label = label
        self.text = text
    ...
    func node() -> PeerInfoScreenItemNode {
        return PeerInfoScreenBioItemNode()
    }
}
...
private final class PeerInfoScreenBioItemNode: PeerInfoScreenItemNode {
    ...
    private var item: PeerInfoScreenBioItem?
    ...
    override func update(width: CGFloat, safeInsets: UIEdgeInsets, presentationData: PresentationData, item: PeerInfoScreenItem, topItem: PeerInfoScreenItem?, bottomItem: PeerInfoScreenItem?, hasCorners: Bool, transition: ContainedViewLayoutTransition) -> CGFloat {
        guard let item = item as? PeerInfoScreenBioItem else {
            return 10.0
        }
        ...
        self.expandNode.attributedText = NSAttributedString(string: presentationData.strings.PeerInfo_BioExpand, font: Font.regular(12), textColor: presentationData.theme.list.itemAccentColor)
        ...
        let additionalSideInset: CGFloat = 0.0
        ...
        var additionalTextSize = self.additionalTextNode.updateLayout(CGSize(width: width - sideInset * 2.0, height: .greatestFiniteMagnitude))
        ...
        if case .multiLine = item.textBehavior, displayMore {
            ...
            additionalTextSize.height = additionalTextSize.height + expandSize.height
        } else {
        ...
        let expandFrame = CGRect(origin: CGPoint(x: sideInset + UIScreenPixel, y: textFrame.maxY - expandSize.height + expandSize.height + 5), size: expandSize)
        ...
        let _: CGFloat = topItem == nil ? 0.0 : UIScreenPixel
