...
final class PeerInfoScreenSettingsItem: PeerInfoScreenItem {
    enum Label {
        case none
        case text(String)
        ...
        var text: String {
            switch self {
                case .none:
                    return ""
            case let .text(text), let .badge(text, _), let .status(_, text, _):
                    return text
            }
        }
    ...
    func node() -> PeerInfoScreenItemNode {
        return PeerInfoScreenSettingsItemNode()
    }
}

private final class PeerInfoScreenSettingsItemNode: PeerInfoScreenItemNode {
    ...
    private var item: PeerInfoScreenSettingsItem?
    ...
    override func update(width: CGFloat, safeInsets: UIEdgeInsets, presentationData: PresentationData, item: PeerInfoScreenItem, topItem: PeerInfoScreenItem?, bottomItem: PeerInfoScreenItem?, hasCorners: Bool, transition: ContainedViewLayoutTransition) -> CGFloat {
        guard let item = item as? PeerInfoScreenSettingsItem else {
            return 10.0
        }
        ...
        if case .badge = item.label {
            labelColorValue = presentationData.theme.list.itemPrimaryTextColor
            labelFont = Font.medium(15.0)
        } else {
        ...
        if case .badge = item.label {
            labelFrame = CGRect(origin: CGPoint(x: width - rightInset + (badgeWidth - labelSize.width) / 2.0, y: floor((height - labelSize.height) / 2.0)), size: labelSize)
        } else {
