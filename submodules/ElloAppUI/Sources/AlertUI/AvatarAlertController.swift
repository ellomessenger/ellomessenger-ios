...
public enum AvatarAlertActionType {
    case genericAction
    case defaultAction
    case destructiveAction
}

public struct AvatarAlertAction {
    public let type: AvatarAlertActionType
    public let title: String
    public let action: () -> Void
    
    public init(type: AvatarAlertActionType, title: String, action: @escaping () -> Void) {
        self.type = type
        self.title = title
        self.action = action
    }
...
public final class AvatarAlertContentActionNode: HighlightableButtonNode {
    private var theme: AlertControllerTheme
    let action: AvatarAlertAction
    
    private let backgroundNode: ASDisplayNode
    
    var highlightedUpdated: (Bool) -> Void = { _ in }
        
    public init(theme: AlertControllerTheme, action: AvatarAlertAction) {
        self.theme = theme
        self.action = action
...
public enum AvatarAlertContentActionLayout {
    case horizontal
    case vertical
}

public final class AvatarAlertContentNode: AlertContentNode {
    private var theme: AlertControllerTheme
    private let actionLayout: AvatarAlertContentActionLayout
    ...
    private let actionNodes: [AvatarAlertContentActionNode]
    ...
    public init(theme: AlertControllerTheme, image: UIImage, title: NSAttributedString?, text: NSAttributedString, actions: [AvatarAlertAction], actionLayout: AvatarAlertContentActionLayout, dismissOnOutsideTap: Bool) {
        self.theme = theme
...
//        self.actionNodesSeparator.backgroundColor = theme.separatorColor
        ...
        self.actionNodes = actions.map { action -> AvatarAlertContentActionNode in
            return AvatarAlertContentActionNode(theme: theme, action: action)
        }
...
//                separatorNode.backgroundColor = theme.separatorColor
    ...
    override public func updateLayout(size: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        let spacing = 16.0
        let imageSize = CGSize(width: alertWidth, height: 106)
        ...
        if let titleNode, let titleSize, titleSize.height != .zero {
            ...
            resultSize = CGSize(
                ...
                height: imageSize.height + spacing + titleSize.height + spacing + textSize.height + actionsHeight + insets.top + insets.bottom
            )
        } else {
            let textFrame = CGRect(
                origin: CGPoint(
            ...
            resultSize = CGSize(
                width: contentWidth + insets.left + insets.right,
                height: imageSize.height + spacing + textSize.height + actionsHeight + insets.top + insets.bottom
            )
        }
...
public func AvatarAlertController(theme: AlertControllerTheme, image: UIImage, title: NSAttributedString?, text: NSAttributedString, actions: [AvatarAlertAction], actionLayout: AvatarAlertContentActionLayout = .horizontal, dismissOnOutsideTap: Bool = true) -> AlertController {
    return AlertController(theme: theme, contentNode: AvatarAlertContentNode(theme: theme, image: image, title: title, text: text, actions: actions, actionLayout: actionLayout, dismissOnOutsideTap: dismissOnOutsideTap))
}

public func standardAvatarAlertController(theme: AlertControllerTheme, 
                                          image: UIImage,
                                          title: String?,
                                          text: String,
                                          actions: [AvatarAlertAction],
                                          actionLayout: AvatarAlertContentActionLayout = .vertical,
                                          allowInputInset: Bool = true,
                                          parseMarkdown: Bool = false,
                                          dismissOnOutsideTap: Bool = true) -> AlertController {
...
//        let boldFont = title == nil ? Font.bold(theme.baseFontSize * 13.0 / 17.0) : Font.semibold(floor(theme.baseFontSize * 13.0 / 17.0))
    ...
    let controller = AlertController(theme: theme, contentNode: AvatarAlertContentNode(theme: theme, image: image, title: title != nil ? NSAttributedString(string: title!, font: Font.semibold(theme.baseFontSize), textColor: theme.primaryColor, paragraphAlignment: .center) : nil, text: attributedText, actions: actions.map { action in
        return AvatarAlertAction(type: action.type, title: action.title, action: {
            dismissImpl?()
            action.action()
        })
