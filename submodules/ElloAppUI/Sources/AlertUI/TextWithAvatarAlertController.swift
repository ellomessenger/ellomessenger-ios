...
private let alertWidth: CGFloat = 340.0

public enum TextWithAvatarAlertActionType {
    case genericAction
    case defaultAction
    case destructiveAction
}

public struct TextWithAvatarAlertAction {
    public let type: TextWithAvatarAlertActionType
    public let title: String
    public let action: () -> Void
    
    public init(type: TextWithAvatarAlertActionType, title: String, action: @escaping () -> Void) {
        self.type = type
        self.title = title
        self.action = action
    }
...
public final class TextWithAvatarAlertContentActionNode: HighlightableButtonNode {
    private var theme: AlertControllerTheme
    let action: TextWithAvatarAlertAction
    
    private let backgroundNode: ASDisplayNode
    
    var highlightedUpdated: (Bool) -> Void = { _ in }
        
    public init(theme: AlertControllerTheme, action: TextWithAvatarAlertAction) {
        self.theme = theme
        self.action = action
...
public enum TextWithAvatarAlertContentActionLayout {
    case horizontal
    case vertical
}

public final class TextWithAvatarAlertContentNode: AlertContentNode {
    private var theme: AlertControllerTheme
    private let actionLayout: TextWithAvatarAlertContentActionLayout
    ...
    private let actionNodes: [TextWithAvatarAlertContentActionNode]
    ...
    public init(theme: AlertControllerTheme, avatar: (context: AccountContext, peer: EnginePeer), title: NSAttributedString?, text: NSAttributedString, actions: [TextWithAvatarAlertAction], actionLayout: TextWithAvatarAlertContentActionLayout, dismissOnOutsideTap: Bool) {
        self.theme = theme
        ...
        self.avatarNode.setPeer(context: avatar.context, peer: avatar.peer)
...
//        self.actionNodesSeparator.backgroundColor = theme.separatorColor
        self.actionNodesSeparator.backgroundColor = UIColor(rgb: 0x3C3C43, alpha: 0.36)
        
        self.actionNodes = actions.map { action -> TextWithAvatarAlertContentActionNode in
            return TextWithAvatarAlertContentActionNode(theme: theme, action: action)
        }
...
//                separatorNode.backgroundColor = theme.separatorColor
                separatorNode.backgroundColor = UIColor(rgb: 0x3C3C43, alpha: 0.36)
    ...
    override public func updateLayout(size: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        let insets = UIEdgeInsets(top: 20.0, left: 16.0, bottom: 16.0, right: 20.0)
        ...
        let spacing = 16.0
        let avatarSize = CGSize(width: 54.0, height: 54.0)
        let avatarOrigin = CGPoint(
            x: insets.left + floor((contentWidth - avatarSize.width) / 2.0),
            y: insets.top
        )
        let avatarFrame = CGRect(origin: avatarOrigin, size: avatarSize)
        transition.updateFrame(node: avatarNode, frame: avatarFrame)
        
        if let titleNode, let titleSize, titleSize.height != .zero {
            let titleFrame = CGRect(
                origin: CGPoint(
                    x: insets.left + floor((contentWidth - titleSize.width) / 2.0),
                    y: avatarFrame.maxY + spacing
                ),
                size: titleSize
            )
            ...
            let textFrame = CGRect(
                origin: CGPoint(
                    x: insets.left + floor((contentWidth - textSize.width) / 2.0),
                    y: titleFrame.maxY + spacing
                ),
                size: textSize
            )
            transition.updateFrame(node: textNode, frame: textFrame.offsetBy(dx: -1.0, dy: -1.0))
            
            resultSize = CGSize(
                width: contentWidth + insets.left + insets.right,
                height: avatarSize.height + spacing + titleSize.height + spacing + textSize.height + actionsHeight + insets.top + insets.bottom
            )
        } else {
            let textFrame = CGRect(
                origin: CGPoint(
            ...
            resultSize = CGSize(
                width: contentWidth + insets.left + insets.right,
                height: avatarSize.height + spacing + textSize.height + actionsHeight + insets.top + insets.bottom
            )
        }
...
public func textWithAvatarAlertController(theme: AlertControllerTheme, avatar: (context: AccountContext, peer: EnginePeer), title: NSAttributedString?, text: NSAttributedString, actions: [TextWithAvatarAlertAction], actionLayout: TextWithAvatarAlertContentActionLayout = .horizontal, dismissOnOutsideTap: Bool = true) -> AlertController {
    return AlertController(theme: theme, contentNode: TextWithAvatarAlertContentNode(theme: theme, avatar: avatar, title: title, text: text, actions: actions, actionLayout: actionLayout, dismissOnOutsideTap: dismissOnOutsideTap))
}

public func standardTextWithAvatarAlertController(theme: AlertControllerTheme, avatar: (context: AccountContext, peer: EnginePeer), title: String?, text: String, actions: [TextWithAvatarAlertAction], actionLayout: TextWithAvatarAlertContentActionLayout = .horizontal, allowInputInset: Bool = true, parseMarkdown: Bool = false, dismissOnOutsideTap: Bool = true) -> AlertController {
...
//        let boldFont = title == nil ? Font.bold(theme.baseFontSize * 13.0 / 17.0) : Font.semibold(floor(theme.baseFontSize * 13.0 / 17.0))
        ...
        let body = MarkdownAttributeSet(font: font, textColor: .black)
        ...
        attributedText = NSAttributedString(
            string: text,
            font: title == nil ? Font.semibold(theme.baseFontSize) : Font.regular(floor(theme.baseFontSize * 16.0 / 17.0)),
            textColor: theme.primaryColor,
            paragraphAlignment: .center
        )
    }
    let controller = AlertController(theme: theme, contentNode: TextWithAvatarAlertContentNode(theme: theme, avatar: avatar, title: title != nil ? NSAttributedString(string: title!, font: Font.semibold(theme.baseFontSize), textColor: theme.primaryColor, paragraphAlignment: .center) : nil, text: attributedText, actions: actions.map { action in
        return TextWithAvatarAlertAction(type: action.type, title: action.title, action: {
            dismissImpl?()
            action.action()
        })
