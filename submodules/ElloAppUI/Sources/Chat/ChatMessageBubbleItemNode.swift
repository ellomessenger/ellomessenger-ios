...
private func contentNodeMessagesAndClassesForItem(_ item: ChatMessageItem) -> ([(Message, AnyClass, ChatMessageEntryAttributes, BubbleItemAttributes)], Bool, Bool) {
    ...
    outer: for (message, itemAttributes) in item.content {
        ...
        inner: for media in message.media {
            if let _ = media as? ElloAppMediaImage {
            ...
            } else if let action = media as? ElloAppMediaAction {
                isAction = true
                ...
                needReactions = false
                break inner
            } else if let invoice = media as? ElloAppMediaInvoice {
                ...
                needReactions = false
            } else if let _ = media as? ElloAppMediaUnsupported {
        ...
        inner: for media in message.media {
            if let webpage = media as? ElloAppMediaWebpage {
    ...
    if !isAction && !Namespaces.Message.allScheduled.contains(firstMessage.id.namespace) {
        ...
        if let channel = firstMessage.peers[firstMessage.id.peerId] as? ElloAppChannel, case let .broadcast(info) = channel.info, info.flags.contains(.hasDiscussionGroup) {
            hasDiscussion = true
        }
...
class ChatMessageBubbleItemNode: ChatMessageItemView, ChatMessagePreviewItemNode {
        ...
        replyRecognizer.shouldBegin = { [weak self] in
            if let strongSelf = self, let item = strongSelf.item {
                ...
                for media in item.content.firstMessage.media {
                    ...
                    else if let media = media as? ElloAppMediaAction {
    ...
    override func asyncLayout() -> (_ item: ChatMessageItem, _ params: ListViewItemLayoutParams, _ mergedTop: ChatMessageMerge, _ mergedBottom: ChatMessageMerge, _ dateHeaderAtBottom: Bool) -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation, ListViewItemApply, Bool) -> Void) {
        ...
        return { item, params, mergedTop, mergedBottom, dateHeaderAtBottom in
            ...
            return ChatMessageBubbleItemNode.beginLayout(selfReference: weakSelf, 
                                                                        item,
                                                                        params,
                                                                        mergedTop,
                                                                        mergedBottom,
    ...
    private static func beginLayout(selfReference: Weak<ChatMessageBubbleItemNode>, 
        ...
        do {
            ...
            if let subject = item.associatedData.subject, case .forwardedMessages = subject {
                ...
                if let forwardInfo = item.content.firstMessage.forwardInfo {
                    ...
                    if effectiveAuthor == nil, let authorSignature = forwardInfo.authorSignature  {
                        effectiveAuthor = ElloAppUser(id: PeerId(namespace: Namespaces.Peer.Empty, id: PeerId.Id._internalFromInt64Value(Int64(authorSignature.persistentHashValue % 32))), accessHash: nil, firstName: authorSignature, lastName: nil, username: nil, phone: nil, photo: [], botInfo: nil, restrictionInfo: nil, flags: [], emojiStatus: nil, isBusiness: false, isPublic: false, botDescription: nil)
                    }
                }
                ...
                if let author = firstMessage.author, author is ElloAppChannel, !incoming {
                    allowAuthor = true
            ...
            if !peerId.isRepliesOrSavedMessages(accountPeerId: item.context.account.peerId) {
                if peerId.isGroupOrChannel && effectiveAuthor != nil {
                    ...
                    if let peer = firstMessage.peers[firstMessage.id.peerId] as? ElloAppChannel, case .broadcast = peer.info {
        ...
        if let forwardInfo = item.content.firstMessage.forwardInfo, forwardInfo.source == nil, forwardInfo.author?.id.namespace == Namespaces.Peer.CloudUser {
            for media in item.content.firstMessage.media {
                if let file = media as? ElloAppMediaFile, file.isMusic {
                    ignoreForward = true
                    break
                }
            }
        ...
        if case .pinnedMessages = item.associatedData.subject {
            ...
            if let peer = item.message.peers[item.message.id.peerId] {
                if let channel = peer as? ElloAppChannel {
                    if case .broadcast = channel.info, !channel.isPrivate {
                        needsShareButton = true
                    }
                }
            }
            ...
            if let info = item.message.forwardInfo {
                if let author = info.author as? ElloAppUser, let _ = author.botInfo, !item.message.media.isEmpty && !(item.message.media.first is ElloAppMediaAction) {
                    needsShareButton = true
                } else if let author = info.author as? ElloAppChannel, case .broadcast = author.info {
                    needsShareButton = true
                }
            }
            
            if !needsShareButton, let author = item.message.author as? ElloAppUser, let _ = author.botInfo, !item.message.media.isEmpty && !(item.message.media.first is ElloAppMediaAction) {
                needsShareButton = true
            }
            if !needsShareButton {
                loop: for media in item.message.media {
                    if media is ElloAppMediaGame || media is ElloAppMediaInvoice {
                        needsShareButton = true
                        break loop
                    } else if let media = media as? ElloAppMediaWebpage, case .Loaded = media.content {
                        needsShareButton = true
                        break loop
                    }
                }
        ...
        switch content {
            case let .message(message, _, _, attributes, _):
                if let peer = message.peers[message.id.peerId] as? ElloAppChannel {
                    if case .broadcast = peer.info {
                    } else {
                        if isCrosspostFromChannel, let sourceReference = sourceReference, let _ = firstMessage.peers[sourceReference.messageId.peerId] as? ElloAppChannel {
                            authorIsChannel = true
                        }
                        authorRank = attributes.rank
                    }
                } else {
                    if isCrosspostFromChannel, let _ = firstMessage.forwardInfo?.source as? ElloAppChannel {
                        authorIsChannel = true
                    }
                    authorRank = attributes.rank
                }
        ...
        for attribute in firstMessage.attributes {
            if let attribute = attribute as? InlineBotMessageAttribute {
                if let peerId = attribute.peerId, let bot = firstMessage.peers[peerId] as? ElloAppUser {
                ...
                if let chatPeer = firstMessage.peers[firstMessage.id.peerId] as? ElloAppChannel, case .group = chatPeer.info, firstMessage.author is ElloAppChannel, !attribute.signature.isEmpty {
        ...
        if let backgroundHiding = backgroundHiding, case .always = backgroundHiding {
            ...
            if inlineBotNameString == nil && (ignoreForward || firstMessage.forwardInfo == nil) && replyMessage == nil {
                if let first = contentPropertiesAndLayouts.first, first.1.hidesSimpleAuthorHeader && !ignoreNameHiding {
                    if let author = firstMessage.author as? ElloAppChannel, case .group = author.info, author.id == firstMessage.id.peerId, !incoming {
                    } else {
                        initialDisplayHeader = false
                    }
                }
        ...
        if initialDisplayHeader && displayAuthorInfo {
            if let peer = firstMessage.peers[firstMessage.id.peerId] as? ElloAppChannel, case .broadcast = peer.info, item.content.firstMessage.adAttribute == nil {
                ...
                } else if let user = effectiveAuthor as? ElloAppUser, let emojiStatus = user.emojiStatus {
            ...
            if mosaicRange.upperBound == contentPropertiesAndLayouts.count || contentPropertiesAndLayouts[contentPropertiesAndLayouts.count - 1].3.isAttachment {
                ...
                for attribute in message.attributes {
                    if let attribute = attribute as? EditedMessageAttribute {
                        ...
                        if let channel = message.peers[message.id.peerId] as? ElloAppChannel, case .group = channel.info {
        ...
        if displayHeader {
            if authorNameString != nil || inlineBotNameString != nil {
                ...
                let sizeAndApply = authorNameLayout(TextNodeLayoutArguments(attributedString: attributedString, 
                                                                            backgroundColor: nil,
                                                                            maximumNumberOfLines: 1,
                                                                            truncationType: .end,
                                                                            constrainedSize: CGSize(width: max(0, maximumNodeWidth - layoutConstants.text.bubbleInsets.left - layoutConstants.text.bubbleInsets.right - credibilityIconWidth - adminBadgeSizeAndApply.0.size.width), 
    ...
    private static func applyLayout(selfReference: Weak<ChatMessageBubbleItemNode>,
        ...
        // Change background bubble color with some state
        if hideBackground {
            backgroundType = .none
        } else if !incoming {
            if isFailed {
                backgroundType = .outgoing(.Failed)
            } else {
...
//        let isFailed = item.content.firstMessage.effectivelyFailed(timestamp: item.context.account.network.getApproximateRemoteTimestamp())
        ...
        if let actionButtonsSizeAndApply = actionButtonsSizeAndApply {
            ...
            if actionButtonsNode !== strongSelf.actionButtonsNode {
                if item.message.peers.contains(where: { $0.0.isAIBots }) {
                    actionButtonsNode.updateButtons(backgroundColor: UIColor(hex: 0x95BBDF),
                                                    backgroundColorHighlighted: UIColor(hex: 0x40525F),
                                                    textColor: UIColor(hex: 0x070708),
                                                    textColorHighlighted: UIColor(hex: 0xFFFFFF))
                }
    ...
    private func gestureRecognized(gesture: TapLongTapOrDoubleTapGesture, location: CGPoint, recognizer: TapLongTapOrDoubleTapGestureRecognizer?) -> InternalBubbleTapAction? {
        ...
        if let item = self.item {
            for media in item.message.media {
                if let file = media as? ElloAppMediaFile, file.duration != nil {
                    ...
                    if let attribute = attribute as? ReplyMessageAttribute {
                        if let replyMessage = item.message.associatedMessages[attribute.messageId] {
                            for media in replyMessage.media {
                                ...
                                if let webpage = media as? ElloAppMediaWebpage, case let .Loaded(content) = webpage.content, webEmbedType(content: content).supportsSeeking {
                                    mediaMessage = replyMessage
                                    forceOpen = true
                                    break
                                }
                        ...
                        for attribute in item.message.attributes {
                            if let attribute = attribute as? InlineBotMessageAttribute {
                                ...
                                if let peerId = attribute.peerId {
                                    if let botPeer = item.message.peers[peerId] as? ElloAppUser, let inlinePlaceholder = botPeer.botInfo?.inlinePlaceholder, !inlinePlaceholder.isEmpty {
                    ...
                    if let item = self.item, let forwardInfo = item.message.forwardInfo {
                        let performAction: () -> Void = {
                            if let sourceMessageId = forwardInfo.sourceMessageId {
                                if let channel = forwardInfo.author as? ElloAppChannel, channel.username == nil {
                                ...
                                item.controllerInteraction.openPeer(peer.id, peer is ElloAppUser ? .info : .chat(textInputState: nil, subject: nil, peekData: nil), nil, false, nil)
