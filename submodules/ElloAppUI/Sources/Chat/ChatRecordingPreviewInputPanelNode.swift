...
final class ChatRecordingPreviewInputPanelNode: ChatInputPanelNode {
    ...
    init(theme: PresentationTheme) {
        ...
        self.waveformBackgroundNode.image = generateStretchableFilledCircleImage(diameter: 33.0, color: theme.chat.inputPanel.actionControlBackgroundColor)
        ...
        self.durationLabel = MediaPlayerTimeTextNode(textColor: theme.chat.inputPanel.actionControlForegroundColor, textFont: UIFont.Custom.sfProDisplay(ofSize: 12, weight: .bold))
...
private final class PlayPauseIconNode: ManagedAnimationNode {
    ...
    init() {
        ...
        self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause_new"), frames: .range(startFrame: 41, endFrame: 41), duration: 0.01))
    }
        ...
        switch previousState {
            case .pause:
                switch state {
                    case .play:
                        if animated {
                            self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause_new"), frames: .range(startFrame: 41, endFrame: 83), duration: self.duration))
                        } else {
                            self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause_new"), frames: .range(startFrame: 0, endFrame: 0), duration: 0.01))
                        }
                    case .pause:
                        break
                }
            case .play:
                switch state {
                    case .pause:
                        if animated {
                            self.trackTo(item: ManagedAnimationItem(source: .local("anim_playpause_new"), frames: .range(startFrame: 0, endFrame: 35), duration: self.duration))
