...
final class ChatMessageInteractiveFileNode: ASDisplayNode {
    ...
    private var file: ElloAppMediaFile?
        ...
        if shouldBeginTranscription {
            if self.transcribeDisposable == nil {
                ...
                if context.sharedContext.immediateExperimentalUISettings.localTranscription {
                    ...
                    let signal: Signal<LocallyTranscribedAudio?, NoError> = context.engine.data.get(ElloAppEngine.EngineData.Item.Messages.Message(id: message.id))
                    |> mapToSignal { message -> Signal<String?, NoError> in
                        guard let message = message else {
                            return .single(nil)
                        }
                        guard let file = message.media.first(where: { $0 is ElloAppMediaFile }) as? ElloAppMediaFile else {
                            return .single(nil)
                        }
...
//                let durationFont = Font.regular(floor(arguments.presentationData.fontSize.baseDisplaySize * 11.0 / 17.0))
                ...
                let titleCustomFont = UIFont.Custom.sfProDisplay(ofSize: 16, weight: .medium)
                let descriptionFont = UIFont.Custom.sfProDisplay(ofSize: 12, weight: .regular)
                let durationFont = UIFont.Custom.sfProDisplay(ofSize: 14, weight: .regular)
                
                let failedMessageState = arguments.message.flags.failedSend
                ...
                for attribute in arguments.file.attributes {
                    if case let .Audio(voice, duration, title, performer, waveform) = attribute {
                        ...
                        if voice {
                            ...
                            candidateTitleString = NSAttributedString(string: title ?? (arguments.file.fileName ?? "Unknown Track"), font: titleCustomFont, textColor: messageTheme.fileTitleColor)
                ...
                if let candidateTitleString = candidateTitleString {
                    ...
                    if failedMessageState {
                        titleString = NSAttributedString(string: arguments.file.fileName ?? "File", font: titleCustomFont, textColor: messageTheme.fileTitleFailedColor)
                    } else {
                        titleString = NSAttributedString(string: arguments.file.fileName ?? "File", font: titleCustomFont, textColor: messageTheme.fileTitleColor)
                    }
                }
                ...
                if let candidateDescriptionString = candidateDescriptionString {
                    ...
                    if failedMessageState {
                        descriptionString = NSAttributedString(string: descriptionText, font: descriptionFont, textColor: UIColor(rgb: 0x070708))
                    } else {
                ...
                let (titleLayout, titleApply) = titleAsyncLayout(TextNodeLayoutArguments(attributedString: titleString, 
                                                                                         backgroundColor: nil,
                                                                                         maximumNumberOfLines: hasThumbnail ? 2 : 1,
                ...
                let (descriptionLayout, descriptionApply) = descriptionAsyncLayout(TextNodeLayoutArguments(attributedString: descriptionString, 
                                                                                                           backgroundColor: nil,
                ...
                if let statusType = arguments.dateAndStatusType {
                    ...
                    for attribute in arguments.message.attributes {
                        if let attribute = attribute as? EditedMessageAttribute {
                            ...
                            if let channel = arguments.message.peers[arguments.message.id.peerId] as? ElloAppChannel, case .group = channel.info {
                ...
                if hasThumbnail {
                    ...
                    if failedMessageState {
                        fileIconImage = arguments.incoming ? principalGraphics.radialFailedIndicatiorFileIconIncoming : principalGraphics.radialFailedIndicatiorFileIconOutgoing
                    } else {
                ...
                return (minLayoutWidth, { boundingWidth in
                    ...
                    if isVoice {
                        ...
                        descriptionFrame = CGRect(origin: CGPoint(x: titleFrame.minX, y: titleFrame.maxY + 2.0), size: descriptionLayout.size)
                    }
                            ...
                            if isVoice {
                                ...
                                let _ = waveformView.update(
                                    ...
                                    component: AnyComponent(AudioWaveformComponent(
                                        ...
                                        gravityMultiplier: AudioWaveformComponent.GravityMultiplier.center,
    ...
    private func updateStatus(animated: Bool) {
        ...
        let failedMessageState = message.flags.failedSend
...
//                            let descriptionFont = Font.with(size: floor(presentationData.fontSize.baseDisplaySize * 13.0 / 17.0), design: .regular, weight: .regular, traits: [.monospacedNumbers])
                            let descriptionFont = UIFont.Custom.sfProDisplay(ofSize: 14, weight: .regular)
...
//                let durationFont = Font.regular(floor(presentationData.fontSize.baseDisplaySize * 11.0 / 17.0))
                let customFont = UIFont.Custom.sfProDisplay(ofSize: 12, weight: .bold)
                downloadingStrings = (durationString, durationString, customFont)
            }
        }
        ...
        if state != .none && self.statusNode == nil {
            ...
            if file.isMusic {
                if file.fileName?.lowercased().hasSuffix(".ogg") == true {
                    ...
                    image = playerAlbumArt(postbox: context.account.postbox, 
                                           engine: context.engine,
                                           fileReference: .message(message: MessageReference(message),
                                                                   media: file),
                                           albumArt: .init(thumbnailResource: ExternalMusicAlbumArtResource(file: .message(message: MessageReference(message), media: file),
                                                                                                            title: title ?? "",
                                                                                                            performer: performer ?? "",
                                                                                                            isThumbnail: true), 
                                                           fullSizeResource: ExternalMusicAlbumArtResource(file: .message(message: MessageReference(message), media: file),
                                                                                                           title: title ?? "",
                                                                                                           performer: performer ?? "",
                                                                                                           isThumbnail: false)),
                                           thumbnail: true, 
...
//            presentationData.theme.theme.chat.message.incoming.
            print("IS VIDEO ===================== \(presentationData.isPreview)")
            print("IS Picture ===================== \(file.isPicture)")
            
            switch state {

            case .none:
                self.statusNode?.backgroundNodeColor = UIColor(rgb: 0x0a49a5)
            case .download:
                
                self.statusNode?.backgroundNodeColor = messageTheme.fileNeedLoadColor.fillColor
                self.statusNode?.foregroundNodeColor = messageTheme.fileNeedLoadColor.foregroundColor
                
            case .play:
                if self.iconNode != nil {
                    self.statusNode?.backgroundNodeColor = presentationData.theme.theme.chat.message.mediaOverlayControlColors.fillColor
                    self.statusNode?.foregroundNodeColor = UIColor.white
                } else {
                    self.statusNode?.backgroundNodeColor = messageTheme.mediaActiveControlColor
                    self.statusNode?.foregroundNodeColor = .clear
                }
            case .pause:
                self.statusNode?.backgroundNodeColor = UIColor.black
                self.statusNode?.foregroundNodeColor = UIColor.white
            case .check(_):
                self.statusNode?.backgroundNodeColor = UIColor(rgb: 0x0a49a5)
                
            case .progress(value: _, cancelEnabled: _, appearance: _):
                
                if self.iconNode != nil {
                    self.statusNode?.backgroundNodeColor = UIColor(rgb: 0x0a49a5)
                } else {
                    self.statusNode?.backgroundNodeColor = UIColor(rgb: 0x000000)
                }
            ...
            case .customIcon(_):
                if self.file != nil {
                    self.statusNode?.backgroundNodeColor = UIColor(rgb: 0x219653)
                    self.statusNode?.foregroundNodeColor = UIColor.white
                }
                
                if failedMessageState {
                    self.statusNode?.backgroundNodeColor = UIColor(rgb: 0xEF4062)
                    self.statusNode?.foregroundNodeColor = UIColor.white
                }
            ...
            //TODO: turn off animation playback audio button
//            switch state {
//            case .pause:
//                self.playbackAudioLevelNode?.startAnimating()
//                self.statusNode?.backgroundNodeColor = .black
//            default:
//                self.playbackAudioLevelNode?.stopAnimating()
//                self.statusNode?.backgroundNodeColor = backgroundNodeColor
//            }
        }
