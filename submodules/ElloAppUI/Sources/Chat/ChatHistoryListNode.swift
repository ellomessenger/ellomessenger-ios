        ...
        if peerId.namespace == Namespaces.Peer.CloudUser || peerId.namespace == Namespaces.Peer.SecretChat {
            ...
            for entry in view.additionalData {
                if case let .peer(_, value) = entry {
                    if let channel = value as? ElloAppChannel, case .group = channel.info {
                        automaticMediaDownloadPeerType = .group
                    }
...
public final class ChatHistoryListNode: ListView, ChatHistoryNode {
    ...
    private let animationNode = AnimationNode(animation: "Preloader")
    ...
    var nextChannelToRead: (peer: EnginePeer, unreadCount: Int, location: ElloAppEngine.NextUnreadChannelLocation)?
    ...
    var openNextChannelToRead: ((EnginePeer, ElloAppEngine.NextUnreadChannelLocation) -> Void)?
    ...
    public init(context: AccountContext, 
                ...
                tagMask: MessageTags?, source: ChatHistoryListSource = .default,
                subject: ChatControllerSubject?,
                controllerInteraction: ChatControllerInteraction,
                selectedMessages: Signal<Set<MessageId>?, NoError>,
                mode: ChatHistoryListMode = .bubbles,
                messageTransitionNode: @escaping () -> ChatMessageTransitionNode? = { nil }) {
        ...
        self.currentPresentationData = ChatPresentationData(theme: ChatPresentationThemeData(theme: presentationData.theme, 
                                                            ...
                                                            largeEmoji: presentationData.largeEmoji,
        ...
        func showSpinner() {
            self.view.addSubview(self.animationNode.view)
            self.animationNode.view.autoresizingMask = [.flexibleWidth, .flexibleHeight]
            self.animationNode.view.frame = self.bounds
            self.animationNode.view.transform = CGAffineTransform(scaleX: -1, y: -1)
        ...
        if case let .custom(messages, at, _) = source {
            ...
            |> map { messages, _, hasMore in
                ...
                return (ChatHistoryViewUpdate.HistoryView(view: MessageHistoryView(tagMask: nil, 
                                                                                   namespaces: .all,
                                                                                   entries: messages.reversed().map {
                    MessageHistoryEntry(message: $0,
                                        isRead: false,
                                        location: nil,
                                        monthLocation: nil,
                                        attributes: MutableMessageHistoryEntryAttributes(authorIsContact: false)) },
                                                                                   ...
                                                                                   holeLater: false,
                                                                                   isLoading: false),
                                                          type: .Generic(type: version > 0 ? ViewUpdateType.Generic : ViewUpdateType.Initial),
                                                          scrollPosition: scrollPosition,
                                                          flashIndicators: false,
                                                          originalScrollPosition: nil,
                                                          initialData: ChatHistoryCombinedInitialData(initialData: nil,
            ...
            |> mapToSignal { location, ignoreMessagesInTimestampRange in
                return chatHistoryViewForLocation(location,
                                                  ...
                                                  scheduled: isScheduledMessages,
                                                  fixedCombinedReadStates: fixedCombinedReadStates.with { $0 },
                                                  tagMask: tagMask,
        ...
        if case let .peer(peerId) = chatLocation, peerId.namespace == Namespaces.Peer.CloudChannel {
            customChannelDiscussionReadState = context.engine.data.subscribe(
                ElloAppEngine.EngineData.Item.Peer.LinkedDiscussionPeerId(id: peerId),
            ...
            |> mapToSignal { discussionPeerId -> Signal<MessageId?, NoError> in
                ...
                return context.engine.data.subscribe(ElloAppEngine.EngineData.Item.Messages.PeerReadCounters(id: discussionPeerId))
        ...
        let defaultReaction = combineLatest(
            context.engine.data.subscribe(ElloAppEngine.EngineData.Item.Peer.Peer(id: context.account.peerId)),
        ...
        let accountPeer = context.engine.data.subscribe(ElloAppEngine.EngineData.Item.Peer.Peer(id: context.account.peerId))
        ...
        let historyViewTransitionDisposable = combineLatest(queue: messageViewQueue,
            ...
            case let .HistoryView(view, type, scrollPosition, flashIndicators, originalScrollPosition, data, id):
                ...
                let associatedData = extractAssociatedData(chatLocation: chatLocation, 
                                                           view: view,
                                                           automaticDownloadNetworkType: networkType,
                ...
                let processedView = ChatHistoryView(originalView: view, 
                                                    filteredEntries: filteredEntries,
                                                    associatedData: associatedData,
                                                    lastHeaderId: lastHeaderId,
                                                    id: id,
                                                    locationInput: update.2,
                                                    ignoreMessagesInTimestampRange: update.3)
                    ...
                    case let .index(index, position, _, _, highlight, displayLink):
                        if case .upperBound = index {
                            if let previous = previous, previous.filteredEntries.isEmpty {
                                updatedScrollPosition = .index(index: index, 
                                                               position: position,
                                                               directionHint: .Down,
                                                               animated: false, 
                                                               highlight: highlight,
...
//                Queue.mainQueue().async {
//                    print(view.isLoading)
//                    if view.isLoading { // case let .Initial(fadeIn) = reason {
//                        showSpinner()
//                    }
//                    else {
//                        hideSpinner()
...
//                }
                ...
                if let strongSelf = self, updatedScrollPosition == nil, case .InteractiveChanges = reason, case let .known(offset) = strongSelf.visibleContentOffset(), abs(offset) <= 0.9, let previous = previous {
                    ...
                    if fillsScreen, let firstNonAdIndex = firstNonAdIndex, previousNumAds == 0, updatedNumAds != 0 {
                        updatedScrollPosition = .index(index: .message(firstNonAdIndex), 
                                                       position: .top(0.0),
                                                       directionHint: .Up,
                                                       animated: false,
                                                       highlight: false,
                ...
                let rawTransition = preparedChatHistoryViewTransition(from: previous, 
                                                                      to: processedView,
                                                                      reason: reason,
                                                                      reverse: reverse,
                                                                      chatLocation: chatLocation,
                                                                      ...
                                                                      scrollAnimationCurve: scrollAnimationCurve,
                                                                      initialData: initialData?.initialData, 
                                                                      keyboardButtonsMessage: view.topTaggedMessages.first,
                                                                      cachedData: initialData?.cachedData,
                                                                      cachedDataMessages: initialData?.cachedDataMessages,
                                                                      readStateData: initialData?.readStateData,
                                                                      flashIndicators: flashIndicators,
                                                                      updatedMessageSelection: previousSelectedMessages != selectedMessages,
                                                                      messageTransitionNode: messageTransitionNode(),
                                                                      allUpdated: updateAllOnEachVersion || forceUpdateAll)
                
                var mappedTransition = mappedChatHistoryViewListTransition(context: context,
                                                                           chatLocation: chatLocation,
                                                                           associatedData: associatedData,
                                                                           controllerInteraction: controllerInteraction,
                                                                           mode: mode,
    ...
    private func processDisplayedItemRangeChanged(displayedRange: ListViewDisplayedItemRange, transactionState: ChatHistoryTransactionOpaqueState) {
                ...
                for i in (indexRange.0 ... indexRange.1) {
                    ...
                    case let .MessageEntry(message, _, _, _, _, _):
                        ...
                        for media in message.media {
                            if let _ = media as? ElloAppMediaUnsupported {
                                contentRequiredValidation = true
                            } else if message.flags.contains(.Incoming), let media = media as? ElloAppMediaMap, let liveBroadcastingTimeout = media.liveBroadcastingTimeout {
                                ...
                                if elloappFile.isAnimatedSticker, (message.id.peerId.namespace == Namespaces.Peer.SecretChat || !elloappFile.previewRepresentations.isEmpty), let size = elloappFile.size, size > 0 && size <= 128 * 1024 {
                                    if message.id.peerId.namespace == Namespaces.Peer.SecretChat {
                                        if elloappFile.fileId.namespace == Namespaces.Media.CloudFile {
                                            var isValidated = false
                                            attributes: for attribute in elloappFile.attributes {
                                ...
                                downloadableResourceIds.append((message.id, elloappFile.resource.id.stringRepresentation))
                            ...
                            } else if let invoice = media as? ElloAppMediaInvoice, let extendedMedia = invoice.extendedMedia, case .preview = extendedMedia {
                    ...
                    case let .MessageGroupEntry(_, messages, _):
                        for (message, _, _, _, _) in messages {
                            ...
                            for media in message.media {
                                if let elloappFile = media as? ElloAppMediaFile {
                                    downloadableResourceIds.append((message.id, elloappFile.resource.id.stringRepresentation))
                                } else if let image = media as? ElloAppMediaImage {
                                    if let representation = image.representations.last {
                                        downloadableResourceIds.append((message.id, representation.resource.id.stringRepresentation))
        ...
        if let loaded = displayedRange.loadedRange, 
           let firstEntry = historyView.filteredEntries.first,
           let lastEntry = historyView.filteredEntries.last {
                if loaded.firstIndex < 5 && historyView.originalView.laterId != nil {
                    let locationInput: ChatHistoryLocation = .Navigation(index: .message(lastEntry.index),
                                                                         anchorIndex: .message(lastEntry.index),
                                                                         count: historyMessageCount,
                                                                         highlight: false)
                    if self.chatHistoryLocationValue?.content != locationInput {
                        self.chatHistoryLocationValue = ChatHistoryLocationInput(content: locationInput,
                                                                                 id: self.takeNextHistoryLocationId())
                    }
                } else if loaded.firstIndex < 5, 
                          historyView.originalView.laterId == nil,
                          !historyView.originalView.holeLater,
                          let chatHistoryLocationValue = self.chatHistoryLocationValue,
                          !chatHistoryLocationValue.isAtUpperBound,
                          historyView.originalView.anchorIndex != .upperBound {
                    if self.chatHistoryLocationValue == historyView.locationInput {
                        self.chatHistoryLocationValue = ChatHistoryLocationInput(content: .Navigation(index: .upperBound,
                    ...
                    let locationInput: ChatHistoryLocation = .Navigation(index: .message(firstEntry.index), 
                                                                         anchorIndex: .message(firstEntry.index),
                                                                         count: historyMessageCount,
                                                                         highlight: false)
                    if self.chatHistoryLocationValue?.content != locationInput {
                        self.chatHistoryLocationValue = ChatHistoryLocationInput(content: locationInput,
    ...
    private func dequeueHistoryViewTransitions() {
        ...
        if let (layoutActionOnViewTransition, layoutCorrelationId) = self.layoutActionOnViewTransition {
            ...
            self.transaction(deleteIndices: mappedTransition.deleteItems,
                             insertIndicesAndItems: transition.insertItems,
                             updateIndicesAndItems: transition.updateItems,
                             options: mappedTransition.options,
                             scrollToItem: mappedTransition.scrollToItem,
                             updateSizeAndInsets: updateSizeAndInsets,
                             stationaryItemRange: mappedTransition.stationaryItemRange,
                             updateOpaqueState: ChatHistoryTransactionOpaqueState(historyView: transition.historyView),
                             completion: { result in
                completion(true, result)
            })
        } else {
            self.transaction(deleteIndices: transition.deleteItems, 
                             insertIndicesAndItems: transition.insertItems,
                             updateIndicesAndItems: transition.updateItems,
    ...
    private func displayUnseenReactionAnimations(messageIds: [MessageId], forceMapping: [MessageId: [ReactionsMessageAttribute.RecentPeer]] = [:]) -> [MessageId] {
        ...
        self.forEachVisibleItemNode { itemNode in
            ...
            case let .custom(fileId):
                if let itemFile = item.message.associatedMedia[MediaId(namespace: Namespaces.Media.CloudFile, id: fileId)] as? ElloAppMediaFile {
                    reactionItem = ReactionItem(
                    ...
                    if !previouslyToggled {
                        ...
                        if state.selecting != isSelected {
                            let messageIds = messages.filter { message -> Bool in
                                for media in message.media {
                                    if media is ElloAppMediaAction {
                                        return false
                                    }
                                }
                                return true
