...
func isPollEffectivelyClosed(message: Message, poll: ElloAppMediaPoll) -> Bool {
    if poll.isClosed {
        return true
...
private final class ChatMessagePollOptionNode: ASDisplayNode {
    ...
    var option: ElloAppMediaPollOption?
...
class ChatMessagePollBubbleContentNode: ChatMessageBubbleContentNode {
    ...
    private var poll: ElloAppMediaPoll?
    ...
    override func asyncLayoutContent() -> (_ item: ChatMessageBubbleContentItem, _ layoutConstants: ChatMessageItemLayoutConstants, _ preparePosition: ChatMessageBubblePreparePosition, _ messageSelection: Bool?, _ constrainedSize: CGSize) -> (ChatMessageBubbleContentProperties, CGSize?, CGFloat, (CGSize, ChatMessageBubbleContentPosition) -> (CGFloat, (CGFloat) -> (CGSize, (ListViewItemUpdateAnimation, Bool, ListViewItemApply?) -> Void))) {
        ...
        var previousPoll: ElloAppMediaPoll?
        if let item = self.item {
            for media in item.message.media {
                if let media = media as? ElloAppMediaPoll {
                    previousPoll = media
                }
            }
        }
        ...
        return { item, layoutConstants, _, _, _ in
            ...
            return (contentProperties, nil, CGFloat.greatestFiniteMagnitude, { constrainedSize, position in
                ...
                if let peer = item.message.peers[item.message.id.peerId] as? ElloAppUser, peer.botInfo != nil {
                    isBotChat = true
                }
                ...
                if let poll = poll {
                    ...
                    let voters: [ElloAppMediaPollOptionVoters]?
                    if isClosed {
                    ...
                    for i in 0 ..< poll.options.count {
                        ...
                        let makeLayout: (_ accountPeerId: PeerId, _ presentationData: ChatPresentationData, _ message: Message, _ poll: ElloAppMediaPoll, _ option: ElloAppMediaPollOption, _ optionResult: ChatMessagePollOptionResult?, _ constrainedWidth: CGFloat) -> (minimumWidth: CGFloat, layout: ((CGFloat) -> (CGSize, (Bool, Bool) -> ChatMessagePollOptionNode)))
    ...
    override func tapActionAtPoint(_ point: CGPoint, gesture: TapLongTapOrDoubleTapGesture, isEstimating: Bool) -> ChatMessageBubbleContentTapAction {
        ...
        if let (index, attributes) = self.textNode.attributesAtPoint(CGPoint(x: point.x - textNodeFrame.minX, y: point.y - textNodeFrame.minY)) {
            ...
            var isBotChat: Bool = false
            if let item = self.item, let peer = item.message.peers[item.message.id.peerId] as? ElloAppUser, peer.botInfo != nil {
                isBotChat = true
            }
...
private enum PeerAvatarReference: Equatable {
    ...
    case image(PeerReference, ElloAppMediaImageRepresentation)
