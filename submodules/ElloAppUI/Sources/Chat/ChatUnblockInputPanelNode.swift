...
final class ChatUnblockInputPanelNode: ChatInputPanelNode {
    ...
    init(theme: PresentationTheme, strings: PresentationStrings) {
        ...
        self.backgroundColor = theme.chat.inputPanel.panelControlBackgroundColor
    ...
    func updateThemeAndStrings(theme: PresentationTheme, strings: PresentationStrings) {
        if self.theme !== theme || self.strings !== strings {
            ...
            self.strings = strings
            self.backgroundColor = theme.chat.inputPanel.panelControlBackgroundColor
            self.button.setAttributedTitle(NSAttributedString(string: self.button.attributedTitle(for: [])?.string ?? "", font: Font.regular(17.0), textColor: theme.chat.inputPanel.panelControlTextColor), for: [])
        }
    }
    ...
    override func updateLayout(width: CGFloat, leftInset: CGFloat, rightInset: CGFloat, bottomInset: CGFloat, additionalSideInsets: UIEdgeInsets, maxHeight: CGFloat, isSecondary: Bool, transition: ContainedViewLayoutTransition, interfaceState: ChatPresentationInterfaceState, metrics: LayoutMetrics, isMediaInputExpanded: Bool) -> CGFloat {
        if self.presentationInterfaceState != interfaceState {
            ...
            if let user = interfaceState.renderedPeer?.peer as? ElloAppUser, user.botInfo != nil {
                string = NSAttributedString(string: strings.Bot_Unblock, font: Font.regular(17.0), textColor: theme.chat.inputPanel.panelControlTextColor)
            } else {
