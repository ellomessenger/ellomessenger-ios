...
class ChatMessageCallBubbleContentNode: ChatMessageBubbleContentNode {
    ...
    private let iconNode: ASImageNode                   // Arrow Incomin/Outgoing
        ...
        return { item, layoutConstants, _, _, _ in
            ...
            return (contentProperties, nil, CGFloat.greatestFiniteMagnitude, { constrainedSize, position in
                ...
                for media in item.message.media {
                    if let action = media as? ElloAppMediaAction, case let .phoneCall(_, discardReason, duration, isVideoValue) = action.action {
                ...
                if incoming {
                    backgroundButtonImage = PresentationResourcesChat.chatBubbleIncomingBackgroundImage(item.presentationData.theme.theme)
                } else {
                    backgroundButtonImage = PresentationResourcesChat.chatBubbleOutgoingBackgroundImage(item.presentationData.theme.theme)
                }
                ...
                var labelFrame = CGRect(origin: CGPoint(x: 24.0, y: 0.0), size: labelSize)
                ...
                boundingSize = CGSize(width: max(titleFrame.size.width, labelFrame.size.width + 14.0), height: 44.0)
                boundingSize.width += layoutConstants.text.bubbleInsets.left + layoutConstants.text.bubbleInsets.right
                boundingSize.height += layoutConstants.text.bubbleInsets.top + layoutConstants.text.bubbleInsets.bottom
                
                boundingSize.width += 124.0
                    ...
                    return (boundingSize, { [weak self] animation, _, _ in
                        if let strongSelf = self {
                            ...
                            if let callIcon = callIcon {
                                ...
                                strongSelf.iconNode.frame = CGRect(origin: CGPoint(x: titleFrame.minX, y: titleFrame.maxY),
                                                                   size: callIcon.size)
                            }
                            
                            if let buttonImage = buttonImage {
                                strongSelf.buttonNode.setImage(buttonImage, for: [])
                                if let backgroundButtonImage {
                                    strongSelf.buttonNode.setBackgroundImage(backgroundButtonImage, for: .normal)
                                    strongSelf.buttonNode.frame = CGRect(origin: CGPoint(x: boundingWidth - backgroundButtonImage.size.width - 16.0, y: 8.0), size: backgroundButtonImage.size)
                                } else {
                                    strongSelf.buttonNode.frame = CGRect(origin: CGPoint(x: boundingWidth - buttonImage.size.width - 16.0, y: 8.0), size: buttonImage.size)
                                }
                            }
                        }
                    })
    ...
    @objc func callButtonPressed() {
        if let item = self.item {
            var isVideo = false
            for media in item.message.media {
                if let action = media as? ElloAppMediaAction, case let .phoneCall(_, _, _, isVideoValue) = action.action {
                    isVideo = isVideoValue
                }
            }
