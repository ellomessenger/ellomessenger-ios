...
final class ChatMessageContextExtractedContentSource: ContextExtractedContentSource {
    ...
    var shouldBeDismissed: Signal<Bool, NoError> {
        ...
        return self.engine.data.subscribe(ElloAppEngine.EngineData.Item.Messages.Message(id: self.message.id))
        |> map { message -> Bool in
            if let _ = message {
                return false
            } else {
    ...
    init(chatNode: ChatControllerNode, engine: ElloAppEngine, message: Message, selectAll: Bool) {
...
final class ChatMessageReactionContextExtractedContentSource: ContextExtractedContentSource {
    ...
    init(chatNode: ChatControllerNode, engine: ElloAppEngine, message: Message, contentView: ContextExtractedContentContainingView) {
        self.chatNode = chatNode
        self.engine = engine
        self.message = message
