...
class ChatTextInputPanelNode: ChatInputPanelNode, ASEditableTextNodeDelegate {
    ...
    init(context: AccountContext, presentationInterfaceState: ChatPresentationInterfaceState, presentationContext: ChatPresentationContext?, presentController: @escaping (ViewController) -> Void) {
        ...
        self.actionButtons = ChatTextInputActionButtonsNode(presentationInterfaceState: presentationInterfaceState, 
        ...
        self.actionButtons.micButton.offsetRecordingControls = { [weak self] in
            if let strongSelf = self, let presentationInterfaceState = strongSelf.presentationInterfaceState {
                if let (width, leftInset, rightInset, bottomInset, additionalSideInsets, maxHeight, metrics, isSecondary, isMediaInputExpanded) = strongSelf.validLayout {
                    let _ = strongSelf.updateLayout(width: width,
                                                    leftInset: leftInset,
                                                    rightInset: rightInset,
                                                    bottomInset: bottomInset,
                                                    additionalSideInsets: additionalSideInsets,
                                                    maxHeight: maxHeight,
                                                    isSecondary: isSecondary,
                                                    transition: .immediate,
                                                    interfaceState: presentationInterfaceState,
                                                    metrics: metrics,
                                                    isMediaInputExpanded: isMediaInputExpanded)
                }
            }
        }
        ...
        //TODO: offed switcher to video recording
    ...
    override func updateAbsoluteRect(_ rect: CGRect, within containerSize: CGSize, transition: ContainedViewLayoutTransition) {
        ...
        if !self.actionButtons.frame.width.isZero {
            self.actionButtons.updateAbsoluteRect(CGRect(origin: rect.origin.offsetBy(dx: self.actionButtons.frame.minX,
                                                                                      dy: self.actionButtons.frame.minY),
                                                         size: self.actionButtons.frame.size),
    ...
    override func updateLayout(width: CGFloat,
                               ...
                               isMediaInputExpanded: Bool) -> CGFloat {
            ...
            if let peer = interfaceState.renderedPeer?.peer, previousState?.renderedPeer?.peer == nil || !peer.isEqual(previousState!.renderedPeer!.peer!) || previousState?.interfaceState.silentPosting != interfaceState.interfaceState.silentPosting || themeUpdated || !self.initializedPlaceholder || previousState?.keyboardButtonsMessage?.id != interfaceState.keyboardButtonsMessage?.id || previousState?.keyboardButtonsMessage?.visibleReplyMarkupPlaceholder != interfaceState.keyboardButtonsMessage?.visibleReplyMarkupPlaceholder || dismissedButtonMessageUpdated || replyMessageUpdated {
                ...
                if let channel = peer as? ElloAppChannel, case .broadcast = channel.info {
                    if interfaceState.interfaceState.silentPosting {
                        ...
                        placeholder = interfaceState.strings.Conversation_InputTextPublicationPlaceholder
                    }
                } else if let channel = peer as? ElloAppChannel, case .group = channel.info, channel.hasPermission(.canBeAnonymous) {
                    ...
                    if peer.id.isAIBots {
                        placeholder = interfaceState.strings.Conversation_AskAIBot
                    } else {
                ...
                if self.currentPlaceholder != placeholder || themeUpdated {
                    ...
                    let baseFontSize = peer.id.isAIBots ? 15 : max(minInputFontSize, interfaceState.fontSize.baseDisplaySize)
                    ...
                    self.textPlaceholderNode.frame = CGRect(origin: CGPoint(x: self.textPlaceholderNode.frame.origin.x , y:  self.view.center.y - placeholderSize.height / 2), size: placeholderSize)
                }
                
                self.actionButtons.sendButtonLongPressEnabled = !isScheduledMessages && !peer.id.isAIBots
            }
        ...
        } else if let peer = interfaceState.renderedPeer?.peer as? ElloAppUser, let _ = peer.botInfo, shouldDisplayMenuButton && interfaceState.editMessageState == nil {
            hasMenuButton = true
        ...
        let audioRecordingContainerHeight: CGFloat = 36
        ...
        if mediaRecordingState != nil || interfaceState.recordedMediaPreview != nil {
            ...
            audioRecordingInfoContainerNode.backgroundColor = .white
            audioRecordingInfoContainerNode.layer.cornerRadius = 20
            audioRecordingInfoContainerNode.frame = CGRect(
                origin: CGPoint(
                    x: leftInset + 13.0 - UIScreenPixel,
                    y: 7.0 - UIScreenPixel
                ),
                size: CGSize(width: baseWidth * 0.744, height: audioRecordingContainerHeight)
            )
            ...
            if let currentAudioRecordingTimeNode = self.audioRecordingTimeNode {
                ...
                self.clippingNode.addSubnode(audioRecordingTimeNode)
            ...
            if let currentAudioRecordingCancelIndicator = self.audioRecordingCancelIndicator {
                ...
                self.clippingNode.addSubnode(audioRecordingCancelIndicator)
            }
...
//                    y: panelHeight - minimalHeight + floor(audioRecordingInfoContainerNode.frame.midY - audioRecordingCancelIndicator.bounds.size.height / 2.0)),
//                size: audioRecordingCancelIndicator.bounds.size)
//            audioRecordingCancelIndicator.frame = audioRecordingCancelIndicatorFrame
            
            let leftPosition = leftInset + 13.0 - UIScreenPixel + audioRecordingInfoContainerNode.frame.width - 15.0
            let audioRecordingCancelIndicatorFrame = CGRect(
                origin: CGPoint(
                    x: leftPosition - audioRecordingCancelIndicator.bounds.size.width,
                    y: audioRecordingInfoContainerNode.frame.midY - audioRecordingCancelIndicator.bounds.size.height / 2),
                size: audioRecordingCancelIndicator.bounds.size)
            audioRecordingCancelIndicator.frame = audioRecordingCancelIndicatorFrame
            ...
            audioRecordingTimeNode.frame = CGRect(origin: CGPoint(x: leftInset + 13.0 + 2.0 + 8.0 + audioRecordingContainerHeight - UIScreenPixel, y: audioRecordingInfoContainerNode.frame.midY - audioRecordingTimeSize.height / 2), size: audioRecordingTimeSize)
            ...
            if let currentAudioRecordingDotNode = self.audioRecordingDotNode, !currentAudioRecordingDotNode.didPlay {
                ...
                audioRecordingDotNode = AnimationNode(animation: "BinRed_new")
            ...
            // Need add Color to Theme
            audioRecordingDotNode.frame = CGRect(origin: CGPoint(x: leftInset + 13.0 + 2.0 - UIScreenPixel, y: audioRecordingInfoContainerNode.frame.midY - (audioRecordingContainerHeight - 4) / 2), size: CGSize(width: audioRecordingContainerHeight - 4, height: audioRecordingContainerHeight - 4))
            audioRecordingDotNode.backgroundColor = UIColor(hexString: "#EF4062")?.withAlphaComponent(0.2)
            audioRecordingDotNode.layer.cornerRadius = (audioRecordingContainerHeight - 4) / 2
            ...
            if let audioRecordingDotNode = self.audioRecordingDotNode {
                ...
                if self.isMediaDeleted {
                    if self.prevInputPanelNode is ChatRecordingPreviewInputPanelNode {
                        ...
                        audioRecordingDotNode.backgroundColor = .clear
            ...
            if let audioRecordingTimeNode = self.audioRecordingTimeNode {
                ...
                transition.updateTransformScale(node: audioRecordingTimeNode, scale: 0)
        ...
        let textInputFrame = CGRect(x: leftInset + textFieldInsets.left, 
                                    y: textFieldInsets.top,
                                    width: baseWidth - textFieldInsets.left - textFieldInsets.right + textInputBackgroundWidthOffset,
        ...
        if !interfaceState.voiceMessagesAvailable {
            ...
            mediaInputDisabled = true
        } else if let channel = interfaceState.renderedPeer?.peer as? ElloAppChannel, channel.hasBannedPermission(.banSendMedia) != nil {
            mediaInputDisabled = true
        } else if let group = interfaceState.renderedPeer?.peer as? ElloAppGroup, group.hasBannedPermission(.banSendMedia) {
            mediaInputDisabled = true
        } else {
    ...
    private final class CurrentEmojiSuggestion {
        ...
        var value: [ElloAppMediaFile]?
        
        init(localPosition: CGPoint, position: EmojiSuggestionPosition, disposable: MetaDisposable, value: [ElloAppMediaFile]?) {
    ...
    func editableTextNodeDidFinishEditing(_ editableTextNode: ASEditableTextNode) {
        ...
        if let presentationInterfaceState = self.presentationInterfaceState {
            if let peer = presentationInterfaceState.renderedPeer?.peer as? ElloAppUser, peer.botInfo != nil, presentationInterfaceState.keyboardButtonsMessage != nil {
