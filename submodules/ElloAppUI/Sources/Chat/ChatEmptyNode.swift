...
private final class ChatEmptyNodeRegularChatContent: ASDisplayNode, ChatEmptyNodeContent {
    ...
    override init() {
        ...
        self.animationNode.setAnimation(name: "HelloPanda")
    ...
    func updateLayout(interfaceState: ChatPresentationInterfaceState, size: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        contentHeight += textSize.height + stickerSpacing + stickerSize.height
        
        let contentRect = CGRect(origin: CGPoint(x: insets.left, y: insets.top), size: CGSize(width: contentWidth, height: contentHeight))
        
        let textFrame = CGRect(origin: CGPoint(x: contentRect.minX + floor((contentRect.width - textSize.width) / 2.0), y: contentRect.minY), size: textSize)
        ...
        transition.updateFrame(node: self.animationNode, frame: stickerFrame)
...
final class ChatEmptyNodeGreetingChatContent: ASDisplayNode, ChatEmptyNodeContent {
...
final class ChatEmptyNodeNearbyChatContent: ASDisplayNode, ChatEmptyNodeStickerContentNode, ChatEmptyNodeContent, UIGestureRecognizerDelegate {
    ...
    func updateLayout(interfaceState: ChatPresentationInterfaceState, size: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        if let item = self.stickerItem {
            ...
            let sticker: Signal<ElloAppMediaFile?, NoError>
...
private final class ChatEmptyNodeSecretChatContent: ASDisplayNode, ChatEmptyNodeContent {
    ...
    func updateLayout(interfaceState: ChatPresentationInterfaceState, size: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        if self.currentTheme !== interfaceState.theme || self.currentStrings !== interfaceState.strings {
            ...
            var incoming = false
            if let renderedPeer = interfaceState.renderedPeer {
                if let chatPeer = renderedPeer.peers[renderedPeer.peerId] as? ElloAppSecretChat {
...
private final class ChatEmptyNodeGroupChatContent: ASDisplayNode, ChatEmptyNodeContent {
    ...
    override init() { 
        self.animationNode = AnimationNode(animation: "HelloPanda")
        self.animationNode.loop()
    ...
    func updateLayout(interfaceState: ChatPresentationInterfaceState, size: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        let animationSpacing: CGFloat = 12.0
        var contentWidth: CGFloat = 100.0
        var contentHeight: CGFloat = 120.0
        contentHeight += animationSpacing
        ...
        var animationAreaHeight: CGFloat = 0.0
        let size = CGSize(width: 120, height: 120)
        animationAreaHeight += size.height
        animationAreaHeight += animationSpacing
        transition.updateFrame(node: self.animationNode, frame: CGRect(origin: CGPoint(x: insets.left + floor((contentWidth - size.width) / 2.0), y: insets.top), size: size))
        
        let contentRect = CGRect(origin: CGPoint(x: insets.left, y: insets.top + animationAreaHeight), size: CGSize(width: contentWidth, height: contentHeight))
...
private final class ChatEmptyNodeCloudChatContent: ASDisplayNode, ChatEmptyNodeContent {
    ...
    override init() {
        self.animationNode = AnimationNode(animation: "ShareProgress")
        self.animationNode.loop()
        self.animationNode.play()
...
private final class ChatEmptyNodeAIBotContent: ASDisplayNode, ChatEmptyNodeContent {
    private let animationNode: AnimationNode
    private let titleNode: ImmediateTextNode
    private let textNode: ImmediateTextNode
    ...
    override init() {
        self.animationNode = AnimationNode(animation: "Hello_AI")
        self.animationNode.loop()
        self.animationNode.play()
...
//        self.backgroundColor = UIColor(hex: 0x27AE60)
    ...
    func updateLayout(interfaceState: ChatPresentationInterfaceState, size: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        if self.currentTheme !== interfaceState.theme || self.currentStrings !== interfaceState.strings {
            ...
            let titleString = interfaceState.strings.Conversation_AIBotInfo_Title
            self.titleNode.attributedText = NSAttributedString(string: titleString, font: titleFont, textColor: .white)
            
            let textString = interfaceState.strings.Conversation_AIBotInfo_Description
            self.textNode.attributedText = NSAttributedString(string: textString, font: Font.regular(12.0), textColor: .white)
        }
        
        let insets = UIEdgeInsets(top: 15.0, left: 15.0, bottom: 15.0, right: 15.0)
        let contentSpacing: CGFloat = 8.0
        var contentSize = CGSize(width: 235.0, height: 0.0)
        ...
        let titleFrame = CGRect(
            origin: CGPoint(
                x: contentRect.minX + floor((contentRect.width - titleSize.width) / 2.0),
                y: contentRect.minY),
            size: titleSize)
        transition.updateFrame(node: self.titleNode, frame: titleFrame)
        ...
        let textFrame = CGRect(
            origin: CGPoint(
                x: contentRect.minX + floor((contentRect.width - textSize.width) / 2.0),
        ...
        let animationFrame = CGRect(
            origin: CGPoint(
                x: contentRect.center.x - floor(animationSize.width / 2.0),
...
private final class ChatEmptyNodeNotificationsContent: ASDisplayNode, ChatEmptyNodeContent {
    private let titleNode: ImmediateTextNode
    private let textNode: ImmediateTextNode
    ...
    func updateLayout(interfaceState: ChatPresentationInterfaceState, size: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        if self.currentTheme !== interfaceState.theme || self.currentStrings !== interfaceState.strings {
            ...
            let titleString = interfaceState.strings.Conversation_Notifications_Title
            self.titleNode.attributedText = NSAttributedString(string: titleString, font: titleFont, textColor: .white)
            
            let textString = interfaceState.strings.Conversation_Notifications_Description
            self.textNode.attributedText = NSAttributedString(string: textString, font: Font.regular(12.0), textColor: .white)
        }
        ...
        contentSize.height = titleSize.height + contentSpacing + textSize.height + contentSpacing
    ...
    func updateLayout(interfaceState: ChatPresentationInterfaceState, emptyType: ChatHistoryNodeLoadState.EmptyType, loadingNode: ChatLoadingNode?, backgroundNode: WallpaperBackgroundNode?, size: CGSize, insets: UIEdgeInsets, transition: ContainedViewLayoutTransition) {
        ...
        if case .replyThread = interfaceState.chatLocation {
            ...
            if peer.id == self.context.account.peerId {
                ...
                contentType = .secret
            } else if let group = peer as? ElloAppGroup, case .creator = group.role {
                contentType = .group
            } else if let channel = peer as? ElloAppChannel, case .group = channel.info, channel.flags.contains(.isCreator) && !channel.flags.contains(.isGigagroup) {
                contentType = .group
                ...
                if peer.id.isAIBots {
                    contentType = .aiBot
                } else if peer.id.isNotificationsBotID {
                    contentType = .systemNotifications
                } else if peer.isDeleted || peer.botInfo != nil || peer.flags.contains(.isSupport) || peer.isScam || interfaceState.peerIsBlocked {
                    contentType = .regular
        ...
        if self.content?.0 != contentType {
            ...
            case .group:
                ...
                self.backgroundNode.updateColor(color: UIColor(hex: 0x0664A8, alpha: 0.3), enableBlur: false, transition: .immediate)
            case .cloud:
            ...
            case .aiBot:
                node = ChatEmptyNodeAIBotContent()
                self.backgroundNode.updateColor(color: UIColor(hex: 0x0A49A5), enableBlur: false, transition: .immediate)
            case .systemNotifications:
                node = ChatEmptyNodeNotificationsContent()
            }
