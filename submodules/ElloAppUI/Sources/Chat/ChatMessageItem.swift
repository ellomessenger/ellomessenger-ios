...
private func messagesShouldBeMerged(accountPeerId: PeerId, _ lhs: Message, _ rhs: Message) -> ChatMessageMerge {
    ...
    if abs(lhsEffectiveTimestamp - rhsEffectiveTimestamp) < Int32(10 * 60) && sameAuthor {
        if let channel = lhs.peers[lhs.id.peerId] as? ElloAppChannel, case .group = channel.info, lhsEffectiveAuthor?.id == channel.id, !lhs.effectivelyIncoming(accountPeerId) {
            return .none
        }
    ...
    public func nodeConfiguredForParams(async: @escaping (@escaping () -> Void) -> Void, params: ListViewItemLayoutParams, synchronousLoads: Bool, previousItem: ListViewItem?, nextItem: ListViewItem?, completion: @escaping (ListViewItemNode, @escaping () -> (Signal<Void, NoError>?, (ListViewItemApply) -> Void)) -> Void) {
        ...
        loop: for media in self.message.media {
            if let elloappFile = media as? ElloAppMediaFile {
                if elloappFile.isVideoSticker {
                    viewClassName = ChatMessageAnimatedStickerItemNode.self
                    break loop
                }
                if elloappFile.isAnimatedSticker, let size = elloappFile.size, size > 0 && size <= 128 * 1024 {
                    if self.message.id.peerId.namespace == Namespaces.Peer.SecretChat {
                        if elloappFile.fileId.namespace == Namespaces.Media.CloudFile {
                            ...
                            for attribute in elloappFile.attributes {
                            ...
                            inner: for attribute in elloappFile.attributes {
                ...
                for attribute in elloappFile.attributes {
                    switch attribute {
                        case .Sticker:
                            if let size = elloappFile.size, size > 0 && size <= 512 * 1024 {
            ...
            } else if media is ElloAppMediaAction {
                viewClassName = ChatMessageBubbleItemNode.self
            } else if media is ElloAppMediaExpiredContent {
                viewClassName = ChatMessageBubbleItemNode.self
            } else if media is ElloAppMediaDice {
                viewClassName = ChatMessageAnimatedStickerItemNode.self
            }
        }
