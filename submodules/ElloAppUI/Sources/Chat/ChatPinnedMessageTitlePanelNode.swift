...
final class ChatPinnedMessageTitlePanelNode: ChatTitleAccessoryPanelNode {
    ...
    init(context: AccountContext, animationCache: AnimationCache?, animationRenderer: MultiAnimationRenderer?) {
        ...
        self.clippingContainer.addSubnode(self.lineNode)
        ...
        self.contextContainer.addSubnode(self.pinIconNode)
    ...
    override func updateLayout(width: CGFloat, leftInset: CGFloat, rightInset: CGFloat, transition: ContainedViewLayoutTransition, interfaceState: ChatPresentationInterfaceState) -> LayoutResult {
        ...
        let leftInsetsPinIcon: CGFloat = 19.0
        ...
        if self.theme !== interfaceState.theme {
            ...
            self.pinIconNode.image = PresentationResourcesChat.chatInputPanelPinnedIconImage(interfaceState.theme)
        ...
        let pinIconNodeSize = self.pinIconNode.measure(CGSize(width: 100.0, height: 100.0))
        self.pinIconNode.frame = CGRect(origin: CGPoint(x: leftInsetsPinIcon, y: (contextContainer.frame.height - pinIconNode.frame.height) / 2), size: pinIconNodeSize)
        
        let closeButtonSize = self.closeButton.measure(CGSize(width: 100.0, height: 100.0))
        self.closeButton.frame = CGRect(origin: CGPoint(x: 0, y: (buttonsContainer.frame.height - closeButton.frame.height) / 2), size: closeButtonSize)
        ...
        transition.updateFrame(node: self.pinIconNode, frame: CGRect(origin: CGPoint(x: leftInsetsPinIcon, y: (contextContainer.frame.height - pinIconNode.frame.height) / 2), size: pinIconNodeSize))
               
        transition.updateFrame(node: self.closeButton, frame: CGRect(origin: CGPoint(x: buttonsContainerSize.width - closeButtonSize.width + 1.0, y: (buttonsContainer.frame.height - closeButton.frame.height) / 2), size: closeButtonSize))
        ...
        self.clippingContainer.frame = CGRect(origin: CGPoint(x: leftInsetsPinIcon * 2 + pinIconNode.frame.width, y: 0), size: CGSize(width: width, height: panelHeight))
        ...
        targetQueue.async { [weak self] in
            ...
            if pinnedMessage.totalCount == 2 {
                if pinnedMessage.index == 0 {
                    ...
                    titleStrings.append(.text(0, NSAttributedString(string: "\(strings.Conversation_PinnedMessage) ", font: UIFont.Custom.sfProDisplay(ofSize: 16, weight: .semibold), textColor: theme.chat.inputPanel.panelControlAccentColor)))
                }
            ...
            if isReplyThread {
                ...
                for media in message.media {
                    if let media = media as? ElloAppMediaInvoice {
            ...
            if mediaUpdated {
                if let updatedMediaReference = updatedMediaReference, imageDimensions != nil {
                    if let imageReference = updatedMediaReference.concrete(ElloAppMediaImage.self) {
                        updateImageSignal = chatMessagePhotoThumbnail(account: context.account, photoReference: imageReference)
                    } else if let fileReference = updatedMediaReference.concrete(ElloAppMediaFile.self) {
            ...
            if isText {
                ...
                messageText = NSAttributedString(string: foldLineBreaks(textString.string), font: textFont, textColor: message.media.isEmpty || message.media.first is ElloAppMediaWebpage ? theme.chat.inputPanel.primaryTextColor : theme.chat.inputPanel.secondaryTextColor)
            }
            

            let textConstrainedSize = CGSize(width: width - textLineInset - contentLeftInset - rightInset - textRightInset - 62.0, height: CGFloat.greatestFiniteMagnitude)
            ...
            Queue.mainQueue().async {
                if let strongSelf = self {
                    ...
                    let textFrame = CGRect(origin: CGPoint(x: 0.0, y: 23.0), size: CGSize(width: textLayout.size.width, height: textLayout.size.height))
                    ...
                    let lineFrame = CGRect(origin: CGPoint(x: 0.0, y: 0.0), size: CGSize(width: 2.0, height: panelHeight))
