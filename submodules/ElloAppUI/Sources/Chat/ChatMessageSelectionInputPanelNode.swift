...
final class ChatMessageSelectionInputPanelNode: ChatInputPanelNode {
    ...
    @objc func forwardButtonPressed() {
        if let _ = self.presentationInterfaceState?.renderedPeer?.peer as? ElloAppSecretChat {
            return
        }
    ...
    override func updateLayout(width: CGFloat, leftInset: CGFloat, rightInset: CGFloat, bottomInset: CGFloat, additionalSideInsets: UIEdgeInsets, maxHeight: CGFloat, isSecondary: Bool, transition: ContainedViewLayoutTransition, interfaceState: ChatPresentationInterfaceState, metrics: LayoutMetrics, isMediaInputExpanded: Bool) -> CGFloat {
        ...
        if let actions = self.actions {
            ...
            self.deleteButton.isEnabled = false
            self.deleteButton.isHidden =  true//self.peerMedia
            self.reportButton.isEnabled = false
        ...
        if self.reportButton.isHidden || (self.peerMedia && self.deleteButton.isHidden && self.reportButton.isHidden) {
            if let peer = interfaceState.renderedPeer?.peer as? ElloAppChannel, case .broadcast = peer.info {
