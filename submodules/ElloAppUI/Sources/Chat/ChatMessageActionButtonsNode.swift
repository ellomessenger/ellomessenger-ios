...
private final class ChatMessageActionButtonNode: ASDisplayNode {
    ...
    var normalBGColor: UIColor? {
        didSet {
            backgroundBlurNode.backgroundColor = normalBGColor
            backgroundContent?.backgroundColor = normalBGColor
        }
    }
    ...
    override func didLoad() {
        ...
        buttonView.highligthedChanged = { [weak self] highlighted in
            if let strongSelf = self {
                strongSelf.isHighlight = highlighted
                if highlighted {
                    if let highlightBGColor = strongSelf.highlightBGColor {
                        strongSelf.backgroundBlurNode.backgroundColor = highlightBGColor
                        strongSelf.backgroundContent?.backgroundColor = highlightBGColor
                        return
                    }
                    ...
                    if let normalBGColor = strongSelf.normalBGColor {
                        strongSelf.backgroundBlurNode.backgroundColor = normalBGColor
                        strongSelf.backgroundContent?.backgroundColor = normalBGColor
                        return
                    }
    ...
    class func asyncLayout(_ maybeNode: ChatMessageActionButtonNode?) -> (_ context: AccountContext, 
                                                                          _ theme: ChatPresentationThemeData,
                                                                          _ bubbleCorners: PresentationChatBubbleCorners,
                                                                          _ strings: PresentationStrings,
                                                                          _ backgroundNode: WallpaperBackgroundNode?,
                                                                          _ message: Message,
                                                                          _ button: ReplyMarkupButton,
                                                                          _ constrainedWidth: CGFloat,
                                                                          _ position: MessageBubbleActionButtonPosition) -> (minimumWidth: CGFloat, layout: ((CGFloat) -> (CGSize, (ListViewItemUpdateAnimation) -> ChatMessageActionButtonNode))) {
...
final class ChatMessageActionButtonsNode: ASDisplayNode {
    ...
    public func updateButtons(backgroundColor: UIColor, backgroundColorHighlighted: UIColor, textColor: UIColor, textColorHighlighted: UIColor) {
        buttonNodes.forEach { item in
            item.highlightBGColor = backgroundColorHighlighted
            item.normalBGColor = backgroundColor
            item.highlightTextColor = textColorHighlighted
            item.normalTextColor = textColor
        }
    }
}
