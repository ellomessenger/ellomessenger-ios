...
class ChatMessageTextBubbleContentNode: ChatMessageBubbleContentNode {
        ...
        return { item, layoutConstants, _, _, _ in
            ...
            return (contentProperties, nil, CGFloat.greatestFiniteMagnitude, { constrainedSize, position in
                ...
                for media in item.message.media {
                    if let webpage = media as? ElloAppMediaWebpage, case let .Loaded(content) = webpage.content, content.type == "elloapp_background" || content.type == "elloapp_theme" {
                ...
                for media in item.message.media {
                    if let file = media as? ElloAppMediaFile, let duration = file.duration {
                    ...
                    } else if media is ElloAppMediaUnsupported {
                ...
                if let entities = entities {
                    ...
                    for entity in entities.sorted(by: { $0.range.lowerBound > $1.range.lowerBound }) {
                        ...
                        updatedAttributes[ChatTextInputAttributes.customEmoji] = ChatTextInputTextCustomEmojiAttribute(interactivelySelectedFromPackId: nil, fileId: fileId, file: item.message.associatedMedia[MediaId(namespace: Namespaces.Media.CloudFile, id: fileId)] as? ElloAppMediaFile)
    ...
    override func tapActionAtPoint(_ point: CGPoint, gesture: TapLongTapOrDoubleTapGesture, isEstimating: Bool) -> ChatMessageBubbleContentTapAction {
        ...
        if let (index, attributes) = self.textNode.textNode.attributesAtPoint(CGPoint(x: point.x - textNodeFrame.minX, y: point.y - textNodeFrame.minY)) {
            if let _ = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.Spoiler)], !(self.dustNode?.isRevealed ?? true)  {
                return .none
            } else if let url = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL)] as? String {
                var concealed = true
                if let (attributeText, fullText) = self.textNode.textNode.attributeSubstring(name: ElloAppTextAttributes.URL, index: index) {
            ...
            } else if let bankCard = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.BankCard)] as? String {
                return .bankCard(bankCard)
            } else if let pre = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.Pre)] as? String {
                return .copy(pre)
            } else {
            ...
            if let point = point {
                ...
                if let (index, attributes) = self.textNode.textNode.attributesAtPoint(CGPoint(x: point.x - textNodeFrame.minX, y: point.y - textNodeFrame.minY)) {
                    let possibleNames: [String] = [
                        ...
                        ElloAppTextAttributes.BankCard
                    ]
                    ...
                    if let _ = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.Spoiler)] {
                        spoilerRects = self.textNode.textNode.attributeRects(name: ElloAppTextAttributes.Spoiler, at: index)
                    }
                }
            }
    ...
    override func peekPreviewContent(at point: CGPoint) -> (Message, ChatMessagePeekPreviewContent)? {
        if let item = self.item {
            let textNodeFrame = self.textNode.textNode.frame
            if let (index, attributes) = self.textNode.textNode.attributesAtPoint(CGPoint(x: point.x - textNodeFrame.minX, y: point.y - textNodeFrame.minY)) {
                if let value = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL)] as? String {
                    if let rects = self.textNode.textNode.attributeRects(name: ElloAppTextAttributes.URL, at: index), !rects.isEmpty {
                        var rect = rects[0]
