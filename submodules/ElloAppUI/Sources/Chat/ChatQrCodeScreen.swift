...
private struct ThemeSettingsThemeEntry: Comparable, Identifiable {
    let index: Int
    let emoticon: String?
    let emojiFile: ElloAppMediaFile?
    let themeReference: PresentationThemeReference?
    let nightMode: Bool
    var selected: Bool
    let theme: PresentationTheme
    let strings: PresentationStrings
    let wallpaper: ElloAppWallpaper?
    let message: Bool
...
private class ThemeSettingsThemeIconItem: ListViewItem {
    ...
    public init(context: AccountContext, emoticon: String?, emojiFile: ElloAppMediaFile?, themeReference: PresentationThemeReference?, nightMode: Bool, selected: Bool, theme: PresentationTheme, strings: PresentationStrings, wallpaper: ElloAppWallpaper?, message: Bool, action: @escaping (String?) -> Void) {
...
final class ChatQrCodeScreen: ViewController {
    ...
    enum Subject {
        /// Peer - peer, String - link
        case peer(Peer, String)
        ...
        var fileName: String {
            ...
            case let .peer(peer, _):
                if let addressName = peer.addressName, !addressName.isEmpty {
                    return "Ello-\(peer.addressName ?? "")"
                } else if let peer = peer as? ElloAppUser {
                    return "Ello-\(peer.phone ?? "")"
                } else {
                    return "Ello-\(Int32.random(in: 0 ..< Int32.max))"
                }
            case let .messages(messages):
                if let message = messages.first, let chatPeer = message.peers[message.id.peerId] as? ElloAppChannel, message.id.namespace == Namespaces.Message.Cloud, let addressName = chatPeer.addressName, !addressName.isEmpty {
                    return "Ello-\(addressName)-\(message.id.id)"
                } else if let message = messages.first {
                    return "Ello-\(message.id.id)"
                } else {
                    return "Ello_message"
                }
            }
...
private class ChatQrCodeScreenNode: ViewControllerTracingNode, UIScrollViewDelegate {
    ...
    private var themes: [ElloAppTheme] = []
    ...
    init(context: AccountContext, presentationData: PresentationData, controller: ChatQrCodeScreen) {
        ...
        case let .peer(peer, link):
            self.contentNode = QrContentNode(context: context, peer: peer, isStatic: true, link: link)
        case let .messages(messages):
        ...
        self.segmentedNode = SegmentedControlNode(theme: SegmentedControlTheme(theme: presentationData.theme), 
                                                  items: [SegmentedControlItem(title: "Video"), SegmentedControlItem(title: "Image")],
                                                  selectedIndex: self.contentNode.hasVideo ? 0 : 1)
        ...
        self.backButton.setImage(UIImage(bundleImageName: "Share/BackButton"), for: .normal)
        ...
        self.animationNode = AnimationNode(animation: self.isDarkAppearance ? "anim_sun_reverse" : "anim_sun", 
                                           colors: iconColors(theme: self.presentationData.theme),
                                           scale: 1.0)
        self.animationNode.isUserInteractionEnabled = false
        
        self.doneButton = SolidRoundedButtonNode(title: self.presentationData.strings.Share_ShareMessage,
                                                 theme: SolidRoundedButtonTheme(theme: self.presentationData.theme),
                                                 height: 45.0,
                                                 cornerRadius: 10.0,
                                                 gloss: false)
...
//        self.contentContainerNode.addSubnode(self.segmentedNode)
        ...
        self.contentNode.addSubnode(self.backButton)

        // Hide Day/Night button
...
//        self.topContentContainerNode.addSubnode(self.cancelButton)
        ...
        self.backButton.addTarget(self, action: #selector(self.cancelButtonPressed), forControlEvents: .touchUpInside)
        ...
        if case let .peer(peer, _) = controller.subject, peer.id != self.context.account.peerId {
            let themeEmoticon = self.context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.ThemeEmoticon(id: peer.id))
        ...
        self.disposable.set(combineLatest(queue: Queue.mainQueue(), animatedEmojiStickers, initiallySelectedEmoticon, self.context.engine.themes.getChatThemes(accountManager: self.context.sharedContext.accountManager), self.selectedEmoticonPromise.get(), self.isDarkAppearancePromise.get()).start(next: { [weak self] animatedEmojiStickers, initiallySelectedEmoticon, themes, selectedEmoticon, isDarkAppearance in
            ...
            let defaultWallpaper: ElloAppWallpaper?
            if isDarkAppearance {
            ...
            if selectedEmoticon == defaultEmoticon {
                ...
                if isDarkAppearance {
                    ...
                    wallpaper = .gradient(ElloAppWallpaper.Gradient(id: nil, colors: [0x3A85F3, 0x6CA5F9], settings: WallpaperSettings())) // presentationTheme.chat.defaultWallpaper
                }
            ...
            strongSelf.contentNode.update(theme: previewTheme, 
                                          wallpaper: wallpaper,
    ...
    private func dequeueTransition() {
        ...
        self.listNode.transaction(deleteIndices: transition.deletions,
    ...
    func updatePresentationData(_ presentationData: PresentationData) {
        ...
        self.titleNode.attributedText = NSAttributedString(string: self.titleNode.attributedText?.string ?? "", 
    ...
    private func animateCrossfade(animateIcon: Bool) {
        ...
        if let snapshotView = self.contentContainerNode.view.snapshotView(afterScreenUpdates: false) {
            ...
            snapshotView.layer.animateAlpha(from: 1.0, 
                                            to: 0.0,
                                            duration: ChatQrCodeScreen.themeCrossfadeDuration,
                                            delay: ChatQrCodeScreen.themeCrossfadeDelay,
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, navigationBarHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        let contentContainerFrame = CGRect(origin: CGPoint(x: sideInset, y: layout.size.height - contentHeight),
                                           size: CGSize(width: width, height: contentHeight))
        let contentFrame = contentContainerFrame
        
        var backgroundFrame = CGRect(origin: CGPoint(x: contentFrame.minX, y: contentFrame.minY), 
                                     size: CGSize(width: contentFrame.width, height: contentFrame.height + 2000.0))
        ...
        transition.updateFrame(node: self.segmentedNode, 
                               frame: CGRect(origin: CGPoint(x: floor((contentFrame.width - segmentedSize.width) / 2.0), y: 12.0),
                                             size: segmentedSize))
        ...
        let backSize = CGSize(width: 32, height: 32)
        let backFrame = CGRect(origin: CGPoint(x: 16, y: layout.safeInsets.top), size: backSize)
        transition.updateFrame(node: self.backButton, frame: backFrame)
        
        let buttonInset: CGFloat = 16.0
        let doneButtonHeight = self.doneButton.updateLayout(width: contentFrame.width - buttonInset * 2.0, transition: transition)
        transition.updateFrame(node: self.doneButton, 
                               frame: CGRect(x: buttonInset,
                                             y: contentHeight - doneButtonHeight - insets.bottom - 6.0,
        ...
        self.listNode.transaction(deleteIndices: [],
                                  insertIndicesAndItems: [],
                                  updateIndicesAndItems: [],
                                  options: [.Synchronous],
                                  scrollToItem: nil,
                                  updateSizeAndInsets: ListViewUpdateSizeAndInsets(size: CGSize(width: contentSize.height,
        ...
        self.contentNode.updateLayout(size: layout.size, 
                                      topInset: 44.0,
                                      bottomInset: contentHeight,
...
private protocol ContentNode: ASDisplayNode {
    ...
    func update(theme: PresentationTheme, wallpaper: ElloAppWallpaper, isDarkAppearance: Bool, selectedEmoticon: String?)
...
private class QrContentNode: ASDisplayNode, ContentNode {
    ...
    private var currentParams: (PresentationTheme, ElloAppWallpaper, Bool, String?)?
    ...
    init(context: AccountContext, peer: Peer, isStatic: Bool = false, link: String) {
        self.context = context
        self.peer = peer
        ...
        self.wallpaperBackgroundNode = createWallpaperBackgroundNode(context: context, 
                                                                     forChatDisplay: true,
                                                                     useSharedAnimationPhase: false,
                                                                     useExperimentalImplementation: context.sharedContext.immediateExperimentalUISettings.experimentalBackground)
        ...
        self.codeForegroundNode = GradientNode(startingAt: CGPoint.zero, 
                                               endingAt: CGPoint(x: 0, y: 1),
                                               with: [.white, .white]) // [UIColor(rgb: 0xE0D8CB), UIColor(rgb: 0xB8D5ED)])
//        self.codeForegroundNode.backgroundColor = .green
//        let gradient = self.codeForegroundNode.view.gradient(color1: UIColor.red, color2: UIColor.black)
//        self.codeForegroundNode.view.layer.insertSublayer(gradient, at: 0)
        self.codeForegroundDimNode = ASDisplayNode()
        self.codeForegroundDimNode.alpha = 0.25
        self.codeForegroundDimNode.backgroundColor = .white
        ...
        if isStatic {
            ...
            codeStaticIconNode.contentMode = .center
            codeStaticIconNode.image = UIImage(bundleImageName: "Share/QrCenterIcon")
            ...
            codeAnimatedIconNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "PlaneLogoPlain"), 
                                       width: 120,
                                       height: 120,
        ...
        let codeText: String
        if let addressName = peer.addressName, !addressName.isEmpty {
            codeText = "@\(peer.addressName ?? "")".capitalized
        } else {
            codeText = peer.debugDisplayTitle.capitalized
        }
        
        self.codeTextNode = ImmediateTextNode()
        self.codeTextNode.displaysAsynchronously = false
        self.codeTextNode.attributedText = NSAttributedString(string: codeText, 
                                                              font: Font.with(size: 25.0, design: .round, weight: .bold, traits: []),
                                                              textColor: .white) // UIColor.black.withAlphaComponent(0.25))
        ...
        self.avatarNode.setSignal(peerAvatarCompleteImage(account: context.account, 
                                                          peer: EnginePeer(peer),
                                                          size: CGSize(width: 80.0, height: 80.0),
                                                          font: avatarPlaceholderFont(size: 40.0),
                                                          fullSize: true,
                                                          clipStyle: .corner(35)))
...
//        self.codeMaskNode.addSubnode(self.codeTextNode)
        self.containerNode.addSubnode(self.codeTextNode)
        ...
        if !link.isEmpty {
            ...
            codeLink = "https://ello.team/\(peer.addressName ?? "")"
        } else if let peer = peer as? ElloAppUser {
            codeLink = "https://ello.team/+\(peer.phone ?? "")"
        } else {
            codeLink = ""
        }
    ...
    func generateImage(completion: @escaping (UIImage?) -> Void) {
        ...
        let copyNode = QrContentNode(context: self.context, peer: self.peer, isStatic: true, link: self.link)
    ...
    func updateLayout(size: CGSize, topInset: CGFloat, bottomInset: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        if size.width > 320.0 {
            avatarSize = CGSize(width: 80.0, height: 80.0)
            codeInset = 45.0
            imageSide = 220.0
        ...
        self.codeTextNode.attributedText = NSAttributedString(string: self.codeTextNode.attributedText?.string ?? "", 
                                                              font: Font.with(size: fontSize, design: .round, weight: .bold, traits: []),
                                                              textColor: .white) // .black.withAlphaComponent(0.25))
        
        let codeBackgroundWidth = min(300.0, size.width - codeInset * 2.0)
        let codeBackgroundHeight = floor(codeBackgroundWidth * 1.1)
        let codeBackgroundFrame = CGRect(x: floor((size.width - codeBackgroundWidth) / 2.0), 
                                         y: (size.height - codeBackgroundHeight) / 2.0,
                                         width: codeBackgroundWidth,
                                         height: codeBackgroundHeight)
        ...
        if let codeForegroundContentNode = self.codeForegroundContentNode {
            codeForegroundContentNode.frame = CGRect(origin: CGPoint(x: -self.codeForegroundNode.frame.minX, 
                                                                     y: -self.codeForegroundNode.frame.minY),
                                                     size: self.wallpaperBackgroundNode.frame.size)
        }
        ...
        let imageApply = makeImageLayout(TransformImageArguments(corners: ImageCorners(), 
                                                                 ...
                                                                 emptyColor: nil,
                                                                 scale: self.isStatic ? 3.0 : nil ))
        let _ = imageApply()
        
        let imageFrame = CGRect(origin: CGPoint(x: floor((codeBackgroundFrame.width - imageSize.width) / 2.0), 
                                                y: floor((codeBackgroundFrame.width - imageSize.height) / 2.0)),
                                size: imageSize)
        
        let maskFrame = CGRect(origin: CGPoint(x: codeBackgroundFrame.minX + imageFrame.minX, 
                                               y: codeBackgroundFrame.minY + imageFrame.minY),
                               size: imageSize)
        
        transition.updateFrame(node: self.codeForegroundNode, frame: maskFrame)
//        transition.updateFrame(node: self.codeMaskNode, frame: maskFrame)
        transition.updateFrame(node: self.codeImageNode, frame: CGRect(origin: .zero, size: imageSize))
//        transition.updateFrame(node: self.codeMaskNode, frame: maskFrame)
//        transition.updateFrame(node: self.codeForegroundDimNode, frame: maskFrame)
//        transition.updateFrame(node: self.codeMaskNode, frame: maskFrame)

        let codeTextSize = self.codeTextNode.updateLayout(CGSize(width: codeBackgroundFrame.width - floor(imageFrame.minX * 1.2), 
                                                                 height: codeBackgroundFrame.height))
        
        transition.updateFrame(node: self.codeTextNode,
                               frame: CGRect(origin: CGPoint(x: floor((containerNode.frame.width - codeTextSize.width) / 2.0),
                                                             y: codeBackgroundFrame.minY + imageFrame.maxY + floor((codeBackgroundHeight - imageFrame.maxY - codeTextSize.height) / 2.0) - 5.0),
                                             size: codeTextSize))
        
        transition.updateFrame(node: self.avatarNode, 
                               frame: CGRect(origin: CGPoint(x: floorToScreenPixels((size.width - avatarSize.width) / 2.0),
                                                             y: codeBackgroundFrame.minY - floor(avatarSize.height * 0.7)),
                                             size: avatarSize))
        
        if let qrCodeSize = self.qrCodeSize {
            let (_, cutoutFrame, _) = qrCodeCutout(size: qrCodeSize, dimensions: imageSize, scale: nil)
            let imageCenter = codeImageNode.frame.center.offsetBy(dx: maskFrame.minX, dy: maskFrame.minY)
            ...
            let backgroundSize = CGSize(width: floorToScreenPixels(cutoutFrame.width - 8.0), 
                                        height: floorToScreenPixels(cutoutFrame.height - 8.0))
            transition.updateFrame(node: self.codeIconBackgroundNode, 
                                   frame: CGRect(origin: CGPoint(x: floorToScreenPixels(codeImageNode.frame.center.x - backgroundSize.width / 2.0),
                                                                 y: floorToScreenPixels(codeImageNode.frame.center.y - backgroundSize.height / 2.0)),
                                                 size: backgroundSize))
//            transition.updatePosition(node: codeIconBackgroundNode, position: imageCenter)
...
private class MessageContentNode: ASDisplayNode, ContentNode {
    ...
    var hasVideo: Bool {
        if let message = self.messages.first, message.media.contains(where: { media in
            if let file = media as? ElloAppMediaFile, file.isVideo {
                return true
            } else {
                return false
            }
    ...
    init(context: AccountContext, messages: [Message], isStatic: Bool = false) {
        ...
        self.linkIconNode.image = UIImage(bundleImageName: "Share/QrCenterIcon")
        ...
        if let message = messages.first, let peer = message.peers[message.id.peerId] {
            self.avatarNode.setSignal(peerAvatarCompleteImage(account: context.account, 
                                                              ...
                                                              font: avatarPlaceholderFont(size: 14.0), 
                                                              fullSize: true))
        }
    }
    ...
    func generateVideo(completion: @escaping(URL) -> Void) {
        guard let message = self.messages.first, let media = message.media.first(where: { $0 is ElloAppMediaFile }) as? ElloAppMediaFile else {
           return
        }
    ...
    func updateLayout(size: CGSize, topInset: CGFloat, bottomInset: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        let title: String
        if let message = self.messages.first, let chatPeer = message.peers[message.id.peerId] as? ElloAppChannel {
        ...
        if let message = self.messages.first {
            ...
            for media in message.media {
                if let image = media as? ElloAppMediaImage, let representation = largestRepresentationForPhoto(image) {
                ...
                } else if let video = media as? ElloAppMediaFile, video.isVideo, let dimensions = video.dimensions?.cgSize {
        ...
        let link: String
        if let message = self.messages.first, let chatPeer = message.peers[message.id.peerId] as? ElloAppChannel, message.id.namespace == Namespaces.Message.Cloud, let addressName = chatPeer.addressName, !addressName.isEmpty {
            link = "https://ello.team/\(addressName)/\(message.id.id)"
        } else {
            link = ""
        }
...
private func renderVideo(context: AccountContext, backgroundImage: UIImage, media: ElloAppMediaFile, videoFrame: CGRect, completion: @escaping (URL?) -> Void) {
