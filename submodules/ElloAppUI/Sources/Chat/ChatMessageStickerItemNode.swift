...
class ChatMessageStickerItemNode: ChatMessageItemView {
    ...
    override func setupItem(_ item: ChatMessageItem, synchronousLoad: Bool) {
        ...
        for media in item.message.media {
            if let elloappFile = media as? ElloAppMediaFile {
                if self.elloappFile != elloappFile {
                    let signal = chatMessageSticker(account: item.context.account, file: elloappFile, small: false, onlyFullSize: self.elloappFile != nil, synchronousLoad: synchronousLoad)
                    self.elloappFile = elloappFile
                    self.imageNode.setSignal(signal, attemptSynchronously: synchronousLoad)
                    self.fetchDisposable.set(freeMediaFileInteractiveFetched(account: item.context.account, fileReference: .message(message: MessageReference(item.message), media: elloappFile)).start())
                }
        ...
        if self.elloappFile == nil && item.presentationData.largeEmoji && messageIsElligibleForLargeEmoji(item.message) {
    ...
    override func asyncLayout() -> (_ item: ChatMessageItem, _ params: ListViewItemLayoutParams, _ mergedTop: ChatMessageMerge, _ mergedBottom: ChatMessageMerge, _ dateHeaderAtBottom: Bool) -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation, ListViewItemApply, Bool) -> Void) {
        ...
        func continueAsyncLayout(_ weakSelf: Weak<ChatMessageStickerItemNode>, _ item: ChatMessageItem, _ params: ListViewItemLayoutParams, _ mergedTop: ChatMessageMerge, _ mergedBottom: ChatMessageMerge, _ dateHeaderAtBottom: Bool) -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation, ListViewItemApply, Bool) -> Void) {
            ...
            if let elloappFile = elloappFile {
            ...
            func finishAsyncLayout(_ animation: ListViewItemUpdateAnimation, _ synchronousLoads: Bool) {
                if let strongSelf = weakSelf.value {
                    ...
                    if let immediateThumbnailData = elloappFile?.immediateThumbnailData {
