...
private func peerButtons(_ state: ChatPresentationInterfaceState) -> [ChatReportPeerTitleButton] {
    ...
    if let peer = state.renderedPeer?.chatMainPeer as? ElloAppUser, let contactStatus = state.contactStatus, let peerStatusSettings = contactStatus.peerStatusSettings {
...
private final class ChatInfoTitlePanelInviteInfoNode: ASDisplayNode {
    ...
    func update(width: CGFloat, theme: PresentationTheme, strings: PresentationStrings, wallpaper: ElloAppWallpaper, chatPeer: Peer, invitedBy: Peer, transition: ContainedViewLayoutTransition) -> CGFloat {
        ...
        if let channel = chatPeer as? ElloAppChannel, case .broadcast = channel.info {
...
private final class ChatInfoTitlePanelPeerNearbyInfoNode: ASDisplayNode {
    ...
    func update(width: CGFloat, theme: PresentationTheme, strings: PresentationStrings, wallpaper: ElloAppWallpaper, chatPeer: Peer, distance: Int32, transition: ContainedViewLayoutTransition) -> CGFloat {
        let primaryTextColor = serviceMessageColorComponents(theme: theme, wallpaper: wallpaper).primaryText
...
final class ChatReportPeerTitlePanelNode: ChatTitleAccessoryPanelNode {
    ...
    override func updateLayout(width: CGFloat, leftInset: CGFloat, rightInset: CGFloat, transition: ContainedViewLayoutTransition, interfaceState: ChatPresentationInterfaceState) -> LayoutResult {
        ...
        if let user = interfaceState.renderedPeer?.peer as? ElloAppUser, let emojiStatusValue = user.emojiStatus {
