    ...
    if let _ = presentationInterfaceState.interfaceState.selectionState {
        ...
        if let currentButton = currentButton, currentButton.action == .clearHistory {
            ...
            if case .scheduledMessages = presentationInterfaceState.subject {
                ...
                if peer is ElloAppUser || peer is ElloAppGroup || peer is ElloAppSecretChat {
                    canClear = true
                } else if let peer = peer as? ElloAppChannel, case .group = peer.info, peer.addressName == nil && presentationInterfaceState.peerGeoLocation == nil {
                    canClear = true
            ...
            // EM-2669
            let disableClearMessage = true
            if canClear && !disableClearMessage {
