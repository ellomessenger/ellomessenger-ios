...
private final class StatusReactionNode: ASDisplayNode {
    ...
    private var resolvedFile: ElloAppMediaFile?
    ...
    func update(context: AccountContext, type: ChatMessageDateAndStatusType, value: MessageReaction.Reaction, file: ElloAppMediaFile?, fileId: Int64?, isSelected: Bool, count: Int, theme: PresentationTheme, wallpaper: ElloAppWallpaper, animationCache: AnimationCache, animationRenderer: MultiAnimationRenderer, animated: Bool) {
        if self.value != value {
            self.value = value
                ...
                if let reactionSettings = reactionSettings, !reactionSettings.displayInline {
                    ...
                    reactionButtonsResult = reactionButtonsContainer.update(
                        ...
                        reactions: arguments.reactions.map { reaction in
                            var centerAnimation: ElloAppMediaFile?
...
func shouldDisplayInlineDateReactions(message: Message, isPremium: Bool, forceInline: Bool) -> Bool {
    ...
    if message.id.peerId.namespace == Namespaces.Peer.CloudUser || message.id.peerId.namespace == Namespaces.Peer.SecretChat {
        if let chatPeer = message.peers[message.id.peerId] as? ElloAppUser, chatPeer.isPremium {
            return false
        }
        if let author = message.author as? ElloAppUser, author.isPremium {
            return false
        }
        if isPremium {
            return false
