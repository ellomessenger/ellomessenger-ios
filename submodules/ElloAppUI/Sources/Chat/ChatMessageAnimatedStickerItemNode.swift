...
class ChatMessageShareButton: HighlightableButtonNode {
    ...
    func update(presentationData: ChatPresentationData, controllerInteraction: ChatControllerInteraction, chatLocation: ChatLocation, subject: ChatControllerSubject?, message: Message, account: Account, disableComments: Bool = false) -> CGSize {
        var isReplies = false
        var replyCount = 0
        if let channel = message.peers[message.id.peerId] as? ElloAppChannel, case .broadcast = channel.info {
...
class ChatMessageAnimatedStickerItemNode: ChatMessageItemView {
    ...
    var elloappFile: ElloAppMediaFile?
    var emojiFile: ElloAppMediaFile?
    var elloappDice: ElloAppMediaDice?
    ...
    private func setupNode(item: ChatMessageItem) {
        ...
        if let elloappDice = self.elloappDice {
            if elloappDice.emoji == "🎰" {
                ...
                let animationNode = ManagedDiceAnimationNode(context: item.context, emoji: elloappDice.emoji.strippedEmoji)
                if !item.message.effectivelyIncoming(item.context.account.peerId) {
    ...
    override func setupItem(_ item: ChatMessageItem, synchronousLoad: Bool) {
        ...
        for media in item.message.media {
            if let elloappFile = media as? ElloAppMediaFile {
                if self.elloappFile?.id != elloappFile.id {
                    self.elloappFile = elloappFile
                    let dimensions = elloappFile.dimensions ?? PixelDimensions(width: 512, height: 512)
                    self.imageNode.setSignal(chatMessageAnimatedSticker(postbox: item.context.account.postbox, file: elloappFile, small: false, size: dimensions.cgSize.aspectFitted(CGSize(width: 384.0, height: 384.0)), thumbnail: false, synchronousLoad: synchronousLoad), attemptSynchronously: synchronousLoad)
                    self.updateVisibility()
                    self.disposable.set(freeMediaFileInteractiveFetched(account: item.context.account, fileReference: .message(message: MessageReference(item.message), media: elloappFile)).start())
                    
                    if elloappFile.isPremiumSticker {
                        if let effect = elloappFile.videoThumbnails.first {
                            self.disposables.add(freeMediaFileResourceInteractiveFetched(account: item.context.account, fileReference: .message(message: MessageReference(item.message), media: elloappFile), resource: effect.resource) .start())
                        }
                    }
                }
                break
            } else if let elloappDice = media as? ElloAppMediaDice {
                self.elloappDice = elloappDice
            }
        }
        
        self.setupNode(item: item)
        
        if let elloappDice = self.elloappDice, let diceNode = self.animationNode as? SlotMachineAnimationNode {
            if let value = elloappDice.value {
                diceNode.setState(value == 0 ? .rolling : .value(value, true))
            } else {
                diceNode.setState(.rolling)
            }
        } else if let elloappDice = self.elloappDice, let diceNode = self.animationNode as? ManagedDiceAnimationNode {
            if let value = elloappDice.value {
                diceNode.setState(value == 0 ? .rolling : .value(value, true))
            } else {
                diceNode.setState(.rolling)
            }
        } else if self.elloappFile == nil && self.elloappDice == nil {
    ...
    private func updateVisibility() {
        ...
        if let elloappFile = self.elloappFile {
            file = elloappFile
    ...
    override func asyncLayout() -> (_ item: ChatMessageItem, _ params: ListViewItemLayoutParams, _ mergedTop: ChatMessageMerge, _ mergedBottom: ChatMessageMerge, _ dateHeaderAtBottom: Bool) -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation, ListViewItemApply, Bool) -> Void) {
        ...
        let elloappFile = self.elloappFile
        let emojiFile = self.emojiFile
        let elloappDice = self.elloappDice
        ...
        func continueAsyncLayout(_ weakSelf: Weak<ChatMessageAnimatedStickerItemNode>, _ item: ChatMessageItem, _ params: ListViewItemLayoutParams, _ mergedTop: ChatMessageMerge, _ mergedBottom: ChatMessageMerge, _ dateHeaderAtBottom: Bool) -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation, ListViewItemApply, Bool) -> Void) {
            ...
            if !(elloappFile?.videoThumbnails.isEmpty ?? true) {
            ...
            case let .peer(peerId):
                if peerId != item.context.account.peerId {
                    if peerId.isGroupOrChannel && item.message.author != nil {
                        ...
                        if let peer = item.message.peers[item.message.id.peerId] as? ElloAppChannel, case .broadcast = peer.info {
            ...
            if case .pinnedMessages = item.associatedData.subject {
                ...
                if !needsShareButton, let author = item.message.author as? ElloAppUser, let _ = author.botInfo, !item.message.media.isEmpty {
                    needsShareButton = true
                }
                if !needsShareButton {
            ...
            var isEmoji = false
            if let _ = elloappDice {
                imageSize = displaySize
            } else if let elloappFile = elloappFile {
                if let dimensions = elloappFile.dimensions {
                    imageSize = dimensions.cgSize.aspectFitted(displaySize)
                } else if let thumbnailSize = elloappFile.previewRepresentations.first?.dimensions {
            ...
            for attribute in item.message.attributes {
                if let attribute = attribute as? EditedMessageAttribute, isEmoji {
                    ...
                    if let channel = item.message.peers[item.message.id.peerId] as? ElloAppChannel, case .group = channel.info {
            ...
            for attribute in item.message.attributes {
                if let attribute = attribute as? InlineBotMessageAttribute {
                    ...
                    if let peerId = attribute.peerId, let bot = item.message.peers[peerId] as? ElloAppUser {
    ...
    private func commitEnqueuedAnimations() {
        ...
        if emojiFile == nil {
            emojiFile = item.message.associatedMedia.first?.value as? ElloAppMediaFile
        }
    ...
    func playPremiumStickerAnimation() {
        guard !self.playedPremiumStickerAnimation, let item = self.item, let file = self.elloappFile, file.isPremiumSticker, let effect = file.videoThumbnails.first else {
            return
        }
    ...
    private func gestureRecognized(gesture: TapLongTapOrDoubleTapGesture, location: CGPoint, recognizer: TapLongTapOrDoubleTapGestureRecognizer?) -> InternalBubbleTapAction? {
        ...
        case .tap:            
                ...
                if let item = self.item, let forwardInfo = item.message.forwardInfo {
                    let performAction: () -> Void = {
                        if let sourceMessageId = forwardInfo.sourceMessageId {
                            if !item.message.id.peerId.isReplies, let channel = forwardInfo.author as? ElloAppChannel, channel.username == nil {
            ...
            if let item = self.item, self.imageNode.frame.contains(location) {
                ...
                } else if let dice = self.elloappDice {
                    return .optionalAction({
    ...
    @objc private func shareButtonPressed() {
        if let item = self.item {
            ...
            if let channel = item.message.peers[item.message.id.peerId] as? ElloAppChannel, case .broadcast = channel.info {
                for attribute in item.message.attributes {
    ...
    override func animateInsertion(_ currentTimestamp: Double, duration: Double, short: Bool) {
        ...
        if let elloappDice = self.elloappDice, let item = self.item, item.message.effectivelyIncoming(item.context.account.peerId) {
            if let value = elloappDice.value, value != 0 {
...
struct AnimatedEmojiSoundsConfiguration {
    ...
    public let sounds: [String: ElloAppMediaFile]
    
    fileprivate init(sounds: [String: ElloAppMediaFile]) {
        self.sounds = sounds
    }
    ...
    static func with(appConfiguration: AppConfiguration, account: Account) -> AnimatedEmojiSoundsConfiguration {
        if let data = appConfiguration.data, let values = data["emojies_sounds"] as? [String: Any] {
            var sounds: [String: ElloAppMediaFile] = [:]
            for (key, value) in values {
                if let dict = value as? [String: String], var fileReferenceString = dict["file_reference_base64"] {
                    ...
                    if let idString = dict["id"], let id = Int64(idString), let accessHashString = dict["access_hash"], let accessHash = Int64(accessHashString), let fileReference = Data(base64Encoded: fileReferenceString) {
                        ...
                        let file = ElloAppMediaFile(fileId: MediaId(namespace: Namespaces.Media.LocalFile, id: 0), partialReference: nil, resource: resource, previewRepresentations: [], videoThumbnails: [], immediateThumbnailData: nil, mimeType: "audio/ogg", size: nil, attributes: [])
                        sounds[key] = file
                    }
                }
            }
