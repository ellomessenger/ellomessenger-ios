...
private let cancelFont = UIFont.Custom.sfProDisplay(ofSize: 16, weight: .semibold)
...
final class ChatTextInputAudioRecordingCancelIndicator: ASDisplayNode {
    ...
    init(theme: PresentationTheme, strings: PresentationStrings, cancel: @escaping () -> Void) {
        ...
        self.cancelButton.setTitle(strings.Common_Cancel, with: cancelFont, with: theme.chat.inputPanel.panelControlCancelButtontColor, for: [])
        ...
        self.cancelButton.frame = CGRect(origin: CGPoint(x: floor((self.bounds.size.width) / 2.0), y: floor((height - cancelSize.height) / 2.0)), size: cancelSize)
