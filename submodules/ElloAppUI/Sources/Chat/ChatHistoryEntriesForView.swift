...
func chatHistoryEntriesForView(
    ...
    if case let .peer(peerId) = location, peerId.namespace == Namespaces.Peer.CloudChannel {
        for additionalEntry in view.additionalData {
            ...
            } else if case let .peer(_, peer) = additionalEntry, let channel = peer as? ElloAppChannel, !channel.flags.contains(.isGigagroup) {
                channelPeer = channel
    ...
    if case let .peer(peerId) = location, case let cachedData = cachedData as? CachedChannelData, let invitedOn = cachedData?.invitedOn  {
        joinMessage = Message(
            ...
            text: "",
            attributes: [],
            media: [ElloAppMediaAction(action: .joinedByRequest)],
            ...
            if case let .peer(peerId) = location, peerId.isReplies {
                ...
                if entries.count <= 3 {
                    loop: for entry in view.entries {
                        ...
                        var isCreator = false
                        if let peer = entry.message.peers[entry.message.id.peerId] as? ElloAppGroup, case .creator = peer.role {
                            isCreator = true
                        } else if let peer = entry.message.peers[entry.message.id.peerId] as? ElloAppChannel, case .group = peer.info, peer.flags.contains(.isCreator) {
                            isCreator = true
                        }
