...
class ChatMessageInteractiveInstantVideoNode: ASDisplayNode {
    ...
    func asyncLayout() -> (_ item: ChatMessageBubbleContentItem, _ width: CGFloat, _ displaySize: CGSize, _ maximumDisplaySize: CGSize, _ scaleProgress: CGFloat, _ statusType: ChatMessageInteractiveInstantVideoNodeStatusType, _ automaticDownload: Bool) -> (ChatMessageInstantVideoItemLayoutResult, (ChatMessageInstantVideoItemLayoutData, ListViewItemUpdateAnimation) -> Void) {
        ...
        return { item, width, displaySize, maximumDisplaySize, scaleProgress, statusDisplayType, automaticDownload in
            ...
            var updatedFile: ElloAppMediaFile?
            ...
            for media in item.message.media {
                ...
                } else if let webPage = media as? ElloAppMediaWebpage, case let .Loaded(content) = webPage.content, let file = content.file {
                    updatedFile = file
                    if let previousFile = previousFile {
                        updatedMedia = previousFile.resource.id != file.resource.id
                    } else if previousFile == nil {
            ...
            return (result, { [weak self] layoutData, animation in
                if let strongSelf = self {
                    ...
                    if let elloappFile = updatedFile {
                        if updatedMedia {
                            ...
                            durationNode.defaultDuration = elloappFile.duration.flatMap(Double.init)
                            
                            let streamVideo = automaticDownload && isMediaStreamable(message: item.message, media: elloappFile) && elloappFile.id?.namespace != Namespaces.Media.LocalFile
                            ...
                            }), content: NativeVideoContent(id: .message(item.message.stableId, elloappFile.fileId), fileReference: .message(message: MessageReference(item.message), media: elloappFile), streamVideo: streamVideo ? .conservative : .none, enableSound: false, fetchAutomatically: false, captureProtected: item.message.isCopyProtected()), priority: .embedded, autoplay: true)
                            ...
                            if isSecretMedia {
                                let updatedSecretPlaceholderSignal = chatSecretMessageVideo(account: item.context.account, videoReference: .message(message: MessageReference(item.message), media: elloappFile))
                    ...
                    if let elloappFile = updatedFile, previousAutomaticDownload != automaticDownload, automaticDownload {
                        strongSelf.fetchDisposable.set(messageMediaFileInteractiveFetched(context: item.context, message: item.message, file: elloappFile, userInitiated: false).start())
                    }
    ...
    private func updateStatus() {
        ...
        var selectedMedia: ElloAppMediaFile?
        for media in item.message.media {
            if let file = media as? ElloAppMediaFile {
                selectedMedia = file
            } else if let webPage = media as? ElloAppMediaWebpage, case let .Loaded(content) = webPage.content, let file = content.file {
        ...
        if case .playbackStatus = status.mediaStatus {
            ...
            if let current = self.playbackStatusNode {
                ...
                playbackStatusNode = InstantVideoRadialStatusNode(color: UIColor.white, progressLineWidth: 15.0, hasSeek: true)
