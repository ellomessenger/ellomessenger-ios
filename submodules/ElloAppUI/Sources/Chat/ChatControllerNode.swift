...
class ChatControllerNode: ASDisplayNode, UIScrollViewDelegate {
    ...
    var isEmptyNodeDisplayed: Bool {
        emptyNode != nil
    }
    public var historyLoadedCallback: VoidClosure?
    ...
    var showNeedMorePromts: EventClosure<BotRequestsCountItem.RequestType>?
    var needUpdatePromtsLabel: VoidClosure?
    ...
    public var isMessageActionsEnabled = true
    private lazy var botProcessingItemNode: BotProcessingItemNode = {
        let item = BotProcessingItem(id: 0, link: "") { [weak self] in
            self?.closeProcessing()
        } minimizeAction: { [weak self] isMinimized in
            guard let self else {
                return
            }
            self.botProcessingItemNode.animateFrameChange(transition: .animated(duration: 0.4, curve: .linear), frame: frameForBotProcessing(isMinimized: isMinimized))
        }
        let itemNode = item.node()
        return itemNode
    }()
    
    private lazy var botRequestsCountItemNode: BotRequestsCountItemNode = {
        let item = BotRequestsCountItem(id: 1, count: 10, type: .text) { [weak self] in
            self?.controller?.presentPurchasePrompts()
        }
    ...
    func frameForBotProcessing(isMinimized: Bool) -> CGRect {
//        if isMinimized {
//            guard var inputFrame = self.inputPanelNode?.frame else {
//                return botProcessingItemNode.frame
//            }
//            inputFrame.size = CGSize(width: inputFrame.width, height: inputFrame.height + safeAreaInsets.bottom)
//            return inputFrame
//        } else {
//            let botProcessingItemNodeFrame = {
//                let origin = CGPoint(x: 0, y: self.navigationBar?.frame.maxY ?? 0)
//                return CGRect(origin: origin, size: CGSize(width: self.contentContainerNode.frame.size.width, height: self.contentContainerNode.frame.size.height - origin.y))
//            }()
//            return botProcessingItemNodeFrame
//        }
        guard let inputFrame = self.inputPanelNode?.frame else {
            let botProcessingItemNodeFrame = {
                let origin = CGPoint(x: 0, y: self.frame.maxY - 78)
                return CGRect(origin: origin, size: CGSize(width: self.contentContainerNode.frame.size.width, height: 78))
            }()
            return botProcessingItemNodeFrame
        }
        let height = inputFrame.height + safeAreaInsets.bottom
        let botProcessingItemNodeFrame = {
            let origin = CGPoint(x: 0, y: self.frame.maxY - height)
            return CGRect(origin: origin, size: CGSize(width: self.contentContainerNode.frame.size.width, height: height))
        }()
        return botProcessingItemNodeFrame
    }
    
    func closeProcessing() {
        botProcessingItemNode.removeFromSupernode()
        isMessageActionsEnabled = true
    }
    
    func addBotProccessing() {
        guard !(self.subnodes?.contains(where: { $0 == botProcessingItemNode }) ?? false) else {
            return
        }
        inputPanelNode?.view.endEditing(true)
        botProcessingItemNode.restartAnimation()
        addSubnode(botProcessingItemNode)
        isMessageActionsEnabled = false
    }
    
    func closeBotRequestsInfo() {
        botRequestsCountItemNode.removeFromSupernode()
    }
    
    func addBotRequestInfo(count: Int, type: BotRequestsCountItem.RequestType) {
        let item = BotRequestsCountItem(id: 1, count: count, type: type) { [weak self] in
            self?.controller?.presentPurchasePrompts()
        }
        
        if (self.subnodes?.contains(where: { $0 == botRequestsCountItemNode }) ?? false) {
            botRequestsCountItemNode.removeFromSupernode()
        }
        
        addSubnode(botRequestsCountItemNode)
        botRequestsCountItemNode.frame = CGRect(x: 0, y: navigationBar?.frame.maxY ?? 0, width: self.bounds.width, height: 46)
        botRequestsCountItemNode.updateViewFrameOnAppearing()
        botRequestsCountItemNode.updateItem(item)
        
        if item.count == 0 {
            showNeedMorePromts?(type)
        }
    }
    ...
    init(context: AccountContext, 
                    ...
                    if hideNames {
                        for media in message.media {
                            if options.hideCaptions {
                                if media is ElloAppMediaImage || media is ElloAppMediaFile {
                                    messageText = ""
                                    break
                                }
                            }
                            if let poll = media as? ElloAppMediaPoll {
                                var updatedMedia = message.media.filter { !($0 is ElloAppMediaPoll) }
                                updatedMedia.append(ElloAppMediaPoll(pollId: poll.pollId, publicity: poll.publicity, kind: poll.kind, text: poll.text, options: poll.options, correctAnswers: poll.correctAnswers, results: ElloAppMediaPollResults(voters: nil, totalVoters: nil, recentVoters: [], solution: nil), isClosed: false, deadlineTimeout: nil))
                                messageMedia = updatedMedia
                            }
                            if let _ = media as? ElloAppMediaDice {
                                hasDice = true
                            }
                        }
                    }
        ...
        self.historyNode = ChatHistoryListNode(context: context, 
                                               updatedPresentationData: controller?.updatedPresentationData ?? (context.sharedContext.currentPresentationData.with({ $0 }), context.sharedContext.presentationData),
                                               chatLocation: chatLocation,
        ...
        self.loadingNode = ChatLoadingNode(theme: self.chatPresentationInterfaceState.theme, 
                                           chatWallpaper: self.chatPresentationInterfaceState.chatWallpaper,
                                           bubbleCorners: self.chatPresentationInterfaceState.bubbleCorners)
        ...
        self.historyNode.setLoadStateUpdated { [weak self] loadState, animated in
            if let strongSelf = self {
                ...
                strongSelf.historyLoadedCallback?()
            }
        }
        ...
        self.textInputPanelNode = ChatTextInputPanelNode(context: context, 
                                                         presentationInterfaceState: chatPresentationInterfaceState,
                                                         presentationContext: ChatPresentationContext(context: context,
        ...
        self.textInputPanelNode?.sendMessage = { [weak self] in
            ...
            if case .scheduledMessages = chatPresentationInterfaceState.subject, chatPresentationInterfaceState.editMessageState == nil {
                controllerInteraction.scheduleCurrentMessage()
            } else {
                if canSendMessage() {
                    sendCurrentMessage()
                }
    ...
    private func updateIsEmpty(_ emptyType: ChatHistoryNodeLoadState.EmptyType?, wasLoading: Bool, animated: Bool) {
        ...
        if let emptyType = emptyType, self.emptyNode == nil {
            ...
            if let (size, insets) = self.validEmptyNodeLayout {
                emptyNode.updateLayout(interfaceState: self.chatPresentationInterfaceState, 
                                       emptyType: emptyType,
                                       loadingNode: wasLoading && self.loadingNode.supernode != nil ? self.loadingNode : nil,
    ...
    func containerLayoutUpdated(_ layout: ContainerViewLayout, 
                                ...
                                transition protoTransition: ContainedViewLayoutTransition,
                                listViewTransaction: (ListViewUpdateSizeAndInsets, CGFloat, Bool, @escaping () -> Void) -> Void,
                                updateExtraNavigationBarBackgroundHeight: (CGFloat, ContainedViewLayoutTransition) -> Void) {
        let transition: ContainedViewLayoutTransition
        ...
        self.botProcessingItemNode.frame = frameForBotProcessing(isMinimized: self.botProcessingItemNode.isMinimized)
        self.contentContainerNode.frame = CGRect(origin: CGPoint(), size: layout.size)
        self.botProcessingItemNode.updateViewFrameOnAppearing()
        ...
        if let titleAccessoryPanelNode = titlePanelForChatPresentationInterfaceState(self.chatPresentationInterfaceState, 
                                                                                     context: self.context,
                                                                                     currentPanel: self.titleAccessoryPanelNode,
            ...
            let layoutResult = titleAccessoryPanelNode.updateLayout(width: layout.size.width, 
                                                                    leftInset: layout.safeInsets.left,
                                                                    rightInset: layout.safeInsets.right,
                                                                    transition: immediatelyLayoutTitleAccessoryPanelNodeAndAnimateAppearance ? .immediate : transition,
        ...
        let inputPanelNodes = inputPanelForChatPresentationIntefaceState(self.chatPresentationInterfaceState,
                                                                         context: self.context,
                                                                         currentPanel: self.inputPanelNode,
        ...
        if let inputPanelNode = inputPanelNodes.primary, !previewing {
            if inputPanelNode !== self.inputPanelNode {
                ...
                let inputPanelHeight = inputPanelNode.updateLayout(width: layout.size.width,
                                                                   leftInset: layout.safeInsets.left,
                                                                   rightInset: layout.safeInsets.right,
                                                                   bottomInset: layout.intrinsicInsets.bottom,
                                                                   additionalSideInsets: layout.additionalInsets,
                                                                   maxHeight: layout.size.height - insets.top - inputPanelBottomInset,
                                                                   isSecondary: false,
                                                                   transition: inputPanelNode.supernode !== self ? .immediate : transition,
                                                                   interfaceState: self.chatPresentationInterfaceState,
                                                                   metrics: layout.metrics,
                                                                   isMediaInputExpanded: self.inputPanelContainerNode.expansionFraction == 1.0)
                ...
                let inputPanelHeight = inputPanelNode.updateLayout(width: layout.size.width,
                                                                   ...
                                                                   bottomInset: layout.intrinsicInsets.bottom,
                                                                   additionalSideInsets: layout.additionalInsets,
                                                                   maxHeight: layout.size.height - insets.top - inputPanelBottomInset - 120.0,
                                                                   isSecondary: false,
    ...
    func updateChatPresentationInterfaceState(_ chatPresentationInterfaceState: ChatPresentationInterfaceState, 
                                              transition: ContainedViewLayoutTransition,
                                              interactive: Bool,
                                              completion: @escaping (ContainedViewLayoutTransition) -> Void) {
        ...
        if self.chatPresentationInterfaceState != chatPresentationInterfaceState && chatPresentationInterfaceState.presentationReady {
            ...
            if themeUpdated {
                ...
                self.backgroundNode.updateBubbleTheme(bubbleTheme: chatPresentationInterfaceState.theme, 
                                                      bubbleCorners: chatPresentationInterfaceState.bubbleCorners)
            }
            ...
            if let peer = chatPresentationInterfaceState.renderedPeer?.peer, let restrictionTextValue = peer.restrictionText(platform: "ios", contentSettings: self.context.currentContentSettings.with { $0 }), !restrictionTextValue.isEmpty {
                ...
                } else if let peer = chatPresentationInterfaceState.renderedPeer?.peer as? ElloAppChannel, case .broadcast = peer.info {
            ...
            if let _ = self.chatPresentationInterfaceState.search, let interfaceInteraction = self.interfaceInteraction {
                ...
                if self.searchNavigationNode == nil {
                    activate = true
                    self.searchNavigationNode = ChatSearchNavigationContentNode(theme: self.chatPresentationInterfaceState.theme, 
                                                                                strings: self.chatPresentationInterfaceState.strings,
                                                                                chatLocation: self.chatPresentationInterfaceState.chatLocation,
    ...
    func dismissInput() {
        ...
        case .none:
            break
        case .inputButtons:
            if let peer = self.chatPresentationInterfaceState.renderedPeer?.peer as? ElloAppUser, peer.botInfo != nil {
            } else {
    ...
    private func panGestureBegan(location: CGPoint) {
        ...
        if let inputHeight = derivedLayoutState.inputNodeHeight, !inputHeight.isZero, keyboardGestureBeginLocation.y < validLayout.size.height - inputHeight - accessoryHeight, !self.inputPanelContainerNode.stableIsExpanded {
            ...
            if let peer = self.chatPresentationInterfaceState.renderedPeer?.peer as? ElloAppUser, peer.botInfo != nil, case .inputButtons = self.chatPresentationInterfaceState.inputMode {
                enableGesture = false
            }
    ...
    func isAIBotChat() -> Bool {
        chatLocation.peerId?.isAIBotID ?? false
    }
    
    func isAIPhoenixBotChat() -> Bool {
        chatLocation.peerId?.isAIPhoenixBotID ?? false
    }

    func isAIBusinessForStudentsBotChat() -> Bool {
        chatLocation.peerId?.isAIBusinessForStudentsBotID ?? false
    }

    func isAICancerPreventationBotChat() -> Bool {
        chatLocation.peerId?.isAICancerPreventationBotID ?? false
    }

    func canSendMessage(type: BotRequestsCountItem.RequestType? = nil) -> Bool {
        guard isAIBotChat() || isAIPhoenixBotChat() || isAIBusinessForStudentsBotChat() || isAICancerPreventationBotChat() else {
            return true
        }
        
        if let info = botRequestsCountItemNode.item, info.count == 0 {
            showNeedMorePromts?(info.type)
            return false
        } else {
            needUpdatePromtsLabel?()
            return true
        }
    }
    ...
    func sendCurrentMessage(silentPosting: Bool? = nil, scheduleTime: Int32? = nil, completion: @escaping () -> Void = {}) {
        if let textInputPanelNode = self.inputPanelNode as? ChatTextInputPanelNode {
            ...
            if let _ = effectivePresentationInterfaceState.interfaceState.editMessage {
                ...
                var firstLockedPremiumEmoji: ElloAppMediaFile?
                ...
                if peerId?.namespace != Namespaces.Peer.SecretChat, let interactiveEmojis = self.interactiveEmojis, interactiveEmojis.emojis.contains(trimmedInputText) {
                    messages.append(.message(text: "", attributes: [], inlineStickers: [:], mediaReference: AnyMediaReference.standalone(media: ElloAppMediaDice(emoji: trimmedInputText)), replyToMessageId: self.chatPresentationInterfaceState.interfaceState.replyMessageId, localGroupingKey: nil, correlationId: nil, bubbleUpEmojiOrStickersets: []))
                } else {
                    ...
                    for text in breakChatInputText(trimChatInputText(inputText)) {
                        if text.length != 0 {
                            ...
                            var webpage: ElloAppMediaWebpage?
