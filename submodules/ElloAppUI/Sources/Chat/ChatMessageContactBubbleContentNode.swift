...
class ChatMessageContactBubbleContentNode: ChatMessageBubbleContentNode {
    ...
    private var contact: ElloAppMediaContact?
    ...
    override func asyncLayoutContent() -> (_ item: ChatMessageBubbleContentItem, _ layoutConstants: ChatMessageItemLayoutConstants, _ preparePosition: ChatMessageBubblePreparePosition, _ messageSelection: Bool?, _ constrainedSize: CGSize) -> (ChatMessageBubbleContentProperties, CGSize?, CGFloat, (CGSize, ChatMessageBubbleContentPosition) -> (CGFloat, (CGFloat) -> (CGSize, (ListViewItemUpdateAnimation, Bool, ListViewItemApply?) -> Void))) {
        ...
        return { item, layoutConstants, _, _, constrainedSize in
            var selectedContact: ElloAppMediaContact?
            for media in item.message.media {
                if let media = media as? ElloAppMediaContact {
            ...
            if let selectedContact = selectedContact {
                ...
                let info: String
                if let previousContact = previousContact,
                   previousContact.isEqual(to: selectedContact),
                   let contactInfo = previousContactInfo {
                    info = contactInfo
                } else {
                    if let vCard = selectedContact.vCardData,
                       let vCardData = vCard.data(using: .utf8),
                       let contactData = DeviceContactExtendedData(vcard: vCardData) {
                        ...
                        if !contactData.basicData.userNames.isEmpty {
                            for phone in contactData.basicData.userNames {
                                if infoComponents.count < infoLineLimit {
                                    infoComponents.append(formatPhoneNumber(phone.value))
                                }
                            }
                        } else {
                            infoComponents.append(formatPhoneNumber(selectedContact.username))
                        }
                        if infoComponents.count < infoLineLimit {
            ...
            return (contentProperties, nil, CGFloat.greatestFiniteMagnitude, { constrainedSize, position in
                ...
                for attribute in item.message.attributes {
                    if let attribute = attribute as? EditedMessageAttribute {
                        ...
                        if let channel = item.message.peers[item.message.id.peerId] as? ElloAppChannel,
                ...
                return (contentWidth, { boundingWidth in
                    ...
                    return (layoutSize, { [weak self] animation, synchronousLoads, _ in
                        if let strongSelf = self {
                            ...
                            if let peerId = selectedContact?.peerId,
                               let peer = item.message.peers[peerId] {
                                strongSelf.avatarNode.setPeer(
                                    context: item.context,
                                    theme: item.presentationData.theme.theme,
                                    peer: EnginePeer(peer),
                                    emptyColor: avatarPlaceholderColor,
                                    synchronousLoad: synchronousLoads
                                )
                            } else if let peerId = selectedContact?.peerId {
                                let firstName = selectedContact?.firstName ?? ""
                                let lastName = selectedContact?.lastName ?? ""
                                if !firstName.isEmpty && !lastName.isEmpty {
                                    customLetters = [String(firstName[..<firstName.index(after: firstName.startIndex)]).uppercased(), String(lastName[..<lastName.index(after: lastName.startIndex)]).uppercased()]
                                ...
                                let peersSignal = item.context.engine.data.get(
                                    ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId)
                                )
                                let paramsSignal = item.context.engine.peers.fetchAndUpdateUserFull(peerId: peerId)
                                |> mapToSignal { peer -> Signal<EnginePeer?, NoError> in
                                    return peersSignal
                                }
                                
                                let _ = (paramsSignal |> deliverOnMainQueue).start(next: { peer in
