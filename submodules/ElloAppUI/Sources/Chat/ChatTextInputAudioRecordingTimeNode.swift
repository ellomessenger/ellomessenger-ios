...
private let customTextFont = UIFont.Custom.sfProDisplay(ofSize: 16, weight: .semibold)
...
final class ChatTextInputAudioRecordingTimeNode: ASDisplayNode {
    ...
    override func calculateSizeThatFits(_ constrainedSize: CGSize) -> CGSize {
        ...
        let (size, apply) = makeLayout(TextNodeLayoutArguments(attributedString: NSAttributedString(string: "00 : 00 : 00", font: customTextFont, textColor: theme.chat.inputPanel.timerTextColor), backgroundColor: nil, maximumNumberOfLines: 1, truncationType: .end, constrainedSize: CGSize(width: 200.0, height: 100.0), alignment: .natural, cutout: nil, insets: UIEdgeInsets()))
        let _ = apply()
    ...
    @objc override public class func draw(_ bounds: CGRect, withParameters parameters: Any?, isCancelled: () -> Bool, isRasterizing: Bool) {
        ...
        if let parameters = parameters as? ChatTextInputAudioRecordingTimeNodeParameters {
            ...
            let string = NSAttributedString(string: text, font: customTextFont, textColor: parameters.theme.chat.inputPanel.timerTextColor)
