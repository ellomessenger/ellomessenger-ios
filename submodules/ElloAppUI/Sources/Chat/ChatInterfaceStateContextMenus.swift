...
private func canEditMessage(accountPeerId: PeerId, limitsConfiguration: EngineConfiguration.Limits, message: Message, reschedule: Bool = false) -> Bool {
    ...
    if message.id.namespace == Namespaces.Message.ScheduledCloud {
        if let peer = message.peers[message.id.peerId], let channel = peer as? ElloAppChannel {
            switch channel.info {
                case .broadcast:
        ...
        if message.media.contains(where: { $0 is ElloAppMediaInvoice }) {
            hasEditRights = false
        } else {
            hasEditRights = true
        }
    ...
    if hasEditRights {
        ...
        for media in message.media {
            if let file = media as? ElloAppMediaFile {
                if file.isSticker || file.isAnimatedSticker || file.isInstantVideo {
                    hasUneditableAttributes = true
                    break
                }
            } else if let _ = media as? ElloAppMediaContact {
                hasUneditableAttributes = true
                break
            } else if let _ = media as? ElloAppMediaExpiredContent {
                hasUneditableAttributes = true
                break
            } else if let _ = media as? ElloAppMediaMap {
                hasUneditableAttributes = true
                break
            } else if let _ = media as? ElloAppMediaPoll {
                hasUneditableAttributes = true
                break
            }  else if let _ = media as? ElloAppMediaDice {
                hasUneditableAttributes = true
                break
            }  else if let _ = media as? ElloAppMediaGame {
                hasUneditableAttributes = true
                break
            }  else if let _ = media as? ElloAppMediaInvoice {
                hasUneditableAttributes = true
                break
            }
        }
...
private func canViewReadStats(message: Message, participantCount: Int?, isMessageRead: Bool, appConfig: AppConfiguration) -> Bool {
    ...
    for media in message.media {
        if let _ = media as? ElloAppMediaAction {
            return false
...
func messageMediaEditingOptions(message: Message) -> MessageMediaEditingOptions {
    ...
    for media in message.media {
        if let _ = media as? ElloAppMediaImage {
...
func updatedChatEditInterfaceMessageState(state: ChatPresentationInterfaceState, message: Message) -> ChatPresentationInterfaceState {
    var updated = state
    for media in message.media {
        if let webpage = media as? ElloAppMediaWebpage, case let .Loaded(content) = webpage.content {
            updated = updated.updatedEditingUrlPreview((content.url, webpage))
        }
    }
    var isPlaintext = true
    for media in message.media {
        if !(media is ElloAppMediaWebpage) {
            isPlaintext = false
            break
        }
    }
...
func contextMenuForChatPresentationInterfaceState(chatPresentationInterfaceState: ChatPresentationInterfaceState, context: AccountContext, messages: [Message], controllerInteraction: ChatControllerInteraction?, selectAll: Bool, interfaceInteraction: ChatPanelInterfaceInteraction?, readStats: MessageReadStats? = nil, messageNode: ChatMessageItemView? = nil) -> Signal<ContextController.Items, NoError> {
    ...
    if messages.count == 1, let _ = messages[0].adAttribute {
        ...
        actions.append(.action(ContextMenuActionItem(text: presentationData.strings.SponsoredMessageMenu_Info, textColor: .primary, textLayout: .twoLinesMax, textFont: .custom(Font.regular(presentationData.listsFontSize.baseDisplaySize - 1.0)), badge: nil, icon: { theme in
            return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Info"), color: theme.actionSheet.controlAccentColor)
        ...
        if !chatPresentationInterfaceState.isPremium && !premiumConfiguration.isPremiumDisabled {
            actions.append(.action(ContextMenuActionItem(text: presentationData.strings.SponsoredMessageMenu_Hide, textColor: .primary, textLayout: .twoLinesMax, textFont: .custom(Font.regular(presentationData.listsFontSize.baseDisplaySize - 1.0)), badge: nil, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Restrict"), color: theme.actionSheet.controlAccentColor)
            }, iconSource: nil, action: { c, _ in
                c.dismiss(completion: {
        ...
        if chatPresentationInterfaceState.copyProtectionEnabled {
            ...
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuCopy, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Copy"), color: theme.actionSheet.controlAccentColor)
            }, action: { _, f in
        ...
        if let author = message.author, let addressName = author.addressName {
            let link = "https://ello.team/\(addressName)"
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuCopyLink, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Link"), color: theme.actionSheet.controlAccentColor)
            }, action: { _, f in
    ...
    if messages.count == 1 {
        for media in messages[0].media {
            ...
            } else if media is ElloAppMediaAction || media is ElloAppMediaExpiredContent {
                isAction = true
            } else if let image = media as? ElloAppMediaImage {
                if !messages[0].containsSecretMedia {
                    loadCopyMediaResource = largestImageRepresentation(image.representations)?.resource
                }
            } else if let dice = media as? ElloAppMediaDice {
...
//            if let channel = messages[0].peers[messages[0].id.peerId] as? ElloAppChannel {
//                if !isAction {
//                    canPin = channel.hasPermission(.pinMessages)
//                }
//            } else if let group = messages[0].peers[messages[0].id.peerId] as? ElloAppGroup {
//                if !isAction {
//                    switch group.role {
//                    case .creator, .admin:
//                        canPin = true
...
//                    }
//                }
//            } else if let _ = messages[0].peers[messages[0].id.peerId] as? ElloAppUser, chatPresentationInterfaceState.explicitelyCanPinMessages {
//                if !isAction {
//                    canPin = true
//                }
//            }
//        }
    ...
    if let peer = messages[0].peers[messages[0].id.peerId] {
        ...
        if !(peer is ElloAppSecretChat) && messages[0].id.namespace != Namespaces.Message.Cloud {
            canPin = false
            canReply = false
        }
    }
    ...
    let loadLimits = context.engine.data.get(
        ElloAppEngine.EngineData.Item.Configuration.Limits(),
        ElloAppEngine.EngineData.Item.Configuration.App()
    )
    ...
    let infoSummaryData = context.engine.data.get(
        ElloAppEngine.EngineData.Item.Peer.LinkedDiscussionPeerId(id: messages[0].id.peerId),
        ElloAppEngine.EngineData.Item.Peer.CanViewStats(id: messages[0].id.peerId),
        ElloAppEngine.EngineData.Item.Peer.ParticipantCount(id: messages[0].id.peerId)
    )
    ...
    let readCounters = context.engine.data.get(ElloAppEngine.EngineData.Item.Messages.PeerReadCounters(id: messages[0].id.peerId))
    ...
    |> map {
        ...
        infoSummaryData,
        readCounters,
        messageViewsPrivacyTips,
        availableReactions,
        sharedData,
        notificationSoundList -> (MessageContextMenuData, [MessageId: ChatUpdatingMessageMedia], InfoSummaryData, AppConfiguration, Bool, Int32, AvailableReactions?, TranslationSettings, LoggingSettings, NotificationSoundList?) in
        ...
        let messageContextMenuData = MessageContextMenuData(
        ...
        return (messageContextMenuData, updatingMessageMedia, infoSummaryData, appConfig, isMessageRead, messageViewsPrivacyTips, availableReactions, translationSettings, loggingSettings, notificationSoundList)
    }
    ...
    |> map { data, updatingMessageMedia, infoSummaryData, appConfig, isMessageRead, messageViewsPrivacyTips, availableReactions, translationSettings, loggingSettings, notificationSoundList -> ContextController.Items in
        ...
        if let starStatus = data.starStatus {
            ...
            for media in messages[0].media {
                if let file = media as? ElloAppMediaFile, file.isPremiumSticker {
            ...
            if !isPremiumSticker || chatPresentationInterfaceState.isPremium {
                actions.append(.action(ContextMenuActionItem(text: starStatus ? chatPresentationInterfaceState.strings.Stickers_RemoveFromFavorites : chatPresentationInterfaceState.strings.Stickers_AddToFavorites, icon: { theme in
                    return generateTintedImage(image: starStatus ? UIImage(bundleImageName: "Chat/Context Menu/Unfave") : UIImage(bundleImageName: "Chat/Context Menu/Fave"), color: theme.actionSheet.controlAccentColor)
                }, action: { _, f in
        ...
        if data.messageActions.options.contains(.rateCall) {
            ...
            for media in message.media {
                if let action = media as? ElloAppMediaAction, case let .phoneCall(id, discardReason, _, isVideoValue) = action.action {
                    ...
                    if discardReason != .busy && discardReason != .missed {
                        if let logName = callLogNameForId(id: id, account: context.account) {
                            ...
                            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Call_ShareStats, icon: { theme in
                                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Forward"), color: theme.actionSheet.controlAccentColor)
                            }, action: { _, f in
                                ...
                                controller.peerSelected = { [weak controller] peer in
                                    ...
                                    if let strongController = controller {
                                        ...
                                        let file = ElloAppMediaFile(fileId: MediaId(namespace: Namespaces.Media.LocalFile, id: id), partialReference: nil, resource: LocalFileReferenceMediaResource(localFilePath: logPath, randomId: id), previewRepresentations: [], videoThumbnails: [], immediateThumbnailData: nil, mimeType: "application/text", size: nil, attributes: [.FileName(fileName: "CallStats.log")])
            ...
            if let callId = callId {
                actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Call_RateCall, icon: { theme in
                    return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Rate"), color: theme.actionSheet.controlAccentColor)
                }, action: { _, f in
        ...
        if !hasRateTranscription {
            for media in message.media {
                if let file = media as? ElloAppMediaFile, let size = file.size, size < 1 * 1024 * 1024, let duration = file.duration, duration < 60, (["audio/mpeg", "audio/mp3", "audio/mpeg3", "audio/ogg"] as [String]).contains(file.mimeType.lowercased()) {
                    ...
                    if !isAlreadyAdded {
                        ...
                        actions.append(.action(ContextMenuActionItem(text: presentationData.strings.Chat_SaveForNotifications, icon: { theme in
                            return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/DownloadTone"), color: theme.actionSheet.controlAccentColor)
                        }, action: { _, f in
                            f(.default)
            ...
            if context.sharedContext.immediateExperimentalUISettings.enableReactionOverrides {
                for media in message.media {
                    if let file = media as? ElloAppMediaFile, file.isAnimatedSticker {
        ...
        if !isPinnedMessages, !isReplyThreadHead, data.canReply {
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuReply, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Reply"), color: theme.actionSheet.controlAccentColor)
            }, action: { _, f in
        ...
        if data.messageActions.options.contains(.sendScheduledNow) {
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.ScheduledMessages_SendNow, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Resend"), color: theme.actionSheet.controlAccentColor)
            }, action: { _, f in
        ...
        if data.messageActions.options.contains(.editScheduledTime) {
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.ScheduledMessages_EditTime, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Schedule"), color: theme.actionSheet.controlAccentColor)
            }, action: { _, f in
        ...
        /* EM-3413: Hide Send logs buttons
         if (loggingSettings.logToFile || loggingSettings.logToConsole) && !downloadableMediaResourceInfos.isEmpty {
            actions.append(.action(ContextMenuActionItem(text: "Send Logs", icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Message"), color: theme.actionSheet.controlAccentColor)
            }, action: { _, f in
 ...
 */
        ...
        if case .peer = chatPresentationInterfaceState.chatLocation, let channel = chatPresentationInterfaceState.renderedPeer?.peer as? ElloAppChannel, case .group = channel.info {
        ...
        if let _ = threadId, !isPinnedMessages {
            ...
            actions.append(.action(ContextMenuActionItem(text: text, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Replies"), color: theme.actionSheet.controlAccentColor)
            }, action: { c, _ in
                c.dismiss(completion: {
        ...
        let isMigrated: Bool
        if chatPresentationInterfaceState.renderedPeer?.peer is ElloAppChannel && message.id.peerId.namespace == Namespaces.Peer.CloudGroup {
            isMigrated = true
        } else {
            isMigrated = false
        }
        ...
        if data.canEdit && !isPinnedMessages && !isMigrated {
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_MessageDialogEdit, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Edit"), color: theme.actionSheet.controlAccentColor)
            }, action: { c, f in
        ...
        var activePoll: ElloAppMediaPoll?
        for media in message.media {
            if let poll = media as? ElloAppMediaPoll, !poll.isClosed, message.id.namespace == Namespaces.Message.Cloud, poll.pollId.namespace == Namespaces.Media.CloudPoll {
        ...
        if let activePoll = activePoll, let voters = activePoll.results.voters {
            ...
            if hasSelected, case .poll = activePoll.kind {
                actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_UnvotePoll, icon: { theme in
                    return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Unvote"), color: theme.actionSheet.controlAccentColor)
                }, action: { _, f in
        ...
        if data.canPin && !isMigrated, case .peer = chatPresentationInterfaceState.chatLocation {
            ...
            if let pinnedSelectedMessageId = pinnedSelectedMessageId {
                actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_Unpin, icon: { theme in
                    return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Unpin"), color: theme.actionSheet.controlAccentColor)
                }, action: { c, _ in
                ...
                actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_Pin, icon: { theme in
                    return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Pin"), color: theme.actionSheet.controlAccentColor)
                }, action: { c, _ in
        ...
        if messageText.isEmpty {
            for media in message.media {
                if let poll = media as? ElloAppMediaPoll {
                    isPoll = true
                    var text = poll.text
        ...
        if (!messageText.isEmpty || resourceAvailable || diceEmoji != nil) && !chatPresentationInterfaceState.copyProtectionEnabled && !message.isCopyProtected() {
            ...
            var isExpired = false
            for media in message.media {
                if let _ = media as? ElloAppMediaExpiredContent {
                    isExpired = true
                }
            }
            if !isExpired {
                    ...
                    actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuCopy, icon: { theme in
                        ...
                        if let diceEmoji = diceEmoji {
                            ...
                            if resourceAvailable {
                                for media in message.media {
                                    if let image = media as? ElloAppMediaImage, let largest = largestImageRepresentation(image.representations) {
                ...
                if let peer = chatPresentationInterfaceState.renderedPeer?.chatMainPeer as? ElloAppChannel, !(peer.addressName ?? "").isEmpty {
                ...
                if canTranslate {
                    actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuTranslate, icon: { theme in
                        return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Translate"), color: theme.actionSheet.controlAccentColor)
                    }, action: { _, f in
            ...
            if resourceAvailable, !message.containsSecretMedia {
                ...
                var isVideo = false
                for media in message.media {
                    if let image = media as? ElloAppMediaImage, let _ = largestImageRepresentation(image.representations) {
                        mediaReference = ImageMediaReference.standalone(media: image).abstract
                        break
                    } else if let file = media as? ElloAppMediaFile, file.isVideo {
                ...
                if let mediaReference = mediaReference {
                    actions.append(.action(ContextMenuActionItem(text: isVideo ? chatPresentationInterfaceState.strings.Gallery_SaveVideo : chatPresentationInterfaceState.strings.Gallery_SaveImage, icon: { theme in
                        return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Save"), color: theme.actionSheet.controlAccentColor)
                    }, action: { _, f in
        ...
        if let activePoll = activePoll, messages[0].forwardInfo == nil {
            ...
            if canStopPoll {
                ...
                actions.append(.action(ContextMenuActionItem(text: stopPollAction, icon: { theme in
                    return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/StopPoll"), color: theme.actionSheet.controlAccentColor)
                }, action: { _, f in
        ...
        if let message = messages.first, message.id.namespace == Namespaces.Message.Cloud, let channel = message.peers[message.id.peerId] as? ElloAppChannel, !(message.media.first is ElloAppMediaAction), !channel.isPrivate, !isReplyThreadHead, !isMigrated {
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuCopyLink, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Link"), color: theme.actionSheet.controlAccentColor)
            }, action: { _, f in
        ...
        if messages.count == 1 {
            ...
            if !hasAutoremove {
                for media in message.media {
                    if let action = media as? ElloAppMediaAction {
                        if let channel = message.peers[message.id.peerId] as? ElloAppChannel {
                    ...
                    if let file = media as? ElloAppMediaFile, !chatPresentationInterfaceState.copyProtectionEnabled && !message.isCopyProtected() {
                        if file.isVideo {
        ...
        if data.messageActions.options.contains(.viewStickerPack) {
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.StickerPack_ViewPack, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Sticker"), color: theme.actionSheet.controlAccentColor)
            }, action: { _, f in
        ...
        if data.messageActions.options.contains(.report) {
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuReport, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Report"), color: theme.actionSheet.controlAccentColor)
        ...
        if let channel = message.peers[message.id.peerId] as? ElloAppChannel, case .broadcast = channel.info, !isMigrated {
            var views: Int = 0
            for attribute in message.attributes {
            ...
            if infoSummaryData.canViewStats, views >= 100 {
                actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextViewStats, icon: { theme in
                    return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Statistics"), color: theme.actionSheet.controlAccentColor)
                }, action: { c, _ in
                    c.dismiss(completion: {
        ...
        if isReplyThreadHead {
            actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.SharedMedia_ViewInChat, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/GoToMessage"), color: theme.actionSheet.controlAccentColor)
            }, action: { c, _ in
                c.dismiss(completion: {
        ...
        if !isPinnedMessages, !isReplyThreadHead, data.canSelect {
            if !selectAll || messages.count == 1 {
                actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuSelect, icon: { theme in
                    return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Select"), color: theme.actionSheet.controlAccentColor)
                }, action: { _, f in
            ...
            if messages.count > 1 {
                actions.append(.action(ContextMenuActionItem(text: chatPresentationInterfaceState.strings.Conversation_ContextMenuSelectAll(Int32(messages.count)), icon: { theme in
                    return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/SelectAll"), color: theme.actionSheet.controlAccentColor)
                }, action: { _, f in
...
func chatAvailableMessageActionsImpl(engine: ElloAppEngine, accountPeerId: PeerId, messageIds: Set<MessageId>, messages: [MessageId: Message] = [:], peers: [PeerId: Peer] = [:]) -> Signal<ChatAvailableMessageActions, NoError> {
    return engine.data.get(
        ElloAppEngine.EngineData.Item.Configuration.Limits(),
        EngineDataMap(Set(messageIds.map(\.peerId)).map(ElloAppEngine.EngineData.Item.Peer.Peer.init)),
        EngineDataMap(Set(messageIds).map(ElloAppEngine.EngineData.Item.Messages.Message.init))
    )
    |> map { limitsConfiguration, peerMap, messageMap -> ChatAvailableMessageActions in
        ...
        for id in messageIds {
            ...
            if let message = getMessage(id) {
                ...
                for media in message.media {
                    if let invoice = media as? ElloAppMediaInvoice, let _ = invoice.extendedMedia {
                        isShareProtected = true
                    } else if let file = media as? ElloAppMediaFile, file.isSticker {
                    ...
                    } else if let action = media as? ElloAppMediaAction, case .phoneCall = action.action {
                ...
                if id.namespace == Namespaces.Message.ScheduledCloud {
                    ...
                    if let peer = getPeer(id.peerId), let channel = peer as? ElloAppChannel {
                        if !message.flags.contains(.Incoming) {
                    ...
                    var isDice = false
                    for media in message.media {
                        if media is ElloAppMediaAction || media is ElloAppMediaExpiredContent {
                            isAction = true
                        }
                        if media is ElloAppMediaDice {
                            isDice = true
                        }
                    }
                    if let channel = peer as? ElloAppChannel {
                        ...
                        if channel.hasPermission(.banMembers), case .group = channel.info {
                            if message.flags.contains(.Incoming) {
                                if message.author is ElloAppUser {
                                    if !hadBanPeerId {
                                        hadBanPeerId = true
                                ...
                                } else if message.author is ElloAppChannel {
                                    if !hadBanPeerId {
                                        hadBanPeerId = true
                                        banPeer = message.author
                                    } else if banPeer?.id != message.author?.id {
...
final class ChatReadReportContextItem: ContextMenuCustomItem {
    ...
    fileprivate let action: (ContextControllerProtocol, @escaping (ContextMenuActionResult) -> Void, MessageReadStats?, [StickerPackCollectionInfo], ElloAppMediaFile?) -> Void

    init(context: AccountContext, message: Message, hasReadReports: Bool, stats: MessageReadStats?, action: @escaping (ContextControllerProtocol, @escaping (ContextMenuActionResult) -> Void, MessageReadStats?, [StickerPackCollectionInfo], ElloAppMediaFile?) -> Void) {
...
private final class ChatReadReportContextItemNode: ASDisplayNode, ContextMenuCustomNode, ContextActionNodeProtocol {
    ...
    private var firstCustomEmojiReaction: ElloAppMediaFile?
    ...
    init(presentationData: PresentationData, item: ChatReadReportContextItem, getController: @escaping () -> ContextControllerProtocol?, actionSelected: @escaping (ContextMenuActionResult) -> Void) {
        ...
        if let reactionsAttribute = item.message.reactionsAttribute, !reactionsAttribute.reactions.isEmpty {
            self.iconNode.image = generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Reactions"), color: presentationData.theme.actionSheet.controlAccentColor)
        } else {
            self.iconNode.image = generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Read"), color: presentationData.theme.actionSheet.controlAccentColor)
        }
        ...
        if !customEmojiFiles.isEmpty {
            ...
            |> mapToSignal { customEmoji -> Signal<([StickerPackCollectionInfo], ElloAppMediaFile?), NoError> in
                var stickerPackSignals: [Signal<StickerPackCollectionInfo?, NoError>] = []
                var existingIds = Set<Int64>()
                var firstCustomEmojiReaction: ElloAppMediaFile?
                ...
                |> map { stickerPacks -> ([StickerPackCollectionInfo], ElloAppMediaFile?) in
...
private final class ChatRateTranscriptionContextItemNode: ASDisplayNode, ContextMenuCustomNode, ContextActionNodeProtocol {
    ...
    init(presentationData: PresentationData, item: ChatRateTranscriptionContextItem, getController: @escaping () -> ContextControllerProtocol?, actionSelected: @escaping (ContextMenuActionResult) -> Void, action: @escaping (Bool) -> Void) {
        ...
        self.upButtonImageNode.image = generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/ThumbsUp"), color: presentationData.theme.contextMenu.badgeFillColor, backgroundColor: nil)
        self.upButtonImageNode.isUserInteractionEnabled = false
        
        self.downButtonImageNode = ASImageNode()
        self.downButtonImageNode.image = generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/ThumbsDown"), color: presentationData.theme.contextMenu.badgeFillColor, backgroundColor: nil)
