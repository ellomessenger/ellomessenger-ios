...
public final class NotificationViewControllerImpl {
    ...
    public func viewDidLoad(view: UIView) {
        ...
        if sharedAccountContext == nil {
            ...
            let applicationBindings = ElloAppApplicationBindings(isMainApp: false, appBundleId: self.initializationData.appBundleId, appBuildType: self.initializationData.appBuildType, containerPath: self.initializationData.appGroupPath, appSpecificScheme: "tgapp", openUrl: { _ in
    ...
    public func didReceive(_ notification: UNNotification, view: UIView) {
        ...
        if let image = media as? ElloAppMediaImage, let thumbnailRepresentation = imageRepresentationLargerThan(image.representations, size: PixelDimensions(width: 120, height: 120)), let largestRepresentation = largestImageRepresentation(image.representations) {
