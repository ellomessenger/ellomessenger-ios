...
private func extractFileMedia(_ message: Message) -> ElloAppMediaFile? {
    var file: ElloAppMediaFile?
    for media in message.media {
        if let media = media as? ElloAppMediaFile {
            file = media
            break
        } else if let media = media as? ElloAppMediaWebpage, case let .Loaded(content) = media.content, let f = content.file {
            file = f
            break
        }
    }
...
final class MessageMediaPlaylistItem: SharedMediaPlaylistItem {
    ...
    var playbackData: SharedMediaPlaybackData? {
        if let file = extractFileMedia(self.message) {
            ...
            let source = SharedMediaPlaybackDataSource.elloappFile(reference: fileReference, isCopyProtected: self.message.isCopyProtected())
    ...
    private func updateState() {
        ...
        if item?.message.id != self.currentlyObservedMessageId {
            ...
            if let id = item?.message.id {
                self.currentlyObservedMessageDisposable.set((self.context.engine.data.subscribe(ElloAppEngine.EngineData.Item.Messages.Message(id: id))
                |> filter { message in
                    if let _ = message {
                        return false
                    } else {
                                ...
                                if case let .random(previous) = navigation, previous {
                                    ...
                                    inner: while true {
                                        if let id = playbackStack.pop() {
                                            inputIndex = self.context.engine.data.get(ElloAppEngine.EngineData.Item.Messages.Message(id: id))
                                            |> map { message in
