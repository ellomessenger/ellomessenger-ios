...
private enum InviteLinksListEntry: ItemListNodeEntry {
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        switch self {
            ...
            case let .mainLink(_, invite, peers, importersCount, _):
            return ItemListPermanentInviteLinkItem(
                ...
                peers: peers,
                displayButton: true,
                displayImporters: true,//!isPublic,
                buttonColor: nil,
...
private func inviteLinkListControllerEntries(presentationData: PresentationData,
                                             exportedInvitation: EngineExportedPeerInvitation?,
                                             peer: EnginePeer?,
                                             invites: [ExportedInvitation]?,
                                             revokedInvites: [ExportedInvitation]?,
                                             importers: PeerInvitationImportersState?,
                                             creators: [ExportedInvitationCreator],
                                             admin: ExportedInvitationCreator?,
                                             tick: Int32) -> [InviteLinksListEntry] {
    ...
    if admin == nil {
        let helpText: String
        if case .user(let user) = peer {
            isProfile = true
            isPublicProfile = user.isPublic
            helpText = presentationData.strings.InviteLink_CreatePrivateLinkHelpUser
    ...
    var isPublic = false
    if isProfile, !isPublicProfile, let invites, let inviteLink = invites.first?.link {
        mainInvite = .link(link: inviteLink, title: nil, isPermanent: true, requestApproval: false, isRevoked: false, adminId: EnginePeer.Id(0), date: 0, startDate: nil, expireDate: nil, usageLimit: nil, count: nil, requestedCount: nil)
    } else if let peer = peer, let address = peer.addressName, !address.isEmpty && admin == nil {
        mainInvite = .link(link: "https://ello.team/\(address)", title: nil, isPermanent: true, requestApproval: false, isRevoked: false, adminId: EnginePeer.Id(0), date: 0, startDate: nil, expireDate: nil, usageLimit: nil, count: nil, requestedCount: nil)
        isPublic = true
    ...
    entries.append(.mainLinkHeader(presentationData.theme, isProfile ? "" : presentationData.strings.InviteLink_InviteLink.uppercased()))
    ...
    if (hasLinks || admin == nil) && !isProfile {
    ...
    if let peer = admin?.peer.peer as? ElloAppUser, peer.botInfo != nil {
        canEditLinks = false
    }
...
public func inviteLinkListController(context: AccountContext,
                                     updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil,
                                     peerId: EnginePeer.Id,
                                     admin: ExportedInvitationCreator?) -> ViewController {
    ...
    let arguments = InviteLinkListControllerArguments(context: context, shareMainLink: { invite in
        ...
        items.append(.action(ContextMenuActionItem(text: presentationData.strings.InviteLink_ContextGetQRCode, icon: { theme in
            ...
            |> deliverOnMainQueue).start(next: { peer in
                controller.present(context.sharedContext.makeChatQrCodeScreen(context: context, peer: peer, inviteLink: invite.link ?? ""), in: .window(.root))
            })
        })))
        
        if case let .link(_, _, _, _, _, adminId, _, _, _, _, _, _) = invite, adminId.toInt64() != 0 {
            items.append(.action(ContextMenuActionItem(text: presentationData.strings.GroupInfo_InviteLink_RevokeLink,
                                                       textColor: .primary,
                                                       icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Delete"),
        ...
        let contextController = ContextController(account: context.account,
                                                  presentationData: presentationData,
                                                  source: .reference(InviteLinkContextReferenceContentSource(controller: controller,
                                                                                                             sourceNode: node)),
    ...
    |> map { presentationData, exportedInvitation, peer, importersContext, importers, invites, revokedInvites, creators, tick -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        let listState = ItemListNodeState(presentationData: ItemListPresentationData(presentationData),
                                          entries: inviteLinkListControllerEntries(presentationData: presentationData,
                                                                                   ...
                                                                                   peer: peer,
                                                                                   invites: invites.hasLoadedOnce ? invites.invitations : nil,
                                                                                   revokedInvites: revokedInvites.hasLoadedOnce ? revokedInvites.invitations : nil,
                                                                                   importers: importers,
                                                                                   creators: creators,
                                                                                   admin: admin,
                                                                                   tick: tick),
