...
//    private let headerNode: TextNode
    ...
    private var copyLinkButtonNode: SolidRoundedButtonNode?
    ...
    public init() {
        ...
        self.fieldNode.backgroundColor = UIColor(rgb: 0xEEEEEF)
        self.fieldNode.cornerRadius = 10.0
        
//        self.headerNode = TextNode()
...
//        self.addSubnode(self.headerNode)
        ...
        self.copyLinkButtonNode?.pressed = { [weak self] in
...
//        let headerLayout = TextNode.asyncLayout(self.headerNode)
        ...
        return { item, params, neighbors in
            ...
            let alignCentrally = item.isCentral ?? (item.invite?.link?.contains("joinchat") ?? true)
            
            let addressFont = Font.regular(!alignCentrally && params.width == 320 ? floor(item.presentationData.fontSize.itemListBaseFontSize * 15.0 / 17.0) : item.presentationData.fontSize.itemListBaseFontSize * 16.0 / 17.0)
            let titleFont = Font.regular(item.presentationData.fontSize.itemListBaseFontSize * 16.0 / 17.0)
            
            let constrainedWidth = alignCentrally 
            ? params.width - leftInset - rightInset - 90.0
            : params.width - leftInset - rightInset - 60.0
            
//            let (headerLayout, headerApply) = headerLayout(TextNodeLayoutArguments(
//                attributedString: NSAttributedString(string: item.presentationData.strings.InviteLink_InviteLink,
//                                                     font: Font.regular(12),
//                                                     textColor: .gray),
//                backgroundColor: nil,
//                maximumNumberOfLines: 1,
//                truncationType: .middle,
//                constrainedSize: CGSize(width: constrainedWidth,
...
//                insets: UIEdgeInsets()))
            ...
            let (addressLayout, addressApply) = makeAddressLayout(TextNodeLayoutArguments(
                attributedString: NSAttributedString(
                    string: item.invite?.link ?? "",/*item.invite.flatMap({ $0.link?.replacingOccurrences(of: "https://", with: "") }) ?? "",*/
                    font: addressFont,
            ...
            let (invitedPeersLayout, invitedPeersApply) = makeInvitedPeersLayout(
                TextNodeLayoutArguments(
                    attributedString: NSAttributedString(string: subtitle, font: titleFont, textColor: subtitleColor),
                    backgroundColor: nil,
                    maximumNumberOfLines: 1,
                    truncationType: .end,
                    constrainedSize: CGSize(
                        width: params.width - params.rightInset - 20.0 - leftInset - rightInset,
                        height: CGFloat.greatestFiniteMagnitude),
                    alignment: .natural,
                    cutout: nil,
                    insets: UIEdgeInsets())
            ...
            var height = verticalInset * 2.0 + fieldHeight + fieldSpacing + buttonHeight + 40.0
...
//                    strongSelf.activateArea.accessibilityValue = item.label
                    ...
                    if let _ = updatedTheme {
                        ...
                        strongSelf.addressButtonIconNode.image = UIImage(named: "MoreLink")
                    }
...
//                    strongSelf.headerNode.frame = CGRect(origin: CGPoint(
//                        x: fieldFrame.minX + 14.0,
//                        y: fieldFrame.minY + floorToScreenPixels((fieldFrame.height - headerLayout.size.height) / 2.0) - 16),
//                                                          size: headerLayout.size)
                    
                    strongSelf.addressNode.frame = CGRect(origin: CGPoint(
                        x: fieldFrame.minX + (alignCentrally ? floorToScreenPixels((fieldFrame.width - addressLayout.size.width) / 2.0) : 14.0),
                        y: fieldFrame.minY + floorToScreenPixels((fieldFrame.height - addressLayout.size.height) / 2.0)),
...
//                        }
                        ...
                        shareButtonNode.icon = UIImage(named: "ShareLink")
                        shareButtonNode.title = item.presentationData.strings.InviteLink_ContextShare
                    ...
                    let buttonWidth = (contentSize.width - 3.0 * leftInset) / 2.0
                    ...
                    let copyLinkButtonNode: SolidRoundedButtonNode
                    if let currentCopyLinkButtonNode = strongSelf.copyLinkButtonNode {
                        copyLinkButtonNode = currentCopyLinkButtonNode
                    } else {
                        let buttonTheme: SolidRoundedButtonTheme
                        if let buttonColor = item.buttonColor {
                            buttonTheme = SolidRoundedButtonTheme(
                                backgroundColor: buttonColor,
                                foregroundColor: item.presentationData.theme.list.itemCheckColors.foregroundColor)
                        } else {
                            buttonTheme = SolidRoundedButtonTheme(theme: item.presentationData.theme)
                        }
                        
                        copyLinkButtonNode = SolidRoundedButtonNode(
                            theme: buttonTheme,
                            font: .regular,
                            fontSize: 16.0,
                            height: buttonHeight,
                            cornerRadius: 11.0)
                        copyLinkButtonNode.icon = UIImage(named: "CopyLink")
                        copyLinkButtonNode.title = item.presentationData.strings.InviteLink_ContextCopyLink.capitalized
                        
                        copyLinkButtonNode.pressed = { [weak self] in
                            self?.item?.copyAction?()
                        }
                        
                        strongSelf.addSubnode(copyLinkButtonNode)
                        strongSelf.copyLinkButtonNode = copyLinkButtonNode
                    }

                    let _ = copyLinkButtonNode.updateLayout(width: buttonWidth, transition: .immediate)
                    copyLinkButtonNode.frame = CGRect(x: 2.0 * leftInset + buttonWidth, y: verticalInset + fieldHeight + fieldSpacing, width: buttonWidth, height: buttonHeight)
                    ...
                    if let avatarsContent = strongSelf.avatarsContent {
                        ...
                        let avatarsNodeFrame = CGRect(
                            origin: CGPoint(
                                x: floorToScreenPixels((params.width - totalWidth) / 2.0),
                                y: fieldFrame.maxY + 76.0),
                            size: avatarsSize)
                    ...
                    strongSelf.invitedPeersNode.frame = CGRect(
                        origin: CGPoint(
                            x: leftOrigin,
                            y: strongSelf.avatarsNode.frame.midY - invitedPeersLayout.size.height / 2),
                        size: invitedPeersLayout.size)
                    
                    strongSelf.avatarsButtonNode.frame = CGRect(x: floorToScreenPixels((params.width - totalWidth) / 2.0), y: fieldFrame.maxY + 76.0, width: totalWidth, height: 32.0)
                    ...
                    strongSelf.copyLinkButtonNode?.isHidden = !item.displayButton
