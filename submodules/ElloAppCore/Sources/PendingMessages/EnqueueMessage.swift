...
func augmentMediaWithReference(_ mediaReference: AnyMediaReference) -> Media {
    if let file = mediaReference.media as? ElloAppMediaFile {
        if file.partialReference != nil {
            return file
        } else {
            return file.withUpdatedPartialReference(mediaReference.partial)
        }
    } else if let image = mediaReference.media as? ElloAppMediaImage {
...
private func convertForwardedMediaForSecretChat(_ media: Media) -> Media {
    if let file = media as? ElloAppMediaFile {
        return ElloAppMediaFile(fileId: MediaId(namespace: Namespaces.Media.LocalFile, id: Int64.random(in: Int64.min ... Int64.max)), partialReference: file.partialReference, resource: file.resource, previewRepresentations: file.previewRepresentations, videoThumbnails: file.videoThumbnails, immediateThumbnailData: file.immediateThumbnailData, mimeType: file.mimeType, size: file.size, attributes: file.attributes)
    } else if let image = media as? ElloAppMediaImage {
        return ElloAppMediaImage(imageId: MediaId(namespace: Namespaces.Media.LocalImage, id: Int64.random(in: Int64.min ... Int64.max)), representations: image.representations, immediateThumbnailData: image.immediateThumbnailData, reference: image.reference, partialReference: image.partialReference, flags: [])
    } else {
        return media
    }
}
        ...
        for (transformedMedia, message) in updatedMessages {
            ...
            switch message {
                case let .message(text, requestedAttributes, inlineStickers, mediaReference, replyToMessageId, localGroupingKey, _, _):
                    ...
                    if let peer = peer as? ElloAppSecretChat {
                        var isAction = false
                        if let _ = mediaReference?.media as? ElloAppMediaAction {
                            isAction = true
                        }
                    ...
                    for attribute in filterMessageAttributesForOutgoingMessage(requestedAttributes) {
                        if let attribute = attribute as? AutoremoveTimeoutMessageAttribute {
                            if let _ = peer as? ElloAppSecretChat {
                    ...
                    if let file = mediaReference?.media as? ElloAppMediaFile, file.isVoice || file.isInstantVideo {
                        ...
                        if let attribute = attribute as? TextEntitiesMessageAttribute {
                            ...
                            for entity in attribute.entities {
                                if case .Hashtag = entity.type {
                                    ...
                                    if let file = inlineStickers[mediaId] as? ElloAppMediaFile {
                                        emojiItems.append(RecentEmojiItem(.file(file)))
                                    } else if let file = transaction.getMedia(mediaId) as? ElloAppMediaFile {
                                        emojiItems.append(RecentEmojiItem(.file(file)))
                                    }
                                }
                            }
                    ...
                    for media in mediaList {
                        if let media = media as? ElloAppMediaMap, media.liveBroadcastingTimeout != nil {
                    ...
                    for attribute in attributes {
                        if let attribute = attribute as? OutgoingScheduleInfoMessageAttribute {
                            if attribute.scheduleTime == scheduleWhenOnlineTimestamp, let presence = peerPresence as? ElloAppUserPresence, case let .present(statusTimestamp) = presence.status, statusTimestamp >= timestamp {
                            } else {
                    ...
                    storeMessages.append(StoreMessage(peerId: peerId, namespace: messageNamespace, globallyUniqueId: randomId, groupingKey: localGroupingKey, threadId: threadId, timestamp: effectiveTimestamp, flags: flags, tags: tags, globalTags: globalTags, localTags: localTags, forwardInfo: nil, authorId: authorId, text: text, attributes: attributes, media: mediaList, likes: 0))
                case let .forward(source, grouping, requestedAttributes, _):
                    ...
                    if let sourceMessage = sourceMessage, let author = sourceMessage.author ?? sourceMessage.peers[sourceMessage.id.peerId] {
                        ...
                        storeMessages.append(StoreMessage(peerId: peerId, namespace: messageNamespace, globallyUniqueId: randomId, groupingKey: localGroupingKey, threadId: threadId, timestamp: effectiveTimestamp, flags: flags, tags: tags, globalTags: globalTags, localTags: [], forwardInfo: forwardInfo, authorId: authorId, text: messageText, attributes: attributes, media: augmentedMediaList, likes: 0))
                    }
            }
        }
        var messageIds: [MessageId?] = []
