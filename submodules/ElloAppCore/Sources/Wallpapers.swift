...
public func elloappWallpapers(postbox: Postbox, network: Network, forceUpdate: Bool = false) -> Signal<[ElloAppWallpaper], NoError> {
    let fetch: ([ElloAppWallpaper]?, Int64?) -> Signal<[ElloAppWallpaper], NoError> = { current, hash in
        network.request(Api.functions.account.getWallPapers(hash: hash ?? 0))
        |> retryRequest
        |> mapToSignal { result -> Signal<([ElloAppWallpaper], Int64), NoError> in
            switch result {
                case let .wallPapers(hash, wallpapers):
                    var items: [ElloAppWallpaper] = []
                    var addedBuiltin = false
                    for apiWallpaper in wallpapers {
                        let wallpaper = ElloAppWallpaper(apiWallpaper: apiWallpaper)
        ...
        |> mapToSignal { items, hash -> Signal<[ElloAppWallpaper], NoError> in
            return postbox.transaction { transaction -> [ElloAppWallpaper] in
                var entries: [OrderedItemListEntry] = []
                for item in items {
                    var intValue = Int32(entries.count)
                    let id = MemoryBuffer(data: Data(bytes: &intValue, count: 4))
                    if let entry = CodableEntry(ElloAppWallpaperNativeCodable(item)) {
    ...
    if forceUpdate {
        ...
        return postbox.transaction { transaction -> ([ElloAppWallpaper], Int64?) in
            ...
            if items.count == 0 {
                ...
                var success = true
                var mappedItems: [ElloAppWallpaper] = []
                for item in items {
                    if let value = item.contents.get(ElloAppWallpaperNativeCodable.self)?.value {
        ...
        |> mapToSignal { current, hash -> Signal<[ElloAppWallpaper], NoError> in
...
public enum UploadWallpaperStatus {
    case progress(Float)
    case complete(ElloAppWallpaper)
}
                ...
                switch resultData {
                    ...
                    case let .inputFile(file):
                        ...
                        |> map { wallpaper -> (UploadWallpaperStatus, MediaResource?) in
                            return (.complete(ElloAppWallpaper(apiWallpaper: wallpaper)), result.resource)
                        }
                    default:
                        return .fail(.generic)
                }
...
public func getWallpaper(network: Network, slug: String) -> Signal<ElloAppWallpaper, GetWallpaperError> {
    ...
    |> map { wallpaper -> ElloAppWallpaper in
        return ElloAppWallpaper(apiWallpaper: wallpaper)
    }
}

public func saveWallpaper(account: Account, wallpaper: ElloAppWallpaper) -> Signal<Void, NoError> {
    return saveUnsaveWallpaper(account: account, wallpaper: wallpaper, unsave: false)
}

public func deleteWallpaper(account: Account, wallpaper: ElloAppWallpaper) -> Signal<Void, NoError> {
    return saveUnsaveWallpaper(account: account, wallpaper: wallpaper, unsave: true)
}

private func saveUnsaveWallpaper(account: Account, wallpaper: ElloAppWallpaper, unsave: Bool) -> Signal<Void, NoError> {
...
public func installWallpaper(account: Account, wallpaper: ElloAppWallpaper) -> Signal<Void, NoError> {
    guard case let .file(file) = wallpaper else {
        return .complete()
    }
