...
public extension Message {
    ...
    var isScam: Bool {
        ...
        for attribute in self.attributes {
            if let attribute = attribute as? InlineBotMessageAttribute, let peerId = attribute.peerId, let bot = self.peers[peerId] as? ElloAppUser, bot.isScam {
               return true
            }
        }
        return false
    ...
    var isFake: Bool {
        ...
        for attribute in self.attributes {
            if let attribute = attribute as? InlineBotMessageAttribute, let peerId = attribute.peerId, let bot = self.peers[peerId] as? ElloAppUser, bot.isFake {
               return true
            }
        }
        return false
...
func locallyRenderedMessage(message: StoreMessage, peers: [PeerId: Peer]) -> Message? {
    ...
    if let peer = peers[id.peerId] {
        ...
        if let group = peer as? ElloAppGroup, let migrationReference = group.migrationReference {
...
public extension Message {
    ...
    func effectivelyIncoming(_ accountPeerId: PeerId) -> Bool {
        if self.id.peerId == accountPeerId {
            ...
            return true
        } else if let channel = self.peers[self.id.peerId] as? ElloAppChannel, case .broadcast = channel.info {
            return true
        } else {
            return false
        }
    ...
    func isCopyProtected() -> Bool {
        if self.flags.contains(.CopyProtected) {
            return true
        } else if let group = self.peers[self.id.peerId] as? ElloAppGroup, group.flags.contains(.copyProtectionEnabled) {
            return true
        } else if let channel = self.peers[self.id.peerId] as? ElloAppChannel, channel.flags.contains(.copyProtectionEnabled) {
            return true
        } else {
            return false
        }
...
public func _internal_parseMediaAttachment(data: Data) -> Media? {
    ...
    if let photo = object as? Api.Photo {
        return elloappMediaImageFromApiPhoto(photo)
    } else if let file = object as? Api.Document {
        return elloappMediaFileFromApiDocument(file)
    } else {
        return nil
    }
}
