...
public func smallestVideoRepresentation(_ representations: [ElloAppMediaImage.VideoRepresentation]) -> ElloAppMediaImage.VideoRepresentation? {
...
public func smallestImageRepresentation(_ representations: [ElloAppMediaImageRepresentation]) -> ElloAppMediaImageRepresentation? {
    if representations.count == 0 {
        return nil
    } else {
        var dimensions = representations[0].dimensions
...
public func largestImageRepresentation(_ representations: [ElloAppMediaImageRepresentation]) -> ElloAppMediaImageRepresentation? {
    if representations.count == 0 {
        return nil
    } else {
        var dimensions = representations[0].dimensions
...
public func imageRepresentationLargerThan(_ representations: [ElloAppMediaImageRepresentation], size: PixelDimensions) -> ElloAppMediaImageRepresentation? {
    if representations.count == 0 {
        return nil
    } else {
        var index: Int?
...
public func progressiveImageRepresentation(_ representations: [ElloAppMediaImageRepresentation]) -> ElloAppMediaImageRepresentation? {
...
public func parseMediaData(data: Data) -> Media? {
    ...
    if let parseBuffer = parseBuffer, let object = Api.parse(parseBuffer) {
        if let photo = object as? Api.Photo {
            ...
            return elloappMediaFileFromApiDocument(document)
        }
    }
    return nil
}
