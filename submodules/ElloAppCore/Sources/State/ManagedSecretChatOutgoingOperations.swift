...
private func decryptedAttributes46(_ attributes: [ElloAppMediaFileAttribute], transaction: Transaction) -> [SecretApi46.DocumentAttribute] {
...
private func decryptedAttributes73(_ attributes: [ElloAppMediaFileAttribute], transaction: Transaction) -> [SecretApi73.DocumentAttribute] {
...
private func decryptedAttributes101(_ attributes: [ElloAppMediaFileAttribute], transaction: Transaction) -> [SecretApi101.DocumentAttribute] {
...
private func decryptedAttributes144(_ attributes: [ElloAppMediaFileAttribute], transaction: Transaction) -> [SecretApi144.DocumentAttribute] {
...
private func boxedDecryptedMessage(transaction: Transaction, message: Message, globallyUniqueId: Int64, uploadedFile: SecretChatOutgoingFile?, thumbnailData: [MediaId: (PixelDimensions, Data)], layer: SecretChatLayer) -> BoxedDecryptedMessage {
    ...
    if let media = media {
        if let image = media as? ElloAppMediaImage, let uploadedFile = uploadedFile, let largestRepresentation = largestImageRepresentation(image.representations) {
            let thumbW: Int32
            let thumbH: Int32
            let thumb: Buffer
        ...
        } else if let webpage = media as? ElloAppMediaWebpage {
            var url: String?
        ...
        } else if let location = media as? ElloAppMediaMap {
            switch layer {
                case .layer8:
                    break
                case .layer46:
            ...
            switch layer {
                ...
                let decryptedMedia: SecretApi46.DecryptedMessageMedia = .decryptedMessageMediaContact(username: contact.username, firstName: contact.firstName, lastName: contact.lastName, userId: Int32(contact.userid))
                    flags |= (1 << 9)
                    return .layer46(.decryptedMessage(flags: flags, randomId: globallyUniqueId, ttl: messageAutoremoveTimeout, message: message.text, media: decryptedMedia, entities: nil, viaBotName: viaBotName, replyToRandomId: replyGlobalId))
                case .layer73:
                    if muted {
                    ...
                    let decryptedMedia: SecretApi73.DecryptedMessageMedia = .decryptedMessageMediaContact(username: contact.username, firstName: contact.firstName, lastName: contact.lastName, userId: Int32(contact.userid))
                    flags |= (1 << 9)
                    return .layer73(.decryptedMessage(flags: flags, randomId: globallyUniqueId, ttl: messageAutoremoveTimeout, message: message.text, media: decryptedMedia, entities: decryptedEntites, viaBotName: viaBotName, replyToRandomId: replyGlobalId, groupedId: message.groupingKey))
                case .layer101:
                    if muted {
                    ...
                    let decryptedMedia: SecretApi101.DecryptedMessageMedia = .decryptedMessageMediaContact(phoneNumber: contact.username, firstName: contact.firstName, lastName: contact.lastName, userId: Int32(contact.userid))
                    flags |= (1 << 9)
                    return .layer101(.decryptedMessage(flags: flags, randomId: globallyUniqueId, ttl: messageAutoremoveTimeout, message: message.text, media: decryptedMedia, entities: decryptedEntites, viaBotName: viaBotName, replyToRandomId: replyGlobalId, groupedId: message.groupingKey))
                case .layer144:
                    if muted {
                    ...
                    let decryptedMedia: SecretApi144.DecryptedMessageMedia = .decryptedMessageMediaContact(phoneNumber: contact.username, firstName: contact.firstName, lastName: contact.lastName, userId: Int32(contact.userid))
                    flags |= (1 << 9)
                    return .layer144(.decryptedMessage(flags: flags, randomId: globallyUniqueId, ttl: messageAutoremoveTimeout, message: message.text, media: decryptedMedia, entities: decryptedEntites, viaBotName: viaBotName, replyToRandomId: replyGlobalId, groupedId: message.groupingKey))
            }
        }
...
private func sendMessage(auxiliaryMethods: AccountAuxiliaryMethods, postbox: Postbox, network: Network, messageId: MessageId, file: SecretChatOutgoingFile?, tagLocalIndex: Int32, wasDelivered: Bool, layer: SecretChatLayer) -> Signal<Void, NoError> {
    ...
    |> mapToSignal { thumbnailData -> Signal<Void, NoError> in
        return postbox.transaction { transaction -> Signal<Void, NoError> in
            if let state = transaction.getPeerChatState(messageId.peerId) as? SecretChatState, let peer = transaction.getPeer(messageId.peerId) as? ElloAppSecretChat {
                    ...
                    |> mapToSignal { result in
                        return postbox.transaction { transaction -> Void in
                            ...
                            transaction.updateMessage(message.id, update: { currentMessage in
                                ...
                                return .update(StoreMessage(id: currentMessage.id, globallyUniqueId: currentMessage.globallyUniqueId, groupingKey: currentMessage.groupingKey, threadId: currentMessage.threadId, timestamp: timestamp, flags: flags, tags: currentMessage.tags, globalTags: currentMessage.globalTags, localTags: currentMessage.localTags, forwardInfo: storeForwardInfo, authorId: currentMessage.author?.id, text: currentMessage.text, attributes: currentMessage.attributes, media: updatedMedia, likes: 0))
                            })
                            
                            maybeReadSecretOutgoingMessage(transaction: transaction, index: MessageIndex(id: message.id, timestamp: timestamp))
                            
                            var sentStickers: [ElloAppMediaFile] = []
                            for media in message.media {
                ...
                return postbox.transaction { transaction -> Void in
                    ...
                    if let messageId = action.messageId {
                        ...
                        transaction.updateMessage(messageId, update: { currentMessage in
                            ...
                            return .update(StoreMessage(id: currentMessage.id, globallyUniqueId: currentMessage.globallyUniqueId, groupingKey: currentMessage.groupingKey, threadId: currentMessage.threadId, timestamp: timestamp, flags: flags, tags: currentMessage.tags, globalTags: currentMessage.globalTags, localTags: currentMessage.localTags, forwardInfo: storeForwardInfo, authorId: currentMessage.author?.id, text: currentMessage.text, attributes: currentMessage.attributes, media: currentMessage.media, likes: 0))
                        })
...
private func sendBoxedDecryptedMessage(postbox: Postbox, network: Network, peer: ElloAppSecretChat, state: SecretChatState, operationIndex: Int32, decryptedMessage: BoxedDecryptedMessage, globallyUniqueId: Int64, file: SecretChatOutgoingFile?, silent: Bool, asService: Bool, wasDelivered: Bool) -> Signal<SendBoxedDecryptedMessageResult, NoError> {
    let payload = Buffer()
...
private func requestTerminateSecretChat(postbox: Postbox, network: Network, peerId: PeerId, tagLocalIndex: Int32, reportSpam: Bool, requestRemoteHistoryRemoval: Bool) -> Signal<Void, NoError> {
    ...
    |> mapToSignal { _ -> Signal<Void, NoError> in
        if reportSpam {
            return postbox.transaction { transaction -> ElloAppSecretChat? in
                if let peer = transaction.getPeer(peerId) as? ElloAppSecretChat {
                    return peer
                } else {
                    return nil
