...
private func activeChannelsFromUpdateGroups(_ groups: [UpdateGroup]) -> Set<PeerId> {
    ...
    for group in groups {
        for chat in group.chats {
            switch chat {
                case .channel:
                    if let channel = parseElloAppGroupOrChannel(chat: chat) as? ElloAppChannel {
...
private func peerIdsRequiringLocalChatStateFromUpdateGroups(_ groups: [UpdateGroup]) -> Set<PeerId> {
    ...
    for group in groups {
        ...
        for chat in group.chats {
            if let channel = parseElloAppGroupOrChannel(chat: chat) as? ElloAppChannel, channelUpdates.contains(channel.id) {
...
private func initialStateWithPeerIds(_ transaction: Transaction, peerIds: Set<PeerId>, activeChannelIds: Set<PeerId>, associatedMessageIds: Set<MessageId>, peerIdsRequiringLocalChatState: Set<PeerId>, locallyGeneratedMessageTimestamps: [PeerId: [(MessageId.Namespace, Int32)]]) -> AccountMutableState {
    ...
    for peerId in activeChannelIds {
        ...
        } else if let channel = transaction.getPeer(peerId) as? ElloAppChannel, channel.participationStatus != .member {
            channelsToPollExplicitely.insert(peerId)
        }
    }
    ...
    for peerId in peerIdsRequiringLocalChatState {
        ...
        if hasValidInclusion {
            ...
            if let peer = transaction.getPeer(peerId) {
                if let channel = peer as? ElloAppChannel, channel.participationStatus != .member {
                    if let notificationSettings = transaction.getPeerNotificationSettings(peerId) {
                        peerChatInfos[peerId] = PeerChatInfo(notificationSettings: notificationSettings)
        ...
        switch update {
            case let .updateChannelTooLong(_, channelId, channelPts):
                ...
                if !channelsToPoll.contains(peerId) {
                    if let channelPts = channelPts, let channelState = state.channelStates[peerId], channelState.pts >= channelPts {
                        Logger.shared.log("State", "channel \(peerId) (\((updatedState.peers[peerId] as? ElloAppChannel)?.title ?? "nil")) skip updateChannelTooLong by pts")
                    } else {
            ...
            case let .updateDeleteChannelMessages(channelId, messages, pts: pts, ptsCount):
                ...
                if let previousState = updatedState.channelStates[peerId] {
                    if previousState.pts >= pts {
                        Logger.shared.log("State", "channel \(peerId) (\((updatedState.peers[peerId] as? ElloAppChannel)?.title ?? "nil")) skip old delete update")
                        ...
                        if !missingUpdatesFromChannels.contains(peerId) {
                            Logger.shared.log("State", "channel \(peerId) (\((updatedState.peers[peerId] as? ElloAppChannel)?.title ?? "nil")) delete pts hole \(previousState.pts) + \(ptsCount) != \(pts)")
                            missingUpdatesFromChannels.insert(peerId)
                        }
                    }
                } else {
                    if !channelsToPoll.contains(peerId) {
                        //Logger.shared.log("State", "channel \(peerId) (\((updatedState.peers[peerId] as? ElloAppChannel)?.title ?? "nil")) state unknown")
                        channelsToPoll.insert(peerId)
                    }
                }
            case let .updateEditChannelMessage(apiMessage, pts, ptsCount):
                if let message = StoreMessage(apiMessage: apiMessage), case let .Id(messageId) = message.id {
                    ...
                    if let previousState = updatedState.channelStates[peerId] {
                        if previousState.pts >= pts {
                            Logger.shared.log("State", "channel \(peerId) (\((updatedState.peers[peerId] as? ElloAppChannel)?.title ?? "nil")) skip old edit update")
                        } else if previousState.pts + ptsCount == pts {
                            ...
                            if !missingUpdatesFromChannels.contains(peerId) {
                                Logger.shared.log("State", "channel \(peerId) (\((updatedState.peers[peerId] as? ElloAppChannel)?.title ?? "nil")) edit message pts hole \(previousState.pts) + \(ptsCount) != \(pts)")
                                missingUpdatesFromChannels.insert(peerId)
                            }
                        }
                    } else {
                ...
                if let previousState = updatedState.channelStates[peerId] {
                    if previousState.pts >= pts {
                        ...
                        switch apiWebpage {
                            ...
                            default:
                                if let webpage = elloappMediaWebpageFromApiWebpage(apiWebpage, url: nil) {
                        ...
                        if !channelsToPoll.contains(peerId) {
                            Logger.shared.log("State", "channel \(peerId) (\((updatedState.peers[peerId] as? ElloAppChannel)?.title ?? "nil")) updateWebPage pts hole \(previousState.pts) + \(ptsCount) != \(pts)")
                            channelsToPoll.insert(peerId)
                        }
                    }
                } else {
            ...
            case let .updateEditMessage(apiMessage, _, _):
                if let message = StoreMessage(apiMessage: apiMessage), case let .Id(messageId) = message.id {
                    ...
                    for media in message.media {
                        if let media = media as? ElloAppMediaAction {
            ...
            case let .updateNewChannelMessage(apiMessage, pts, ptsCount):
                if let message = StoreMessage(apiMessage: apiMessage) {
                    if let previousState = updatedState.channelStates[message.id.peerId] {
                        ...
                        Logger.shared.log("State", "channel \(message.id.peerId) (\((updatedState.peers[message.id.peerId] as? ElloAppChannel)?.title ?? "nil")) skip old message \(message.id) (\(messageText))")
                        } else if previousState.pts + ptsCount == pts {
                            if let preCachedResources = apiMessage.preCachedResources {
                                for (resource, data) in preCachedResources {
                                    updatedState.addPreCachedResource(resource, data: data)
                            ...
                            if !missingUpdatesFromChannels.contains(message.id.peerId) {
                                Logger.shared.log("State", "channel \(message.id.peerId) (\((updatedState.peers[message.id.peerId] as? ElloAppChannel)?.title ?? "nil")) message pts hole \(previousState.pts) + \(ptsCount) != \(pts)")
                                ;
                        ...
                        if !channelsToPoll.contains(message.id.peerId) {
                            Logger.shared.log("State", "channel \(message.id.peerId) (\((updatedState.peers[message.id.peerId] as? ElloAppChannel)?.title ?? "nil")) state unknown")
                ...
                if popup {
                    ...
                    if updatedState.peers[peerId] == nil {
                        updatedState.updatePeer(peerId, { peer in
                            if peer == nil {
                                return ElloAppUser(id: peerId, accessHash: nil, firstName: "Ello Notifications", lastName: nil, username: nil, phone: nil, photo: [], botInfo: BotUserInfo(flags: [], inlinePlaceholder: nil), email: nil, country: nil, gender: nil, birthday: nil, restrictionInfo: nil, flags: [.isVerified], emojiStatus: nil, isBusiness: false, isPublic: false, botDescription: nil)
                            } else {
                                return peer
                            }
                        })
                    ...
                    if alreadyStored {
                        ...
                        let message = StoreMessage(peerId: peerId, namespace: Namespaces.Message.Local, globallyUniqueId: nil, groupingKey: nil, threadId: nil, timestamp: date, flags: [.Incoming], tags: [], globalTags: [], localTags: [], forwardInfo: nil, authorId: peerId, text: messageText, attributes: attributes, media: medias, likes: 0)
                        updatedState.addMessages([message], location: .UpperHistoryBlock)
                    }
                }
            ...
            case let .updateNotifySettings(apiPeer, apiNotificationSettings):
                switch apiPeer {
                    case let .notifyPeer(peer):
                        let notificationSettings = ElloAppPeerNotificationSettings(apiSettings: apiNotificationSettings)
            ...
            case let .updateChatDefaultBannedRights(peer, defaultBannedRights, version):
                updatedState.updatePeer(peer.peerId, { peer in
                    if let group = peer as? ElloAppGroup {//, group.version == version - 1 {
                        return group.updateDefaultBannedRights(ElloAppChatBannedRights(apiBannedRights: defaultBannedRights), version: max(group.version, Int(version)))
                    } else if let channel = peer as? ElloAppChannel {//, group.version == version - 1 {
                        return channel.withUpdatedDefaultBannedRights(ElloAppChatBannedRights(apiBannedRights: defaultBannedRights))
                    } else {
                        return peer
                    }
                })
            case let .updatePinnedChannelMessages(flags, channelId, messages, pts, ptsCount):
                ...
                if let previousState = updatedState.channelStates[peerId] {
                    if previousState.pts >= pts {
                        Logger.shared.log("State", "channel \(peerId) (\((updatedState.peers[peerId] as? ElloAppChannel)?.title ?? "nil")) skip old pinned messages update")
                    } else if previousState.pts + ptsCount == pts {
                        ...
                        if !missingUpdatesFromChannels.contains(peerId) {
                            Logger.shared.log("State", "channel \(peerId) (\((updatedState.peers[peerId] as? ElloAppChannel)?.title ?? "nil")) pinned messages pts hole \(previousState.pts) + \(ptsCount) != \(pts)")
                            missingUpdatesFromChannels.insert(peerId)
                        }
                    }
                } else {
            ...
            case let .updateUserPhoto(userId, _, photo, _):
                updatedState.updatePeer(PeerId(namespace: Namespaces.Peer.CloudUser, id: PeerId.Id._internalFromInt64Value(userId)), { peer in
                    if let user = peer as? ElloAppUser {
                        return user.withUpdatedPhoto(parsedElloAppProfilePhoto(photo))
                    } else {
                        return peer
                    }
                })
            ...
            case let .updateTheme(theme):
                updatedState.updateTheme(ElloAppTheme(apiTheme: theme))
                    ...
                    for dialog in dialogs {
                        switch dialog {
                            case let .dialog(_, peer, topMessage, readInboxMaxId, readOutboxMaxId, unreadCount, unreadMentionsCount, unreadReactionsCount, notifySettings, pts, _, folderId):
                                ...
                                for chat in chats {
                                    if chat.peerId == peerId {
                                        if let groupOrChannel = parseElloAppGroupOrChannel(chat: chat) {
                                            if let group = groupOrChannel as? ElloAppGroup {
                                            ...
                                            } else if let channel = groupOrChannel as? ElloAppChannel {
                                ...
                                let notificationSettings = ElloAppPeerNotificationSettings(apiSettings: notifySettings)
                                updatedState.updateNotificationSettings(.peer(peer.peerId), notificationSettings: notificationSettings)
...
func pollChannelOnce(postbox: Postbox, network: Network, peerId: PeerId, stateManager: AccountStateManager, delayCompletion: Bool) -> Signal<Int32, NoError> {
    return postbox.transaction { transaction -> Signal<Int32, NoError> in
        ...
        if hasValidInclusion {
            if let notificationSettings = transaction.getPeerNotificationSettings(peerId) as? ElloAppPeerNotificationSettings {
                peerChatInfos[peerId] = PeerChatInfo(notificationSettings: notificationSettings)
            }
        }
            ...
            switch result {
                case let .peerDialogs(dialogs, messages, chats, users, _):
                    ...
                    loop: for dialog in dialogs {
                        ...
                        notificationSettings[peerId] = ElloAppPeerNotificationSettings(apiSettings: apiNotificationSettings)
...
private func verifyTransaction(_ transaction: Transaction, finalState: AccountMutableState) -> Bool {
    ...
    if !missingPeerIds.isEmpty {
        ...
        // TODO: - Research infinity Update
        debugPrint("🔴 Infinity Update \(#function)")
//        return false
    }
...
func replayFinalState(
    accountManager: AccountManager<ElloAppAccountManagerTypes>,
    postbox: Postbox,
    ...
    var updatedWebpages: [MediaId: ElloAppMediaWebpage] = [:]
    ...
    var recentlyUsedStickers: [MediaId: (MessageIndex, ElloAppMediaFile)] = [:]
    var slowModeLastMessageTimeouts:[PeerId : Int32] = [:]
    var recentlyUsedGifs: [MediaId: (MessageIndex, ElloAppMediaFile)] = [:]
    var syncRecentGifs = false
    var langPackDifferences: [String: [Api.LangPackDifference]] = [:]
    var pollLangPacks = Set<String>()
    var updatedThemes: [Int64: ElloAppTheme] = [:]
    ...
    for operation in optimizedOperations(finalState.state.operations) {
        switch operation {
            ...
            case let .UpdateMessagePoll(pollId, apiPoll, results):
                if let poll = transaction.getMedia(pollId) as? ElloAppMediaPoll {
                    ...
                    if let apiPoll = apiPoll {
                        ...
                        case let .poll(id, flags, question, answers, closePeriod, _):
                            let publicity: ElloAppMediaPollPublicity
                            if (flags & (1 << 1)) != 0 {
                                publicity = .public
                            } else {
                                publicity = .anonymous
                            }
                            let kind: ElloAppMediaPollKind
                            if (flags & (1 << 3)) != 0 {
                                kind = .quiz
                            } else {
                                kind = .poll(multipleAnswers: (flags & (1 << 2)) != 0)
                            }
                            updatedPoll = ElloAppMediaPoll(pollId: MediaId(namespace: Namespaces.Media.CloudPoll, id: id), publicity: publicity, kind: kind, text: question, options: answers.map(ElloAppMediaPollOption.init(apiOption:)), correctAnswers: nil, results: poll.results, isClosed: (flags & (1 << 0)) != 0, deadlineTimeout: closePeriod)
                        }
                    }
                    updatedPoll = updatedPoll.withUpdatedResults(ElloAppMediaPollResults(apiResults: results), min: resultsMin)
                    updateMessageMedia(transaction: transaction, id: pollId, media: updatedPoll)
                }
            case let .UpdateMedia(id, media):
                if let media = media as? ElloAppMediaWebpage {
                        ...
                        transaction.updateMessage(mainChannelMessage, update: { currentMessage in
                            ...
                            loop: for j in 0 ..< attributes.count {
                                if let attribute = attributes[j] as? ReplyThreadMessageAttribute {
                                    ...
                                    return .update(StoreMessage(id: currentMessage.id, globallyUniqueId: currentMessage.globallyUniqueId, groupingKey: currentMessage.groupingKey, threadId: currentMessage.threadId, timestamp: currentMessage.timestamp, flags: StoreMessageFlags(currentMessage.flags), tags: currentMessage.tags, globalTags: currentMessage.globalTags, localTags: currentMessage.localTags, forwardInfo: storeForwardInfo, authorId: currentMessage.author?.id, text: currentMessage.text, attributes: attributes, media: currentMessage.media, likes: 0))
                                }
                            }
                            return .skip
                        })
            ...
            case let .MergeApiUsers(users):
                var peers: [Peer] = []
                for user in users {
                    if let elloappUser = ElloAppUser.merge(transaction.getPeer(user.peerId) as? ElloAppUser, rhs: user) {
                        peers.append(elloappUser)
                    }
                }
            
                if let updatedAccountPeer = peers.first(where: { $0.id == accountPeerId }) as? ElloAppUser, let previousAccountPeer = transaction.getPeer(accountPeerId) as? ElloAppUser {
            ...
            case let .UpdatePeer(id, f):
                if let peer = f(transaction.getPeer(id)) {
                    if id == accountPeerId, let updatedAccountPeer = peer as? ElloAppUser, let previousAccountPeer = transaction.getPeer(accountPeerId) as? ElloAppUser {
                        if updatedAccountPeer.isPremium != previousAccountPeer.isPremium {
                            isPremiumUpdated = true
                        }
                    }
            ...
            case let .UpdateMessagesPinned(messageIds, pinned):
                for id in messageIds {
                    transaction.updateMessage(id, update: { currentMessage in
                        ...
                        return .update(StoreMessage(id: currentMessage.id, globallyUniqueId: currentMessage.globallyUniqueId, groupingKey: currentMessage.groupingKey, threadId: currentMessage.threadId, timestamp: currentMessage.timestamp, flags: StoreMessageFlags(currentMessage.flags), tags: tags, globalTags: currentMessage.globalTags, localTags: currentMessage.localTags, forwardInfo: storeForwardInfo, authorId: currentMessage.author?.id, text: currentMessage.text, attributes: attributes, media: currentMessage.media, likes: 0))
                    })
                }
            case let .MergePeerPresences(statuses, explicit):
                ...
                for (peerId, status) in statuses {
                    if peerId == accountPeerId {
                        ...
                        let presence = ElloAppUserPresence(apiStatus: status)
            ...
            case let .UpdateExtendedMedia(messageId, apiExtendedMedia):
                transaction.updateMessage(messageId, update: { currentMessage in
                    ...
                    let invoice = media.first(where: { $0 is ElloAppMediaInvoice }) as? ElloAppMediaInvoice
                    ...
                    let updatedExtendedMedia: ElloAppExtendedMedia?
                    ...
                    if let updatedExtendedMedia = updatedExtendedMedia, var invoice = invoice {
                        if let currentExtendedMedia = currentExtendedMedia, case .full = currentExtendedMedia, case .preview = updatedExtendedMedia {
                            ...
                            media = media.filter { !($0 is ElloAppMediaInvoice) }
                            invoice = invoice.withUpdatedExtendedMedia(updatedExtendedMedia)
                            media.append(invoice)
                        }
                    }
                    
                    return .update(StoreMessage(id: currentMessage.id, globallyUniqueId: currentMessage.globallyUniqueId, groupingKey: currentMessage.groupingKey, threadId: currentMessage.threadId, timestamp: currentMessage.timestamp, flags: StoreMessageFlags(currentMessage.flags), tags: currentMessage.tags, globalTags: currentMessage.globalTags, localTags: currentMessage.localTags, forwardInfo: storeForwardInfo, authorId: currentMessage.author?.id, text: currentMessage.text, attributes: currentMessage.attributes, media: media, likes: 0))
                })
        }
    }
...
//    }
                    ...
                    case let .add(apiSet):
                        ...
                        if case let .stickerSet(set, packs, keywords, documents) = apiSet {
                            ...
                            for apiDocument in documents {
                                if let file = elloappMediaFileFromApiDocument(apiDocument), let id = file.id {
    ...
    if !recentlyUsedStickers.isEmpty {
        let stickerFiles: [ElloAppMediaFile] = recentlyUsedStickers.values.sorted(by: {
            return $0.0 < $1.0
        }).map({ $0.1 })
    ...
    if syncRecentGifs {
        ...
        let gifFiles: [ElloAppMediaFile] = recentlyUsedGifs.values.sorted(by: {
            return $0.0 < $1.0
        }).map({ $0.1 })
...
//        addSynchronizeAttachMenuBotsOperation(transaction: transaction)
    ...
    for chatPeerId in updatedSecretChatTypingActivities {
        if let peer = transaction.getPeer(chatPeerId) as? ElloAppSecretChat {
            ...
            if processSecretChatIncomingEncryptedOperations(transaction: transaction, peerId: peerId) {
                ...
                if !processResult.addedMessages.isEmpty {
                    ...
                    if let groupId = currentInclusion.groupId, groupId == Namespaces.PeerGroup.archive {
                        if let peer = transaction.getPeer(peerId) as? ElloAppSecretChat {
    ...
    if !updatedThemes.isEmpty {
        ...
        let themes = entries.map { entry -> ElloAppTheme in
            let theme = entry.contents.get(ElloAppThemeNativeCodable.self)!
        ...
        for theme in themes {
            ...
            if let entry = CodableEntry(ElloAppThemeNativeCodable(theme)) {
