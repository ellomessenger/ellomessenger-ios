...
private func removeChat(transaction: Transaction, 
                        postbox: Postbox,
                        network: Network,
                        stateManager: AccountStateManager,
                        peer: Peer,
                        operation: CloudChatRemoveChatOperation) -> Signal<Void, NoError> {
    if peer.id.namespace == Namespaces.Peer.CloudChannel {
        if let inputChannel = apiInputChannel(peer) {
            let paidSignal: Signal<PaidSubscriptionItem, NoError>
            if let peer = peer as? ElloAppChannel, peer.isPaid {
                paidSignal = network.request(Api.paidSubscription.unsubscribe(peerId: Int(peer.id.id._internalGetInt64Value()), peerType: 4))
                |> retryRequest
            } else {
                paidSignal = .complete()
            }
            ...
            |> mapToSignal { _, updates, _ -> Signal<Void, NoError> in
                if let updates = updates {
                    stateManager.addUpdates(updates)
                }
                return .complete()
