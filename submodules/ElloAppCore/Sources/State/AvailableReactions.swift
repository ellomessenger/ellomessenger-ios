...
public final class AvailableReactions: Equatable, Codable {
    public final class Reaction: Equatable, Codable {
        ...
        public let staticIcon: ElloAppMediaFile
        public let appearAnimation: ElloAppMediaFile
        public let selectAnimation: ElloAppMediaFile
        public let activateAnimation: ElloAppMediaFile
        public let effectAnimation: ElloAppMediaFile
        public let aroundAnimation: ElloAppMediaFile?
        public let centerAnimation: ElloAppMediaFile?
        ...
        public init(
            ...
            title: String,
            staticIcon: ElloAppMediaFile,
            appearAnimation: ElloAppMediaFile,
            selectAnimation: ElloAppMediaFile,
            activateAnimation: ElloAppMediaFile,
            effectAnimation: ElloAppMediaFile,
            aroundAnimation: ElloAppMediaFile?,
            centerAnimation: ElloAppMediaFile?
        ) {
        ...
        public init(from decoder: Decoder) throws {
            ...
            self.staticIcon = ElloAppMediaFile(decoder: PostboxDecoder(buffer: MemoryBuffer(data: staticIconData.data)))
            
            let appearAnimationData = try container.decode(AdaptedPostboxDecoder.RawObjectData.self, forKey: .appearAnimation)
            self.appearAnimation = ElloAppMediaFile(decoder: PostboxDecoder(buffer: MemoryBuffer(data: appearAnimationData.data)))
            
            let selectAnimationData = try container.decode(AdaptedPostboxDecoder.RawObjectData.self, forKey: .selectAnimation)
            self.selectAnimation = ElloAppMediaFile(decoder: PostboxDecoder(buffer: MemoryBuffer(data: selectAnimationData.data)))
            
            let activateAnimationData = try container.decode(AdaptedPostboxDecoder.RawObjectData.self, forKey: .activateAnimation)
            self.activateAnimation = ElloAppMediaFile(decoder: PostboxDecoder(buffer: MemoryBuffer(data: activateAnimationData.data)))
            
            let effectAnimationData = try container.decode(AdaptedPostboxDecoder.RawObjectData.self, forKey: .effectAnimation)
            self.effectAnimation = ElloAppMediaFile(decoder: PostboxDecoder(buffer: MemoryBuffer(data: effectAnimationData.data)))
            
            if let aroundAnimationData = try container.decodeIfPresent(AdaptedPostboxDecoder.RawObjectData.self, forKey: .aroundAnimation) {
                self.aroundAnimation = ElloAppMediaFile(decoder: PostboxDecoder(buffer: MemoryBuffer(data: aroundAnimationData.data)))
            } else {
            ...
            if let centerAnimationData = try container.decodeIfPresent(AdaptedPostboxDecoder.RawObjectData.self, forKey: .centerAnimation) {
                self.centerAnimation = ElloAppMediaFile(decoder: PostboxDecoder(buffer: MemoryBuffer(data: centerAnimationData.data)))
            } else {
...
private extension AvailableReactions.Reaction {
    convenience init?(apiReaction: Api.AvailableReaction) {
        ...
        case let .availableReaction(flags, reaction, title, staticIcon, appearAnimation, selectAnimation, activateAnimation, effectAnimation, aroundAnimation, centerIcon):
            guard let staticIconFile = elloappMediaFileFromApiDocument(staticIcon) else {
                return nil
            }
            guard let appearAnimationFile = elloappMediaFileFromApiDocument(appearAnimation) else {
                return nil
            }
            guard let selectAnimationFile = elloappMediaFileFromApiDocument(selectAnimation) else {
                return nil
            }
            guard let activateAnimationFile = elloappMediaFileFromApiDocument(activateAnimation) else {
                return nil
            }
            guard let effectAnimationFile = elloappMediaFileFromApiDocument(effectAnimation) else {
                return nil
            }
            let aroundAnimationFile = aroundAnimation.flatMap { elloappMediaFileFromApiDocument($0) }
            let centerAnimationFile = centerIcon.flatMap { elloappMediaFileFromApiDocument($0) }
