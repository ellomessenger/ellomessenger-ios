...
private final class UpdatedWebpageSubscriberContext {
    let subscribers = Bag<(ElloAppMediaWebpage) -> Void>()
}
...
public final class AccountStateManager {
    ...
    public func updatedWebpage(_ webpageId: MediaId) -> Signal<ElloAppMediaWebpage, NoError> {
        let queue = self.queue
        return Signal { [weak self] subscriber in
            let disposable = MetaDisposable()
            queue.async {
    ...
    private func notifyUpdatedWebpages(_ updatedWebpages: [MediaId: ElloAppMediaWebpage]) {
...
public func messagesForNotification(transaction: Transaction, id: MessageId, alwaysReturnMessage: Bool) -> (messages: [Message], notify: Bool, sound: PeerMessageSound, displayContents: Bool) {
    ...
    if let notificationSettings = transaction.getPeerNotificationSettings(notificationPeerId) as? ElloAppPeerNotificationSettings {
        ...
        if let globalNotificationSettings = transaction.getPreferencesEntry(key: PreferencesKeys.globalNotifications)?.get(GlobalNotificationSettings.self) {
            ...
            } else if id.peerId.namespace == Namespaces.Peer.CloudChannel, let peer = peer as? ElloAppChannel, case .broadcast = peer.info {
