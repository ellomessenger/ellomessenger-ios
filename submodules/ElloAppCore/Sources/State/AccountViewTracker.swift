...
private func pollMessages(entries: [MessageHistoryEntry]) -> (Set<MessageId>, [MessageId: Message]) {
    ...
    for entry in entries {
        for media in entry.message.media {
            if let poll = media as? ElloAppMediaPoll, poll.pollId.namespace == Namespaces.Media.CloudPoll, entry.message.id.namespace == Namespaces.Message.Cloud, !poll.isClosed {
                messageIds.insert(entry.message.id)
            ...
            |> mapToSignal { result in
                ...
                return account.postbox.transaction { transaction -> Void in
                    ...
                    for apiUser in users {
                        if let user = ElloAppUser.merge(transaction.getPeer(apiUser.peerId) as? ElloAppUser, rhs: apiUser) {
                            peers.append(user)
                                    ...
                                    if let webpage = webpage {
                                        transaction.updateMessage(messageId, update: { currentMessage in
                                            ...
                                            for i in 0 ..< media.count {
                                                if let _ = media[i] as? ElloAppMediaWebpage {
                                                    media[i] = webpage
                                                    break
                                                }
                                            }
        ...
        self.queue.async {
            ...
            if !addedMessageIds.isEmpty {
                for (_, messageIds) in messagesIdsGroupedByPeerId(Set(addedMessageIds)) {
                    ...
                    if let account = self.account {
                        let signal = account.postbox.transaction { transaction -> [ElloAppMediaFile] in
                            var result: [ElloAppMediaFile] = []
                            ...
                            if let message = transaction.getMessage(id) {
                                ...
                                if consume {
                                    transaction.updateMessage(id, update: { currentMessage in
                                        ...
                                        return .update(StoreMessage(id: currentMessage.id, globallyUniqueId: currentMessage.globallyUniqueId, groupingKey: currentMessage.groupingKey, threadId: currentMessage.threadId, timestamp: currentMessage.timestamp, flags: StoreMessageFlags(currentMessage.flags), tags: currentMessage.tags, globalTags: currentMessage.globalTags, localTags: currentMessage.localTags, forwardInfo: currentMessage.forwardInfo.flatMap(StoreMessageForwardInfo.init), authorId: currentMessage.author?.id, text: currentMessage.text, attributes: attributes, media: currentMessage.media, likes: 0))
                                    })
