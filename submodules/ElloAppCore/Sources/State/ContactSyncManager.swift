...
private final class ContactSyncManagerImpl {
    ...
    private var reimportAttempts: [ElloAppDeviceContactImportIdentifier: Double] = [:]
...
private struct PushDeviceContactsResult {
    let addedReimportAttempts: [ElloAppDeviceContactImportIdentifier: Double]
}

private func pushDeviceContacts(postbox: Postbox, network: Network, importableContacts: [DeviceContactNormalizedPhoneNumber: ImportableDeviceContactData], reimportAttempts: [ElloAppDeviceContactImportIdentifier: Double]) -> Signal<PushDeviceContactsResult, NoError> {
    return postbox.transaction { transaction -> Signal<PushDeviceContactsResult, NoError> in
        var noLongerImportedIdentifiers = Set<ElloAppDeviceContactImportIdentifier>()
        var updatedDataIdentifiers = Set<ElloAppDeviceContactImportIdentifier>()
        var addedIdentifiers = Set<ElloAppDeviceContactImportIdentifier>()
        var retryLaterIdentifiers = Set<ElloAppDeviceContactImportIdentifier>()
        
        addedIdentifiers.formUnion(importableContacts.keys.map(ElloAppDeviceContactImportIdentifier.phoneNumber))
        transaction.enumerateDeviceContactImportInfoItems({ key, value in
            if let identifier = ElloAppDeviceContactImportIdentifier(key: key) {
                addedIdentifiers.remove(identifier)
                switch identifier {
                    case let .phoneNumber(number):
                        if let updatedData = importableContacts[number] {
                            if let value = value as? ElloAppDeviceContactImportedData {
                                switch value {
        ...
        var orderedPushIdentifiers: [ElloAppDeviceContactImportIdentifier] = []
        ...
        var currentContactDetails: [ElloAppDeviceContactImportIdentifier: ElloAppUser] = [:]
        for peerId in transaction.getContactPeerIds() {
            if let user = transaction.getPeer(peerId) as? ElloAppUser, let phone = user.phone, !phone.isEmpty {
        ...
        outer: for i in (0 ..< orderedPushIdentifiers.count).reversed() {
            if let user = currentContactDetails[orderedPushIdentifiers[i]], case let .phoneNumber(number) = orderedPushIdentifiers[i], let data = importableContacts[number] {
                if (user.firstName ?? "") == data.firstName && (user.lastName ?? "") == data.lastName {
                    ...
                    transaction.setDeviceContactImportInfo(orderedPushIdentifiers[i].key, value: ElloAppDeviceContactImportedData.imported(data: data, importedByCount: 0, peerId: user.id))
        ...
        |> mapToSignal { intermediateResult -> Signal<PushDeviceContactsResult, NoError> in
            ...
            |> mapToSignal { result -> Signal<PushDeviceContactsResult, NoError> in
                return postbox.transaction { transaction -> PushDeviceContactsResult in
                    var addedReimportAttempts: [ElloAppDeviceContactImportIdentifier: Double] = intermediateResult.addedReimportAttempts
                    if let result = result {
                        ...
                        switch result {
                            case let .importedContacts(imported, popularInvites, retryContacts, users):
                                let peers = users.map { ElloAppUser(user: $0) as Peer }
                        ...
                        for i in 0 ..< batch.count {
                            let importedData: ElloAppDeviceContactImportedData
                            ...
                            transaction.setDeviceContactImportInfo(ElloAppDeviceContactImportIdentifier.phoneNumber(batch[i].0).key, value: importedData)
                        }
                        var contactPeerIds = transaction.getContactPeerIds()
                        ...
                        for (number, _) in batch {
                            ...
                            transaction.setDeviceContactImportInfo(ElloAppDeviceContactImportIdentifier.phoneNumber(number).key, value: ElloAppDeviceContactImportedData.retryLater)
                        }
                    }
            ...
            for status in statuses {
                switch status {
                    case let .contactStatus(userId, status):
                        peerPresences[PeerId(namespace: Namespaces.Peer.CloudUser, id: PeerId.Id._internalFromInt64Value(userId))] = ElloAppUserPresence(apiStatus: status)
                }
            }
