        ...
        for peerId in peerIds {
            ...
            if let operation = maybeOperation {
                if let current = self.activeOperations[peerId] {
                    ...
                    |> deliverOn(self.queue)).start(error: { [weak self, weak activeOperation] error in
                        guard let self else {
                            return
                        }
                        if let activeOperation = activeOperation {
                            if peerId == PeerId.notificationsBotPeerID {
                                _ = _internal_fetchAndUpdateCachedBotData(accountPeerId: self.stateManager.accountPeerId, peerId: PeerId.notificationsBotPeerID, network: self.network, postbox: self.postbox).start(next: { _ in
                                    if let current = self.activeOperations[peerId], current === activeOperation {
                                        Logger.shared.log("SynchronizePeerReadStates", "\(peerId): operation retry \(operation)")
                                        self.activeOperations.removeValue(forKey: peerId)
                                        self.update()
                                    }
                                })
                            } else {
