            ...
            let manager = MultipartUploadManager(headerSize: headerSize, data: dataSignal, encryptionKey: encryptionKey, hintFileSize: hintFileSize, hintFileIsLarge: hintFileIsLarge, forceNoBigParts: forceNoBigParts, useLargerParts: useLargerParts, increaseParallelParts: increaseParallelParts, uploadPart: { part in
                ...
                if let result = result {
                    if let encryptionKey = encryptionKey {
                        ...
                        case let .resource(resource):
                            switch resource {
                            case .media(media: let media, _):
                                switch media {
                                case .standalone(let media as ElloAppMediaFile): 
                                    media.fileName ?? "file.jpg"
                                default: 
                                    "file.jpg"
                                }
                            default: 
                        ...
                        if let _ = result.bigTotalParts {
                            let inputFile = Api.InputFile.inputFileBig(id: result.id, parts: result.partCount, name: fileName)
                            subscriber.putNext(.inputFile(inputFile))
                        } else {
                            let inputFile = Api.InputFile.inputFile(id: result.id, parts: result.partCount, name: fileName, md5Checksum: result.md5Digest)
                            subscriber.putNext(.inputFile(inputFile))
                        }
                    }
