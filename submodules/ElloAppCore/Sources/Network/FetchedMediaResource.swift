...
extension MediaResourceReference {
    var apiFileReference: Data? {
        if let resource = self.resource as? ElloAppCloudMediaResourceWithFileReference {
...
final class ElloAppCloudMediaResourceFetchInfo: MediaResourceFetchInfo {
...
public func fetchedMediaResource(mediaBox: MediaBox, reference: MediaResourceReference, ranges: [(Range<Int64>, MediaBoxFetchPriority)]?, statsCategory: MediaResourceStatsCategory = .generic, reportResultStatus: Bool = false, preferBackgroundReferenceRevalidation: Bool = false, continueInBackground: Bool = false) -> Signal<FetchResourceSourceType, FetchResourceError> {
    ...
    case let .media(media, _):
        if let file = media.media as? ElloAppMediaFile {
    ...
    if let ranges = ranges {
        let signals = ranges.map { (range, priority) -> Signal<Void, FetchResourceError> in
            return mediaBox.fetchedResourceData(reference.resource, in: range, priority: priority, parameters: MediaResourceFetchParameters(tag: ElloAppMediaResourceFetchTag(statsCategory: statsCategory), info: ElloAppCloudMediaResourceFetchInfo(reference: reference, preferBackgroundReferenceRevalidation: preferBackgroundReferenceRevalidation, continueInBackground: continueInBackground), isRandomAccessAllowed: isRandomAccessAllowed))
        }
        ...
        return mediaBox.fetchedResource(reference.resource, parameters: MediaResourceFetchParameters(tag: ElloAppMediaResourceFetchTag(statsCategory: statsCategory), info: ElloAppCloudMediaResourceFetchInfo(reference: reference, preferBackgroundReferenceRevalidation: preferBackgroundReferenceRevalidation, continueInBackground: continueInBackground), isRandomAccessAllowed: isRandomAccessAllowed), implNext: reportResultStatus)
    }
}
...
public func stickerPackFileReference(_ file: ElloAppMediaFile) -> FileMediaReference {
...
private func findMediaResource(media: Media, previousMedia: Media?, resource: MediaResource) -> ElloAppMediaResource? {
    ...
    } else if let game = media as? ElloAppMediaGame {
...
public func findMediaResourceById(message: EngineMessage, resourceId: MediaResourceId) -> ElloAppMediaResource? {
...
func findMediaResourceById(media: Media, resourceId: MediaResourceId) -> ElloAppMediaResource? {
    if let image = media as? ElloAppMediaImage {
        for representation in image.representations {
...
private func findUpdatedMediaResource(media: Media, previousMedia: Media?, resource: MediaResource) -> ElloAppMediaResource? {
...
final class MediaReferenceRevalidationContext {
    ...
    func customEmoji(postbox: Postbox, network: Network, background: Bool, fileId: Int64) -> Signal<ElloAppMediaFile, RevalidateMediaReferenceError> {
        ...
        |> mapToSignal { result -> Signal<ElloAppMediaFile, RevalidateMediaReferenceError> in
    ...
    func webPage(postbox: Postbox, network: Network, background: Bool, webPage: WebpageReference) -> Signal<ElloAppMediaWebpage, RevalidateMediaReferenceError> {
        ...
        }) |> mapToSignal { next -> Signal<ElloAppMediaWebpage, RevalidateMediaReferenceError> in
            if let next = next as? ElloAppMediaWebpage {
                return .single(next)
            } else {
                return .fail(.generic)
            }
    ...
    func savedGifs(postbox: Postbox, network: Network, background: Bool) -> Signal<[ElloAppMediaFile], RevalidateMediaReferenceError> {
        return self.genericItem(key: .savedGifs, background: background, request: { next, error in
            let loadRecentGifs: Signal<[ElloAppMediaFile], NoError> = postbox.transaction { transaction -> [ElloAppMediaFile] in
                return transaction.getOrderedListItems(collectionId: Namespaces.OrderedItemList.CloudRecentGifs).compactMap({ item -> ElloAppMediaFile? in
            ...
            |> mapToSignal { _ -> Signal<[ElloAppMediaFile], NoError> in
                return .complete()
            }
            |> then(loadRecentGifs)
        ...
        }) |> mapToSignal { next -> Signal<[ElloAppMediaFile], RevalidateMediaReferenceError> in
            if let next = next as? [ElloAppMediaFile] {
                return .single(next)
            } else {
                return .fail(.generic)
            }
    ...
    func wallpapers(postbox: Postbox, network: Network, background: Bool, wallpaper: WallpaperReference?) -> Signal<[ElloAppWallpaper], RevalidateMediaReferenceError> {
        let key: MediaReferenceRevalidationKey
        ...
        return self.genericItem(key: key, background: background, request: { next, error in
            let signal: Signal<[ElloAppWallpaper]?, RevalidateMediaReferenceError>
            if let wallpaper = wallpaper, case let .slug(slug) = wallpaper {
                ...
                signal = elloappWallpapers(postbox: postbox, network: network, forceUpdate: true)
                |> last
                |> mapError { _ -> RevalidateMediaReferenceError in
                }
            }
        ...
        }) |> mapToSignal { next -> Signal<[ElloAppWallpaper], RevalidateMediaReferenceError> in
            if let next = next as? [ElloAppWallpaper] {
                return .single(next)
            } else {
                return .fail(.generic)
            }
    ...
    func themes(postbox: Postbox, network: Network, background: Bool) -> Signal<[ElloAppTheme], RevalidateMediaReferenceError> {
        return self.genericItem(key: .themes, background: background, request: { next, error in
            return (elloappThemes(postbox: postbox, network: network, accountManager: nil, forceUpdate: true)
            |> take(1)
            |> mapError { _ -> RevalidateMediaReferenceError in
            }).start(next: { value in
                next(value)
        ...
        }) |> mapToSignal { next -> Signal<[ElloAppTheme], RevalidateMediaReferenceError> in
            if let next = next as? [ElloAppTheme] {
                return .single(next)
            } else {
                return .fail(.generic)
            }
    ...
    func peerAvatars(postbox: Postbox, network: Network, background: Bool, peer: PeerReference) -> Signal<[ElloAppPeerPhoto], RevalidateMediaReferenceError> {
        ...
        }) |> mapToSignal { next -> Signal<[ElloAppPeerPhoto], RevalidateMediaReferenceError> in
            if let next = next as? [ElloAppPeerPhoto] {
                return .single(next)
            } else {
                return .fail(.generic)
            }
    ...
    func notificationSoundList(postbox: Postbox, network: Network, background: Bool) -> Signal<[ElloAppMediaFile], RevalidateMediaReferenceError> {
        return self.genericItem(key: .notificationSoundList, background: background, request: { next, error in
            ...
            |> map { result -> [ElloAppMediaFile] in
                guard let result = result else {
                    return []
                }
...
struct RevalidatedMediaResource {
    let updatedResource: ElloAppMediaResource
    let updatedReference: MediaResourceReference?
}

func revalidateMediaResourceReference(postbox: Postbox, network: Network, revalidationContext: MediaReferenceRevalidationContext, info: ElloAppCloudMediaResourceFetchInfo, resource: MediaResource) -> Signal<RevalidatedMediaResource, RevalidateMediaReferenceError> {
    ...
    if case let .media(media, resource) = updatedReference {
        if case let .message(messageReference, mediaValue) = media {
            if let file = mediaValue as? ElloAppMediaFile {
            ...
            } else if let image = mediaValue as? ElloAppMediaImage {
