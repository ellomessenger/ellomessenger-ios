...
func initializedNetwork(accountId: AccountRecordId, arguments: NetworkInitializationArguments, supplementary: Bool, datacenterId: Int, keychain: Keychain, basePath: String, testingEnvironment: Bool, languageCode: String?, proxySettings: ProxySettings?, networkSettings: NetworkSettings?, phoneNumber: String?) -> Signal<Network, NoError> {
    return Signal { subscriber in
        ...
        queue.async {
            ...
            let currentServerType = ServerType.currentServerType
 
            if testingEnvironment {
                seedAddressList = [
                    1: [currentServerType.rawValue]
                ]
            } else {
                seedAddressList = [
                    1: [currentServerType.rawValue]
...
public final class Network: NSObject, MTRequestMessageServiceDelegate {
    ...
    let mtProto: MTProto
    public let requestService: MTRequestMessageService
    let basePath: String
    ...
    public func request<T>(_ data: (FunctionDescription, Buffer, DeserializeFunctionResponse<T>), tag: NetworkRequestDependencyTag? = nil, automaticFloodWait: Bool = true) -> Signal<T, MTRpcError> {
        ...
        return Signal { subscriber in
...
#if DEBUG
            print("Request base == \n\(self):\(#file):\(#line): \n\(data.0.name): \(data.0.parameters) : \(String(data: data.1.makeData() as Data, encoding: .utf8) ?? "")")
#endif
            request.setPayload(data.1.makeData() as Data, 
                ...
                if let result = data.2.parse(Buffer(data: response)) {
                    #if DEBUG
                    print("Result == \n\(self):\(#file):\(#line): \n\(data.0.name): \(data.0.parameters) : \(result)")
                    #endif
                    return BoxedMessage(result)
                }
            ...
            request.completed = { (boxedResponse, timestamp, error) -> () in
                if let error = error {
#if DEBUG
                    print("Error == \n\(self):\(#file):\(#line): \n\(data.0.name): \(data.0.parameters) : error - \(error)")
#endif
                    subscriber.putError(error)
                } else {
                    if let result = (boxedResponse as! BoxedMessage).body as? T {
...
#if DEBUG
                        print("Error === \n\(self):\(#file):\(#line): \n\(data.0.name): \(data.0.parameters) : \(500) : TL_VERIFICATION_ERROR")
#endif
                        subscriber.putError(MTRpcError(errorCode: 500, errorDescription: "TL_VERIFICATION_ERROR"))
                    }
                }
...
#if DEBUG
            print("Request ===== \n\(self):\(#file):\(#line): \n===== \(data.0.name): \(data.0.parameters) : \(request)")
#endif
            ...
            return ActionDisposable { [weak requestService] in
#if DEBUG
                print("Dispose == \n\(self):\(#file):\(#line): \n===== \(data.0.name): \(data.0.parameters) : request removed from queue: \(String(describing:internalId))")
#endif
...
public func retryRequestTriple<T>(signal: Signal<T, MTRpcError>) -> Signal<T, MTRpcError> {
    return signal
    |> retry(retryOnError: { error in
        #if DEBUG
        print("Request retry cause of error: \(error)")
        #endif
        return true
    }, delayIncrement: 0.2, maxDelay: 5.0, maxRetries: 3, onQueue: Queue.concurrentDefaultQueue())
//    |> retry(0.2, maxDelay: 5.0, onQueue: Queue.concurrentDefaultQueue())
}
