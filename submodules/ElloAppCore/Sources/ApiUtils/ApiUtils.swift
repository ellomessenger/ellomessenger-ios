...
func apiInputPeer(_ peer: Peer) -> Api.InputPeer? {
    switch peer {
    case let user as ElloAppUser where user.accessHash != nil:
...
func apiInputChannel(_ peer: Peer) -> Api.InputChannel? {
    if let channel = peer as? ElloAppChannel, let accessHash = channel.accessHash {
        ...
        return Api.InputChannel.inputChannel(channelId: channel.id.id._internalGetInt64Value(), accessHash: 0)
    } else {
        return nil
    }
}
...
func apiInputUser(_ peer: Peer) -> Api.InputUser? {
    if let user = peer as? ElloAppUser, let accessHash = user.accessHash {
...
func apiInputSecretChat(_ peer: Peer) -> Api.InputEncryptedChat? {
    if let chat = peer as? ElloAppSecretChat {
