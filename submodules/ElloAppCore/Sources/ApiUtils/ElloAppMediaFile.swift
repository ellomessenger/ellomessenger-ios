...
func dimensionsForFileAttributes(_ attributes: [ElloAppMediaFileAttribute]) -> PixelDimensions? {
...
func durationForFileAttributes(_ attributes: [ElloAppMediaFileAttribute]) -> Int32? {
    for attribute in attributes {
        switch attribute {
...
public extension ElloAppMediaFile {
...
func elloappMediaFileAttributesFromApiAttributes(_ attributes: [Api.DocumentAttribute]) -> [ElloAppMediaFileAttribute] {
    var result: [ElloAppMediaFileAttribute] = []
    for attribute in attributes {
        switch attribute {
...
public func fileNameFromFileAttributes(_ attributes: [ElloAppMediaFileAttribute]) -> String? {
    for attribute in attributes {
...
func elloappMediaFileThumbnailRepresentationsFromApiSizes(datacenterId: Int32, documentId: Int64, accessHash: Int64, fileReference: Data?, sizes: [Api.PhotoSize]) -> (immediateThumbnail: Data?, representations:  [ElloAppMediaImageRepresentation]) {
...
func elloappMediaFileFromApiDocument(_ document: Api.Document) -> ElloAppMediaFile? {
    switch document {
        case let .document(_, id, accessHash, fileReference, _, mimeType, size, thumbs, videoThumbs, dcId, attributes):
            var parsedAttributes = elloappMediaFileAttributesFromApiAttributes(attributes)
            parsedAttributes.append(.hintIsValidated)
            
            let (immediateThumbnail, previewRepresentations) = elloappMediaFileThumbnailRepresentationsFromApiSizes(datacenterId: dcId, documentId: id, accessHash: accessHash, fileReference: fileReference.makeData(), sizes: thumbs ?? [])
            
            var videoThumbnails: [ElloAppMediaFile.VideoThumbnail] = []
            if let videoThumbs = videoThumbs {
                for thumb in videoThumbs {
                    ...
                    case let .videoSize(_, type, w, h, _, _):
                        ...
                        videoThumbnails.append(ElloAppMediaFile.VideoThumbnail(
            ...
            return ElloAppMediaFile(fileId: MediaId(namespace: Namespaces.Media.CloudFile, id: id), partialReference: nil, resource: CloudDocumentMediaResource(datacenterId: Int(dcId), fileId: id, accessHash: accessHash, size: size, fileReference: fileReference.makeData(), fileName: fileNameFromFileAttributes(parsedAttributes)), previewRepresentations: previewRepresentations, videoThumbnails: videoThumbnails, immediateThumbnailData: immediateThumbnail, mimeType: mimeType, size: size, attributes: parsedAttributes)
        case .documentEmpty:
            return nil
    }
}
