    ...
    init(apiBlock: Api.PageBlock) {
        switch apiBlock {
            ...
            case let .pageBlockChannel(channel: apiChat):
                self = .channelBanner(parseElloAppGroupOrChannel(chat: apiChat) as? ElloAppChannel)
...
extension InstantPage {
    convenience init(apiPage: Api.Page) {
        ...
        for photo in photos {
            if let image = elloappMediaImageFromApiPhoto(photo), let id = image.id {
                media[id] = image
            }
        }
        for file in files {
            if let file = elloappMediaFileFromApiDocument(file), let id = file.id {
                media[id] = file
            }
        }
