...
public enum ChatContextResultMessage: PostboxCoding, Equatable, Codable {
    ...
    case mapLocation(media: ElloAppMediaMap, replyMarkup: ReplyMarkupMessageAttribute?)
    case contact(media: ElloAppMediaContact, replyMarkup: ReplyMarkupMessageAttribute?)
    case invoice(media: ElloAppMediaInvoice, replyMarkup: ReplyMarkupMessageAttribute?)
    ...
    public init(decoder: PostboxDecoder) {
        switch decoder.decodeInt32ForKey("_v", orElse: 0) {
            ...
            case 2:
                self = .mapLocation(media: decoder.decodeObjectForKey("l") as! ElloAppMediaMap, replyMarkup: decoder.decodeObjectForKey("m") as? ReplyMarkupMessageAttribute)
            case 3:
                self = .contact(media: decoder.decodeObjectForKey("c") as! ElloAppMediaContact, replyMarkup: decoder.decodeObjectForKey("m") as? ReplyMarkupMessageAttribute)
            case 4:
                self = .invoice(media: decoder.decodeObjectForKey("i") as! ElloAppMediaInvoice, replyMarkup: decoder.decodeObjectForKey("m") as? ReplyMarkupMessageAttribute)
            default:
...
public enum ChatContextResult: Equatable, Codable {
    ...
    public struct ExternalReference: Equatable, Codable {
        ...
        public let content: ElloAppMediaWebFile?
        public let thumbnail: ElloAppMediaWebFile?
        ...
        public init(
            ...
            description: String?,
            url: String?,
            content: ElloAppMediaWebFile?,
            thumbnail: ElloAppMediaWebFile?,
    ...
    public struct InternalReference: Equatable, Codable {
        ...
        public let file: ElloAppMediaFile?
        public let message: ChatContextResultMessage
    ...
    init(apiMessage: Api.BotInlineMessage) {
        switch apiMessage {
            ...
            case let .botInlineMessageMediaGeo(_, geo, heading, period, proximityNotificationRadius, replyMarkup):
                let media = elloappMediaMapFromApiGeoPoint(geo, title: nil, address: nil, provider: nil, venueId: nil, venueType: nil, liveBroadcastingTimeout: period, liveProximityNotificationRadius: proximityNotificationRadius, heading: heading)
                var parsedReplyMarkup: ReplyMarkupMessageAttribute?
                if let replyMarkup = replyMarkup {
                    parsedReplyMarkup = ReplyMarkupMessageAttribute(apiMarkup: replyMarkup)
                }
            ...
            case let .botInlineMessageMediaVenue(_, geo, title, address, provider, venueId, venueType, replyMarkup):
                let media = elloappMediaMapFromApiGeoPoint(geo, title: title, address: address, provider: provider, venueId: venueId, venueType: venueType, liveBroadcastingTimeout: nil, liveProximityNotificationRadius: nil, heading: nil)
                var parsedReplyMarkup: ReplyMarkupMessageAttribute?
                if let replyMarkup = replyMarkup {
                    parsedReplyMarkup = ReplyMarkupMessageAttribute(apiMarkup: replyMarkup)
                }
                self = .mapLocation(media: media, replyMarkup: parsedReplyMarkup)
            case let .botInlineMessageMediaContact(_, username, firstName, lastName, vcard, replyMarkup):
                let media = ElloAppMediaContact(firstName: firstName, lastName: lastName, username: username, peerId: nil, vCardData: vcard.isEmpty ? nil : vcard, userid: 0)
                var parsedReplyMarkup: ReplyMarkupMessageAttribute?
                if let replyMarkup = replyMarkup {
                    parsedReplyMarkup = ReplyMarkupMessageAttribute(apiMarkup: replyMarkup)
                }
            ...
            case let .botInlineMessageMediaInvoice(flags, title, description, photo, currency, totalAmount, replyMarkup):
                ...
                self = .invoice(media: ElloAppMediaInvoice(title: title, description: description, photo: photo.flatMap(ElloAppMediaWebFile.init), receiptMessageId: nil, currency: currency, totalAmount: totalAmount, startParam: "", extendedMedia: nil, flags: parsedFlags), replyMarkup: parsedReplyMarkup)
        }
    }
}
    ...
    init(apiResult: Api.BotInlineResult, queryId: Int64) {
        switch apiResult {
            case let .botInlineResult(_, id, type, title, description, url, thumb, content, sendMessage):
                self = .externalReference(ChatContextResult.ExternalReference(queryId: queryId, id: id, type: type, title: title, description: description, url: url, content: content.flatMap(ElloAppMediaWebFile.init), thumbnail: thumb.flatMap(ElloAppMediaWebFile.init), message: ChatContextResultMessage(apiMessage: sendMessage)))
            case let .botInlineMediaResult(_, id, type, photo, document, title, description, sendMessage):
                ...
                if let photo = photo, let parsedImage = elloappMediaImageFromApiPhoto(photo) {
                    image = parsedImage
                }
                if let document = document, let parsedFile = elloappMediaFileFromApiDocument(document) {
                    file = parsedFile
                }
