...
public func tagsForStoreMessage(incoming: Bool, attributes: [MessageAttribute], media: [Media], textEntities: [MessageTextEntity]?, isPinned: Bool) -> (MessageTags, GlobalMessageTags) {
    ...
    for attachment in media {
        if let _ = attachment as? ElloAppMediaImage {
            if !isSecret {
                tags.insert(.photoOrVideo)
                tags.insert(.photo)
            }
        } else if let file = attachment as? ElloAppMediaFile {
        ...
        } else if let webpage = attachment as? ElloAppMediaWebpage, case .Loaded = webpage.content {
            tags.insert(.webPage)
        } else if let action = attachment as? ElloAppMediaAction {
        ...
        } else if let location = attachment as? ElloAppMediaMap, location.liveBroadcastingTimeout != nil {
    ...
    if let textEntities = textEntities, !textEntities.isEmpty && !tags.contains(.webPage) {
        for entity in textEntities {
            switch entity.type {
                case .Url, .Email:
                    if media.isEmpty || !(media.first is ElloAppMediaWebpage) {
                        tags.insert(.webPage)
                    }
                default:
                    break
...
func apiMessagePeerId(_ messsage: Api.Message) -> PeerId? {
    switch messsage {
        case let .message(_, _, _, messagePeerId, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _):
...
func apiMessagePeerIds(_ message: Api.Message) -> [PeerId] {
    switch message {
        case let .message(_, _, fromId, chatPeerId, fwdHeader, viaBotId, _, _, _, media, _, entities, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _):
...
func apiMessageAssociatedMessageIds(_ message: Api.Message) -> [MessageId]? {
    switch message {
        case let .message(_, _, _, chatPeerId, _, _, replyTo, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _):
...
func textMediaAndExpirationTimerFromApiMedia(_ media: Api.MessageMedia?, _ peerId: PeerId) -> (Media?, Int32?, Bool?) {
    if let media = media {
        ...
        case let .messageMediaPhoto(_, photo, ttlSeconds):
            if let photo = photo {
                if let mediaImage = elloappMediaImageFromApiPhoto(photo) {
                    return (mediaImage, ttlSeconds, nil)
                }
            } else {
                return (ElloAppMediaExpiredContent(data: .image), nil, nil)
            }
        case let .messageMediaContact(username, firstName, lastName, vcard, userId):
            ...
            let mediaContact = ElloAppMediaContact(firstName: firstName, lastName: lastName, username: username, peerId: contactPeerId, vCardData: vcard.isEmpty ? nil : vcard, userid: userId)
            
            return (mediaContact, nil, nil)
        case let .messageMediaGeo(geo):
            let mediaMap = elloappMediaMapFromApiGeoPoint(geo, title: nil, address: nil, provider: nil, venueId: nil, venueType: nil, liveBroadcastingTimeout: nil, liveProximityNotificationRadius: nil, heading: nil)
            return (mediaMap, nil, nil)
        case let .messageMediaVenue(geo, title, address, provider, venueId, venueType):
            let mediaMap = elloappMediaMapFromApiGeoPoint(geo, title: title, address: address, provider: provider, venueId: venueId, venueType: venueType, liveBroadcastingTimeout: nil, liveProximityNotificationRadius: nil, heading: nil)
            return (mediaMap, nil, nil)
        case let .messageMediaGeoLive(_, geo, heading, period, proximityNotificationRadius):
            let mediaMap = elloappMediaMapFromApiGeoPoint(geo, title: nil, address: nil, provider: nil, venueId: nil, venueType: nil, liveBroadcastingTimeout: period, liveProximityNotificationRadius: proximityNotificationRadius, heading: heading)
            return (mediaMap, nil, nil)
        case let .messageMediaDocument(flags, document, ttlSeconds):
            if let document = document {
                if let mediaFile = elloappMediaFileFromApiDocument(document) {
                    return (mediaFile, ttlSeconds, (flags & (1 << 3)) != 0)
                }
            } else {
                return (ElloAppMediaExpiredContent(data: .file), nil, nil)
            }
        case let .messageMediaWebPage(webpage):
            if let mediaWebpage = elloappMediaWebpageFromApiWebpage(webpage, url: nil) {
                return (mediaWebpage, nil, nil)
            }
        case .messageMediaUnsupported:
            return (ElloAppMediaUnsupported(), nil, nil)
        case .messageMediaEmpty:
            break
        case let .messageMediaGame(game):
            return (ElloAppMediaGame(apiGame: game), nil, nil)
        case let .messageMediaInvoice(flags, title, description, photo, receiptMsgId, currency, totalAmount, startParam, apiExtendedMedia):
            var parsedFlags = ElloAppMediaInvoiceFlags()
            if (flags & (1 << 3)) != 0 {
            ...
            let extendedMedia: ElloAppExtendedMedia?
            ...
            return (ElloAppMediaInvoice(title: title, description: description, photo: photo.flatMap(ElloAppMediaWebFile.init), receiptMessageId: receiptMsgId.flatMap { MessageId(peerId: peerId, namespace: Namespaces.Message.Cloud, id: $0) }, currency: currency, totalAmount: totalAmount, startParam: startParam, extendedMedia: extendedMedia, flags: parsedFlags), nil, nil)
        case let .messageMediaPoll(poll, results):
            ...
            case let .poll(id, flags, question, answers, closePeriod, _):
                ...
                return (ElloAppMediaPoll(pollId: MediaId(namespace: Namespaces.Media.CloudPoll, id: id), publicity: publicity, kind: kind, text: question, options: answers.map(ElloAppMediaPollOption.init(apiOption:)), correctAnswers: nil, results: ElloAppMediaPollResults(apiResults: results), isClosed: (flags & (1 << 0)) != 0, deadlineTimeout: closePeriod), nil, nil)
            }
        case let .messageMediaDice(value, emoticon):
            return (ElloAppMediaDice(emoji: emoticon, value: value), nil, nil)
        }
    }
...
extension StoreMessage {
    convenience init?(apiMessage: Api.Message, namespace: MessageId.Namespace = Namespaces.Message.Cloud) {
        switch apiMessage {
            case let .message(flags, id, fromId, chatPeerId, fwdFrom, viaBotId, replyTo, date, message, media, replyMarkup, entities, views, forwards, replies, editDate, postAuthor, groupingId, reactions, restrictionReason, ttlPeriod, _, _, _, _, _, _, likes, isLiked):
                let resolvedFromId = fromId?.peerId ?? chatPeerId.peerId
                ...
                for case let file as ElloAppMediaFile in medias {
                ...
                if let entities = entities, !entities.isEmpty {
                    ...
                    loop: for media in medias {
                        switch media {
                            case _ as ElloAppMediaContact,
                                 _ as ElloAppMediaMap:
                                noEntities = true
                            break loop
                            default:
                                break
            ...
            self.init(id: MessageId(peerId: peerId, namespace: namespace, id: id), globallyUniqueId: nil, groupingKey: groupingId, threadId: threadId, timestamp: date, flags: storeFlags, tags: tags, globalTags: globalTags, localTags: [], forwardInfo: forwardInfo, authorId: authorId, text: messageText, attributes: attributes, media: medias, likes: likes, isLiked: isLiked.boolValue)
            case .messageEmpty:
                return nil
            case let .messageService(flags, id, fromId, chatPeerId, replyTo, date, action, ttlPeriod):
                ...
                var media: [Media] = []
                if let action = elloappMediaActionFromApiAction(action) {
                    media.append(action)
                }
                ...
                self.init(id: MessageId(peerId: peerId, namespace: namespace, id: id), globallyUniqueId: nil, groupingKey: nil, threadId: threadId, timestamp: date, flags: storeFlags, tags: tags, globalTags: globalTags, localTags: [], forwardInfo: nil, authorId: authorId, text: "", attributes: attributes, media: media, likes: 0)
            }
    }
}
