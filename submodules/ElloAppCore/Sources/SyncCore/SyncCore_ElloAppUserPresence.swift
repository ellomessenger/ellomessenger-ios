...
public final class ElloAppUserPresence: PeerPresence, Equatable {
    ...
    public static func ==(lhs: ElloAppUserPresence, rhs: ElloAppUserPresence) -> Bool {
        return lhs.status == rhs.status && lhs.lastActivity == rhs.lastActivity
    }
    
    public func isEqual(to: PeerPresence) -> Bool {
        if let to = to as? ElloAppUserPresence {
            return self == to
        } else {
            return false
        }
