...
public struct ElloAppWallpaperNativeCodable: Codable {
    public let value: ElloAppWallpaper

    public init(_ value: ElloAppWallpaper) {
        self.value = value
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        case 2:
            ...
            let representations = (try container.decode([AdaptedPostboxDecoder.RawObjectData].self, forKey: "i")).map { itemData in
                return ElloAppMediaImageRepresentation(decoder: PostboxDecoder(buffer: MemoryBuffer(data: itemData.data)))
            }
        ...
        case 3:
            ...
            if let fileData = try container.decodeIfPresent(AdaptedPostboxDecoder.RawObjectData.self, forKey: "file") {
                let file = ElloAppMediaFile(decoder: PostboxDecoder(buffer: MemoryBuffer(data: fileData.data)))
                self.value = .file(ElloAppWallpaper.File(
        ...
        case 4:
            ...
            self.value = .gradient(ElloAppWallpaper.Gradient(
...
public enum ElloAppWallpaper: Equatable {
    ...
    case image([ElloAppMediaImageRepresentation], WallpaperSettings)
    case file(File)
    ...
    public static func ==(lhs: ElloAppWallpaper, rhs: ElloAppWallpaper) -> Bool {
        switch lhs {
            case let .builtin(settings):
    ...
    public func isBasicallyEqual(to wallpaper: ElloAppWallpaper) -> Bool {
        switch self {
            case .builtin:
    ...
    public func withUpdatedSettings(_ settings: WallpaperSettings) -> ElloAppWallpaper {
        switch self {
        case .builtin:
