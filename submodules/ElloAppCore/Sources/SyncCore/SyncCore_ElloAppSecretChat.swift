...
public final class ElloAppSecretChat: Peer, Equatable {
    ...
    public var indexName: PeerIndexNameRepresentation {
        return .title(title: "", description: "", addressName: nil)
    }
    ...
    public func isEqual(_ other: Peer) -> Bool {
        if let other = other as? ElloAppSecretChat {
            return self == other
        } else {
            return false
        }
    ...
    public static func ==(lhs: ElloAppSecretChat, rhs: ElloAppSecretChat) -> Bool {
    ...
    public func withUpdatedEmbeddedState(_ embeddedState: SecretChatEmbeddedPeerState) -> ElloAppSecretChat {
        return ElloAppSecretChat(id: self.id, creationDate: self.creationDate, regularPeerId: self.regularPeerId, accessHash: self.accessHash, role: self.role, embeddedState: embeddedState, messageAutoremoveTimeout: self.messageAutoremoveTimeout)
    }
    
    public func withUpdatedMessageAutoremoveTimeout(_ messageAutoremoveTimeout: Int32?) -> ElloAppSecretChat {
        return ElloAppSecretChat(id: self.id, creationDate: self.creationDate, regularPeerId: self.regularPeerId, accessHash: self.accessHash, role: self.role, embeddedState: self.embeddedState, messageAutoremoveTimeout: messageAutoremoveTimeout)
    }
}
