...
public enum ElloAppMediaActionType: PostboxCoding, Equatable {
    case unknown
    case groupCreated(title: String)
    case addedMembers(peerIds: [PeerId])
    case removedMembers(peerIds: [PeerId])
    case photoUpdated(image: ElloAppMediaImage?)
    ...
    public init(decoder: PostboxDecoder) {
        ...
        switch rawValue {
            ...
            case 4:
                self = .photoUpdated(image: decoder.decodeObjectForKey("image") as? ElloAppMediaImage)
            case 5:
...
public final class ElloAppMediaAction: Media {
    public let id: MediaId? = nil
    public var peerIds: [PeerId] {
        return self.action.peerIds
    }
    
    public let action: ElloAppMediaActionType
    ...
    public init(action: ElloAppMediaActionType) {
        self.action = action
    }
    
    public init(decoder: PostboxDecoder) {
        self.action = ElloAppMediaActionType(decoder: decoder)
    }
    ...
    public func isEqual(to other: Media) -> Bool {
        if let other = other as? ElloAppMediaAction {
