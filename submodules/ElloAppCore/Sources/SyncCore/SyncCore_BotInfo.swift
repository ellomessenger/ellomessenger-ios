...
public final class BotInfo: PostboxCoding, Equatable {
    ...
    public let video: ElloAppMediaFile?
    public let commands: [BotCommand]
    public let menuButton: BotMenuButton
    
    public init(description: String, photo: ElloAppMediaImage?, video: ElloAppMediaFile?, commands: [BotCommand], menuButton: BotMenuButton) {
        self.description = description
        self.photo = photo
        self.video = video
    ...
    public init(decoder: PostboxDecoder) {
        ...
        if let video = decoder.decodeObjectForKey("vid", decoder: { ElloAppMediaFile(decoder: $0) }) as? ElloAppMediaFile {
            self.video = video
        } else {
            self.video = nil
        }
