...
public enum ElloAppMediaImageReferenceDecodingError: Error {
    case generic
}

public enum ElloAppMediaImageReference: PostboxCoding, Equatable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard let object = postboxDecoder.decodeRootObject() as? ElloAppMediaImageReference else {
            throw ElloAppMediaImageReferenceDecodingError.generic
        }
        self = object
    }
    ...
    public static func ==(lhs: ElloAppMediaImageReference, rhs: ElloAppMediaImageReference) -> Bool {
        switch lhs {
            case let .cloud(imageId, accessHash, fileReference):
...
public struct ElloAppMediaImageFlags: OptionSet {
    ...
    public static let hasStickers = ElloAppMediaImageFlags(rawValue: 1 << 0)
}

public enum ElloAppMediaImageDecodingError: Error {
    case generic
}

public final class ElloAppMediaImage: Media, Equatable, Codable {
    enum CodingKeys: String, CodingKey {
        case data
    }
    ...
    public final class VideoRepresentation: Equatable, PostboxCoding {
        ...
        public init(dimensions: PixelDimensions, resource: ElloAppMediaResource, startTimestamp: Double?) {
    ...
    public let representations: [ElloAppMediaImageRepresentation]
    public let videoRepresentations: [ElloAppMediaImage.VideoRepresentation]
    public let immediateThumbnailData: Data?
    public let reference: ElloAppMediaImageReference?
    public let partialReference: PartialMediaReference?
    public let peerIds: [PeerId] = []
    public let flags: ElloAppMediaImageFlags
    
    public var id: MediaId? {
        return self.imageId
    }
    
    public init(imageId: MediaId, representations: [ElloAppMediaImageRepresentation], videoRepresentations: [ElloAppMediaImage.VideoRepresentation] = [], immediateThumbnailData: Data?, reference: ElloAppMediaImageReference?, partialReference: PartialMediaReference?, flags: ElloAppMediaImageFlags) {
    ...
    public init(decoder: PostboxDecoder) {
        ...
        self.reference = decoder.decodeObjectForKey("rf", decoder: { ElloAppMediaImageReference(decoder: $0) }) as? ElloAppMediaImageReference
        self.partialReference = decoder.decodeAnyObjectForKey("prf", decoder: { PartialMediaReference(decoder: $0) }) as? PartialMediaReference
        self.flags = ElloAppMediaImageFlags(rawValue: decoder.decodeInt32ForKey("fl", orElse: 0))
    }
    ...
    public required init(from decoder: Decoder) throws {
        ...
        let data = try container.decode(Data.self, forKey: .data)
        let postboxDecoder = PostboxDecoder(buffer: MemoryBuffer(data: data))
        guard let object = postboxDecoder.decodeRootObject() as? ElloAppMediaImage else {
            throw ElloAppMediaImageDecodingError.generic
        }
    ...
    public func representationForDisplayAtSize(_ size: PixelDimensions) -> ElloAppMediaImageRepresentation? {
    ...
    public func isEqual(to other: Media) -> Bool {
        if let other = other as? ElloAppMediaImage {
    ...
    public static func ==(lhs: ElloAppMediaImage, rhs: ElloAppMediaImage) -> Bool {
        return lhs.isEqual(to: rhs)
    }
    
    public func withUpdatedPartialReference(_ partialReference: PartialMediaReference?) -> ElloAppMediaImage {
        return ElloAppMediaImage(imageId: self.imageId, representations: self.representations, videoRepresentations: self.videoRepresentations, immediateThumbnailData: self.immediateThumbnailData, reference: self.reference, partialReference: partialReference, flags: self.flags)
    }
}

public final class ElloAppMediaImageRepresentation: PostboxCoding, Hashable, CustomStringConvertible {
    public let dimensions: PixelDimensions
    public let resource: ElloAppMediaResource
    public let progressiveSizes: [Int32]
    public let immediateThumbnailData: Data?
    
    public init(dimensions: PixelDimensions, resource: ElloAppMediaResource, progressiveSizes: [Int32], immediateThumbnailData: Data?) {
        self.dimensions = dimensions
        self.resource = resource
    ...
    public func isSemanticallyEqual(to other: ElloAppMediaImageRepresentation) -> Bool {
...
public func ==(lhs: ElloAppMediaImageRepresentation, rhs: ElloAppMediaImageRepresentation) -> Bool {
    if lhs.dimensions != rhs.dimensions {
        return false
    }
    if !lhs.resource.isEqual(to: rhs.resource) {
