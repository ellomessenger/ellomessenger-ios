...
public struct PremiumPromoConfiguration: Codable, Equatable {
    ...
    private struct DictionaryPair: Codable {
        ...
        init(_ key: String, value: ElloAppMediaFile) {
            self.key = key
            self.value = value
        }
        ...
        init(from decoder: Decoder) throws {
            ...
            self.value = try container.decode(ElloAppMediaFile.self, forKey: "v")
        }
    ...
    public var videos: [String: ElloAppMediaFile]
    ...
    init(status: String, statusEntities: [MessageTextEntity], videos: [String: ElloAppMediaFile], premiumProductOptions: [PremiumProductOption]) {
        self.status = status
