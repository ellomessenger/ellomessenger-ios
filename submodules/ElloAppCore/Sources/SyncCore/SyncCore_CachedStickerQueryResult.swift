...
public final class CachedStickerQueryResult: Codable {
    public let items: [ElloAppMediaFile]
    public let hash: Int64
    public let timestamp: Int32
    
    public init(items: [ElloAppMediaFile], hash: Int64, timestamp: Int32) {
        self.items = items
        self.hash = hash
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.items = (try container.decode([AdaptedPostboxDecoder.RawObjectData].self, forKey: "it")).map { itemData in
            return ElloAppMediaFile(decoder: PostboxDecoder(buffer: MemoryBuffer(data: itemData.data)))
        }
