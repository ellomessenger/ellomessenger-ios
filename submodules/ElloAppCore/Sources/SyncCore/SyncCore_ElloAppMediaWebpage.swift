...
private enum ElloAppMediaWebpageAttributeTypes: Int32 {
    case unsupported
    case theme
}

public enum ElloAppMediaWebpageAttribute: PostboxCoding, Equatable {
    case unsupported
    case theme(ElloAppMediaWebpageThemeAttribute)
    
    public init(decoder: PostboxDecoder) {
        switch decoder.decodeInt32ForKey("r", orElse: 0) {
            case ElloAppMediaWebpageAttributeTypes.theme.rawValue:
                self = .theme(decoder.decodeObjectForKey("a", decoder: { ElloAppMediaWebpageThemeAttribute(decoder: $0) }) as! ElloAppMediaWebpageThemeAttribute)
            default:
                self = .unsupported
        }
    }
    ...
    public func encode(_ encoder: PostboxEncoder) {
        switch self {
            case .unsupported:
                encoder.encodeInt32(ElloAppMediaWebpageAttributeTypes.unsupported.rawValue, forKey: "r")
            case let .theme(attribute):
                encoder.encodeInt32(ElloAppMediaWebpageAttributeTypes.theme.rawValue, forKey: "r")
                encoder.encodeObject(attribute, forKey: "a")
        }
    }
}

public final class ElloAppMediaWebpageThemeAttribute: PostboxCoding, Equatable {
    public static func == (lhs: ElloAppMediaWebpageThemeAttribute, rhs: ElloAppMediaWebpageThemeAttribute) -> Bool {
    ...
    public let settings: ElloAppThemeSettings?
    
    public init(files: [ElloAppMediaFile], settings: ElloAppThemeSettings?) {
        self.files = files
    ...
    public init(decoder: PostboxDecoder) {
        ...
        self.settings = decoder.decode(ElloAppThemeSettings.self, forKey: "settings")
    }
...
public final class ElloAppMediaWebpageLoadedContent: PostboxCoding, Equatable {
    ...
    public let attributes: [ElloAppMediaWebpageAttribute]
    public let instantPage: InstantPage?
    
    public init(url: String, displayUrl: String, hash: Int32, type: String?, websiteName: String?, title: String?, text: String?, embedUrl: String?, embedType: String?, embedSize: PixelDimensions?, duration: Int?, author: String?, image: ElloAppMediaImage?, file: ElloAppMediaFile?, attributes: [ElloAppMediaWebpageAttribute], instantPage: InstantPage?) {
        self.url = url
    ...
    public init(decoder: PostboxDecoder) {
        ...
        if let image = decoder.decodeObjectForKey("im") as? ElloAppMediaImage {
            self.image = image
        } else {
            self.image = nil
        }
        
        if let file = decoder.decodeObjectForKey("fi") as? ElloAppMediaFile {
            self.file = file
        } else {
            self.file = nil
        }
        
        var effectiveAttributes: [ElloAppMediaWebpageAttribute] = []
        if let attributes = decoder.decodeObjectArrayWithDecoderForKey("attr") as [ElloAppMediaWebpageAttribute]? {
            effectiveAttributes.append(contentsOf: attributes)
        }
        if let legacyFiles = decoder.decodeOptionalObjectArrayWithDecoderForKey("fis") as [ElloAppMediaFile]? {
            effectiveAttributes.append(.theme(ElloAppMediaWebpageThemeAttribute(files: legacyFiles, settings: nil)))
        }
...
public func ==(lhs: ElloAppMediaWebpageLoadedContent, rhs: ElloAppMediaWebpageLoadedContent) -> Bool {
    if lhs.url != rhs.url
...
public enum ElloAppMediaWebpageContent {
    case Pending(Int32, String?)
    case Loaded(ElloAppMediaWebpageLoadedContent)
}

public final class ElloAppMediaWebpage: Media, Equatable {
    ...
    public let content: ElloAppMediaWebpageContent
    
    public init(webpageId: MediaId, content: ElloAppMediaWebpageContent) {
    ...
    public init(decoder: PostboxDecoder) {
        ...
        if decoder.decodeInt32ForKey("ct", orElse: 0) == 0 {
            ...
            self.content = .Loaded(ElloAppMediaWebpageLoadedContent(decoder: decoder))
        }
    }
    ...
    public func isEqual(to other: Media) -> Bool {
        if let other = other as? ElloAppMediaWebpage, self.webpageId == other.webpageId {
            return self == other
        }
        return false
    }
    ...
    public static func ==(lhs: ElloAppMediaWebpage, rhs: ElloAppMediaWebpage) -> Bool {
