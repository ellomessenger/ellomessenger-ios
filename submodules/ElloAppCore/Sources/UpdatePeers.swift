...
public func updatePeers(transaction: Transaction, peers: [Peer], update: (Peer?, Peer) -> Peer?) {
    transaction.updatePeersInternal(peers, update: { previous, updated in
        ...
        if let previous = previous as? ElloAppUser, let updatedUser = updated as? ElloAppUser {
            updated = ElloAppUser.merge(lhs: previous, rhs: updatedUser)
        } else if let previous = previous as? ElloAppChannel, let updatedChannel = updated as? ElloAppChannel {
        ...
        switch peerId.namespace {
            ...
            case Namespaces.Peer.CloudGroup:
                if let group = updated as? ElloAppGroup {
            ...
            case Namespaces.Peer.CloudChannel:
                if let channel = updated as? ElloAppChannel {
            ...
            case Namespaces.Peer.SecretChat:
                if let secretChat = updated as? ElloAppSecretChat {
                    let isActive: Bool
...
func updatePeerPresences(transaction: Transaction, accountPeerId: PeerId, peerPresences: [PeerId: Api.User]) {
    ...
    for (peerId, user) in peerPresences {
        guard let presence = ElloAppUserPresence(apiUser: user) else {
            continue
        }
    ...
    transaction.updatePeerPresencesInternal(presences: parsedPresences, merge: { previous, updated in
        if let previous = previous as? ElloAppUserPresence, let updated = updated as? ElloAppUserPresence, previous.lastActivity != updated.lastActivity {
            return ElloAppUserPresence(status: updated.status, lastActivity: max(previous.lastActivity, updated.lastActivity))
        }
        return updated
    })
}
...
func updatePeerPresenceLastActivities(transaction: Transaction, accountPeerId: PeerId, activities: [PeerId: Int32]) {
    ...
    for (peerId, timestamp) in activities {
        transaction.updatePeerPresenceInternal(peerId: peerId, update: { previous in
            if let previous = previous as? ElloAppUserPresence, previous.lastActivity < timestamp {
                ...
                return ElloAppUserPresence(status: updatedStatus, lastActivity: timestamp)
            }
            return previous
        })
    }
