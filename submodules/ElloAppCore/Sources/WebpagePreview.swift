...
public func webpagePreview(account: Account, url: String, webpageId: MediaId? = nil) -> Signal<ElloAppMediaWebpage?, NoError> {
    return webpagePreviewWithProgress(account: account, url: url)
    |> mapToSignal { next -> Signal<ElloAppMediaWebpage?, NoError> in
...
public enum WebpagePreviewWithProgressResult {
    case result(ElloAppMediaWebpage?)
    case progress(Float)
}
...
public func webpagePreviewWithProgress(account: Account, url: String, webpageId: MediaId? = nil) -> Signal<WebpagePreviewWithProgressResult, NoError> {
    return account.postbox.transaction { transaction -> Signal<WebpagePreviewWithProgressResult, NoError> in
        if let webpageId = webpageId, let webpage = transaction.getMedia(webpageId) as? ElloAppMediaWebpage {
                    ...
                    case let .result(result):
                        ...
                        switch result {
                            case let .messageMediaWebPage(webpage):
                                if let media = elloappMediaWebpageFromApiWebpage(webpage, url: url) {
...
public func actualizedWebpage(postbox: Postbox, network: Network, webpage: ElloAppMediaWebpage) -> Signal<ElloAppMediaWebpage, NoError> {
    if case let .Loaded(content) = webpage.content {
        return network.request(Api.functions.messages.getWebPage(url: content.url, hash: content.hash))
            ...
            |> mapToSignal { result -> Signal<ElloAppMediaWebpage, NoError> in
                if let result = result, let updatedWebpage = elloappMediaWebpageFromApiWebpage(result, url: nil), case .Loaded = updatedWebpage.content, updatedWebpage.webpageId == webpage.webpageId {
                    return postbox.transaction { transaction -> ElloAppMediaWebpage in
...
func updatedRemoteWebpage(postbox: Postbox, network: Network, webPage: WebpageReference) -> Signal<ElloAppMediaWebpage?, NoError> {
    if case let .webPage(id, url) = webPage.content {
        ...
        |> mapToSignal { result -> Signal<ElloAppMediaWebpage?, NoError> in
            if let result = result, let updatedWebpage = elloappMediaWebpageFromApiWebpage(result, url: nil), case .Loaded = updatedWebpage.content, updatedWebpage.webpageId.id == id {
                return postbox.transaction { transaction -> ElloAppMediaWebpage? in
