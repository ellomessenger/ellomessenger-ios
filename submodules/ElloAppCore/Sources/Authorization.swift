...
func switchToAuthorizedAccount(transaction: AccountManagerModifier<ElloAppAccountManagerTypes>, account: UnauthorizedAccount) {
...
public func sendAuthorizationCode(accountManager: AccountManager<ElloAppAccountManagerTypes>, account: UnauthorizedAccount, phoneNumber: String, apiId: Int32, apiHash: String, syncContacts: Bool) -> Signal<UnauthorizedAccount, AuthorizationCodeRequestError> {
...
public enum AuthorizationCodeVerificationError: LocalizedError {
    case invalidCode
    case limitExceeded
    case generic(String = "")
    case codeExpired
...
public func authorizeNewWith(username: String,
                             password: String,
                             account: UnauthorizedAccount,
                             accountManager: AccountManager<ElloAppAccountManagerTypes>) -> Signal<Api.authNew.RegValidation, AuthorizationCodeVerificationError> {
    return account.network.request(Api.authNew.signIn(username: username, password: password), automaticFloodWait: false)
    |> timeout(kTimeout, queue: .mainQueue(), alternate: .fail(MTRpcError(errorCode: 400, errorDescription: "INTERNAL_TIMEOUT")))
    |> mapError { error -> AuthorizationCodeVerificationError in
        return .generic(error.errorDescription)
    }
    |> mapToSignal { result -> Signal<Api.authNew.RegValidation, AuthorizationCodeVerificationError> in
        return .single(result)
    }
}

//public func authorizeWith(username: String, password: String, account: UnauthorizedAccount, accountManager: AccountManager<ElloAppAccountManagerTypes>) -> Signal<AuthorizeWithCodeResult, AuthorizationCodeVerificationError> {
//    
...
//                switch result {
//                    case .signUp:
//                        return .single(.signUp(AuthorizationSignUpData(number: username, codeHash: password, code: .phoneCode(""), termsOfService: nil, syncContacts: false)))
//                    case let .password(hint):
//                        transaction.setState(UnauthorizedAccountState(isTestingEnvironment: account.testingEnvironment, masterDatacenterId: account.masterDatacenterId, contents: .passwordEntry(hint: hint, number: username, code: .phoneCode(""), suggestReset: false, syncContacts: false)))
//                        return .single(.loggedIn)
//                    case let .authorization(authorization):
//                        switch authorization {
//                            case let .authorization(_, _, _, user):
//                                let user = ElloAppUser(user: user)
...
//                                    return .loggedIn
//                                }
//                            case .authorizationSignUpRequired(_, _):
//                                return .single(.signUp(AuthorizationSignUpData(number: username, codeHash: password, code: .phoneCode(""), termsOfService: nil, syncContacts: false)))
//                        }
//                }
//            }
...
public func authorizeWithPassword(accountManager: AccountManager<ElloAppAccountManagerTypes>, account: UnauthorizedAccount, password: String, syncContacts: Bool) -> Signal<Void, AuthorizationPasswordVerificationError> {
...
public func loginWithRecoveredAccountData(accountManager: AccountManager<ElloAppAccountManagerTypes>, account: UnauthorizedAccount, recoveredAccountData: RecoveredAccountData, syncContacts: Bool) -> Signal<Never, NoError> {
    return account.postbox.transaction { transaction -> Signal<Void, NoError> in
...
public func resetPassword(with email: String, network: Network) -> Signal<Api.Response.SendVerificationCode, ResetPasswordError> {
    return network.request(Api.forgotPassword.forgotPassword(email))
    |> mapError { error -> ResetPasswordError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
    |> mapToSignal { result -> Signal<Api.Response.SendVerificationCode, ResetPasswordError> in
        print(result)
...
public func createNewPassword(with email: String, code: String, password: String, network: Network) -> Signal<Api.forgotPassword.CreateNewPasswordResponse, ResetPasswordError> {
    return network.request(Api.forgotPassword.createNewPassword(email, code: code, password: password))
    |> mapError { error -> ResetPasswordError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
    |> mapToSignal { result -> Signal<Api.forgotPassword.CreateNewPasswordResponse, ResetPasswordError> in
        print(result)
...
// MARK: - Change email
public enum BaseError: LocalizedError, CustomStringConvertible {
    case message(String)
...
public func changeEmailCode(with email: String, network: Network) -> Signal<Api.Response.SendVerificationCode, BaseError> {
    return network.request(Api.changeEmail.changeEmailCode(email))
    |> mapError { error -> BaseError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
    |> mapToSignal { result -> Signal<Api.Response.SendVerificationCode, BaseError> in
        print(result)
...
public func saveNewEmail(with email: String, code: String, network: Network) -> Signal<Api.Response.BaseResponse, BaseError> {
    return network.request(Api.changeEmail.saveNewEmail(email, code: code))
    |> mapError { error -> BaseError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
    |> mapToSignal { result -> Signal<Api.Response.BaseResponse, BaseError> in
        print(result)
        return .single(result)
    }
}

public func changePassword(with password: String, newPassword: String, code: String, network: Network) -> Signal<Api.Response.BaseResponse, BaseError> {
    return network.request(Api.changePassword.changePassword(password, newPassword: newPassword, code: code))
    |> mapError { error -> BaseError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
...
public func sendVerifyCode(with email: String, network: Network) -> Signal<Api.Response.SendVerificationCode, BaseError> {
    return network.request(Api.sendVerifyCode.sendVerifyCode(email))
    |> mapError { error -> BaseError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
...
public func getFullUser(network: Network) -> Signal<Api.UserFull, BaseError> {
    return network.request(Api.functions.users.getFullUser(id: .inputUserSelf))
    |> mapError { error -> BaseError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
    |> mapToSignal { result -> Signal<Api.UserFull, BaseError> in
        print(result)
        switch result {
        case let .userFull(fullUser, _, _):
            return .single(fullUser)
        }
    }
}

public func checkUsername(username: String, network: Network) -> Signal<Api.Bool, BaseError> {
    return network.request(Api.functions.channels.checkUsername(channel: .inputChannelEmpty, username: username))
    |> mapError { error -> BaseError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
    |> mapToSignal { result -> Signal<Api.Bool, BaseError> in
        print(result)
...
public func checkEmailAvailability(email: String, network: Network) -> Signal<Api.Bool, BaseError> {
    return network.request(Api.authNew.checkEmailAvailability(email: email))
    |> mapError { error -> BaseError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
...
public func deleteAccount(email: String, password: String, network: Network) -> Signal<Api.Response.SendVerificationCode, BaseError> {
    return network.request(Api.deleteAccount.deleteAccount(email, password: password))
    |> mapError { error -> BaseError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
...
public func confirmDeleteAccount(code: String, network: Network) -> Signal<Api.Response.BaseResponse, BaseError> {
    return network.request(Api.deleteAccount.confirmDeleteAccount(code))
    |> mapError { error -> BaseError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
...
public func getDeleteAccountInfo(network: Network) -> Signal<Api.Response.DeleteAccountResponse, BaseError> {
    return network.request(Api.deleteAccount.getDeleteAccountInfo())
    |> mapError { error -> BaseError in
        debugPrint(error)
        return .message(error.errorDescription)
    }
    |> mapToSignal { result -> Signal<Api.Response.DeleteAccountResponse, BaseError> in
        print(result)
...
public func signUpWith(username: String, password: String, account: UnauthorizedAccount, accountManager: AccountManager<ElloAppAccountManagerTypes>) -> Signal<Void, SignUpError> {
    return account.postbox.transaction { transaction -> Signal<Void, SignUpError> in
            return account.network.request(Api.functions.auth.signUp(phoneNumber: username, phoneCodeHash: password, firstName: username, lastName: ""))
...
public func signUpNewWith(username: String, 
                          password: String,
                          email: String,
                          gender: String,
                          countryCode:String,
                          ...
                          type:String,
                          account: UnauthorizedAccount,
                          referalCode:String? = nil) -> Signal<Api.authNew.RegValidation, SignUpError> {
    return account.network.request(Api.authNew.signUp(username: username,
                                                      password: password,
                                                      email: email,
    ...
    |> mapToSignal { result -> Signal<Api.authNew.RegValidation, SignUpError> in
        return .single(result)
    }
}

public func signUpWithName(accountManager: AccountManager<ElloAppAccountManagerTypes>, account: UnauthorizedAccount, firstName: String, lastName: String, avatarData: Data?, avatarVideo: Signal<UploadedPeerPhotoData?, NoError>?, videoStartTimestamp: Double?, forcedPasswordSetupNotice: @escaping (Int32) -> (NoticeEntryKey, CodableEntry)?) -> Signal<Void, SignUpError> {
    return account.postbox.transaction { transaction -> Signal<Void, SignUpError> in
...
public func signUpCodeVerification(email: String, code: String, account: UnauthorizedAccount, accountManager: AccountManager<ElloAppAccountManagerTypes>) -> Signal<AuthorizeWithCodeResult, AuthorizationCodeVerificationError> {
    return account.network.request(Api.authNew.signUpConfirmationRequest(username: email, verificationCode: code))
    |> mapError { error -> AuthorizationCodeVerificationError in
        return .generic(error.errorDescription)
    }
    |> mapToSignal { result -> Signal<AuthorizeWithCodeResult, AuthorizationCodeVerificationError> in
        debugPrint(result)
        let peerId = PeerId(Int64(result.user.id))
        return account.postbox.transaction { transaction -> Signal<AuthorizeWithCodeResult, NoError> in
            let state = AuthorizedAccountState(isTestingEnvironment: account.testingEnvironment, masterDatacenterId: account.masterDatacenterId, peerId: peerId, state: nil)
            initializedAppSettingsAfterLogin(transaction: transaction, appVersion: account.networkArguments.appVersion, syncContacts: false)
            transaction.setState(state)
...
/// Code verification for Sign In
public func signInCodeVerification(username: String, code: String, account: UnauthorizedAccount, accountManager: AccountManager<ElloAppAccountManagerTypes>) -> Signal<AuthorizeWithCodeResult, AuthorizationCodeVerificationError> {
    return account.network.request(Api.authNew.signInConfirmationRequest(username: username, verificationCode: code))
    |> mapError { error -> AuthorizationCodeVerificationError in
        return .generic(error.errorDescription)
    }
    |> mapToSignal { result -> Signal<AuthorizeWithCodeResult, AuthorizationCodeVerificationError> in
...
/// Common code verifications, legacy method
public func commonCodeVerification(email: String, code: String, account: UnauthorizedAccount) -> Signal<Void, SignUpError> {
    return account.postbox.transaction { transaction -> Signal<Void, SignUpError> in
        return account.network.request(Api.authNew.confirmEmail(email, code: code))
        |> mapError{ error -> SignUpError in
            return .generic(error.errorDescription)
        }
        |> mapToSignal{ response -> Signal<Void, SignUpError> in
                .single(Void())
        }
    }
    |> mapError { _ -> SignUpError in
...
/// Resend verification code to not verified user
public func resendSignUpConfirmationRequest(username: String,
                                            account: UnauthorizedAccount) -> Signal<Api.authNew.RegValidation, AuthorizationCodeVerificationError> {
    return account.network.request(Api.authNew.resendSignUpConfirmationRequest(username: username))
    |> mapError { error -> AuthorizationCodeVerificationError in
        return .generic(error.errorDescription)
    }
    |> mapToSignal { result -> Signal<Api.authNew.RegValidation, AuthorizationCodeVerificationError> in
