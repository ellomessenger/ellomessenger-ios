...
func _internal_stopGroupCall(account: Account, peerId: PeerId, callId: Int64, accessHash: Int64) -> Signal<Never, StopGroupCallError> {
    ...
    |> mapToSignal { result -> Signal<Never, StopGroupCallError> in
        return account.postbox.transaction { transaction -> Void in
            ...
            if var peer = transaction.getPeer(peerId) as? ElloAppChannel {
            ...
            if var peer = transaction.getPeer(peerId) as? ElloAppGroup {
                var flags = peer.flags
                flags.remove(.hasVoiceChat)
                flags.remove(.hasActiveVoiceChat)
    ...
    return postbox.transaction { transaction -> Api.InputPeer? in
        ...
        |> mapToSignal { result in
            ...
            case let .joinAsPeers(_, chats, _):
                ...
                let peers = chats.compactMap(parseElloAppGroupOrChannel)
                for chat in chats {
                    if let groupOrChannel = parseElloAppGroupOrChannel(chat: chat) {
                        ...
                        case let .chat(_, _, _, _, _, participantsCount, _, _, _, _, _):
                            subscribers[groupOrChannel.id] = participantsCount
                        default:
                            break
                        }
