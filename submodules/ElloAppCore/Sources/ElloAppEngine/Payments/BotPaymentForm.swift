...
func _internal_fetchBotPaymentInvoice(postbox: Postbox, network: Network, source: BotPaymentInvoiceSource) -> Signal<ElloAppMediaInvoice, BotPaymentFormRequestError> {
    ...
    |> mapToSignal { invoice -> Signal<ElloAppMediaInvoice, BotPaymentFormRequestError> in
        ...
        |> mapToSignal { result -> Signal<ElloAppMediaInvoice, BotPaymentFormRequestError> in
            return postbox.transaction { transaction -> ElloAppMediaInvoice in
                switch result {
                case let .paymentForm(_, _, _, title, description, photo, invoice, _, _, _, _, _, _, _, _):
                    ...
                    return ElloAppMediaInvoice(title: title, description: description, photo: photo.flatMap(ElloAppMediaWebFile.init), receiptMessageId: nil, currency: parsedInvoice.currency, totalAmount: 0, startParam: "", extendedMedia: nil, flags: parsedFlags)
                }
            }
                ...
                switch result {
                    case let .paymentForm(flags, id, botId, title, description, photo, invoice, providerId, url, nativeProvider, nativeParams, additionalMethods, savedInfo, savedCredentials, apiUsers):
                        ...
                        for user in apiUsers {
                            let parsed = ElloAppUser(user: user)
                            peers.append(parsed)
                        }
                            ...
                            for media in message.media {
                                if let action = media as? ElloAppMediaAction {
                                    if case .paymentSent = action.action {
                                        ...
                                        case let .slug(slug):
                                            for media in message.media {
                                                if let action = media as? ElloAppMediaAction, case let .paymentSent(_, _, invoiceSlug?, _, _) = action.action, invoiceSlug == slug {
...
public struct BotPaymentReceipt : Equatable {
    ...
    public let invoiceMedia: ElloAppMediaInvoice
        ...
        |> mapToSignal { result -> Signal<BotPaymentReceipt, RequestBotPaymentReceiptError> in
            return account.postbox.transaction { transaction -> BotPaymentReceipt in
                ...
                case let .paymentReceipt(_, _, botId, _, title, description, photo, invoice, info, shipping, tipAmount, currency, totalAmount, credentialsTitle, users):
                    var peers: [Peer] = []
                    for user in users {
                        peers.append(ElloAppUser(user: user))
                    }
                    ...
                    let invoiceMedia = ElloAppMediaInvoice(
                        title: title,
                        description: description,
                        photo: photo.flatMap(ElloAppMediaWebFile.init),
