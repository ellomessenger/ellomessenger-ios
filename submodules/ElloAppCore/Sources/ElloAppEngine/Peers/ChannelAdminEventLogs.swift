...
public enum AdminLogEventAction {
    ...
    case changePhoto(prev: ([ElloAppMediaImageRepresentation], [ElloAppMediaImage.VideoRepresentation]), new: ([ElloAppMediaImageRepresentation], [ElloAppMediaImage.VideoRepresentation]))
    ...
    case updateDefaultBannedRights(prev: ElloAppChatBannedRights, new: ElloAppChatBannedRights)
            ...
            return network.request(Api.functions.channels.getAdminLog(flags: flags, channel: inputChannel, q: query ?? "", eventsFilter: eventsFilter, admins: inputAdmins, maxId: maxId, minId: minId, limit: limit)) |> mapToSignal { result in
                ...
                case let .adminLogResults(apiEvents, apiChats, apiUsers):
                    ...
                    for apiChat in apiChats {
                        if let peer = parseElloAppGroupOrChannel(chat: apiChat) {
                            peers[peer.id] = peer
                        }
                    }
                    for apiUser in apiUsers {
                        let peer = ElloAppUser(user: apiUser)
                        peers[peer.id] = peer
                    }
                            ...
                            case let .channelAdminLogEvent(id, date, userId, apiAction):
                                ...
                                switch apiAction {
                                    ...
                                    case let .channelAdminLogEventActionChangePhoto(prev, new):
                                        let previousImage = elloappMediaImageFromApiPhoto(prev)
                                        let newImage = elloappMediaImageFromApiPhoto(new)
                                    ...
                                    case let .channelAdminLogEventActionDefaultBannedRights(prevBannedRights, newBannedRights):
                                        action = .updateDefaultBannedRights(prev: ElloAppChatBannedRights(apiBannedRights: prevBannedRights), new: ElloAppChatBannedRights(apiBannedRights: newBannedRights))
