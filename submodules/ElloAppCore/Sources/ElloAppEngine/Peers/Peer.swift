...
public enum EnginePeer: Equatable {
    ...
    public enum IndexName: Equatable {
        case title(title: String, description: String, addressName: String?)
        case personName(first: String, last: String, addressName: String?, phoneNumber: String?)

        public var isEmpty: Bool {
            switch self {
            case let .title(title, description, addressName):
                if !title.isEmpty {
                    return false
                }
    ...
    case user(ElloAppUser)
    case legacyGroup(ElloAppGroup)
    case channel(ElloAppChannel)
    case secretChat(ElloAppSecretChat)
    ...
    public var isAIPhoenixBot: Bool {
        id.isAIPhoenixBotID
    }
    
    public var isAIBusinessForStudentsBot: Bool {
        id.isAIBusinessForStudentsBotID
    }
    
    public var isAICancerPreventationBot: Bool {
        id.isAICancerPreventationBotID
    }
    
    public var isNotificationsBot: Bool {
        id.isNotificationsBotID
    }
}
...
public extension EnginePeer.NotificationSettings {
    init(_ notificationSettings: ElloAppPeerNotificationSettings) {
        self.init(
    ...
    func _asNotificationSettings() -> ElloAppPeerNotificationSettings {
        return ElloAppPeerNotificationSettings(
...
public extension EnginePeer.Presence {
    init(_ presence: PeerPresence) {
        if let presence = presence as? ElloAppUserPresence {
    ...
    func _asPresence() -> ElloAppUserPresence {
        ...
        return ElloAppUserPresence(status: mappedStatus, lastActivity: self.lastActivity)
    }
}
...
public extension EnginePeer.IndexName {
    init(_ indexName: PeerIndexNameRepresentation) {
        switch indexName {
        case let .title(title, description, addressName):
            self = .title(title: title, description: description, addressName: addressName)
        case let .personName(first, last, addressName, phoneNumber):
    ...
    func _asIndexName() -> PeerIndexNameRepresentation {
        switch self {
        case let .title(title, description, addressName):
            return .title(title: title, description: description, addressName: addressName)
        case let .personName(first, last, addressName, phoneNumber):
...
public extension EnginePeer {
    ...
    var profileImageRepresentations: [ElloAppMediaImageRepresentation] {
        return self._asPeer().profileImageRepresentations
    }

    var smallProfileImage: ElloAppMediaImageRepresentation? {
        return self._asPeer().smallProfileImage
    }

    var largeProfileImage: ElloAppMediaImageRepresentation? {
...
public extension EnginePeer {
    ...
    var isPhotoEmpty:Bool {
        switch self {
        case let .user(user):
            return user.photo.isEmpty
        case let .legacyGroup(legacyGroup):
            return legacyGroup.photo.isEmpty
        case let .channel(channel):
            return channel.photo.isEmpty
        case let .secretChat(secretChat):
            return nil == secretChat.largeProfileImage && nil == secretChat.smallProfileImage
        }
    }
}
