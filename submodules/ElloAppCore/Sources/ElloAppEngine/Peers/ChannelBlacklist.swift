...
func _internal_updateChannelMemberBannedRights(account: Account, peerId: PeerId, memberId: PeerId, rights: ElloAppChatBannedRights?) -> Signal<(ChannelParticipant?, RenderedChannelParticipant?, Bool), NoError> {
                    ...
                    return account.postbox.transaction { transaction -> (ChannelParticipant?, RenderedChannelParticipant?, Bool) in
                        transaction.updatePeerCachedData(peerIds: Set([peerId]), update: { _, cachedData -> CachedPeerData? in
                            if let cachedData = cachedData as? CachedChannelData {
                                ...
                                if let memberPeer = memberPeer as? ElloAppUser, let _ = memberPeer.botInfo {
                                    if isMember != wasMember {
                                        if isMember {
                                            var updatedBotInfos = updatedData.botInfos
                                            if let botInfo = updatedBotInfos.first(where: { $0.peerId == memberPeer.id }) {
                                                updatedBotInfos.append(CachedPeerBotInfo(peerId: memberPeer.id, botInfo: botInfo.botInfo))
                                            }
...
func _internal_updateDefaultChannelMemberBannedRights(account: Account, peerId: PeerId, rights: ElloAppChatBannedRights) -> Signal<Never, NoError> {
