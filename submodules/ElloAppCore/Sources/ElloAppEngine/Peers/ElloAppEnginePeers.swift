...
public extension ElloAppEngine {
    ...
    final class Peers {
        ...
        public func removeCustomNotificationSettings(peerIds: [PeerId]) -> Signal<Never, NoError> {
            return self.account.postbox.transaction { transaction -> Void in
                for peerId in peerIds {
                    ElloAppCore.updatePeerNotificationSoundInteractive(transaction: transaction, peerId: peerId, sound: .default)
                    ElloAppCore.updatePeerMuteSetting(transaction: transaction, peerId: peerId, muteInterval: nil)
                    ElloAppCore.updatePeerDisplayPreviewsSetting(transaction: transaction, peerId: peerId, displayPreviews: .default)
                }
            }
            |> ignoreValues
        }
        ...
        public func updateChannelMemberBannedRights(peerId: PeerId, memberId: PeerId, rights: ElloAppChatBannedRights?) -> Signal<(ChannelParticipant?, RenderedChannelParticipant?, Bool), NoError> {
            return _internal_updateChannelMemberBannedRights(account: self.account, peerId: peerId, memberId: memberId, rights: rights)
        }

        public func updateDefaultChannelMemberBannedRights(peerId: PeerId, rights: ElloAppChatBannedRights) -> Signal<Never, NoError> {
            return _internal_updateDefaultChannelMemberBannedRights(account: self.account, peerId: peerId, rights: rights)
        }

        public func createChannel(title: String, 
                                  description: String?,
            ...
            return _internal_createChannel(account: self.account, title: title, description: description, payType: payType, cost: cost, category: category, country: country, adult: adult, startDate: startDate, endDate: endDate, genre: genre, subGenre: subGenre, name: name)
        }
        ...
        public func requestPeerPhotos(peerId: PeerId) -> Signal<[ElloAppPeerPhoto], NoError> {
        ...
        public func joinPaidChannel(peerId: PeerId) -> Signal<RenderedChannelParticipant?, JoinPaidChannelError> {
            return _internal_joinPaidChannel(account: self.account, peerId: peerId)
        }
        ...
        public func updateChannelAdminRights(peerId: PeerId, adminId: PeerId, rights: ElloAppChatAdminRights?, rank: String?) -> Signal<(ChannelParticipant?, RenderedChannelParticipant), UpdateChannelAdminRightsError> {
        ...
        public func fetchAndUpdateCachedBotData(peerId: PeerId) -> Signal<Bool, NoError> {
            return _internal_fetchAndUpdateCachedBotData(accountPeerId: peerId, peerId: peerId, network: self.account.network, postbox: self.account.postbox)
        }
        
        public func fetchAndUpdateUserFull(peerId: PeerId) -> Signal<Bool, NoError> {
            return _internal_fetchAndUpdateUserFull(accountPeerId: peerId, peerId: peerId, network: self.account.network, postbox: self.account.postbox)
        }
        ...
        public func updatePeerTitle(peerId: PeerId, title: String) -> Signal<Void, UpdatePeerTitleOrDescriptionError> {
            return _internal_updatePeerTitle(account: self.account, peerId: peerId, title: title)
        }

        public func updatePeerDescription(peerId: PeerId, description: String?) -> Signal<Void, UpdatePeerTitleOrDescriptionError> {
            ...
            return self.account.postbox.transaction { transaction -> (peer: EnginePeer, unreadCount: Int, location: NextUnreadChannelLocation)? in
                func getForFilter(predicate: ChatListFilterPredicate?, isArchived: Bool) -> (peer: EnginePeer, unreadCount: Int)? {
                    ...
                    for listId in peerIds {
                        ...
                        guard let channel = peer as? ElloAppChannel, case .broadcast = channel.info else {
                            continue
                        }
        ...
        public func mostRecentSecretChat(id: EnginePeer.Id) -> Signal<EnginePeer.Id?, NoError> {
            return self.account.postbox.transaction { transaction -> EnginePeer.Id? in
                ...
                for associatedId in filteredPeerIds {
                    if let state = (transaction.getPeer(associatedId) as? ElloAppSecretChat)?.embeddedState {
                        switch state {
        ...
        public func resetAllPeerNotificationSettings() -> Signal<Never, NoError> {
            return self.account.postbox.transaction { transaction -> Void in
                transaction.resetAllPeerNotificationSettings(ElloAppPeerNotificationSettings.defaultSettings)
            }
            |> ignoreValues
        }
    }
    ...
    final class AIChatBot {
        ...
        public func startBot() -> Signal<Never, NoError> {
            _internal_startAIBot(account: account)
        }
        
        public func stopBot() -> Signal<Never, NoError> {
            _internal_stoptAIBot(account: account)
        }
        
        public func subscriptionInfo(botId:Int64? = nil) -> Signal<AISubscriptionInfoItem, NoError> {
            _internal_subscriptionInfo(account: account, botId: botId)
        }
        
        public func subscribe(type: Int, quantity: Int) -> Signal<AISubscriptionInfoItem, AISubscriptionError> {
            _internal_subscribe(account: account, type: type, quantity: quantity)
        }
        
        public func subscribeIAP(type: Int, quantity: Int, amount: Float, payload: String) -> Signal<AISubscriptionInfoItem, BaseError> {
            _internal_subscribeIAP(account: account, type: type, quantity: quantity, amount: amount, payload: payload)
        }
        
        public func getAllBots() -> Signal<AIGetAllBotsItem, NoError> {
            _internal_getAllBots(account: account)
        }
    }
}
