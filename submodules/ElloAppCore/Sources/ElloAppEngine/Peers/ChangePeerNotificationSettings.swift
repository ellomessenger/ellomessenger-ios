...
func _internal_togglePeerMuted(account: Account, peerId: PeerId) -> Signal<Void, NoError> {
    return account.postbox.transaction { transaction -> Void in
        if let peer = transaction.getPeer(peerId) {
            ...
            let currentSettings = transaction.getPeerNotificationSettings(notificationPeerId) as? ElloAppPeerNotificationSettings
            let previousSettings: ElloAppPeerNotificationSettings
            if let currentSettings = currentSettings {
                previousSettings = currentSettings
            } else {
                previousSettings = ElloAppPeerNotificationSettings.defaultSettings
            }
            
            let updatedSettings: ElloAppPeerNotificationSettings
