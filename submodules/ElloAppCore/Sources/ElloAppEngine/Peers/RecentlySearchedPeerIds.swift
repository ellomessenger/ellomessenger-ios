...
public struct RecentlySearchedPeer: Equatable {
    ...
    public let presence: ElloAppUserPresence?
    public let notificationSettings: ElloAppPeerNotificationSettings?
                ...
                if let peerView = view.views[.peer(peerId: peerId, components: .all)] as? PeerView {
                    ...
                    if let peer = peerView.peers[peerId] {
                        if let associatedPeerId = peer.associatedPeerId {
                            presence = peerView.peerPresences[associatedPeerId] as? ElloAppUserPresence
                        } else {
                            presence = peerView.peerPresences[peerId] as? ElloAppUserPresence
                        }
                    ...
                    result.append(RecentlySearchedPeer(peer: RenderedPeer(peerId: peerId, peers: SimpleDictionary(peerView.peers), associatedMedia: peerView.media), presence: presence, notificationSettings: peerView.notificationSettings as? ElloAppPeerNotificationSettings, unreadCount: unreadCount, subpeerSummary: subpeerSummary))
                }
            }
