...
public enum UpdatePeerPhotoStatus {
    case progress(Float)
    case complete([ElloAppMediaImageRepresentation])
}
...
func _internal_updateAccountPhoto(account: Account, resource: MediaResource?, videoResource: MediaResource?, videoStartTimestamp: Double?, mapResourceToAvatarSizes: @escaping (MediaResource, [ElloAppMediaImageRepresentation]) -> Signal<[Int: Data], NoError>) -> Signal<UpdatePeerPhotoStatus, UploadPeerPhotoError> {
...
func _internal_uploadedPeerPhoto(postbox: Postbox, network: Network, resource: MediaResource) -> Signal<UploadedPeerPhotoData, NoError> {
    return multipartUpload(network: network, postbox: postbox, source: .resource(.standalone(resource: resource)), encrypt: false, tag: ElloAppMediaResourceFetchTag(statsCategory: .image), hintFileSize: nil, hintFileIsLarge: false, forceNoBigParts: false)
...
func _internal_uploadedPeerVideo(postbox: Postbox, network: Network, messageMediaPreuploadManager: MessageMediaPreuploadManager?, resource: MediaResource) -> Signal<UploadedPeerPhotoData, NoError> {
    if let messageMediaPreuploadManager = messageMediaPreuploadManager {
        return messageMediaPreuploadManager.upload(network: network, postbox: postbox, source: .resource(.standalone(resource: resource)), encrypt: false, tag: ElloAppMediaResourceFetchTag(statsCategory: .video), hintFileSize: nil, hintFileIsLarge: false)
        |> map { result -> UploadedPeerPhotoData in
            return UploadedPeerPhotoData(resource: resource, content: .result(result))
        }
        |> `catch` { _ -> Signal<UploadedPeerPhotoData, NoError> in
        ...
        return multipartUpload(network: network, postbox: postbox, source: .resource(.standalone(resource: resource)), encrypt: false, tag: ElloAppMediaResourceFetchTag(statsCategory: .video), hintFileSize: nil, hintFileIsLarge: false, forceNoBigParts: false)
       |> map { result -> UploadedPeerPhotoData in
           return UploadedPeerPhotoData(resource: resource, content: .result(result))
       }
       |> `catch` { _ -> Signal<UploadedPeerPhotoData, NoError> in
                        ...
                        switch resultData {
                            ...
                            case let .inputFile(file):
                                ...
                                if peer is ElloAppUser {
                                                    ...
                                                    for size in sizes {
                                                        switch size {
                                                            case let .photoSize(_, w, h, _):
                                                                representations.append(ElloAppMediaImageRepresentation(dimensions: PixelDimensions(width: w, height: h), resource: CloudPeerPhotoSizeMediaResource(datacenterId: dcId, photoId: id, sizeSpec: w <= 200 ? .small : .fullSize, volumeId: nil, localId: nil), progressiveSizes: [], immediateThumbnailData: nil))
                                                            case let .photoSizeProgressive(_, w, h, sizes):
                                                                representations.append(ElloAppMediaImageRepresentation(dimensions: PixelDimensions(width: w, height: h), resource: CloudPeerPhotoSizeMediaResource(datacenterId: dcId, photoId: id, sizeSpec: w <= 200 ? .small : .fullSize, volumeId: nil, localId: nil), progressiveSizes: sizes, immediateThumbnailData: nil))
                                                            default:
                                                                break
                                                        }
                                                    }
                                    ...
                                    if let peer = peer as? ElloAppGroup {
                                    ...
                                    } else if let peer = peer as? ElloAppChannel, let inputChannel = apiInputChannel(peer) {
                                    ...
                                    |> mapToSignal { updates -> Signal<(UpdatePeerPhotoStatus, MediaResource?, MediaResource?), UploadPeerPhotoError> in
                                        guard let chat = updates.chats.first, chat.peerId == peer.id, let groupOrChannel = parseElloAppGroupOrChannel(chat: chat) else {
...
func _internal_updatePeerPhotoExisting(network: Network, reference: ElloAppMediaImageReference) -> Signal<ElloAppMediaImage?, NoError> {
    switch reference {
        case let .cloud(imageId, accessHash, fileReference):
            ...
            |> mapToSignal { photo -> Signal<ElloAppMediaImage?, NoError> in
                if case let .photo(photo, _) = photo {
                    return .single(elloappMediaImageFromApiPhoto(photo))
                } else {
                    return .complete()
                }
            }
...
func _internal_removeAccountPhoto(network: Network, reference: ElloAppMediaImageReference?) -> Signal<Void, NoError> {
