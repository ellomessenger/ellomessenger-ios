...
func fetchAndUpdateSupplementalCachedPeerData(peerId rawPeerId: PeerId, accountPeerId: PeerId, network: Network, postbox: Postbox) -> Signal<Bool, NoError> {
    ...
    |> mapToSignal { _ -> Signal<Bool, NoError> in
        return postbox.transaction { transaction -> Signal<Bool, NoError> in
            ...
            let peer: Peer
            if let secretChat = rawPeer as? ElloAppSecretChat {
            ...
            if peer.id.namespace == Namespaces.Peer.SecretChat {
                return postbox.transaction { transaction -> Bool in
                    ...
                    if let peer = transaction.getPeer(peer.id), let associatedPeerId = peer.associatedPeerId, !transaction.isPeerContact(peerId: associatedPeerId) {
                        if let peer = peer as? ElloAppSecretChat, case .creator = peer.role {
                ...
                |> mapToSignal { peerSettings -> Signal<Bool, NoError> in
                    ...
                    switch peerSettings {
                        case let .peerSettings(settings, chats, users):
                            ...
                            for chat in chats {
                                if let peer = parseElloAppGroupOrChannel(chat: chat) {
                                    peers.append(peer)
                                }
                            }
                            for user in users {
...
public func fetchUpdatedCachedPeerData(peerId rawPeerId: PeerId, accountPeerId: PeerId, network: Network, postbox: Postbox) -> Signal<CachedPeerData?, NoError> {
    return postbox.transaction { transaction -> CachedPeerData? in
        guard let rawPeer = transaction.getPeer(rawPeerId) else {
            return nil
        }
                    ...
                    return postbox.transaction { transaction -> Bool in
                        ...
                        case let .userFull(fullUser, chats, users):
                            ...
                            for user in users {
                                ...
                                peers.append(elloappUser)
                                peerPresences[elloappUser.id] = user
                                if elloappUser.id == accountPeerId {
                                    accountUser = user
                                }
                            }
                            
                            switch fullUser {
                            case let .userFull(_, _, _, _, _, userFullNotifySettings, _, _, _, _, _, _, _, _, _, _, _, _, _, _):
                                updatePeers(transaction: transaction, peers: peers, update: { previous, updated -> Peer in
                                    if previous?.id == accountPeerId, let accountUser = accountUser, let user = ElloAppUser.merge(previous as? ElloAppUser, rhs: accountUser) {
                                        return user
                                    }
                                    return updated
                                })
                                transaction.updateCurrentPeerNotificationSettings([peerId: ElloAppPeerNotificationSettings(apiSettings: userFullNotifySettings)])
                                updatePeerPresences(transaction: transaction, accountPeerId: accountPeerId, peerPresences: peerPresences)
                            }
                            transaction.updatePeerCachedData(peerIds: [peerId], update: { peerId, current in
                                ...
                                switch fullUser {
                                case let .userFull(userFullFlags, _, userFullAbout, userFullSettings, profilePhoto, _, userFullBotInfo, userEmail, userCountry, userGender, userBirthday, userFullPinnedMsgId, userFullCommonChatsCount, _, userFullTtlPeriod, userFullThemeEmoticon, _, _, _, userPremiumGiftOptions):
                                        ...
                                        let canPinMessages = false /*(userFullFlags & (1 << 7)) != 0*/
                                        ...
                                        let photo = profilePhoto.flatMap { elloappMediaImageFromApiPhoto($0) }
                                        ...
                                        return previous.withUpdatedAbout(userFullAbout).withUpdatedBotInfo(botInfo).withUpdatedCommonGroupCount(userFullCommonChatsCount).withUpdatedIsBlocked(isBlocked).withUpdatedVoiceCallsAvailable(voiceCallsAvailable).withUpdatedVideoCallsAvailable(videoCallsAvailable).withUpdatedCallsPrivate(callsPrivate).withUpdatedCanPinMessages(canPinMessages).withUpdatedPeerStatusSettings(peerStatusSettings).withUpdatedPinnedMessageId(pinnedMessageId).withUpdatedHasScheduledMessages(hasScheduledMessages)
                                            ...
                                            .withEmail(userEmail)
                                            .withCounty(userCountry)
                                            .withGender(userGender)
                                            .withBirthday(userBirthday)
                                }
                            })
                        }
                        return true
                    ...
                    return postbox.transaction { transaction -> Bool in
                        ...
                        case let .chatFull(fullChat, chats, users):
                            ...
                            case let .chatFull(_, _, _, _, _, notifySettings, _, _, _, _, _, _, _, _, _, _, _):
                                transaction.updateCurrentPeerNotificationSettings([peerId: ElloAppPeerNotificationSettings(apiSettings: notifySettings)])
                            case .channelFull:
                                break
                            }
                            ...
                            case let .chatFull(chatFullFlags, _, chatFullAbout, chatFullParticipants, chatFullChatPhoto, _, chatFullExportedInvite, chatFullBotInfo, chatFullPinnedMsgId, _, chatFullCall, _, chatFullGroupcallDefaultJoinAs, chatFullThemeEmoticon, chatFullRequestsPending, _, allowedReactions):
                                ...
                                let photo: ElloAppMediaImage? = chatFullChatPhoto.flatMap(elloappMediaImageFromApiPhoto)
...
//                                                channelFlags.insert(.preHistoryEnabled)
                                            ...
                                            let photo = elloappMediaImageFromApiPhoto(chatPhoto)
...
func _internal_fetchAndUpdateCachedBotData(accountPeerId: PeerId, peerId rawPeerId: PeerId, network: Network, postbox: Postbox) -> Signal<Bool, NoError> {
    return postbox.combinedView(keys: [.basicPeer(rawPeerId)])
    |> mapToSignal { views -> Signal<Bool, NoError> in
        return .single(true)
    }
    ...
    |> mapToSignal { _ -> Signal<Bool, NoError> in
        ...
        |> mapToSignal { inputUser, maybePeer, peerId -> Signal<Bool, NoError> in
            let userInput: Api.InputUser = .inputUser(userId: peerId.toInt64(), accessHash: 0)
            return network.request(Api.functions.users.getFullUser(id: userInput))
            |> retryRequest
            |> mapToSignal { result -> Signal<Bool, NoError> in
                return postbox.transaction { transaction -> Bool in
                    switch result {
...
func _internal_fetchAndUpdateUserFull(accountPeerId: PeerId, peerId rawPeerId: PeerId, network: Network, postbox: Postbox) -> Signal<Bool, NoError> {
    let userInput: Api.InputUser = .inputUser(userId: rawPeerId.toInt64(), accessHash: 0)
    return network.request(Api.functions.users.getFullUser(id: userInput))
            ...
            case let .userFull(fullUser, chats, users):
                ...
                case let .userFull(_, _, _, _, _, userFullNotifySettings, _, _, _, _, _, _, _, _, _, _, _, _, _, _):
                    ...
                    transaction.updateCurrentPeerNotificationSettings(
                        [rawPeerId: ElloAppPeerNotificationSettings(apiSettings: userFullNotifySettings)]
                    )
                    updatePeerPresences(
                ...
                transaction.updatePeerCachedData(peerIds: [rawPeerId], update: { peerId, current in
                    let previous: CachedUserData
                    if let current = current as? CachedUserData {
                        previous = current
                    } else {
