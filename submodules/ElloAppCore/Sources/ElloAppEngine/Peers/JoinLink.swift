...
public enum ExternalJoiningChatState {
    public struct Invite: Equatable {
        ...
        public let photoRepresentation: ElloAppMediaImageRepresentation?
    ...
    |> mapToSignal { result -> Signal<ExternalJoiningChatState, JoinLinkInfoError> in
        if let result = result {
            ...
            case let .chatInvite(flags, title, about, invitePhoto, participantsCount, participants):
                let photo = elloappMediaImageFromApiPhoto(invitePhoto).flatMap({ smallestImageRepresentation($0.representations) })
                ...
                return .single(.invite(ExternalJoiningChatState.Invite(flags: flags, title: title, about: about, photoRepresentation: photo, participantsCount: participantsCount, participants: participants?.map({ EnginePeer(ElloAppUser(user: $0)) }))))
            ...
            case let .chatInvitePeek(chat, expires):
                ...
                return .single(.invalidHash)
            case let .chatInviteUser(user: user, _):
                let peer = ElloAppUser(user: user)
                return account.postbox.transaction({ (transaction) -> ExternalJoiningChatState in
                    updatePeers(transaction: transaction, peers: [peer], update: { (previous, updated) -> Peer? in
                        return updated
                    })
