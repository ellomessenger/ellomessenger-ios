...
public enum ElloAppEngineAuthorizationState {
...
public extension ElloAppEngineUnauthorized {
    final class Auth {
        ...
        public func exportAuthTransferToken(accountManager: AccountManager<ElloAppAccountManagerTypes>, otherAccountUserIds: [PeerId.Id], syncContacts: Bool) -> Signal<ExportAuthTransferTokenResult, ExportAuthTransferTokenError> {
        ...
        public func state() -> Signal<ElloAppEngineAuthorizationState?, NoError> {
            return self.account.postbox.stateView()
            |> map { view -> ElloAppEngineAuthorizationState? in
...
public extension SomeElloAppEngine {
    final class Auth {
        private let engine: SomeElloAppEngine

        init(engine: SomeElloAppEngine) {
            self.engine = engine
        }
