        ...
        if let message = transaction.getMessage(messageId), message.tags.contains(.unseenReaction) {
            ...
            if updateMessage {
                transaction.updateMessage(message.id, update: { currentMessage in
                    ...
                    return .update(StoreMessage(id: currentMessage.id, globallyUniqueId: currentMessage.globallyUniqueId, groupingKey: currentMessage.groupingKey, threadId: currentMessage.threadId, timestamp: currentMessage.timestamp, flags: StoreMessageFlags(currentMessage.flags), tags: tags, globalTags: currentMessage.globalTags, localTags: currentMessage.localTags, forwardInfo: storeForwardInfo, authorId: currentMessage.author?.id, text: currentMessage.text, attributes: updatedAttributes, media: currentMessage.media, likes: 0))
                })
            }
        }
    }
    ...
    if let message = transaction.getMessage(messageId) {
                ...
                if (attribute.countdownBeginTime == nil || attribute.countdownBeginTime == 0) && message.containsSecretMedia {
                    ...
                    if message.id.peerId.namespace == Namespaces.Peer.SecretChat {
                        ...
                        for i in 0 ..< updatedMedia.count {
                            if let _ = updatedMedia[i] as? ElloAppMediaImage {
                                updatedMedia[i] = ElloAppMediaExpiredContent(data: .image)
                            } else if let _ = updatedMedia[i] as? ElloAppMediaFile {
                                updatedMedia[i] = ElloAppMediaExpiredContent(data: .file)
                            }
                        }
                    }
                }
        ...
        if updateMessage {
            transaction.updateMessage(message.id, update: { currentMessage in
                ...
                return .update(StoreMessage(id: currentMessage.id, globallyUniqueId: currentMessage.globallyUniqueId, groupingKey: currentMessage.groupingKey, threadId: currentMessage.threadId, timestamp: currentMessage.timestamp, flags: StoreMessageFlags(currentMessage.flags), tags: updatedTags, globalTags: currentMessage.globalTags, localTags: currentMessage.localTags, forwardInfo: storeForwardInfo, authorId: currentMessage.author?.id, text: currentMessage.text, attributes: updatedAttributes, media: updatedMedia, likes: 0))
            })
        }
    }
}
