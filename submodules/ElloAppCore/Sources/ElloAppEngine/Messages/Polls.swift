...
func _internal_requestMessageSelectPollOption(account: Account, messageId: MessageId, opaqueIdentifiers: [Data]) -> Signal<ElloAppMediaPoll?, RequestMessageSelectPollOptionError> {
    ...
    |> mapToSignal { peer in
        if let inputPeer = apiInputPeer(peer) {
            ...
            |> mapToSignal { result -> Signal<ElloAppMediaPoll?, RequestMessageSelectPollOptionError> in
                return account.postbox.transaction { transaction -> ElloAppMediaPoll? in
                    var resultPoll: ElloAppMediaPoll?
                    switch result {
                    case let .updates(updates, _, _, _, _):
                        for update in updates {
                            ...
                            case let .updateMessagePoll(_, id, poll, results):
                                ...
                                resultPoll = transaction.getMedia(pollId) as? ElloAppMediaPoll
                                if let poll = poll {
                                    switch poll {
                                    case let .poll(_, flags, question, answers, closePeriod, _):
                                        ...
                                        resultPoll = ElloAppMediaPoll(pollId: pollId, publicity: publicity, kind: kind, text: question, options: answers.map(ElloAppMediaPollOption.init(apiOption:)), correctAnswers: nil, results: ElloAppMediaPollResults(apiResults: results), isClosed: (flags & (1 << 0)) != 0, deadlineTimeout: closePeriod)
                                    }
                                }
                                ...
                                resultPoll = resultPoll?.withUpdatedResults(ElloAppMediaPollResults(apiResults: results), min: resultsMin)
...
func _internal_requestClosePoll(postbox: Postbox, network: Network, stateManager: AccountStateManager, messageId: MessageId) -> Signal<Void, NoError> {
    return postbox.transaction { transaction -> (ElloAppMediaPoll, Api.InputPeer)? in
                    ...
                    return account.postbox.transaction { transaction -> ([RenderedPeer], Int, String?) in
                        ...
                        case let .votesList(_, count, votes, users, nextOffset):
                            ...
                            for apiUser in users {
                                peers.append(ElloAppUser(user: apiUser))
                            }
...
private final class PollResultsContextImpl {
    ...
    init(queue: Queue, account: Account, messageId: MessageId, poll: ElloAppMediaPoll) {
        self.queue = queue
...
public final class PollResultsContext {
    ...
    init(account: Account, messageId: MessageId, poll: ElloAppMediaPoll) {
