...
public final class AttachMenuBots: Equatable, Codable {
    public final class Bot: Equatable, Codable {
        ...
        private struct IconPair: Codable {
            ...
            init(_ name: IconName, value: ElloAppMediaFile) {
                self.name = name
                self.value = value
            }
            ...
            init(from decoder: Decoder) throws {
                ...
                self.value = ElloAppMediaFile(decoder: PostboxDecoder(buffer: MemoryBuffer(data: data.data)))
            }
        ...
        public let icons: [IconName: ElloAppMediaFile]
        ...
        public init(
            peerId: PeerId,
            name: String,
            icons: [IconName: ElloAppMediaFile],
            peerTypes: PeerFlags,
            hasSettings: Bool
        ) {
            self.peerId = peerId
        ...
        public init(from decoder: Decoder) throws {
            ...
            var icons: [IconName: ElloAppMediaFile] = [:]
                            ...
                            for bot in bots {
                                switch bot {
                                    case let .attachMenuBot(flags, botId, name, apiPeerTypes, botIcons):
                                        var icons: [AttachMenuBots.Bot.IconName: ElloAppMediaFile] = [:]
                                        for icon in botIcons {
                                            switch icon {
                                                case let .attachMenuBotIcon(_, name, icon, _):
                                                    if let iconName = AttachMenuBots.Bot.IconName(string: name), let icon = elloappMediaFileFromApiDocument(icon) {
                                                        icons[iconName] = icon
                                                    }
                                            }
                                        }
...
public struct AttachMenuBot {
    ...
    public let icons: [AttachMenuBots.Bot.IconName: ElloAppMediaFile]
    public let peerTypes: AttachMenuBots.Bot.PeerFlags
    public let hasSettings: Bool
    
    init(peer: Peer, shortName: String, icons: [AttachMenuBots.Bot.IconName: ElloAppMediaFile], peerTypes: AttachMenuBots.Bot.PeerFlags, hasSettings: Bool) {
        self.peer = peer
