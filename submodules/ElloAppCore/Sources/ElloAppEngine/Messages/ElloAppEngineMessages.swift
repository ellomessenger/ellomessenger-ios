...
public extension ElloAppEngine {
    final class Messages {
        ...
        public func requestMessageSelectPollOption(messageId: MessageId, opaqueIdentifiers: [Data]) -> Signal<ElloAppMediaPoll?, RequestMessageSelectPollOptionError> {
        ...
        public func pollResults(messageId: MessageId, poll: ElloAppMediaPoll) -> PollResultsContext {
        ...
        public func shareMessageLink(peerId: PeerId, messageId: MessageId) -> Signal<String?, NoError> {
            return _internal_shareMessageLink(account: self.account, peerId: peerId, messageId: messageId)
        }
                ...
                for id in ids {
                    transaction.updateMessage(id, update: { currentMessage in
                        ...
                        for i in 0 ..< updatedMedia.count {
                            if let media = updatedMedia[i] as? ElloAppMediaMap, let _ = media.liveBroadcastingTimeout {
                                updatedMedia[i] = ElloAppMediaMap(latitude: media.latitude, longitude: media.longitude, heading: media.heading, accuracyRadius: media.accuracyRadius, geoPlace: media.geoPlace, venue: media.venue, liveBroadcastingTimeout: max(0, timestamp - currentMessage.timestamp - 1), liveProximityNotificationRadius: nil)
                            }
                        }
                        return .update(StoreMessage(id: currentMessage.id, globallyUniqueId: currentMessage.globallyUniqueId, groupingKey: currentMessage.groupingKey, threadId: currentMessage.threadId, timestamp: currentMessage.timestamp, flags: StoreMessageFlags(currentMessage.flags), tags: currentMessage.tags, globalTags: currentMessage.globalTags, localTags: currentMessage.localTags, forwardInfo: storeForwardInfo, authorId: currentMessage.author?.id, text: currentMessage.text, attributes: currentMessage.attributes, media: updatedMedia, likes: 0))
                    })
                }
            }
            |> ignoreValues
