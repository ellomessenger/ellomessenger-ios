        ...
        |> mapToSignal { result -> Signal<[MessageIndex], NoError> in
            return account.postbox.transaction { transaction -> [MessageIndex] in
                ...
                for user in users {
                    ...
                    if let presence = ElloAppUserPresence(apiUser: user) {
                        peerPresences[elloappUser.id] = presence
                    }
                }
...
public class FeedHistory {
    ...
    public func parse(chat: Api.Chat) -> Peer? {
        parseElloAppGroupOrChannel(chat: chat)
    }
    
    public func parse(message: Api.Message) -> StoreMessage? {
        StoreMessage(apiMessage: message)
    }
}
