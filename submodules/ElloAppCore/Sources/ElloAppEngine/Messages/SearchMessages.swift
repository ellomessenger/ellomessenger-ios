    ...
    switch location {
        case let .peer(peerId, fromId, tags, topMsgId, minDate, maxDate):
            ...
            remoteSearchResult = account.postbox.transaction { transaction -> (peer: Peer, additionalPeer: Peer?, from: Peer?)? in
                ...
                if let _ = peer as? ElloAppChannel, let cachedData = transaction.getPeerCachedData(peerId: peerId) as? CachedChannelData, let migrationReference = cachedData.migrationReference {
        ...
        case let .general(tags, minDate, maxDate), let .group(_, tags, minDate, maxDate):
            ...
            |> mapToSignal { (nextRate, lowerBound, inputPeer) in
                return account.network.request(Api.functions.messages.searchGlobal(flags: flags, 
                                                                                   folderId: folderId,
                                                                                   q: query, filter: filter,
                                                                                   minDate: minDate ?? 0,
                                                                                   maxDate: maxDate ?? (Int32.max - 1),
                                                                                   offsetRate: nextRate,
                                                                                   offsetPeer: inputPeer,
                                                                                   offsetId: lowerBound?.id.id ?? 0,
                                                                                   limit: limit),
    ...
    if let inputUser = peer.inputUser {
        ...
        |> mapToSignal { result -> Signal<Peer, UpdatedRemotePeerError> in
            ...
            return postbox.transaction { transaction -> Peer? in
                guard let peer = ElloAppUser.merge(transaction.getPeer(apiUser.peerId) as? ElloAppUser, rhs: apiUser) else {
                    return nil
                }
        ...
        |> mapToSignal { result -> Signal<Peer, UpdatedRemotePeerError> in
            ...
            if let updatedPeer = chats.first.flatMap(parseElloAppGroupOrChannel), updatedPeer.id == peer.id {
                return postbox.transaction { transaction -> Peer in
