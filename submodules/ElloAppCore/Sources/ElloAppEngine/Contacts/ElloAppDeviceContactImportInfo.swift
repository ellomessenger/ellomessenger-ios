...
enum ElloAppDeviceContactImportIdentifier: Hashable, Comparable, Equatable {
    ...
    static func <(lhs: ElloAppDeviceContactImportIdentifier, rhs: ElloAppDeviceContactImportIdentifier) -> Bool {
        switch lhs {
    ...
    return postbox.transaction { transaction -> [String: Int32] in
        ...
        for (id, numbers) in contacts {
            ...
            for number in numbers {
                if let value = transaction.getDeviceContactImportInfo(ElloAppDeviceContactImportIdentifier.phoneNumber(number).key) as? ElloAppDeviceContactImportedData, case let .imported(_, importedByCount, _) = value {
