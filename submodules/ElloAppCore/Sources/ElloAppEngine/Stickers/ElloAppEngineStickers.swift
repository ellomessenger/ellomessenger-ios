...
public extension ElloAppEngine {
    final class Stickers {
        ...
        public func toggleStickerSaved(file: ElloAppMediaFile, saved: Bool) -> Signal<SavedStickerResult, AddSavedStickerError> {
        ...
        public func resolveInlineStickers(fileIds: [Int64]) -> Signal<[Int64: ElloAppMediaFile], NoError> {
            return _internal_resolveInlineStickers(postbox: self.account.postbox, network: self.account.network, fileIds: fileIds)
        }
    }
}

func _internal_resolveInlineStickers(postbox: Postbox, network: Network, fileIds: [Int64]) -> Signal<[Int64: ElloAppMediaFile], NoError> {
    return postbox.transaction { transaction -> [Int64: ElloAppMediaFile] in
        var cachedFiles: [Int64: ElloAppMediaFile] = [:]
        for fileId in fileIds {
            if let file = transaction.getMedia(MediaId(namespace: Namespaces.Media.CloudFile, id: fileId)) as? ElloAppMediaFile {
                cachedFiles[fileId] = file
            }
        }
        return cachedFiles
    }
    |> mapToSignal { cachedFiles -> Signal<[Int64: ElloAppMediaFile], NoError> in
        ...
        |> mapToSignal { result -> Signal<[Int64: ElloAppMediaFile], NoError> in
            guard let result = result else {
                return .single(cachedFiles)
            }
            return postbox.transaction { transaction -> [Int64: ElloAppMediaFile] in
                var resultFiles: [Int64: ElloAppMediaFile] = cachedFiles
