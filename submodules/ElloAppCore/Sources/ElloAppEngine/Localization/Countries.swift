...
public struct Country: Codable, Hashable {
    ...
    public struct CountryCode: Codable, Hashable {
...
func _internal_getCountriesList(accountManager: AccountManager<ElloAppAccountManagerTypes>, network: Network, langCode: String?, forceUpdate: Bool = false) -> Signal<[Country], NoError> {
...
extension Country {
    ...
    public func flagEmoji() -> String {
        guard let localizedName else { return "" }
        
        let base: UInt32 = 127397
        var s = ""
        for v in localizedName.unicodeScalars {
