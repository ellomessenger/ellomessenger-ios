...
final class ThemeGridControllerInteraction {
    let openWallpaper: (ElloAppWallpaper) -> Void
    ...
    init(openWallpaper: @escaping (ElloAppWallpaper) -> Void, toggleWallpaperSelection: @escaping (ThemeGridControllerEntry.StableId, Bool) -> Void, deleteSelectedWallpapers: @escaping () -> Void, shareSelectedWallpapers: @escaping () -> Void) {
...
private func selectedWallpapers(entries: [ThemeGridControllerEntry]?, state: ThemeGridControllerNodeState) -> [ElloAppWallpaper] {
    guard let entries = entries, state.editing else {
        return []
    }
    var wallpapers: [ElloAppWallpaper] = []
...
final class ThemeGridControllerNode: ASDisplayNode {
    ...
    init(context: AccountContext, presentationData: PresentationData, presentPreviewController: @escaping (WallpaperListSource) -> Void, presentGallery: @escaping () -> Void, presentColors: @escaping () -> Void, emptyStateUpdated: @escaping (Bool) -> Void, deleteWallpapers: @escaping ([ElloAppWallpaper], @escaping () -> Void) -> Void, shareWallpapers: @escaping ([ElloAppWallpaper]) -> Void, resetWallpapers: @escaping () -> Void, popViewController: @escaping () -> Void) {
        ...
        |> map { wallpapers, deletedWallpaperIds, presentationData -> (ThemeGridEntryTransition, Bool) in
            ...
            var sortedWallpapers: [ElloAppWallpaper] = []
            if presentationData.theme.overallDarkAppearance {
                var localWallpapers: [ElloAppWallpaper] = []
                var darkWallpapers: [ElloAppWallpaper] = []
                for wallpaper in wallpapers {
    ...
    func updateWallpapers() {
        self.wallpapersPromise.set(combineLatest(queue: .mainQueue(),
            elloappWallpapers(postbox: self.context.account.postbox, network: self.context.account.network),
