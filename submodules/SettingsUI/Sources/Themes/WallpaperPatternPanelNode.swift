...
private struct WallpaperPatternEntry: Comparable, Identifiable {
    ...
    func item(context: AccountContext, action: @escaping (ElloAppWallpaper) -> Void) -> ListViewItem {
...
private class WallpaperPatternItem: ListViewItem {
    ...
    let wallpaper: ElloAppWallpaper
    let selected: Bool
    let action: (ElloAppWallpaper) -> Void
    
    public init(context: AccountContext, wallpaper: ElloAppWallpaper, selected: Bool, action: @escaping (ElloAppWallpaper) -> Void) {
...
final class WallpaperPatternPanelNode: ASDisplayNode {
    ...
    private var currentWallpaper: ElloAppWallpaper?
    ...
    var patternChanged: ((ElloAppWallpaper?, Int32?, Bool) -> Void)?
    ...
    init(context: AccountContext, theme: PresentationTheme, strings: PresentationStrings) {
        ...
        self.disposable = ((elloappWallpapers(postbox: context.account.postbox, network: context.account.network)
        |> map { wallpapers -> [ElloAppWallpaper] in
    ...
    func updateWallpapers() {
        ...
        for wallpaper in self.wallpapers {
            ...
            if case let .file(file) = updatedWallpaper {
                ...
                updatedWallpaper = .file(ElloAppWallpaper.File(id: file.id, accessHash: file.accessHash, isCreator: file.isCreator, isDefault: file.isDefault, isPattern: updatedWallpaper.isPattern, isDark: file.isDark, slug: file.slug, file: file.file, settings: settings))
            }
    ...
    func didAppear(initialWallpaper: ElloAppWallpaper? = nil, intensity: Int32? = nil) {
        let wallpaper: ElloAppWallpaper?
