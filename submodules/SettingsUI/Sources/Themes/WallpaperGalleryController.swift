...
public enum WallpaperListSource {
    case list(wallpapers: [ElloAppWallpaper], central: ElloAppWallpaper, type: WallpaperListType)
    case wallpaper(ElloAppWallpaper, WallpaperPresentationOptions?, [UInt32], Int32?, Int32?, Message?)
    case slug(String, ElloAppMediaFile?, WallpaperPresentationOptions?, [UInt32], Int32?, Int32?, Message?)
    case asset(PHAsset)
...
public enum WallpaperGalleryEntry: Equatable {
    case wallpaper(ElloAppWallpaper, Message?)
    case asset(PHAsset)
    case contextResult(ChatContextResult)
...
private func updatedFileWallpaper(wallpaper: ElloAppWallpaper, colors: [UInt32], intensity: Int32?, rotation: Int32?) -> ElloAppWallpaper {
...
private func updatedFileWallpaper(id: Int64? = nil, accessHash: Int64? = nil, slug: String, file: ElloAppMediaFile, colors: [UInt32], intensity: Int32?, rotation: Int32?) -> ElloAppWallpaper {
    ...
    return .file(ElloAppWallpaper.File(id: id ?? 0, accessHash: accessHash ?? 0, isCreator: false, isDefault: false, isPattern: isPattern, isDark: false, slug: slug, file: file, settings: WallpaperSettings(colors: colorValues, intensity: intensityValue, rotation: rotation)))
}
...
public class WallpaperGalleryController: ViewController {
    ...
    private var patternInitialWallpaper: ElloAppWallpaper?
    private var patternPanelEnabled = false
    private var colorsPanelEnabled = false

    private var savedPatternWallpaper: ElloAppWallpaper?
                    ...
                    if !strongSelf.entries.isEmpty {
                        ...
                        switch entry {
                            case let .wallpaper(wallpaper, _):
                                ...
                                let completion: (ElloAppWallpaper) -> Void = { wallpaper in
                                ...
                                let applyWallpaper: (ElloAppWallpaper) -> Void = { wallpaper in
                        ...
                        case let .file(file):
                            if !file.settings.colors.isEmpty {
                                if file.settings.colors.count >= 2 {
                                    strongSelf.updateEntries(wallpaper: .gradient(ElloAppWallpaper.Gradient(id: nil, colors: file.settings.colors, settings: WallpaperSettings(rotation: file.settings.rotation))))
                                } else {
    ...
    private func updateEntries(wallpaper: ElloAppWallpaper, preview: Bool = false) {
        guard self.validLayout != nil, let centralEntryIndex = self.galleryNode.pager.centralItemNode()?.index else {
            return
        }
    ...
    private func updateEntries(pattern: ElloAppWallpaper?, intensity: Int32? = nil, preview: Bool = false) {
        ...
        for entry in self.entries {
            ...
            if !entryColors.isEmpty {
                if let pattern = pattern, case let .file(file) = pattern {
                    ...
                    let newWallpaper = ElloAppWallpaper.file(ElloAppWallpaper.File(id: file.id, accessHash: file.accessHash, isCreator: file.isCreator, isDefault: file.isDefault, isPattern: pattern.isPattern, isDark: file.isDark, slug: file.slug, file: file.file, settings: newSettings))
                    updatedEntries.append(.wallpaper(newWallpaper, nil))
                } else {
                    if entryColors.count == 1 {
                        let newWallpaper = ElloAppWallpaper.color(entryColors[0])
                        updatedEntries.append(.wallpaper(newWallpaper, nil))
                    } else {
                        let newWallpaper = ElloAppWallpaper.gradient(ElloAppWallpaper.Gradient(id: nil, colors: entryColors, settings: WallpaperSettings(rotation: nil)))
                        updatedEntries.append(.wallpaper(newWallpaper, nil))
                    }
                }
            }
    ...
    override public func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        if let current = self.colorsPanelNode {
            ...
            colorsPanelNode.colorsChanged = { [weak self] colors, _, _ in
                ...
                var wallpaper: ElloAppWallpaper = .gradient(ElloAppWallpaper.Gradient(id: nil, colors: colors.map { $0.rgb }, settings: WallpaperSettings(blur: false, motion: false, colors: [], intensity: nil, rotation: nil)))
    ...
    private func actionPressed() {
        ...
        switch wallpaper {
            case .image:
                ...
                |> filter { status -> Bool in
                    ...
                    if case let .uploaded(uploadedWallpaper, resultWallpaper) = status, uploadedWallpaper == wallpaper, case let .file(file) = resultWallpaper {
                        ...
                        let shareController = ShareController(context: context, subject: .url("https://ello.team/bg/\(file.slug)\(optionsString)"))
            ...
            case let .file(file):
                ...
                controller = ShareController(context: context, subject: .url("https://ello.team/bg/\(file.slug)\(optionsString)"))
            case let .color(color):
                controller = ShareController(context: context, subject: .url("https://ello.team/bg/\(UIColor(rgb: color).hexString)"))
            case let .gradient(gradient):
                var colorsString = ""
                ...
                controller = ShareController(context: context, subject: .url("https://ello.team/bg/\(colorsString)"))
            default:
                break
        }
