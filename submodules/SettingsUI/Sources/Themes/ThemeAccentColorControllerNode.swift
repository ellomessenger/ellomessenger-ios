...
struct ThemeColorState {
    ...
    var initialWallpaper: ElloAppWallpaper?
    var backgroundColors: [HSBColor]
    
    fileprivate var preview: Bool
    fileprivate var previousPatternWallpaper: ElloAppWallpaper?
    var patternWallpaper: ElloAppWallpaper?
    ...
    init(section: ThemeColorSection, accentColor: HSBColor, outgoingAccentColor: HSBColor?, initialWallpaper: ElloAppWallpaper?, backgroundColors: [HSBColor], patternWallpaper: ElloAppWallpaper?, patternIntensity: Int32, animateMessageColors: Bool, defaultMessagesColor: HSBColor?, messagesColors: [HSBColor], selectedColor: Int = 0, rotation: Int32 = 0) {
...
final class ThemeAccentColorControllerNode: ASDisplayNode, UIScrollViewDelegate {
    ...
    private var patternWallpaper: ElloAppWallpaper?
    ...
    init(context: AccountContext, mode: ThemeAccentColorControllerMode, theme: PresentationTheme, wallpaper: ElloAppWallpaper, dismiss: @escaping () -> Void, apply: @escaping (ThemeColorState, UIColor?) -> Void, ready: Promise<Bool>) {
        ...
        |> map { state -> (PresentationTheme?, ElloAppWallpaper, UIColor, [HSBColor], Int32, PatternWallpaperArguments, Bool) in
            ...
            var wallpaper: ElloAppWallpaper
            ...
            if !backgroundColors.isEmpty {
                if let patternWallpaper = state.patternWallpaper, case let .file(file) = patternWallpaper {
                    ...
                    wallpaper = .gradient(ElloAppWallpaper.Gradient(id: nil, colors: backgroundColors.map { $0.rgb }, settings: WallpaperSettings(rotation: state.rotation)))
                } else {
                ...
                switch theme {
                    case .dayClassic:
                        ...
                        suggestedWallpaper = .gradient(ElloAppWallpaper.Gradient(id: nil, colors: [topColor.rgb, bottomColor.rgb], settings: WallpaperSettings()))
