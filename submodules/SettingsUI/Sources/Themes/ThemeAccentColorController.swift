...
private let randomBackgroundColors: [Int32] = [0x007aff, 0x0A49A5]

extension ElloAppThemeSettings {
    convenience init(baseTheme: ElloAppBaseTheme, accentColor: UIColor, outgoingAccentColor: UIColor?, messageColors: [UInt32], animateMessageColors: Bool, wallpaper: ElloAppWallpaper?) {
...
enum ThemeAccentColorControllerMode {
    ...
    case edit(settings: ElloAppThemeSettings?, theme: PresentationTheme, wallpaper: ElloAppWallpaper?, generalThemeReference: PresentationThemeReference?, defaultThemeReference: PresentationThemeReference?, create: Bool, completion: (PresentationTheme, ElloAppThemeSettings?) -> Void)
...
final class ThemeAccentColorController: ViewController {
    ...
    override func loadDisplayNode() {
        ...
        let initialWallpaper: ElloAppWallpaper
        ...
        self.displayNode = ThemeAccentColorControllerNode(context: self.context, mode: self.mode, theme: theme, wallpaper: initialWallpaper, dismiss: { [weak self] in
            ...
            if let strongSelf = self {
                ...
                var coloredWallpaper: ElloAppWallpaper?
                if !state.backgroundColors.isEmpty {
                    if let patternWallpaper = state.patternWallpaper {
                        ...
                        coloredWallpaper = .gradient(ElloAppWallpaper.Gradient(id: nil, colors: state.backgroundColors.map { $0.rgb }, settings: WallpaperSettings(rotation: state.rotation)))
                    } else {
                ...
                if case let .edit(themeSettings, theme, _, generalThemeReference, _, _, completion) = strongSelf.mode {
                    ...
                    |> deliverOnMainQueue).start(completed: {
                        ...
                        var baseTheme: ElloAppBaseTheme?
                        ...
                        if hasSettings, let baseTheme = baseTheme {
                            settings = ElloAppThemeSettings(baseTheme: baseTheme, accentColor: state.accentColor.color, outgoingAccentColor: state.outgoingAccentColor?.color, messageColors: state.messagesColors.map { $0.rgb }, animateMessageColors: state.animateMessageColors, wallpaper: coloredWallpaper)
                        }
                    ...
                    let settings = ElloAppThemeSettings(baseTheme: baseTheme, accentColor: state.accentColor.rgb, outgoingAccentColor: state.outgoingAccentColor?.rgb, messageColors: state.messagesColors.map { $0.rgb }, animateMessageColors: state.animateMessageColors, wallpaper: wallpaper)
                    ...
                    } else if let theme = elloappTheme {
        ...
        let _ = (combineLatest(
            ...
            elloappWallpapers(postbox: context.account.postbox, network: context.account.network) |> take(1)
        )
        |> deliverOnMainQueue).start(next: { [weak self] sharedData, wallpapers in
            ...
            func extractWallpaperParameters(_ wallpaper: ElloAppWallpaper?) {
            ...
            if let themeReference = strongSelf.mode.themeReference {
                ...
                func extractBuiltinWallpaper(_ currentWallpaper: ElloAppWallpaper) {
                    if case let .builtin(settings) = currentWallpaper {
                        var defaultPatternWallpaper: ElloAppWallpaper?
