...
final class WallpaperGalleryItemNode: GalleryItemNode {
    ...
    var action: (() -> Void)?
    var requestPatternPanel: ((Bool, ElloAppWallpaper) -> Void)?
    ...
    private var initialWallpaper: ElloAppWallpaper?
        ...
        if self.entry != entry || self.arguments.colorPreview != previousArguments.colorPreview {
            ...
            switch entry {
                ...
                case let .contextResult(result):
                    ...
                    var imageResource: ElloAppMediaResource?
                    var thumbnailDimensions: CGSize?
                    var thumbnailResource: ElloAppMediaResource?
                    switch result {
                    ...
                    if let imageResource = imageResource, let imageDimensions = imageDimensions {
                        ...
                        if let thumbnailResource = thumbnailResource, let thumbnailDimensions = thumbnailDimensions {
                            representations.append(ElloAppMediaImageRepresentation(dimensions: PixelDimensions(thumbnailDimensions), resource: thumbnailResource, progressiveSizes: [], immediateThumbnailData: nil))
                        }
                        representations.append(ElloAppMediaImageRepresentation(dimensions: PixelDimensions(imageDimensions), resource: imageResource, progressiveSizes: [], immediateThumbnailData: nil))
                        let tmpImage = ElloAppMediaImage(imageId: MediaId(namespace: 0, id: 0), representations: representations, immediateThumbnailData: nil, reference: nil, partialReference: nil, flags: [])
    ...
    private func updateMessagesLayout(layout: ContainerViewLayout, offset: CGPoint, transition: ContainedViewLayoutTransition) {
        ...
        peers[peerId] = ElloAppUser(id: peerId, accessHash: nil, firstName: self.presentationData.strings.Appearance_PreviewReplyAuthor, lastName: "", username: nil, phone: nil, photo: [], botInfo: nil, restrictionInfo: nil, flags: [], emojiStatus: nil, isBusiness: false, isPublic: false, botDescription: nil)
        peers[otherPeerId] = ElloAppUser(id: otherPeerId, accessHash: nil, firstName: self.presentationData.strings.Appearance_PreviewReplyAuthor, lastName: "", username: nil, phone: nil, photo: [], botInfo: nil, restrictionInfo: nil, flags: [], emojiStatus: nil, isBusiness: false, isPublic: false, botDescription: nil)
        
        var topMessageText = ""
        var bottomMessageText = ""
        var currentWallpaper: ElloAppWallpaper = self.presentationData.chatWallpaper
