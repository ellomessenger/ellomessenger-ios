...
public enum ThemePreviewSource {
    case settings(PresentationThemeReference, ElloAppWallpaper?, Bool)
    case theme(ElloAppTheme)
    case slug(String, ElloAppMediaFile)
    case themeSettings(String, ElloAppThemeSettings)
...
public final class ThemePreviewController: ViewController {
    ...
    private let theme = Promise<ElloAppTheme?>()
        ...
        switch source {
            case let .theme(theme):
                ...
                |> then(
                    ...
                    |> map(Optional.init)
                    |> `catch` { _ -> Signal<ElloAppTheme?, NoError> in
                        return .single(nil)
                    }
                    |> filter { $0 != nil }
                ))
                ...
                |> then(
                    ...
                    |> mapToSignal { theme in
                        if let file = theme?.file {
                            return elloappThemeData(account: context.account, accountManager: context.sharedContext.accountManager, reference: .standalone(resource: file.resource))
    ...
    private func apply() {
        ...
        var resolvedWallpaper: ElloAppWallpaper?
        ...
        |> mapToSignal { theme -> Signal<(PresentationThemeReference, Bool), NoError> in
            ...
            switch theme {
                case let .cloud(info):
                    ...
                    return elloappThemes(postbox: context.account.postbox, network: context.account.network, accountManager: context.sharedContext.accountManager)
                    |> take(1)
                    |> map { themes -> Bool in
    ...
    @objc private func actionPressed() {
        ...
        switch self.source {
            case .settings:
                return
            case let .theme(theme):
                subject = .url("https://ello.team/addtheme/\(theme.slug)")
                preferredAction = .default
            case let .slug(slug, _), let .themeSettings(slug, _):
                subject = .url("https://ello.team/addtheme/\(slug)")
                preferredAction = .default
