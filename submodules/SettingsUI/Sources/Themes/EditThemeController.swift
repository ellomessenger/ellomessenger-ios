...
private enum EditThemeControllerEntry: ItemListNodeEntry {
    ...
    case chatPreview(PresentationTheme, PresentationTheme, ElloAppWallpaper, PresentationFontSize, PresentationChatBubbleCorners, PresentationStrings, PresentationDateTimeFormat, PresentationPersonNameOrder, [ChatPreviewMessageItem])
...
public enum EditThemeControllerMode: Equatable {
    case create(PresentationTheme?, ElloAppThemeSettings?)
...
public func editThemeController(context: AccountContext, mode: EditThemeControllerMode, navigateToChat: ((PeerId) -> Void)? = nil, completion: ((PresentationThemeReference) -> Void)? = nil) -> ViewController {
    ...
    let settingsPromise = Promise<ElloAppThemeSettings?>(nil)
    ...
    let arguments = EditThemeControllerArguments(context: context, updateState: { f in
        ...
        let presentationData = context.sharedContext.currentPresentationData.with { $0 }
        let controller = legacyICloudFilePicker(theme: presentationData.theme, mode: .import, documentTypes: ["org.elloapp.ElloApp-iOS.theme"], completion: { urls in
            if let url = urls.first{
                if let data = try? Data(contentsOf: url), let theme = makePresentationTheme(data: data) {
                    if case let .file(file) = theme.chat.defaultWallpaper, file.id == 0 {
                        ...
                        |> mapToSignal { wallpaper -> Signal<ElloAppWallpaper?, NoError> in
                            if let wallpaper = wallpaper, case let .file(file) = wallpaper.wallpaper {
                                ...
                                |> mapToSignal { _, fullSizeData, complete -> Signal<ElloAppWallpaper?, NoError> in
        ...
        |> take(1)).start(next: { theme, previousSettings in
            ...
            var updatedSettings: ElloAppThemeSettings?
            ...
            if let themeSettings = previousSettings {
                if updatedTheme.referenceTheme == .night {
                    ...
                    updatedSettings = ElloAppThemeSettings(baseTheme: .tinted, accentColor: themeSettings.accentColor, outgoingAccentColor: themeSettings.outgoingAccentColor, messageColors: themeSettings.messageColors, animateMessageColors: themeSettings.animateMessageColors, wallpaper: themeSettings.wallpaper)
                    ...
                    updatedSettings = ElloAppThemeSettings(baseTheme: .night, accentColor: themeSettings.accentColor, outgoingAccentColor: themeSettings.outgoingAccentColor, messageColors: themeSettings.messageColors, animateMessageColors: themeSettings.animateMessageColors, wallpaper: themeSettings.wallpaper)
                    if let settings = updatedSettings, let theme = makePresentationTheme(settings: settings) {
                        updatedTheme = theme
                    }
                }
            ...
            rightNavigationButton = ItemListNavigationButton(content: .text(presentationData.strings.Common_Done), style: .bold, enabled: isComplete, action: {
                ...
                |> take(1)).start(next: { previewTheme, settings in
                    let saveThemeTemplateFile: (String, LocalFileMediaResource, @escaping () -> Void) -> Void = { title, resource, completion in
                        let file = ElloAppMediaFile(fileId: MediaId(namespace: Namespaces.Media.LocalFile, id: resource.fileId), partialReference: nil, resource: resource, previewRepresentations: [], videoThumbnails: [], immediateThumbnailData: nil, mimeType: "application/x-tgtheme-ios", size: nil, attributes: [.FileName(fileName: "\(title).tgios-theme")])
                    ...
                    let resolvedWallpaper: ElloAppWallpaper?
