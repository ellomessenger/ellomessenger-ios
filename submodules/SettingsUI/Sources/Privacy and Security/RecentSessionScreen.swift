...
private class RecentSessionScreenNode: ViewControllerTracingNode, UIScrollViewDelegate {
    ...
    private var deviceIconNode: ASImageNode
    ...
    private var ipIconNode: ASImageNode
    ...
    private var locationIconNode: ASImageNode
...
//        self.contentBackgroundNode.backgroundColor = backgroundColor
        ...
        self.cancelButton = SolidRoundedButtonNode(theme: SolidRoundedButtonTheme(backgroundColor: self.presentationData.theme.list.itemBlocksBackgroundColor,
                                                                                  foregroundColor: self.presentationData.theme.list.itemAccentColor),
                                                      font: .regular,
                                                      fontSize: 16.0,
                                                      height: 54.0,
                                                      cornerRadius: 14.0,
        ...
        self.terminateButton = SolidRoundedButtonNode(theme: SolidRoundedButtonTheme(backgroundColor: .clear, // self.presentationData.theme.list.itemBlocksBackgroundColor,
                                                                                     foregroundColor: self.presentationData.theme.list.itemDestructiveColor),
                                                      font: .regular,
                                                      fontSize: 16.0, 
                                                      height: 54.0,
                                                      cornerRadius: 14.0,
...
//                }
        ...
        self.titleNode.attributedText = NSAttributedString(string: title, font: Font.regular(20.0), textColor: textColor)
        self.textNode.attributedText = NSAttributedString(string: subtitle, font: Font.regular(14.0), textColor: /* subtitleActive ? accentColor : */ secondaryTextColor)
        
        self.deviceIconNode.image = UIImage(bundleImageName: "Settings/Popup/Device")
        self.deviceIconNode.contentMode = .center
        self.deviceTitleNode.attributedText = NSAttributedString(string: deviceTitle, font: Font.regular(14.0), textColor: secondaryTextColor)
        self.deviceValueNode.attributedText = NSAttributedString(string: device, font: Font.regular(16.0), textColor: textColor)
        ...
        self.ipIconNode.image = UIImage(bundleImageName: "Settings/Popup/IP")
        self.ipIconNode.contentMode = .center
        self.ipTitleNode.attributedText = NSAttributedString(string: self.presentationData.strings.AuthSessions_View_IP, font: Font.regular(14.0), textColor: secondaryTextColor)
        self.ipValueNode.attributedText = NSAttributedString(string: ip, font: Font.regular(16.0), textColor: textColor)
        ...
        self.locationIconNode.image = UIImage(bundleImageName: "Settings/Popup/Location")
        self.locationIconNode.contentMode = .center
        self.locationTitleNode.attributedText = NSAttributedString(string: self.presentationData.strings.AuthSessions_View_Location.removedUNKNOWN, font: Font.regular(14.0), textColor: secondaryTextColor)
        self.locationValueNode.attributedText = NSAttributedString(string: location.removedUNKNOWN, font: Font.regular(16.0), textColor: textColor)
        self.locationInfoNode.attributedText = NSAttributedString(string: self.presentationData.strings.AuthSessions_View_LocationInfo.removedUNKNOWN, font: Font.regular(13.0), textColor: secondaryTextColor)
...
//    EM-1833 Manage devices and Sessions
//        if hasIncomingCalls {
...
//        self.contentBackgroundNode.backgroundColor =  self.presentationData.theme.list.blocksBackgroundColor
        ...
        self.titleNode.attributedText = NSAttributedString(string: self.titleNode.attributedText?.string ?? "", font: Font.regular(20.0), textColor: self.presentationData.theme.list.itemPrimaryTextColor)
        
        let subtitleColor: UIColor
        if case let .session(session) = self.subject, session.isCurrent {
            subtitleColor = self.presentationData.theme.list.itemSecondaryTextColor //self.presentationData.theme.list.itemAccentColor
        } else {
            subtitleColor = self.presentationData.theme.list.itemSecondaryTextColor
        }
        
        self.textNode.attributedText = NSAttributedString(string: self.textNode.attributedText?.string ?? "", font: Font.regular(14.0), textColor: subtitleColor)
        ...
        self.deviceTitleNode.attributedText = NSAttributedString(string: self.deviceTitleNode.attributedText?.string ?? "", 
                                                                 font: Font.regular(14.0),
                                                                 textColor: self.presentationData.theme.list.itemSecondaryTextColor)
        
        self.locationTitleNode.attributedText = NSAttributedString(string: self.locationTitleNode.attributedText?.string.removedUNKNOWN ?? "",
                                                                   font: Font.regular(14.0),
                                                                   textColor: self.presentationData.theme.list.itemSecondaryTextColor)
        
        self.ipTitleNode.attributedText = NSAttributedString(string: self.ipTitleNode.attributedText?.string ?? "",
                                                             font: Font.regular(14.0),
                                                             textColor: self.presentationData.theme.list.itemSecondaryTextColor)
        
        self.deviceValueNode.attributedText = NSAttributedString(string: self.deviceValueNode.attributedText?.string ?? "", 
                                                                 font: Font.regular(16.0),
                                                                 textColor: self.presentationData.theme.list.itemPrimaryTextColor)
        
        self.locationValueNode.attributedText = NSAttributedString(string: self.locationValueNode.attributedText?.string.removedUNKNOWN ?? "",
                                                                   font: Font.regular(16.0),
                                                                   textColor: self.presentationData.theme.list.itemPrimaryTextColor)
        
        self.ipValueNode.attributedText = NSAttributedString(string: self.ipValueNode.attributedText?.string ?? "",
                                                             font: Font.regular(16.0),
                                                             textColor: self.presentationData.theme.list.itemPrimaryTextColor)
        
        self.locationInfoNode.attributedText = NSAttributedString(string: self.locationInfoNode.attributedText?.string.removedUNKNOWN ?? "",
        ...
        self.cancelButton.updateTheme(SolidRoundedButtonTheme(backgroundColor: self.presentationData.theme.list.itemBlocksBackgroundColor,
                                                              foregroundColor: self.presentationData.theme.list.itemAccentColor))

        self.terminateButton.updateTheme(SolidRoundedButtonTheme(backgroundColor: .clear, // self.presentationData.theme.list.itemBlocksBackgroundColor,
                                                                 foregroundColor: self.presentationData.theme.list.itemDestructiveColor))
    }
...
//        let bottomInset: CGFloat = 10.0 + cleanInsets.bottom
        ...
        let fieldItemHeight: CGFloat = 62.0
        let fieldFrame = CGRect(x: 0, y: textFrame.maxY + 24.0, width: width, height: fieldItemHeight * 3.0)
        
        let leftTextInset = 66.0
        let titleYOffset = 33.0
        let valueYOffset = 10.0
        let titleWidth = width - inset - leftTextInset
    
        let deviceValueTextSize = self.deviceValueNode.updateLayout(CGSize(width: titleWidth, height: fieldItemHeight))
        let deviceValueTextFrame = CGRect(origin: CGPoint(x: leftTextInset,
                                                          y: fieldFrame.minY + valueYOffset),
                                          size: deviceValueTextSize)
        transition.updateFrame(node: self.deviceValueNode, frame: deviceValueTextFrame)
        
        transition.updateFrame(node: self.deviceIconNode, frame: CGRect(x: inset, y: fieldFrame.minY, width: leftTextInset - inset, height: fieldItemHeight))
        let deviceTitleTextSize = self.deviceTitleNode.updateLayout(CGSize(width: titleWidth, height: fieldItemHeight))
        let deviceTitleTextFrame = CGRect(origin: CGPoint(x: leftTextInset,
                                                          y: fieldFrame.minY + titleYOffset),
                                          size: deviceTitleTextSize)
        transition.updateFrame(node: self.deviceTitleNode, frame: deviceTitleTextFrame)
        
        let locationIsHidden = (self.locationValueNode.attributedText?.string.removedUNKNOWN ?? "").trimmingCharacters(in:.whitespacesAndNewlines).isEmpty
        let firstSeparatorY = fieldFrame.minY + fieldItemHeight
        transition.updateFrame(node: self.firstSeparatorNode, frame: CGRect(x: leftTextInset, y: firstSeparatorY,
                                                                            width: fieldFrame.width - leftTextInset - inset, 
                                                                            height: (locationIsHidden ? 0 : UIScreenPixel)))
        
        self.locationIconNode.isHidden = locationIsHidden
        transition.updateFrame(node: self.locationIconNode, frame: locationIsHidden ? CGRectZero : CGRect(x: inset, y: firstSeparatorY,
                                                                                                          width: leftTextInset - inset,
                                                                                                          height: fieldItemHeight))
        
        let locationValueTextSize = locationIsHidden ? CGSizeZero : self.locationValueNode.updateLayout(CGSize(width: titleWidth, 
                                                                                                               height: fieldItemHeight))
        let locationValueTextFrame = CGRect(origin: CGPoint(x: leftTextInset,
                                                            y: firstSeparatorY + (locationIsHidden ? 0 : valueYOffset)),
                                            size: locationValueTextSize)
        transition.updateFrame(node: self.locationValueNode, frame: locationValueTextFrame)

        let locationTitleTextSize = self.locationTitleNode.updateLayout(CGSize(width: locationIsHidden ? 0 : titleWidth,
                                                                               height: locationIsHidden ? 0 : fieldItemHeight))
        let locationTitleTextFrame = CGRect(origin: CGPoint(x: leftTextInset,
                                                            y: firstSeparatorY + (locationIsHidden ? 0 : titleYOffset)),
                                            size: locationTitleTextSize)
        transition.updateFrame(node: self.locationTitleNode, frame: locationTitleTextFrame)
        
        let locationInfoTextSize = CGSize.zero // self.locationInfoNode.updateLayout(CGSize(width: fieldFrame.width - inset * 2.0, height: fieldItemHeight * 2.0))
...
//        transition.updateFrame(node: self.locationInfoNode, frame: locationInfoTextFrame)
        ...
        let secondSeparatorY = fieldFrame.minY + fieldItemHeight + (locationIsHidden ? 0 : fieldItemHeight)
        transition.updateFrame(node: self.secondSeparatorNode, frame: CGRect(x: leftTextInset, y: secondSeparatorY,
                                                                             width: fieldFrame.width - leftTextInset - inset, height: UIScreenPixel))
        
        let ipIsHidden = (self.ipValueNode.attributedText?.string.removedUNKNOWN ?? "").trimmingCharacters(in:.whitespacesAndNewlines).isEmpty
        self.ipIconNode.isHidden = ipIsHidden
        self.ipValueNode.isHidden = ipIsHidden
        self.ipTitleNode.isHidden = ipIsHidden
        
        transition.updateFrame(node: self.ipIconNode, frame: ipIsHidden ? CGRectZero : CGRect(x: inset, y: secondSeparatorY,
                                                                                              width: leftTextInset - inset,
                                                                                              height: fieldItemHeight))
        let ipValueTextSize = self.ipValueNode.updateLayout(CGSize(width: titleWidth, height: fieldItemHeight))
        let ipValueTextFrame = CGRect(origin: CGPoint(x: leftTextInset,
                                                      y: secondSeparatorY + valueYOffset),
                                      size: ipValueTextSize)
        transition.updateFrame(node: self.ipValueNode, frame: ipValueTextFrame)
        
        let ipTitleTextSize = self.ipTitleNode.updateLayout(CGSize(width: titleWidth, height: fieldItemHeight))
        let ipTitleTextFrame = CGRect(origin: CGPoint(x: leftTextInset,
                                                      y: secondSeparatorY + titleYOffset),
                                      size: ipTitleTextSize)
        transition.updateFrame(node: self.ipTitleNode, frame: ipTitleTextFrame)
                
        var contentHeight = fieldFrame.maxY // locationInfoTextFrame.maxY + bottomInset + 64.0
        ...
        var fieldHeight = fieldFrame.maxY
        if isCurrent {
            ...
            transition.updateFrame(node: self.terminateButton, frame: CGRect(x: inset,
                                                                             y: contentHeight,
                                                                             width: width,
                                                                             height: doneButtonHeight))
            contentHeight += doneButtonHeight
            fieldHeight += doneButtonHeight
        }
        
        transition.updateFrame(node: self.fieldBackgroundNode, frame: CGRect(x: inset, y: 0, width: width - inset - inset, height: fieldHeight))
        
        let cancelButtonHeight = self.cancelButton.updateLayout(width: width - inset * 2.0, transition: transition)
        contentHeight += cancelButtonHeight + insets.bottom + 16.0 // Cancel button
        ...
        transition.updateFrame(node: self.cancelButton, frame: CGRect(x: inset,
                                                                      y: contentHeight - cancelButtonHeight - insets.bottom - 8.0,
                                                                      width: width,
