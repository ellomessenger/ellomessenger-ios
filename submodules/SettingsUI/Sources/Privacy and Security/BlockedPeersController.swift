...
private enum BlockedPeersEntry: ItemListNodeEntry {
    ...
    var section: ItemListSectionId {
        ...
        case .info:
            return BlockedPeersSection.info.rawValue
        case .peerItem:
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        case let .add(theme, text):
            return ItemListPeerActionItem(presentationData: presentationData, 
                                          icon: PresentationResourcesItemList.blockAccentIcon(theme),
                                          title: text,
        ...
        case let .peerItem(_, _, strings, dateTimeFormat, nameDisplayOrder, peer, editing, enabled):
            let revealOptions = ItemListPeerItemRevealOptions(options: [ItemListPeerItemRevealOption(type: .default, title: strings.BlockedUsers_Unblock, action: {
        ...
        case let .info(text):
            return ItemListTextEmptyStateItem(presentationData: presentationData, text: text, sectionId: self.section)
        }
    }
}
...
private func blockedPeersControllerEntries(presentationData: PresentationData, state: BlockedPeersControllerState, blockedPeersState: BlockedPeersContextState) -> [BlockedPeersEntry] {
    ...
    if !blockedPeersState.peers.isEmpty || !blockedPeersState.canLoadMore {
        ...
        entries.append(.info(presentationData.strings.BlockedUsers_Info))
        var index: Int32 = 0
        for peer in blockedPeersState.peers {
            if !peer.peerId.isAIBots {
...
public func blockedPeersController(context: AccountContext, blockedPeersContext: BlockedPeersContext) -> ViewController {
    ...
    let arguments = BlockedPeersControllerArguments(context: context, setPeerIdWithRevealedOptions: { peerId, fromPeerId in
        ...
        let controller = context.sharedContext.makePeerSelectionController(PeerSelectionControllerParams(context: context, filter: [.onlyPrivateChats, .excludeSavedMessages, .removeSearchHeader, .excludeRecent, .doNotSearchMessages, .excludeBots], title: presentationData.strings.BlockedUsers_SelectUserTitle))
