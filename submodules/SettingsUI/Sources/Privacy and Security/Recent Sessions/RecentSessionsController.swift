    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        case let .terminateOtherSessions(_, text):
            return ItemListPeerActionItem(presentationData: presentationData, 
                                          icon: PresentationResourcesItemList.block2DestructiveIcon(presentationData.theme),
...
//    entries.append(.header(SortIndex(section: 0, item: 0), presentationData.strings.AuthSessions_HeaderInfo))
    ...
    if !sessionsState.sessions.isEmpty {
        ...
        entries.append(.currentSessionHeader(SortIndex(section: 1, item: 0), presentationData.strings.AuthSessions_ThisDevice))
        ...
        if sessionsState.sessions.count > 1 || enableQRLogin {
            if sessionsState.sessions.count > 1 {
                ...
                entries.append(.currentSessionInfo(SortIndex(section: 1, item: 3), presentationData.strings.AuthSessions_TerminateOtherSessionsHelp)) // "Logs out all devices except for this one."
...
//                entries.append(.currentAddDevice(SortIndex(section: 1, item: 4), presentationData.strings.AuthSessions_AddDevice))
            ...
            if !filteredPendingSessions.isEmpty {
                ...
                for i in 0 ..< filteredPendingSessions.count {
                    if !existingSessionIds.contains(filteredPendingSessions[i].hash) {
                        ...
                        entries.append(.pendingSession(index: Int32(i), 
                                                       sortIndex: SortIndex(section: 2, item: i),
                                                       strings: presentationData.strings,
                                                       dateTimeFormat: presentationData.dateTimeFormat,
                                                       session: filteredPendingSessions[i],
                                                       enabled: state.removingSessionId != filteredPendingSessions[i].hash && !state.terminatingOtherSessions,
                                                       editing: state.editing,
                                                       revealed: state.sessionIdWithRevealedOptions == filteredPendingSessions[i].hash))
                    }
                }
            ...
            for i in 0 ..< filteredSessions.count {
                if !existingSessionIds.contains(filteredSessions[i].hash) {
                    ...
                    entries.append(.session(index: Int32(i), 
                                            sortIndex: SortIndex(section: 5, item: i),
                                            strings: presentationData.strings,
                                            dateTimeFormat: presentationData.dateTimeFormat,
                                            session: filteredSessions[i],
                                            enabled: state.removingSessionId != filteredSessions[i].hash && !state.terminatingOtherSessions,
                                            editing: state.editing,
                                            revealed: state.sessionIdWithRevealedOptions == filteredSessions[i].hash))
                }
            }
            
//            entries.append(.devicesInfo(SortIndex(section: 5, item: filteredSessions.count), presentationData.strings.AuthSessions_ElloAvailable))
...
private func recentSessionsControllerEntries(presentationData: PresentationData, state: RecentSessionsControllerState, websites: [WebAuthorization]?, peers: [PeerId : Peer]?) -> [RecentSessionsEntry] {
    ...
    if let websites = websites, let peers = peers {
        ...
        if websites.count > 0 {
            ...
            entries.append(.otherSessionsHeader(SortIndex(section: 0, item: 2), presentationData.strings.AuthSessions_LoggedInWithElloApp))
...
public func recentSessionsController(context: AccountContext, 
                                     activeSessionsContext: ActiveSessionsContext,
                                     webSessionsContext: WebSessionsContext,
                                     websitesOnly: Bool) -> ViewController & RecentSessionsController {
    ...
    let arguments = RecentSessionsControllerArguments(context: context, setSessionIdWithRevealedOptions: { sessionId, fromSessionId in
        ...
        controller.setItemGroups([
            ActionSheetItemGroup(items: [
                ...
                ActionSheetButtonItem(title: presentationData.strings.AuthSessions_TerminateOtherSessions, color: .accent, action: {
                    dismissAction()
        ...
        context.sharedContext.openExternalUrl(context: context, urlContext: .generic, url: "https://elloapp.com/apps", forceExternal: true, presentationData: context.sharedContext.currentPresentationData.with { $0 }, navigationController: nil, dismissInput: {})
        ...
        context.sharedContext.openExternalUrl(context: context, urlContext: .generic, url: "https://elloapp.com/edesktop", forceExternal: true, presentationData: context.sharedContext.currentPresentationData.with { $0 }, navigationController: nil, dismissInput: {})
    }, openWebLink: {
        context.sharedContext.openExternalUrl(context: context, urlContext: .generic, url: "https://web.elloapp.com", forceExternal: true, presentationData: context.sharedContext.currentPresentationData.with { $0 }, navigationController: nil, dismissInput: {})
    })
