...
private func trimmedLocationName(_ session: RecentAccountSession) -> String {
    return session.country.replacingOccurrences(of: "United Arab Emirates", with: "UAE").removedUNKNOWN
}
...
//            }
            ...
            titleAttributedString = NSAttributedString(string: deviceString.removedUNKNOWN, font: titleFont, textColor: item.presentationData.theme.list.itemPrimaryTextColor)
            appAttributedString = NSAttributedString(string: appString.removedUNKNOWN, font: textFont, textColor: item.presentationData.theme.list.itemPrimaryTextColor)
