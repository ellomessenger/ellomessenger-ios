    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        switch self {
            case let .peerItem(_, _, strings, dateTimeFormat, nameDisplayOrder, peer, editing, enabled):
                ...
                if let group = peer.peer as? ElloAppGroup {
        ...
        |> deliverOnMainQueue).start(next: { [weak controller] result in
            ...
            |> mapToSignal { peers -> Signal<[SelectivePrivacyPeer], NoError> in
                ...
                return context.engine.data.get(
                    EngineDataMap(filteredPeerIds.map(ElloAppEngine.EngineData.Item.Peer.Peer.init)),
                    EngineDataMap(filteredPeerIds.map(ElloAppEngine.EngineData.Item.Peer.ParticipantCount.init))
                )
