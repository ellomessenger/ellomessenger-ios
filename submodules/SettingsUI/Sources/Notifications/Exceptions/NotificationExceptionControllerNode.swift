...
public struct NotificationExceptionWrapper : Equatable {
    let settings: ElloAppPeerNotificationSettings
    let date: TimeInterval?
    let peer: Peer
    init(settings: ElloAppPeerNotificationSettings, peer: Peer, date: TimeInterval? = nil) {
    ...
    func withUpdatedSettings(_ settings: ElloAppPeerNotificationSettings) -> NotificationExceptionWrapper {
        return NotificationExceptionWrapper(settings: settings, peer: self.peer, date: self.date)
    }
    
    func updateSettings(_ f: (ElloAppPeerNotificationSettings) -> ElloAppPeerNotificationSettings) -> NotificationExceptionWrapper {
            ...
            if let value = values[peerId] {
                ...
                switch sound {
                    case .default:
                        break
                    default:
                        values[peerId] = NotificationExceptionWrapper(settings: ElloAppPeerNotificationSettings(muteState: .default, messageSound: sound, displayPreviews: .default), peer: peer, date: Date().timeIntervalSince1970)
                }
            }
            return values
        }
            ...
            if let value = values[peerId] {
                ...
                switch muteState {
                    case .default:
                        break
                    default:
                        values[peerId] = NotificationExceptionWrapper(settings: ElloAppPeerNotificationSettings(muteState: muteState, messageSound: .default, displayPreviews: .default), peer: peer, date: Date().timeIntervalSince1970)
                }
            }
            return values
        }
        ...
        let apply:([PeerId : NotificationExceptionWrapper], PeerId, PeerNotificationDisplayPreviews) -> [PeerId : NotificationExceptionWrapper] = { values, peerId, displayPreviews in
            ...
            if let value = values[peerId] {
                ...
                case .default:
                    break
                default:
                    values[peerId] = NotificationExceptionWrapper(settings: ElloAppPeerNotificationSettings(muteState: .unmuted, messageSound: .default, displayPreviews: displayPreviews), peer: peer, date: Date().timeIntervalSince1970)
                }
            }
            return values
        }
...
private func notificationsExceptionEntries(presentationData: PresentationData, notificationSoundList: NotificationSoundList?, state: NotificationExceptionState, query: String? = nil, foundPeers: [RenderedPeer] = []) -> [NotificationExceptionEntry] {
    ...
    for (_, value) in state.mode.settings.filter({ (_, value) in
        ...
        if let lhsPeer = lhs.value.peer as? ElloAppUser, let rhsPeer = rhs.value.peer as? ElloAppUser {
    ...
    if state.isSearchMode {
        for renderedPeer in foundPeers {
            ...
            switch state.mode {
                ...
                case .groups:
                    if let channel = peer as? ElloAppChannel, case .broadcast = channel.info {
                    } else if peer is ElloAppGroup {
                    } else {
                        continue
                    }
                case .users:
                    if peer is ElloAppUser {
                    } else {
                        continue
                    }
            }
...
private enum NotificationExceptionEntry : ItemListNodeEntry {
    ...
    case peer(index: Int, peer: Peer, theme: PresentationTheme, strings: PresentationStrings, dateFormat: PresentationDateTimeFormat, nameDisplayOrder: PresentationPersonNameOrder, description: String, notificationSettings: ElloAppPeerNotificationSettings, revealed: Bool, editing: Bool, isSearching: Bool)
...
final class NotificationExceptionsControllerNode: ViewControllerTracingNode {
    ...
    init(context: AccountContext, presentationData: PresentationData, navigationBar: NavigationBar, mode: NotificationExceptionMode, updatedMode:@escaping(NotificationExceptionMode)->Void, requestActivateSearch: @escaping () -> Void, requestDeactivateSearch: @escaping (Bool) -> Void, updateCanStartEditing: @escaping (Bool?) -> Void, present: @escaping (ViewController, Any?) -> Void, pushController: @escaping (ViewController) -> Void) {
        ...
        let updateNotificationsView: (@escaping () -> Void) -> Void = { completion in
            updateState { current in
                ...
                updateNotificationsDisposable.set((context.engine.data.subscribe(EngineDataMap(
                    peerIds.map(ElloAppEngine.EngineData.Item.Peer.NotificationSettings.init)
                ))
                |> deliverOnMainQueue).start(next: { notificationSettingsMap in
                    let _ = (context.engine.data.get(
                        EngineDataMap(notificationSettingsMap.keys.map(ElloAppEngine.EngineData.Item.Peer.Peer.init)),
                        EngineDataMap(notificationSettingsMap.keys.map(ElloAppEngine.EngineData.Item.Peer.NotificationSettings.init))
                    )
        ...
        let presentPeerSettings: (PeerId, @escaping () -> Void) -> Void = { [weak self] peerId, completion in
            ...
            let _ = (context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId))
            |> deliverOnMainQueue).start(next: { peer in
                ...
                presentControllerImpl?(notificationPeerExceptionController(context: context, peer: peer._asPeer(), mode: mode, updatePeerSound: { peerId, sound in
                    _ = updatePeerSound(peer.id, sound).start(next: { _ in
                        ...
                        _ = combineLatest(updatePeerSound(peer.id, sound), context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId)) |> deliverOnMainQueue).start(next: { _, peer in
                            if let peer = peer {
                    ...
                    updateNotificationsDisposable.set(nil)
                    _ = combineLatest(updatePeerNotificationInterval(peerId, muteInterval), context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId)) |> deliverOnMainQueue).start(next: { _, peer in
                        if let peer = peer {
                            updateState { value in
                    ...
                    updateNotificationsDisposable.set(nil)
                    _ = combineLatest(updatePeerDisplayPreviews(peerId, displayPreviews), context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId)) |> deliverOnMainQueue).start(next: { _, peer in
                        if let peer = peer {
                            updateState { value in
                    ...
                    |> map { _ -> EnginePeer? in }
                    |> then(
                        context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId))
                    )).start(next: { peer in
                        guard let peer = peer else {
                            return
                        }
...
private final class NotificationExceptionsSearchContainerNode: SearchDisplayControllerContentNode {
    ...
    init(context: AccountContext, mode: NotificationExceptionMode, arguments: NotificationExceptionArguments) {
        ...
        let updateNotificationsView: (@escaping () -> Void) -> Void = { completion in
            updateNotificationsDisposable.set(context.engine.data.subscribe(EngineDataMap(
                mode.peerIds.map(ElloAppEngine.EngineData.Item.Peer.NotificationSettings.init)
