...
private final class ThemePickerControllerArguments {
    ...
    let selectTheme: (ElloAppBaseTheme?, PresentationThemeReference, Bool) -> Void
    let previewTheme: (PresentationThemeReference, Bool, Bool, [Int64: PresentationThemeAccentColor]) -> Void
    let selectAccentColor: (ElloAppBaseTheme?, PresentationThemeAccentColor?) -> Void
    ...
    init(context: AccountContext, selectTheme: @escaping (ElloAppBaseTheme?, PresentationThemeReference, Bool) -> Void, previewTheme: @escaping (PresentationThemeReference, Bool, Bool, [Int64: PresentationThemeAccentColor]) -> Void, selectAccentColor: @escaping (ElloAppBaseTheme?, PresentationThemeAccentColor?) -> Void, openAccentColorPicker: @escaping (PresentationThemeReference, Bool) -> Void, editTheme: @escaping (PresentationCloudTheme) -> Void, editCurrentTheme: @escaping () -> Void, createNewTheme: @escaping () -> Void, themeContextAction: @escaping (Bool, PresentationThemeReference, ASDisplayNode, ContextGesture?) -> Void, colorContextAction: @escaping (Bool, PresentationThemeReference, ThemeSettingsColorOption?, ASDisplayNode, ContextGesture?) -> Void) {
...
private enum ThemePickerControllerEntry: ItemListNodeEntry {
    ...
    case themes(PresentationTheme, PresentationStrings, [PresentationThemeReference], PresentationThemeReference, Bool, [String: [StickerPackItem]], [Int64: PresentationThemeAccentColor], [Int64: ElloAppWallpaper])
    case customHeader(PresentationTheme, String)
    case chatPreview(PresentationTheme, ElloAppWallpaper, PresentationFontSize, PresentationChatBubbleCorners, PresentationStrings, PresentationDateTimeFormat, PresentationPersonNameOrder, [ChatPreviewMessageItem])
    case theme(PresentationTheme, PresentationStrings, [PresentationThemeReference], [PresentationThemeReference], PresentationThemeReference, [Int64: PresentationThemeAccentColor], [Int64: ElloAppWallpaper], PresentationThemeAccentColor?, [Int64: ElloAppBaseTheme])
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        switch self {
            ...
            case let .accentColor(theme, generalThemeReference, currentTheme, themes, color):
                ...
                let baseTheme: ElloAppBaseTheme?
...
public func themePickerController(context: AccountContext, focusOnItemTag: ThemeSettingsEntryTag? = nil) -> ViewController {
    ...
    var selectThemeImpl: ((ElloAppBaseTheme?, PresentationThemeReference, Bool) -> Void)?
    var selectAccentColorImpl: ((ElloAppBaseTheme?, PresentationThemeAccentColor?) -> Void)?
    var openAccentColorPickerImpl: ((PresentationThemeReference, Bool) -> Void)?
    
    let _ = elloappWallpapers(postbox: context.account.postbox, network: context.account.network).start()
    
    let cloudThemes = Promise<[ElloAppTheme]>()
    let updatedCloudThemes = elloappThemes(postbox: context.account.postbox, network: context.account.network, accountManager: context.sharedContext.accountManager)
    ...
    let arguments = ThemePickerControllerArguments(context: context, selectTheme: { baseTheme, theme, preset in
        ...
        let _ = (context.sharedContext.accountManager.transaction { transaction -> (PresentationThemeAccentColor?, ElloAppWallpaper?) in
            let settings = transaction.getSharedData(ApplicationSpecificSharedDataKeys.presentationThemeSettings)?.get(PresentationThemeSettings.self) ?? PresentationThemeSettings.defaultSettings
        ...
        |> map { accentColor, wallpaper -> (PresentationThemeAccentColor?, ElloAppWallpaper) in
            let effectiveWallpaper: ElloAppWallpaper
        ...
        |> mapToSignal { accentColor, wallpaper -> Signal<(PresentationThemeAccentColor?, ElloAppWallpaper), NoError> in
        ...
        |> mapToSignal { accentColor, wallpaper -> Signal<(PresentationTheme?, ElloAppWallpaper?), NoError> in
            ...
            if case let .cloud(theme) = reference {
                if theme.theme.isCreator {
                    ...
                    items.append(.action(ContextMenuActionItem(text: strings.Theme_Context_ChangeColors, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/ApplyTheme"), color: theme.contextMenu.primaryColor) }, action: { c, f in
                        ...
                        let resolvedWallpaper: Signal<ElloAppWallpaper, NoError>
                        if case let .file(file) = theme.chat.defaultWallpaper, file.id == 0 {
                            ...
                            |> map { cachedWallpaper -> ElloAppWallpaper in
                ...
                items.append(.action(ContextMenuActionItem(text: presentationData.strings.Appearance_ShareTheme, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Share"), color: theme.contextMenu.primaryColor) }, action: { c, f in
                    c.dismiss(completion: {
                        let shareController = ShareController(context: context, subject: .url("https://ello.team/addtheme/\(theme.theme.slug)"), preferredAction: .default)
        ...
        let _ = (context.sharedContext.accountManager.transaction { transaction -> (ThemeSettingsColorOption?, ElloAppWallpaper?) in
            let settings = transaction.getSharedData(ApplicationSpecificSharedDataKeys.presentationThemeSettings)?.get(PresentationThemeSettings.self) ?? PresentationThemeSettings.defaultSettings
            var wallpaper: ElloAppWallpaper?
            if let accentColor = accentColor {
                switch accentColor {
            ...
            return (accentColor, wallpaper)
        } |> mapToSignal { accentColor, wallpaper -> Signal<(PresentationTheme?, PresentationThemeReference, Bool, ElloAppWallpaper?), NoError> in
            let generalThemeReference: PresentationThemeReference
            ...
            let wallpaperSignal: Signal<ElloAppWallpaper, NoError>
            ...
            |> map { wallpaper, serviceBackgroundColor -> (PresentationTheme?, PresentationThemeReference, ElloAppWallpaper) in
                if let accentColor = accentColor, case let .theme(themeReference) = accentColor {
            ...
            |> mapToSignal { theme, reference, wallpaper in
                if case let .cloud(info) = reference {
                    ...
                    |> map { cloudThemeExists -> (PresentationTheme?, PresentationThemeReference, Bool, ElloAppWallpaper) in
        ...
        |> deliverOnMainQueue).start(next: { theme, effectiveThemeReference, cloudThemeExists, wallpaper in
            ...
            if let accentColor = accentColor {
                if case let .accentColor(color) = accentColor, color.baseColor != .custom {
                    ...
                    if cloudTheme.theme.isCreator && cloudThemeExists {
                        ...
                        items.append(.action(ContextMenuActionItem(text: strings.Theme_Context_ChangeColors, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/ApplyTheme"), color: theme.contextMenu.primaryColor) }, action: { c, f in
                            ...
                            |> deliverOnMainQueue).start(next: { wallpaper in
                                ...
                                var settings: ElloAppThemeSettings?
                    ...
                    items.append(.action(ContextMenuActionItem(text: presentationData.strings.Appearance_ShareTheme, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Share"), color: theme.contextMenu.primaryColor) }, action: { c, f in
                        c.dismiss(completion: {
                            let shareController = ShareController(context: context, subject: .url("https://ello.team/addtheme/\(cloudTheme.theme.slug)"), preferredAction: .default)
                            shareController.actionCompleted = {
                                let presentationData = context.sharedContext.currentPresentationData.with { $0 }
                                presentControllerImpl?(UndoOverlayController(presentationData: presentationData, content: .linkCopied(text: presentationData.strings.Conversation_LinkCopied), elevatedLayout: false, animateInAsReplacement: false, action: { _ in return false }), nil)
                            }
    ...
    |> map { [weak switchNode] presentationData, sharedData, cloudThemes, removedThemeIndexes, animatedEmojiStickers, nightModePreview -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        if nightModePreview {
            let preferredBaseTheme: ElloAppBaseTheme = .night
    ...
    selectThemeImpl = { baseTheme, theme, preset in
        ...
        if case let .file(file) = presentationTheme.chat.defaultWallpaper, file.id == 0 {
            resolvedWallpaper = cachedWallpaper(account: context.account, slug: file.slug, settings: file.settings)
            |> map { wallpaper -> ElloAppWallpaper? in
        ...
        |> map { resolvedWallpaper, currentTheme -> Bool in
            ...
            var updatedBaseTheme: ElloAppBaseTheme?
    ...
    selectAccentColorImpl = { currentBaseTheme, accentColor in
        var wallpaperSignal: Signal<ElloAppWallpaper?, NoError> = .single(nil)
