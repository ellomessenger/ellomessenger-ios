...
private func storageUsageControllerEntries(presentationData: PresentationData, cacheSettings: CacheStorageSettings, cacheStats: CacheUsageStatsResult?, state: StorageUsageState) -> [StorageUsageEntry] {
    ...
    if let cacheStats = cacheStats, case let .result(stats) = cacheStats {
        ...
        for (peerId, categories) in stats.media {
            ...
            if let group = stats.peers[peerId] as? ElloAppGroup, let migrationReference = group.migrationReference, let channel = stats.peers[migrationReference.peerId] {
        ...
        let elloappCacheSize = Int64(peerSizes + stats.otherSize + stats.cacheSize + stats.tempSize)
        let totalElloAppSize = elloappCacheSize + stats.immutableSize
        
        var categories: [StorageUsageCategory] = []
        let totalSpace = max(totalDiskSpace(), 1)
        let freeSpace = freeDiskSpace()
        let otherAppsSpace = totalSpace - freeSpace - totalElloAppSize
        
        let totalSpaceValue = CGFloat(totalSpace)
        
        if elloappCacheSize > 0 {
            categories.append(StorageUsageCategory(title: presentationData.strings.ClearCache_StorageCache, size: totalElloAppSize, fraction: CGFloat(totalElloAppSize) / totalSpaceValue, color: presentationData.theme.list.itemBarChart.color1))
        } else {
            categories.append(StorageUsageCategory(title: presentationData.strings.ClearCache_StorageServiceFiles, size: totalElloAppSize, fraction: CGFloat(totalElloAppSize) / totalSpaceValue, color: presentationData.theme.list.itemBarChart.color1))
        }
        ...
        entries.append(.clearAll(presentationData.theme, presentationData.strings.ClearCache_ClearCache, elloappCacheSize > 0))
...
func cacheUsageStats(context: AccountContext) -> Signal<CacheUsageStatsResult?, NoError> {
    ...
    return .single(nil)
    |> then(context.engine.resources.collectCacheUsageStats(additionalCachePaths: additionalPaths, logFilesPath: context.sharedContext.applicationBindings.containerPath + "/elloapp-data/logs")
    |> map(Optional.init))
}
                ...
                if var categories = stats.media[peerId], let peer = stats.peers[peerId] {
                    if let channel = peer as? ElloAppChannel, case .group = channel.info {
                        for (_, peer) in stats.peers {
                            if let group = peer as? ElloAppGroup, let migrationReference = group.migrationReference, migrationReference.peerId == peerId {
