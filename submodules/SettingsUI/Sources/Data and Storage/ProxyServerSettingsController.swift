...
private func shareLink(for server: ProxyServerSettings) -> String {
    ...
    case let .mtp(secret):
        ...
        link = "https://ello.team/proxy?server=\(server.host)&port=\(server.port)"
        link += "&secret=\(secret.addingPercentEncoding(withAllowedCharacters: CharacterSet.urlQueryValueAllowed) ?? "")"
    case let .socks5(username, password):
        link = "https://ello.team/socks?server=\(server.host)&port=\(server.port)"
...
private func proxyServerSettingsControllerEntries(presentationData: PresentationData, state: ProxyServerSettingsControllerState, pasteboardSettings: ProxyServerSettings?) -> [ProxySettingsEntry] {
    ...
    entries.append(.modeMtp(presentationData.theme, presentationData.strings.SocksProxySetup_ProxyElloApp, state.mode == .mtp))
...
func proxyServerSettingsController(context: AccountContext? = nil, presentationData: PresentationData, updatedPresentationData: Signal<PresentationData, NoError>, accountManager: AccountManager<ElloAppAccountManagerTypes>, postbox: Postbox, network: Network, currentSettings: ProxyServerSettings?) -> ViewController {
