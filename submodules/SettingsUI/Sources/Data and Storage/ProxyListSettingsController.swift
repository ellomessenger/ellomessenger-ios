    ...
    for server in proxySettings.servers {
        ...
        if proxySettings.enabled && server == proxySettings.activeServer {
            ...
            switch server.connection {
                ...
                case .mtp:
                    text = strings.SocksProxySetup_ProxyElloApp
            }
            switch status {
                case .notAvailable:
...
public func proxySettingsController(accountManager: AccountManager<ElloAppAccountManagerTypes>, context: AccountContext? = nil, postbox: Postbox, network: Network, mode: ProxySettingsControllerMode, presentationData: PresentationData, updatedPresentationData: Signal<PresentationData, NoError>) -> ViewController {
            ...
            |> deliverOnMainQueue).start(next: { settings in
                ...
                for server in settings.servers {
                    ...
                    case let .mtp(secret):
                        ...
                        string = "https://ello.team/proxy?server=\(server.host)&port=\(server.port)"
                        string += "&secret=\((secret as NSString).addingPercentEncoding(withAllowedCharacters: CharacterSet.urlQueryValueAllowed) ?? "")"
                    case let .socks5(username, password):
                        string = "https://ello.team/socks?server=\(server.host)&port=\(server.port)"
