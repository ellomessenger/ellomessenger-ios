...
private enum NotificationsPeerCategoryEntry: ItemListNodeEntry {
    ...
    case exception(Int32, PresentationTheme, PresentationStrings, PresentationDateTimeFormat, PresentationPersonNameOrder, Peer, String, ElloAppPeerNotificationSettings, Bool, Bool)
        ...
        updateState { current in
            ...
            |> deliverOnMainQueue).start(next: { combinedPeerNotificationSettings in
                let _ = (context.engine.data.get(
                    EngineDataMap(combinedPeerNotificationSettings.keys.map(ElloAppEngine.EngineData.Item.Peer.Peer.init)),
                    EngineDataMap(combinedPeerNotificationSettings.keys.map(ElloAppEngine.EngineData.Item.Peer.NotificationSettings.init))
                )
        ...
        |> deliverOnMainQueue).start(next: { peer in
            ...
            pushControllerImpl?(notificationPeerExceptionController(context: context, peer: peer._asPeer(), mode: mode, updatePeerSound: { peerId, sound in
                ...
                let _ = (
                    ...
                    |> map { _ -> EnginePeer? in }
                    |> then(context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId)))
                ).start(next: { peer in
                    guard let peer = peer else {
                        return
                    }
