...
public func fetchMediaData(context: AccountContext, postbox: Postbox, mediaReference: AnyMediaReference) -> Signal<(FetchMediaDataState, Bool), NoError> {
    ...
    if let image = mediaReference.media as? ElloAppMediaImage {
    ...
    } else if let webpage = mediaReference.media as? ElloAppMediaWebpage, case let .Loaded(content) = webpage.content {
