...
public enum OpenInItem {
    case url(url: String)
    case location(location: ElloAppMediaMap, directions: OpenInLocationDirections?)
}
        ...
        case let .location(location, directions):
            ...
            options.append(OpenInOption(identifier: "googleMaps", application: .other(title: "Google Maps", identifier: 585027354, scheme: "comgooglemaps-x-callback", store: nil), action: {
                ...
                if let directions = directions {
                    ...
                    return .openUrl(url: "comgooglemaps-x-callback://?daddr=\(coordinates)&directionsmode=\(directionsMode)&x-success=elloapp://?resume=true&x-source=ElloApp")
                } else {
                    if let venue = location.venue, let venueId = venue.id, let provider = venue.provider, provider == "gplaces" {
                        ...
                        return .openUrl(url: "comgooglemaps-x-callback://?center=\(coordinates)&q=\(coordinates)&x-success=elloapp://?resume=true&x-source=ElloApp")
                    }
                }
            }))
            ...
            options.append(OpenInOption(identifier: "moovit", application: .other(title: "Moovit", identifier: 498477945, scheme: "moovit", store: nil), action: {
                if let _ = directions {
                    ...
                    return .openUrl(url: "moovit://directions?dest_lat=\(lat)&dest_lon=\(lon)&dest_name=\(destName)&partner_id=ElloApp")
                } else {
                    return .openUrl(url: "moovit://nearby?lat=\(lat)&lon=\(lon)&partner_id=ElloApp")
                }
            }))
