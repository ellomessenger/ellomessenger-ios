...
static TexturedShape elloapp_sphere, elloapp_plane;
...
static int elloapp_sphere_texture, elloapp_plane_texture;
...
void set_elloapp_textures(int a_elloapp_sphere, int a_elloapp_plane)
{
    elloapp_sphere_texture = a_elloapp_sphere;
    elloapp_plane_texture = a_elloapp_plane;
}
...
void on_surface_created() {
    ...
    // ElloApp
    elloapp_sphere = create_textured_rectangle(CSizeMake(148, 148), elloapp_sphere_texture);
    elloapp_plane = create_textured_rectangle(CSizeMake(148, 148), elloapp_plane_texture);
    // elloapp_plane = create_textured_rectangle(CSizeMake(82, 74), elloapp_plane_texture);
    // elloapp_plane.params.anchor=xyzMake(6, -5, 0);
...
void on_draw_frame() {
    ...
    /*
    rglNormalDraw();
    elloapp_sphere.params.alpha = 1;
    //elloapp_sphere.params.position.y = -offset_y;
    draw_textured_shape(&elloapp_sphere, main_matrix, NORMAL);
    ...
    */
    ...
    if (current_page == 0)
    {
        ...
        elloapp_sphere.params.alpha = alpha;
        elloapp_sphere.params.scale = xyzMake(scale, scale, 1);

        elloapp_plane.params.alpha=1;
        ...
        elloapp_plane.params.position = xyzMake(dx, dy, 0);

        float scale = (cos(tt)+1)*.5;

        elloapp_plane.params.scale = xyzMake(cos(tt)*scale, scale, 1);
...
//powerful_mask.params.alpha = 0.0;
    ...
    if (current_page == 0)
    {
        ...
        if (direct == 0) {
            ...
            elloapp_sphere.params.alpha = t(0, 1, 0, duration_const*.8, Linear);
            ...
            if (tt < D2R(125)) {
                glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
                draw_textured_shape(&elloapp_plane, main_matrix, NORMAL_ONE);
            }
        }
    }
    else if (current_page==1)
    {
        ...
        if (direct == 1) {
            ...
            elloapp_sphere.params.alpha = t(1, 0, 0, duration_const, Linear);
            draw_textured_shape(&elloapp_sphere, main_matrix, NORMAL);
            ...
            elloapp_plane.params.position = xyzMake(dx, dy, 0);// CGPointMake(planeCenter.x + dx, planeCenter.y + dy);
