...
public func channelStatsController(context: AccountContext, 
                                   updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil,
                                   peerId: PeerId,
                                   statsDatacenterId: Int32?) -> ViewController {
    ...
    let statsContext = ChannelStatsContext(postbox: context.account.postbox, 
    ...
    let messageView = context.account.viewTracker.aroundMessageHistoryViewForLocation(.peer(peerId: peerId), 
                                                                                      index: .upperBound,
    ...
    |> map { presentationData, data, messageView, longLoading -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        if data == nil {
            if longLoading {
                emptyStateItem = ItemListLoadingEmptyImageItem(theme: presentationData.theme, 
                                                               emptyTitle: presentationData.strings.Stats_Loading,
                                                               emptyText:  presentationData.strings.Stats_LoadingText)
            } else {
                emptyStateItem = ItemListLoadingEmptyImageItem(theme: presentationData.theme, 
                                                               emptyTitle: presentationData.strings.Stats_EmptyStatsTitle,
                                                               emptyText: presentationData.strings.Stats_EmptyStatsText)
            }
        }
        ...
        let controllerState = ItemListControllerState(presentationData: ItemListPresentationData(presentationData), 
                                                      title: .text(presentationData.strings.ChannelInfo_Stats),
        ...
        let listState = ItemListNodeState(presentationData: ItemListPresentationData(presentationData),
                                          entries: channelStatsControllerEntries(data: data,
                                                                                 messages: messages,
                                                                                 interactions: interactions,
                                                                                 presentationData: presentationData),
                                          style: .blocks,
                                          emptyStateItem: emptyStateItem,
                                          crossfadeState: previous == nil,
    ...
    contextActionImpl = { [weak controller] messageId, sourceNode, gesture in
        ...
        let contextController = ContextController(account: context.account, 
                                                  presentationData: presentationData,
                                                  source: .extracted(ChannelStatsContextExtractedContentSource(controller: controller,
