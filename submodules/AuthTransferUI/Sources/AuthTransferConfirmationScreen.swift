    ...
    init(context: AccountContext, presentationData: PresentationData, action: @escaping () -> Void) {
        ...
        for textNode in self.textNodes {
            ...
            textNode.tapAttributeAction = { attributes, _ in
                if let _ = attributes[NSAttributedString.Key(rawValue: "URL")] {
                    context.sharedContext.openExternalUrl(context: context, urlContext: .generic, url: "https://elloapp.net/edesktop", forceExternal: true, presentationData: context.sharedContext.currentPresentationData.with { $0 }, navigationController: nil, dismissInput: {})
                }
            }
        }
