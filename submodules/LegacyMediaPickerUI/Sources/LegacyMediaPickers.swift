...
public func legacyAssetPickerItemGenerator() -> ((Any?, NSAttributedString?, String?, String?) -> [AnyHashable : Any]?) {
    return { anyDict, caption, hash, uniqueId in
        ...
        let stickers = (dict["stickers"] as? [Data])?.compactMap { data -> FileMediaReference? in
            ...
            if let file = decoder.decodeRootObject() as? ElloAppMediaFile {
    ...
    return Signal { subscriber in
        if let previewImage = UIImage(data: data) {
            ...
            if let thumbnailData = thumbnailImage.jpegData(compressionQuality: 0.4) {
                ...
                previewRepresentations.append(ElloAppMediaImageRepresentation(dimensions: PixelDimensions(thumbnailSize), resource: resource, progressiveSizes: [], immediateThumbnailData: nil))
            }
            ...
            var fileAttributes: [ElloAppMediaFileAttribute] = []
            ...
            let media = ElloAppMediaFile(fileId: MediaId(namespace: Namespaces.Media.LocalFile, id: Int64.random(in: Int64.min ... Int64.max)), partialReference: nil, resource: resource, previewRepresentations: previewRepresentations, videoThumbnails: [], immediateThumbnailData: nil, mimeType: "video/mp4", size: nil, attributes: fileAttributes)
                    ...
                    switch item.item {
                        case let .image(data, thumbnail, caption, stickers):
                            ...
                            if let thumbnail = thumbnail {
                                ...
                                if let thumbnailData = thumbnailImage.jpegData(compressionQuality: 0.4) {
                                    account.postbox.mediaBox.storeResourceData(resource.id, data: thumbnailData)
                                    representations.append(ElloAppMediaImageRepresentation(dimensions: PixelDimensions(thumbnailSize), resource: resource, progressiveSizes: [], immediateThumbnailData: nil))
                                }
                            }
                            switch data {
                                case let .image(image):
                                    ...
                                    if let scaledImage = TGScaleImageToPixelSize(image, scaledSize) {
                                        if let scaledImageData = compressImageToJPEG(scaledImage, quality: 0.6) {
                                            ...
                                            representations.append(ElloAppMediaImageRepresentation(dimensions: PixelDimensions(scaledSize), resource: resource, progressiveSizes: [], immediateThumbnailData: nil))
                                            
                                            var imageFlags: ElloAppMediaImageFlags = []
                                                                                        
                                            var attributes: [MessageAttribute] = []
                                            
                                            var stickerFiles: [ElloAppMediaFile] = []
                                            if !stickers.isEmpty {
                                            ...
                                            let media = ElloAppMediaImage(imageId: MediaId(namespace: Namespaces.Media.LocalImage, id: randomId), representations: representations, immediateThumbnailData: nil, reference: nil, partialReference: nil, flags: imageFlags)
                                ...
                                case let .asset(asset):
                                    ...
                                    let media = ElloAppMediaImage(imageId: MediaId(namespace: Namespaces.Media.LocalImage, id: randomId), representations: representations, immediateThumbnailData: nil, reference: nil, partialReference: nil, flags: [])
                                    var attributes: [MessageAttribute] = []
                                    if let timer = item.timer, timer > 0 && timer <= 60 {
                                        attributes.append(AutoremoveTimeoutMessageAttribute(timeout: Int32(timer), countdownBeginTime: nil))
                                    }
                        ...
                        case let .file(data, thumbnail, mimeType, name, caption):
                            switch data {
                                case let .tempFile(path):
                                    ...
                                    let media = ElloAppMediaFile(fileId: MediaId(namespace: Namespaces.Media.LocalFile, id: randomId), partialReference: nil, resource: resource, previewRepresentations: previewRepresentations, videoThumbnails: [], immediateThumbnailData: nil, mimeType: mimeType, size: nil, attributes: [.FileName(fileName: name)])
                                ...
                                case let .asset(asset):
                                    ...
                                    arc4random_buf(&randomId, 8)
                                    let resource = PhotoLibraryMediaResource(localIdentifier: asset.localIdentifier, uniqueId: Int64.random(in: Int64.min ... Int64.max))
                                    let media = ElloAppMediaFile(fileId: MediaId(namespace: Namespaces.Media.LocalFile, id: randomId), partialReference: nil, resource: resource, previewRepresentations: [], videoThumbnails: [], immediateThumbnailData: nil, mimeType: mimeType, size: nil, attributes: [.FileName(fileName: name)])
                        ...
                        case let .video(data, thumbnail, adjustments, caption, asFile, asAnimation, stickers):
                            ...
                            if let adjustments = adjustments {
                                ...
                                if let dict = adjustments.dictionary(),
