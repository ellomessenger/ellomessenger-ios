...
public func loadServerCountryCodes(accountManager: AccountManager<ElloAppAccountManagerTypes>, engine: ElloAppEngineUnauthorized, completion: @escaping () -> Void) {
...
public func loadServerCountryCodes(accountManager: AccountManager<ElloAppAccountManagerTypes>, engine: ElloAppEngine, completion: @escaping () -> Void) {
    let _ = (engine.localization.getCountriesList(accountManager: accountManager, langCode: nil)
    |> deliverOnMainQueue).start(next: { countries in
        countryCodes = countries
