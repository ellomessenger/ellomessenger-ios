    ...
    |> mapToSignal { state -> Signal<(ChatListFilterPresetControllerState, [EngineRenderedPeer], [EngineRenderedPeer]), NoError> in
        ...
        if missingPeers {
            return context.engine.data.get(
                EngineDataMap(
                    state.additionallyIncludePeers.map { peerId -> ElloAppEngine.EngineData.Item.Peer.RenderedPeer in
                        return ElloAppEngine.EngineData.Item.Peer.RenderedPeer(id: peerId)
                    }
                ),
                EngineDataMap(
                    state.additionallyExcludePeers.map { peerId -> ElloAppEngine.EngineData.Item.Peer.RenderedPeer in
                        return ElloAppEngine.EngineData.Item.Peer.RenderedPeer(id: peerId)
                    }
                )
            )
