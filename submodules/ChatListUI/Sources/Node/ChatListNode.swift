            ...
            if foundAllPeers {
                ...
                return engine.data.get(EngineDataMap(
                    activitiesByPeerId.keys.filter { key in
                        ...
                        return ElloAppEngine.EngineData.Item.Peer.Peer(id: key.peerId)
                    }
                ))
    ...
    private func dequeueTransition() {
        if let (transition, completion) = self.enqueuedTransition {
            ...
            self.transaction(deleteIndices: transition.deleteItems, 
                             ...
                             updateOpaqueState: ChatListOpaqueTransactionState(chatListView: transition.chatListView),
        ...
        switch option {
            case .previous(unread: true), .next(unread: true):
                ...
                |> mapToSignal { index -> Signal<(EngineChatList.Item.Index, EnginePeer)?, NoError> in
                    if let index = index {
                        return engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: index.messageIndex.id.peerId))
            ...
            case let .peerId(peerId):
                let _ = (self.context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: peerId))
                |> deliverOnMainQueue).start(next: { [weak self] peer in
