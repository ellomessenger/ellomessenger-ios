...
public class ChatListItem: ListViewItem, ChatListSearchItemNeighbour {
    ...
    public func nodeConfiguredForParams(async: @escaping (@escaping () -> Void) -> Void, 
...
private let pinIcon = ItemListRevealOptionIcon.image(image: UIImage(named: "Chat List/RevealOption/Pinned")!)
...
private let unpinIcon = ItemListRevealOptionIcon.image(image: UIImage(named: "Chat List/RevealOption/Unpin")!)
...
private let muteIcon = ItemListRevealOptionIcon.image(image: UIImage(named: "Chat List/RevealOption/Mute")!)
...
private let unmuteIcon = ItemListRevealOptionIcon.image(image: UIImage(named: "Chat List/RevealOption/UnMute")!)
...
private let deleteIcon = ItemListRevealOptionIcon.image(image: UIImage(named: "Chat List/RevealOption/Trash")!)
...
private let readIcon = ItemListRevealOptionIcon.image(image: UIImage(named: "Chat List/RevealOption/Read")!)
...
private let unreadIcon = ItemListRevealOptionIcon.image(image: UIImage(named: "Chat List/RevealOption/UnRead")!)
...
private func leftRevealOptions(strings: PresentationStrings, theme: PresentationTheme, isUnread: Bool, isEditing: Bool, isPinned: Bool, isSavedMessages: Bool, groupId: EngineChatList.Group, peer: EnginePeer, filterData: ChatListItemFilterData?) -> [ItemListRevealOption] {
    ...
    var options: [ItemListRevealOption] = []
    if isUnread {
        options.append(ItemListRevealOption(key: RevealOptionKey.toggleMarkedUnread.rawValue, title: strings.DialogList_Read, icon: readIcon, color: theme.list.itemDisclosureActions.accent.fillColor, textColor: theme.list.itemDisclosureActions.neutral1.foregroundColor))
    } else {
...
class ChatListItemNode: ItemListRevealOptionsItemNode {
    ...
    let peerTypeIconNode: ASImageNode
    let adultIconNode: ASImageNode
    ...
    required init() {
        ...
        self.peerTypeIconNode = ASImageNode()
        self.peerTypeIconNode.isLayerBacked = true
        self.peerTypeIconNode.displaysAsynchronously = false
        self.peerTypeIconNode.displayWithoutProcessing = true
        
        self.adultIconNode = ASImageNode()
        self.adultIconNode.isLayerBacked = true
        self.adultIconNode.displaysAsynchronously = false
        self.adultIconNode.displayWithoutProcessing = true
        ...
        self.contextContainer.addSubnode(self.peerTypeIconNode)
        self.contextContainer.addSubnode(self.adultIconNode)
    ...
    func setupItem(item: ChatListItem, synchronousLoads: Bool) {
        ...
        if let peer = peer {
            ...
            self.avatarNode.setPeer(context: item.context, 
                ...
                |> deliverOnMainQueue).start(next: { [weak self] peerView in
                    ...
                    if let cachedPeerData = cachedPeerData as? CachedUserData {
                        if let photo = cachedPeerData.photo, let video = smallestVideoRepresentation(photo.videoRepresentations), let peerReference = PeerReference(peer._asPeer()) {
                            ...
                            let videoFileReference = FileMediaReference.avatarList(peer: peerReference, media: ElloAppMediaFile(fileId: MediaId(namespace: Namespaces.Media.LocalFile, id: 0), partialReference: nil, resource: video.resource, previewRepresentations: photo.representations, videoThumbnails: [], immediateThumbnailData: photo.immediateThumbnailData, mimeType: "video/mp4", size: nil, attributes: [.Animated, .Video(duration: 0, size: video.dimensions, flags: [])]))
        ...
        return { item, params, first, last, firstWithHeader, nextIsPinned in
            ...
            if let messageValue = messages.last {
                for media in messageValue.media {
                    if let media = media as? ElloAppMediaAction, case .historyCleared = media.action {
                        messages = []
                    }
                }
            }
            ...
            var currentPeerTypeIconImage: UIImage?
            var currentAdultIconImage: UIImage?
            ...
            switch contentPeer {
                case let .chat(itemPeer):
                    var (peer, initialHideAuthor, messageText, spoilers, customEmojiRanges) = chatListItemStrings(
                        ...
                        enableMediaEmoji: !enableChatListPhotos,
                        isPeerGroup: isPeerGroup)
            ...
            switch contentData {
                case let .chat(itemPeer, _, _, text, spoilers, customEmojiRanges):
                    ...
                    if inlineAuthorPrefix == nil, let draftState = draftState {
                        ...
                        if !message.text.isEmpty && entities.count > 0 {
                            ...
                            if let spoilers = spoilers {
                                for range in spoilers {
                                    mutableString.addAttribute(NSAttributedString.Key(rawValue: ElloAppTextAttributes.Spoiler), value: true, range: range)
                                }
                            }
                        ...
                        } else if let _ = message.peers[message.id.peerId] as? ElloAppSecretChat {
                            displayMediaPreviews = false
                        }
                            ...
                            for message in messages {
                                ...
                                inner: for media in message.media {
                                    if let image = media as? ElloAppMediaImage {
                                        ...
                                        break inner
                                    } else if let webpage = media as? ElloAppMediaWebpage, case let .Loaded(content) = webpage.content {
                                        let imageTypes = ["photo", "video", "embed", "gif", "document", "elloapp_album"]
            ...
            switch contentData {
                case let .chat(itemPeer, _, _, _, _, _):
                    if let message = messages.last, case let .user(author) = message.author, displayAsMessage {
                        ...
                        if let channel = itemPeer.chatMainPeer?._asPeer() as? ElloAppChannel {
                            if channel.isPrivate {
                                if channel.isGroup {
                                    titleColor = theme.titlePrivateGroupColor
                                } else {
                                    titleColor = theme.titlePrivateChannelColor
                                }
                            }
                        }
                        ...
                        titleAttributedString = NSAttributedString(
                            string: displayTitle,
                            font: titleFont,
                            textColor: item.index.messageIndex.id.peerId.namespace == Namespaces.Peer.SecretChat
                            ? theme.secretTitleColor
                            : titleColor
                        )
                    }
                    else {
                        titleAttributedString = NSAttributedString(string: " ", font: titleFont)
                    }
            ...
            switch contentData {
            case let .chat(itemPeer, _, _, _, _, _):
                if let channel = itemPeer.peer?._asPeer() as? ElloAppChannel {
                    if channel.isPrivate {
                        currentPeerTypeIconImage = generateTintedImage(
                            image: PresentationResourcesChatList.privateChannelIcon(),
                            color: channel.isGroup ? theme.titlePrivateGroupColor : theme.titlePrivateChannelColor,
                            backgroundColor: nil
                        )
                    } else {
                        currentPeerTypeIconImage = PresentationResourcesChatList.peerTypeIcon(payType: channel.payType)
                    }
                    
                    if channel.isAdult {
                        currentAdultIconImage = PresentationResourcesChatList.adultIcon()
                    }
                }
            case .group: break
            }
            ...
            if let currentPeerTypeIconImage = currentPeerTypeIconImage {
                titleIconsWidth += currentPeerTypeIconImage.size.width + 4
            }
            
            if let currentAdultIconImage = currentAdultIconImage {
                titleIconsWidth += currentAdultIconImage.size.width + 4
            }
            ...
            let titleLayoutArguments = TextNodeLayoutArguments(
                attributedString: titleAttributedString,
                backgroundColor: nil,
                maximumNumberOfLines: 1,
                truncationType: .end,
                constrainedSize: CGSize(width: titleRectWidth, height: CGFloat.greatestFiniteMagnitude),
                alignment: .natural,
                cutout: nil,
                insets: UIEdgeInsets())
            let (measureLayout, measureApply) = makeMeasureLayout(titleLayoutArguments)
            ...
            let rawContentRect = CGRect(
                origin: CGPoint(x: 2.0, y: layoutOffset + 8.0),
                size: CGSize(width: rawContentWidth, height: itemHeight - 12.0 - 9.0))
            ...
            // Chats cell size, if change height need change separatorNode position
            ...
            return (layout, { [weak self] synchronousLoads, animated in
                if let strongSelf = self {
                    ...
                    let avatarFrame = CGRect(
                        origin: CGPoint(
                            x: leftInset - avatarLeftInset + editingOffset + 10.0 + revealOffset,
                            y: floor((itemHeight - avatarDiameter) / 2.0)), 
                    ...
                    transition.updateFrame(node: strongSelf.dateNode, frame: CGRect(
                        origin: CGPoint(
                            x: contentRect.origin.x + contentRect.size.width - dateLayout.size.width,
                            y: avatarFrame.minY + (titleLayout.size.height - dateLayout.size.height) / 2.0),
                        size: dateLayout.size))
                    
                    let statusSize = CGSize(width: 24.0, height: 24.0)
                    strongSelf.statusNode.frame = CGRect(
                        origin: CGPoint(
                            x: contentRect.origin.x + contentRect.size.width - dateLayout.size.width - statusSize.width, 
                            y: avatarFrame.minY + (titleLayout.size.height - statusSize.height) / 2.0),
                        size: statusSize)
                    ...
                    if let currentPeerTypeIconImage = currentPeerTypeIconImage {
                        strongSelf.peerTypeIconNode.image = currentPeerTypeIconImage
                        strongSelf.peerTypeIconNode.isHidden = false
                        transition.updateFrame(
                            node: strongSelf.peerTypeIconNode,
                            frame: CGRect(
                                origin: CGPoint(
                                    x: contentRect.origin.x + titleOffset,
                                    y: avatarFrame.minY + (titleLayout.size.height - currentPeerTypeIconImage.size.height) / 2.0
                                ),
                                size: currentPeerTypeIconImage.size
                            )
                        )
                        titleOffset += currentPeerTypeIconImage.size.width + 4.0
                    } else {
                        strongSelf.peerTypeIconNode.image = nil
                        strongSelf.peerTypeIconNode.isHidden = true
                    }
                    
                    if let currentAdultIconImage = currentAdultIconImage {
                        strongSelf.adultIconNode.image = currentAdultIconImage
                        strongSelf.adultIconNode.isHidden = false
                        transition.updateFrame(
                            node: strongSelf.adultIconNode,
                            frame: CGRect(
                                origin: CGPoint(
                                    x: contentRect.origin.x + titleOffset,
                                    y: avatarFrame.minY + (titleLayout.size.height - currentAdultIconImage.size.height) / 2.0
                                ),
                                size: currentAdultIconImage.size
                            )
                        )
                        titleOffset += currentAdultIconImage.size.width + 4.0
                    } else {
                        strongSelf.adultIconNode.image = nil
                        strongSelf.adultIconNode.isHidden = true
                    }
                    ...
                    let titleFrame = CGRect(
                        origin: CGPoint(x: contentRect.origin.x + titleOffset, y: avatarFrame.minY),
                    ...
                    let textNodeFrame = CGRect(
                        origin: CGPoint(
                            ...
                            y: titleFrame.maxY + UIScreenPixel + (authorLayout.size.height.isZero ? 0.0 : (authorLayout.size.height - 3.0))),
                    ...
                    if let currentMutedIconImage = currentMutedIconImage {
                        ...
                        transition.updateFrame(node: strongSelf.mutedIconNode, frame: CGRect(origin: CGPoint(x: nextTitleIconOrigin, y: floorToScreenPixels(titleFrame.midY - currentMutedIconImage.size.height / 2.0) - UIScreenPixel), size: currentMutedIconImage.size))
...
//                self.contextContainer.insertSubnode(videoNode, aboveSubnode: self.avatarNode)
    ...
    override func updateRevealOffset(offset: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        if let item = self.item, let params = self.layoutParams?.5, let currentItemHeight = self.currentItemHeight, let countersSize = self.layoutParams?.6 {
            ...
            if !peerTypeIconNode.isHidden {
                let peerTypeIconFrame = peerTypeIconNode.frame
                transition.updateFrameAdditive(
                    node: peerTypeIconNode,
                    frame: CGRect(
                        origin: CGPoint(
                            x: contentRect.minX + titleOffset,
                            y: peerTypeIconFrame.minY),
                        size: peerTypeIconFrame.size)
                )
                titleOffset += peerTypeIconFrame.size.width + 4.0
            }
            
            if !adultIconNode.isHidden {
                let adultIconFrame = adultIconNode.frame
                transition.updateFrameAdditive(
                    node: adultIconNode,
                    frame: CGRect(
                        origin: CGPoint(
                            x: contentRect.minX + titleOffset,
                            y: adultIconFrame.minY),
                        size: adultIconFrame.size)
                )
                titleOffset += adultIconFrame.size.width + 4.0
            }
            ...
            transition.updateFrameAdditive(node: self.mutedIconNode, frame: CGRect(origin: CGPoint(x: nextTitleIconOrigin, y: mutedIconFrame.minY), size: mutedIconFrame.size))
                ...
                case RevealOptionKey.delete.rawValue:
                    ...
                    if case let .peer(messages, _, _, _, _, _, _, _, _, _, _, _, _) = item.content, let message = messages.first {
                        for media in message.media {
                            if let action = media as? ElloAppMediaAction, action.action == .peerJoined {
                                joined = true
                            }
                        }
                    }
