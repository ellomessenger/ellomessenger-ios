...
private final class InfoPageNode: ASDisplayNode {
    ...
    func asyncLayout() -> (_ theme: PresentationTheme, _ strings: PresentationStrings, _ width: CGFloat, _ index: Int) -> (CGFloat, () -> Void) {
        ...
        return { [weak self] theme, strings, width, index in
            ...
            let imageSize = CGSize(width: 180.0, height: 160.0)
            self?.animationNode.frame = CGRect(origin: CGPoint(x: UIScreen.main.bounds.width/2 - imageSize.width/2, y: textSpacing), size: imageSize)
            ...
            return (contentHeight, {
                ...
                if strongSelf.theme !== theme {
                    ...
                    switch index {
                    case 0:
                        strongSelf.animationNode.setAnimation(name: "YourArchive")
                        strongSelf.animationNode.loop()
                    case 1:
                        strongSelf.animationNode.setAnimation(name: "Muted Chats")
                        strongSelf.animationNode.loop()
                    default:
                        strongSelf.animationNode.setAnimation(name: "Pinned Chats")
                        strongSelf.animationNode.loop()
                    }
                }
                ...
                let titleFrame = CGRect(origin: CGPoint(x: floor((width - titleLayout.size.width) / 2.0), y: topContentInset + imageSize.height/2), size: titleLayout.size)
...
class ChatListArchiveInfoItemNode: ListViewItemNode, UIScrollViewDelegate {
    ...
    func asyncLayout() -> (_ item: ChatListArchiveInfoItem, _ params: ListViewItemLayoutParams, _ isLast: Bool) -> (ListViewItemNodeLayout, () -> Void) {
        ...
        return { item, params, last in
            ...
            let layout = ListViewItemNodeLayout(contentSize: CGSize(width: params.width, height: maxHeight + 160/2), insets: UIEdgeInsets())
