            ...
            case let .peer(_, peer, theme, strings, timeFormat, nameSortOrder, nameDisplayOrder):
                ...
                if primaryPeer.id.isReplies {
                    ...
                    if user.flags.contains(.isSupport) && !servicePeer {
                        ...
                        let presence = peer.presence ?? ElloAppUserPresence(status: .none, lastActivity: 0)
...
public enum ChatListSearchEntry: Comparable, Identifiable {
    ...
    public func item(context: AccountContext, 
                     ...
                     toggleAllPaused: @escaping () -> Void) -> ListViewItem {
        switch self {
            ...
            case let .localPeer(peer, associatedPeer, unreadBadge, _, theme, strings, nameSortOrder, nameDisplayOrder, expandType):
                ...
                return ContactsPeerItem(presentationData: ItemListPresentationData(presentationData), 
                                        ...
                                        context: context,
                                        peerMode: .generalSearch,
                                        peer: .peer(peer: primaryPeer, 
                                                    chatPeer: chatPeer),
                                        status: .none,
                                        badge: badge,
                                        enabled: enabled,
                                        selection: .none,
                                        editing: ContactsPeerItemEditing(editable: false,
                                                                         editing: false,
                                                                         revealed: false),
                                        index: nil,
                                        header: header,
            ...
            case let .globalPeer(peer, unreadBadge, _, theme, strings, nameSortOrder, nameDisplayOrder, expandType):
                ...
                if filter.contains(.onlyPrivateChats) {
                    if !(peer.peer is ElloAppUser || peer.peer is ElloAppSecretChat) {
                        enabled = false
                    }
                }
                if filter.contains(.onlyGroups) {
                    if let _ = peer.peer as? ElloAppGroup {
                ...
                if let subscribers = peer.subscribers, subscribers != 0 {
                    ...
                    } else if let channel = peer.peer as? ElloAppChannel, case .broadcast = channel.info {
                        suffixString = ", \(strings.Conversation_StatusSubscribers(subscribers))"
                    } else {
                ...
                return ContactsPeerItem(presentationData: ItemListPresentationData(presentationData), 
                                        ...
                                        context: context,
                                        peerMode: .generalSearch,
                                        peer: .peer(peer: EnginePeer(peer.peer), chatPeer: EnginePeer(peer.peer)), 
                                        status: .addressName(suffixString), 
                                        badge: badge,
                                        enabled: enabled,
                                        selection: .none,
                                        editing: ContactsPeerItemEditing(editable: false, editing: false, revealed: false),
            ...
            case let .message(message, peer, readState, presentationData, _, selected, displayCustomHeader, orderingKey, _, _, allPaused):
                ...
                if isMedia {
                    ...
                    return ChatListItem(presentationData: presentationData, 
                                        ...
                                        selected: false,
                                        header: tagMask == nil ? header : nil,
                                        enableContextActions: false,
                                        hiddenOffset: false,
                                        interaction: interaction)
                }
            case let .addContact(phoneNumber, theme, strings):
                return ContactsAddItem(theme: theme, 
                                       strings: strings,
                                       phoneNumber: phoneNumber,
                                       header: ChatListSearchItemHeader(type: .phoneNumber,
                                                                        theme: theme,
                                                                        strings: strings,
                                                                        actionTitle: nil,
                                                                        action: nil),
...
final class ChatListSearchListPaneNode: ASDisplayNode, ChatListSearchPaneNode {
    ...
    init(context: AccountContext, animationCache: AnimationCache, animationRenderer: MultiAnimationRenderer, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, interaction: ChatListSearchInteraction, key: ChatListSearchPaneKey, peersFilter: ChatListNodePeersFilter, groupId: EngineChatList.Group?, searchQuery: Signal<String?, NoError>, searchOptions: Signal<ChatListSearchOptions?, NoError>, navigationController: NavigationController?) {
        ...
        // TODO: Need update animation sticker
            ...
            |> mapToSignal { entries, recentDownloadItems -> Signal<(inProgressItems: [DownloadItem], doneItems: [RenderedRecentDownloadItem]), NoError> in
                ...
                for entry in entries {
                    ...
                    case let .messageId(id):
                        itemSignals.append(context.engine.data.get(ElloAppEngine.EngineData.Item.Messages.Message(id: id))
        ...
        |> mapToSignal { [weak self] query, options, downloadItems -> Signal<([ChatListSearchEntry], Bool)?, NoError> in
            ...
            if key == .downloads {
                ...
                func messageMatchesTokens(message: EngineMessage, tokens: [ValueBoxKey]) -> Bool {
                    for media in message.media {
                        ...
                        } else if let _ = media as? ElloAppMediaImage {
                ...
                |> map { presentationData, selectionState -> ([ChatListSearchEntry], Bool)? in
                    ...
                    for item in downloadItems.inProgressItems.sorted(by: { $0.priority < $1.priority }) {
                        ...
                        if let group = item.message.peers[message.id.peerId] as? ElloAppGroup, let migrationReference = group.migrationReference {
            ...
            if let query = query, case .chats = key {
                ...
                |> mapToSignal { local, allRecentlySearched -> Signal<([EnginePeer.Id: Optional<EnginePeer.NotificationSettings>], [EnginePeer.Id: Int], [EngineRenderedPeer], Set<EnginePeer.Id>), NoError> in
                    ...
                    return context.engine.data.subscribe(
                        EngineDataMap(
                            peerIds.map { peerId -> ElloAppEngine.EngineData.Item.Peer.NotificationSettings in
                                return ElloAppEngine.EngineData.Item.Peer.NotificationSettings(id: peerId)
                            }
                        ),
                        EngineDataMap(
                            peerIds.map { peerId -> ElloAppEngine.EngineData.Item.Messages.PeerUnreadCount in
                                return ElloAppEngine.EngineData.Item.Messages.PeerUnreadCount(id: peerId)
                            }
                        )
                    )
            ...
            if peersFilter.contains(.doNotSearchMessages) {
                ...
                |> map { result, updatedState -> ChatListSearchMessagesResult in
                    return ChatListSearchMessagesResult(query: finalQuery, 
                                                        messages: result.messages.map({ EngineMessage($0) }).sorted(by: { $0.index > $1.index }),
                                                        readStates: result.readStates.mapValues(EnginePeerReadCounters.init),
            ...
            |> map { accountPeer, foundLocalPeers, foundRemotePeers, foundRemoteMessages, presentationData, searchState, selectionState, resolvedMessage, recentPeers -> ([ChatListSearchEntry], Bool)? in
                ...
                if let message = resolvedMessage {
                    var peer = EngineRenderedPeer(message: message)
                    if let group = message.peers[message.id.peerId] as? ElloAppGroup, let migrationReference = group.migrationReference {
                        if let channelPeer = message.peers[migrationReference.peerId] {
                            peer = EngineRenderedPeer(peer: EnginePeer(channelPeer))
                        }
                    }
                ...
                if let strongSelf = self {
                    ...
                    if emptyResults {
                        ...
                        if let query = transition.query, !query.isEmpty {
                            ...
                            emptyResultsText = strongSelf.presentationData.strings.ChatList_Search_NoResultsQueryDescription
                        } else {
...
private final class ChatListSearchShimmerNode: ASDisplayNode {
    ...
    func update(context: AccountContext, size: CGSize, presentationData: PresentationData, animationCache: AnimationCache, animationRenderer: MultiAnimationRenderer, key: ChatListSearchPaneKey, hasSelection: Bool, transition: ContainedViewLayoutTransition) {
        if self.currentParams?.size != size || self.currentParams?.presentationData !== presentationData || self.currentParams?.key != key {
            ...
            let peer1: EnginePeer = .user(ElloAppUser(id: EnginePeer.Id(namespace: Namespaces.Peer.CloudUser, id: EnginePeer.Id.Id._internalFromInt64Value(0)), accessHash: nil, firstName: "FirstName", lastName: nil, username: nil, phone: nil, photo: [], botInfo: nil, email: nil, country: nil, gender: nil, birthday: nil, restrictionInfo: nil, flags: [], emojiStatus: nil, isBusiness: false, isPublic: false, botDescription: nil))
            ...
            let items = (0 ..< 2).compactMap { _ -> ListViewItem? in
                switch key {
                    case .chats, .downloads:
                        ...
                        return ChatListItem(presentationData: chatListPresentationData, 
                                            context: context,
                                            peerGroupId: .root,
                                            filterData: nil,
                                            index: EngineChatList.Item.Index(pinningIndex: 0,
                                                                             messageIndex: EngineMessage.Index(id: EngineMessage.Id(peerId: peer1.id, 
                                                                                                                                    namespace: 0,
                                                                                                                                    id: 0),
                                                                                                               timestamp: timestamp1)),
                                            content: .peer(messages: [message], 
                    ...
                    case .links:
                        ...
                        media.append(.webpage(ElloAppMediaWebpage(webpageId: EngineMedia.Id(namespace: 0, id: 0), content: .Loaded(ElloAppMediaWebpageLoadedContent(url: "https://elloapp.com", displayUrl: "https://elloapp.com", hash: 0, type: nil, websiteName: "Ello", title: "Ello Ello", text: "Ello", embedUrl: nil, embedType: nil, embedSize: nil, duration: nil, author: nil, image: nil, file: nil, attributes: [], instantPage: nil)))))
                        let message = EngineMessage(
                            stableId: 0,
                            stableVersion: 0,
                            id: EngineMessage.Id(peerId: peer1.id, namespace: 0, id: 0),
                    ...
                    case .files:
                        var media: [EngineMedia] = []
                        media.append(.file(ElloAppMediaFile(fileId: EngineMedia.Id(namespace: 0, id: 0), partialReference: nil, resource: LocalFileMediaResource(fileId: 0), previewRepresentations: [], videoThumbnails: [], immediateThumbnailData: nil, mimeType: "application/text", size: 0, attributes: [.FileName(fileName: "Text.txt")])))
                        let message = EngineMessage(
                            stableId: 0,
                            stableVersion: 0,
                            id: EngineMessage.Id(peerId: peer1.id, namespace: 0, id: 0),
            ...
            for i in 0 ..< items.count {
                items[i].nodeConfiguredForParams(async: { f in f() }, 
                                                 params: ListViewItemLayoutParams(width: size.width,
                                                                                  leftInset: 0.0,
                                                                                  rightInset: 0.0,
                                                                                  availableHeight: 100.0),
                                                 synchronousLoads: false,
                                                 previousItem: i == 0 ? nil : items[i - 1],
                                                 nextItem: (i == items.count - 1) ? nil : items[i + 1],
                ...
                if key == .media {
                    ...
                    while currentY < size.height {
                        ...
                        if let itemNode = itemNodes[sampleIndex] as? ChatListItemNode {
                            ...
                            var isVoice = false
                            if let media = itemNode.currentMedia as? ElloAppMediaFile {
