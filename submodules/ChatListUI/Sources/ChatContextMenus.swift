...
//    return .single([])
    ...
    return combineLatest(
        ...
        context.engine.data.get(
            ElloAppEngine.EngineData.Item.Configuration.ApplicationSpecificPreference(key: ApplicationSpecificPreferencesKeys.chatArchiveSettings)
        )
    )
    |> map { [weak chatListController] unreadChatListPeerIds, chatArchiveSettingsPreference -> [ContextMenuItem] in
        ...
        if !unreadChatListPeerIds.isEmpty {
            items.append(.action(ContextMenuActionItem(text: strings.ChatList_Context_MarkAllAsRead, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/MarkAsRead"), color: theme.contextMenu.badgeFillColor) }, action: { _, f in
        ...
        items.append(.action(ContextMenuActionItem(text: isPinned ? strings.ChatList_Context_HideArchive : strings.ChatList_Context_UnhideArchive, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: isPinned ? "Chat/Context Menu/Unpin": "Chat/Context Menu/Pin"), color: theme.contextMenu.badgeFillColor) }, action: { [weak chatListController] _, f in
...
//    return .single([])
    ...
    return combineLatest(
        context.engine.data.get(ElloAppEngine.EngineData.Item.Messages.ChatListGroup(id: peerId)),
...
//            let isPinned = pinnedItemIds.contains(.peer(peerId))
            ...
            let renderedPeer = context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.RenderedPeer(id: peerId))
            ...
            |> mapToSignal { renderedPeer -> Signal<[ContextMenuItem], NoError> in
                ...
                return context.engine.data.get(
                    ElloAppEngine.EngineData.Item.Peer.IsContact(id: peer.id),
                    ElloAppEngine.EngineData.Item.Peer.NotificationSettings(id: peer.id),
                    ElloAppEngine.EngineData.Item.Messages.PeerReadCounters(id: peer.id)
                )
                |> map { [weak chatListController] isContact, notificationSettings, readCounters -> [ContextMenuItem] in
                    ...
                    if !isSavedMessages, case let .user(peer) = peer, !peer.flags.contains(.isSupport), peer.botInfo == nil && !peer.isDeleted {
                        if !isContact {
                            items.append(.action(ContextMenuActionItem(text: strings.ChatList_Context_AddToContacts, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/AddUser"), color: theme.contextMenu.badgeFillColor) }, action: { _, f in
                    ...
                    if case let .chatList(currentFilter) = source {
                        if let currentFilter = currentFilter, case let .filter(id, title, emoticon, data) = currentFilter {
                            items.append(.action(ContextMenuActionItem(text: strings.ChatList_Context_RemoveFromFolder, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/RemoveFromFolder"), color: theme.contextMenu.badgeFillColor) }, action: { c, _ in
                            ...
                            if hasFolders {
                                items.append(.action(ContextMenuActionItem(text: strings.ChatList_Context_AddToFolder, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Folder"), color: theme.contextMenu.badgeFillColor) }, action: { c, _ in
                                    ...
                                    for filter in filters {
                                        if case let .filter(_, title, _, data) = filter {
                                            ...
                                            updatedItems.append(.action(ContextMenuActionItem(text: title, icon: { theme in
                                                ...
                                                return generateTintedImage(image: UIImage(bundleImageName: imageName), color: theme.contextMenu.badgeFillColor)
                                            }, action: { c, f in
                                                c.dismiss(completion: {
                                    ...
                                    updatedItems.append(.action(ContextMenuActionItem(text: strings.ChatList_Context_Back, icon: { theme in
                                        return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Back"), color: theme.contextMenu.badgeFillColor)
                                    }, action: { c, _ in
                    ...
                    if isUnread {
                        items.append(.action(ContextMenuActionItem(text: strings.ChatList_Context_MarkAsRead, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/MarkAsRead"), color: theme.contextMenu.badgeFillColor) }, action: { _, f in
                            let _ = context.engine.messages.togglePeersUnreadMarkInteractively(peerIds: [peerId], setToValue: nil).start()
                            f(.default)
                        })))
                    } else {
                        items.append(.action(ContextMenuActionItem(text: strings.ChatList_Context_MarkAsUnread, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/MarkAsUnread"), color: theme.contextMenu.badgeFillColor) }, action: { _, f in
                            let _ = context.engine.messages.togglePeersUnreadMarkInteractively(peerIds: [peerId], setToValue: nil).start()
                            f(.default)
                        })))
                    }
                    ...
                    if let group = peerGroup {
                        if archiveEnabled {
                            ...
                            items.append(.action(ContextMenuActionItem(text: isArchived ? strings.ChatList_Context_Unarchive : strings.ChatList_Context_Archive, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: isArchived ? "Chat/Context Menu/Unarchive" : "Chat/Context Menu/Archive"), color: theme.contextMenu.badgeFillColor) }, action: { _, f in
                                if isArchived {
...
//                            items.append(.action(ContextMenuActionItem(text: isPinned ? strings.ChatList_Context_Unpin : strings.ChatList_Context_Pin, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: isPinned ? "Chat/Context Menu/Unpin" : "Chat/Context Menu/Pin"), color: theme.contextMenu.badgeFillColor) }, action: { c, f in
...
//                        }
                        ...
                        if !isSavedMessages {
                            ...
                            items.append(.action(ContextMenuActionItem(text: isMuted ? strings.ChatList_Context_Unmute : strings.ChatList_Context_Mute, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: isMuted ? "Chat/Context Menu/Unmute" : "Chat/Context Menu/Muted"), color: theme.contextMenu.badgeFillColor) }, action: { _, f in
                        ...
                        if case .search = source {
                            if case let .channel(peer) = peer {
                                ...
                                items.append(.action(ContextMenuActionItem(text: text, icon: { theme in generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Add"), color: theme.contextMenu.badgeFillColor) }, action: { _, f in
