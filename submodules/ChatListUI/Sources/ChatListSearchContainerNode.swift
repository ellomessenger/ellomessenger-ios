...
public final class ChatListSearchContainerNode: SearchDisplayControllerContentNode {
    ...
    public init(context: AccountContext, 
                ...
                hasDownloads: Bool,
                initialFilter: ChatListSearchFilter = .chats,
                openPeer originalOpenPeer: @escaping (EnginePeer, EnginePeer?, Bool) -> Void,
                openDisabledPeer: @escaping (EnginePeer) -> Void,
                openRecentPeerOptions: @escaping (EnginePeer) -> Void,
                openMessage originalOpenMessage: @escaping (EnginePeer, EngineMessage.Id, Bool) -> Void,
        ...
        self.paneContainerNode = ChatListSearchPaneContainerNode(context: context, 
                                                                 animationCache: animationCache,
                                                                 ...
                                                                 searchOptions: self.searchOptions.get(),
                                                                 navigationController: navigationController)
    ...
    override public func searchTextUpdated(text: String) {
        ...
        self.suggestedDates.set(.single(suggestDates(for: text, 
            ...
            if let current = self.selectionPanelNode {
                ...
                selectionPanelNode = ChatListSearchMessageSelectionPanelNode(context: self.context, deleteMessages: { [weak self] in
                    ...
                    let _ = (strongSelf.context.engine.data.get(EngineDataMap(
                        messageIds.map { id -> ElloAppEngine.EngineData.Item.Messages.Message in
                            return ElloAppEngine.EngineData.Item.Messages.Message(id: id)
                        }
                    ))
                    ...
                    |> deliverOnMainQueue).start(next: { messages in
                        if let strongSelf = self, !messages.isEmpty {
                            ...
                            for message in messages {
                                if let user = message.author?._asPeer() as? ElloAppUser {
                                    if user.botInfo != nil {
                                        type = .bot
                                    } else {
                                        type = .user
                                    ...
                                    break
                                } else if let channel = message.peers[message.id.peerId] as? ElloAppChannel, case .broadcast = channel.info {
                                    type = .channel
//                                    type = .group
                                    break
                                }
