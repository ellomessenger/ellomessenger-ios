...
private final class ChatListContainerItemNode: ASDisplayNode {
    ...
    init(context: AccountContext, groupId: EngineChatList.Group, filter: ChatListFilter?, previewing: Bool, controlsHistoryPreload: Bool, presentationData: PresentationData, animationCache: AnimationCache, animationRenderer: MultiAnimationRenderer, becameEmpty: @escaping (ChatListFilter?) -> Void, emptyAction: @escaping (ChatListFilter?) -> Void, tipsAction: @escaping () -> Void, recommendedAction: @escaping () -> Void) {
...
final class ChatListContainerNode: ASDisplayNode, UIGestureRecognizerDelegate {
    ...
    init(context: AccountContext, groupId: EngineChatList.Group, previewing: Bool, controlsHistoryPreload: Bool, presentationData: PresentationData, animationCache: AnimationCache, animationRenderer: MultiAnimationRenderer, filterBecameEmpty: @escaping (ChatListFilter?) -> Void, filterEmptyAction: @escaping (ChatListFilter?) -> Void, tipsAction: @escaping () -> Void, recommendedAction: @escaping () -> Void) {
        self.context = context
        ...
        }, tipsAction: { [weak self] in
            self?.tipsAction()
        }, recommendedAction: { [weak self] in
            self?.recommendedAction()
        })
...
final class ChatListControllerNode: ASDisplayNode {
    ...
    init(context: AccountContext, groupId: EngineChatList.Group, filter: ChatListFilter?, previewing: Bool, controlsHistoryPreload: Bool, presentationData: PresentationData, animationCache: AnimationCache, animationRenderer: MultiAnimationRenderer, controller: ChatListControllerImpl) {
        ...
        recomendedAction = { [weak self] in
            self?.context.sharedContext.openFeedController(activateRecomended: true)
        }
