...
private let elloappReleaseDate = Date(timeIntervalSince1970: 1376438400.0)
            ...
            do {
                func process(_ date: Date) {
                    ...
                    if stringComponents.count < 3 {
                        for i in 0..<8 {
                            if let date = calendar.date(byAdding: .year, value: -i, to: resultDate), date < now, date > elloappReleaseDate {
                                let lowerDate = getLowerDate(for: resultDate)
                                result.append((lowerDate, date, nil))
                            }
                        }
                    } else if resultDate < now, date > elloappReleaseDate {
                        let lowerDate = getLowerDate(for: resultDate)
                        result.append((lowerDate, resultDate, nil))
                    }
                }
                let dd = try NSDataDetector(types: NSTextCheckingResult.CheckingType.date.rawValue)
                if let match = dd.firstMatch(in: string, options: [], range: NSMakeRange(0, string.utf16.count)), let date = match.date, date > elloappReleaseDate {
                    process(date)
                } else if let match = dd.firstMatch(in: string.replacingOccurrences(of: ".", with: "/"), options: [], range: NSMakeRange(0, string.utf16.count)), let date = match.date, date > elloappReleaseDate {
                    process(date)
                }
            } catch {
    ...
    for (month, value) in months {
        ...
        if cleanString == shortMonthName || (cleanString.count >= shortMonthName.count && monthName.hasPrefix(cleanString)) {
            if cleanDigits.count == 4, let year = Int(cleanDigits) {
                ...
                if upperDate <= now && upperDate > elloappReleaseDate {
