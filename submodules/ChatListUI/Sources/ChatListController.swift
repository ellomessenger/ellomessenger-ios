...
public class ChatListControllerImpl: ElloAppBaseController, ChatListController {
    ...
    public init(context: AccountContext, groupId: PeerGroupId, filter: ChatListFilter? = nil, controlsHistoryPreload: Bool, hideNetworkActivityStatus: Bool = false, previewing: Bool = false, enableDebugActions: Bool) {
        ...
        if !previewing {
            if self.groupId == .root && self.filter == nil {
                self.tabBarItem.title = PresentationTheme.isTabbarTitleShown ? self.presentationData.strings.DialogList_Title : ""
...
//                }
                ...
                let addBarButtonItem = UIBarButtonItem(
                    image: PresentationResourcesRootController.navigationAddIcon(self.presentationData.theme),
                    style: .plain,
                    target: self,
                    action: #selector(self.addButtonPressed))
                addBarButtonItem.accessibilityLabel = self.presentationData.strings.VoiceOver_Navigation_Compose
                
                let quickLinksButtonItem = UIBarButtonItem(
                    image: PresentationResourcesRootController.navigationQuickLinksIcon(self.presentationData.theme),
                    style: .plain,
                    target: self,
                    action: #selector(self.quickLinksButtonPressed))
                
                let aiBotRightBarButtonItem = UIBarButtonItem(
                    image: PresentationResourcesRootController.navigationAIBotIcon(self.presentationData.theme),
                    style: .plain,
                    target: self,
                    action: #selector(self.aiBotRightButtonPressed))
                
                self.navigationItem.rightBarButtonItems = [quickLinksButtonItem, aiBotRightBarButtonItem, addBarButtonItem]
...
//                self.navigationItem.rightBarButtonItem = rightBarButtonItem
        ...
        if filter != nil || groupId != .root {
            ...
            peerStatus = context.engine.data.subscribe(ElloAppEngine.EngineData.Item.Peer.Peer(id: context.account.peerId))
                    ...
                    if stateAndFilterId.state.editing {
                        ...
                        if case .root = strongSelf.groupId {
                            ...
                            if isReorderingTabs {
                                ...
                                let addBarButtonItem = UIBarButtonItem(
                                    image: PresentationResourcesRootController.navigationAddIcon(strongSelf.presentationData.theme),
                                    style: .plain,
                                    target: strongSelf,
                                    action: #selector(strongSelf.addButtonPressed)
                                )
                                
                                let quickLinksButtonItem = UIBarButtonItem(
                                    image: PresentationResourcesRootController.navigationQuickLinksIcon(strongSelf.presentationData.theme),
                                    style: .plain,
                                    target: self,
                                    action: #selector(strongSelf.quickLinksButtonPressed))
                                
                                let aiBotRightBarButtonItem = UIBarButtonItem(
                                    image: PresentationResourcesRootController.navigationAIBotIcon(strongSelf.presentationData.theme),
                                    style: .plain,
                                    target: strongSelf,
                                    action: #selector(strongSelf.aiBotRightButtonPressed)
                                )
                                
                                addBarButtonItem.accessibilityLabel = strongSelf.presentationData.strings.VoiceOver_Navigation_Compose
                                if !(strongSelf.navigationItem.rightBarButtonItems?.contains(where: { $0.accessibilityLabel == addBarButtonItem.accessibilityLabel }) ?? false) {
                                    strongSelf.navigationItem.setRightBarButtonItems([quickLinksButtonItem, aiBotRightBarButtonItem, addBarButtonItem], animated: true)
                                }
                            }
...
//                            strongSelf.navigationItem.setRightBarButton(editItem, animated: true)
                        ...
                        switch networkState {
                            ...
                            case .updating:
                                strongSelf.titleView.title = NetworkStatusTitle(text: strongSelf.presentationData.strings.State_Updating, 
                                                                                activity: true,
                                                                                hasProxy: isRoot && hasProxy,
                                                                                connectsViaProxy: connectsViaProxy,
                                                                                isPasscodeSet: isRoot && isPasscodeSet,
                                                                                isManuallyLocked: isRoot && isManuallyLocked,
                                                                                peerStatus: peerStatus)
                            case .online:
                                strongSelf.titleView.setTitle(NetworkStatusTitle(text: defaultTitle, 
                                                                                 activity: false,
                                                                                 hasProxy: isRoot && hasProxy,
                                                                                 connectsViaProxy: connectsViaProxy,
                                                                                 isPasscodeSet: isRoot && isPasscodeSet,
                                                                                 isManuallyLocked: isRoot && isManuallyLocked,
                                                                                 peerStatus: peerStatus),
                                                              animated: (previousEditingAndNetworkState?.0 ?? false) != stateAndFilterId.state.editing)
                        }
        ...
        if !previewing {
            ...
            let entriesWithFetchStatuses = Signal<[(entry: FetchManagerEntrySummary, progress: Double)], NoError> { subscriber in
                ...
                final class StateHolder {
                    ...
                    func update(engine: ElloAppEngine, entries: [FetchManagerEntrySummary]) {
                        if entries.isEmpty {
...
//        }
    ...
    override public func loadDisplayNode() {
            ...
            if let strongSelf = self {
                if let navigationController = strongSelf.navigationController as? NavigationController {
                    ...
                    strongSelf.context.sharedContext.navigateToChatController(NavigateToChatControllerParams(
                                                ...
                                                chatListFilter: strongSelf.chatListDisplayNode.containerNode.currentItemNode.chatListFilter?.id,
        ...
        self.chatListDisplayNode.requestAddContact = { [weak self] userName in
            if let strongSelf = self {
        ...
        self.chatListDisplayNode.tipsAction = { [weak self] in
            ...
            _ = (self.context.engine.peers.resolvePeerByName(name: self.presentationData.strings.ChatList_EmptyChatListPeerTips)
        ...
        self.chatListDisplayNode.peerContextAction = { [weak self] peer, source, node, gesture, location in
            ...
            if let peer = peer._asPeer() as? ElloAppChannel {
                // Show paid channel preview only for members
                if peer.isFree || peer.participationStatus == .member {
                    strongSelf.presentInGlobalOverlay(contextController)
                }
            } else {
                strongSelf.presentInGlobalOverlay(contextController)
...
//        }
        ...
        if !self.didSuggestLocalization {
            ...
            let suggestedLocalization = self.context.engine.data.get(ElloAppEngine.EngineData.Item.Configuration.SuggestedLocalization())
            ...
            Queue.mainQueue().after(1.0, {
                let _ = (
                    self.context.engine.data.get(ElloAppEngine.EngineData.Item.Notices.Notice(key: ApplicationSpecificNotice.forcedPasswordSetupKey()))
                    |> map { entry -> Int32? in
    ...
    private func reloadFilters(firstUpdate: (() -> Void)? = nil) {
        ...
        self.filterDisposable.set((combineLatest(queue: .mainQueue(),
            ...
            self.context.engine.data.get(ElloAppEngine.EngineData.Item.Configuration.UserLimits(isPremium: false))
        )
...
//            }
    ...
    @objc private func composePressed() {
        ...
        if !hasComposeController {
//            let controller = self.context.sharedContext.makeComposeController(context: self.context)
            let controller = inviteLinkListController(context: self.context, peerId: context.account.peerId, admin: nil)
            navigationController.pushViewController(controller)
        }
    }
    
    @objc private func quickLinksButtonPressed() {
        guard let chatListQuickLinksViewController = ChatListQuickLinksViewController.controller(with: getAppBundle()) else {return }
        chatListQuickLinksViewController.onItemTap = {[weak self] item in
            guard let self else { return }
            switch item {
            case .tips:
                self.openUrl(url: "https://ello.team/ellotips", concealed: false, external: false)
            case .earnInstant:
                self.navigationController?.pushViewController(ELProfileController.layalityViewController(accountContext: context), animated: true)
            case .contacts:
                self.navigationController?.popViewController(animated: true)
                context.sharedContext.openContactsController()
            case .broadcast:
                addButtonPressed()
            case .elloPay:
                ELProfileController.walletsController(network: context.account.network) {[weak self] controller in
        ...
        chatListQuickLinksViewController.onTapBack = {[weak self]in
            self?.navigationController?.popViewController(animated: true)
        }
        
        let adopt = AdaptingController(viewController: chatListQuickLinksViewController)
        self.navigationController?.pushViewController(adopt, animated: true)
    }
    
    @objc private func aiBotRightButtonPressed() {
        showAiSpace()
    }
        
    private func showAiSpace() {
        if let navigationController = self.navigationController as? NavigationController {
            ...
            if let vc = AIUniverseViewController.controller {
                ...
                vc.onSelectedBot = {[weak self] botPeerId in
                    guard let self else { return }
//                    let peerId = PeerId.aiBotPeerID
//                    _ = (self.context.engine.peers.fetchAndUpdateCachedBotData(peerId: peerId) |> deliverOnMainQueue
...
//                    }).start()
                    ...
                    _ = (combineLatest(
                        context.engine.peers.fetchAndUpdateCachedBotData(peerId: botPeerId),
                        context.engine.peers.fetchAndUpdateCachedPeerData(peerId: context.account.peerId))
                         |> take(1)
                         |> deliverOnMainQueue).start(next: {[weak navigationController, weak context] _ in
                        guard let navigationController = navigationController, let context else {
                            return
                        }
                        ...
                        context.sharedContext.navigateToChatController(NavigateToChatControllerParams(
                            navigationController: navigationController,
                            context: context,
                            chatLocation: .peer(id: botPeerId)))
                    })
                }
    ...
    private func openUrl(url: String, concealed: Bool, external: Bool) {
        self.context.sharedContext.openResolvedUrl(.externalUrl(url),
                                                context: self.context,
                                                urlContext: .generic,
            ...
            if let navigationController = self.navigationController as? NavigationController {
                self.context.sharedContext.navigateToChatController(NavigateToChatControllerParams(navigationController: navigationController, context: self.context, chatLocation: .peer(id: peerId), keepStack: .always))
                                                ...
                                                present: {[weak self] c, a in self?.present(c, in: .window(.root), with: a) },
                                                dismissInput: {[weak self]in self?.view.endEditing(true) },
                                                contentContext: nil)
    }
    ...
    func deletePeerChat(peerId: PeerId, joined: Bool) {
        let _ = (self.context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.RenderedPeer(id: peerId))
    ...
    private func schedulePeerChatRemoval(peer: EngineRenderedPeer, 
                                         type: InteractiveMessagesDeletionType,
                                         deleteGloballyIfPossible: Bool,
