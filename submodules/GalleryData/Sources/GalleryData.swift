...
public enum ChatMessageGalleryControllerData {
    case url(String)
    case pass(ElloAppMediaFile)
    case instantPage(InstantPageGalleryController, Int, Media)
    case map(ElloAppMediaMap)
    case stickerPack(StickerPackReference)
    case audio(ElloAppMediaFile)
    case document(ElloAppMediaFile, Bool)
...
public func chatMessageGalleryControllerData(context: AccountContext, chatLocation: ChatLocation?, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>?, message: Message, navigationController: NavigationController?, standalone: Bool, reverseMessageGalleryOrder: Bool, mode: ChatControllerInteractionOpenMessageMode, source: GalleryControllerItemSource?, synchronousLoad: Bool, actionInteraction: GalleryControllerActionInteraction?) -> ChatMessageGalleryControllerData? {
    ...
    var instantPageMedia: (ElloAppMediaWebpage, [InstantPageGalleryEntry])?
    if message.media.isEmpty, let entities = message.textEntitiesAttribute?.entities, entities.count == 1, let firstEntity = entities.first, case let .CustomEmoji(_, fileId) = firstEntity.type, let file = message.associatedMedia[MediaId(namespace: Namespaces.Media.CloudFile, id: fileId)] as? ElloAppMediaFile {
    ...
    for media in message.media {
        if let invoice = media as? ElloAppMediaInvoice, let extendedMedia = invoice.extendedMedia, case let .full(fullMedia) = extendedMedia {
            ...
            if let file = content.file {
                ...
                } else if ["photo", "document", "video", "gif", "elloapp_album"].contains(content.type) {
                    galleryMedia = image
                }
            }
        ...
        } else if let mapMedia = media as? ElloAppMediaMap {
            galleryMedia = mapMedia
        } else if let contactMedia = media as? ElloAppMediaContact {
    ...
    if let (webPage, instantPageMedia) = instantPageMedia, let galleryMedia = galleryMedia {
        ...
        if let mapMedia = galleryMedia as? ElloAppMediaMap {
            return .map(mapMedia)
        } else if let file = galleryMedia as? ElloAppMediaFile, (file.isSticker || file.isAnimatedSticker) {
            for attribute in file.attributes {
        ...
        } else if let file = galleryMedia as? ElloAppMediaFile, file.isAnimatedSticker {
            return nil
        } else if let file = galleryMedia as? ElloAppMediaFile, file.isMusic || file.isVoice || file.isInstantVideo {
            return .audio(file)
        } else if let file = galleryMedia as? ElloAppMediaFile, file.mimeType == "application/vnd.apple.pkpass" || (file.fileName != nil && file.fileName!.lowercased().hasSuffix(".pkpass")) {
            return .pass(file)
        } else {
            if let file = galleryMedia as? ElloAppMediaFile {
