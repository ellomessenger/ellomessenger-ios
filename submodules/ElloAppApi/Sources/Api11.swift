...
public extension Api {
    ...
    public func serialize(_ buffer: Buffer, _ boxed: Swift.Bool) {
        switch self {
        case .message(let flags, let id, let fromId, let peerId, let fwdFrom, let viaBotId, let replyTo, let date, let message, let media, let replyMarkup, let entities, let views, let forwards, let replies, let editDate, let postAuthor, let groupedId, let reactions, let restrictionReason, let ttlPeriod, _, _, _, _, _, _, _, _):
            if boxed {
            ...
            if Int(flags) & Int(1 << 28) != 0 {serializeInt32(forwards!, buffer: buffer, boxed: false)}
    ...
    public func descriptionFields() -> (String, [(String, Any)]) {
        switch self {
                case .message(let flags, let id, let fromId, let peerId, let fwdFrom, let viaBotId, let replyTo, let date, let message, let media, let replyMarkup, let entities, let views, let forwards, let replies, let editDate, let postAuthor, let groupedId, let reactions, let restrictionReason, let ttlPeriod, let iaMediaSales, let title, let price, let quantity, let isPaid, let medias, let likes, let isLiked):
            return ("message", [("flags", String(describing: flags)), ("id", String(describing: id)), ("fromId", String(describing: fromId)), ("peerId", String(describing: peerId)), ("fwdFrom", String(describing: fwdFrom)), ("viaBotId", String(describing: viaBotId)), ("replyTo", String(describing: replyTo)), ("date", String(describing: date)), ("message", String(describing: message)), ("media", String(describing: media)), ("replyMarkup", String(describing: replyMarkup)), ("entities", String(describing: entities)), ("views", String(describing: views)), ("forwards", String(describing: forwards)), ("replies", String(describing: replies)), ("editDate", String(describing: editDate)), ("postAuthor", String(describing: postAuthor)), ("groupedId", String(describing: groupedId)), ("reactions", String(describing: reactions)), ("restrictionReason", String(describing: restrictionReason)), ("ttlPeriod", String(describing: ttlPeriod)), ("iaMediaSales", String(describing: iaMediaSales)), ("title", String(describing: title)), ("price", String(describing: price)), ("quantity", String(describing: quantity)), ("isPaid", String(describing: isPaid)), ("medias", String(describing: medias)), ("likes", String(describing: likes)), ("isLiked", String(describing: isLiked))])
                case .messageEmpty(let flags, let id, let peerId):
        ...
        public static func parse_message(_ reader: BufferReader) -> Message? {
            ...
            var isMediaSale = Api.Bool.boolFalse
            var title: String?
            var price: Double?
            var quantity: Int32?
            var isPaid: Bool?
            var medias: [Api.MessageMedia] = []
            if (Int(_1!) & (1 << 12)) != 0 {
                isMediaSale = .boolTrue
                title = parseString(reader)
                price = reader.readDouble()
                quantity = reader.readInt32()
                if let signature = reader.readInt32() {
                    isPaid = Api.parse(reader, signature: signature) as? Api.Bool
                    debugPrint("isPayed \(String(describing: isPaid))")
                }
                debugPrint("isMediaSale \(isMediaSale)")
                debugPrint(String(describing: title))
                debugPrint(String(describing: price))
                debugPrint(String(describing: quantity))
                let magic = reader.readInt32()
                if magic == 0x1cb5c415 {
                    let count = reader.readInt32() ?? 0
                    for _ in 0..<count {
                        if let signature = reader.readInt32() {
                            guard let object = Api.parse(reader, signature: signature) as? Api.MessageMedia else {
                                continue
                            }
                            medias.append(object)
                        }
                    }
                }
            }
            ...
            var _14: Int32?
            if Int(_1!) & Int(1 << 28) != 0 {_14 = reader.readInt32() }
            ...
            var likes: Int32?
            if Int(_1!) & Int(1 << 29) != 0 { likes = reader.readInt32() }
            let isLiked = Int(_1!) & (1 << 30) != 0 ? Api.Bool.boolTrue : Api.Bool.boolFalse
            ...
            let _c14 = (Int(_1!) & Int(1 << 28) == 0) || _14 != nil
