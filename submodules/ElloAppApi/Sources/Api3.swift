...
public extension Api {
    ...
    switch self {
                ...
                case .channelParticipantAdmin(let flags, let userId, let inviterId, let promotedBy, let date, let adminRights, let rank):
                    ...
                    serializeInt32(flags, buffer: buffer, boxed: false)
                    serializeInt64(userId, buffer: buffer, boxed: false)
                    if Int(flags) & Int(1 << 3) != 0 {serializeInt64(inviterId!, buffer: buffer, boxed: false)}
    ...
    public func descriptionFields() -> (String, [(String, Any)]) {
        ...
        public static func parse_channelParticipantAdmin(_ reader: BufferReader) -> ChannelParticipant? {
            ...
            _2 = reader.readInt64()
            var _3: Int64?
            if Int(_1!) & Int(1 << 3) != 0 {_3 = reader.readInt64() }
            var _4: Int64?
            _4 = reader.readInt64()
            var _5: Int32?
            _5 = reader.readInt32()
...
public extension Api {
    indirect enum Chat: TypeConstructorDescription {
        case channel(flags: Int32, id: Int64, accessHash: Int64?, title: String, username: String?, photo: Api.ChatPhoto, date: Int32, restrictionReason: [Api.RestrictionReason]?, adminRights: Api.ChatAdminRights?, bannedRights: Api.ChatBannedRights?, defaultBannedRights: Api.ChatBannedRights?, participantsCount: Int32?, payType: Int32?, cost: Double?, category: String?, country: String?, isAdult: Api.Bool, startDate: Int64?, endDate: Int64?)
        case channelForbidden(flags: Int32, id: Int64, accessHash: Int64, title: String, untilDate: Int32?)
        case chat(flags: Int32, id: Int64, title: String, userName: String?, photo: Api.ChatPhoto, participantsCount: Int32, date: Int32, version: Int32, migratedTo: Api.InputChannel?, adminRights: Api.ChatAdminRights?, defaultBannedRights: Api.ChatBannedRights?)
        case chatEmpty(id: Int64)
        case chatForbidden(id: Int64, title: String)
        
        public func serialize(_ buffer: Buffer, _ boxed: Swift.Bool) {
            switch self {
            case .channel(let flags, let id, let accessHash, let title, let username, let photo, let date, let restrictionReason, let adminRights, let bannedRights, let defaultBannedRights, let participantsCount, let payType, let cost, let category, let country, let isAdult, let startDate, let endDate):
                if boxed {
                ...
                if Int(flags) & Int(1 << 30) != 0 {
                    if let payType = payType {
                        serializeInt32(payType, buffer: buffer, boxed: false)
                    }
                    if let cost = cost {
                        serializeDouble(cost, buffer: buffer, boxed: false)
                    }
                }
                if Int(flags) & Int(1 << 10) != 0 {
                    if let category = category {
                        serializeString(category, buffer: buffer, boxed: false)
                    }
                ...
                if Int(flags) & Int(1 << 3) != 0 {
                    if let startDate { serializeInt64(startDate, buffer: buffer, boxed: false) }
                    if let endDate { serializeInt64(endDate, buffer: buffer, boxed: false) }
                }
                if (Int(flags) & Int(1 << 16) != 0) {
                    isAdult.serialize(buffer, true)
                }
                break
            case .channelForbidden(let flags, let id, let accessHash, let title, let untilDate):
                ...
                break
            case .chat(let flags, let id, let title, let userName, let photo, let participantsCount, let date, let version, let migratedTo, let adminRights, let defaultBannedRights):
                if boxed {
                ...
                if Int(flags) & Int(1 << 26) != 0 { serializeString(userName!, buffer: buffer, boxed: false) }
                if Int(flags) & Int(1 << 6) != 0 {migratedTo!.serialize(buffer, true)}
                if Int(flags) & Int(1 << 14) != 0 {adminRights!.serialize(buffer, true)}
                if Int(flags) & Int(1 << 18) != 0 {defaultBannedRights!.serialize(buffer, true)}
                if Int(flags) & Int(1 << 4) != 0 {Api.Bool.boolTrue.serialize(buffer, true)}
                break
        ...
        public func descriptionFields() -> (String, [(String, Any)]) {
            ...
            case .channel(let flags, let id, let accessHash, let title, let username, let photo, let date, let restrictionReason, let adminRights, let bannedRights, let defaultBannedRights, let participantsCount, let payType, let cost, let category, let country, let isAdult, let startDate, let endDate):
                return ("channel", [
                    ...
                    ("accessHash", String(describing: accessHash)),
                    ("title", String(describing: title)),
                    ("username", String(describing: username)),
                    ("photo", String(describing: photo)),
                    ("date", String(describing: date)),
                    ("restrictionReason", String(describing: restrictionReason)),
                    ("adminRights", String(describing: adminRights)),
                    ("bannedRights", String(describing: bannedRights)),
                    ("defaultBannedRights", String(describing: defaultBannedRights)),
                    ("participantsCount", String(describing: participantsCount)),
                    ("payType", String(describing: payType)),
                    ("cost", String(describing: cost)),
                    ("category", String(describing: category)),
                    ("country", String(describing: country)),
                    ("isAdult", String(describing: isAdult)),
            ...
            case .chat(let flags, let id, let title, let userName, let photo, let participantsCount, let date, let version, let migratedTo, let adminRights, let defaultBannedRights):
                return ("chat", [("flags", String(describing: flags)), ("id", String(describing: id)), ("title", String(describing: title)), ("userName", String(describing: userName)), ("photo", String(describing: photo)), ("participantsCount", String(describing: participantsCount)), ("date", String(describing: date)), ("version", String(describing: version)), ("migratedTo", String(describing: migratedTo)), ("adminRights", String(describing: adminRights)), ("defaultBannedRights", String(describing: defaultBannedRights))])
            case .chatEmpty(let id):
        ...
        public static func parse_channel(_ reader: BufferReader) -> Chat? {
            ...
            _1 = reader.readInt32()
            if Int(_1!) & (1 << 4) != 0 {
                debugPrint("showHistory")
            }
            
            let flags2 = reader.readInt32()
            if Int(flags2!) & (1 << 2) != 0 {
                debugPrint("hideFromChatList")
            }
            ...
            if Int(_1!) & Int(1 << 30) != 0 {
                payType = reader.readInt32()
                cost = reader.readDouble()
            }
            var category: String?
            var country: String?
            if Int(_1!) & Int(1 << 10) != 0 {
                category = parseString(reader)
                country = parseString(reader)
            }
            ...
            let isAdult: Api.Bool = (Int(_1!) & Int(1 << 16) != 0) ? .boolTrue : .boolFalse
            if Int(flags2!) & Int(1 << 11) != 0 {
                _ = parseString(reader) // Genre
            }
            if Int(flags2!) & Int(1 << 12) != 0 {
                _ = parseString(reader) // Subgenre
            }
            if Int(flags2!) & Int(1 << 13) != 0 {
                _ = parseString(reader) // Channel name
            }
            
            _ = Int(flags2!) & Int(1 << 14) != 0 // isRecomended
            ...
            if _c1 && _c2 && _c3 && _c4 && _c5 && _c6 && _c7 && _c8 && _c9 && _c10 && _c11 && _c12 {
                return Api.Chat.channel(flags: _1!, id: _2!, accessHash: _3, title: _4!, username: _5, photo: _6!, date: _7!, restrictionReason: _8, adminRights: _9, bannedRights: _10, defaultBannedRights: _11, participantsCount: _12, payType: payType, cost: cost, category: category, country: country, isAdult: isAdult, startDate: startDate, endDate: endDate)
            }
            else {
                return nil
            }
        ...
        public static func parse_chat(_ reader: BufferReader) -> Chat? {
            ...
            if _c1 && _c2 && _c3 && _c4 && _c5 && _c6 && _c7 && _c8 && _c9 && _c10 {
                return Api.Chat.chat(flags: _1!, id: _2!, title: _3!, userName: userName, photo: _4!, participantsCount: _5!, date: _6!, version: _7!, migratedTo: _8, adminRights: _9, defaultBannedRights: _10)
            }
            else {
                return nil
            }
...
public extension Api {
    ...
    public func serialize(_ buffer: Buffer, _ boxed: Swift.Bool) {
    switch self {
                case .channelFull(let flags, let flags2, let id, let about, let participantsCount, let adminsCount, let kickedCount, let bannedCount, let onlineCount, let readInboxMaxId, let readOutboxMaxId, let unreadCount, let chatPhoto, let notifySettings, let exportedInvite, let botInfo, _, _, let pinnedMsgId, let stickerset, let availableMinId, let folderId, let linkedChatId, let location, let slowmodeSeconds, let slowmodeNextSendDate, let statsDc, let pts, let call, let ttlPeriod, let pendingSuggestions, let groupcallDefaultJoinAs, let themeEmoticon, let requestsPending, let recentRequesters, let defaultSendAs, let availableReactions, let payType, let cost):
                    if boxed {
...
//                    if Int(flags) & Int(1 << 4) != 0 {serializeInt32(migratedFromMaxId!, buffer: buffer, boxed: false)}
                    if Int(flags) & Int(1 << 4) != 0 {serializeInt32(payType!, buffer: buffer, boxed: false)}
                    if Int(flags) & Int(1 << 4) != 0 {serializeDouble(cost!, buffer: buffer, boxed: false)}
    ...
    public func descriptionFields() -> (String, [(String, Any)]) {
        ...
        public static func parse_channelFull(_ reader: BufferReader) -> ChatFull? {
            ...
            let category = parseString(reader)
            let country = parseString(reader)
            print(category as Any, country as Any)
...
//            if Int(_1!) & Int(1 << 4) != 0 {_18 = reader.readInt32() }
        ...
        public static func parse_chatFull(_ reader: BufferReader) -> ChatFull? {
            ...
            let about = parseString(reader)
            var participants: Api.ChatParticipants?
            if let signature = reader.readInt32() {
                participants = Api.parse(reader, signature: signature) as? Api.ChatParticipants
            }
            var chatPhoto: Api.Photo?
            if Int(flags) & Int(1 << 2) != 0, let signature = reader.readInt32() {
                chatPhoto = Api.parse(reader, signature: signature) as? Api.Photo
            }
            var notifySettings: Api.PeerNotifySettings?
            if let signature = reader.readInt32() {
                notifySettings = Api.parse(reader, signature: signature) as? Api.PeerNotifySettings
            }
            var exportedInvite: Api.ExportedChatInvite?
            if Int(flags) & Int(1 << 13) != 0, let signature = reader.readInt32() {
                exportedInvite = Api.parse(reader, signature: signature) as? Api.ExportedChatInvite
            }
            var botInfo: [Api.BotInfo]?
            if Int(flags) & Int(1 << 3) != 0, let _ = reader.readInt32() {
                botInfo = Api.parseVector(reader, elementSignature: 0, elementType: Api.BotInfo.self)
            }
            ...
            var call: Api.InputGroupCall?
            if Int(flags) & Int(1 << 12) != 0, let signature = reader.readInt32() {
                call = Api.parse(reader, signature: signature) as? Api.InputGroupCall
            }
            var ttlPeriod: Int32?
            if Int(flags) & Int(1 << 14) != 0 {
                ttlPeriod = reader.readInt32()
            }
            var groupcallDefaultJoinAs: Api.Peer?
            if Int(flags) & Int(1 << 15) != 0, let signature = reader.readInt32() {
                groupcallDefaultJoinAs = Api.parse(reader, signature: signature) as? Api.Peer
            }
            var themeEmoticon: String?
            if Int(flags) & Int(1 << 16) != 0 {
                themeEmoticon = parseString(reader)
            }
            var requestsPending: Int32?
            if Int(flags) & Int(1 << 17) != 0 {
                requestsPending = reader.readInt32()
            }
            var recentRequesters: [Int64]?
            if Int(flags) & Int(1 << 17) != 0, let _ = reader.readInt32() {
                recentRequesters = Api.parseVector(reader, elementSignature: 570911930, elementType: Int64.self)
            }
            var availableReactions: Api.ChatReactions?
            if Int(flags) & Int(1 << 18) != 0, let signature = reader.readInt32() {
                availableReactions = Api.parse(reader, signature: signature) as? Api.ChatReactions
            }
            var userName: String?
            if Int(flags) & Int(1 << 10) != 0 {
                userName = parseString(reader)
            ...
            let _c2 = id != nil
            let _c3 = about != nil
            let _c4 = participants != nil
            let _c5 = (Int(flags) & Int(1 << 2) == 0) || chatPhoto != nil
            let _c6 = notifySettings != nil
            let _c7 = (Int(flags) & Int(1 << 13) == 0) || exportedInvite != nil
            let _c8 = (Int(flags) & Int(1 << 3) == 0) || botInfo != nil
            let _c9 = (Int(flags) & Int(1 << 6) == 0) || pinnedMsgId != nil
            let _c10 = (Int(flags) & Int(1 << 11) == 0) || folderId != nil
            let _c11 = (Int(flags) & Int(1 << 12) == 0) || call != nil
            let _c12 = (Int(flags) & Int(1 << 14) == 0) || ttlPeriod != nil
            let _c13 = (Int(flags) & Int(1 << 15) == 0) || groupcallDefaultJoinAs != nil
            let _c14 = (Int(flags) & Int(1 << 16) == 0) || themeEmoticon != nil
            let _c15 = (Int(flags) & Int(1 << 17) == 0) || requestsPending != nil
            let _c16 = (Int(flags) & Int(1 << 17) == 0) || recentRequesters != nil
            let _c17 = (Int(flags) & Int(1 << 18) == 0) || availableReactions != nil
            guard _c2 && _c3 && _c4 && _c5 && _c6 && _c7 && _c8 && _c9 && _c10 && _c11 && _c12 && _c13 && _c14 && _c15 && _c16 && _c17 else {
                return nil
            }
            
            return Api.ChatFull.chatFull(
                flags: flags,
                id: id!,
                about: about!,
...
public extension Api {
    indirect enum ChatInvite: TypeConstructorDescription {
        ...
        case chatInviteUser(user: Api.User, expires: Int32)
    ...
    switch self {
                ...
                case .chatInvitePeek(let chat, let expires):
                    ...
                    break
    case .chatInviteUser(user: let user, expires: let expires):
        if boxed {
            buffer.appendInt32(1516793212)
        }
    ...
    public func descriptionFields() -> (String, [(String, Any)]) {
        ...
        case .chatInviteUser(user: let user, expires: let expires):
            return ("chatInviteUser", [("user", String(describing: user)), ("expires", String(describing: expires))])
        }
    }
        ...
        public static func parse_chatInviteAlready(_ reader: BufferReader) -> ChatInvite? {
            guard let _1 = reader.readInt32() else {
                return nil
            }
            ...
            if Int(_1) & Int(1 << 1) != 0 {
                apiUser = Api.parse(reader, signature: signature) as? Api.User
            } else {
                apiChat = Api.parse(reader, signature: signature) as? Api.Chat
            }
            if let apiUser {
                return Api.ChatInvite.chatInviteUser(user: apiUser, expires: 0)
            }
            
            guard let apiChat else {
                return nil
            }
            
            return Api.ChatInvite.chatInviteAlready(chat: apiChat)
        }
        ...
        public static func parse_chatInviteUser(_ reader: BufferReader) -> ChatInvite? {
            guard let signature = reader.readInt32() else {
                return nil
            }
            guard let apiUser = Api.parse(reader, signature: signature) as? Api.User else {
                return nil
            }
            guard let expires = reader.readInt32() else {
                return nil
            }
            
            return Api.ChatInvite.chatInviteUser(user: apiUser, expires: expires)
        }
