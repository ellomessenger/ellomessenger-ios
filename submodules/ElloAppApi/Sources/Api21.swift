...
public extension Api {
    enum User: TypeConstructorDescription {
        case user(flags: Int32, id: Int64, accessHash: Int64?, firstName: String?, lastName: String?, username: String?, phone: String?, photo: Api.UserProfilePhoto?, status: Api.UserStatus?, botInfoVersion: Int32?, email: String?, country: String?, gender: String?, birthday: Int32?, restrictionReason: [Api.RestrictionReason]?, botInlinePlaceholder: String?, langCode: String?, emojiStatus: Api.EmojiStatus?, isBusiness: Bool, isPublic: Bool, botDescription: String?)
        case userEmpty(id: Int64)
    
    public func serialize(_ buffer: Buffer, _ boxed: Swift.Bool) {
    switch self {
    case .user(let flags, let id, let accessHash, let firstName, let lastName, let username, let phone, let photo, let status, let botInfoVersion, let email, let country, let gender, let birthday, let restrictionReason, let botInlinePlaceholder, let langCode, let emojiStatus, let isBusiness, let isPublic, let botDescription):
                    if boxed {
                    ...
                    if Int(flags) & Int(1 << 7) != 0 {isBusiness.serialize(buffer, true)}
                    if Int(flags) & Int(1 << 8) != 0 {isPublic.serialize(buffer, true)}
                    ...
                    if Int(flags) & Int(1 << 23) != 0 {serializeString(email!, buffer: buffer, boxed: false)}
                    ...
                    if Int(flags) & Int(1 << 26) != 0 {serializeInt32(birthday!, buffer: buffer, boxed: false)}
                    if Int(flags) & Int(1 << 30) != 0 {emojiStatus!.serialize(buffer, true)}
                    if Int(flags) & Int(1 << 9) != 0 {serializeString(botDescription!, buffer: buffer, boxed: false)}
                    break
    ...
    public func descriptionFields() -> (String, [(String, Any)]) {
        switch self {
                case .user(let flags, let id, let accessHash, let firstName, let lastName, let username, let phone, let photo, let status, let botInfoVersion, let email, let country, let gender, let birthday, let restrictionReason, let botInlinePlaceholder, let langCode, let emojiStatus, let isBusiness, let isPublic, let botDescription):
                return ("user", [("flags", String(describing: flags)), ("id", String(describing: id)), ("accessHash", String(describing: accessHash)), ("firstName", String(describing: firstName)), ("lastName", String(describing: lastName)), ("username", String(describing: username)), ("phone", String(describing: phone)), ("photo", String(describing: photo)), ("status", String(describing: status)), ("botInfoVersion", String(describing: botInfoVersion)), ("email", String(describing: email)), ("country", String(describing: country)), ("gender", String(describing: gender)), ("birthday", String(describing: birthday)), ("restrictionReason", String(describing: restrictionReason)), ("botInlinePlaceholder", String(describing: botInlinePlaceholder)), ("langCode", String(describing: langCode)), ("emojiStatus", String(describing: emojiStatus)), ("isBusiness", String(describing: isBusiness)), ("isPublic", String(describing: isPublic)), ("botDescription", String(describing: botDescription))])
                case .userEmpty(let id):
        ...
        public static func parse_user(_ reader: BufferReader) -> User? {
            ...
            var isBusiness: Bool = .boolFalse
            if Int(_1!) & Int(1 << 7) != 0 { isBusiness = .boolTrue }
            
            var isPublic: Bool = .boolFalse
            if Int(_1!) & Int(1 << 8) != 0 { isPublic = .boolTrue }
            ...
            /// Not Receiving from backend on not full profile
            var email: String?
            if Int(_1!) & Int(1 << 23) != 0 {
                email = parseString(reader)
            ...
            if Int(_1!) & (1 << 9) != 0 {
                botDescription = parseString(reader) // bot_description
            }
            ...
            if _c1 && _c2 && _c3 && _c4 && _c5 && _c6 && _c7 && _c8 && _c9 && _c10 && _c11 && _c12 && _c13 && _c14 {
                return Api.User.user(flags: _1!, id: _2!, accessHash: _3, firstName: _4, lastName: _5, username: _6, phone: _7, photo: _8, status: _9, botInfoVersion: _10, email: email, country: country, gender: gender, birthday: birthday, restrictionReason: _11, botInlinePlaceholder: _12, langCode: _13, emojiStatus: _14, isBusiness: isBusiness, isPublic: isPublic, botDescription: botDescription)
            }
            else {
                return nil
            }
...
public extension Api {
    enum UserFull: TypeConstructorDescription {
        case userFull(flags: Int32, id: Int64, about: String?, settings: Api.PeerSettings, profilePhoto: Api.Photo?, notifySettings: Api.PeerNotifySettings, botInfo: Api.BotInfo?, email: String?, country: String?, gender: String?, birthday: Int32?, pinnedMsgId: Int32?, commonChatsCount: Int32, folderId: Int32?, ttlPeriod: Int32?, themeEmoticon: String?, privateForwardName: String?, botGroupAdminRights: Api.ChatAdminRights?, botBroadcastAdminRights: Api.ChatAdminRights?, premiumGifts: [Api.PremiumGiftOption]?)
        
        public func serialize(_ buffer: Buffer, _ boxed: Swift.Bool) {
            switch self {
            case .userFull(let flags, let id, let about, let settings, let profilePhoto, let notifySettings, let botInfo, let email, let country, let gender, let birthday, let pinnedMsgId, let commonChatsCount, let folderId, let ttlPeriod, let themeEmoticon, let privateForwardName, let botGroupAdminRights, let botBroadcastAdminRights, let premiumGifts):
                if boxed {
