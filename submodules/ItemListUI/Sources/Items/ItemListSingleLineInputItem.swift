...
public class ItemListSingleLineInputItemNode: ListViewItemNode, UITextFieldDelegate, ItemListItemNode, ItemListItemFocusableNode {
    ...
    override public func didLoad() {
        ...
        if let item = self.item {
            self.textNode.textField.typingAttributes = [NSAttributedString.Key.font: Font.regular(floor(item.presentationData.fontSize.itemListBaseFontSize * 15.0 / 17.0))]
            self.textNode.textField.font = Font.regular(floor(item.presentationData.fontSize.itemListBaseFontSize * 15.0 / 17.0))
            
            self.textNode.textField.textColor = item.textColor
    ...
    public func asyncLayout() -> (_ item: ItemListSingleLineInputItem, _ params: ListViewItemLayoutParams, _ neighbors: ItemListNeighbors) -> (ListViewItemNodeLayout, () -> Void) {
        ...
        return { item, params, neighbors in
            ...
            titleString.addAttributes([NSAttributedString.Key.font: Font.regular(floor(item.presentationData.fontSize.itemListBaseFontSize * 15.0 / 17.0))], range: NSMakeRange(0, titleString.length))
