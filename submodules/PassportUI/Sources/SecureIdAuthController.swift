...
final class SecureIdAuthControllerInteraction {
    ...
    let openMention: (ElloAppPeerMention) -> Void
    let deleteAll: () -> Void
    
    fileprivate init(updateState: @escaping ((SecureIdAuthControllerState) -> SecureIdAuthControllerState) -> Void, present: @escaping (ViewController, Any?) -> Void, push: @escaping (ViewController) -> Void, checkPassword: @escaping (String) -> Void, openPasswordHelp: @escaping () -> Void, setupPassword: @escaping () -> Void, grant: @escaping () -> Void, openUrl: @escaping (String) -> Void, openMention: @escaping (ElloAppPeerMention) -> Void, deleteAll: @escaping () -> Void) {
        ...
        switch self.mode {
            case let .form(peerId, scope, publicKey, callbackUrl, _, _):
                ...
                |> mapToSignal { form, configuration -> Signal<SecureIdEncryptedFormData, RequestSecureIdFormError> in
                    return context.engine.data.get(
                        ...
                        ElloAppEngine.EngineData.Item.Peer.Peer(id: form.peerId)
                    )
            ...
            case .list:
                self.formDisposable = (combineLatest(
                    ...
                    context.engine.data.get(ElloAppEngine.EngineData.Item.Peer.Peer(id: context.account.peerId)) |> castError(GetAllSecureIdValuesError.self) |> mapToSignal { accountPeer -> Signal<EnginePeer, GetAllSecureIdValuesError> in
