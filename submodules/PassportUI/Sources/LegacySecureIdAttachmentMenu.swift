...
func presentLegacySecureIdAttachmentMenu(context: AccountContext, present: @escaping (ViewController) -> Void, validLayout: ContainerViewLayout, type: SecureIdAttachmentMenuType, recognizeDocumentData: Bool, completion: @escaping ([ElloAppMediaResource], SecureIdRecognizedDocumentData?) -> Void) {
    ...
    guard let attachMenu = TGPassportAttachMenu.present(with: legacyController.context, parentController: emptyController, menuController: nil, title: "", intent: mappedIntent, uploadAction: { signal, completed in
        if let signal = signal {
            ...
            |> mapToSignal { resources -> Signal<([ElloAppMediaResource], SecureIdRecognizedDocumentData?), NoError> in
                switch type {
                    case .generic, .idCard:
                        if recognizeDocumentData {
                            return recognizedResources(postbox: context.account.postbox, resources: resources, shouldBeDriversLicense: false)
                                |> map { data -> ([ElloAppMediaResource], SecureIdRecognizedDocumentData?) in
...
private func processedLegacySecureIdAttachmentItems(postbox: Postbox, signal: SSignal) -> Signal<[ElloAppMediaResource], NoError> {
    ...
    |> map { image -> [ElloAppMediaResource] in
    ...
    let collectedItems: Signal<[ElloAppMediaResource], NoError> = collectedSignal
    |> reduceLeft(value: [] as [ElloAppMediaResource], f: { (list: [ElloAppMediaResource], rest: [ElloAppMediaResource]) -> [ElloAppMediaResource] in
        var list = list
...
private func recognizedResources(postbox: Postbox, resources: [ElloAppMediaResource], shouldBeDriversLicense: Bool) -> Signal<SecureIdRecognizedDocumentData?, NoError> {
