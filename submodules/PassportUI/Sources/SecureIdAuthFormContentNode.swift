...
final class SecureIdAuthFormContentNode: ASDisplayNode, SecureIdAuthContentNode, UITextFieldDelegate {
    ...
    init(theme: PresentationTheme, strings: PresentationStrings, nameDisplayOrder: PresentationPersonNameOrder, peer: Peer, privacyPolicyUrl: String?, form: SecureIdForm, primaryLanguageByCountry: [String: String], openField: @escaping (SecureIdParsedRequestedFormField) -> Void, openURL: @escaping (String) -> Void, openMention: @escaping (ElloAppPeerMention) -> Void, requestLayout: @escaping () -> Void) {
        ...
        if let privacyPolicyUrl = privacyPolicyUrl {
            ...
            let privacyPolicyLinkAttributes = MarkdownAttributeSet(font: infoFont, textColor: theme.list.itemAccentColor, additionalAttributes: [NSAttributedString.Key.underlineStyle.rawValue: NSUnderlineStyle.single.rawValue as NSNumber, ElloAppTextAttributes.URL: privacyPolicyUrl])
        ...
        self.textNode.highlightAttributeAction = { attributes in
            ...
            } else if let _ = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.PeerMention)] {
                return NSAttributedString.Key(rawValue: ElloAppTextAttributes.PeerMention)
            } else {
                return nil
            }
        }
        self.textNode.tapAttributeAction = { attributes, _ in
            if let url = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.URL)] as? String {
                openURL(url)
            } else if let mention = attributes[NSAttributedString.Key(rawValue: ElloAppTextAttributes.PeerMention)] as? ElloAppPeerMention {
                openMention(mention)
            }
        }
