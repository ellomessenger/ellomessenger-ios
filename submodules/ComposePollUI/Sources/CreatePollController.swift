...
public final class ComposedPoll {
    ...
    public init(
        publicity: ElloAppMediaPollPublicity,
        kind: ElloAppMediaPollKind,
        text: String,
        options: [ElloAppMediaPollOption],
        correctAnswers: [Data]?,
        results: ElloAppMediaPollResults,
...
public func createPollController(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)? = nil, peer: EnginePeer, isQuiz: Bool? = nil, completion: @escaping (ComposedPoll) -> Void) -> AttachmentContainable {
    ...
    let signal = combineLatest(queue: .mainQueue(),
        presentationData,
        statePromise.get(),
        context.engine.data.subscribe(ElloAppEngine.EngineData.Item.Configuration.Limits())
    )
    |> map { presentationData, state, limitsConfiguration -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        rightNavigationButton = ItemListNavigationButton(content: .text(presentationData.strings.CreatePoll_Create), style: .bold, enabled: enabled, action: {
            ...
            var options: [ElloAppMediaPollOption] = []
            ...
            for i in 0 ..< state.options.count {
                ...
                if !optionText.isEmpty {
                    ...
                    options.append(ElloAppMediaPollOption(text: optionText, opaqueIdentifier: optionData))
            ...
            var resolvedSolution: ElloAppMediaPollResults.Solution?
            ...
            if state.isQuiz {
                ...
                if !state.solutionText.value.string.isEmpty {
                    ...
                    resolvedSolution = ElloAppMediaPollResults.Solution(text: state.solutionText.value.string, entities: entities)
                }
            } else {
            ...
            completion(ComposedPoll(
                ...
                results: ElloAppMediaPollResults(voters: nil, totalVoters: nil, recentVoters: [], solution: resolvedSolution),
