...
class LegacyWebSearchItem: NSObject, TGMediaEditableItem, TGMediaSelectableItem {
    ...
    private(set) var thumbnailResource: ElloAppMediaResource?
    private(set) var imageResource: ElloAppMediaResource?
    ...
    init(result: ChatContextResult, thumbnailResource: ElloAppMediaResource?, imageResource: ElloAppMediaResource?, dimensions: CGSize, thumbnailImage: Signal<UIImage, NoError>, originalImage: Signal<UIImage, NoError>, progress: Signal<Float, NoError>) {
        self.result = result
...
func legacyWebSearchItem(account: Account, result: ChatContextResult) -> LegacyWebSearchItem? {
    ...
    if let imageResource = imageResource {
        ...
        let tmpImage = ElloAppMediaImage(imageId: EngineMedia.Id(namespace: 0, id: 0), representations: representations, immediateThumbnailData: immediateThumbnailData, reference: nil, partialReference: nil, flags: [])
