...
private let productIdentifiers = [
    "org.elloapp.elloappPremium.annual",
    "org.elloapp.elloappPremium.semiannual",
    "org.elloapp.elloappPremium.monthly",
    "org.elloapp.elloappPremium.twelveMonths",
    "org.elloapp.elloappPremium.sixMonths",
    "org.elloapp.elloappPremium.threeMonths"
]
...
public final class InAppPurchaseManager: NSObject {
    public final class Product: Equatable {
        ...
        public var isSubscription: Bool {
            self.skProduct.subscriptionGroupIdentifier != nil
        }
    ...
    public init(engine: ElloAppEngine) {
        self.engine = engine
...
private func pendingInAppPurchaseState(engine: ElloAppEngine, productId: String) -> Signal<PendingInAppPurchaseState?, NoError> {
    let key = ValueBoxKey(length: 8)
    key.setInt64(0, value: Int64(bitPattern: productId.persistentHashValue))
    
    return engine.data.get(ElloAppEngine.EngineData.Item.ItemCache.Item(collectionId: ApplicationSpecificItemCacheCollectionId.pendingInAppPurchaseState, id: key))
    |> map { entry -> PendingInAppPurchaseState? in
        return entry?.get(PendingInAppPurchaseState.self)
    }
}

private func updatePendingInAppPurchaseState(engine: ElloAppEngine, productId: String, content: PendingInAppPurchaseState?) -> Signal<Never, NoError> {
    let key = ValueBoxKey(length: 8)
    key.setInt64(0, value: Int64(bitPattern: productId.persistentHashValue))
