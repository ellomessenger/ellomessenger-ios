...
if [ -z "$CONFIGURATION" ] || [ -z "$MODE" ] ; then
	echo "Usage: sh deploy-elloapp.sh CONFIGURATION [cached|full]"
	exit 1
fi
...
VERIFY_PATH="ElloAppVerifyBuild.ipa"

rm -f "$VERIFY_PATH"
cp "$IPA_PATH" "$VERIFY_PATH"

BAZEL_HTTP_CACHE_URL="$BAZEL_HTTP_CACHE_URL" sh buildbox/build-elloapp.sh verify
