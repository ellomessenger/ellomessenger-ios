...
if [ -z "$CONFIGURATION" ]; then
	echo "Usage: sh deploy-elloapp.sh CONFIGURATION"
	exit 1
fi

if [ ! `which setup-elloapp-build.sh` ]; then
	echo "setup-elloapp-build.sh not found in PATH $PATH"
	exit 1
fi
...
source `which setup-elloapp-build.sh`
setup_elloapp_build "$CONFIGURATION" "$BASE_DIR/$BUILDBOX_DIR/transient-data"
...
if [ "$CONFIGURATION" == "hockeyapp" ] || [ "$CONFIGURATION" == "appcenter-experimental" ] || [ "$CONFIGURATION" == "appcenter-experimental-2" ] || [ "$CONFIGURATION" == "appstore-development" ]; then
	...
	FASTLANE_PASSWORD="$ELLOAPP_BUILD_APPSTORE_PASSWORD"
	FASTLANE_ITC_TEAM_NAME="$ELLOAPP_BUILD_APPSTORE_TEAM_NAME"
	FASTLANE_ITC_USERNAME="$ELLOAPP_BUILD_APPSTORE_USERNAME"
else
...
IPA_PATH="$OUTPUT_PATH/ElloApp.ipa"
DSYM_PATH="$OUTPUT_PATH/ElloApp.DSYMs.zip"
