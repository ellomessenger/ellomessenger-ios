...
python3 build-system/Make/Make.py \
    ...
    build \
    --configurationPath="$HOME/elloapp-configuration" \
...
for f in bazel-out/applebin_ios-ios_arm*-opt-ST-*/bin/ElloApp/ElloApp.ipa; do
	cp "$f" $OUTPUT_PATH/
done

mkdir -p build/DSYMs
for f in bazel-out/applebin_ios-ios_arm*-opt-ST-*/bin/ElloApp/*.dSYM; do
	cp -R "$f" build/DSYMs/
done

zip -r "./$OUTPUT_PATH/ElloApp.DSYMs.zip" build/DSYMs 1>/dev/null
